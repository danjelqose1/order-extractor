const LOCAL_API_BASE = "http://127.0.0.1:5055";
const RENDER_API_BASE = "https://order-extractor-kdih.onrender.com";
const API_BASE_OVERRIDE = localStorage.getItem("loe.apiBase");
const IS_LOCAL_FRONTEND = window.location.protocol === "file:"
  || ["127.0.0.1", "localhost", "::1"].includes(window.location.hostname);
const API_BASE = API_BASE_OVERRIDE || (IS_LOCAL_FRONTEND ? LOCAL_API_BASE : RENDER_API_BASE);
console.log("API_BASE:", API_BASE);
try{
  localStorage.removeItem("loe.openaiKey");
  localStorage.removeItem("openai.apiKey");
  if (Object.prototype.hasOwnProperty.call(window, "OPENAI_API_KEY")){
    window.OPENAI_API_KEY = undefined;
    delete window.OPENAI_API_KEY;
  }
}catch{
  // Storage can be unavailable in private or restricted browser contexts.
}
const HISTORY_CACHE_KEY_BASE = "loe.history.cache.v1";
const HISTORY_CACHE_LIMIT = 200;
const DEFAULT_APPEARANCE_SETTINGS = Object.freeze({
  headerFontSize: 20,
  orderInfoFontSize: 14,
  bodyFontSize: 12.5,
  fontFamily: "system-sans",
  rowDensity: "normal",
  accentStyle: "none",
});
const MOTHER_SHEET_SELECTOR = "#motherSheet, .mother-sheet";
const ANALYSIS_MAX_DATASET_CHARS = 50000;
const ANALYSIS_SNAPSHOT_TTL_MS = 7 * 60 * 1000;
const ANALYSIS_SNAPSHOT_STORAGE_KEY = "loe.analysis.snapshots.v2";
const ANALYSIS_SNAPSHOT_VERSION = 2;
const ANALYSIS_MAX_SNAPSHOTS = 40;
const INVOICE_PRICES_KEY = "loe.invoices.prices.v1";
const INVOICE_JOBS_KEY = "loe.invoices.jobs.v1";
const INVOICE_COUNTER_KEY = "loe.invoices.counter.v1";
const TYPE_CORRECTIONS_STORAGE_KEY = "invoice_type_corrections_v1";
const IGU_INNER_FILL_MM = 5;
const SCAN_ALLOWED_EXTENSIONS = new Set(["jpg", "jpeg", "png", "webp"]);
const SCAN_PREVIEW_MAX_SIDE = 1600;
const SCAN_EXPORT_MAX_SIDE = 2800;
const IGU_SETTINGS_DEFAULTS = Object.freeze({
  spacerSource: "item",
  spacerOverride: null,
  density: 1.18,
  boxWeightKg: 6.5,
  wasteSpacerEnabled: false,
  wasteSpacerPct: 3,
  wasteButylEnabled: false,
  wasteButylPct: 5,
});

const DEFAULT_TYPE_CORRECTIONS = [
  {
    raw: "3 vetri 33.1F + 4F + 4 termico 44mm",
    corrected: "3 vetri 33.1F+14+4F+14+4 termico 44mm",
  },
  {
    raw: "3 vetri 33.1F + 4F + 33.1 LowE 44mm",
    corrected: "3 vetri 33.1F+12+4F+14+33.1 LowE 44mm",
  },
  {
    raw: "3 vetri 33.1F cald 4F cald 33.1 LowE 44mm",
    corrected: "3 vetri 33.1F+12+4F+14+33.1 LowE 44mm",
  },
  {
    raw: "3 vetri 4F + 4F + 4 LowE 44mm",
    corrected: "3 vetri 4F+14+4F+16+4 LowE 44mm",
  },
  {
    raw: "3 vetri 33.1 LOWE + c.caldo + 4F + c.caldo + 33.1STAINATO 44mm",
    corrected: "3 vetri 33.1LOWE+12+4F+14+33.1STAINATO 44mm",
  },
  {
    raw: "3 vetri 33.1 LOWE + c.caldo + 4F + c.caldo + 33.1Satinato 44mm",
    corrected: "3 vetri 33.1LOWE+12+4F+14+33.1Satinato 44mm",
  },
  {
    raw: "3 vetri 33.1LOWE + 4F + 4 SATINATO 44mm",
    corrected: "3 vetri 33.1LOWE+14+4F+14+4 SATINATO 44mm",
  },
];

let TYPE_CORRECTIONS = DEFAULT_TYPE_CORRECTIONS.map(entry => ({ ...entry }));

// ✅ Danko Rounding: Global functions for Processing + Mother Sheet
window.dankoRound = function(valueMm) {
  if (!Number.isFinite(valueMm)) return valueMm;
  const lastDigit = valueMm % 10;
  if (lastDigit === 1 || lastDigit === 2) return valueMm - lastDigit;        // → 0
  if (lastDigit === 6 || lastDigit === 7) return valueMm - (lastDigit - 5);  // → 5
  if (lastDigit === 4) return valueMm + (5 - lastDigit);                     // → 5
  if (lastDigit === 9) return valueMm + (10 - lastDigit);                    // → next 0
  return valueMm; // keep unchanged
};

window.applyDankoRuleToDim = function(dimStr) {
  if (!dimStr || !/^\d+x\d+$/.test(dimStr)) {
    return { w: null, h: null, text: dimStr, changed: false };
  }
  const [w0, h0] = dimStr.split("x").map(n => parseInt(n, 10));
  const w = window.dankoRound(w0);
  const h = window.dankoRound(h0);
  const changed = (w !== w0) || (h !== h0);
  return { w, h, text: `${w}x${h}`, changed, original: `${w0}x${h0}` };
};

const renderTargets = {
  extract: { tableId: "tableWrap", summaryId: "summary", orderInfoId: "orderInfo" },
  history: { tableId: "historyTableWrap", summaryId: "historySummary", orderInfoId: "historyOrderInfo" }
};

const renderState = {
  extract: { rows: [], displayed: [] },
  history: { rows: [], displayed: [] }
};

let groupState = false;
let appearanceSettings = { ...DEFAULT_APPEARANCE_SETTINGS };
let motherSheetObserver = null;

setGlobalAppearanceVariables(appearanceSettings);
ensureMotherSheetObserver();

const historyState = {
  query: "",
  client: "",
  status: "",
  year: "all",
  dateFrom: "",
  dateTo: "",
  approvedOnly: false,
  limit: 25,
  offset: 0,
  hasMore: false,
  items: [],
  selectedOrder: null,
  offline: false,
  loading: false,
  error: "",
  loadedOnce: false,
  loadingPromise: Promise.resolve(),
  needsRefresh: false,
  queue: "",
  queueItems: {},
  queueCounts: {},
  queueLoading: false,
  selectedIds: new Set(),
  selectedOrders: new Map(),
};

const manualOrdersState = {
  items: [],
  rows: [],
  editingId: null,
  viewOnly: false,
  loaded: false,
  loading: false,
  search: "",
  status: "",
  saveStatus: "draft",
  duplicateCheckToken: 0,
  nextRowId: 1,
};

const appState = {
  extract: {
    rows: [],
    originalRows: [],
    rowWarnings: {},
    rowDiagnoses: {},
    warnings: [],
    draftId: null,
    status: "idle",
    savedOrderId: null,
    source_hash: "",
    extractionMethod: "",
    orderNumber: "",
    appliedCorrections: [],
    notes: "",
    declaredUnits: null,
    declaredArea: null,
    parsedUnits: 0,
    parsedArea: 0,
    orderTotalDiagnostics: null,
    confidence: null,
    extractedAt: null,
    nextRid: 0,
    client_name: "",
    client: "—",
  },
  historyDetail: {
    order: null,
    rows: [],
    originalRows: [],
    rowWarnings: {},
    rowDiagnoses: {},
    warnings: [],
    notes: "",
    declaredUnits: null,
    declaredArea: null,
    parsedUnits: 0,
    parsedArea: 0,
    orderTotalDiagnostics: null,
    nextRid: 0,
  },
  processing: {
    cart: [],
    rows: [],
    preview: { text: "", groups: [], meta: {} },
    grouped: false,
    options: {
      restartPerGroup: false,
      decimalSeparator: "comma",
      normalizeLPtoG: true,
      mergeAcrossOrders: false,
      autoDanko: false,
    },
    headerOverrides: {},
    rounding: {
      manualApplied: false,
      message: "",
    },
  },
  labels: {
    jobs: [],
    nextId: 1,
  },
  invoices: {
    jobs: [],
    activeJobId: null,
    priceLists: {
      glass: getDefaultGlassPrices(),
      spacer: getDefaultSpacerPrices(),
    },
    pagination: {
      currentPage: 1,
      pageSize: 5,
      totalPages: 1,
    },
    filters: { client: "", order: "", status: "" },
    counter: 1,
  },
};

let lastExtractPdfFile = null;
let activeReviewIssueIndex = -1;
let currentExtractReviewIssues = [];
const sourcePdfState = {
  extract: { document: null, page: 1, pages: 0, renderToken: 0 },
  order: { document: null, page: 1, pages: 0, renderToken: 0 },
};
const activityState = {
  items: [],
  filter: "all",
  nextId: 1,
  retryHandlers: new Map(),
};

const processingState = {
  processedRows: [],
  previewGroups: [],
};

const spacerProcessingState = {
  sections: [],
  text: "No processed order loaded",
  hasPreviewGroups: false,
  mergeAcrossOrders: true,
  pdfPreview: false,
  liveHtmlPreview: false,
  layoutModel: null,
};

const workspaceState = {
  sessionId: (window.crypto && crypto.randomUUID) ? crypto.randomUUID() : `workspace-${Date.now()}-${Math.random().toString(16).slice(2)}`,
  queue: null,
  recentFiles: [],
  moduleFiles: [],
  processingJobs: [],
  loading: false,
  assistantLoading: false,
  smartChatLoading: false,
  selectedOrderId: "",
  selectedOrderNumber: "",
  selectedOrderIds: new Set(),
  selectedOrderNumbers: new Map(),
  lastAgentAction: null,
  latestPendingAction: null,
};

const awaState = {
  summary: null,
  timeline: [],
  suggestions: [],
  loading: false,
  chatLoading: false,
};

const telegramFilesState = {
  items: [],
  loading: false,
  loadedOnce: false,
  query: "",
  status: "",
  touched: "false",
  untouchedCount: 0,
  queuedCount: 0,
  processingCount: 0,
  failedCount: 0,
  events: null,
  eventReconnectPending: false,
  selectedFile: null,
};

/**
 * @typedef {Object} ScanPage
 * @property {string} id
 * @property {string} originalSrc
 * @property {number} rotation
 * @property {{ x: number; y: number; width: number; height: number } | null} crop
 * @property {"original" | "grayscale" | "document" | "bw"} filter
 * @property {number} brightness
 * @property {number} contrast
 * @property {number} orderIndex
 */
const scanStudioState = {
  pages: [],
  selectedPageId: "",
  draggingPageId: "",
  cropMode: false,
  perspectiveMode: false,
  cropStart: null,
  cropDraft: null,
  draggingPerspectiveIndex: -1,
  renderToken: 0,
  preview: {
    imageCanvas: null,
    baseCanvas: null,
    scale: 1,
    offsetX: 0,
    offsetY: 0,
    width: 0,
    height: 0,
  },
	};
	const scanStudioImageCache = new Map();
	const globalScanDropState = {
	  dragCounter: 0,
	  loading: false,
	  pendingFiles: [],
	  messageTimer: null,
	  cleanupHandlers: [],
	};

	const analysisState = {
  initialized: false,
  loading: false,
  dataset: null,
  orders: [],
  metrics: null,
  topClients: [],
  topTypes: [],
  topDimensions: [],
  currentPeriod: null,
  previousPeriod: null,
  insights: [],
  advancedDataset: null,
  tolerance: 1,
  orientationAgnostic: true,
  distributionMetric: "area",
  lastSnapshot: null,
  controls: {
    startDate: "",
    endDate: "",
    comparePrevious: true,
    client: "",
    glassType: "",
    allTime: false,
  },
  sort: {
    clients: { key: "area", direction: "desc" },
    types: { key: "area", direction: "desc" },
    dimensions: { key: "qty", direction: "desc" },
  },
  promise: Promise.resolve(),
  sessionId: null,
  allOrdersCache: [],
  allOrdersPromise: null,
  allOrdersDirty: true,
  revalidating: false,
  hasRenderedSnapshot: false,
  currentSnapshotKey: "",
  currentSnapshotComputedAt: "",
  renderRunId: 0,
  ai: {
    loading: false,
    lastQuestion: "",
  },
  orderRowCache: new Map(),
  orderRowRequests: new Map(),
};

const tabs = document.querySelectorAll(".tab-bar .tab");
const panels = {
  extract: document.getElementById("tabExtract"),
  workspace: document.getElementById("tabWorkspace"),
  awa: document.getElementById("tabAwa"),
  telegram: document.getElementById("tabTelegram"),
  scanstudio: document.getElementById("tabScanStudio"),
  pdfeditor: document.getElementById("tabPdfEditor"),
  history: document.getElementById("tabHistory"),
  manual: document.getElementById("tabManualOrders"),
  orderdetail: document.getElementById("tabOrderDetail"),
  processing: document.getElementById("tabProcessing"),
  spacer: document.getElementById("tabSpacer"),
  labels: document.getElementById("tabLabels"),
  analysis: document.getElementById("tabAnalysis"),
  invoices: document.getElementById("tabInvoices"),
  settings: document.getElementById("tabSettings"),
};

const PAGE_META = Object.freeze({
  extract: {
    eyebrow: "Operations",
    title: "Overview",
    subtitle: "Monitor incoming orders, review work, and production readiness.",
  },
  newOrder: {
    eyebrow: "Orders",
    title: "New Order",
    subtitle: "Upload a PDF or paste order text, then review the extracted result.",
  },
  workspace: {
    eyebrow: "Workflow",
    title: "Production",
    subtitle: "Manage approved orders, production files, and factory workflows.",
  },
  awa: {
    eyebrow: "Automation",
    title: "AWA Beta",
    subtitle: "Review supervised automation suggestions before anything runs.",
  },
  telegram: {
    eyebrow: "Files",
    title: "Documents",
    subtitle: "Review incoming PDFs and open document preparation tools.",
  },
  history: {
    eyebrow: "Orders",
    title: "Orders",
    subtitle: "Find, review, export, and continue previously extracted orders.",
  },
  manual: {
    eyebrow: "Orders",
    title: "Manual Orders",
    subtitle: "Create and manage orders entered without a source PDF.",
  },
  orderdetail: {
    eyebrow: "Orders",
    title: "Order Detail",
    subtitle: "Review the order lifecycle, extracted items, source document, and production files.",
  },
  processing: {
    eyebrow: "Factory",
    title: "Production Processing",
    subtitle: "Prepare approved orders for the production floor.",
  },
  spacer: {
    eyebrow: "Production",
    title: "Spacer Processing",
    subtitle: "Prepare spacer output from approved order data.",
  },
  labels: {
    eyebrow: "Output",
    title: "Label Studio",
    subtitle: "Generate and print production labels from approved jobs.",
  },
  analysis: {
    eyebrow: "Insights",
    title: "Analytics",
    subtitle: "Track order volume, materials, clients, and production patterns.",
  },
  pdfeditor: {
    eyebrow: "Tools",
    title: "PDF Editor",
    subtitle: "Review and adjust order PDFs before extraction.",
  },
  scanstudio: {
    eyebrow: "Tools",
    title: "Scan Studio",
    subtitle: "Clean, rotate, crop, and export scanned order pages.",
  },
  invoices: {
    eyebrow: "Finance",
    title: "Invoices",
    subtitle: "Build invoices from approved orders and configured prices.",
  },
  settings: {
    eyebrow: "System",
    title: "Settings",
    subtitle: "Review interface and backend connection preferences.",
  },
});

const appSidebar = document.querySelector(".app-sidebar");
const mobileNavToggle = document.getElementById("mobileNavToggle");
const mobilePageTitle = document.getElementById("mobilePageTitle");
const pageEyebrow = document.getElementById("pageEyebrow");
const pageTitle = document.getElementById("pageTitle");
const pageSubtitle = document.getElementById("pageSubtitle");
const environmentBadge = document.getElementById("environmentBadge");
const environmentLabel = document.getElementById("environmentLabel");
const environmentDetail = document.getElementById("environmentDetail");
const sidebarEnvironmentLabel = document.getElementById("sidebarEnvironmentLabel");
const mobileEnvironmentLabel = document.getElementById("mobileEnvironmentLabel");
const overviewDashboard = document.getElementById("overviewDashboard");
const newOrderWorkspace = document.getElementById("newOrderWorkspace");
const overviewHeadline = document.getElementById("overviewHeadline");
const overviewStatus = document.getElementById("overviewStatus");
const overviewAttentionList = document.getElementById("overviewAttentionList");
const overviewRecentOrders = document.getElementById("overviewRecentOrders");
const overviewNeedsReview = document.getElementById("overviewNeedsReview");
const overviewReadyProduction = document.getElementById("overviewReadyProduction");
const overviewInProduction = document.getElementById("overviewInProduction");
const overviewCompleted = document.getElementById("overviewCompleted");
const settingsThemeMode = document.getElementById("settingsThemeMode");
const settingsEnvironmentMode = document.getElementById("settingsEnvironmentMode");
const settingsApiEndpoint = document.getElementById("settingsApiEndpoint");
const activityCenterToggle = document.getElementById("activityCenterToggle");
const activityCenterBadge = document.getElementById("activityCenterBadge");
const activityCenter = document.getElementById("activityCenter");
const activityCenterClose = document.getElementById("activityCenterClose");
const activityCenterSummary = document.getElementById("activityCenterSummary");
const activityCenterList = document.getElementById("activityCenterList");
const activityClearCompleted = document.getElementById("activityClearCompleted");

const iguWorkspaceModule = document.getElementById("iguWorkspaceModule");
const workspaceMaterialsSlot = document.getElementById("workspaceMaterialsSlot");
if (iguWorkspaceModule && workspaceMaterialsSlot){
  workspaceMaterialsSlot.appendChild(iguWorkspaceModule);
}

function getEnvironmentMeta(){
  try{
    const url = new URL(API_BASE, window.location.href);
    const local = ["localhost", "127.0.0.1", "::1"].includes(url.hostname);
    return {
      label: local ? "Local backend" : "Cloud backend",
      detail: local ? `${url.hostname}${url.port ? `:${url.port}` : ""}` : url.hostname,
      local,
    };
  }catch{
    return { label: "Custom backend", detail: "Configured API", local: false };
  }
}

function setMobileNavOpen(open){
  if (!appSidebar || !mobileNavToggle) return;
  appSidebar.classList.toggle("mobile-nav-open", open);
  mobileNavToggle.setAttribute("aria-expanded", String(open));
  mobileNavToggle.setAttribute("aria-label", open ? "Close navigation" : "Open navigation");
}

function syncSettingsSummary(){
  const environment = getEnvironmentMeta();
  if (settingsThemeMode){
    settingsThemeMode.textContent = window.matchMedia("(prefers-color-scheme: dark)").matches ? "System dark" : "System light";
  }
  if (settingsEnvironmentMode) settingsEnvironmentMode.textContent = environment.label;
  if (settingsApiEndpoint) settingsApiEndpoint.textContent = environment.detail;
}

function setActivityCenterOpen(open){
  if (!activityCenter || !activityCenterToggle) return;
  activityCenter.hidden = !open;
  activityCenterToggle.setAttribute("aria-expanded", String(open));
}

function activityTimeLabel(value){
  const timestamp = new Date(value || 0).getTime();
  if (!timestamp) return "Just now";
  const seconds = Math.max(0, Math.round((Date.now() - timestamp) / 1000));
  if (seconds < 60) return "Just now";
  if (seconds < 3600) return `${Math.floor(seconds / 60)}m ago`;
  return `${Math.floor(seconds / 3600)}h ago`;
}

function renderActivityCenter(){
  const running = activityState.items.filter(item => item.status === "running").length;
  const failed = activityState.items.filter(item => item.status === "failed").length;
  const attention = running + failed;
  if (activityCenterBadge){
    activityCenterBadge.hidden = attention === 0;
    activityCenterBadge.textContent = String(attention);
  }
  if (activityCenterSummary){
    activityCenterSummary.textContent = running
      ? `${running} running · ${failed} need${failed === 1 ? "s" : ""} attention`
      : (failed ? `${failed} task${failed === 1 ? "" : "s"} need attention` : "No active tasks.");
  }
  document.querySelectorAll("[data-activity-filter]").forEach(button => {
    button.classList.toggle("active", button.dataset.activityFilter === activityState.filter);
  });
  if (!activityCenterList) return;
  const visible = activityState.items.filter(item => {
    if (activityState.filter === "running") return item.status === "running";
    if (activityState.filter === "failed") return item.status === "failed";
    return true;
  });
  if (!visible.length){
    activityCenterList.innerHTML = '<div class="activity-empty">No tasks in this view.</div>';
    return;
  }
  activityCenterList.innerHTML = visible.map(item => {
    const progress = Math.max(0, Math.min(100, Number(item.progress || 0)));
    const statusLabel = item.status === "running"
      ? `${progress}%`
      : (item.status === "failed" ? "Needs attention" : "Completed");
    const downloads = (item.downloads || []).map(download => {
      const url = workspaceFileUrl(download.url);
      return url ? `<a class="btn small" href="${escapeHtml(url)}" target="_blank" rel="noopener">${escapeHtml(download.label || "Download")}</a>` : "";
    }).join("");
    return `<article class="activity-item ${escapeHtml(item.status)}" data-activity-id="${escapeHtml(item.id)}">
      <div class="activity-item-status" aria-hidden="true">${item.status === "completed" ? "✓" : (item.status === "failed" ? "!" : "↻")}</div>
      <div class="activity-item-content">
        <div class="activity-item-heading">
          <strong>${escapeHtml(item.title)}</strong>
          <span>${escapeHtml(statusLabel)}</span>
        </div>
        <p>${escapeHtml(item.error || item.detail || "")}</p>
        ${item.status === "running" ? `<div class="activity-progress"><span style="width:${progress}%"></span></div>` : ""}
        <div class="activity-item-footer">
          <small>${escapeHtml(activityTimeLabel(item.updatedAt || item.createdAt))}</small>
          <div class="activity-item-actions">
            ${downloads}
            ${item.openTab ? `<button type="button" class="btn small tertiary" data-activity-open="${escapeHtml(item.openTab)}">Open</button>` : ""}
            ${item.status === "failed" && activityState.retryHandlers.has(item.id) ? '<button type="button" class="btn small" data-activity-retry>Retry</button>' : ""}
            ${item.status !== "running" ? '<button type="button" class="btn small tertiary" data-activity-dismiss aria-label="Dismiss activity">Dismiss</button>' : ""}
          </div>
        </div>
      </div>
    </article>`;
  }).join("");
}

function startBackgroundActivity(title, options = {}){
  const id = options.id || `activity-${Date.now()}-${activityState.nextId++}`;
  const existingIndex = activityState.items.findIndex(item => item.id === id);
  const item = {
    id,
    type: options.type || "task",
    title,
    detail: options.detail || "Starting…",
    status: "running",
    progress: Number(options.progress ?? 5),
    createdAt: options.createdAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    downloads: [],
    openTab: options.openTab || "",
    error: "",
  };
  if (existingIndex >= 0) activityState.items.splice(existingIndex, 1);
  activityState.items.unshift(item);
  activityState.items = activityState.items.slice(0, 50);
  if (typeof options.retry === "function") activityState.retryHandlers.set(id, options.retry);
  renderActivityCenter();
  return id;
}

function updateBackgroundActivity(id, patch = {}){
  const item = activityState.items.find(entry => entry.id === id);
  if (!item) return;
  Object.assign(item, patch, { updatedAt: new Date().toISOString() });
  renderActivityCenter();
}

function completeBackgroundActivity(id, patch = {}){
  updateBackgroundActivity(id, { ...patch, status: "completed", progress: 100, error: "" });
}

function failBackgroundActivity(id, error, patch = {}){
  updateBackgroundActivity(id, {
    ...patch,
    status: "failed",
    progress: 100,
    error: error?.message || String(error || "Task failed."),
  });
  setActivityCenterOpen(true);
}

activityCenterToggle?.addEventListener("click", ()=> {
  setActivityCenterOpen(activityCenter.hidden);
});
activityCenterClose?.addEventListener("click", ()=> setActivityCenterOpen(false));
activityClearCompleted?.addEventListener("click", ()=> {
  activityState.items = activityState.items.filter(item => item.status !== "completed");
  renderActivityCenter();
});
document.querySelectorAll("[data-activity-filter]").forEach(button => {
  button.addEventListener("click", ()=> {
    activityState.filter = button.dataset.activityFilter || "all";
    renderActivityCenter();
  });
});
activityCenterList?.addEventListener("click", async event => {
  const itemElement = event.target.closest("[data-activity-id]");
  const id = itemElement?.dataset.activityId;
  if (!id) return;
  if (event.target.closest("[data-activity-dismiss]")){
    activityState.items = activityState.items.filter(item => item.id !== id);
    activityState.retryHandlers.delete(id);
    renderActivityCenter();
    return;
  }
  const openButton = event.target.closest("[data-activity-open]");
  if (openButton){
    activateTab(openButton.dataset.activityOpen);
    setActivityCenterOpen(false);
    return;
  }
  if (event.target.closest("[data-activity-retry]")){
    const retry = activityState.retryHandlers.get(id);
    if (!retry) return;
    updateBackgroundActivity(id, { status: "running", progress: 5, detail: "Retrying…", error: "" });
    try{
      await retry(id);
    }catch(error){
      failBackgroundActivity(id, error);
    }
  }
});

function updateAppChrome(name){
  const meta = PAGE_META[name] || PAGE_META.extract;
  if (pageEyebrow) pageEyebrow.textContent = meta.eyebrow;
  if (pageTitle) pageTitle.textContent = meta.title;
  if (pageSubtitle) pageSubtitle.textContent = meta.subtitle;
  if (mobilePageTitle) mobilePageTitle.textContent = meta.title;
  document.title = `${meta.title} — Order Extractor`;

  const environment = getEnvironmentMeta();
  if (environmentLabel) environmentLabel.textContent = environment.label;
  if (environmentDetail) environmentDetail.textContent = environment.detail;
  if (sidebarEnvironmentLabel) sidebarEnvironmentLabel.textContent = environment.label;
  if (mobileEnvironmentLabel) mobileEnvironmentLabel.textContent = environment.label;
  if (environmentBadge){
    environmentBadge.classList.toggle("is-local", environment.local);
  }
  if (appSidebar){
    appSidebar.classList.toggle("is-local-environment", environment.local);
  }
  syncSettingsSummary();
}

function setNewOrderWorkspaceOpen(open, options = {}){
  if (!overviewDashboard || !newOrderWorkspace) return;
  overviewDashboard.hidden = !!open;
  newOrderWorkspace.hidden = !open;
  if (open){
    updateAppChrome("newOrder");
    window.scrollTo({ top: 0, behavior: options.instant ? "auto" : "smooth" });
    if (options.focus !== false){
      window.setTimeout(()=> document.getElementById("pdfDropZone")?.focus?.(), 0);
    }
  }else{
    updateAppChrome("extract");
    if (options.load !== false) loadOverview();
  }
}

function overviewOrderId(order){
  return String(order?.order_id ?? order?.id ?? "");
}

function overviewOrderNumber(order){
  const values = Array.isArray(order?.order_numbers) ? order.order_numbers.filter(Boolean) : [];
  return String(order?.order_number || values.join(", ") || "—");
}

function overviewOrderRowHtml(order, options = {}){
  const id = overviewOrderId(order);
  const orderNumber = overviewOrderNumber(order);
  const client = getClientName(order, order?.client_name || "—");
  const dateValue = order?.approved_at || order?.updated_at || order?.created_at;
  const warnings = Number(order?.warnings_count || 0);
  const units = Number(order?.total_pieces ?? order?.units_total ?? 0);
  const meta = [
    units ? `${units} pcs` : "",
    warnings ? `${warnings} warning${warnings === 1 ? "" : "s"}` : "",
    dateValue ? formatDate(dateValue) : "",
  ].filter(Boolean).join(" · ");
  return `<button type="button" class="overview-order-row" data-overview-order-id="${escapeHtml(id)}" ${id ? "" : "disabled"}>
    <span class="overview-order-main">
      <span class="overview-order-number mono">${escapeHtml(orderNumber)}</span>
      <span class="overview-order-client">${escapeHtml(client)}</span>
      <span class="overview-order-meta">${escapeHtml(meta || options.emptyMeta || "Order ready")}</span>
    </span>
    <span class="overview-order-status">${historyStatusBadgeHtml(order?.status)}</span>
    <span class="overview-order-arrow" aria-hidden="true">→</span>
  </button>`;
}

let overviewLoading = false;

async function loadOverview(){
  if (!overviewDashboard || overviewLoading) return;
  overviewLoading = true;
  if (overviewStatus) overviewStatus.textContent = "Refreshing operational overview…";
  const [queueResult, recentResult] = await Promise.allSettled([
    fetchWorkspaceQueue(),
    fetchOrders({ year: "all", limit: 6, offset: 0 }),
  ]);

  const queue = queueResult.status === "fulfilled" ? queueResult.value : null;
  const recentData = recentResult.status === "fulfilled" ? recentResult.value : null;
  const cached = readHistoryCache();
  const groups = queue?.groups || {};
  const counts = queue?.counts || {};
  const needsReviewItems = Array.isArray(groups.needs_review)
    ? groups.needs_review
    : cached.filter(item => ["draft", "reviewed"].includes(normalizeHistoryStatusValue(item.status)));
  const recentItems = Array.isArray(recentData?.items) ? recentData.items : cached.slice(0, 6);
  const needsReviewCount = Number(counts.needs_review ?? needsReviewItems.length);
  const readyCount = Number(counts.approved_ready ?? 0);
  const inProductionCount = Number(counts.processing_done ?? 0) + Number(counts.labels_ready ?? 0);
  const completedCount = Number(counts.finished ?? 0);

  if (overviewNeedsReview) overviewNeedsReview.textContent = String(needsReviewCount);
  if (overviewReadyProduction) overviewReadyProduction.textContent = String(readyCount);
  if (overviewInProduction) overviewInProduction.textContent = String(inProductionCount);
  if (overviewCompleted) overviewCompleted.textContent = String(completedCount);
  if (overviewHeadline){
    overviewHeadline.textContent = needsReviewCount
      ? `${needsReviewCount} order${needsReviewCount === 1 ? "" : "s"} need review and ${readyCount} are ready for production.`
      : `${readyCount} order${readyCount === 1 ? "" : "s"} are ready for production. Nothing currently needs review.`;
  }
  if (overviewAttentionList){
    overviewAttentionList.innerHTML = needsReviewItems.length
      ? needsReviewItems.slice(0, 4).map(item => overviewOrderRowHtml(item)).join("")
      : '<div class="overview-empty success">No orders need review right now.</div>';
  }
  if (overviewRecentOrders){
    overviewRecentOrders.innerHTML = recentItems.length
      ? recentItems.slice(0, 5).map(item => overviewOrderRowHtml(item)).join("")
      : '<div class="overview-empty">No saved orders yet.</div>';
  }

  if (queue) workspaceState.queue = queue;
  const failures = [queueResult, recentResult].filter(result => result.status === "rejected").length;
  if (overviewStatus){
    overviewStatus.textContent = failures
      ? "Some live data was unavailable. Showing the latest available information."
      : `Updated ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
  }
  overviewLoading = false;
}

const saveToast = document.getElementById("saveToast");
const saveToastMessage = document.getElementById("saveToastMessage");
const saveToastView = document.getElementById("saveToastView");
const saveToastDismiss = document.getElementById("saveToastDismiss");
let saveToastTimer = null;
const processingToast = document.getElementById("processingToast");
const processingToastMessage = document.getElementById("processingToastMessage");
const processingToastDismiss = document.getElementById("processingToastDismiss");
let processingToastTimer = null;

const workspaceRefreshBtn = document.getElementById("workspaceRefresh");
const workspaceQueueEl = document.getElementById("workspaceQueue");
const workspaceQueueStatusEl = document.getElementById("workspaceQueueStatus");
const workspaceSelectionStatusEl = document.getElementById("workspaceSelectionStatus");
const workspaceProcessSelectedBtn = document.getElementById("workspaceProcessSelected");
const workspaceClearSelectionBtn = document.getElementById("workspaceClearSelection");
const workspaceRecentFilesEl = document.getElementById("workspaceRecentFiles");
const workspaceFilesStatusEl = document.getElementById("workspaceFilesStatus");
const workspaceChatLogEl = document.getElementById("workspaceChatLog");
const workspaceAssistantFilesEl = document.getElementById("workspaceAssistantFiles");
const workspaceCommandForm = document.getElementById("workspaceCommandForm");
const workspaceCommandInput = document.getElementById("workspaceCommandInput");
const workspaceSendBtn = document.getElementById("workspaceSend");
const workspaceAssistantStatusEl = document.getElementById("workspaceAssistantStatus");
const workspacePromptButtons = document.getElementById("workspacePromptButtons");
const workspaceSmartChatLogEl = document.getElementById("workspaceSmartChatLog");
const workspaceSmartChatActionsEl = document.getElementById("workspaceSmartChatActions");
const workspaceSmartChatForm = document.getElementById("workspaceSmartChatForm");
const workspaceSmartChatInput = document.getElementById("workspaceSmartChatInput");
const workspaceSmartChatSendBtn = document.getElementById("workspaceSmartChatSend");
const workspaceSmartChatStatusEl = document.getElementById("workspaceSmartChatStatus");
const workspaceSmartPromptButtons = document.getElementById("workspaceSmartPromptButtons");
const awaRefreshBtn = document.getElementById("awaRefresh");
const awaStatusEl = document.getElementById("awaStatus");
const awaSuggestedTodayEl = document.getElementById("awaSuggestedToday");
const awaReadyToProcessEl = document.getElementById("awaReadyToProcess");
const awaNeedsReviewEl = document.getElementById("awaNeedsReview");
const awaWaitingApprovalEl = document.getElementById("awaWaitingApproval");
const awaSuggestionsEl = document.getElementById("awaSuggestions");
const awaTimelineEl = document.getElementById("awaTimeline");
const awaChatLogEl = document.getElementById("awaChatLog");
const awaChatForm = document.getElementById("awaChatForm");
const awaChatInput = document.getElementById("awaChatInput");
const awaChatSendBtn = document.getElementById("awaChatSend");
const telegramFilesListEl = document.getElementById("telegramFilesList");
const telegramFilesStatusEl = document.getElementById("telegramFilesStatus");
const telegramFilesSearchInput = document.getElementById("telegramFilesSearch");
const telegramFilesStatusFilter = document.getElementById("telegramFilesStatusFilter");
const telegramFilesTouchedFilter = document.getElementById("telegramFilesTouchedFilter");
const telegramFilesRefreshBtn = document.getElementById("telegramFilesRefresh");
const telegramFilesBadge = document.getElementById("telegramFilesBadge");
const telegramLiveStatusEl = document.getElementById("telegramLiveStatus");
const telegramPdfModal = document.getElementById("telegramPdfModal");
const telegramPdfTitle = document.getElementById("telegramPdfTitle");
const telegramPdfMeta = document.getElementById("telegramPdfMeta");
const telegramPdfFrame = document.getElementById("telegramPdfFrame");
const telegramPdfPrintBtn = document.getElementById("telegramPdfPrint");
const telegramPdfDownloadLink = document.getElementById("telegramPdfDownload");
const telegramPdfCloseBtn = document.getElementById("telegramPdfClose");

const statusEl = document.getElementById("status");
const errorEl = document.getElementById("error");
const historyStatusEl = document.getElementById("historyStatus");
const historyOfflineBadge = document.getElementById("historyOfflineBadge");
const historyPageEl = document.getElementById("historyPage");
const historyDetailEmpty = document.getElementById("historyDetailEmpty");
const historyDrawerLayer = document.getElementById("historyDrawerLayer");
const historyDrawerClose = document.getElementById("historyDrawerClose");
const historyRawTextEl = document.getElementById("historyRawText");
const historyStatusTimelineEl = document.getElementById("historyStatusTimeline");
const historyPrintBtn = document.getElementById("historyPrint");
const historyCsvBtn = document.getElementById("historyCsv");
const historyMetaEl = document.getElementById("historyMeta");
const historyProcessingBtn = document.getElementById("historyProcessing");
const historyInvoiceBtn = document.getElementById("historyInvoice");
const historyArchiveBtn = document.getElementById("historyArchive");
const historyInvoiceAllBtn = document.getElementById("historyInvoiceAll");
const orderDetailTitle = document.getElementById("orderDetailTitle");
const orderDetailStatus = document.getElementById("orderDetailStatus");
const orderDetailClient = document.getElementById("orderDetailClient");
const orderLifecycleEl = document.getElementById("orderLifecycle");
const orderDetailReviewSummary = document.getElementById("orderDetailReviewSummary");
const orderDetailSourceMeta = document.getElementById("orderDetailSourceMeta");
const orderDetailSourceEmpty = document.getElementById("orderDetailSourceEmpty");
const orderDetailSourceFrame = document.getElementById("orderDetailSourceFrame");
const orderDetailSourceHighlight = document.getElementById("orderDetailSourceHighlight");
const orderDetailFiles = document.getElementById("orderDetailFiles");
const orderSourceCanvas = document.getElementById("orderSourceCanvas");
const orderSourcePageLabel = document.getElementById("orderSourcePageLabel");
const orderSourcePrev = document.getElementById("orderSourcePrev");
const orderSourceNext = document.getElementById("orderSourceNext");
const extractSourceMeta = document.getElementById("extractSourceMeta");
const extractPdfPreviewEmpty = document.getElementById("extractPdfPreviewEmpty");
const extractPdfPreview = document.getElementById("extractPdfPreview");
const extractPdfCanvas = document.getElementById("extractPdfCanvas");
const extractPdfPageLabel = document.getElementById("extractPdfPageLabel");
const extractPdfPrev = document.getElementById("extractPdfPrev");
const extractPdfNext = document.getElementById("extractPdfNext");
const extractSourceHighlight = document.getElementById("extractSourceHighlight");
const extractIssueCount = document.getElementById("extractIssueCount");
const extractReviewIssuesEl = document.getElementById("extractReviewIssues");
const extractPrevIssueBtn = document.getElementById("extractPrevIssue");
const extractNextIssueBtn = document.getElementById("extractNextIssue");
const historySearchInput = document.getElementById("historySearch");
const historyClientFilterInput = document.getElementById("historyClientFilter");
const historyStatusFilterSelect = document.getElementById("historyStatusFilter");
const historyDateFromInput = document.getElementById("historyDateFrom");
const historyDateToInput = document.getElementById("historyDateTo");
const historyApprovedOnlyToggle = document.getElementById("historyApprovedOnly");
const historyQueueStatus = document.getElementById("historyQueueStatus");
const historyQueueActiveLabel = document.getElementById("historyQueueActiveLabel");
const historySelectionCount = document.getElementById("historySelectionCount");
const historySelectVisible = document.getElementById("historySelectVisible");
const historyClearSelection = document.getElementById("historyClearSelection");
const historyBatchApprove = document.getElementById("historyBatchApprove");
const historyBatchProduction = document.getElementById("historyBatchProduction");
const historyBatchArchive = document.getElementById("historyBatchArchive");
const processingOrderList = document.getElementById("processingOrderList");
const processingMetaEl = document.getElementById("processingMeta");
const processingPreviewEl = document.getElementById("processingPreview");
const processingClearBtn = document.getElementById("processingClear");
const processingRestartToggle = document.getElementById("processingRestartToggle");
const processingNormalizeToggle = document.getElementById("processingNormalizeToggle");
const processingDecimalSelect = document.getElementById("processingDecimalSelect");
const processingMergeOrdersToggle = document.getElementById("processingMergeOrdersToggle");
const processingHeaderSelect = document.getElementById("processingHeaderSelect");
const processingHeaderInput = document.getElementById("processingHeaderInput");
const processingHeaderApply = document.getElementById("processingHeaderApply");
const processingHeaderReset = document.getElementById("processingHeaderReset");
const processingHeaderHint = document.getElementById("processingHeaderHint");
const processingRoundingBtn = document.getElementById("processingRoundingToggle");
const processingGroupBtn = document.getElementById("processingGroup");
const processingCopyBtn = document.getElementById("processingCopy");
const processingExportPdfBtn = document.getElementById("processingExportPdf");
const processingExportCsvBtn = document.getElementById("processingExportCsv");
const spacerOutputEl = document.getElementById("spacerOutput");
const spacerMergeOrdersToggle = document.getElementById("spacerMergeOrdersToggle");
const spacerPdfPreviewToggle = document.getElementById("spacerPdfPreviewToggle");
const spacerWysiwygToggle = document.getElementById("spacerWysiwygToggle");
const spacerModeDebugEl = document.getElementById("spacerModeDebug");
const spacerGeneratePdfBtn = document.getElementById("spacerGeneratePdf");
const spacerCopyAllBtn = document.getElementById("spacerCopyAll");
const processingRoundingStatus = document.getElementById("processingRoundingStatus");
const processingAutoDankoToggle = document.getElementById("processingAutoDankoToggle");
const scanStatusEl = document.getElementById("scanStatus");
const scanDropZone = document.getElementById("scanDropZone");
const scanFileInput = document.getElementById("scanFileInput");
const scanPageListEl = document.getElementById("scanPageList");
const scanRotateLeftBtn = document.getElementById("scanRotateLeft");
const scanRotateRightBtn = document.getElementById("scanRotateRight");
const scanCropToggleBtn = document.getElementById("scanCropToggle");
const scanCropResetBtn = document.getElementById("scanCropReset");
const scanPerspectiveToggleBtn = document.getElementById("scanPerspectiveToggle");
const scanPerspectiveResetBtn = document.getElementById("scanPerspectiveReset");
const scanFilterSelect = document.getElementById("scanFilter");
const scanBrightnessInput = document.getElementById("scanBrightness");
const scanBrightnessValue = document.getElementById("scanBrightnessValue");
const scanContrastInput = document.getElementById("scanContrast");
const scanContrastValue = document.getElementById("scanContrastValue");
const scanPrintCurrentBtn = document.getElementById("scanPrintCurrent");
	const scanPrintAllBtn = document.getElementById("scanPrintAll");
	const scanExportPdfBtn = document.getElementById("scanExportPdf");
	const scanExportImageBtn = document.getElementById("scanExportImage");
	const scanPreviewStage = document.getElementById("scanPreviewStage");
	const scanPreviewCanvas = document.getElementById("scanPreviewCanvas");
	const scanOverlayCanvas = document.getElementById("scanOverlayCanvas");
	const globalScanDropOverlay = document.getElementById("globalScanDropOverlay");
	const globalScanDropTitle = document.getElementById("globalScanDropTitle");
	const globalScanDropHelper = document.getElementById("globalScanDropHelper");
	const globalScanDropMessage = document.getElementById("globalScanDropMessage");
	const pdfEditorStatusEl = document.getElementById("pdfEditorStatus");
	const pdfEditorUnsavedEl = document.getElementById("pdfEditorUnsaved");
	const pdfEditorFileInput = document.getElementById("pdfEditorFileInput");
	const pdfEditorImageInput = document.getElementById("pdfEditorImageInput");
	const pdfEditorUploadBtn = document.getElementById("pdfEditorUploadBtn");
	const pdfEditorDropZone = document.getElementById("pdfEditorDropZone");
	const pdfEditorEmpty = document.getElementById("pdfEditorEmpty");
	const pdfEditorDocumentWrap = document.getElementById("pdfEditorDocumentWrap");
	const pdfEditorStage = document.getElementById("pdfEditorStage");
	const pdfEditorPageFrame = document.getElementById("pdfEditorPageFrame");
	const pdfEditorCanvas = document.getElementById("pdfEditorCanvas");
	const pdfEditorTextLayer = document.getElementById("pdfEditorTextLayer");
	const pdfEditorOverlayLayer = document.getElementById("pdfEditorOverlayLayer");
	const pdfEditorInlineEditor = document.getElementById("pdfEditorInlineEditor");
	const pdfEditorInlineText = document.getElementById("pdfEditorInlineText");
	const pdfEditorInlineCancel = document.getElementById("pdfEditorInlineCancel");
	const pdfEditorInlineSave = document.getElementById("pdfEditorInlineSave");
	const pdfEditorThumbs = document.getElementById("pdfEditorThumbs");
	const pdfEditorPageCount = document.getElementById("pdfEditorPageCount");
	const pdfEditorPageIndicator = document.getElementById("pdfEditorPageIndicator");
	const pdfEditorPrevPage = document.getElementById("pdfEditorPrevPage");
	const pdfEditorNextPage = document.getElementById("pdfEditorNextPage");
	const pdfEditorZoomOut = document.getElementById("pdfEditorZoomOut");
	const pdfEditorZoomIn = document.getElementById("pdfEditorZoomIn");
	const pdfEditorFitWidth = document.getElementById("pdfEditorFitWidth");
	const pdfEditorRotateLeft = document.getElementById("pdfEditorRotateLeft");
	const pdfEditorRotateRight = document.getElementById("pdfEditorRotateRight");
	const pdfEditorDeletePage = document.getElementById("pdfEditorDeletePage");
	const pdfEditorAddImage = document.getElementById("pdfEditorAddImage");
	const pdfEditorExport = document.getElementById("pdfEditorExport");
	const pdfEditorSendExtraction = document.getElementById("pdfEditorSendExtraction");
	const pdfEditorToolLabel = document.getElementById("pdfEditorToolLabel");
	const pdfEditorSelectedProps = document.getElementById("pdfEditorSelectedProps");
	const pdfEditorTextEdits = document.getElementById("pdfEditorTextEdits");
	const labelsJobsWrap = document.getElementById("labelsJobsWrap");
const labelsSummaryEl = document.getElementById("labelsSummary");
const labelsStatusEl = document.getElementById("labelsStatus");
const labelsFromProcessingBtn = document.getElementById("labelsFromProcessing");
const analysisStatusEl = document.getElementById("analysisStatus");
const analysisDateStartInput = document.getElementById("analysisDateStart");
const analysisDateEndInput = document.getElementById("analysisDateEnd");
const analysisCompareToggle = document.getElementById("analysisCompareToggle");
const analysisClientFilter = document.getElementById("analysisClientFilter");
const analysisTypeFilter = document.getElementById("analysisTypeFilter");
const analysisRefreshBtn = document.getElementById("analysisRefresh");
const analysisExportBtn = document.getElementById("analysisExport");
const analysisRangePresetsEl = document.getElementById("analysisRangePresets");
const analysisPeriodMetaEl = document.getElementById("analysisPeriodMeta");
const analysisFreshnessEl = document.getElementById("analysisFreshness");
const analysisKpisEl = document.getElementById("analysisKpis");
const analysisInsightsEl = document.getElementById("analysisInsights");
const analysisAreaChartEl = document.getElementById("analysisAreaChart");
const analysisUnitsChartEl = document.getElementById("analysisUnitsChart");
const analysisOrdersChartEl = document.getElementById("analysisOrdersChart");
const analysisTopClientsChartEl = document.getElementById("analysisTopClientsChart");
const analysisTopClientsBarsEl = document.getElementById("analysisTopClientsBars");
const analysisTypeDistributionChartEl = document.getElementById("analysisTypeDistributionChart");
const analysisDimensionsChartEl = document.getElementById("analysisDimensionsChart");
const analysisAreaByTypeChartEl = document.getElementById("analysisAreaByTypeChart");
const analysisDistributionMetricEl = document.getElementById("analysisDistributionMetric");
const analysisTopClientsHead = document.getElementById("analysisTopClientsHead");
const analysisTopTypesHead = document.getElementById("analysisTopTypesHead");
const analysisDimensionsHead = document.getElementById("analysisDimensionsHead");
const analysisKpiOrdersEl = document.getElementById("analysisKpiOrders");
const analysisKpiUnitsEl = document.getElementById("analysisKpiUnits");
const analysisKpiAreaEl = document.getElementById("analysisKpiArea");
const analysisKpiAvgAreaEl = document.getElementById("analysisKpiAvgArea");
const analysisKpiAvgUnitsEl = document.getElementById("analysisKpiAvgUnits");
const analysisTopClientsBody = document.getElementById("analysisTopClientsBody");
const analysisTopTypesBody = document.getElementById("analysisTopTypesBody");
const analysisDimensionsBody = document.getElementById("analysisDimensionsBody");
const analysisTopOrdersBody = document.getElementById("analysisTopOrdersBody");
const analysisToleranceInput = document.getElementById("analysisTolerance");
const analysisOrientationToggle = document.getElementById("analysisOrientation");
const analysisRecalcBtn = document.getElementById("analysisRecalc");
const analysisQuestionInput = document.getElementById("analysisQuestion");
const analysisAskBtn = document.getElementById("analysisAsk");
const analysisAnswerEl = document.getElementById("analysisAnswer");
const analysisQuickActionsEl = document.getElementById("analysisQuickActions");
const analysisDebugToggle = document.getElementById("analysisDebugToggle");
const wysiwygToggle = document.getElementById("processingWysiwygToggle");
const wysiwygPreview = document.getElementById("motherSheetPreview");
const invoiceJobsWrap = document.getElementById("invoiceJobsWrap");
const invoiceStatusEl = document.getElementById("invoiceStatus");
const invoiceSummaryEl = document.getElementById("invoiceSummary");
const invoiceDetailMeta = document.getElementById("invoiceDetailMeta");
const invoiceLinesWrap = document.getElementById("invoiceLinesWrap");
const invoiceTotalsEl = document.getElementById("invoiceTotals");
const invoiceMissingNotice = document.getElementById("invoiceMissingNotice");
const invoiceDiscountMode = document.getElementById("invoiceDiscountMode");
const invoiceDiscountValue = document.getElementById("invoiceDiscountValue");
const invoiceVatValue = document.getElementById("invoiceVatValue");
const invoiceStatusSelect = document.getElementById("invoiceStatusSelect");
const invoiceRecalcBtn = document.getElementById("invoiceRecalc");
const invoiceGeneratePdfBtn = document.getElementById("invoiceGeneratePdf");
const invoiceDeleteActiveBtn = document.getElementById("invoiceDeleteActive");
const invoiceAddOrderBtn = document.getElementById("invoiceAddOrderBtn");
const invoiceFilterClient = document.getElementById("invoiceFilterClient");
const invoiceFilterOrder = document.getElementById("invoiceFilterOrder");
const invoiceFilterStatus = document.getElementById("invoiceFilterStatus");
const invoiceGlassPricesWrap = document.getElementById("invoiceGlassPrices");
const invoiceSpacerPricesWrap = document.getElementById("invoiceSpacerPrices");
const invoicePagination = document.getElementById("invoicePagination");
const invoicePaginationPrev = document.getElementById("invoicePaginationPrev");
const invoicePaginationNext = document.getElementById("invoicePaginationNext");
const invoiceAddGlassBtn = document.getElementById("invoiceAddGlassPrice");
const invoiceAddSpacerBtn = document.getElementById("invoiceAddSpacerPrice");
const invoiceSavePricesBtn = document.getElementById("invoiceSavePrices");
const invoicePricesStatus = document.getElementById("invoicePricesStatus");
const invoiceRefreshPricesBtn = document.getElementById("invoiceRefreshPrices");
const invoicePromptLayer = document.getElementById("invoicePrompt");
const invoicePromptTitle = document.getElementById("invoicePromptTitle");
const invoicePromptText = document.getElementById("invoicePromptText");
const invoicePromptFields = document.getElementById("invoicePromptFields");
const invoicePromptDescriptor = document.getElementById("invoicePromptDescriptor");
const invoicePromptValue = document.getElementById("invoicePromptValue");
const invoicePromptCancel = document.getElementById("invoicePromptCancel");
const invoicePromptSave = document.getElementById("invoicePromptSave");
const invoiceAddOrderModal = document.getElementById("invoiceAddOrderModal");
const invoiceAddOrderList = document.getElementById("invoiceAddOrderList");
const invoiceAddOrderStatus = document.getElementById("invoiceAddOrderStatus");
const invoiceAddOrderClose = document.getElementById("invoiceAddOrderClose");
const invoiceAddOrderHint = document.getElementById("invoiceAddOrderHint");
const invoiceAddOrderSearch = document.getElementById("invoiceAddOrderSearch");
const typeCorrectionsPanel = document.getElementById("typeCorrectionsPanel");
const typeCorrectionsTable = document.getElementById("typeCorrectionsTable");
const typeCorrectionsAddBtn = document.getElementById("typeCorrectionsAdd");
const typeCorrectionsModal = document.getElementById("typeCorrectionsModal");
const typeCorrectionsRawInput = document.getElementById("typeCorrectionsRaw");
const typeCorrectionsCorrectedInput = document.getElementById("typeCorrectionsCorrected");
const typeCorrectionsSaveBtn = document.getElementById("typeCorrectionsSave");
const typeCorrectionsCancelBtn = document.getElementById("typeCorrectionsCancel");

const groupToggles = document.querySelectorAll("[data-group-toggle]");

function activateTab(name){
	  if (name === "extract"){
	    setNewOrderWorkspaceOpen(false, { load: false });
	  }else{
	    updateAppChrome(name);
	  }
	  setMobileNavOpen(false);
	  const primaryTab = name === "orderdetail" ? "history" : name;
	  tabs.forEach(btn => {
	    const isTarget = btn.dataset.tab === primaryTab;
	    if (isTarget){
	      btn.classList.add("active");
	    }else{
	      btn.classList.remove("active");
	    }
	  });
	  const navParentByTab = {
	    workspace: "production",
	    processing: "production",
	    labels: "production",
	    awa: "production",
	    telegram: "documents",
	    pdfeditor: "documents",
	    scanstudio: "documents",
	  };
	  const activeNavSection = navParentByTab[name] || "";
	  document.querySelectorAll("[data-nav-section]").forEach(btn => {
	    btn.classList.toggle("section-active", btn.dataset.navSection === activeNavSection);
	  });
	  Object.entries(panels).forEach(([key, panel])=>{
    if (!panel) return;
    const isTarget = key === name;
    if (isTarget){
      panel.classList.add("active");
      panel.hidden = false;
      panel.setAttribute("aria-hidden", "false");
    }else{
      panel.classList.remove("active");
      panel.hidden = true;
      panel.setAttribute("aria-hidden", "true");
    }
  });
  if (name === "extract"){
    loadOverview();
  }else if (name === "history"){
    ensureHistoryLoaded();
    loadHistoryWorkQueue();
  }else if (name === "manual"){
    ensureManualOrdersReady();
  }else if (name === "workspace"){
    loadWorkspace();
  }else if (name === "awa"){
    loadAwa();
  }else if (name === "telegram"){
    loadTelegramFiles();
	  }else if (name === "scanstudio"){
	    renderScanPageList();
	    syncScanControlsFromSelectedPage();
	    renderScanPreview();
	    queueMicrotask(()=> consumePendingGlobalScanDrop());
	  }else if (name === "pdfeditor"){
	    renderPdfEditor();
	  }else if (name === "processing"){
    hideWorkspaceProcessingJobView();
    ensureHistoryLoaded();
    rebuildProcessingRows();
    updateProcessingUI();
  }else if (name === "spacer"){
    updateSpacerProcessingUI();
  }else if (name === "analysis"){
    ensureAnalysisReady();
    if (analysisState.initialized){
      refreshAnalysis({ reason: "tab-open" });
    }
  }else if (name === "labels"){
    updateLabelsUI();
  }else if (name === "invoices"){
    ensureInvoicesLoaded();
    syncInvoiceConfigFromServer({ silent: true });
    updateInvoicesUI();
  }
}

tabs.forEach(btn=>{
  btn.addEventListener("click", ()=> activateTab(btn.dataset.tab));
});

document.getElementById("overviewNewOrder")?.addEventListener("click", ()=>{
  setNewOrderWorkspaceOpen(true);
});

document.getElementById("newOrderBack")?.addEventListener("click", ()=>{
  setNewOrderWorkspaceOpen(false);
  window.scrollTo({ top: 0, behavior: "smooth" });
});

document.getElementById("overviewViewOrders")?.addEventListener("click", ()=>{
  activateTab("history");
});

document.getElementById("overviewRefresh")?.addEventListener("click", ()=>{
  loadOverview();
});

overviewDashboard?.addEventListener("click", async event=>{
  const routeButton = event.target.closest("[data-overview-route]");
  if (routeButton){
    activateTab(routeButton.dataset.overviewRoute);
    return;
  }
  const orderButton = event.target.closest("[data-overview-order-id]");
  if (!orderButton?.dataset.overviewOrderId) return;
  activateTab("history");
  await openOrderFromList(orderButton.dataset.overviewOrderId);
});

document.getElementById("workspaceOpenAwa")?.addEventListener("click", ()=>{
  activateTab("awa");
});

window.matchMedia("(prefers-color-scheme: dark)").addEventListener?.("change", syncSettingsSummary);

if (mobileNavToggle){
  mobileNavToggle.addEventListener("click", ()=>{
    const open = mobileNavToggle.getAttribute("aria-expanded") !== "true";
    setMobileNavOpen(open);
  });
}

document.addEventListener("keydown", event=>{
  if (event.key === "Escape") setMobileNavOpen(false);
});

document.addEventListener("click", event=>{
  if (!appSidebar?.classList.contains("mobile-nav-open")) return;
  if (!appSidebar.contains(event.target)) setMobileNavOpen(false);
});

window.addEventListener("resize", ()=>{
  if (!window.matchMedia("(max-width: 768px)").matches) setMobileNavOpen(false);
});

updateAppChrome("extract");
loadOverview();

Object.entries(panels).forEach(([key, panel])=>{
  if (!panel) return;
  const isActive = key === "extract";
  panel.hidden = !isActive;
  panel.setAttribute("aria-hidden", isActive ? "false" : "true");
});

function showSavedToast(orderId){
  if (!orderId) return;
  saveToast.dataset.orderId = orderId;
  saveToastMessage.textContent = `Saved as #${orderId}`;
  saveToast.hidden = false;
  clearTimeout(saveToastTimer);
  saveToastTimer = setTimeout(()=> hideSavedToast(), 6000);
}

function hideSavedToast(){
  saveToast.hidden = true;
  saveToastMessage.textContent = "";
  delete saveToast.dataset.orderId;
}

saveToastDismiss.addEventListener("click", hideSavedToast);
saveToastView.addEventListener("click", ()=>{
  const id = Number(saveToast.dataset.orderId);
  if (!id) return;
  hideSavedToast();
  openSharedOrderDetail(id, { activateHistoryTab: true });
});

function showProcessingToast(message = "Added to Processing"){
  if (!processingToast || !processingToastMessage) return;
  processingToastMessage.textContent = message;
  processingToast.hidden = false;
  clearTimeout(processingToastTimer);
  processingToastTimer = setTimeout(()=> hideProcessingToast(), 4000);
}

function hideProcessingToast(){
  if (!processingToast) return;
  processingToast.hidden = true;
  if (processingToastMessage) processingToastMessage.textContent = "";
}

if (processingToastDismiss){
  processingToastDismiss.addEventListener("click", hideProcessingToast);
}

function setGroupState(value){
  groupState = !!value;
  groupToggles.forEach(toggle => {
    if (toggle.checked !== groupState){
      toggle.checked = groupState;
    }
  });
  updateExtractUI();
  updateHistoryDetailUI();
}

groupToggles.forEach(toggle=>{
  toggle.addEventListener("change", (event)=> setGroupState(event.target.checked));
});

setGroupState(false);

function clampNumber(value, min, max, fallback){
  const num = Number(value);
  if (!Number.isFinite(num)) return fallback;
  return Math.min(Math.max(num, min), max);
}

function setAnalysisStatus(message){
  if (analysisStatusEl){
    analysisStatusEl.textContent = message || "";
  }
}

function pickFirstString(values, fallback = ""){
  if (values == null) return fallback;
  if (!Array.isArray(values)){
    const str = String(values).trim();
    return str || fallback;
  }
  for (const value of values){
    if (Array.isArray(value)){
      const nested = pickFirstString(value);
      if (nested) return nested;
    }else if (value != null){
      const str = String(value).trim();
      if (str) return str;
    }
  }
  return fallback;
}

function getClientName(record, fallback = "—"){
  if (!record) return fallback;
  const values = [
    record.client_name,
    record.clientName,
    record.client,
    record.client_hint,
  ];
  for (const value of values){
    if (value == null) continue;
    const str = String(value).trim();
    if (str && str !== "—" && str !== "-") return str;
  }
  return fallback;
}

function pickFirstNumber(values){
  if (values == null) return null;
  if (!Array.isArray(values)){
    const numeric = typeof values === "string" ? values.replace(",", ".") : values;
    const num = Number(numeric);
    return Number.isFinite(num) ? num : null;
  }
  for (const value of values){
    if (Array.isArray(value)){
      const nested = pickFirstNumber(value);
      if (nested != null) return nested;
    }else if (value != null && value !== ""){
      const normalized = typeof value === "string" ? value.replace(",", ".") : value;
      const num = Number(normalized);
      if (Number.isFinite(num)) return num;
    }
  }
  return null;
}

function findNumeric(values){
  if (values == null) return null;
  if (!Array.isArray(values)){
    return normalizeDimensionNumber(values);
  }
  for (const value of values){
    if (Array.isArray(value)){
      const nested = findNumeric(value);
      if (nested != null) return nested;
    }else{
      const parsed = normalizeDimensionNumber(value);
      if (parsed != null) return parsed;
    }
  }
  return null;
}

function extractOrderDate(order){
  if (!order || typeof order !== "object") return null;
  const candidates = [
    order.created_at,
    order.createdAt,
    order.created,
    order.inserted_at,
    order.insertedAt,
    order.timestamp,
    order.date,
  ];
  for (const candidate of candidates){
    if (candidate == null || candidate === "") continue;
    if (typeof candidate === "number"){
      const date = new Date(candidate);
      if (!Number.isNaN(date.getTime())) return date;
      continue;
    }
    const str = String(candidate).trim();
    if (!str) continue;
    const date = new Date(str);
    if (!Number.isNaN(date.getTime())) return date;
  }
  return null;
}

function getOrderLabel(order){
  if (!order) return "—";
  const numbers = Array.isArray(order.order_numbers)
    ? order.order_numbers.map(value => value == null ? "" : String(value).trim()).filter(Boolean)
    : [];
  if (numbers.length){
    return numbers.join(", ");
  }
  const fallback = pickFirstString([
    order.order_number,
    order.orderNumber,
    order.order_no,
    order.orderNo,
    order.order,
    order.orderId,
    order.order_id,
    order.id,
  ]);
  if (fallback){
    return fallback;
  }
  return order.id != null ? `#${order.id}` : "—";
}

function computeRowArea(row){
  if (!row) return 0;
  const directArea = pickFirstNumber([
    row.area,
    row.area_m2,
    row.areaM2,
    row.computed_area,
    row.computedArea,
  ]);
  if (directArea != null) return directArea;
  let width = normalizeDimensionNumber(row.width);
  let height = normalizeDimensionNumber(row.height);
  if (width == null || height == null){
    const dimensionLabel = pickFirstString([
      row.dimension,
      row.dimension_display,
      row.dim,
      row.dimensionDisplay,
    ]);
    if (dimensionLabel){
      const parsed = parseDimensionTokens(dimensionLabel);
      if (width == null && parsed.width != null) width = parsed.width;
      if (height == null && parsed.height != null) height = parsed.height;
    }
  }
  if (width != null && height != null){
    return (width * height) / 1000000;
  }
  return 0;
}

function mapRowForAnalysis(row, orderLabel, clientLabel){
  if (!row) return null;
  const dimensionDisplay = pickFirstString([
    row.dimension,
    row.dimension_display,
    row.dim,
    row.dimensionDisplay,
  ]);
  let width = findNumeric([
    row.width,
    row.width_mm,
    row.width_mm_calc,
    row.widthValue,
    row.widthNumeric,
  ]);
  let height = findNumeric([
    row.height,
    row.height_mm,
    row.height_mm_calc,
    row.heightValue,
    row.heightNumeric,
  ]);
  let widthDisplay = pickFirstString([
    row.width_display,
    row.widthDisplay,
    row.width_text,
  ]);
  let heightDisplay = pickFirstString([
    row.height_display,
    row.heightDisplay,
    row.height_text,
  ]);
  if ((width == null || height == null) && dimensionDisplay){
    const parsed = parseDimensionTokens(dimensionDisplay);
    if (width == null && parsed.width != null) width = parsed.width;
    if (height == null && parsed.height != null) height = parsed.height;
    if (!widthDisplay && parsed.widthDisplay) widthDisplay = parsed.widthDisplay;
    if (!heightDisplay && parsed.heightDisplay) heightDisplay = parsed.heightDisplay;
  }
  if (!widthDisplay && width != null){
    widthDisplay = String(width);
  }
  if (!heightDisplay && height != null){
    heightDisplay = String(height);
  }
  const qty = Math.max(0, pickFirstNumber([
    row.quantity,
    row.qty,
    row.Qty,
    row.QTY,
    row.qty_total,
    row.total_qty,
    row.quantity_total,
    row.qtyTotal,
  ]) || 0);
  const type = pickFirstString([
    row.type,
    row.composition_raw,
    row.composition,
    row.composition_display,
    row.glass_type,
    row.glassType,
    row.description,
  ], "");
  const widthNumeric = Number.isFinite(width) ? width : null;
  const heightNumeric = Number.isFinite(height) ? height : null;
  const widthDisplayValue = (() => {
    const display = widthDisplay != null ? String(widthDisplay).trim() : "";
    if (display) return display;
    return widthNumeric != null ? String(widthNumeric) : null;
  })();
  const heightDisplayValue = (() => {
    const display = heightDisplay != null ? String(heightDisplay).trim() : "";
    if (display) return display;
    return heightNumeric != null ? String(heightNumeric) : null;
  })();
  const orderIdValue = orderLabel && orderLabel.trim().length ? orderLabel.trim() : "—";
  const clientValue = clientLabel && clientLabel.trim().length ? clientLabel.trim() : "—";
  const dimensionLabel = dimensionDisplay && dimensionDisplay.trim().length ? dimensionDisplay.trim() : null;
  return {
    type,
    width: widthNumeric,
    height: heightNumeric,
    widthDisplay: widthDisplayValue,
    heightDisplay: heightDisplayValue,
    qty,
    orderId: orderIdValue,
    client: clientValue,
    dimensionDisplay: dimensionLabel,
  };
}

function cloneRowArray(rows){
  return Array.isArray(rows) ? rows.map(item => ({ ...item })) : [];
}

async function resolveOrderRowsForAnalysis(order){
  if (!order || order.id == null){
    return [];
  }
  if (Array.isArray(order.rows) && order.rows.length){
    analysisState.orderRowCache.set(order.id, cloneRowArray(order.rows));
    return cloneRowArray(order.rows);
  }
  if (analysisState.orderRowCache.has(order.id)){
    return cloneRowArray(analysisState.orderRowCache.get(order.id));
  }
  if (analysisState.orderRowRequests.has(order.id)){
    const pending = await analysisState.orderRowRequests.get(order.id);
    return cloneRowArray(pending);
  }
  const fetchPromise = (async ()=>{
    try{
      const detail = await fetchOrder(order.id);
      if (detail && Array.isArray(detail.rows) && detail.rows.length){
        analysisState.orderRowCache.set(order.id, cloneRowArray(detail.rows));
        return cloneRowArray(detail.rows);
      }
    }catch(error){
      console.warn("Analysis: failed to fetch order rows", error);
    }
    const cachedItems = readHistoryCache();
    const cachedOrder = cachedItems.find(item => item && item.id === order.id);
    if (cachedOrder && Array.isArray(cachedOrder.rows) && cachedOrder.rows.length){
      analysisState.orderRowCache.set(order.id, cloneRowArray(cachedOrder.rows));
      return cloneRowArray(cachedOrder.rows);
    }
    return [];
  })();
  analysisState.orderRowRequests.set(order.id, fetchPromise);
  try{
    const result = await fetchPromise;
    return cloneRowArray(result);
  }finally{
    analysisState.orderRowRequests.delete(order.id);
  }
}

async function mapHistoryOrderForAnalysis(order){
  const orderLabel = getOrderLabel(order);
  const client = pickFirstString([
    order.client,
    order.client_hint,
    order.client_name,
    order.clientName,
    order.customer,
  ], "—");
  const createdAt = pickFirstString([
    order.created_at,
    order.createdAt,
    order.created,
  ]) || null;
  const parsedUnits = pickFirstNumber([
    order.parsed_units,
    order.units_total,
    order.total_units,
    order.units,
  ]) || 0;
  const parsedArea = pickFirstNumber([
    order.parsed_area,
    order.area_total,
    order.total_area,
    order.area,
  ]) || 0;
  const orderRows = await resolveOrderRowsForAnalysis(order);
  const rows = orderRows
    .map(row => mapRowForAnalysis(row, orderLabel, client))
    .filter(Boolean);
  return {
    id: order.id,
    order_numbers: Array.isArray(order.order_numbers)
      ? order.order_numbers
      : (orderLabel && orderLabel !== "—"
        ? orderLabel.split(/\s*,\s*/).map(token => token.trim()).filter(Boolean)
        : []),
    order_number: pickFirstString([order.order_number, order.orderNumber]) || undefined,
    client,
    created_at: createdAt,
    parsed_units: parsedUnits,
    parsed_area: parsedArea,
    rows,
  };
}

function mapProcessingOrderForAnalysis(entry){
  if (!entry) return null;
  const label = entry.orderLabel || getOrderLabel(entry);
  const client = pickFirstString([
    entry.client,
    entry.client_label,
    entry.clientLabel,
  ], "—");
  const rows = Array.isArray(entry.rows)
    ? entry.rows.map(row => mapRowForAnalysis(row, label, client)).filter(Boolean)
    : [];
  const units = rows.reduce((sum, row)=> sum + (Number(row.qty) || 0), 0);
  const area = rows.reduce((sum, row)=> sum + computeRowArea(row), 0);
  const orderNumbers = label ? label.split(/\s*,\s*/).map(token => token.trim()).filter(Boolean) : [];
  return {
    id: entry.id ?? label,
    order_numbers: orderNumbers,
    order_number: orderNumbers.length === 1 ? orderNumbers[0] : undefined,
    client,
    created_at: entry.createdAt || new Date().toISOString(),
    parsed_units: units,
    parsed_area: area,
    rows,
  };
}

function sortOrdersNewestFirst(items){
  const list = Array.isArray(items) ? items.slice() : [];
  return list.sort((a, b)=>{
    const da = extractOrderDate(a);
    const db = extractOrderDate(b);
    const ta = da ? da.getTime() : 0;
    const tb = db ? db.getTime() : 0;
    return tb - ta;
  });
}

async function fetchAllApprovedOrders(){
  if (!analysisState.allOrdersDirty && Array.isArray(analysisState.allOrdersCache)){
    return analysisState.allOrdersCache.slice();
  }
  if (analysisState.allOrdersPromise){
    const cached = await analysisState.allOrdersPromise;
    return cached.slice();
  }
  const task = (async ()=>{
    const aggregated = [];
    let offset = 0;
    const limit = 100;
    let hasMore = true;
	    while (hasMore){
	      const params = { limit, offset, status: "approved", year: "all" };
	      let data;
	      try{
	        data = await fetchOrders(params);
	      }catch(error){
        console.warn("Analysis: failed to fetch all orders batch", error);
        break;
      }
      const items = Array.isArray(data?.items) ? data.items : [];
      aggregated.push(...items);
      hasMore = !!data?.has_more && items.length > 0;
      offset += limit;
      if (!hasMore){
        break;
      }
    }
    analysisState.allOrdersCache = aggregated;
    analysisState.allOrdersDirty = false;
    return aggregated.slice();
  })();
  analysisState.allOrdersPromise = task.finally(()=>{
    analysisState.allOrdersPromise = null;
  });
  return task;
}

async function buildAllTimeAnalysisDataset(options = {}){
  const tolerance = options.tolerance;
  const orientationAgnostic = options.orientationAgnostic;
  let approvedOrders = [];
  try{
    approvedOrders = await fetchAllApprovedOrders();
  }catch(error){
    console.warn("Analysis: unable to load all orders", error);
    approvedOrders = [];
  }
  const sortedAll = sortOrdersNewestFirst(approvedOrders);
  const orders = [];
  for (const order of sortedAll){
    orders.push(await mapHistoryOrderForAnalysis(order));
  }
  const processingOrders = Array.isArray(appState.processing.cart)
    ? appState.processing.cart.map(mapProcessingOrderForAnalysis).filter(Boolean)
    : [];
  const aggregates = computeAggregatesForAllTime(orders, processingOrders, {
    tolerance,
    orientationAgnostic,
  });
  return {
    scope: "all-time",
    orders,
    processing_orders: processingOrders,
    aggregates,
    meta: { truncated: false, notes: [] },
  };
}

function mergeOrdersForAnalysis(dataset){
  if (!dataset) return [];
  const base = Array.isArray(dataset.orders) ? dataset.orders.map(order => JSON.parse(JSON.stringify(order))) : [];
  if (Array.isArray(dataset.processing_orders)){
    dataset.processing_orders.forEach(order => {
      base.push(JSON.parse(JSON.stringify(order)));
    });
  }
  return base;
}

function collectAnalysisRows(orders){
  const rows = [];
  orders.forEach(order => {
    if (!order || !Array.isArray(order.rows)) return;
    order.rows.forEach(row => {
      if (row) rows.push({ ...row });
    });
  });
  return rows;
}

function getOrderTimestampForAnalysis(order){
  if (!order) return 0;
  const created = order.created_at || order.createdAt || null;
  if (created){
    const date = new Date(created);
    if (!Number.isNaN(date.getTime())) return date.getTime();
  }
  return 0;
}

function computeAggregatesForAllTime(orders, processing, options){
  const tolerance = Number(options?.tolerance) || 0;
  const orientationAgnostic = !!options?.orientationAgnostic;
  const totals = { orders: (orders?.length || 0) + (processing?.length || 0), units: 0, area_m2: 0 };
  const byClientMap = new Map();
  const byTypeMap = new Map();
  const dimensionRows = [];
  const recentRows = [];

  function addClientSample(order, units, area){
    const clientName = (order.client || "—").trim() || "—";
    if (!byClientMap.has(clientName)){
      byClientMap.set(clientName, {
        client: clientName,
        units: 0,
        area: 0,
        orders: new Set(),
      });
    }
    const entry = byClientMap.get(clientName);
    entry.units += units;
    entry.area += area;
    const orderKey = order.id != null ? String(order.id) : (Array.isArray(order.order_numbers) && order.order_numbers.length ? order.order_numbers.join("|") : order.order_number || order.orderId || Math.random().toString(16).slice(2));
    entry.orders.add(orderKey);
  }

  function foldOrder(order){
    if (!order || !Array.isArray(order.rows)) return;
    let orderUnits = 0;
    let orderArea = 0;
    order.rows.forEach(row => {
      if (!row) return;
      const qty = Number(row.qty) || 0;
      orderUnits += qty;
      const area = computeRowArea(row);
      orderArea += area;
      const typeKey = (row.type || "—").trim() || "—";
      if (!byTypeMap.has(typeKey)){
        byTypeMap.set(typeKey, { type: typeKey, lines: 0, units: 0, area: 0 });
      }
      const typeEntry = byTypeMap.get(typeKey);
      typeEntry.lines += 1;
      typeEntry.units += qty;
      typeEntry.area += area;
      dimensionRows.push({ ...row, orderId: row.orderId || getOrderLabel(order), client: row.client || order.client || "—" });
      recentRows.push({
        orderId: row.orderId || getOrderLabel(order),
        client: row.client || order.client || "—",
        type: row.type || "",
        width: row.width ?? null,
        height: row.height ?? null,
        widthDisplay: row.widthDisplay || null,
        heightDisplay: row.heightDisplay || null,
        qty,
        created_at: order.created_at || null,
      });
    });
    totals.units += orderUnits;
    totals.area_m2 += orderArea;
    addClientSample(order, orderUnits, orderArea);
  }

  (orders || []).forEach(foldOrder);
  (processing || []).forEach(foldOrder);

  const by_client = Array.from(byClientMap.values()).map(entry => ({
    client: entry.client,
    orders: entry.orders.size,
    units: entry.units,
    area_m2: Number(entry.area.toFixed(3)),
  })).sort((a, b)=>{
    if (b.area_m2 !== a.area_m2) return b.area_m2 - a.area_m2;
    return a.client.localeCompare(b.client);
  }).slice(0, 25);

  const by_type = Array.from(byTypeMap.values()).map(entry => ({
    type: entry.type,
    lines: entry.lines,
    units: entry.units,
    area_m2: Number(entry.area.toFixed(3)),
  })).sort((a, b)=>{
    if (b.units !== a.units) return b.units - a.units;
    return a.type.localeCompare(b.type);
  }).slice(0, 25);

  const dimBuckets = buildDimensionBuckets(dimensionRows, tolerance, orientationAgnostic).map(bucket => ({
    dim: bucket.dimension,
    qty: bucket.qty,
    orders: bucket.orders,
    clients: bucket.clients,
  }));

  recentRows.sort((a, b)=>{
    const ta = a.created_at ? new Date(a.created_at).getTime() : 0;
    const tb = b.created_at ? new Date(b.created_at).getTime() : 0;
    return tb - ta;
  });

  return {
    totals: {
      orders: totals.orders,
      units: totals.units,
      area_m2: Number(totals.area_m2.toFixed(3)),
    },
    by_client,
    by_type,
    by_dim: dimBuckets,
    recent_samples: recentRows.slice(0, 50),
  };
}

function prepareDatasetForTransmission(dataset, options){
  const ordersSource = Array.isArray(dataset?.orders) ? dataset.orders : [];
  const processingSource = Array.isArray(dataset?.processing_orders) ? dataset.processing_orders : [];
  const aggregates = dataset?.aggregates
    ? JSON.parse(JSON.stringify(dataset.aggregates))
    : computeAggregatesForAllTime(ordersSource, processingSource, {
        tolerance: options?.tolerance,
        orientationAgnostic: options?.orientationAgnostic,
      });
  const payloadMeta = {
    truncated: !!(dataset?.meta && dataset.meta.truncated),
    notes: Array.isArray(dataset?.meta?.notes) ? dataset.meta.notes.slice() : [],
  };
  const payload = {
    scope: dataset?.scope || "all-time",
    meta: payloadMeta,
    aggregates,
    orders: ordersSource.map(order => JSON.parse(JSON.stringify(order))),
    processing_orders: processingSource.map(order => JSON.parse(JSON.stringify(order))),
  };

  let serialized = JSON.stringify(payload);
  if (serialized.length > ANALYSIS_MAX_DATASET_CHARS){
    payload.meta.truncated = true;
    let trimmedOrders = 0;
    while (serialized.length > ANALYSIS_MAX_DATASET_CHARS && payload.orders.length > 0){
      payload.orders.pop();
      trimmedOrders += 1;
      serialized = JSON.stringify(payload);
    }
    if (trimmedOrders > 0){
      payload.meta.notes.push(`orders list trimmed to newest ${payload.orders.length} to fit token budget`);
    }
    let trimmedProcessing = 0;
    while (serialized.length > ANALYSIS_MAX_DATASET_CHARS && payload.processing_orders.length > 0){
      payload.processing_orders.pop();
      trimmedProcessing += 1;
      serialized = JSON.stringify(payload);
    }
    if (trimmedProcessing > 0){
      payload.meta.notes.push("processing orders trimmed to fit token budget");
    }
    if (serialized.length > ANALYSIS_MAX_DATASET_CHARS){
      payload.orders = [];
      payload.processing_orders = [];
      serialized = JSON.stringify(payload);
    }
  }

  payload.meta.size = serialized.length;
  return { dataset: payload, serialized, truncated: !!payload.meta.truncated };
}

function getAnalysisSessionId(){
  if (analysisState.sessionId){
    return analysisState.sessionId;
  }
  let sessionId = "local";
  try{
    const storage = window.localStorage;
    if (storage){
      const existing = storage.getItem("analysisSessionId");
      if (existing && existing.trim().length){
        sessionId = existing.trim();
      }else{
        const generated = (window.crypto && typeof window.crypto.randomUUID === "function")
          ? window.crypto.randomUUID()
          : `session-${Date.now()}-${Math.random().toString(16).slice(2)}`;
        storage.setItem("analysisSessionId", generated);
        sessionId = generated;
      }
    }
  }catch{
    sessionId = `session-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  }
  analysisState.sessionId = sessionId || "local";
  return analysisState.sessionId;
}

/**
 * @typedef {Object} AnalysisSnapshotFilters
 * @property {string} startDate
 * @property {string} endDate
 * @property {boolean} compareToPrevious
 * @property {string} client
 * @property {string} glassType
 * @property {number} tolerance
 * @property {boolean} orientationAgnostic
 * @property {boolean} allTime
 */

/**
 * @typedef {Object} AnalysisSnapshotPayload
 * @property {AnalysisSnapshotFilters} filters
 * @property {Object} kpis
 * @property {Object} charts
 * @property {Array<Object>} topClients
 * @property {Array<Object>} topGlassTypes
 * @property {Array<Object>} topDimensions
 * @property {Array<Object>} insights
 * @property {{clients:string[], glassTypes:string[]}} availableFilters
 * @property {Object} period
 * @property {Object|null} previousPeriod
 * @property {Object} current
 * @property {Object|null} previous
 * @property {string} computedAt
 * @property {"fresh"|"stale"} cacheStatus
 */

function createStableHash(value){
  const text = JSON.stringify(value || {});
  let hash = 2166136261;
  for (let index = 0; index < text.length; index += 1){
    hash ^= text.charCodeAt(index);
    hash = Math.imul(hash, 16777619);
  }
  return `a${(hash >>> 0).toString(16).padStart(8, "0")}`;
}

function normalizeSnapshotFilters(settings){
  return {
    startDate: String(settings?.startDate || ""),
    endDate: String(settings?.endDate || ""),
    compareToPrevious: !!settings?.comparePrevious,
    client: String(settings?.client || ""),
    glassType: String(settings?.glassType || ""),
    tolerance: Number(settings?.tolerance) || 0,
    orientationAgnostic: !!settings?.orientationAgnostic,
    allTime: !!settings?.allTime,
  };
}

function buildSnapshotCacheKey(filters){
  const normalized = normalizeSnapshotFilters(filters);
  return createStableHash(normalized);
}

const analysisSnapshotRepo = {
  map: new Map(),
  hydrated: false,
};

function hydrateAnalysisSnapshotRepo(){
  if (analysisSnapshotRepo.hydrated) return;
  analysisSnapshotRepo.hydrated = true;
  try{
    const raw = localStorage.getItem(ANALYSIS_SNAPSHOT_STORAGE_KEY);
    if (!raw) return;
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return;
    parsed.forEach(entry => {
      if (!entry || typeof entry !== "object") return;
      if (entry.version !== ANALYSIS_SNAPSHOT_VERSION) return;
      const key = String(entry.key || "").trim();
      if (!key) return;
      analysisSnapshotRepo.map.set(key, entry);
    });
  }catch{
    /* ignore local cache parse failures */
  }
}

function persistAnalysisSnapshotRepo(){
  try{
    const entries = Array.from(analysisSnapshotRepo.map.values())
      .sort((a, b)=> {
        const ta = new Date(a?.payload?.computedAt || 0).getTime();
        const tb = new Date(b?.payload?.computedAt || 0).getTime();
        return tb - ta;
      })
      .slice(0, ANALYSIS_MAX_SNAPSHOTS);
    localStorage.setItem(ANALYSIS_SNAPSHOT_STORAGE_KEY, JSON.stringify(entries));
  }catch{
    /* ignore local cache quota failures */
  }
}

function getAnalysisSnapshot(filters){
  hydrateAnalysisSnapshotRepo();
  const key = buildSnapshotCacheKey(filters);
  return analysisSnapshotRepo.map.get(key) || null;
}

function saveAnalysisSnapshot(snapshot){
  if (!snapshot || typeof snapshot !== "object") return;
  hydrateAnalysisSnapshotRepo();
  const key = String(snapshot.key || "").trim();
  if (!key) return;
  analysisSnapshotRepo.map.set(key, snapshot);
  persistAnalysisSnapshotRepo();
}

function getSnapshotAgeMs(snapshot){
  const computedAt = snapshot?.payload?.computedAt;
  const computedTs = computedAt ? new Date(computedAt).getTime() : 0;
  if (!computedTs) return Number.POSITIVE_INFINITY;
  return Math.max(0, Date.now() - computedTs);
}

function shouldRevalidateSnapshot(snapshot, options = {}){
  if (options.force) return true;
  if (!snapshot) return true;
  if (options.sourceDirty) return true;
  const ageMs = getSnapshotAgeMs(snapshot);
  return ageMs > ANALYSIS_SNAPSHOT_TTL_MS;
}

function formatSnapshotFreshness(computedAt, isRefreshing = false){
  if (!computedAt){
    return isRefreshing ? "Refreshing…" : "";
  }
  const timestamp = new Date(computedAt).getTime();
  if (!timestamp){
    return isRefreshing ? "Refreshing…" : "";
  }
  const diffMs = Math.max(0, Date.now() - timestamp);
  let label = "Updated just now";
  if (diffMs >= 60 * 1000){
    const minutes = Math.floor(diffMs / (60 * 1000));
    if (minutes < 60){
      label = `Updated ${minutes} min ago`;
    }else{
      const hours = Math.floor(minutes / 60);
      label = `Updated ${hours}h ago`;
    }
  }
  if (isRefreshing){
    label += " • Refreshing…";
  }
  return label;
}

function setAnalysisFreshnessLabel(computedAt, isRefreshing = false){
  if (!analysisFreshnessEl) return;
  analysisFreshnessEl.textContent = formatSnapshotFreshness(computedAt, isRefreshing);
}

function formatNumber(value, { decimals = 0 } = {}){
  const number = Number(value) || 0;
  return number.toLocaleString(undefined, {
    minimumFractionDigits: decimals,
    maximumFractionDigits: decimals,
  });
}

function formatAreaMetric(value, decimals = 3){
  return `${formatNumber(value, { decimals })}\u202Fm²`;
}

function renderNoDataRow(bodyEl, colspan){
  if (!bodyEl) return;
  bodyEl.innerHTML = `<tr><td colspan="${colspan}" class="muted">No data.</td></tr>`;
}

function buildDimensionBuckets(rows, tolerance, orientationAgnostic){
  const buckets = [];
  const tol = Number.isFinite(tolerance) ? tolerance : 1;
  rows.forEach(row => {
    if (!row) return;
    let width = normalizeDimensionNumber(row.width);
    let height = normalizeDimensionNumber(row.height);
    let widthDisplay = row.widthDisplay || null;
    let heightDisplay = row.heightDisplay || null;
    if ((width == null || height == null) && row.dimensionDisplay){
      const parsed = parseDimensionTokens(row.dimensionDisplay);
      if (width == null && parsed.width != null) width = parsed.width;
      if (height == null && parsed.height != null) height = parsed.height;
      if (!widthDisplay && parsed.widthDisplay) widthDisplay = parsed.widthDisplay;
      if (!heightDisplay && parsed.heightDisplay) heightDisplay = parsed.heightDisplay;
    }
    if (!Number.isFinite(width) || !Number.isFinite(height)) return;
    const qty = Number(row.qty) || 0;
    if (qty <= 0) return;
    let canonicalWidth = width;
    let canonicalHeight = height;
    let canonicalWidthDisplay = widthDisplay;
    let canonicalHeightDisplay = heightDisplay;
    if (orientationAgnostic && canonicalWidth > canonicalHeight){
      [canonicalWidth, canonicalHeight] = [canonicalHeight, canonicalWidth];
      [canonicalWidthDisplay, canonicalHeightDisplay] = [canonicalHeightDisplay, canonicalWidthDisplay];
    }
    let bucket = null;
    for (const candidate of buckets){
      if (Math.abs(candidate.widthRef - canonicalWidth) <= tol && Math.abs(candidate.heightRef - canonicalHeight) <= tol){
        bucket = candidate;
        break;
      }
    }
    if (!bucket){
      bucket = {
        widthValues: [],
        heightValues: [],
        widthDisplays: [],
        heightDisplays: [],
        widthDecimals: 0,
        heightDecimals: 0,
        widthRef: canonicalWidth,
        heightRef: canonicalHeight,
        qty: 0,
        orderIds: new Set(),
        clients: new Set(),
      };
      buckets.push(bucket);
    }
    bucket.widthValues.push(canonicalWidth);
    bucket.heightValues.push(canonicalHeight);
    if (canonicalWidthDisplay) bucket.widthDisplays.push(canonicalWidthDisplay);
    if (canonicalHeightDisplay) bucket.heightDisplays.push(canonicalHeightDisplay);
    bucket.widthDecimals = Math.max(bucket.widthDecimals, decimalPlacesFromDisplay(canonicalWidthDisplay));
    bucket.heightDecimals = Math.max(bucket.heightDecimals, decimalPlacesFromDisplay(canonicalHeightDisplay));
    bucket.qty += qty;
    const orderIdValue = row.orderId != null ? String(row.orderId).trim() : "";
    if (orderIdValue && orderIdValue !== "—") bucket.orderIds.add(orderIdValue);
    const clientValue = row.client != null ? String(row.client).trim() : "";
    if (clientValue && clientValue !== "—") bucket.clients.add(clientValue);
  });
  return buckets.map(bucket => {
    const widthNumeric = calculateRepresentativeNumber(bucket.widthValues, bucket.widthDecimals) ?? bucket.widthRef;
    const heightNumeric = calculateRepresentativeNumber(bucket.heightValues, bucket.heightDecimals) ?? bucket.heightRef;
    const widthDisplay = selectRepresentativeDisplay(bucket.widthDisplays, widthNumeric, bucket.widthDecimals);
    const heightDisplay = selectRepresentativeDisplay(bucket.heightDisplays, heightNumeric, bucket.heightDecimals);
    return {
      dimension: `${widthDisplay} × ${heightDisplay}`,
      qty: bucket.qty,
      orders: bucket.orderIds.size,
      clients: bucket.clients.size,
      repeatScore: Number((bucket.qty * (1 + (bucket.orderIds.size / 5) + (bucket.clients.size / 5))).toFixed(2)),
      orderSpread: bucket.orderIds.size,
      clientSpread: bucket.clients.size,
      batchingOpportunity: bucket.orderIds.size >= 3 && bucket.clients.size >= 2 && bucket.qty >= 8,
      sortQty: bucket.qty,
    };
  }).sort((a, b)=>{
    if (b.sortQty !== a.sortQty) return b.sortQty - a.sortQty;
    return a.dimension.localeCompare(b.dimension);
  }).slice(0, 20);
}

function formatPercent(value, decimals = 1){
  const num = Number(value);
  if (!Number.isFinite(num)) return "—";
  return `${formatNumber(num, { decimals })}%`;
}

function formatSignedPercent(value, decimals = 1){
  const num = Number(value);
  if (!Number.isFinite(num)) return "—";
  const sign = num > 0 ? "+" : "";
  return `${sign}${formatNumber(num, { decimals })}%`;
}

function computePercentDelta(currentValue, previousValue){
  const current = Number(currentValue) || 0;
  const previous = Number(previousValue) || 0;
  if (previous === 0){
    if (current === 0) return 0;
    return null;
  }
  return ((current - previous) / previous) * 100;
}

function toDateInputValue(date){
  if (!(date instanceof Date) || Number.isNaN(date.getTime())) return "";
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const day = String(date.getDate()).padStart(2, "0");
  return `${year}-${month}-${day}`;
}

function parseDateInput(value, endOfDay = false){
  if (!value) return null;
  const date = new Date(`${value}T00:00:00`);
  if (Number.isNaN(date.getTime())) return null;
  if (endOfDay){
    date.setHours(23, 59, 59, 999);
  }
  return date;
}

function dateKeyFromDate(date){
  if (!(date instanceof Date) || Number.isNaN(date.getTime())) return "";
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const day = String(date.getDate()).padStart(2, "0");
  return `${year}-${month}-${day}`;
}

function formatDateLabel(dateKey){
  if (!dateKey) return "—";
  const date = new Date(`${dateKey}T00:00:00`);
  if (Number.isNaN(date.getTime())) return dateKey;
  return date.toLocaleDateString(undefined, { month: "short", day: "numeric" });
}

function compareStrings(a, b){
  return String(a || "").localeCompare(String(b || ""), undefined, { sensitivity: "base" });
}

function sortByConfig(rows, sortConfig){
  const list = Array.isArray(rows) ? rows.slice() : [];
  const rawKey = sortConfig?.key || "";
  const keyMap = {
    area: "area_m2",
    share: "share_area_pct",
    growth: "growth_pct",
  };
  const key = keyMap[rawKey] || rawKey;
  const direction = sortConfig?.direction === "asc" ? 1 : -1;
  if (!key) return list;
  return list.sort((a, b)=>{
    const left = a?.[key];
    const right = b?.[key];
    if (typeof left === "string" || typeof right === "string"){
      return compareStrings(left, right) * direction;
    }
    const leftNum = Number(left);
    const rightNum = Number(right);
    const safeLeft = Number.isFinite(leftNum) ? leftNum : -Infinity;
    const safeRight = Number.isFinite(rightNum) ? rightNum : -Infinity;
    if (safeLeft === safeRight){
      const fallbackKey = key === "client" ? "client" : key === "type" ? "type" : "dimension";
      return compareStrings(a?.[fallbackKey], b?.[fallbackKey]);
    }
    return (safeLeft - safeRight) * direction;
  });
}

function toggleSortState(tableName, key){
  const target = analysisState.sort?.[tableName];
  if (!target) return;
  if (target.key === key){
    target.direction = target.direction === "desc" ? "asc" : "desc";
  }else{
    const textKeys = new Set(["client", "type", "dimension"]);
    target.key = key;
    target.direction = textKeys.has(key) ? "asc" : "desc";
  }
}

function renderSortIndicators(){
  const allButtons = document.querySelectorAll("#tabAnalysis thead button[data-table][data-sort]");
  allButtons.forEach(button => {
    const tableName = button.dataset.table;
    const sortKey = button.dataset.sort;
    const state = analysisState.sort?.[tableName];
    const baseLabel = button.textContent.replace(/\s*[↑↓]\s*$/, "");
    if (state && state.key === sortKey){
      button.classList.add("active");
      button.textContent = `${baseLabel} ${state.direction === "desc" ? "↓" : "↑"}`;
    }else{
      button.classList.remove("active");
      button.textContent = baseLabel;
    }
  });
}

function setAnalysisControlDefaults(orders){
  if (!Array.isArray(orders) || !orders.length) return;
  if (analysisState.controls.startDate && analysisState.controls.endDate){
    return;
  }
  const datedOrders = orders
    .map(order => extractOrderDate(order))
    .filter(date => date instanceof Date && !Number.isNaN(date.getTime()))
    .sort((a, b)=> a.getTime() - b.getTime());
  if (!datedOrders.length) return;
  const latest = datedOrders[datedOrders.length - 1];
  const start = new Date(latest.getTime());
  start.setDate(start.getDate() - 29);
  analysisState.controls.startDate = toDateInputValue(start);
  analysisState.controls.endDate = toDateInputValue(latest);
  if (analysisDateStartInput) analysisDateStartInput.value = analysisState.controls.startDate;
  if (analysisDateEndInput) analysisDateEndInput.value = analysisState.controls.endDate;
}

function populateAnalysisFilterOptions(orders){
  const currentClient = analysisState.controls.client || "";
  const currentType = analysisState.controls.glassType || "";
  const clients = new Set();
  const types = new Set();
  (orders || []).forEach(order => {
    const client = (order?.client || "—").trim() || "—";
    clients.add(client);
    if (Array.isArray(order?.rows)){
      order.rows.forEach(row => {
        const type = (row?.type || "—").trim() || "—";
        types.add(type);
      });
    }
  });
  if (analysisClientFilter){
    const options = ['<option value="">All clients</option>']
      .concat(Array.from(clients).sort(compareStrings).map(client => `<option value="${escapeHtml(client)}">${escapeHtml(client)}</option>`));
    analysisClientFilter.innerHTML = options.join("");
    analysisClientFilter.value = clients.has(currentClient) ? currentClient : "";
    analysisState.controls.client = analysisClientFilter.value || "";
  }
  if (analysisTypeFilter){
    const options = ['<option value="">All glass types</option>']
      .concat(Array.from(types).sort(compareStrings).map(type => `<option value="${escapeHtml(type)}">${escapeHtml(type)}</option>`));
    analysisTypeFilter.innerHTML = options.join("");
    analysisTypeFilter.value = types.has(currentType) ? currentType : "";
    analysisState.controls.glassType = analysisTypeFilter.value || "";
  }
}

function populateAnalysisFilterOptionsFromSnapshot(availableFilters){
  const clients = Array.isArray(availableFilters?.clients) ? availableFilters.clients : [];
  const types = Array.isArray(availableFilters?.glassTypes) ? availableFilters.glassTypes : [];
  if (analysisClientFilter && clients.length){
    const selected = analysisState.controls.client || "";
    const options = ['<option value="">All clients</option>']
      .concat(clients.slice().sort(compareStrings).map(client => `<option value="${escapeHtml(client)}">${escapeHtml(client)}</option>`));
    analysisClientFilter.innerHTML = options.join("");
    analysisClientFilter.value = clients.includes(selected) ? selected : "";
    analysisState.controls.client = analysisClientFilter.value || "";
  }
  if (analysisTypeFilter && types.length){
    const selected = analysisState.controls.glassType || "";
    const options = ['<option value="">All glass types</option>']
      .concat(types.slice().sort(compareStrings).map(type => `<option value="${escapeHtml(type)}">${escapeHtml(type)}</option>`));
    analysisTypeFilter.innerHTML = options.join("");
    analysisTypeFilter.value = types.includes(selected) ? selected : "";
    analysisState.controls.glassType = analysisTypeFilter.value || "";
  }
}

function readAnalysisSettings(){
  let tolerance = analysisToleranceInput ? Number(analysisToleranceInput.value) : analysisState.tolerance;
  tolerance = clampNumber(tolerance, 0, 5, analysisState.tolerance || 1);
  if (analysisToleranceInput){
    analysisToleranceInput.value = String(tolerance);
  }
  const orientationAgnostic = analysisOrientationToggle ? !!analysisOrientationToggle.checked : true;
  analysisState.tolerance = tolerance;
  analysisState.orientationAgnostic = orientationAgnostic;
  if (analysisDateStartInput){
    analysisState.controls.startDate = (analysisDateStartInput.value || "").trim();
  }
  if (analysisDateEndInput){
    analysisState.controls.endDate = (analysisDateEndInput.value || "").trim();
  }
  if (analysisCompareToggle){
    analysisState.controls.comparePrevious = !!analysisCompareToggle.checked;
  }
  if (analysisClientFilter){
    analysisState.controls.client = (analysisClientFilter.value || "").trim();
  }
  if (analysisTypeFilter){
    analysisState.controls.glassType = (analysisTypeFilter.value || "").trim();
  }
  if (
    analysisState.controls.startDate &&
    analysisState.controls.endDate &&
    analysisState.controls.startDate > analysisState.controls.endDate
  ){
    const tmp = analysisState.controls.startDate;
    analysisState.controls.startDate = analysisState.controls.endDate;
    analysisState.controls.endDate = tmp;
    if (analysisDateStartInput) analysisDateStartInput.value = analysisState.controls.startDate;
    if (analysisDateEndInput) analysisDateEndInput.value = analysisState.controls.endDate;
  }
  return {
    tolerance,
    orientationAgnostic,
    startDate: analysisState.controls.startDate,
    endDate: analysisState.controls.endDate,
    comparePrevious: analysisState.controls.comparePrevious,
    client: analysisState.controls.client,
    glassType: analysisState.controls.glassType,
    allTime: analysisState.controls.allTime,
  };
}

function derivePreviousPeriod(startDate, endDate){
  if (!startDate || !endDate) return null;
  const start = parseDateInput(startDate, false);
  const end = parseDateInput(endDate, true);
  if (!start || !end) return null;
  const rangeMs = end.getTime() - start.getTime();
  if (rangeMs < 0) return null;
  const prevEnd = new Date(start.getTime() - 1);
  const prevStart = new Date(prevEnd.getTime() - rangeMs);
  return {
    start: toDateInputValue(prevStart),
    end: toDateInputValue(prevEnd),
  };
}

function orderMatchesDateRange(order, startDate, endDate){
  if (!startDate && !endDate) return true;
  const timestamp = getOrderTimestampForAnalysis(order);
  if (!timestamp) return false;
  const start = startDate ? parseDateInput(startDate, false) : null;
  const end = endDate ? parseDateInput(endDate, true) : null;
  if (start && timestamp < start.getTime()) return false;
  if (end && timestamp > end.getTime()) return false;
  return true;
}

function buildOrderRecord(order, glassTypeFilter){
  if (!order || !Array.isArray(order.rows)) return null;
  const orderId = getOrderLabel(order);
  const client = (order.client || "—").trim() || "—";
  const filteredRows = [];
  let units = 0;
  let area = 0;
  const typeSet = new Set();
  order.rows.forEach(row => {
    if (!row) return;
    const type = (row.type || "—").trim() || "—";
    if (glassTypeFilter && type !== glassTypeFilter) return;
    const qty = Number(row.qty) || 0;
    const rowArea = computeRowArea(row);
    units += qty;
    area += rowArea;
    typeSet.add(type);
    filteredRows.push({
      ...row,
      type,
      qty,
      orderId: row.orderId || orderId,
      client: row.client || client,
      area_m2: rowArea,
    });
  });
  if (!filteredRows.length){
    return null;
  }
  return {
    orderId,
    client,
    created_at: order.created_at || order.createdAt || null,
    units,
    area_m2: area,
    rows: filteredRows,
    lineCount: filteredRows.length,
    distinctTypes: typeSet,
  };
}

function buildPeriodRecords(orders, settings, period){
  const records = [];
  (orders || []).forEach(order => {
    if (!order) return;
    if (!orderMatchesDateRange(order, period?.start || "", period?.end || "")){
      return;
    }
    if (settings.client){
      const client = (order.client || "—").trim() || "—";
      if (client !== settings.client) return;
    }
    const record = buildOrderRecord(order, settings.glassType);
    if (!record) return;
    records.push(record);
  });
  return records;
}

function computeDailySeries(records, period){
  const map = new Map();
  function ensureBucket(dateKey){
    if (!map.has(dateKey)){
      map.set(dateKey, { date: dateKey, orders: 0, units: 0, area_m2: 0 });
    }
    return map.get(dateKey);
  }
  records.forEach(record => {
    const date = extractOrderDate(record) || extractOrderDate({ created_at: record.created_at });
    if (!(date instanceof Date) || Number.isNaN(date.getTime())) return;
    const key = dateKeyFromDate(date);
    if (!key) return;
    const bucket = ensureBucket(key);
    bucket.orders += 1;
    bucket.units += Number(record.units) || 0;
    bucket.area_m2 += Number(record.area_m2) || 0;
  });
  const rangeStart = period?.start ? parseDateInput(period.start, false) : null;
  const rangeEnd = period?.end ? parseDateInput(period.end, false) : null;
  if (rangeStart && rangeEnd){
    const cursor = new Date(rangeStart.getTime());
    while (cursor.getTime() <= rangeEnd.getTime()){
      ensureBucket(dateKeyFromDate(cursor));
      cursor.setDate(cursor.getDate() + 1);
    }
  }
  return Array.from(map.values()).sort((a, b)=> compareStrings(a.date, b.date)).map(item => ({
    ...item,
    label: formatDateLabel(item.date),
    avgAreaPerOrder: item.orders ? item.area_m2 / item.orders : 0,
    avgUnitsPerOrder: item.orders ? item.units / item.orders : 0,
  }));
}

function computePeriodAnalytics(records, settings, period){
  const totals = {
    orders: records.length,
    units: 0,
    area_m2: 0,
    avg_area_per_order: 0,
    avg_units_per_order: 0,
    distinct_clients: 0,
    distinct_types: 0,
  };
  const clientMap = new Map();
  const typeMap = new Map();
  const allRows = [];
  const typeSet = new Set();
  const orderContributions = [];
  records.forEach(record => {
    const units = Number(record.units) || 0;
    const area = Number(record.area_m2) || 0;
    totals.units += units;
    totals.area_m2 += area;
    const client = record.client || "—";
    if (!clientMap.has(client)){
      clientMap.set(client, { client, orders: 0, units: 0, area_m2: 0 });
    }
    const clientEntry = clientMap.get(client);
    clientEntry.orders += 1;
    clientEntry.units += units;
    clientEntry.area_m2 += area;
    orderContributions.push({
      order_id: record.orderId,
      client,
      area_m2: area,
      units,
    });
    (record.rows || []).forEach(row => {
      const type = (row.type || "—").trim() || "—";
      typeSet.add(type);
      if (!typeMap.has(type)){
        typeMap.set(type, { type, lines: 0, units: 0, area_m2: 0 });
      }
      const entry = typeMap.get(type);
      entry.lines += 1;
      entry.units += Number(row.qty) || 0;
      entry.area_m2 += Number(row.area_m2) || 0;
      allRows.push({
        ...row,
        orderId: row.orderId || record.orderId,
        client: row.client || record.client,
      });
    });
  });
  totals.avg_area_per_order = totals.orders ? totals.area_m2 / totals.orders : 0;
  totals.avg_units_per_order = totals.orders ? totals.units / totals.orders : 0;
  totals.distinct_clients = clientMap.size;
  totals.distinct_types = typeSet.size;
  const topClients = Array.from(clientMap.values()).map(entry => ({
    ...entry,
    share_area_pct: totals.area_m2 ? (entry.area_m2 / totals.area_m2) * 100 : 0,
  }));
  const topTypes = Array.from(typeMap.values()).map(entry => ({
    ...entry,
    share_area_pct: totals.area_m2 ? (entry.area_m2 / totals.area_m2) * 100 : 0,
  }));
  const dimensions = buildDimensionBuckets(allRows, settings.tolerance, settings.orientationAgnostic);
  const contributionSorted = orderContributions
    .map(entry => ({
      ...entry,
      area_share_pct: totals.area_m2 ? (entry.area_m2 / totals.area_m2) * 100 : 0,
    }))
    .sort((a, b)=> b.area_share_pct - a.area_share_pct);
  const daily = computeDailySeries(records, period);
  return {
    period,
    totals,
    topClients,
    topTypes,
    topDimensions: dimensions,
    orderContributions: contributionSorted,
    daily,
  };
}

function indexBy(rows, key){
  const map = new Map();
  (rows || []).forEach(row => {
    const value = row?.[key];
    if (value == null) return;
    map.set(String(value), row);
  });
  return map;
}

function attachGrowth(rows, previousRows, keyField, metricField){
  const previousMap = indexBy(previousRows, keyField);
  return (rows || []).map(row => {
    const prev = previousMap.get(String(row[keyField]));
    const previousMetric = prev ? Number(prev[metricField]) || 0 : 0;
    const currentMetric = Number(row[metricField]) || 0;
    return {
      ...row,
      growth_pct: computePercentDelta(currentMetric, previousMetric),
    };
  });
}

function buildAnalysisContext(orders, settings){
  const period = { start: settings.startDate || "", end: settings.endDate || "" };
  const currentRecords = buildPeriodRecords(orders, settings, period);
  const current = computePeriodAnalytics(currentRecords, settings, period);
  let previousPeriod = null;
  let previous = null;
  if (settings.comparePrevious && settings.startDate && settings.endDate){
    previousPeriod = derivePreviousPeriod(settings.startDate, settings.endDate);
    if (previousPeriod){
      const previousRecords = buildPeriodRecords(orders, settings, previousPeriod);
      previous = computePeriodAnalytics(previousRecords, settings, previousPeriod);
    }
  }
  current.topClients = attachGrowth(current.topClients, previous?.topClients || [], "client", "area_m2");
  current.topTypes = attachGrowth(current.topTypes, previous?.topTypes || [], "type", "area_m2");
  return {
    period,
    previousPeriod,
    current,
    previous,
    settings: {
      client: settings.client,
      glassType: settings.glassType,
      comparePrevious: settings.comparePrevious,
      tolerance: settings.tolerance,
      orientationAgnostic: settings.orientationAgnostic,
      distributionMetric: analysisState.distributionMetric,
    },
  };
}

function deltaClass(delta){
  if (!Number.isFinite(delta) || delta === 0) return "flat";
  return delta > 0 ? "up" : "down";
}

function makeSparkline(values, color = "#4c5cff"){
  const clean = (values || []).map(v => Number(v) || 0);
  if (!clean.length){
    return '<svg class="analysis-sparkline" viewBox="0 0 70 24"></svg>';
  }
  const min = Math.min(...clean);
  const max = Math.max(...clean);
  const range = max - min || 1;
  const step = clean.length > 1 ? 68 / (clean.length - 1) : 68;
  const points = clean.map((value, index) => {
    const x = 1 + (index * step);
    const y = 22 - (((value - min) / range) * 18);
    return `${x.toFixed(2)},${y.toFixed(2)}`;
  }).join(" ");
  return `<svg class="analysis-sparkline" viewBox="0 0 70 24" preserveAspectRatio="none">
    <polyline fill="none" stroke="${color}" stroke-width="2" points="${points}" />
  </svg>`;
}

function renderAnalysisLoadingState(){
  if (analysisKpisEl){
    analysisKpisEl.innerHTML = Array.from({ length: 6 }, ()=> `
      <article class="analysis-kpi-card analysis-skeleton-card" aria-hidden="true">
        <span class="analysis-skeleton analysis-skeleton-label"></span>
        <span class="analysis-skeleton analysis-skeleton-value"></span>
        <span class="analysis-skeleton analysis-skeleton-line"></span>
      </article>
    `).join("");
  }
  [
    analysisTypeDistributionChartEl,
    analysisOrdersChartEl,
    analysisDimensionsChartEl,
    analysisAreaByTypeChartEl,
    analysisTopClientsBarsEl,
  ].forEach(container => {
    if (container){
      container.innerHTML = '<div class="analysis-chart-loading" aria-label="Loading chart"><span class="analysis-skeleton"></span><span class="analysis-skeleton"></span><span class="analysis-skeleton"></span></div>';
    }
  });
  if (analysisTopClientsBody) analysisTopClientsBody.innerHTML = '<tr><td colspan="6" class="muted">Loading client metrics…</td></tr>';
  if (analysisTopOrdersBody) analysisTopOrdersBody.innerHTML = '<tr><td colspan="3" class="muted">Loading order metrics…</td></tr>';
  if (analysisTopTypesBody) analysisTopTypesBody.innerHTML = '<tr><td colspan="6" class="muted">Loading glass mix…</td></tr>';
  if (analysisDimensionsBody) analysisDimensionsBody.innerHTML = '<tr><td colspan="8" class="muted">Loading dimension patterns…</td></tr>';
  renderInsights([], null, true);
}

function renderAnalysisKpis(context){
  if (!analysisKpisEl) return;
  const current = context?.current?.totals || {
    orders: 0, units: 0, area_m2: 0, avg_area_per_order: 0, avg_units_per_order: 0, distinct_clients: 0, distinct_types: 0,
  };
  const previous = context?.previous?.totals || null;
  const daily = context?.current?.daily || [];
  const topClient = (context?.current?.topClients || [])
    .slice()
    .sort((a, b)=> (Number(b.area_m2) || 0) - (Number(a.area_m2) || 0))[0] || null;
  const cards = [
    {
      key: "orders",
      label: "Total Orders",
      value: formatNumber(current.orders),
      detail: `${formatNumber(current.avg_orders_per_day || 0, { decimals: 1 })} per day`,
      delta: computePercentDelta(current.orders, previous?.orders || 0),
      trend: daily.map(item => item.orders),
    },
    {
      key: "area",
      label: "Total m²",
      value: formatAreaMetric(current.area_m2),
      delta: computePercentDelta(current.area_m2, previous?.area_m2 || 0),
      trend: daily.map(item => item.area_m2),
    },
    {
      key: "units",
      label: "Total Units",
      value: formatNumber(current.units),
      delta: computePercentDelta(current.units, previous?.units || 0),
      trend: daily.map(item => item.units),
    },
    {
      key: "averageArea",
      label: "Avg m² / Order",
      value: formatAreaMetric(current.avg_area_per_order || 0),
      detail: `${formatNumber(current.avg_units_per_order || 0, { decimals: 1 })} units per order`,
      delta: computePercentDelta(current.avg_area_per_order, previous?.avg_area_per_order || 0),
      trend: daily.map(item => item.avgAreaPerOrder),
    },
    {
      key: "clients",
      label: "Active Clients",
      value: formatNumber(current.distinct_clients || 0),
      detail: `${formatNumber(current.distinct_types || 0)} glass types`,
      delta: computePercentDelta(current.distinct_clients, previous?.distinct_clients || 0),
      trend: [],
    },
    {
      key: "topClient",
      label: "Top Client",
      value: topClient ? topClient.client : "—",
      detail: topClient ? `${formatAreaMetric(topClient.area_m2)} | ${formatNumber(topClient.orders)} ${Number(topClient.orders) === 1 ? "order" : "orders"}` : "No client data",
      delta: topClient ? topClient.growth_pct : null,
      trend: topClient ? [0, Number(topClient.area_m2) || 0] : [],
    },
  ];
  const html = cards.map(card => {
    const delta = card.delta;
    const deltaText = previous
      ? (delta == null ? "new / no baseline" : formatSignedPercent(delta))
      : "comparison off";
    const deltaState = deltaClass(delta);
    const trendColor = deltaState === "down" ? "#b91c1c" : deltaState === "up" ? "#0f766e" : "#64748b";
    return `<article class="analysis-kpi-card">
      <div class="analysis-kpi-label">${escapeHtml(card.label)}</div>
      <div class="analysis-kpi-value">${escapeHtml(card.value)}</div>
      ${card.detail ? `<div class="analysis-kpi-detail">${escapeHtml(card.detail)}</div>` : ""}
      <div class="analysis-kpi-footer">
        <span class="analysis-delta ${deltaState}">${escapeHtml(deltaText)}</span>
        ${makeSparkline(card.trend, trendColor)}
      </div>
    </article>`;
  }).join("");
  analysisKpisEl.innerHTML = html;
}

function renderBarList(container, rows, valueKey, labelFormatter){
  if (!container) return;
  const list = Array.isArray(rows) ? rows.slice(0, 8) : [];
  if (!list.length){
    container.innerHTML = '<div class="analysis-chart-empty">No data for selected filters.</div>';
    return;
  }
  const maxValue = Math.max(...list.map(item => Number(item[valueKey]) || 0), 1);
  container.innerHTML = `<div class="analysis-bars">${list.map(item => {
    const value = Number(item[valueKey]) || 0;
    const widthPct = Math.max(3, (value / maxValue) * 100);
    const displayValue = labelFormatter(item);
    return `<div class="analysis-bar-row" title="${escapeHtml(`${item.label}: ${displayValue}`)}">
      <span class="analysis-bar-label">${escapeHtml(item.label)}</span>
      <span class="analysis-bar-track" aria-hidden="true"><span class="analysis-bar-fill" style="width:${widthPct}%"></span></span>
      <span class="analysis-bar-value">${escapeHtml(displayValue)}</span>
    </div>`;
  }).join("")}</div>`;
}

function renderLineChart(container, series, key, stroke){
  if (!container) return;
  const values = (series || []).map(item => Number(item[key]) || 0);
  if (!values.length){
    container.innerHTML = '<div class="analysis-chart-empty">No data for selected filters.</div>';
    return;
  }
  const width = 560;
  const height = 160;
  const padX = 20;
  const padY = 16;
  const innerWidth = width - (padX * 2);
  const innerHeight = height - (padY * 2);
  const max = Math.max(...values, 0);
  const min = Math.min(...values, 0);
  const range = max - min || 1;
  const step = values.length > 1 ? innerWidth / (values.length - 1) : innerWidth;
  const points = values.map((value, index) => {
    const x = padX + (index * step);
    const y = padY + innerHeight - (((value - min) / range) * innerHeight);
    return `${x.toFixed(2)},${y.toFixed(2)}`;
  }).join(" ");
  const baselineY = padY + innerHeight;
  container.innerHTML = `<svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-hidden="true">
    <line x1="${padX}" y1="${baselineY}" x2="${width - padX}" y2="${baselineY}" stroke="#d9dce5" stroke-width="1"></line>
    <polyline fill="none" stroke="${stroke}" stroke-width="2.5" points="${points}"></polyline>
  </svg>`;
}

function renderBarChart(container, series, key, fillColor){
  if (!container) return;
  const values = (series || []).map(item => Number(item[key]) || 0);
  if (!values.length){
    container.innerHTML = '<div class="analysis-chart-empty">No data for selected filters.</div>';
    return;
  }
  const width = 560;
  const height = 160;
  const padX = 20;
  const padY = 16;
  const innerWidth = width - (padX * 2);
  const innerHeight = height - (padY * 2);
  const max = Math.max(...values, 1);
  const barWidth = Math.max(3, innerWidth / Math.max(values.length, 1) - 2);
  let cursorX = padX;
  const bars = values.map(value => {
    const h = (value / max) * innerHeight;
    const y = padY + (innerHeight - h);
    const markup = `<rect x="${cursorX.toFixed(2)}" y="${y.toFixed(2)}" width="${barWidth.toFixed(2)}" height="${h.toFixed(2)}" rx="2" ry="2" fill="${fillColor}"></rect>`;
    cursorX += barWidth + 2;
    return markup;
  }).join("");
  const baselineY = padY + innerHeight;
  container.innerHTML = `<svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-hidden="true">
    <line x1="${padX}" y1="${baselineY}" x2="${width - padX}" y2="${baselineY}" stroke="#d9dce5" stroke-width="1"></line>
    ${bars}
  </svg>`;
}

function renderTopClients(rows){
  if (!analysisTopClientsBody) return;
  const sorted = sortByConfig(rows, analysisState.sort.clients);
  if (!sorted.length){
    renderNoDataRow(analysisTopClientsBody, 6);
    return;
  }
  analysisTopClientsBody.innerHTML = sorted.map(entry => `<tr>
    <td>${escapeHtml(entry.client)}</td>
    <td>${formatNumber(entry.orders)}</td>
    <td>${formatNumber(entry.units)}</td>
    <td>${formatAreaMetric(entry.area_m2)}</td>
    <td>${formatPercent(entry.share_area_pct)}</td>
    <td class="${deltaClass(entry.growth_pct)}">${entry.growth_pct == null ? "—" : formatSignedPercent(entry.growth_pct)}</td>
  </tr>`).join("");
}

function renderTopTypes(rows){
  if (!analysisTopTypesBody) return;
  const sorted = sortByConfig(rows, analysisState.sort.types);
  if (!sorted.length){
    renderNoDataRow(analysisTopTypesBody, 6);
    return;
  }
  analysisTopTypesBody.innerHTML = sorted.map(entry => `<tr>
    <td>${escapeHtml(entry.type)}</td>
    <td>${formatNumber(entry.lines)}</td>
    <td>${formatNumber(entry.units)}</td>
    <td>${formatAreaMetric(entry.area_m2)}</td>
    <td>${formatPercent(entry.share_area_pct)}</td>
    <td class="${deltaClass(entry.growth_pct)}">${entry.growth_pct == null ? "—" : formatSignedPercent(entry.growth_pct)}</td>
  </tr>`).join("");
}

function renderTopDimensions(rows){
  if (!analysisDimensionsBody) return;
  const normalized = (rows || []).map(entry => ({
    ...entry,
    batching: entry.batchingOpportunity ? 1 : 0,
  }));
  const sorted = sortByConfig(normalized, analysisState.sort.dimensions);
  if (!sorted.length){
    renderNoDataRow(analysisDimensionsBody, 8);
    return;
  }
  analysisDimensionsBody.innerHTML = sorted.map(entry => `<tr>
    <td>${escapeHtml(entry.dimension)}</td>
    <td>${formatNumber(entry.qty)}</td>
    <td>${formatNumber(entry.orders)}</td>
    <td>${formatNumber(entry.clients)}</td>
    <td>${formatNumber(entry.repeatScore, { decimals: 2 })}</td>
    <td>${formatNumber(entry.orderSpread)}</td>
    <td>${formatNumber(entry.clientSpread)}</td>
    <td>${entry.batchingOpportunity ? '<span class="pill">Yes</span>' : '<span class="muted">No</span>'}</td>
  </tr>`).join("");
}

const ANALYSIS_CHART_COLORS = ["#2557d6", "#14a3b8", "#16835f", "#d48a1f", "#7c5cff", "#c03636", "#64748b", "#93a4b8"];

function renderPieChart(container, rows, valueKey, labelFormatter){
  if (!container) return;
  const source = (Array.isArray(rows) ? rows : [])
    .map((item, index) => ({
      ...item,
      value: Number(item[valueKey]) || 0,
      color: ANALYSIS_CHART_COLORS[index % ANALYSIS_CHART_COLORS.length],
    }))
    .filter(item => item.value > 0)
    .slice(0, 8);
  const total = source.reduce((sum, item)=> sum + item.value, 0);
  if (!source.length || total <= 0){
    container.innerHTML = '<div class="analysis-chart-empty">No data for selected filters.</div>';
    return;
  }
  let cursor = 0;
  const gradient = source.map(item => {
    const start = cursor;
    const end = cursor + ((item.value / total) * 100);
    cursor = end;
    return `${item.color} ${start.toFixed(2)}% ${end.toFixed(2)}%`;
  }).join(", ");
  const leader = source[0];
  container.innerHTML = `<div class="analysis-pie-chart">
    <div class="analysis-donut" style="background:conic-gradient(${gradient})">
      <div class="analysis-donut-center">
        <strong>${escapeHtml(formatPercent((leader.value / total) * 100))}</strong>
        <span>${escapeHtml(leader.label)}</span>
      </div>
    </div>
    <div class="analysis-legend">
      ${source.map(item => {
        const pct = (item.value / total) * 100;
        return `<div class="analysis-legend-row">
          <span class="analysis-legend-swatch" style="background:${item.color}"></span>
          <span class="analysis-legend-label">${escapeHtml(item.label)}</span>
          <span class="analysis-legend-value">${escapeHtml(formatPercent(pct))} · ${escapeHtml(labelFormatter(item))}</span>
        </div>`;
      }).join("")}
    </div>
  </div>`;
}

function renderDailyOrderBars(container, series){
  if (!container) return;
  const rows = Array.isArray(series) ? series : [];
  const values = rows.map(item => Number(item.orders) || 0);
  if (!values.length){
    container.innerHTML = '<div class="analysis-chart-empty">No data for selected filters.</div>';
    return;
  }
  const width = 620;
  const height = 190;
  const padLeft = 34;
  const padRight = 14;
  const padTop = 18;
  const padBottom = 34;
  const innerWidth = width - padLeft - padRight;
  const innerHeight = height - padTop - padBottom;
  const max = Math.max(...values, 1);
  const slot = innerWidth / Math.max(values.length, 1);
  const barWidth = Math.max(4, Math.min(28, slot * 0.62));
  const bars = values.map((value, index) => {
    const x = padLeft + (index * slot) + ((slot - barWidth) / 2);
    const h = Math.max(value > 0 ? 3 : 0, (value / max) * innerHeight);
    const y = padTop + innerHeight - h;
    const label = rows[index]?.label || rows[index]?.date || "";
    return `<rect x="${x.toFixed(2)}" y="${y.toFixed(2)}" width="${barWidth.toFixed(2)}" height="${h.toFixed(2)}" rx="4" fill="#2557d6"><title>${escapeHtml(`${label}: ${formatNumber(value)} orders`)}</title></rect>`;
  }).join("");
  const firstLabel = rows[0]?.label || rows[0]?.date || "";
  const lastLabel = rows[rows.length - 1]?.label || rows[rows.length - 1]?.date || "";
  const half = max / 2;
  const halfY = padTop + (innerHeight / 2);
  container.innerHTML = `<svg class="analysis-axis-chart" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="Orders per day">
    <line x1="${padLeft}" y1="${padTop}" x2="${width - padRight}" y2="${padTop}" stroke="#e7ebf2" stroke-width="1"></line>
    <line x1="${padLeft}" y1="${halfY}" x2="${width - padRight}" y2="${halfY}" stroke="#e7ebf2" stroke-width="1"></line>
    <line x1="${padLeft}" y1="${padTop + innerHeight}" x2="${width - padRight}" y2="${padTop + innerHeight}" stroke="#d9e0eb" stroke-width="1"></line>
    <line x1="${padLeft}" y1="${padTop}" x2="${padLeft}" y2="${padTop + innerHeight}" stroke="#d9e0eb" stroke-width="1"></line>
    <text x="${padLeft}" y="13" fill="#667085" font-size="12">${escapeHtml(String(max))}</text>
    <text x="${padLeft + 4}" y="${halfY - 4}" fill="#667085" font-size="11">${escapeHtml(formatNumber(half, { decimals: Number.isInteger(half) ? 0 : 1 }))}</text>
    <text x="${padLeft + 4}" y="${padTop + innerHeight - 4}" fill="#667085" font-size="11">0</text>
    ${bars}
    <text x="${padLeft}" y="${height - 10}" fill="#667085" font-size="12">${escapeHtml(firstLabel)}</text>
    <text x="${width - padRight}" y="${height - 10}" text-anchor="end" fill="#667085" font-size="12">${escapeHtml(lastLabel)}</text>
  </svg>`;
}

function renderTopOrders(context){
  if (!analysisTopOrdersBody) return;
  const rows = (context?.current?.orderContributions || [])
    .slice()
    .sort((a, b)=> (Number(b.area_m2) || 0) - (Number(a.area_m2) || 0))
    .slice(0, 10);
  if (!rows.length){
    renderNoDataRow(analysisTopOrdersBody, 3);
    return;
  }
  analysisTopOrdersBody.innerHTML = rows.map(entry => `<tr>
    <td>
      <strong>${escapeHtml(entry.client || "—")}</strong>
      <div class="muted small">${escapeHtml(entry.order_id || "—")}</div>
    </td>
    <td>${formatAreaMetric(entry.area_m2)}</td>
    <td>${formatNumber(entry.units)}</td>
  </tr>`).join("");
}

function renderCharts(context){
  const current = context?.current || {};
  const metric = analysisState.distributionMetric || "area";
  const topTypes = (current.topTypes || []).slice(0, 10).map(item => ({
    label: item.type,
    area: item.area_m2,
    units: item.units,
    lines: item.lines,
  }));
  const metricKey = metric === "units" ? "units" : metric === "lines" ? "lines" : "area";
  const metricLabel = metric === "units" ? (item => formatNumber(item.units)) : metric === "lines" ? (item => formatNumber(item.lines)) : (item => formatAreaMetric(item.area));
  renderPieChart(analysisTypeDistributionChartEl, topTypes, metricKey, metricLabel);
  renderDailyOrderBars(analysisOrdersChartEl, current.daily || []);
  renderBarList(
    analysisDimensionsChartEl,
    (current.topDimensions || []).slice(0, 8).map(item => ({
      label: item.dimension,
      qty: item.qty,
    })),
    "qty",
    item => `${formatNumber(item.qty)} pcs`
  );
  renderBarList(analysisAreaByTypeChartEl, topTypes, "area", item => formatAreaMetric(item.area));
  renderBarList(
    analysisTopClientsBarsEl,
    (current.topClients || []).slice(0, 8).map(item => ({
      label: item.client,
      area_m2: item.area_m2,
      share_area_pct: item.share_area_pct,
    })),
    "area_m2",
    item => formatPercent(item.share_area_pct)
  );
}

function renderPeriodMeta(context){
  if (!analysisPeriodMetaEl) return;
  const period = context?.period || {};
  const previous = context?.previousPeriod || null;
  const rangeText = (period.start && period.end)
    ? `${period.start} – ${period.end}`
    : "All available dates";
  const filterParts = [];
  if (analysisState.controls.client) filterParts.push(`client: ${analysisState.controls.client}`);
  if (analysisState.controls.glassType) filterParts.push(`glass type: ${analysisState.controls.glassType}`);
  const filterText = filterParts.length ? ` | Filters: ${filterParts.join(", ")}` : "";
  const compareText = previous
    ? ` | Comparison: ${previous.start} – ${previous.end}`
    : (analysisState.controls.comparePrevious ? " | Comparison unavailable (set date range)." : " | Comparison off");
  analysisPeriodMetaEl.textContent = `Period: ${rangeText}${compareText}${filterText}`;
}

function formatInsightMetaValue(value){
  if (value == null) return "—";
  if (typeof value === "object"){
    try{
      return JSON.stringify(value);
    }catch{
      return String(value);
    }
  }
  return String(value);
}

function renderInsights(signals, context, loading = false){
  if (!analysisInsightsEl) return;
  if (loading){
    analysisInsightsEl.innerHTML = '<div class="analysis-insight-card"><p class="muted small" style="margin:0">Computing deterministic insights…</p></div>';
    return;
  }
  const list = Array.isArray(signals) ? signals : [];
  if (!list.length){
    analysisInsightsEl.innerHTML = '<div class="analysis-insight-card"><p class="muted small" style="margin:0">No deterministic alerts for the selected filters.</p></div>';
    return;
  }
  analysisInsightsEl.innerHTML = list.map(signal => {
    const severity = (signal.severity || "low").toLowerCase();
    const meta = signal.meta && typeof signal.meta === "object" ? signal.meta : {};
    const metaLines = Object.entries(meta)
      .slice(0, 5)
      .map(([key, value]) => `<div><strong>${escapeHtml(key)}:</strong> ${escapeHtml(formatInsightMetaValue(value))}</div>`)
      .join("");
    return `<article class="analysis-insight-card ${escapeHtml(severity)}">
      <div class="analysis-insight-meta">
        <span>${escapeHtml((signal.type || "signal").replace(/_/g, " "))}</span>
        <span>${escapeHtml(severity)}</span>
      </div>
      <h4 class="analysis-insight-title">${escapeHtml(signal.title || "Insight")}</h4>
      <p class="analysis-insight-explain">${escapeHtml(signal.explanation || "")}</p>
      <details>
        <summary>Why this insight</summary>
        <div class="muted small" style="margin-top:6px">${metaLines || "No metadata."}</div>
      </details>
    </article>`;
  }).join("");
  updateWorkspaceSelectionUI();
}

function buildSignalsSummary(context){
  const mappedDimensions = (context?.current?.topDimensions || []).map(entry => ({
    dimension: entry.dimension,
    qty: entry.qty,
    orders: entry.orders,
    clients: entry.clients,
    repeat_score: entry.repeatScore,
    order_spread: entry.orderSpread,
    client_spread: entry.clientSpread,
    batching_opportunity: !!entry.batchingOpportunity,
  }));
  const mappedPrevDimensions = (context?.previous?.topDimensions || []).map(entry => ({
    dimension: entry.dimension,
    qty: entry.qty,
    orders: entry.orders,
    clients: entry.clients,
    repeat_score: entry.repeatScore,
    order_spread: entry.orderSpread,
    client_spread: entry.clientSpread,
    batching_opportunity: !!entry.batchingOpportunity,
  }));
  return {
    compare_enabled: !!context?.previous,
    period: context?.period || {},
    previous_period: context?.previousPeriod || {},
    current: {
      totals: context?.current?.totals || {},
      top_clients: context?.current?.topClients || [],
      top_types: context?.current?.topTypes || [],
      top_dimensions: mappedDimensions,
      order_contributions: context?.current?.orderContributions || [],
      daily: context?.current?.daily || [],
    },
    previous: context?.previous ? {
      totals: context.previous.totals || {},
      top_clients: context.previous.topClients || [],
      top_types: context.previous.topTypes || [],
      top_dimensions: mappedPrevDimensions,
      order_contributions: context.previous.orderContributions || [],
      daily: context.previous.daily || [],
    } : {},
    meta: {
      thresholds: {
        growth_delta_medium_pct: 10,
        growth_delta_high_pct: 20,
        client_share_medium_pct: 35,
        client_share_high_pct: 45,
        mix_shift_medium_pct: 10,
        mix_shift_high_pct: 16,
      },
    },
  };
}

function buildAdvancedAnalysisDataset(context, insights = analysisState.insights || []){
  return {
    scope: "filtered-period",
    meta: {
      generated_at: new Date().toISOString(),
      period: context?.period || {},
      previous_period: context?.previousPeriod || {},
      filters: {
        client: analysisState.controls.client || "",
        glass_type: analysisState.controls.glassType || "",
      },
    },
    aggregates: {
      current: context?.current || {},
      previous: context?.previous || null,
      compare_enabled: !!context?.previous,
      insights: insights || [],
    },
    orders: [],
    processing_orders: [],
  };
}

function buildAnalysisSnapshotPayload(filters, context, insights, computedAt, cacheStatus = "fresh"){
  const availableClients = Array.from(new Set((analysisState.orders || []).map(order => {
    const client = (order?.client || "—").trim() || "—";
    return client;
  }))).sort(compareStrings);
  const availableTypes = (() => {
    const typeSet = new Set();
    (analysisState.orders || []).forEach(order => {
      if (!order || !Array.isArray(order.rows)) return;
      order.rows.forEach(row => {
        const type = (row?.type || "—").trim() || "—";
        typeSet.add(type);
      });
    });
    return Array.from(typeSet).sort(compareStrings);
  })();
  return {
    filters: normalizeSnapshotFilters(filters),
    period: context?.period || {},
    previousPeriod: context?.previousPeriod || null,
    current: context?.current || {},
    previous: context?.previous || null,
    kpis: context?.current?.totals || {},
    charts: {
      daily: context?.current?.daily || [],
      topClients: context?.current?.topClients || [],
      topGlassTypes: context?.current?.topTypes || [],
    },
    topClients: context?.current?.topClients || [],
    topGlassTypes: context?.current?.topTypes || [],
    topDimensions: context?.current?.topDimensions || [],
    insights: Array.isArray(insights) ? insights : [],
    availableFilters: {
      clients: availableClients,
      glassTypes: availableTypes,
    },
    computedAt: computedAt || new Date().toISOString(),
    cacheStatus,
  };
}

function applyAnalysisSnapshotPayload(payload, options = {}){
  if (!payload || typeof payload !== "object"){
    return;
  }
  const payloadFilters = payload.filters && typeof payload.filters === "object" ? payload.filters : {};
  analysisState.controls.startDate = String(payloadFilters.startDate || "");
  analysisState.controls.endDate = String(payloadFilters.endDate || "");
  analysisState.controls.comparePrevious = payloadFilters.compareToPrevious !== false;
  analysisState.controls.client = String(payloadFilters.client || "");
  analysisState.controls.glassType = String(payloadFilters.glassType || "");
  analysisState.controls.allTime = !!payloadFilters.allTime;
  analysisState.tolerance = Number(payloadFilters.tolerance ?? analysisState.tolerance) || 0;
  analysisState.orientationAgnostic = payloadFilters.orientationAgnostic !== false;
  if (analysisDateStartInput) analysisDateStartInput.value = analysisState.controls.startDate;
  if (analysisDateEndInput) analysisDateEndInput.value = analysisState.controls.endDate;
  if (analysisCompareToggle) analysisCompareToggle.checked = analysisState.controls.comparePrevious;
  if (analysisToleranceInput) analysisToleranceInput.value = String(analysisState.tolerance);
  if (analysisOrientationToggle) analysisOrientationToggle.checked = analysisState.orientationAgnostic;
  populateAnalysisFilterOptionsFromSnapshot(payload.availableFilters);
  const context = {
    period: payload.period || {},
    previousPeriod: payload.previousPeriod || null,
    current: payload.current || { totals: payload.kpis || {} },
    previous: payload.previous || null,
  };
  analysisState.currentPeriod = context.current;
  analysisState.previousPeriod = context.previous;
  analysisState.metrics = context.current?.totals || payload.kpis || null;
  analysisState.topClients = Array.isArray(payload.topClients) ? payload.topClients.slice() : [];
  analysisState.topTypes = Array.isArray(payload.topGlassTypes) ? payload.topGlassTypes.slice() : [];
  analysisState.topDimensions = Array.isArray(payload.topDimensions) ? payload.topDimensions.slice() : [];
  analysisState.insights = Array.isArray(payload.insights) ? payload.insights.slice() : [];
  analysisState.currentSnapshotComputedAt = payload.computedAt || "";
  analysisState.lastSnapshot = payload;
  analysisState.hasRenderedSnapshot = true;
  analysisState.allOrdersDirty = false;
  analysisState.advancedDataset = buildAdvancedAnalysisDataset(context, analysisState.insights);
  renderPeriodMeta(context);
  renderAnalysisKpis(context);
  renderTopClients(analysisState.topClients);
  renderTopTypes(analysisState.topTypes);
  renderTopDimensions(analysisState.topDimensions);
  renderCharts(context);
  renderTopOrders(context);
  renderSortIndicators();
  renderInsights(analysisState.insights, context, false);
  setAnalysisFreshnessLabel(payload.computedAt || "", !!options.refreshing);
  updateAnalysisRangePresetState();
  if (analysisExportBtn) analysisExportBtn.disabled = false;
}

async function ensureAnalysisOrdersLoaded(settings, options = {}){
  const forceReload = !!options.forceReload;
  const alreadyLoaded = analysisState.dataset && Array.isArray(analysisState.orders) && analysisState.orders.length > 0;
  if (alreadyLoaded && !forceReload && !analysisState.allOrdersDirty){
    return;
  }
  await ensureHistoryLoaded();
  const dataset = await buildAllTimeAnalysisDataset(settings);
  analysisState.dataset = dataset;
  analysisState.orders = mergeOrdersForAnalysis(dataset);
  populateAnalysisFilterOptions(analysisState.orders);
  setAnalysisControlDefaults(analysisState.orders);
  analysisState.allOrdersDirty = false;
}

function setAnalysisRefreshingState(isRefreshing, { manual = false } = {}){
  analysisState.revalidating = !!isRefreshing;
  setAnalysisFreshnessLabel(analysisState.currentSnapshotComputedAt || "", analysisState.revalidating);
  if (analysisRefreshBtn){
    analysisRefreshBtn.disabled = !!manual && !!isRefreshing;
    analysisRefreshBtn.textContent = (manual && isRefreshing) ? "Loading…" : "Apply filters";
  }
}

async function computeAnalysisSnapshot(settings){
  const filters = normalizeSnapshotFilters(settings);
  const runId = ++analysisState.renderRunId;
  const payload = await fetchAnalysisSummary(settings);
  if (runId !== analysisState.renderRunId){
    return null;
  }
  const key = buildSnapshotCacheKey(filters);
  saveAnalysisSnapshot({
    version: ANALYSIS_SNAPSHOT_VERSION,
    key,
    filters,
    payload,
  });
  return payload;
}

async function refreshAnalysis(options = {}){
  const opts = typeof options === "object" && options != null ? options : { force: !!options };
  const force = !!opts.force;
  const manual = !!opts.manual || force;
  const settings = readAnalysisSettings();
  const filters = normalizeSnapshotFilters(settings);
  const snapshotKey = buildSnapshotCacheKey(filters);
  const cachedEntry = getAnalysisSnapshot(filters);
  const hasCached = !!cachedEntry?.payload;
  const stale = hasCached ? shouldRevalidateSnapshot(cachedEntry, { sourceDirty: analysisState.allOrdersDirty }) : true;

  if (analysisState.loading){
    if (!force){
      return analysisState.promise;
    }
    await analysisState.promise;
  }

  if (hasCached && !force){
    const cachedPayload = {
      ...cachedEntry.payload,
      cacheStatus: stale ? "stale" : "fresh",
    };
    analysisState.currentSnapshotKey = snapshotKey;
    applyAnalysisSnapshotPayload(cachedPayload, { refreshing: stale });
    setAnalysisStatus("");
    if (!stale){
      return cachedPayload;
    }
  }else{
    setAnalysisStatus(analysisState.hasRenderedSnapshot ? "Refreshing selected filters…" : "Loading analytics…");
    if (!analysisState.hasRenderedSnapshot){
      renderAnalysisLoadingState();
    }
  }

  const shouldCompute = force || !hasCached || stale;
  if (!shouldCompute){
    return cachedEntry.payload;
  }

  setAnalysisRefreshingState(true, { manual });
  analysisState.loading = true;
  const task = (async ()=>{
    try{
      const payload = await computeAnalysisSnapshot(settings);
      if (!payload){
        return hasCached ? cachedEntry.payload : null;
      }
      const latestFilters = normalizeSnapshotFilters(readAnalysisSettings());
      const latestKey = buildSnapshotCacheKey(latestFilters);
      if (latestKey === snapshotKey){
        analysisState.currentSnapshotKey = snapshotKey;
        applyAnalysisSnapshotPayload(payload, { refreshing: false });
        setAnalysisStatus("");
      }
      return payload;
    }catch(error){
      console.error("Analysis refresh failed:", error);
      if (hasCached){
        setAnalysisStatus("Refresh failed — showing cached analysis.");
        applyAnalysisSnapshotPayload(cachedEntry.payload, { refreshing: false });
        return cachedEntry.payload;
      }
      setAnalysisStatus("Analysis unavailable — check connection.");
      analysisState.dataset = null;
      analysisState.orders = [];
      analysisState.metrics = null;
      analysisState.topClients = [];
      analysisState.topTypes = [];
      analysisState.topDimensions = [];
      analysisState.currentPeriod = null;
      analysisState.previousPeriod = null;
      analysisState.insights = [];
      analysisState.lastSnapshot = null;
      analysisState.advancedDataset = null;
      analysisState.currentSnapshotComputedAt = "";
      renderAnalysisKpis({ current: { totals: null } });
      renderTopClients([]);
      renderTopTypes([]);
      renderTopDimensions([]);
      renderCharts({ current: { daily: [], topClients: [], topTypes: [] } });
      renderTopOrders({ current: { orderContributions: [] } });
      renderInsights([], null, false);
      setAnalysisFreshnessLabel("", false);
      if (analysisExportBtn) analysisExportBtn.disabled = true;
      return null;
    }finally{
      analysisState.loading = false;
      setAnalysisRefreshingState(false, { manual });
    }
  })();
  analysisState.promise = task;
  return task;
}

function recalculateDimensionBuckets(){
  readAnalysisSettings();
  refreshAnalysis({ reason: "dimension-recalc" });
}

function ensureAnalysisReady(){
  if (!analysisState.initialized){
    initAnalysis();
  }
}

function bindAnalysisSortHandlers(){
  const handler = event => {
    const button = event.target.closest("button[data-table][data-sort]");
    if (!button) return;
    event.preventDefault();
    const tableName = button.dataset.table;
    const sortKey = button.dataset.sort;
    toggleSortState(tableName, sortKey);
    const context = {
      current: {
        totals: analysisState.metrics,
        topClients: analysisState.topClients,
        topTypes: analysisState.topTypes,
        topDimensions: analysisState.topDimensions,
        daily: analysisState.currentPeriod?.daily || [],
      },
      previous: analysisState.previousPeriod,
      period: { start: analysisState.controls.startDate, end: analysisState.controls.endDate },
      previousPeriod: derivePreviousPeriod(analysisState.controls.startDate, analysisState.controls.endDate),
    };
    renderTopClients(analysisState.topClients);
    renderTopTypes(analysisState.topTypes);
    renderTopDimensions(analysisState.topDimensions);
    renderSortIndicators();
    renderCharts({
      current: {
        daily: analysisState.currentPeriod?.daily || [],
        topClients: analysisState.topClients,
        topTypes: analysisState.topTypes,
      },
    });
    renderPeriodMeta(context);
  };
  if (analysisTopClientsHead) analysisTopClientsHead.addEventListener("click", handler);
  if (analysisTopTypesHead) analysisTopTypesHead.addEventListener("click", handler);
  if (analysisDimensionsHead) analysisDimensionsHead.addEventListener("click", handler);
}

function updateAnalysisRangePresetState(){
  if (!analysisRangePresetsEl) return;
  let activeRange = "";
  if (analysisState.controls.allTime){
    activeRange = "all";
  }else{
    const start = parseDateInput(analysisState.controls.startDate, false);
    const end = parseDateInput(analysisState.controls.endDate, false);
    if (start && end){
      const inclusiveDays = Math.round((end.getTime() - start.getTime()) / (24 * 60 * 60 * 1000)) + 1;
      if ([7, 30, 90].includes(inclusiveDays)){
        activeRange = String(inclusiveDays);
      }
    }
  }
  analysisRangePresetsEl.querySelectorAll("button[data-analysis-range]").forEach(button => {
    button.classList.toggle("active", button.dataset.analysisRange === activeRange);
  });
  if (analysisCompareToggle){
    analysisCompareToggle.disabled = analysisState.controls.allTime;
  }
}

function markAnalysisFiltersPending({ dateChanged = false } = {}){
  if (dateChanged){
    analysisState.controls.allTime = false;
  }
  readAnalysisSettings();
  updateAnalysisRangePresetState();
  setAnalysisStatus("Filters changed — apply to update.");
}

function applyAnalysisRangePreset(value){
  const range = String(value || "");
  if (range === "all"){
    analysisState.controls.allTime = true;
    analysisState.controls.startDate = "";
    analysisState.controls.endDate = "";
    analysisState.controls.comparePrevious = false;
    if (analysisDateStartInput) analysisDateStartInput.value = "";
    if (analysisDateEndInput) analysisDateEndInput.value = "";
    if (analysisCompareToggle) analysisCompareToggle.checked = false;
  }else{
    const days = Number(range);
    if (![7, 30, 90].includes(days)) return;
    const existingEnd = parseDateInput(analysisState.controls.endDate, false);
    const end = existingEnd || new Date();
    const start = new Date(end.getTime());
    start.setDate(start.getDate() - (days - 1));
    analysisState.controls.allTime = false;
    analysisState.controls.startDate = toDateInputValue(start);
    analysisState.controls.endDate = toDateInputValue(end);
    if (analysisDateStartInput) analysisDateStartInput.value = analysisState.controls.startDate;
    if (analysisDateEndInput) analysisDateEndInput.value = analysisState.controls.endDate;
  }
  updateAnalysisRangePresetState();
  refreshAnalysis({ force: true, manual: true, reason: "range-preset" });
}

function analysisCsvCell(value){
  const text = value == null ? "" : String(value);
  return `"${text.replace(/"/g, '""')}"`;
}

function exportAnalysisSnapshot(){
  const snapshot = analysisState.lastSnapshot;
  if (!snapshot) return;
  const lines = [];
  const addRow = values => lines.push(values.map(analysisCsvCell).join(","));
  addRow(["Analytics report", "Value"]);
  addRow(["Period start", snapshot.period?.start || "All time"]);
  addRow(["Period end", snapshot.period?.end || "All time"]);
  addRow(["Generated", snapshot.computedAt || ""]);
  lines.push("");
  addRow(["Metric", "Value"]);
  Object.entries(snapshot.kpis || {}).forEach(([key, value]) => addRow([key, value]));
  lines.push("");
  addRow(["Top clients", "Orders", "Units", "Area m2", "Share %", "Growth %"]);
  (snapshot.topClients || []).forEach(item => addRow([
    item.client,
    item.orders,
    item.units,
    item.area_m2,
    item.share_area_pct,
    item.growth_pct,
  ]));
  lines.push("");
  addRow(["Glass types", "Lines", "Units", "Area m2", "Share %", "Growth %"]);
  (snapshot.topGlassTypes || []).forEach(item => addRow([
    item.type,
    item.lines,
    item.units,
    item.area_m2,
    item.share_area_pct,
    item.growth_pct,
  ]));
  lines.push("");
  addRow(["Dimensions", "Quantity", "Orders", "Clients", "Repeat score", "Batching"]);
  (snapshot.topDimensions || []).forEach(item => addRow([
    item.dimension,
    item.qty,
    item.orders,
    item.clients,
    item.repeatScore,
    item.batchingOpportunity ? "Yes" : "No",
  ]));
  const blob = new Blob(["\ufeff" + lines.join("\r\n")], { type: "text/csv;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `analytics-${snapshot.period?.start || "all"}-${snapshot.period?.end || "time"}.csv`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
}

function bindAnalysisFilterHandlers(){
  if (analysisDateStartInput) analysisDateStartInput.addEventListener("change", ()=> markAnalysisFiltersPending({ dateChanged: true }));
  if (analysisDateEndInput) analysisDateEndInput.addEventListener("change", ()=> markAnalysisFiltersPending({ dateChanged: true }));
  if (analysisCompareToggle) analysisCompareToggle.addEventListener("change", ()=> markAnalysisFiltersPending());
  if (analysisClientFilter) analysisClientFilter.addEventListener("change", ()=> markAnalysisFiltersPending());
  if (analysisTypeFilter) analysisTypeFilter.addEventListener("change", ()=> markAnalysisFiltersPending());
  if (analysisRefreshBtn){
    analysisRefreshBtn.addEventListener("click", ()=> refreshAnalysis({ force: true, manual: true, reason: "manual-refresh" }));
  }
  if (analysisExportBtn){
    analysisExportBtn.addEventListener("click", exportAnalysisSnapshot);
  }
  if (analysisRangePresetsEl){
    analysisRangePresetsEl.addEventListener("click", event => {
      const button = event.target.closest("button[data-analysis-range]");
      if (!button) return;
      applyAnalysisRangePreset(button.dataset.analysisRange);
    });
  }
  if (analysisDistributionMetricEl){
    analysisDistributionMetricEl.addEventListener("click", event => {
      const button = event.target.closest("button[data-metric]");
      if (!button) return;
      const metric = button.dataset.metric;
      if (!metric) return;
      analysisState.distributionMetric = metric;
      analysisDistributionMetricEl.querySelectorAll("button[data-metric]").forEach(item => {
        item.classList.toggle("active", item.dataset.metric === metric);
      });
      renderCharts({
        current: {
          daily: analysisState.currentPeriod?.daily || [],
          topClients: analysisState.topClients,
          topTypes: analysisState.topTypes,
        },
      });
    });
  }
}

function initAnalysis(){
  if (analysisState.initialized) return;
  analysisState.initialized = true;
  readAnalysisSettings();
  renderAnalysisLoadingState();
  setAnalysisFreshnessLabel("", false);
  updateAnalysisRangePresetState();
  bindAnalysisFilterHandlers();
  bindAnalysisSortHandlers();
  refreshAnalysis({ reason: "init" });
  if (analysisToleranceInput){
    analysisToleranceInput.addEventListener("change", ()=> recalculateDimensionBuckets());
    analysisToleranceInput.addEventListener("input", ()=> readAnalysisSettings());
  }
  if (analysisOrientationToggle){
    analysisOrientationToggle.addEventListener("change", ()=> recalculateDimensionBuckets());
  }
  if (analysisRecalcBtn){
    analysisRecalcBtn.addEventListener("click", ()=> recalculateDimensionBuckets());
  }
  if (analysisAskBtn){
    analysisAskBtn.addEventListener("click", ()=> submitAnalysisQuestion());
  }
  if (analysisQuickActionsEl){
    analysisQuickActionsEl.addEventListener("click", event => {
      const button = event.target.closest("button[data-question]");
      if (!button || !analysisQuestionInput) return;
      analysisQuestionInput.value = button.dataset.question || "";
      analysisQuestionInput.focus();
    });
  }
  if (analysisQuestionInput){
    analysisQuestionInput.addEventListener("keydown", event => {
      if (event.key === "Enter" && (event.metaKey || event.ctrlKey)){
        event.preventDefault();
        submitAnalysisQuestion();
      }
    });
  }
  if (analysisDebugToggle){
    analysisDebugToggle.addEventListener("click", ()=>{
      const dataset = analysisState.dataset;
      if (!dataset){
        console.log("[analysis debug] No dataset loaded.");
        return;
      }
      const ordersCount = Array.isArray(dataset.orders) ? dataset.orders.length : 0;
      const orderRows = Array.isArray(dataset.orders)
        ? dataset.orders.reduce((sum, order)=> sum + (Array.isArray(order.rows) ? order.rows.length : 0), 0)
        : 0;
      const processingCount = Array.isArray(dataset.processing_orders) ? dataset.processing_orders.length : 0;
      const processingRows = Array.isArray(dataset.processing_orders)
        ? dataset.processing_orders.reduce((sum, order)=> sum + (Array.isArray(order.rows) ? order.rows.length : 0), 0)
        : 0;
      console.log(`[analysis debug] orders=${ordersCount} rows=${orderRows}, processing=${processingCount} rows=${processingRows}`);
    });
  }
}

function setAnalysisAnswerPlaceholder(message, muted = true){
  if (!analysisAnswerEl) return;
  analysisAnswerEl.textContent = message;
  if (muted){
    analysisAnswerEl.classList.add("muted");
  }else{
    analysisAnswerEl.classList.remove("muted");
  }
}

function formatMarkdownInline(text){
  let html = escapeHtml(text);
  html = html.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>");
  html = html.replace(/\*(.+?)\*/g, "<em>$1</em>");
  html = html.replace(/`([^`]+)`/g, "<code>$1</code>");
  return html;
}

function simpleMarkdownToHtml(text){
  const lines = String(text || "").split(/\r?\n/);
  const parts = [];
  let inList = false;
  lines.forEach(line => {
    if (/^\s*-\s+/.test(line)){
      if (!inList){
        parts.push("<ul>");
        inList = true;
      }
      const content = line.replace(/^\s*-\s+/, "");
      parts.push(`<li>${formatMarkdownInline(content)}</li>`);
    }else{
      if (inList){
        parts.push("</ul>");
        inList = false;
      }
      if (!line.trim()){
        parts.push("<br>");
      }else{
        parts.push(`<p>${formatMarkdownInline(line)}</p>`);
      }
    }
  });
  if (inList){
    parts.push("</ul>");
  }
  return parts.join("");
}

function renderAnalysisAnswer(markdown){
  if (!analysisAnswerEl) return;
  if (!markdown){
    setAnalysisAnswerPlaceholder("AI analysis is unavailable. The dashboard above still works offline.", true);
    return;
  }
  analysisAnswerEl.innerHTML = simpleMarkdownToHtml(markdown);
  analysisAnswerEl.classList.remove("muted");
}

async function submitAnalysisQuestion(){
  if (!analysisQuestionInput) return;
  const question = analysisQuestionInput.value.trim();
  if (!question){
    setAnalysisAnswerPlaceholder("Ask a question to see AI insights.", true);
    analysisQuestionInput.focus();
    return;
  }
  ensureAnalysisReady();
  if (analysisState.loading){
    await analysisState.promise;
  }
  if (!analysisState.advancedDataset){
    await refreshAnalysis();
  }
  if (!analysisState.advancedDataset){
    setAnalysisAnswerPlaceholder("AI analysis is unavailable. The dashboard above still works offline.", true);
    return;
  }
  const context = {
    period: { start: analysisState.controls.startDate, end: analysisState.controls.endDate },
    previousPeriod: derivePreviousPeriod(analysisState.controls.startDate, analysisState.controls.endDate),
    current: {
      totals: analysisState.metrics || {},
      topClients: analysisState.topClients || [],
      topTypes: analysisState.topTypes || [],
      topDimensions: analysisState.topDimensions || [],
      orderContributions: analysisState.currentPeriod?.orderContributions || [],
      daily: analysisState.currentPeriod?.daily || [],
    },
    previous: analysisState.previousPeriod || null,
  };
  let payloadDataset = analysisState.advancedDataset
    ? JSON.parse(JSON.stringify(analysisState.advancedDataset))
    : buildAdvancedAnalysisDataset(context);
  payloadDataset.orders = [];
  payloadDataset.processing_orders = [];
  payloadDataset.meta = payloadDataset.meta && typeof payloadDataset.meta === "object"
    ? payloadDataset.meta
    : {};
  payloadDataset.meta.notes = Array.isArray(payloadDataset.meta.notes) ? payloadDataset.meta.notes : [];
  let serialized = JSON.stringify(payloadDataset);
  if (serialized.length > ANALYSIS_MAX_DATASET_CHARS){
    payloadDataset.meta.truncated = true;
    const current = payloadDataset.aggregates?.current;
    if (current && Array.isArray(current.daily) && current.daily.length > 60){
      current.daily = current.daily.slice(-60);
      payloadDataset.meta.notes.push("daily series trimmed to last 60 points");
    }
    if (current && Array.isArray(current.topClients) && current.topClients.length > 20){
      current.topClients = current.topClients.slice(0, 20);
      payloadDataset.meta.notes.push("top clients trimmed to 20");
    }
    if (current && Array.isArray(current.topTypes) && current.topTypes.length > 20){
      current.topTypes = current.topTypes.slice(0, 20);
      payloadDataset.meta.notes.push("top types trimmed to 20");
    }
    if (current && Array.isArray(current.topDimensions) && current.topDimensions.length > 20){
      current.topDimensions = current.topDimensions.slice(0, 20);
      payloadDataset.meta.notes.push("top dimensions trimmed to 20");
    }
    serialized = JSON.stringify(payloadDataset);
  }
  payloadDataset.meta.size = serialized.length;
  const sessionId = getAnalysisSessionId();
  const headers = { "Content-Type": "application/json" };
  if (sessionId){
    headers["x-session-id"] = sessionId;
  }
  const payload = {
    question,
    dataset: payloadDataset,
    settings: {
      tolerance_mm: analysisState.tolerance,
      orient_agnostic: analysisState.orientationAgnostic,
      compare_previous: analysisState.controls.comparePrevious,
      period_start: analysisState.controls.startDate || null,
      period_end: analysisState.controls.endDate || null,
      client_filter: analysisState.controls.client || null,
      glass_type_filter: analysisState.controls.glassType || null,
      decimal_separator: appState.processing.options.decimalSeparator === "dot" ? "dot" : "comma",
    },
  };
  analysisState.ai.loading = true;
  analysisState.ai.lastQuestion = question;
  if (analysisAskBtn) analysisAskBtn.disabled = true;
  setAnalysisAnswerPlaceholder("Thinking…", true);
  try{
    const res = await fetch(API_BASE + "/analysis/ask", {
      method: "POST",
      headers,
      body: JSON.stringify(payload),
    });
    if (!res.ok){
      throw new Error(`HTTP ${res.status}`);
    }
    const data = await res.json();
    if (data && typeof data.answerMarkdown === "string"){
      renderAnalysisAnswer(data.answerMarkdown);
      analysisQuestionInput.value = "";
      analysisQuestionInput.focus();
    }else{
      setAnalysisAnswerPlaceholder("AI analysis is unavailable. The dashboard above still works offline.", true);
    }
  }catch(error){
    console.warn("AI analysis request failed:", error);
    setAnalysisAnswerPlaceholder("AI analysis is unavailable. The dashboard above still works offline.", true);
  }finally{
    analysisState.ai.loading = false;
    if (analysisAskBtn) analysisAskBtn.disabled = false;
    if (analysisQuestionInput) analysisQuestionInput.focus();
  }
}

function setGlobalAppearanceVariables(settings){
  if (!document.documentElement) return;
  const rootStyle = document.documentElement.style;
  rootStyle.setProperty("--ms-header-size", `${settings.headerFontSize}px`);
  rootStyle.setProperty("--ms-subheader-size", `${settings.orderInfoFontSize}px`);
  rootStyle.setProperty("--ms-body-size", `${settings.bodyFontSize}px`);
  rootStyle.setProperty("--ms-body-line-height", "1.4");
  rootStyle.setProperty("--ms-accent-color", "#2F6BFF");
  let baseFont = "var(--ms-sans-system)";
  let headerFont = "var(--ms-sans-system)";
  let rowsFont = "var(--ms-sans-system)";
  if (settings.fontFamily === "inter"){
    baseFont = headerFont = rowsFont = "var(--ms-inter-stack)";
  }else if (settings.fontFamily === "roboto"){
    baseFont = headerFont = rowsFont = "var(--ms-roboto-stack)";
  }else if (settings.fontFamily === "mono-rows"){
    rowsFont = "var(--ms-mono-stack)";
  }
  rootStyle.setProperty("--ms-family-base", baseFont);
  rootStyle.setProperty("--ms-family-header", headerFont);
  rootStyle.setProperty("--ms-family-rows", rowsFont);
  const densityAdjustments = {
    compact: "-2px",
    normal: "0px",
    roomy: "2px",
  };
  rootStyle.setProperty("--ms-row-spacing-adjust", densityAdjustments[settings.rowDensity] || "0px");
}

function applyAppearanceToNode(node, settings){
  if (!node || !(node instanceof Element)) return;
  const fontClasses = ["font-system","font-inter","font-roboto","font-mono-rows"];
  const densityClasses = ["density-compact","density-normal","density-roomy"];
  const accentClasses = ["accent-none","accent-thinrule","accent-leftbar"];
  node.classList.remove(...fontClasses, ...densityClasses, ...accentClasses);
  const fontMap = {
    "system-sans": ["font-system"],
    inter: ["font-inter"],
    roboto: ["font-roboto"],
    "mono-rows": ["font-system","font-mono-rows"],
  };
  const densityMap = {
    compact: "density-compact",
    normal: "density-normal",
    roomy: "density-roomy",
  };
  const accentMap = {
    none: "accent-none",
    thinrule: "accent-thinrule",
    leftbar: "accent-leftbar",
  };
  (fontMap[settings.fontFamily] || fontMap["system-sans"]).forEach(cls => node.classList.add(cls));
  node.classList.add(densityMap[settings.rowDensity] || densityMap.normal);
  node.classList.add(accentMap[settings.accentStyle] || accentMap.none);
  node.style.setProperty("--ms-header-size", `${settings.headerFontSize}px`);
  node.style.setProperty("--ms-subheader-size", `${settings.orderInfoFontSize}px`);
  node.style.setProperty("--ms-body-size", `${settings.bodyFontSize}px`);
  node.style.setProperty("--ms-body-line-height", "1.4");
  node.style.setProperty("--ms-accent-color", "#2F6BFF");
}

function ensureMotherSheetObserver(){
  if (motherSheetObserver || typeof MutationObserver !== "function" || !document.body) return;
  motherSheetObserver = new MutationObserver(mutations => {
    mutations.forEach(mutation => {
      mutation.addedNodes.forEach(node => {
        if (!(node instanceof Element)) return;
        if (node.matches(MOTHER_SHEET_SELECTOR)){
          applyAppearanceToNode(node, appearanceSettings || DEFAULT_APPEARANCE_SETTINGS);
        }else{
          node.querySelectorAll?.(MOTHER_SHEET_SELECTOR).forEach(child => {
            applyAppearanceToNode(child, appearanceSettings || DEFAULT_APPEARANCE_SETTINGS);
          });
        }
      });
    });
  });
  motherSheetObserver.observe(document.body, { childList: true, subtree: true });
}

function setStatusMessage(message, options = {}){
  if (!statusEl) return;
  statusEl.textContent = "";
  if (options.showSpinner){
    const spinner = document.createElement("span");
    spinner.className = "spinner";
    statusEl.appendChild(spinner);
    statusEl.appendChild(document.createTextNode(" "));
  }
  statusEl.appendChild(document.createTextNode(String(message || "")));
}

function setScanStatus(message, isError = false){
  if (!scanStatusEl) return;
  scanStatusEl.textContent = message || "";
  scanStatusEl.style.color = isError ? "#b91c1c" : "var(--muted)";
}

function clampScanValue(value, min, max){
  return Math.min(max, Math.max(min, value));
}

function createScanId(){
  if (window.crypto && typeof window.crypto.randomUUID === "function"){
    return window.crypto.randomUUID();
  }
  return `scan-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

function normalizeScanRotation(rotation){
  const normalized = Number(rotation) || 0;
  const wrapped = normalized % 360;
  return wrapped < 0 ? wrapped + 360 : wrapped;
}

function getScanSortedPages(){
  return [...scanStudioState.pages].sort((a, b) => (a.orderIndex || 0) - (b.orderIndex || 0));
}

function normalizeScanPageOrder(){
  getScanSortedPages().forEach((page, index)=>{
    page.orderIndex = index;
  });
}

function getSelectedScanPage(){
  if (!scanStudioState.selectedPageId) return null;
  return scanStudioState.pages.find(page => page.id === scanStudioState.selectedPageId) || null;
}

function createScanCanvas(width, height){
  const canvas = document.createElement("canvas");
  canvas.width = Math.max(1, Math.round(width));
  canvas.height = Math.max(1, Math.round(height));
  return canvas;
}

function isValidScanCrop(crop){
  if (!crop || typeof crop !== "object") return false;
  const { x, y, width, height } = crop;
  if (![x, y, width, height].every(Number.isFinite)) return false;
  return width > 0.001 && height > 0.001;
}

function getDefaultScanPerspective(){
  return [
    { x: 0, y: 0 },
    { x: 1, y: 0 },
    { x: 1, y: 1 },
    { x: 0, y: 1 },
  ];
}

function normalizeScanPerspective(points){
  const base = Array.isArray(points) && points.length === 4 ? points : getDefaultScanPerspective();
  return base.map(point => ({
    x: clampScanValue(Number(point?.x) || 0, 0, 1),
    y: clampScanValue(Number(point?.y) || 0, 0, 1),
  }));
}

function isScanPerspectiveIdentity(points){
  const normalized = normalizeScanPerspective(points);
  const defaults = getDefaultScanPerspective();
  return normalized.every((point, index)=> (
    Math.abs(point.x - defaults[index].x) < 0.0001
      && Math.abs(point.y - defaults[index].y) < 0.0001
  ));
}

function getScanFileExtension(fileName){
  const lower = String(fileName || "").toLowerCase();
  const idx = lower.lastIndexOf(".");
  if (idx < 0) return "";
  return lower.slice(idx + 1);
}

function isScanImageFile(file){
  if (!file) return false;
  const ext = getScanFileExtension(file.name || "");
  if (ext && SCAN_ALLOWED_EXTENSIONS.has(ext)) return true;
  const mime = String(file.type || "").toLowerCase();
  return mime === "image/jpeg" || mime === "image/png" || mime === "image/webp";
}

function loadScanImage(src){
  return new Promise((resolve, reject)=>{
    const img = new Image();
    img.onload = ()=> resolve(img);
    img.onerror = ()=> reject(new Error("Failed to decode image."));
    img.src = src;
  });
}

function ensureScanPageExists(){
  const pages = getScanSortedPages();
  if (!pages.length){
    scanStudioState.selectedPageId = "";
    return;
  }
  if (!getSelectedScanPage()){
    scanStudioState.selectedPageId = pages[0].id;
  }
}

function clearScanDragHoverState(){
  if (!scanPageListEl) return;
  scanPageListEl.querySelectorAll(".scan-page-item.drag-over").forEach(node => node.classList.remove("drag-over"));
}

function renderScanPageList(){
  if (!scanPageListEl) return;
  normalizeScanPageOrder();
  ensureScanPageExists();
  const pages = getScanSortedPages();
  if (!pages.length){
    scanPageListEl.innerHTML = `<div class="scan-page-empty">No pages uploaded yet.</div>`;
    updateScanControlState();
    return;
  }
  const html = pages.map((page, index)=>{
    const isActive = page.id === scanStudioState.selectedPageId;
    const filterLabel = page.filter === "bw" ? "B&W" : (page.filter === "document" ? "Doc" : (page.filter === "grayscale" ? "Gray" : "Orig"));
    return `<div class="scan-page-item${isActive ? " active" : ""}" draggable="true" data-scan-page-id="${escapeHtml(page.id)}">
      <img class="scan-thumb" src="${escapeHtml(page.originalSrc)}" alt="Page ${index + 1}" />
      <div class="scan-page-meta">
        <div class="scan-page-name">Page ${index + 1} · ${escapeHtml(page.fileName || "image")}</div>
        <div class="scan-page-sub">Rot ${normalizeScanRotation(page.rotation)}° · ${filterLabel}</div>
        <div class="scan-page-actions">
          <button type="button" class="btn small" data-scan-action="select" data-scan-page-id="${escapeHtml(page.id)}">Open</button>
          <button type="button" class="btn small" data-scan-action="up" data-scan-page-id="${escapeHtml(page.id)}">Up</button>
          <button type="button" class="btn small" data-scan-action="down" data-scan-page-id="${escapeHtml(page.id)}">Down</button>
          <button type="button" class="btn small danger" data-scan-action="delete" data-scan-page-id="${escapeHtml(page.id)}">Delete</button>
        </div>
      </div>
    </div>`;
  }).join("");
  scanPageListEl.innerHTML = html;
  updateScanControlState();
}

function updateScanControlState(){
  const page = getSelectedScanPage();
  const disabled = !page;
  const controls = [
    scanRotateLeftBtn,
    scanRotateRightBtn,
    scanCropToggleBtn,
    scanCropResetBtn,
    scanPerspectiveToggleBtn,
    scanPerspectiveResetBtn,
    scanFilterSelect,
    scanBrightnessInput,
    scanContrastInput,
    scanPrintCurrentBtn,
    scanPrintAllBtn,
    scanExportPdfBtn,
    scanExportImageBtn,
  ];
  controls.forEach(control => {
    if (control) control.disabled = disabled;
  });
  if (scanPrintAllBtn){
    scanPrintAllBtn.disabled = scanStudioState.pages.length === 0;
  }
  if (scanExportPdfBtn){
    scanExportPdfBtn.disabled = scanStudioState.pages.length === 0;
  }
  updateScanSliderProgress(scanBrightnessInput);
  updateScanSliderProgress(scanContrastInput);
}

function updateScanSliderProgress(input){
  if (!input) return;
  const min = Number(input.min);
  const max = Number(input.max);
  const value = Number(input.value);
  if (!Number.isFinite(min) || !Number.isFinite(max) || !Number.isFinite(value) || max <= min){
    input.style.setProperty("--scan-range-progress", "50%");
    return;
  }
  const ratio = (value - min) / (max - min);
  const pct = Math.min(100, Math.max(0, ratio * 100));
  input.style.setProperty("--scan-range-progress", `${pct}%`);
}

function syncScanControlsFromSelectedPage(){
  const page = getSelectedScanPage();
  updateScanControlState();
  if (!page){
    if (scanFilterSelect) scanFilterSelect.value = "original";
    if (scanBrightnessInput) scanBrightnessInput.value = "0";
    if (scanContrastInput) scanContrastInput.value = "0";
    if (scanBrightnessValue) scanBrightnessValue.textContent = "0";
    if (scanContrastValue) scanContrastValue.textContent = "0";
    updateScanSliderProgress(scanBrightnessInput);
    updateScanSliderProgress(scanContrastInput);
    if (scanCropToggleBtn) scanCropToggleBtn.classList.remove("active");
    if (scanPerspectiveToggleBtn) scanPerspectiveToggleBtn.classList.remove("active");
    return;
  }
  if (scanFilterSelect) scanFilterSelect.value = page.filter || "original";
  if (scanBrightnessInput) scanBrightnessInput.value = String(Number(page.brightness) || 0);
  if (scanContrastInput) scanContrastInput.value = String(Number(page.contrast) || 0);
  if (scanBrightnessValue) scanBrightnessValue.textContent = String(Math.round(Number(page.brightness) || 0));
  if (scanContrastValue) scanContrastValue.textContent = String(Math.round(Number(page.contrast) || 0));
  updateScanSliderProgress(scanBrightnessInput);
  updateScanSliderProgress(scanContrastInput);
  if (scanCropToggleBtn) scanCropToggleBtn.classList.toggle("active", !!scanStudioState.cropMode);
  if (scanPerspectiveToggleBtn) scanPerspectiveToggleBtn.classList.toggle("active", !!scanStudioState.perspectiveMode);
}

function prepareScanPreviewCanvases(){
  if (!scanPreviewStage || !scanPreviewCanvas || !scanOverlayCanvas){
    return null;
  }
  const stageRect = scanPreviewStage.getBoundingClientRect();
  const width = Math.max(320, Math.floor(stageRect.width || 0));
  const height = Math.max(240, Math.floor(stageRect.height || 0));
  const dpr = window.devicePixelRatio || 1;
  const pxWidth = Math.max(1, Math.round(width * dpr));
  const pxHeight = Math.max(1, Math.round(height * dpr));

  [scanPreviewCanvas, scanOverlayCanvas].forEach(canvas => {
    if (canvas.width !== pxWidth || canvas.height !== pxHeight){
      canvas.width = pxWidth;
      canvas.height = pxHeight;
    }
    canvas.style.width = `${width}px`;
    canvas.style.height = `${height}px`;
  });

  const previewCtx = scanPreviewCanvas.getContext("2d");
  const overlayCtx = scanOverlayCanvas.getContext("2d");
  previewCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
  overlayCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
  return { previewCtx, overlayCtx, width, height };
}

function drawEmptyScanPreview(message){
  const prepared = prepareScanPreviewCanvases();
  if (!prepared) return;
  const { previewCtx, overlayCtx, width, height } = prepared;
  previewCtx.clearRect(0, 0, width, height);
  overlayCtx.clearRect(0, 0, width, height);
  previewCtx.fillStyle = "#0b1220";
  previewCtx.fillRect(0, 0, width, height);
  previewCtx.fillStyle = "#8fa0b8";
  previewCtx.font = "13px system-ui, -apple-system, Segoe UI, sans-serif";
  previewCtx.textAlign = "center";
  previewCtx.fillText(message || "Upload images to begin.", width / 2, height / 2);
  scanStudioState.preview.imageCanvas = null;
  scanStudioState.preview.baseCanvas = null;
  scanStudioState.preview.width = 0;
  scanStudioState.preview.height = 0;
}

function drawScanPreviewCanvas(imageCanvas, baseCanvas){
  const prepared = prepareScanPreviewCanvases();
  if (!prepared) return;
  const { previewCtx, width, height } = prepared;
  previewCtx.clearRect(0, 0, width, height);
  previewCtx.fillStyle = "#0b1220";
  previewCtx.fillRect(0, 0, width, height);

  const imgW = imageCanvas.width;
  const imgH = imageCanvas.height;
  if (!imgW || !imgH){
    drawEmptyScanPreview("Unable to render page.");
    return;
  }
  const scale = Math.min(width / imgW, height / imgH);
  const drawW = imgW * scale;
  const drawH = imgH * scale;
  const offsetX = (width - drawW) / 2;
  const offsetY = (height - drawH) / 2;

  previewCtx.imageSmoothingEnabled = true;
  previewCtx.imageSmoothingQuality = "high";
  previewCtx.drawImage(imageCanvas, offsetX, offsetY, drawW, drawH);

  scanStudioState.preview.imageCanvas = imageCanvas;
  scanStudioState.preview.baseCanvas = baseCanvas;
  scanStudioState.preview.scale = scale;
  scanStudioState.preview.offsetX = offsetX;
  scanStudioState.preview.offsetY = offsetY;
  scanStudioState.preview.width = imgW;
  scanStudioState.preview.height = imgH;
}

function drawScanCropOverlay(ctx, page){
  const transform = scanStudioState.preview;
  const imageWidth = transform.width || 0;
  const imageHeight = transform.height || 0;
  if (!imageWidth || !imageHeight) return;
  const crop = scanStudioState.cropDraft || (isValidScanCrop(page?.crop) ? page.crop : null);
  if (!crop) return;

  const x = transform.offsetX + (crop.x * imageWidth * transform.scale);
  const y = transform.offsetY + (crop.y * imageHeight * transform.scale);
  const width = crop.width * imageWidth * transform.scale;
  const height = crop.height * imageHeight * transform.scale;
  if (width <= 1 || height <= 1) return;

  ctx.save();
  ctx.fillStyle = "rgba(14,23,42,0.45)";
  ctx.beginPath();
  ctx.rect(transform.offsetX, transform.offsetY, imageWidth * transform.scale, imageHeight * transform.scale);
  ctx.rect(x, y, width, height);
  ctx.fill("evenodd");
  ctx.strokeStyle = "#38bdf8";
  ctx.lineWidth = 2;
  ctx.setLineDash([6, 4]);
  ctx.strokeRect(x, y, width, height);
  ctx.setLineDash([]);
  ctx.restore();
}

function drawScanPerspectiveOverlay(ctx, page){
  const transform = scanStudioState.preview;
  const imageWidth = transform.width || 0;
  const imageHeight = transform.height || 0;
  if (!imageWidth || !imageHeight) return;
  const points = normalizeScanPerspective(page?.perspective);
  const toDisplay = point => ({
    x: transform.offsetX + (point.x * imageWidth * transform.scale),
    y: transform.offsetY + (point.y * imageHeight * transform.scale),
  });
  const displayPoints = points.map(toDisplay);

  ctx.save();
  ctx.strokeStyle = "#22c55e";
  ctx.fillStyle = "rgba(34,197,94,0.18)";
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(displayPoints[0].x, displayPoints[0].y);
  for (let i = 1; i < displayPoints.length; i += 1){
    ctx.lineTo(displayPoints[i].x, displayPoints[i].y);
  }
  ctx.closePath();
  ctx.fill();
  ctx.stroke();
  displayPoints.forEach((point, index)=>{
    const active = index === scanStudioState.draggingPerspectiveIndex;
    ctx.beginPath();
    ctx.arc(point.x, point.y, active ? 6 : 5, 0, Math.PI * 2);
    ctx.fillStyle = active ? "#16a34a" : "#ffffff";
    ctx.strokeStyle = "#16a34a";
    ctx.lineWidth = 2;
    ctx.fill();
    ctx.stroke();
  });
  ctx.restore();
}

function drawScanOverlay(){
  if (!scanOverlayCanvas) return;
  const prepared = prepareScanPreviewCanvases();
  if (!prepared) return;
  const { overlayCtx, width, height } = prepared;
  overlayCtx.clearRect(0, 0, width, height);
  const page = getSelectedScanPage();
  if (!page) return;
  if (scanStudioState.cropMode){
    drawScanCropOverlay(overlayCtx, page);
  }
  if (scanStudioState.perspectiveMode){
    drawScanPerspectiveOverlay(overlayCtx, page);
  }
}

function getScanPointerNormalized(event){
  if (!scanPreviewStage || !scanStudioState.preview.width || !scanStudioState.preview.height) return null;
  const rect = scanPreviewStage.getBoundingClientRect();
  const x = event.clientX - rect.left;
  const y = event.clientY - rect.top;
  const transform = scanStudioState.preview;
  const relativeX = (x - transform.offsetX) / (transform.width * transform.scale);
  const relativeY = (y - transform.offsetY) / (transform.height * transform.scale);
  if (!Number.isFinite(relativeX) || !Number.isFinite(relativeY)) return null;
  return {
    x: clampScanValue(relativeX, 0, 1),
    y: clampScanValue(relativeY, 0, 1),
    rawX: x,
    rawY: y,
  };
}

function getScanPerspectiveHandleIndex(pointer, points){
  if (!pointer || !Array.isArray(points)) return -1;
  const transform = scanStudioState.preview;
  let bestIndex = -1;
  let bestDistance = 14;
  points.forEach((point, index)=>{
    const px = transform.offsetX + (point.x * transform.width * transform.scale);
    const py = transform.offsetY + (point.y * transform.height * transform.scale);
    const dx = pointer.rawX - px;
    const dy = pointer.rawY - py;
    const distance = Math.hypot(dx, dy);
    if (distance <= bestDistance){
      bestDistance = distance;
      bestIndex = index;
    }
  });
  return bestIndex;
}

function resizeScanImageToLimit(image, maxSide){
  const width = image.naturalWidth || image.width || 1;
  const height = image.naturalHeight || image.height || 1;
  const scale = Math.min(1, maxSide / Math.max(width, height));
  const targetWidth = Math.max(1, Math.round(width * scale));
  const targetHeight = Math.max(1, Math.round(height * scale));
  const canvas = createScanCanvas(targetWidth, targetHeight);
  const ctx = canvas.getContext("2d");
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = "high";
  ctx.drawImage(image, 0, 0, targetWidth, targetHeight);
  return canvas;
}

function applyScanRotation(canvas, rotation){
  const normalized = normalizeScanRotation(rotation);
  if (normalized === 0) return canvas;
  const swap = normalized === 90 || normalized === 270;
  const output = createScanCanvas(swap ? canvas.height : canvas.width, swap ? canvas.width : canvas.height);
  const ctx = output.getContext("2d");
  ctx.save();
  if (normalized === 90){
    ctx.translate(output.width, 0);
    ctx.rotate(Math.PI / 2);
  }else if (normalized === 180){
    ctx.translate(output.width, output.height);
    ctx.rotate(Math.PI);
  }else if (normalized === 270){
    ctx.translate(0, output.height);
    ctx.rotate(-Math.PI / 2);
  }
  ctx.drawImage(canvas, 0, 0);
  ctx.restore();
  return output;
}

function scanSampleBilinear(data, width, height, x, y){
  const x0 = clampScanValue(Math.floor(x), 0, width - 1);
  const y0 = clampScanValue(Math.floor(y), 0, height - 1);
  const x1 = clampScanValue(x0 + 1, 0, width - 1);
  const y1 = clampScanValue(y0 + 1, 0, height - 1);
  const dx = x - x0;
  const dy = y - y0;

  const i00 = (y0 * width + x0) * 4;
  const i10 = (y0 * width + x1) * 4;
  const i01 = (y1 * width + x0) * 4;
  const i11 = (y1 * width + x1) * 4;
  const out = [0, 0, 0, 0];

  for (let c = 0; c < 4; c += 1){
    const top = data[i00 + c] * (1 - dx) + data[i10 + c] * dx;
    const bottom = data[i01 + c] * (1 - dx) + data[i11 + c] * dx;
    out[c] = top * (1 - dy) + bottom * dy;
  }
  return out;
}

function applyScanPerspective(canvas, perspective){
  const points = normalizeScanPerspective(perspective);
  if (!points || points.length !== 4) return canvas;
  const srcCtx = canvas.getContext("2d");
  const srcImageData = srcCtx.getImageData(0, 0, canvas.width, canvas.height);
  const source = srcImageData.data;
  const width = canvas.width;
  const height = canvas.height;
  const output = createScanCanvas(width, height);
  const outCtx = output.getContext("2d");
  const outImageData = outCtx.createImageData(width, height);
  const target = outImageData.data;

  const p0 = { x: points[0].x * (width - 1), y: points[0].y * (height - 1) };
  const p1 = { x: points[1].x * (width - 1), y: points[1].y * (height - 1) };
  const p2 = { x: points[2].x * (width - 1), y: points[2].y * (height - 1) };
  const p3 = { x: points[3].x * (width - 1), y: points[3].y * (height - 1) };

  for (let y = 0; y < height; y += 1){
    const v = height <= 1 ? 0 : (y / (height - 1));
    for (let x = 0; x < width; x += 1){
      const u = width <= 1 ? 0 : (x / (width - 1));
      const sourceX =
        ((1 - u) * (1 - v) * p0.x) +
        (u * (1 - v) * p1.x) +
        (u * v * p2.x) +
        ((1 - u) * v * p3.x);
      const sourceY =
        ((1 - u) * (1 - v) * p0.y) +
        (u * (1 - v) * p1.y) +
        (u * v * p2.y) +
        ((1 - u) * v * p3.y);
      const [r, g, b, a] = scanSampleBilinear(source, width, height, sourceX, sourceY);
      const offset = (y * width + x) * 4;
      target[offset] = r;
      target[offset + 1] = g;
      target[offset + 2] = b;
      target[offset + 3] = a;
    }
  }

  outCtx.putImageData(outImageData, 0, 0);
  return output;
}

function applyScanCrop(canvas, crop){
  if (!isValidScanCrop(crop)) return canvas;
  const x = Math.round(clampScanValue(crop.x, 0, 1) * canvas.width);
  const y = Math.round(clampScanValue(crop.y, 0, 1) * canvas.height);
  const width = Math.round(clampScanValue(crop.width, 0, 1) * canvas.width);
  const height = Math.round(clampScanValue(crop.height, 0, 1) * canvas.height);
  if (width < 2 || height < 2) return canvas;
  const maxWidth = Math.min(width, canvas.width - x);
  const maxHeight = Math.min(height, canvas.height - y);
  if (maxWidth < 2 || maxHeight < 2) return canvas;
  const output = createScanCanvas(maxWidth, maxHeight);
  const ctx = output.getContext("2d");
  ctx.drawImage(canvas, x, y, maxWidth, maxHeight, 0, 0, maxWidth, maxHeight);
  return output;
}

function applyScanFilters(canvas, page){
  const output = createScanCanvas(canvas.width, canvas.height);
  const ctx = output.getContext("2d");
  ctx.drawImage(canvas, 0, 0);
  const imageData = ctx.getImageData(0, 0, output.width, output.height);
  const data = imageData.data;

  const filter = page.filter || "original";
  const brightness = clampScanValue(Number(page.brightness) || 0, -100, 100);
  const contrast = clampScanValue(Number(page.contrast) || 0, -100, 100);
  const brightnessShift = brightness * 2.55;
  const contrastFactor = 1 + (contrast / 100) * 1.8;
  const documentBoost = filter === "document" ? 1.28 : 1;
  const bwThreshold = 148;

  for (let i = 0; i < data.length; i += 4){
    let r = data[i];
    let g = data[i + 1];
    let b = data[i + 2];

    if (filter !== "original"){
      const lum = 0.2126 * r + 0.7152 * g + 0.0722 * b;
      r = lum;
      g = lum;
      b = lum;
    }

    if (filter === "document"){
      r = ((r - 128) * documentBoost) + 128;
      g = ((g - 128) * documentBoost) + 128;
      b = ((b - 128) * documentBoost) + 128;
    }

    r = ((r - 128) * contrastFactor) + 128 + brightnessShift;
    g = ((g - 128) * contrastFactor) + 128 + brightnessShift;
    b = ((b - 128) * contrastFactor) + 128 + brightnessShift;

    if (filter === "bw"){
      const avg = (r + g + b) / 3;
      const value = avg >= bwThreshold ? 255 : 0;
      r = value;
      g = value;
      b = value;
    }else if (filter === "document"){
      const avg = (r + g + b) / 3;
      if (avg > 220){
        r = 255;
        g = 255;
        b = 255;
      }else if (avg < 35){
        r = 0;
        g = 0;
        b = 0;
      }
    }

    data[i] = clampScanValue(Math.round(r), 0, 255);
    data[i + 1] = clampScanValue(Math.round(g), 0, 255);
    data[i + 2] = clampScanValue(Math.round(b), 0, 255);
  }

  ctx.putImageData(imageData, 0, 0);
  return output;
}

async function renderScanPageCanvas(page, options = {}){
  if (!page) return null;
  const cacheEntry = scanStudioImageCache.get(page.id);
  if (!cacheEntry || !cacheEntry.image) return null;
  const maxSide = Number(options.maxSide) || SCAN_PREVIEW_MAX_SIDE;
  const ignoreCrop = !!options.ignoreCrop;

  const scaled = resizeScanImageToLimit(cacheEntry.image, maxSide);
  const rotated = applyScanRotation(scaled, page.rotation);
  const usePerspective = page.perspective && !isScanPerspectiveIdentity(page.perspective);
  const perspective = usePerspective ? applyScanPerspective(rotated, page.perspective) : rotated;
  const baseCanvas = perspective;
  const cropped = ignoreCrop ? baseCanvas : applyScanCrop(baseCanvas, page.crop);
  const outputCanvas = applyScanFilters(cropped, page);

  return { baseCanvas, outputCanvas };
}

async function renderScanPreview(){
  const page = getSelectedScanPage();
  if (!page){
    drawEmptyScanPreview("Upload images to begin.");
    return;
  }
  const token = ++scanStudioState.renderToken;
  const result = await renderScanPageCanvas(page, {
    maxSide: SCAN_PREVIEW_MAX_SIDE,
    ignoreCrop: scanStudioState.cropMode || scanStudioState.perspectiveMode,
  });
  if (token !== scanStudioState.renderToken) return;
  if (!result){
    drawEmptyScanPreview("Preview unavailable.");
    return;
  }
  drawScanPreviewCanvas(result.outputCanvas, result.baseCanvas);
  drawScanOverlay();
}

function moveScanPageToIndex(pageId, targetIndex){
  const ordered = getScanSortedPages();
  const sourceIndex = ordered.findIndex(page => page.id === pageId);
  if (sourceIndex < 0) return;
  const boundedTarget = clampScanValue(targetIndex, 0, ordered.length - 1);
  if (boundedTarget === sourceIndex) return;
  const [moved] = ordered.splice(sourceIndex, 1);
  ordered.splice(boundedTarget, 0, moved);
  ordered.forEach((page, index)=>{
    page.orderIndex = index;
  });
  renderScanPageList();
  renderScanPreview();
}

function selectScanPage(pageId){
  const exists = scanStudioState.pages.some(page => page.id === pageId);
  if (!exists) return;
  scanStudioState.selectedPageId = pageId;
  syncScanControlsFromSelectedPage();
  renderScanPageList();
  renderScanPreview();
}

function resetScanModes(){
  scanStudioState.cropMode = false;
  scanStudioState.perspectiveMode = false;
  scanStudioState.cropDraft = null;
  scanStudioState.draggingPerspectiveIndex = -1;
}

function removeScanPage(pageId){
  const index = scanStudioState.pages.findIndex(page => page.id === pageId);
  if (index < 0) return;
  const [removed] = scanStudioState.pages.splice(index, 1);
  if (removed?.originalSrc && removed.originalSrc.startsWith("blob:")){
    try{ URL.revokeObjectURL(removed.originalSrc); }catch{/* noop */}
  }
  scanStudioImageCache.delete(pageId);
  normalizeScanPageOrder();
  if (scanStudioState.selectedPageId === pageId){
    scanStudioState.selectedPageId = scanStudioState.pages[0]?.id || "";
  }
  resetScanModes();
  syncScanControlsFromSelectedPage();
  renderScanPageList();
  renderScanPreview();
}

async function addScanFiles(fileList){
  const files = Array.from(fileList || []);
  if (!files.length){
    setScanStatus("No files selected.");
    return;
  }
  const validFiles = files.filter(isScanImageFile);
  const rejected = files.length - validFiles.length;
  if (!validFiles.length){
    setScanStatus("Only JPG, PNG, or WebP images are supported.", true);
    return;
  }

  setScanStatus("Loading images…");
  const createdPages = [];
  for (const file of validFiles){
    const src = URL.createObjectURL(file);
    try{
      const image = await loadScanImage(src);
      const page = {
        id: createScanId(),
        originalSrc: src,
        rotation: 0,
        crop: null,
        filter: "original",
        brightness: 0,
        contrast: 0,
        orderIndex: scanStudioState.pages.length + createdPages.length,
        fileName: file.name || "image",
        perspective: null,
      };
      scanStudioImageCache.set(page.id, {
        image,
        width: image.naturalWidth || image.width || 0,
        height: image.naturalHeight || image.height || 0,
        mimeType: file.type || "image/png",
      });
      createdPages.push(page);
    }catch(error){
      console.warn("Scan image load failed", error);
      try{ URL.revokeObjectURL(src); }catch{/* noop */}
    }
  }

  if (!createdPages.length){
    setScanStatus("Image load failed.", true);
    return;
  }

  scanStudioState.pages.push(...createdPages);
  normalizeScanPageOrder();
  if (!scanStudioState.selectedPageId){
    scanStudioState.selectedPageId = createdPages[0].id;
  }else{
    scanStudioState.selectedPageId = createdPages[createdPages.length - 1].id;
  }

  renderScanPageList();
  syncScanControlsFromSelectedPage();
  await renderScanPreview();

  const loadedCount = createdPages.length;
  const suffix = loadedCount === 1 ? "" : "s";
  if (rejected > 0){
    setScanStatus(`Loaded ${loadedCount} page${suffix}. Skipped ${rejected} unsupported file${rejected === 1 ? "" : "s"}.`);
	  }else{
	    setScanStatus(`Loaded ${loadedCount} page${suffix}.`);
	  }
	}

	function isScanStudioActive(){
	  return !!(panels.scanstudio && panels.scanstudio.classList.contains("active"));
	}

	function isFileDragEvent(event){
	  const types = Array.from(event.dataTransfer?.types || []);
	  return types.includes("Files");
	}

	function getPotentialGlobalFileDropMode(event){
	  if (!isFileDragEvent(event)) return "";
	  const items = Array.from(event.dataTransfer?.items || []).filter(item => item.kind === "file");
	  if (!items.length) return "scan";
	  let hasImage = false;
	  let hasPdf = false;
	  let hasUnknown = false;
	  items.forEach(item => {
	    const mime = String(item.type || "").toLowerCase();
	    if (!mime){
	      hasUnknown = true;
	    }else if (mime === "application/pdf"){
	      hasPdf = true;
	    }else if (mime === "image/jpeg" || mime === "image/png" || mime === "image/webp"){
	      hasImage = true;
	    }
	  });
	  if (hasPdf) return "pdf";
	  if (hasImage || hasUnknown) return "scan";
	  return "";
	}

	function isPotentialGlobalScanFileDrag(event){
	  return getPotentialGlobalFileDropMode(event) === "scan";
	}

	function getSupportedGlobalScanFiles(fileList){
	  return Array.from(fileList || []).filter(isScanImageFile);
	}

	function getGlobalPdfFile(fileList){
	  return Array.from(fileList || []).find(isPdfFile) || null;
	}

	function resetGlobalScanDragState(){
	  globalScanDropState.dragCounter = 0;
	  if (!globalScanDropState.loading && globalScanDropOverlay){
	    globalScanDropOverlay.hidden = true;
	  }
	}

	function setGlobalScanDropOverlay(visible, options = {}){
	  if (!globalScanDropOverlay) return;
	  const loading = !!options.loading;
	  const mode = options.mode || "scan";
	  globalScanDropState.loading = loading;
	  globalScanDropOverlay.hidden = !visible;
	  if (globalScanDropTitle){
	    globalScanDropTitle.textContent = mode === "pdf"
	      ? (loading ? "Opening Dashboard..." : "Drop to extract on Dashboard")
	      : (loading ? "Opening Scan Studio..." : "Drop to open in Scan Studio");
	  }
	  if (globalScanDropHelper){
	    globalScanDropHelper.textContent = mode === "pdf"
	      ? (loading ? "PDF extraction is starting automatically" : "PDFs will be extracted automatically")
	      : (loading ? "Images are being loaded automatically" : "Images will be loaded automatically");
	  }
	}

	function showGlobalScanDropMessage(message, isError = false){
	  if (!globalScanDropMessage) return;
	  globalScanDropMessage.textContent = message || "";
	  globalScanDropMessage.classList.toggle("error", !!isError);
	  globalScanDropMessage.hidden = !message;
	  if (globalScanDropState.messageTimer){
	    window.clearTimeout(globalScanDropState.messageTimer);
	  }
	  if (message){
	    globalScanDropState.messageTimer = window.setTimeout(()=>{
	      globalScanDropMessage.hidden = true;
	      globalScanDropState.messageTimer = null;
	    }, 3600);
	  }
	}

	async function loadGlobalScanFiles(files){
	  const supported = getSupportedGlobalScanFiles(files);
	  if (!supported.length){
	    setGlobalScanDropOverlay(false);
	    showGlobalScanDropMessage("Unsupported file. Scan Studio accepts JPG, PNG, or WebP images.", true);
	    return;
	  }
	  setGlobalScanDropOverlay(true, { loading: true });
	  try{
	    await addScanFiles(supported);
	  }catch(error){
	    console.error("Global Scan Studio file load failed", error);
	    setScanStatus("Could not load the dropped file.", true);
	    showGlobalScanDropMessage("Could not load the dropped file.", true);
	  }finally{
	    globalScanDropState.pendingFiles = [];
	    globalScanDropState.loading = false;
	    setGlobalScanDropOverlay(false);
	  }
	}

	async function loadGlobalPdfOnDashboard(file){
	  if (!file || !isPdfFile(file)){
	    setGlobalScanDropOverlay(false);
	    showGlobalScanDropMessage("Unsupported PDF drop.", true);
	    return;
	  }
	  setGlobalScanDropOverlay(true, { loading: true, mode: "pdf" });
	  try{
	    activateTab("extract");
	    await new Promise(resolve => requestAnimationFrame(resolve));
	    await handlePdfExtraction(file);
	  }catch(error){
	    console.error("Global Dashboard PDF load failed", error);
	    setStatusMessage("");
	    setErrorMessage("Error: " + (error.message || error));
	    showGlobalScanDropMessage("Could not extract the dropped PDF.", true);
	  }finally{
	    globalScanDropState.loading = false;
	    setGlobalScanDropOverlay(false);
	  }
	}

	function consumePendingGlobalScanDrop(){
	  if (!isScanStudioActive() || !globalScanDropState.pendingFiles.length) return;
	  const files = globalScanDropState.pendingFiles;
	  globalScanDropState.pendingFiles = [];
	  loadGlobalScanFiles(files);
	}

	function shouldIgnoreGlobalScanDropTarget(target){
	  const element = target && target.nodeType === Node.ELEMENT_NODE ? target : target?.parentElement;
	  if (!element || typeof element.closest !== "function") return false;
	  return !!element.closest("#scanDropZone, #pdfDropZone, #pdfEditorDropZone, #tabPdfEditor, input[type='file']");
	}

	function handleGlobalScanDragEnter(event){
	  if (shouldIgnoreGlobalScanDropTarget(event.target)) return;
	  const mode = getPotentialGlobalFileDropMode(event);
	  if (!mode) return;
	  globalScanDropState.dragCounter += 1;
	  setGlobalScanDropOverlay(true, { mode });
	}

	function handleGlobalScanDragOver(event){
	  if (!isFileDragEvent(event)) return;
	  if (shouldIgnoreGlobalScanDropTarget(event.target)) return;
	  event.preventDefault();
	  const mode = getPotentialGlobalFileDropMode(event);
	  if (event.dataTransfer){
	    event.dataTransfer.dropEffect = mode ? "copy" : "none";
	  }
	  if (mode){
	    setGlobalScanDropOverlay(true, { mode });
	  }
	}

	function handleGlobalScanDragLeave(event){
	  if (shouldIgnoreGlobalScanDropTarget(event.target)) return;
	  if (!getPotentialGlobalFileDropMode(event)) return;
	  const leftWindow = event.clientX <= 0
	    || event.clientY <= 0
	    || event.clientX >= window.innerWidth
	    || event.clientY >= window.innerHeight;
	  if (leftWindow){
	    resetGlobalScanDragState();
	    return;
	  }
	  globalScanDropState.dragCounter = Math.max(0, globalScanDropState.dragCounter - 1);
	  if (globalScanDropState.dragCounter === 0){
	    resetGlobalScanDragState();
	  }
	}

	function handleGlobalScanDrop(event){
	  if (!isFileDragEvent(event) || shouldIgnoreGlobalScanDropTarget(event.target)) return;
	  event.preventDefault();
	  resetGlobalScanDragState();
	  const files = Array.from(event.dataTransfer?.files || []);
	  const pdfFile = getGlobalPdfFile(files);
	  if (pdfFile){
	    loadGlobalPdfOnDashboard(pdfFile);
	    return;
	  }
	  const supported = getSupportedGlobalScanFiles(files);
	  if (!supported.length){
	    showGlobalScanDropMessage("Unsupported file. Scan Studio accepts JPG, PNG, or WebP images.", true);
	    return;
	  }
	  if (isScanStudioActive()){
	    loadGlobalScanFiles(supported);
	    return;
	  }
	  globalScanDropState.pendingFiles = supported;
	  setGlobalScanDropOverlay(true, { loading: true });
	  activateTab("scanstudio");
	}

	function initGlobalScanDropRouting(){
	  const registrations = [
	    [window, "dragenter", handleGlobalScanDragEnter],
	    [window, "dragover", handleGlobalScanDragOver],
	    [window, "dragleave", handleGlobalScanDragLeave],
	    [window, "drop", handleGlobalScanDrop],
	    [window, "dragend", resetGlobalScanDragState],
	    [window, "blur", resetGlobalScanDragState],
	    [window, "keydown", event => {
	      if (event.key === "Escape") resetGlobalScanDragState();
	    }],
	  ];
	  registrations.forEach(([target, type, handler])=>{
	    target.addEventListener(type, handler);
	    globalScanDropState.cleanupHandlers.push(()=> target.removeEventListener(type, handler));
	  });
	}

	function cleanupGlobalScanDropRouting(){
	  globalScanDropState.cleanupHandlers.splice(0).forEach(cleanup => cleanup());
	  if (globalScanDropState.messageTimer){
	    window.clearTimeout(globalScanDropState.messageTimer);
	    globalScanDropState.messageTimer = null;
	  }
	  globalScanDropState.pendingFiles = [];
	  resetGlobalScanDragState();
	}

	async function renderSelectedScanPageForExport(){
  const page = getSelectedScanPage();
  if (!page) return null;
  return renderScanPageCanvas(page, {
    maxSide: SCAN_EXPORT_MAX_SIDE,
    ignoreCrop: false,
  });
}

async function renderAllScanPagesForExport(){
  const pages = getScanSortedPages();
  const rendered = [];
  for (const page of pages){
    const result = await renderScanPageCanvas(page, {
      maxSide: SCAN_EXPORT_MAX_SIDE,
      ignoreCrop: false,
    });
    if (result?.outputCanvas){
      rendered.push({ page, canvas: result.outputCanvas });
    }
  }
  return rendered;
}

function scanCanvasToBlob(canvas, type = "image/png", quality = 0.95){
  return new Promise((resolve, reject)=>{
    canvas.toBlob(blob => {
      if (!blob){
        reject(new Error("Failed to generate file."));
        return;
      }
      resolve(blob);
    }, type, quality);
  });
}

function downloadScanBlob(blob, fileName){
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = fileName;
  link.click();
  URL.revokeObjectURL(url);
}

function getScanDownloadBaseName(page){
  const raw = String(page?.fileName || "scan-page").trim();
  const clean = raw.replace(/\.[a-z0-9]+$/i, "").replace(/[^a-z0-9-_]+/gi, "-").replace(/^-+|-+$/g, "");
  return clean || "scan-page";
}

async function exportScanCurrentImage(){
  const page = getSelectedScanPage();
  if (!page){
    setScanStatus("Select a page first.", true);
    return;
  }
  try{
    setScanStatus("Exporting cleaned image…");
    const rendered = await renderSelectedScanPageForExport();
    if (!rendered?.outputCanvas){
      throw new Error("Nothing to export.");
    }
    const blob = await scanCanvasToBlob(rendered.outputCanvas, "image/png", 0.95);
    downloadScanBlob(blob, `${getScanDownloadBaseName(page)}-cleaned.png`);
    setScanStatus("Cleaned image exported.");
  }catch(error){
    setScanStatus(error?.message || "Image export failed.", true);
  }
}

async function exportScanPdf(){
  const renderedPages = await renderAllScanPagesForExport();
  if (!renderedPages.length){
    setScanStatus("No pages to export.", true);
    return;
  }
  try{
    setScanStatus("Building PDF…");
    await ensurePdfLib();
    const { PDFDocument } = window.PDFLib;
    const pdfDoc = await PDFDocument.create();
    const a4Portrait = { width: 595.28, height: 841.89 };
    const a4Landscape = { width: 841.89, height: 595.28 };

    for (const entry of renderedPages){
      const canvas = entry.canvas;
      const imageData = canvas.toDataURL("image/png");
      const embedded = await pdfDoc.embedPng(imageData);
      const imageRatio = embedded.width / embedded.height;
      const pageSize = imageRatio > 1 ? a4Landscape : a4Portrait;
      const page = pdfDoc.addPage([pageSize.width, pageSize.height]);
      const margin = 24;
      const maxWidth = pageSize.width - (margin * 2);
      const maxHeight = pageSize.height - (margin * 2);
      const fitScale = Math.min(maxWidth / embedded.width, maxHeight / embedded.height);
      const drawWidth = embedded.width * fitScale;
      const drawHeight = embedded.height * fitScale;
      const x = (pageSize.width - drawWidth) / 2;
      const y = (pageSize.height - drawHeight) / 2;
      page.drawImage(embedded, { x, y, width: drawWidth, height: drawHeight });
    }

    const bytes = await pdfDoc.save();
    const blob = new Blob([bytes], { type: "application/pdf" });
    downloadScanBlob(blob, "scan-studio.pdf");
    setScanStatus("PDF exported.");
  }catch(error){
    console.error("Scan Studio PDF export failed", error);
    setScanStatus("PDF export failed.", true);
  }
}

async function printScanPages(scope = "current"){
  let pages = [];
  if (scope === "all"){
    pages = await renderAllScanPagesForExport();
  }else{
    const rendered = await renderSelectedScanPageForExport();
    const selected = getSelectedScanPage();
    if (rendered?.outputCanvas && selected){
      pages = [{ page: selected, canvas: rendered.outputCanvas }];
    }
  }
  if (!pages.length){
    setScanStatus("No pages to print.", true);
    return;
  }
  try{
    setScanStatus("Preparing print preview…");
    const images = pages.map(entry => entry.canvas.toDataURL("image/png"));
    const printWindow = window.open("", "_blank");
    if (!printWindow){
      setScanStatus("Allow popups to print.", true);
      return;
    }
    const body = images.map((src, index)=> (
      `<div class="print-page${index < images.length - 1 ? " page-break" : ""}"><img src="${src}" alt="Scan page ${index + 1}" /></div>`
    )).join("");
    printWindow.document.open();
    printWindow.document.write(`<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Scan Studio Print</title>
  <style>
    html,body{margin:0;padding:0;background:#fff}
    .print-page{padding:10mm;box-sizing:border-box}
    .print-page img{width:100%;height:auto;display:block}
    .page-break{page-break-after:always}
  </style>
</head>
<body>${body}</body>
</html>`);
    printWindow.document.close();
    const triggerPrint = ()=>{
      printWindow.focus();
      printWindow.print();
    };
    if (printWindow.document.readyState === "complete"){
      setTimeout(triggerPrint, 50);
    }else{
      printWindow.addEventListener("load", ()=> setTimeout(triggerPrint, 50), { once: true });
    }
    setScanStatus("Print dialog opened.");
  }catch(error){
    console.error("Scan print failed", error);
    setScanStatus("Print failed.", true);
  }
}

function handleScanOverlayPointerDown(event){
  const page = getSelectedScanPage();
  if (!page) return;
  const pointer = getScanPointerNormalized(event);
  if (!pointer) return;

  if (scanStudioState.perspectiveMode){
    page.perspective = normalizeScanPerspective(page.perspective);
    const handleIndex = getScanPerspectiveHandleIndex(pointer, page.perspective);
    if (handleIndex >= 0){
      scanStudioState.draggingPerspectiveIndex = handleIndex;
      scanOverlayCanvas?.setPointerCapture?.(event.pointerId);
      drawScanOverlay();
      return;
    }
  }

  if (scanStudioState.cropMode){
    scanStudioState.cropStart = { x: pointer.x, y: pointer.y };
    scanStudioState.cropDraft = { x: pointer.x, y: pointer.y, width: 0, height: 0 };
    scanOverlayCanvas?.setPointerCapture?.(event.pointerId);
    drawScanOverlay();
  }
}

function handleScanOverlayPointerMove(event){
  const page = getSelectedScanPage();
  if (!page) return;
  const pointer = getScanPointerNormalized(event);
  if (!pointer) return;

  if (scanStudioState.perspectiveMode && scanStudioState.draggingPerspectiveIndex >= 0){
    page.perspective = normalizeScanPerspective(page.perspective);
    page.perspective[scanStudioState.draggingPerspectiveIndex] = {
      x: pointer.x,
      y: pointer.y,
    };
    renderScanPreview();
    return;
  }

  if (scanStudioState.cropMode && scanStudioState.cropStart){
    const start = scanStudioState.cropStart;
    const x = Math.min(start.x, pointer.x);
    const y = Math.min(start.y, pointer.y);
    const width = Math.abs(pointer.x - start.x);
    const height = Math.abs(pointer.y - start.y);
    scanStudioState.cropDraft = { x, y, width, height };
    drawScanOverlay();
  }
}

function finalizeScanOverlayPointer(){
  const page = getSelectedScanPage();
  if (!page) return;

  if (scanStudioState.perspectiveMode && scanStudioState.draggingPerspectiveIndex >= 0){
    scanStudioState.draggingPerspectiveIndex = -1;
    renderScanPreview();
  }

  if (scanStudioState.cropMode && scanStudioState.cropStart){
    const crop = scanStudioState.cropDraft;
    page.crop = isValidScanCrop(crop) ? crop : null;
    scanStudioState.cropStart = null;
    scanStudioState.cropDraft = null;
    renderScanPreview();
  }
}

function handleScanPageListClick(event){
  const actionButton = event.target.closest("[data-scan-action][data-scan-page-id]");
  if (actionButton){
    const action = actionButton.dataset.scanAction;
    const pageId = actionButton.dataset.scanPageId;
    if (!pageId) return;
    if (action === "select"){
      selectScanPage(pageId);
    }else if (action === "delete"){
      removeScanPage(pageId);
      setScanStatus("Page removed.");
    }else if (action === "up"){
      const ordered = getScanSortedPages();
      const index = ordered.findIndex(page => page.id === pageId);
      moveScanPageToIndex(pageId, index - 1);
    }else if (action === "down"){
      const ordered = getScanSortedPages();
      const index = ordered.findIndex(page => page.id === pageId);
      moveScanPageToIndex(pageId, index + 1);
    }
    return;
  }

  const pageNode = event.target.closest("[data-scan-page-id]");
  if (pageNode){
    const pageId = pageNode.dataset.scanPageId;
    if (pageId) selectScanPage(pageId);
  }
}

async function initScanStudio(){
  if (!scanDropZone || !scanFileInput || !scanPageListEl){
    return;
  }

  const prevent = event => {
    event.preventDefault();
    event.stopPropagation();
  };

  ["dragenter", "dragover"].forEach(type => {
    scanDropZone.addEventListener(type, event => {
      prevent(event);
      scanDropZone.classList.add("hover");
      if (event.dataTransfer){
        event.dataTransfer.dropEffect = "copy";
      }
    });
  });
  ["dragleave", "dragend"].forEach(type => {
    scanDropZone.addEventListener(type, event => {
      prevent(event);
      scanDropZone.classList.remove("hover");
    });
  });
  scanDropZone.addEventListener("drop", async event => {
    prevent(event);
    scanDropZone.classList.remove("hover");
    await addScanFiles(event.dataTransfer?.files || []);
  });
  scanDropZone.addEventListener("click", ()=> scanFileInput.click());
  scanFileInput.addEventListener("change", async event => {
    const files = event.target.files;
    event.target.value = "";
    await addScanFiles(files || []);
  });

  scanPageListEl.addEventListener("click", handleScanPageListClick);
  scanPageListEl.addEventListener("dragstart", event => {
    const node = event.target.closest("[data-scan-page-id]");
    if (!node) return;
    scanStudioState.draggingPageId = node.dataset.scanPageId || "";
    if (event.dataTransfer){
      event.dataTransfer.effectAllowed = "move";
      event.dataTransfer.setData("text/plain", scanStudioState.draggingPageId);
    }
  });
  scanPageListEl.addEventListener("dragover", event => {
    event.preventDefault();
    const node = event.target.closest("[data-scan-page-id]");
    clearScanDragHoverState();
    if (node && node.dataset.scanPageId !== scanStudioState.draggingPageId){
      node.classList.add("drag-over");
    }
  });
  scanPageListEl.addEventListener("drop", event => {
    event.preventDefault();
    const targetNode = event.target.closest("[data-scan-page-id]");
    clearScanDragHoverState();
    if (!targetNode) return;
    const targetId = targetNode.dataset.scanPageId;
    const sourceId = scanStudioState.draggingPageId || event.dataTransfer?.getData("text/plain");
    if (!sourceId || !targetId || sourceId === targetId) return;
    const ordered = getScanSortedPages();
    const targetIndex = ordered.findIndex(page => page.id === targetId);
    moveScanPageToIndex(sourceId, targetIndex);
  });
  scanPageListEl.addEventListener("dragend", ()=> {
    scanStudioState.draggingPageId = "";
    clearScanDragHoverState();
  });

  if (scanRotateLeftBtn){
    scanRotateLeftBtn.addEventListener("click", ()=>{
      const page = getSelectedScanPage();
      if (!page) return;
      page.rotation = normalizeScanRotation((page.rotation || 0) - 90);
      renderScanPageList();
      renderScanPreview();
    });
  }
  if (scanRotateRightBtn){
    scanRotateRightBtn.addEventListener("click", ()=>{
      const page = getSelectedScanPage();
      if (!page) return;
      page.rotation = normalizeScanRotation((page.rotation || 0) + 90);
      renderScanPageList();
      renderScanPreview();
    });
  }
  if (scanCropToggleBtn){
    scanCropToggleBtn.addEventListener("click", ()=>{
      const page = getSelectedScanPage();
      if (!page) return;
      scanStudioState.cropMode = !scanStudioState.cropMode;
      if (scanStudioState.cropMode){
        scanStudioState.perspectiveMode = false;
      }
      scanStudioState.cropDraft = null;
      scanStudioState.cropStart = null;
      syncScanControlsFromSelectedPage();
      renderScanPreview();
    });
  }
  if (scanCropResetBtn){
    scanCropResetBtn.addEventListener("click", ()=>{
      const page = getSelectedScanPage();
      if (!page) return;
      page.crop = null;
      scanStudioState.cropDraft = null;
      renderScanPreview();
    });
  }
  if (scanPerspectiveToggleBtn){
    scanPerspectiveToggleBtn.addEventListener("click", ()=>{
      const page = getSelectedScanPage();
      if (!page) return;
      scanStudioState.perspectiveMode = !scanStudioState.perspectiveMode;
      if (scanStudioState.perspectiveMode){
        scanStudioState.cropMode = false;
        page.perspective = normalizeScanPerspective(page.perspective);
      }else{
        scanStudioState.draggingPerspectiveIndex = -1;
      }
      syncScanControlsFromSelectedPage();
      renderScanPreview();
    });
  }
  if (scanPerspectiveResetBtn){
    scanPerspectiveResetBtn.addEventListener("click", ()=>{
      const page = getSelectedScanPage();
      if (!page) return;
      page.perspective = null;
      scanStudioState.draggingPerspectiveIndex = -1;
      renderScanPreview();
    });
  }
  if (scanFilterSelect){
    scanFilterSelect.addEventListener("change", event => {
      const page = getSelectedScanPage();
      if (!page) return;
      const value = String(event.target.value || "original");
      if (!["original", "grayscale", "document", "bw"].includes(value)) return;
      page.filter = value;
      renderScanPageList();
      renderScanPreview();
    });
  }
  if (scanBrightnessInput){
    scanBrightnessInput.addEventListener("input", event => {
      const page = getSelectedScanPage();
      if (!page) return;
      const value = clampScanValue(Number(event.target.value) || 0, -100, 100);
      page.brightness = value;
      updateScanSliderProgress(scanBrightnessInput);
      if (scanBrightnessValue) scanBrightnessValue.textContent = String(Math.round(value));
      renderScanPreview();
    });
  }
  if (scanContrastInput){
    scanContrastInput.addEventListener("input", event => {
      const page = getSelectedScanPage();
      if (!page) return;
      const value = clampScanValue(Number(event.target.value) || 0, -100, 100);
      page.contrast = value;
      updateScanSliderProgress(scanContrastInput);
      if (scanContrastValue) scanContrastValue.textContent = String(Math.round(value));
      renderScanPreview();
    });
  }

  if (scanPrintCurrentBtn){
    scanPrintCurrentBtn.addEventListener("click", ()=> printScanPages("current"));
  }
  if (scanPrintAllBtn){
    scanPrintAllBtn.addEventListener("click", ()=> printScanPages("all"));
  }
  if (scanExportPdfBtn){
    scanExportPdfBtn.addEventListener("click", ()=> exportScanPdf());
  }
  if (scanExportImageBtn){
    scanExportImageBtn.addEventListener("click", ()=> exportScanCurrentImage());
  }

  if (scanOverlayCanvas){
    scanOverlayCanvas.addEventListener("pointerdown", handleScanOverlayPointerDown);
    scanOverlayCanvas.addEventListener("pointermove", handleScanOverlayPointerMove);
    scanOverlayCanvas.addEventListener("pointerup", finalizeScanOverlayPointer);
    scanOverlayCanvas.addEventListener("pointercancel", finalizeScanOverlayPointer);
    scanOverlayCanvas.addEventListener("pointerleave", ()=>{
      if (!scanStudioState.cropStart && scanStudioState.draggingPerspectiveIndex < 0) return;
      finalizeScanOverlayPointer();
    });
  }

  window.addEventListener("resize", ()=>{
    if (panels.scanstudio && panels.scanstudio.classList.contains("active")){
      renderScanPreview();
    }
  });

  renderScanPageList();
  syncScanControlsFromSelectedPage();
  drawEmptyScanPreview("Upload images to begin.");
}

function escapeHtml(value){
  if (value === null || value === undefined) return "";
  return String(value)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function normalizeHeaderForDisplay(raw, enableLPtoG = true){
  const base = (raw || "").trim().replace(/\s+/g, " ") || "(Header not set)";
  if (!enableLPtoG) return base;
  return base.replace(/\bLP\b/gi, "G");
}

function parseDimensionTokens(dimension){
  const dim = (dimension || "").trim();
  if (!dim) return { width: null, height: null, widthDisplay: "?", heightDisplay: "?", invalid: true };
  const compact = dim.replace(/\s+/g, "");
  const match = compact.match(/^(\d{1,5}(?:[.,]\d{1,3})?)(?:[xX×])(\d{1,5}(?:[.,]\d{1,3})?)$/);
  if (match){
    const parseToken = (value)=>{
      const normalized = value.replace(",", ".");
      const parsed = Number(normalized);
      return Number.isFinite(parsed) ? parsed : null;
    };
    const widthNum = parseToken(match[1]);
    const heightNum = parseToken(match[2]);
    return {
      width: widthNum,
      height: heightNum,
      widthDisplay: match[1],
      heightDisplay: match[2],
      invalid: !Number.isFinite(widthNum) || !Number.isFinite(heightNum),
    };
  }
  const tokens = dim.split(/[xX×]/);
  const widthDisplay = (tokens[0] || "?").trim() || "?";
  const heightDisplay = (tokens[1] || "?").trim() || "?";
  return {
    width: null,
    height: null,
    widthDisplay,
    heightDisplay,
    invalid: true,
  };
}

function formatProcessingNumber(value, fallback, separator){
  if (value == null || Number.isNaN(value)){
    return (fallback !== undefined && fallback !== null && String(fallback).length) ? String(fallback) : "?";
  }
  const str = String(value);
  if (separator === "comma"){
    return str.replace(".", ",");
  }
  return str;
}

function formatM2(value, separator){
  const numeric = Number.isFinite(value) ? value : 0;
  const fixed = numeric.toFixed(3);
  return separator === "comma" ? fixed.replace(".", ",") : fixed;
}

function toNumericArea(value){
  if (value == null) return null;
  const normalized = Number(String(value).replace(",", "."));
  return Number.isFinite(normalized) ? normalized : null;
}

function computeGroupArea(rows){
  const safeRows = Array.isArray(rows) ? rows : [];
  let total = 0;
  let hasValue = false;
  safeRows.forEach(row => {
    if (!row) return;
    const candidate = row.__original?.pdfAreaValue ?? row.__original?.area ?? row.area ?? row.area_m2 ?? row.areaM2;
    const areaValue = toNumericArea(candidate);
    if (areaValue != null){
      total += areaValue;
      hasValue = true;
    }
  });
  if (!hasValue) return null;
  return Math.round(total * 1000) / 1000;
}

const DIMENSION_TOLERANCE_MM = 1;
const UNKNOWN_CLIENT_LABEL = "(Unknown Client)";

function normalizeDimensionNumber(value){
  if (typeof value === "number" && Number.isFinite(value)) return value;
  if (typeof value === "string"){
    const normalized = Number(value.replace(",", "."));
    if (Number.isFinite(normalized)) return normalized;
  }
  return null;
}

function decimalPlacesFromDisplay(display){
  if (!display) return 0;
  const normalized = String(display).trim().replace(",", ".");
  if (!normalized.length) return 0;
  const parts = normalized.split(".");
  if (parts.length !== 2) return 0;
  if (!/^\d+$/.test(parts[0]) || !/^\d+$/.test(parts[1])) return 0;
  return parts[1].length;
}

function calculateRepresentativeNumber(values, decimals){
  if (!values.length) return null;
  const precision = Math.min(Number.isFinite(decimals) ? decimals : 0, 3);
  const factor = Math.pow(10, precision);
  if (!factor || !Number.isFinite(factor)) return values[values.length - 1];
  const sum = values.reduce((acc, val)=> acc + val, 0);
  return Math.round((sum / values.length) * factor) / factor;
}

function selectRepresentativeDisplay(displays, numericValue, decimals){
  const cleaned = (displays || [])
    .map(value => String(value || "").trim())
    .filter(Boolean);
  if (cleaned.length){
    let winner = cleaned[0];
    let best = 0;
    const counts = new Map();
    cleaned.forEach(value=>{
      const count = (counts.get(value) || 0) + 1;
      counts.set(value, count);
      if (count > best){
        best = count;
        winner = value;
      }
    });
    return winner;
  }
  if (numericValue != null){
    const precision = Math.min(Number.isFinite(decimals) ? decimals : 0, 3);
    if (precision > 0){
      return numericValue.toFixed(precision);
    }
    return String(Math.round(numericValue));
  }
  return "?";
}

function collapseGroupDimensions(items, tolerance = DIMENSION_TOLERANCE_MM){
  const buckets = [];
  items.forEach(item => {
    const widthNum = normalizeDimensionNumber(item.width);
    const heightNum = normalizeDimensionNumber(item.height);
    const widthKey = (item.widthDisplay || "").trim().toLowerCase();
    const heightKey = (item.heightDisplay || "").trim().toLowerCase();
    const widthDecimals = decimalPlacesFromDisplay(item.widthDisplay);
    const heightDecimals = decimalPlacesFromDisplay(item.heightDisplay);

    let bucket = null;
    for (const candidate of buckets){
      if (
        candidate.hasNumeric &&
        widthNum != null &&
        heightNum != null &&
        Math.abs(candidate.widthReference - widthNum) <= tolerance &&
        Math.abs(candidate.heightReference - heightNum) <= tolerance
      ){
        bucket = candidate;
        break;
      }
      if (
        !candidate.hasNumeric &&
        widthNum == null &&
        heightNum == null &&
        candidate.widthKey &&
        candidate.widthKey === widthKey &&
        candidate.heightKey &&
        candidate.heightKey === heightKey
      ){
        bucket = candidate;
        break;
      }
    }

    if (!bucket){
      bucket = {
        widthValues: [],
        heightValues: [],
        widthDisplays: [],
        heightDisplays: [],
        widthDecimals: 0,
        heightDecimals: 0,
        widthReference: widthNum != null ? widthNum : 0,
        heightReference: heightNum != null ? heightNum : 0,
        hasNumeric: widthNum != null && heightNum != null,
        widthKey: widthKey || null,
        heightKey: heightKey || null,
        qty: 0,
        invalid: false,
        orderIds: new Set(),
        clients: new Set(),
        positions: new Set(),
        originRows: [],
        areaTotal: 0,
      };
      buckets.push(bucket);
    }

    if (widthNum != null){
      bucket.widthValues.push(widthNum);
      bucket.widthDecimals = Math.max(bucket.widthDecimals, widthDecimals);
      bucket.widthReference = calculateRepresentativeNumber(bucket.widthValues, bucket.widthDecimals);
    }
    if (heightNum != null){
      bucket.heightValues.push(heightNum);
      bucket.heightDecimals = Math.max(bucket.heightDecimals, heightDecimals);
      bucket.heightReference = calculateRepresentativeNumber(bucket.heightValues, bucket.heightDecimals);
    }

    if (widthNum != null && heightNum != null){
      bucket.hasNumeric = true;
    }

    if (item.widthDisplay) bucket.widthDisplays.push(item.widthDisplay);
    if (item.heightDisplay) bucket.heightDisplays.push(item.heightDisplay);
    bucket.qty += Number(item.qty || 0);
    bucket.invalid = bucket.invalid || !!item.invalidDimension;
    if (item.orderId) bucket.orderIds.add(item.orderId);
    if (item.client) bucket.clients.add(item.client);
    const positionValue = pickPositionValue(item);
    if (positionValue) bucket.positions.add(positionValue);
    bucket.originRows.push(buildOriginRowPayload(item));
    const areaValue = toNumericArea(item.area ?? item.area_m2 ?? item.areaM2);
    if (areaValue != null){
      bucket.areaTotal += areaValue;
    }
  });

  return buckets.map(bucket => {
    const widthNumeric = bucket.widthValues.length ? calculateRepresentativeNumber(bucket.widthValues, bucket.widthDecimals) : null;
    const heightNumeric = bucket.heightValues.length ? calculateRepresentativeNumber(bucket.heightValues, bucket.heightDecimals) : null;
    const widthDisplay = selectRepresentativeDisplay(bucket.widthDisplays, widthNumeric, bucket.widthDecimals);
    const heightDisplay = selectRepresentativeDisplay(bucket.heightDisplays, heightNumeric, bucket.heightDecimals);
    const orderIdsList = Array.from(bucket.orderIds).filter(Boolean);
    const clientsList = Array.from(bucket.clients).filter(Boolean);
    const positionsList = dedupeAndSortPositions(Array.from(bucket.positions || []));
    const position = positionsList.length === 1
      ? positionsList[0]
      : (positionsList.length ? "(Grouped)" : "");
    const orderId = orderIdsList.join(", ");
    const client = clientsList.length === 1 ? clientsList[0] : (clientsList.length ? "(Mixed)" : "—");
    return {
      width: widthNumeric,
      height: heightNumeric,
      widthDisplay,
      heightDisplay,
      qty: bucket.qty,
      invalid: bucket.invalid,
      orderId,
      client,
      position,
      originRows: bucket.originRows.map(origin => ({ ...origin })),
      positions: positionsList,
      sortWidth: Number.isFinite(widthNumeric) ? widthNumeric : Number.POSITIVE_INFINITY,
      sortHeight: Number.isFinite(heightNumeric) ? heightNumeric : Number.POSITIVE_INFINITY,
      sortWidthKey: String(widthDisplay || "").toLowerCase(),
      sortHeightKey: String(heightDisplay || "").toLowerCase(),
      area: Number.isFinite(bucket.areaTotal) ? Number(bucket.areaTotal.toFixed(3)) : null,
    };
  });
}

function generateMotherSheet(rows, options){
  const {
    restartPerGroup,
    decimalSeparator,
    normalizeLPtoG,
    headerOverrides = {},
    groupDimensions = false,
    mergeAcrossOrders = false,
  } = options;
  const grouped = new Map();
  const orderSet = new Set();
  const clientSet = new Set();
  (rows || []).forEach(row => {
    if (!row) return;
    const key = (row.composition_raw || row.composition || "").trim() || "(Header not set)";
    if (!grouped.has(key)) grouped.set(key, []);
    const normalizedRow = {
      ...row,
      widthDisplay: row.widthDisplay ?? (Number.isFinite(row.width) ? String(row.width) : row.widthDisplay),
      heightDisplay: row.heightDisplay ?? (Number.isFinite(row.height) ? String(row.height) : row.heightDisplay),
    };
    grouped.get(key).push(normalizedRow);
    if (row.orderId) orderSet.add(row.orderId);
    const clientValue = row.client && String(row.client).trim();
    if (clientValue) clientSet.add(clientValue);
  });

  const sortCollapsedEntries = (entries)=>{
    return entries.sort((a, b)=>{
      const aWidthFinite = Number.isFinite(a.sortWidth);
      const bWidthFinite = Number.isFinite(b.sortWidth);
      if (aWidthFinite && bWidthFinite && a.sortWidth !== b.sortWidth){
        return a.sortWidth - b.sortWidth;
      }
      if (aWidthFinite && !bWidthFinite) return -1;
      if (!aWidthFinite && bWidthFinite) return 1;

      const aHeightFinite = Number.isFinite(a.sortHeight);
      const bHeightFinite = Number.isFinite(b.sortHeight);
      if (aHeightFinite && bHeightFinite && a.sortHeight !== b.sortHeight){
        return a.sortHeight - b.sortHeight;
      }
      if (aHeightFinite && !bHeightFinite) return -1;
      if (!aHeightFinite && bHeightFinite) return 1;

      if (a.sortWidthKey !== b.sortWidthKey){
        return a.sortWidthKey.localeCompare(b.sortWidthKey);
      }
      if (a.sortHeightKey !== b.sortHeightKey){
        return a.sortHeightKey.localeCompare(b.sortHeightKey);
      }
      return 0;
    });
  };

  const groups = [];
  let runningIndex = 1;
  let totalLines = 0;

  for (const [raw, items] of grouped.entries()){
    const override = headerOverrides[raw];
    const display = override && override.trim().length ? override.trim() : normalizeHeaderForDisplay(raw, normalizeLPtoG);
    let localIndex = restartPerGroup ? 1 : runningIndex;
    const assignIndex = ()=> restartPerGroup ? localIndex++ : runningIndex++;

    const groupLines = [];
    const sections = [];

    if (mergeAcrossOrders){
      let working = [];
      if (groupDimensions){
        working = collapseGroupDimensions(items, DIMENSION_TOLERANCE_MM);
        sortCollapsedEntries(working);
      }else{
        working = items.map(item => {
          const positionValue = pickPositionValue(item);
          return {
            width: item.width,
            height: item.height,
            widthDisplay: item.widthDisplay,
            heightDisplay: item.heightDisplay,
            qty: item.qty ?? 0,
            invalid: !!item.invalidDimension,
            orderId: item.orderId || "",
            client: item.client && String(item.client).trim().length ? item.client : UNKNOWN_CLIENT_LABEL,
            position: positionValue || "",
            positions: positionValue ? [positionValue] : [],
            sortWidth: Number.isFinite(item.width) ? item.width : Number.POSITIVE_INFINITY,
            sortHeight: Number.isFinite(item.height) ? item.height : Number.POSITIVE_INFINITY,
            sortWidthKey: String(item.widthDisplay || "").toLowerCase(),
            sortHeightKey: String(item.heightDisplay || "").toLowerCase(),
            danko: item.danko || null,
            area: item.area ?? null,
            areaDisplay: item.areaDisplay ?? null,
            areaSource: item.areaSource ?? null,
            areaRaw: item.areaRaw ?? null,
            originRows: [buildOriginRowPayload(item)],
          };
        });
      }

      working.forEach(entry => {
        const idx = assignIndex();
        const line = {
          idx,
          display_no: idx,
          width: entry.width,
          height: entry.height,
          widthDisplay: entry.widthDisplay,
          heightDisplay: entry.heightDisplay,
          qty: entry.qty ?? 0,
          orderId: entry.orderId || "",
          client: entry.client || UNKNOWN_CLIENT_LABEL,
          composition_raw: raw,
          composition_display: display,
          invalid: !!entry.invalid,
          danko: entry.danko || null,
          area: entry.area ?? null,
          areaDisplay: entry.areaDisplay ?? null,
          areaSource: entry.areaSource ?? null,
          areaRaw: entry.areaRaw ?? null,
          position: entry.position || "",
          positions: Array.isArray(entry.positions) && entry.positions.length
            ? entry.positions.slice()
            : (entry.position ? [entry.position] : []),
          originRows: Array.isArray(entry.originRows) && entry.originRows.length
            ? entry.originRows.map(origin => ({ ...origin }))
            : [buildOriginRowPayload(entry)],
        };
        groupLines.push(line);
      });

      if (restartPerGroup) runningIndex = localIndex;
      totalLines += groupLines.length;
      const groupAreaValue = computeGroupArea(groupLines);
      const groupAreaDisplay = groupAreaValue != null ? `${formatM2(groupAreaValue, decimalSeparator)} m²` : null;
      const groupHeaderText = groupAreaDisplay ? `${display} — Area: ${groupAreaDisplay}` : display;
      groups.push({
        raw,
        display,
        headerText: groupHeaderText,
        lines: groupLines,
        sections: null,
        areaValue: groupAreaValue,
        areaDisplay: groupAreaDisplay,
        areaSource: groupAreaDisplay ? "pdf" : null,
      });
      continue;
    }

    const orderBuckets = new Map();
    items.forEach(item => {
      const orderKey = (item.orderId || "").trim() || "—";
      if (!orderBuckets.has(orderKey)) orderBuckets.set(orderKey, []);
      orderBuckets.get(orderKey).push(item);
    });

    for (const [orderKey, orderItems] of orderBuckets.entries()){
      const clientName = orderItems.find(row => row.client && String(row.client).trim().length)?.client || "";
      const clientDisplay = clientName && clientName.trim().length ? clientName.trim() : UNKNOWN_CLIENT_LABEL;
      let entryList = [];
      if (groupDimensions){
        entryList = collapseGroupDimensions(orderItems, DIMENSION_TOLERANCE_MM).map(entry => {
          entry.orderId = orderKey;
          entry.client = clientDisplay;
          return entry;
        });
        sortCollapsedEntries(entryList);
      }else{
        entryList = orderItems.map(item => {
          const positionValue = pickPositionValue(item);
          return {
            width: item.width,
            height: item.height,
            widthDisplay: item.widthDisplay,
            heightDisplay: item.heightDisplay,
            qty: item.qty ?? 0,
            invalid: !!item.invalidDimension,
            orderId: item.orderId || orderKey,
            client: item.client && String(item.client).trim().length ? item.client : clientDisplay,
            position: positionValue || "",
            positions: positionValue ? [positionValue] : [],
            sortWidth: Number.isFinite(item.width) ? item.width : Number.POSITIVE_INFINITY,
            sortHeight: Number.isFinite(item.height) ? item.height : Number.POSITIVE_INFINITY,
            sortWidthKey: String(item.widthDisplay || "").toLowerCase(),
            sortHeightKey: String(item.heightDisplay || "").toLowerCase(),
            danko: item.danko || null,
            area: item.area ?? null,
            areaDisplay: item.areaDisplay ?? null,
            areaSource: item.areaSource ?? null,
            areaRaw: item.areaRaw ?? null,
            originRows: [buildOriginRowPayload(item)],
          };
        });
      }

      const section = {
        orderId: orderKey,
        client: clientDisplay,
        orderHeaderText: `[Order ${orderKey} — ${clientDisplay}]`,
        lines: [],
      };

      entryList.forEach(entry => {
        const idx = assignIndex();
        const line = {
          idx,
          display_no: idx,
          width: entry.width,
          height: entry.height,
          widthDisplay: entry.widthDisplay,
          heightDisplay: entry.heightDisplay,
          qty: entry.qty ?? 0,
          orderId: entry.orderId || orderKey,
          client: entry.client || clientDisplay,
          composition_raw: raw,
          composition_display: display,
          invalid: !!entry.invalid,
          danko: entry.danko || null,
          area: entry.area ?? null,
          areaDisplay: entry.areaDisplay ?? null,
          areaSource: entry.areaSource ?? null,
          areaRaw: entry.areaRaw ?? null,
          position: entry.position || "",
          positions: Array.isArray(entry.positions) && entry.positions.length
            ? entry.positions.slice()
            : (entry.position ? [entry.position] : []),
          originRows: Array.isArray(entry.originRows) && entry.originRows.length
            ? entry.originRows.map(origin => ({ ...origin }))
            : [buildOriginRowPayload(entry)],
        };
        section.lines.push(line);
        groupLines.push(line);
      });
      sections.push(section);
    }

    if (restartPerGroup) runningIndex = localIndex;
    totalLines += groupLines.length;
    const groupAreaValue = computeGroupArea(groupLines);
    const groupAreaDisplay = groupAreaValue != null ? `${formatM2(groupAreaValue, decimalSeparator)} m²` : null;
    const groupHeaderText = groupAreaDisplay ? `${display} — Area: ${groupAreaDisplay}` : display;
    groups.push({
      raw,
      display,
      headerText: groupHeaderText,
      lines: groupLines,
      sections,
      areaValue: groupAreaValue,
      areaDisplay: groupAreaDisplay,
      areaSource: groupAreaDisplay ? "pdf" : null,
    });
  }

  const orders = Array.from(orderSet).filter(Boolean);
  const clients = Array.from(clientSet);
  const clientLabel = clients.length === 1 ? clients[0] : (clients.length ? "(Mixed)" : UNKNOWN_CLIENT_LABEL);
  const today = new Date();
  const headerLine = `Mother Sheet – Client: ${clientLabel || UNKNOWN_CLIENT_LABEL} | Orders: ${orders.length ? orders.join(", ") : "—"} | Date: ${today.toLocaleDateString()}`;

  const linesOut = [headerLine, ""];
  groups.forEach((group, groupIndex) => {
    const groupHeader = group && group.headerText ? group.headerText : ((group && group.display) ? group.display : "(Header not set)");
    linesOut.push(groupHeader);
    if (!mergeAcrossOrders && group.sections && group.sections.length){
      group.sections.forEach((section, sectionIndex) => {
        const orderHeaderText = typeof section.orderHeaderText === "string" && section.orderHeaderText.trim().length
          ? section.orderHeaderText
          : (() => {
              const orderLabel = section.orderId && section.orderId.trim().length ? section.orderId : "—";
              const clientLabelForSection = section.client && section.client.trim().length ? section.client : UNKNOWN_CLIENT_LABEL;
              return `[Order ${orderLabel} — ${clientLabelForSection}]`;
            })();
        linesOut.push(orderHeaderText);
        section.lines.forEach(line => {
          const widthText = formatProcessingNumber(line.width, line.widthDisplay, decimalSeparator);
          const heightText = formatProcessingNumber(line.height, line.heightDisplay, decimalSeparator);
          const qtyText = Number(line.qty || 0);
          const warning = line.invalid ? "  ⚠" : "";
          const approx = line.danko && line.danko.changed ? " ≈" : "";
          linesOut.push(`${line.idx} – ${widthText} × ${heightText}${approx} × ${qtyText}${warning}`);
          if (line.danko && line.danko.changed && line.danko.original){
            linesOut.push(`   (Rounded from ${line.danko.original.replace(/x/g, "×")})`);
          }
        });
        if (sectionIndex < group.sections.length - 1){
          linesOut.push("");
        }
      });
    }else{
      group.lines.forEach(line => {
        const widthText = formatProcessingNumber(line.width, line.widthDisplay, decimalSeparator);
        const heightText = formatProcessingNumber(line.height, line.heightDisplay, decimalSeparator);
        const qtyText = Number(line.qty || 0);
        const warning = line.invalid ? "  ⚠" : "";
        const approx = line.danko && line.danko.changed ? " ≈" : "";
        linesOut.push(`${line.idx} – ${widthText} × ${heightText}${approx} × ${qtyText}${warning}`);
        if (line.danko && line.danko.changed && line.danko.original){
          linesOut.push(`   (Rounded from ${line.danko.original.replace(/x/g, "×")})`);
        }
      });
    }
    if (groupIndex < groups.length - 1){
      linesOut.push("");
    }
  });

  const meta = {
    clientLabel,
    orders,
    date: today,
    rows: totalLines,
    decimalSeparator,
  };

  return {
    text: linesOut.join("\n").trim(),
    groups,
    meta,
  };
}

function convertOrderToProcessingEntry(order){
  const id = Number(order.id) || order.id;
  const orderSource = String(order.source || "pdf").toLowerCase();
  const orderLabel = (Array.isArray(order.order_numbers) && order.order_numbers.length)
    ? order.order_numbers.join(", ")
    : (order.order_number || `#${order.id}`);
  const client = order.client_name || order.clientName || order.client || order.client_hint || "—";
  const rows = (order.rows || []).map((row, idx) => {
    const dimension = row.dimension || (
      row.width_mm != null && row.height_mm != null
        ? `${row.width_mm}x${row.height_mm}`
        : ""
    );
    const parsed = parseDimensionTokens(dimension);
    const areaRaw = row.final_area_m2 ?? row.area_display ?? row.area ?? row.area_m2 ?? row.areaM2 ?? null;
    const areaValue = toNumericArea(areaRaw);
    const areaDisplay = areaRaw != null ? String(areaRaw) : (areaValue != null ? areaValue.toFixed(3) : null);
    const positionValue = [
      row.position,
      row.position_label,
      row.positionLabel,
      row.pos,
      row.position_display,
    ].map(value => (value == null ? "" : String(value).trim())).find(value => value.length) || "";
    return {
      key: row.id || `${order.id}-${idx}`,
      composition_raw: (row.type || row.glass_type || "").trim() || "(Header not set)",
      width: parsed.width,
      height: parsed.height,
      widthDisplay: parsed.widthDisplay,
      heightDisplay: parsed.heightDisplay,
      qty: Number(row.quantity || 0) || 0,
      orderId: orderLabel,
      client,
      position: positionValue,
      invalidDimension: parsed.invalid,
      area: areaValue != null ? areaValue : null,
      areaDisplay,
      areaSource: areaRaw != null ? orderSource : null,
      areaRaw: areaRaw != null ? String(areaRaw) : null,
      source: orderSource,
    };
  });
  return {
    id,
    orderLabel,
    client,
    createdAt: order.created_at || null,
    rows,
  };
}

function toSpacerNumber(value){
  const numeric = Number(value);
  return Number.isFinite(numeric) ? numeric : null;
}

function formatSpacerNumber(value){
  if (!Number.isFinite(value)) return "";
  const rounded = Number(value.toFixed(3));
  return Number.isFinite(rounded) ? String(rounded) : String(value);
}

function syncProcessingStateProcessedRows(){
  const preview = appState.processing.preview || { groups: [] };
  const groups = Array.isArray(preview.groups) ? preview.groups : [];
  const mapPreviewRow = (row, headerText) => {
    const width = toSpacerNumber(row?.width_mm ?? row?.width ?? row?.__original?.width);
    const height = toSpacerNumber(row?.height_mm ?? row?.height ?? row?.__original?.height);
    const quantity = toSpacerNumber(row?.qty ?? row?.quantity);
    const position = row?.position != null ? String(row.position) : "";
    const displayNoRaw = toSpacerNumber(row?.display_no ?? row?.idx);
    const display_no = Number.isFinite(displayNoRaw) ? displayNoRaw : null;
    return {
      position,
      width_mm: width,
      height_mm: height,
      quantity,
      display_no,
      _headerText: headerText,
    };
  };

  const normalizeOrderId = (value) => {
    const text = value != null ? String(value).trim() : "";
    return text || "—";
  };

  const previewGroups = [];
  groups.forEach(group => {
    const groupHeaderText = typeof group?.headerText === "string" ? group.headerText : "";
    const sections = Array.isArray(group?.sections) && group.sections.length ? group.sections : null;
    if (sections){
      sections.forEach(section => {
        const orderHeaderText = typeof section?.orderHeaderText === "string" ? section.orderHeaderText : "";
        const blockHeaderText = [groupHeaderText, orderHeaderText].filter(Boolean).join("\n");
        const sectionRows = Array.isArray(section?.lines) ? section.lines : [];
        const sectionOrderId = normalizeOrderId(section?.orderId);
        previewGroups.push({
          orderId: sectionOrderId,
          headerText: blockHeaderText,
          rows: sectionRows.map(row => mapPreviewRow(row, blockHeaderText)),
        });
      });
      return;
    }

    const groupRows = Array.isArray(group?.lines) ? group.lines : [];
    const orderBuckets = new Map();
    const orderedOrderIds = [];
    groupRows.forEach(row => {
      const rowOrderId = normalizeOrderId(row?.orderId);
      if (!orderBuckets.has(rowOrderId)){
        orderBuckets.set(rowOrderId, []);
        orderedOrderIds.push(rowOrderId);
      }
      orderBuckets.get(rowOrderId).push(row);
    });
    if (!orderedOrderIds.length){
      previewGroups.push({
        orderId: "—",
        headerText: groupHeaderText,
        rows: [],
      });
      return;
    }
    orderedOrderIds.forEach(orderId => {
      const rowsForOrder = orderBuckets.get(orderId) || [];
      previewGroups.push({
        orderId,
        headerText: groupHeaderText,
        rows: rowsForOrder.map(row => mapPreviewRow(row, groupHeaderText)),
      });
    });
  });

  processingState.previewGroups = previewGroups;
  processingState.processedRows = previewGroups.flatMap(group => group.rows || []);
}

function buildHeightGroupsFromRows(rows){
  const heightBuckets = new Map();
  const safeRows = Array.isArray(rows) ? rows : [];
  safeRows.forEach(row => {
    if (!row || typeof row !== "object") return;
    const height = toSpacerNumber(row.height_mm);
    const width = toSpacerNumber(row.width_mm);
    const quantity = toSpacerNumber(row.quantity);
    if (height == null || width == null || quantity == null || quantity <= 0) return;
    if (!heightBuckets.has(height)){
      heightBuckets.set(height, {
        height,
        items: [],
        total_qty: 0,
      });
    }
    const group = heightBuckets.get(height);
    group.items.push({
      position: row.position != null ? String(row.position) : "",
      width_mm: width,
      quantity,
      display_no: Number.isFinite(toSpacerNumber(row.display_no)) ? toSpacerNumber(row.display_no) : null,
    });
    group.total_qty += quantity;
  });

  return Array.from(heightBuckets.values()).sort((a, b) => {
    if (b.total_qty !== a.total_qty){
      return b.total_qty - a.total_qty;
    }
    return a.height - b.height;
  });
}

function buildSpacerSections(previewGroups, mergeAcrossOrders){
  const safePreviewGroups = Array.isArray(previewGroups) ? previewGroups : [];
  if (mergeAcrossOrders){
    const allRows = safePreviewGroups.flatMap(group => {
      const headerText = typeof group?.headerText === "string" ? group.headerText : "";
      const rows = Array.isArray(group?.rows) ? group.rows : [];
      return rows.map(row => ({ ...row, _header: headerText }));
    });
    const groups = buildHeightGroupsFromRows(allRows);
    return groups.length ? [{ headerText: "", showHeader: false, groups }] : [];
  }

  const sections = [];
  safePreviewGroups.forEach(previewGroup => {
    const rows = Array.isArray(previewGroup?.rows) ? previewGroup.rows : [];
    const groups = buildHeightGroupsFromRows(rows);
    if (!groups.length) return;
    const headerRaw = typeof previewGroup?.headerText === "string" ? previewGroup.headerText : "";
    sections.push({
      headerText: headerRaw || "(No header)",
      showHeader: true,
      groups,
    });
  });
  return sections;
}

function renderSeparatedGroup(headerText, rows){
  const groups = buildHeightGroupsFromRows(rows);
  if (!groups.length) return null;
  return {
    headerText: headerText || "(No header)",
    showHeader: true,
    groups,
  };
}

function buildPerOrderSections(previewGroups){
  const safePreviewGroups = Array.isArray(previewGroups) ? previewGroups : [];
  const buckets = [];
  const bucketMap = new Map();
  safePreviewGroups.forEach(group => {
    if (!group || typeof group !== "object") return;
    const orderId = (group.orderId != null ? String(group.orderId).trim() : "") || "—";
    if (!bucketMap.has(orderId)){
      const bucket = { orderId, groups: [] };
      bucketMap.set(orderId, bucket);
      buckets.push(bucket);
    }
    bucketMap.get(orderId).groups.push(group);
  });

  const sections = [];
  buckets.forEach(bucket => {
    const groupSections = [];
    bucket.groups.forEach(group => {
      const headerText = typeof group?.headerText === "string" ? group.headerText : "";
      const rows = Array.isArray(group?.rows) ? group.rows : [];
      const rendered = renderSeparatedGroup(headerText, rows);
      if (rendered){
        groupSections.push({
          ...rendered,
          kind: "previewGroup",
          orderId: bucket.orderId,
        });
      }
    });
    if (!groupSections.length) return;
    const hasExplicitOrderLine = groupSections.some(section => /\[Order[^\]]+\]/.test(section.headerText || ""));
    if (!hasExplicitOrderLine){
      sections.push({
        kind: "orderDivider",
        orderId: bucket.orderId,
        headerText: `[Order ${bucket.orderId}]`,
        showHeader: true,
        groups: [],
      });
    }
    sections.push(...groupSections);
  });
  return sections;
}

function computeSpacerOutput(previewGroups, mergeAcrossOrders){
  const safePreviewGroups = Array.isArray(previewGroups) ? previewGroups : [];
  if (!safePreviewGroups.length){
    return {
      sections: [],
      text: "No processed order loaded",
      hasPreviewGroups: false,
      modeLabel: mergeAcrossOrders ? "Mode: Merged" : "Mode: Per-Order",
    };
  }

  let sections = [];
  if (mergeAcrossOrders){
    sections = buildSpacerSections(safePreviewGroups, true);
  }else{
    sections = buildPerOrderSections(safePreviewGroups);
  }

  const text = sections.length
    ? formatSpacerProcessingText(sections)
    : "No valid spacer rows in processed order";

  return {
    sections,
    text,
    hasPreviewGroups: true,
    modeLabel: mergeAcrossOrders ? "Mode: Merged" : "Mode: Per-Order",
  };
}

function formatSpacerProcessingText(sections){
  const safeSections = Array.isArray(sections) ? sections : [];
  if (!safeSections.length) return "";
  const lines = [];
  safeSections.forEach((section, sectionIndex) => {
    if (section.kind === "orderDivider"){
      lines.push(section.headerText || "[Order —]");
      lines.push("");
      return;
    }
    if (section.showHeader){
      lines.push(section.headerText || "(No header)");
      lines.push("");
      lines.push("----------------------------------------");
      lines.push("");
    }

    const groups = Array.isArray(section.groups) ? section.groups : [];
    groups.forEach(group => {
      lines.push(`${formatSpacerNumber(group.height)}x${formatSpacerNumber(group.total_qty)}`);
      lines.push("");
      const items = Array.isArray(group.items) ? group.items : [];
      items.forEach(item => {
        const hasDisplayNo = Number.isFinite(toSpacerNumber(item.display_no));
        const label = hasDisplayNo ? formatSpacerNumber(item.display_no) : String(item.position ?? "");
        lines.push(`${label}-${formatSpacerNumber(item.width_mm)}x${formatSpacerNumber(item.quantity)}`);
      });
      lines.push("");
      lines.push("----------------------------------------");
      lines.push("");
    });

    const next = safeSections[sectionIndex + 1];
    if (next && next.kind === "orderDivider"){
      lines.push("");
    }else if (sectionIndex < safeSections.length - 1){
      lines.push("");
    }
  });
  return lines.join("\n").trim();
}

const SPACER_MM_TO_PT = 2.83464567;
const SPACER_MM_TO_PX = 96 / 25.4;
const SPACER_PT_TO_PX = 96 / 72;
let spacerMeasureRoot = null;
let spacerMeasureCanvasCtx = null;

function getSpacerModeDisplayText(){
  return spacerProcessingState.mergeAcrossOrders ? "Mode: Merged" : "Mode: Per Order";
}

function getSpacerLayoutConfig(){
  const pageWidthMm = 210;
  const pageHeightMm = 297;
  const marginMm = 10;
  const columnGapMm = 8;
  const titleSizePt = 14;
  const headerSizePt = 11;
  const heightSizePt = 13;
  const rowSizePt = 11;
  const modeSizePt = 10;
  const titleLeadingPt = 17;
  const modeLeadingPt = 13;
  const headerLeadingPt = 13.5;
  const heightLeadingPt = 16;
  const rowLeadingPt = 13.5;
  const blankLeadingPt = 13.5;
  const headerRuleTopGapPt = 3;
  const headerRuleBottomGapPt = 8;
  const groupRuleTopGapPt = 3;
  const groupRuleBottomGapPt = 13.5;
  const headerRuleThicknessPt = 1.6;
  const groupRuleThicknessPt = 1.0;
  const titleBlockHeightPt = titleLeadingPt + modeLeadingPt + 4;

  const pageWidthPt = pageWidthMm * SPACER_MM_TO_PT;
  const pageHeightPt = pageHeightMm * SPACER_MM_TO_PT;
  const marginPt = marginMm * SPACER_MM_TO_PT;
  const columnGapPt = columnGapMm * SPACER_MM_TO_PT;
  const columnWidthPt = (pageWidthPt - (marginPt * 2) - columnGapPt) / 2;
  const columnTopPt = pageHeightPt - marginPt;
  const firstPageColumnTopPt = columnTopPt - titleBlockHeightPt;
  const columnHeightPt = columnTopPt - marginPt;
  const firstPageColumnHeightPt = firstPageColumnTopPt - marginPt;

  const pageWidthPx = pageWidthMm * SPACER_MM_TO_PX;
  const pageHeightPx = pageHeightMm * SPACER_MM_TO_PX;
  const marginPx = marginMm * SPACER_MM_TO_PX;
  const columnGapPx = columnGapMm * SPACER_MM_TO_PX;
  const columnWidthPx = (pageWidthPx - (marginPx * 2) - columnGapPx) / 2;
  const titleBlockHeightPx = titleBlockHeightPt * SPACER_PT_TO_PX;
  const columnHeightPx = pageHeightPx - (marginPx * 2);
  const firstPageColumnHeightPx = columnHeightPx - titleBlockHeightPx;

  return {
    pageWidthMm,
    pageHeightMm,
    marginMm,
    columnGapMm,
    titleSizePt,
    headerSizePt,
    heightSizePt,
    rowSizePt,
    modeSizePt,
    titleLeadingPt,
    modeLeadingPt,
    headerLeadingPt,
    heightLeadingPt,
    rowLeadingPt,
    blankLeadingPt,
    headerRuleTopGapPt,
    headerRuleBottomGapPt,
    groupRuleTopGapPt,
    groupRuleBottomGapPt,
    headerRuleThicknessPt,
    groupRuleThicknessPt,
    titleBlockHeightPt,
    pageWidthPt,
    pageHeightPt,
    marginPt,
    columnGapPt,
    columnWidthPt,
    columnTopPt,
    firstPageColumnTopPt,
    columnHeightPt,
    firstPageColumnHeightPt,
    pageWidthPx,
    pageHeightPx,
    marginPx,
    columnGapPx,
    columnWidthPx,
    columnHeightPx,
    firstPageColumnHeightPx,
  };
}

function getSpacerLayoutSignature(){
  const sections = Array.isArray(spacerProcessingState.sections) ? spacerProcessingState.sections : [];
  return JSON.stringify({
    mergeAcrossOrders: !!spacerProcessingState.mergeAcrossOrders,
    sections,
  });
}

function ensureSpacerMeasureRoot(){
  if (spacerMeasureRoot && document.body.contains(spacerMeasureRoot)){
    return spacerMeasureRoot;
  }
  spacerMeasureRoot = document.createElement("div");
  spacerMeasureRoot.className = "spacer-measure-root";
  document.body.appendChild(spacerMeasureRoot);
  return spacerMeasureRoot;
}

function getSpacerMeasureCanvasCtx(){
  if (spacerMeasureCanvasCtx){
    return spacerMeasureCanvasCtx;
  }
  const canvas = document.createElement("canvas");
  spacerMeasureCanvasCtx = canvas.getContext("2d");
  return spacerMeasureCanvasCtx;
}

function measureSpacerTextWidthPx(text, fontSizePt, fontWeight = 400){
  const ctx = getSpacerMeasureCanvasCtx();
  if (!ctx){
    return String(text || "").length * (fontSizePt * SPACER_PT_TO_PX * 0.6);
  }
  const sizePx = fontSizePt * SPACER_PT_TO_PX;
  const weight = Number(fontWeight) >= 600 ? "700" : "400";
  ctx.font = `${weight} ${sizePx}px "Courier New", Courier, monospace`;
  return ctx.measureText(String(text || "")).width;
}

function wrapSpacerLineToWidthPx(line, maxWidthPx, fontSizePt, fontWeight = 400){
  const source = String(line ?? "");
  if (!source.length) return [""];
  if (measureSpacerTextWidthPx(source, fontSizePt, fontWeight) <= maxWidthPx){
    return [source];
  }
  const words = source.split(" ");
  if (words.length <= 1){
    const chunks = [];
    let current = "";
    for (const ch of source){
      const candidate = current + ch;
      if (current && measureSpacerTextWidthPx(candidate, fontSizePt, fontWeight) > maxWidthPx){
        chunks.push(current);
        current = ch;
      }else{
        current = candidate;
      }
    }
    if (current) chunks.push(current);
    return chunks.length ? chunks : [source];
  }
  const out = [];
  let current = "";
  words.forEach(word => {
    const candidate = current ? `${current} ${word}` : word;
    if (current && measureSpacerTextWidthPx(candidate, fontSizePt, fontWeight) > maxWidthPx){
      out.push(current);
      current = word;
    }else{
      current = candidate;
    }
  });
  if (current) out.push(current);
  return out.length ? out : [source];
}

function buildSpacerLayoutBlocks(sections, layoutConfig){
  const safeSections = Array.isArray(sections) ? sections : [];
  const blocks = [];
  const pushHeaderBlock = (headerText) => {
    const rawLines = String(headerText ?? "").split(/\r?\n/);
    const wrappedLines = [];
    rawLines.forEach(line => {
      const nextLines = wrapSpacerLineToWidthPx(line, layoutConfig.columnWidthPx, layoutConfig.headerSizePt, 400);
      nextLines.forEach(next => wrappedLines.push(next));
    });
    if (!wrappedLines.length){
      wrappedLines.push("");
    }
    blocks.push({
      type: "header",
      lines: wrappedLines,
      key: `header|${wrappedLines.join("\n")}`,
    });
  };

  safeSections.forEach(section => {
    if (!section || typeof section !== "object") return;
    if (section.kind === "orderDivider"){
      pushHeaderBlock(section.headerText || `[Order ${section.orderId || "—"}]`);
      return;
    }
    if (section.showHeader){
      pushHeaderBlock(section.headerText || "(No header)");
    }
    const groups = Array.isArray(section.groups) ? section.groups : [];
    groups.forEach(group => {
      const heightLine = `${formatSpacerNumber(group.height)} x ${formatSpacerNumber(group.total_qty)}`;
      const items = Array.isArray(group?.items) ? group.items : [];
      const rowLines = items.map(item => {
        const hasDisplayNo = Number.isFinite(toSpacerNumber(item.display_no));
        const label = hasDisplayNo ? formatSpacerNumber(item.display_no) : String(item.position ?? "");
        return `${label} – ${formatSpacerNumber(item.width_mm)} x ${formatSpacerNumber(item.quantity)}`;
      });
      const lines = [
        heightLine,
        "",
        ...rowLines,
        "----------------------------------------",
        "",
      ];
      blocks.push({
        type: "group",
        lines,
        heightLine,
        rowLines,
        key: `group|${lines.join("\n")}`,
      });
    });
  });
  return blocks;
}

function createSpacerBlockElement(block){
  const blockEl = document.createElement("div");
  blockEl.className = "spacer-block";
  if (!block || typeof block !== "object"){
    return blockEl;
  }

  const appendLine = (text, lineClass) => {
    const lineEl = document.createElement("div");
    lineEl.className = `spacer-block-line ${lineClass}`;
    lineEl.textContent = text;
    blockEl.appendChild(lineEl);
  };
  const appendSpacer = (className) => {
    const spacerEl = document.createElement("div");
    spacerEl.className = `spacer-block-spacer ${className}`;
    blockEl.appendChild(spacerEl);
  };
  const appendRule = (className) => {
    const ruleEl = document.createElement("div");
    ruleEl.className = `spacer-block-rule ${className}`;
    blockEl.appendChild(ruleEl);
  };

  if (block.type === "header"){
    const lines = Array.isArray(block.lines) ? block.lines : [""];
    lines.forEach(line => appendLine(line, "header"));
    appendSpacer("header-rule-top");
    appendRule("header");
    appendSpacer("header-rule-bottom");
    return blockEl;
  }

  if (block.type === "group"){
    appendLine(block.heightLine || "", "height");
    appendSpacer("group-blank");
    const rowLines = Array.isArray(block.rowLines) ? block.rowLines : [];
    rowLines.forEach(line => appendLine(line, "row"));
    appendSpacer("group-rule-top");
    appendRule("group");
    appendSpacer("group-rule-bottom");
    return blockEl;
  }

  return blockEl;
}

function measureSpacerBlockHeightPx(block, layoutConfig, cache){
  const key = `${layoutConfig.columnWidthPx}|${block.key || ""}`;
  if (cache.has(key)){
    return cache.get(key);
  }
  const root = ensureSpacerMeasureRoot();
  root.style.width = `${layoutConfig.columnWidthPx}px`;
  root.replaceChildren();
  const blockEl = createSpacerBlockElement(block);
  root.appendChild(blockEl);
  const measured = blockEl.getBoundingClientRect().height;
  cache.set(key, measured);
  return measured;
}

function paginateSpacerBlocks(blocks, layoutConfig){
  const safeBlocks = Array.isArray(blocks) ? blocks : [];
  if (!safeBlocks.length){
    return [];
  }
  const maxColumnHeight = (pageIndex) => (
    pageIndex === 0
      ? layoutConfig.firstPageColumnHeightPx
      : layoutConfig.columnHeightPx
  );
  const pages = [
    { index: 0, columns: [[], []], usedHeight: [0, 0] },
  ];
  let pageRef = pages[0];
  let columnIndex = 0;
  const measurementCache = new Map();

  const advanceCursor = () => {
    if (columnIndex === 0){
      columnIndex = 1;
      return;
    }
    const nextPage = {
      index: pages.length,
      columns: [[], []],
      usedHeight: [0, 0],
    };
    pages.push(nextPage);
    pageRef = nextPage;
    columnIndex = 0;
  };

  safeBlocks.forEach(block => {
    const blockHeight = measureSpacerBlockHeightPx(block, layoutConfig, measurementCache);
    const currentMax = maxColumnHeight(pageRef.index);
    const currentUsed = pageRef.usedHeight[columnIndex];
    const columnHasContent = currentUsed > 0;
    if (columnHasContent && (currentUsed + blockHeight) > currentMax){
      advanceCursor();
    }
    pageRef.columns[columnIndex].push(block);
    pageRef.usedHeight[columnIndex] += blockHeight;
  });

  return pages.map(page => ({
    index: page.index,
    leftBlocks: page.columns[0],
    rightBlocks: page.columns[1],
  }));
}

function buildSpacerLayoutModel(){
  const layoutConfig = getSpacerLayoutConfig();
  const sections = Array.isArray(spacerProcessingState.sections) ? spacerProcessingState.sections : [];
  const blocks = buildSpacerLayoutBlocks(sections, layoutConfig);
  const pages = paginateSpacerBlocks(blocks, layoutConfig);
  return {
    signature: getSpacerLayoutSignature(),
    title: "Spacer Processing",
    modeText: getSpacerModeDisplayText(),
    config: layoutConfig,
    blocks,
    pages,
  };
}

function ensureSpacerLayoutModel(force = false){
  const signature = getSpacerLayoutSignature();
  if (!force && spacerProcessingState.layoutModel && spacerProcessingState.layoutModel.signature === signature){
    return spacerProcessingState.layoutModel;
  }
  const nextModel = buildSpacerLayoutModel();
  spacerProcessingState.layoutModel = nextModel;
  return nextModel;
}

function renderSpacerWysiwygPreview(layoutModel){
  if (!spacerOutputEl){
    return;
  }
  spacerOutputEl.replaceChildren();
  const safePages = Array.isArray(layoutModel?.pages) ? layoutModel.pages : [];
  if (!safePages.length){
    spacerOutputEl.textContent = spacerProcessingState.text;
    spacerOutputEl.classList.add("empty");
    return;
  }

  const pagesWrap = document.createElement("div");
  pagesWrap.className = "spacer-pages";

  safePages.forEach((page, pageIndex) => {
    const pageEl = document.createElement("div");
    pageEl.className = "spacer-page";

    if (pageIndex === 0){
      const headerEl = document.createElement("div");
      headerEl.className = "spacer-page-header";
      const titleEl = document.createElement("div");
      titleEl.className = "spacer-page-title";
      titleEl.textContent = layoutModel.title || "Spacer Processing";
      headerEl.appendChild(titleEl);
      const modeEl = document.createElement("div");
      modeEl.className = "spacer-page-mode";
      modeEl.textContent = layoutModel.modeText || getSpacerModeDisplayText();
      headerEl.appendChild(modeEl);
      pageEl.appendChild(headerEl);
    }

    const columnsEl = document.createElement("div");
    columnsEl.className = `spacer-page-columns${pageIndex === 0 ? " with-header" : ""}`;
    const leftCol = document.createElement("div");
    leftCol.className = "spacer-page-column left";
    const rightCol = document.createElement("div");
    rightCol.className = "spacer-page-column right";

    const leftBlocks = Array.isArray(page?.leftBlocks) ? page.leftBlocks : [];
    const rightBlocks = Array.isArray(page?.rightBlocks) ? page.rightBlocks : [];
    leftBlocks.forEach(block => {
      leftCol.appendChild(createSpacerBlockElement(block));
    });
    rightBlocks.forEach(block => {
      rightCol.appendChild(createSpacerBlockElement(block));
    });

    columnsEl.appendChild(leftCol);
    columnsEl.appendChild(rightCol);
    pageEl.appendChild(columnsEl);
    pagesWrap.appendChild(pageEl);
  });

  spacerOutputEl.appendChild(pagesWrap);
}

function updateSpacerProcessingUI(){
  const previewGroups = Array.isArray(processingState.previewGroups) ? processingState.previewGroups : [];
  const mergeAcrossOrders = !!spacerProcessingState.mergeAcrossOrders;
  const pdfPreview = !!spacerProcessingState.pdfPreview;
  const liveHtmlPreview = !!spacerProcessingState.liveHtmlPreview;
  if (spacerMergeOrdersToggle && spacerMergeOrdersToggle.checked !== mergeAcrossOrders){
    spacerMergeOrdersToggle.checked = mergeAcrossOrders;
  }
  if (spacerPdfPreviewToggle && spacerPdfPreviewToggle.checked !== pdfPreview){
    spacerPdfPreviewToggle.checked = pdfPreview;
  }
  if (spacerWysiwygToggle && spacerWysiwygToggle.checked !== liveHtmlPreview){
    spacerWysiwygToggle.checked = liveHtmlPreview;
  }
  const output = computeSpacerOutput(previewGroups, mergeAcrossOrders);
  spacerProcessingState.sections = output.sections;
  spacerProcessingState.text = output.text;
  spacerProcessingState.hasPreviewGroups = output.hasPreviewGroups;
  if (spacerModeDebugEl){
    spacerModeDebugEl.textContent = output.modeLabel;
  }
  if (spacerOutputEl){
    const hasSections = spacerProcessingState.sections.length > 0;
    const useWysiwyg = liveHtmlPreview && hasSections;
    spacerOutputEl.classList.toggle("empty", !hasSections);
    spacerOutputEl.classList.toggle("pdf-preview", pdfPreview && !useWysiwyg);
    spacerOutputEl.classList.toggle("wysiwyg", useWysiwyg);
    if (useWysiwyg){
      const layoutModel = ensureSpacerLayoutModel();
      renderSpacerWysiwygPreview(layoutModel);
    }else{
      spacerOutputEl.textContent = spacerProcessingState.text;
    }
  }
}

function rebuildProcessingRows(){
  const combined = [];
  appState.processing.cart.forEach(entry => {
    entry.rows.forEach(row => {
      const clone = { ...row };
      clone.rawDimension = deriveRawDimension(clone);
      combined.push(clone);
    });
  });
  appState.processing.rows = combined.map(row => {
    const clone = { ...row };
    clone.rawDimension = deriveRawDimension(clone);
    const widthNumeric = Number.isFinite(clone.width) ? Number(clone.width) : null;
    const heightNumeric = Number.isFinite(clone.height) ? Number(clone.height) : null;
    const pdfArea = toNumericArea(clone.areaDisplay ?? clone.areaRaw ?? clone.area ?? clone.area_m2 ?? clone.areaM2 ?? null);
    const fallbackArea = (widthNumeric != null && heightNumeric != null)
      ? Number(((widthNumeric * heightNumeric) / 1_000_000).toFixed(3))
      : (Number.isFinite(clone.area) ? Number(clone.area) : null);
    const resolvedArea = pdfArea != null ? pdfArea : fallbackArea;
    const areaDisplay = clone.areaDisplay != null
      ? String(clone.areaDisplay)
      : (clone.areaRaw != null ? String(clone.areaRaw) : (resolvedArea != null ? resolvedArea.toFixed(3) : null));
    clone.__original = {
      width: widthNumeric,
      height: heightNumeric,
      widthDisplay: clone.widthDisplay != null ? String(clone.widthDisplay) : (widthNumeric != null ? String(widthNumeric) : null),
      heightDisplay: clone.heightDisplay != null ? String(clone.heightDisplay) : (heightNumeric != null ? String(heightNumeric) : null),
      area: resolvedArea,
      pdfAreaValue: pdfArea,
      areaDisplay,
      dimension: clone.rawDimension,
    };
    clone.area = resolvedArea != null ? resolvedArea : 0;
    clone.areaDisplay = areaDisplay;
    if (!clone.areaSource && pdfArea != null){
      clone.areaSource = "pdf";
    }
    return clone;
  });
  applyProcessingRoundingToRows();
  const validKeys = new Set(appState.processing.rows.map(row => (row.composition_raw || "").trim() || "(Header not set)"));
  Object.keys(appState.processing.headerOverrides).forEach(key => {
    if (!validKeys.has(key)){
      delete appState.processing.headerOverrides[key];
    }
  });
  recalcProcessingPreview();
  if (!appState.processing.rows.length){
    appState.processing.preview = {
      text: "",
      groups: [],
      meta: { clientLabel: "—", orders: [], date: new Date(), rows: 0, decimalSeparator: appState.processing.options.decimalSeparator },
    };
    setProcessingRoundingMessage("");
  }
  syncProcessingStateProcessedRows();
  updateSpacerProcessingUI();
}

function formatProcessingMeta(meta){
  const dateStr = meta.date ? new Date(meta.date).toLocaleDateString() : new Date().toLocaleDateString();
  const ordersPart = (meta.orders && meta.orders.length) ? meta.orders.join(", ") : "—";
  const clientPart = meta.clientLabel || "—";
  return `Mother Sheet – Client: ${clientPart} | Orders: ${ordersPart} | Date: ${dateStr}`;
}

function buildDankoDisplay(width, height, displayW, displayH, rounded){
  const baseWidth = Number.isFinite(width) ? width : null;
  const baseHeight = Number.isFinite(height) ? height : null;
  if (!rounded || rounded.changed !== true){
    return {
      widthValue: baseWidth,
      heightValue: baseHeight,
      widthDisplay: displayW,
      heightDisplay: displayH,
      text: displayW && displayH ? `${displayW} × ${displayH}` : `${displayW || baseWidth || "?"} × ${displayH || baseHeight || "?"}`,
      tooltip: null,
      changed: false,
      area: baseWidth != null && baseHeight != null ? (baseWidth * baseHeight) / 1_000_000 : null,
    };
  }
  const widthRounded = rounded.w;
  const heightRounded = rounded.h;
  const text = `${widthRounded} × ${heightRounded}`;
  const tooltip = rounded.original ? `Rounded from ${rounded.original.replace(/x/g, "×")}` : null;
  const area = Number.isFinite(widthRounded) && Number.isFinite(heightRounded)
    ? (widthRounded * heightRounded) / 1_000_000
    : (baseWidth != null && baseHeight != null ? (baseWidth * baseHeight) / 1_000_000 : null);
  return {
    widthValue: widthRounded,
    heightValue: heightRounded,
    widthDisplay: String(widthRounded),
    heightDisplay: String(heightRounded),
    text,
    tooltip,
    changed: true,
    area,
  };
}

function deriveRawDimension(row){
  const candidates = [
    row.rawDimension,
    row.dimension,
    row.dimension_display,
    row.dim,
    row.widthDisplay && row.heightDisplay ? `${row.widthDisplay}x${row.heightDisplay}` : null,
    Number.isFinite(row.width) && Number.isFinite(row.height) ? `${Math.round(row.width)}x${Math.round(row.height)}` : null,
  ];
  const found = candidates.find(value => value && String(value).trim().length);
  return found ? String(found).replace(/\s+/g, "") : "";
}

function isProcessingRoundingActive(){
  return !!(appState.processing.options.autoDanko || appState.processing.rounding.manualApplied);
}

function applyProcessingRoundingToRows(){
  const rows = Array.isArray(appState.processing.rows) ? appState.processing.rows : [];
  const active = isProcessingRoundingActive();
  rows.forEach(row => {
    if (!row.__original){
      const dimension = deriveRawDimension(row);
      const widthNumeric = Number.isFinite(row.width) ? Number(row.width) : null;
      const heightNumeric = Number.isFinite(row.height) ? Number(row.height) : null;
      const pdfArea = toNumericArea(row.areaDisplay ?? row.areaRaw ?? row.area ?? row.area_m2 ?? row.areaM2 ?? null);
      const fallbackArea = (widthNumeric != null && heightNumeric != null)
        ? Number(((widthNumeric * heightNumeric) / 1_000_000).toFixed(3))
        : (Number.isFinite(row.area) ? Number(row.area) : null);
      const resolvedArea = pdfArea != null ? pdfArea : fallbackArea;
      const areaDisplay = row.areaDisplay != null
        ? String(row.areaDisplay)
        : (row.areaRaw != null ? String(row.areaRaw) : (resolvedArea != null ? resolvedArea.toFixed(3) : null));
      row.__original = {
        width: widthNumeric,
        height: heightNumeric,
        widthDisplay: row.widthDisplay != null ? String(row.widthDisplay) : (widthNumeric != null ? String(widthNumeric) : null),
        heightDisplay: row.heightDisplay != null ? String(row.heightDisplay) : (heightNumeric != null ? String(heightNumeric) : null),
        area: resolvedArea,
        pdfAreaValue: pdfArea,
        areaDisplay,
        dimension,
      };
    }
    const original = row.__original;
    const originalArea = original.pdfAreaValue != null ? original.pdfAreaValue : original.area;
    const originalAreaDisplay = original.areaDisplay != null
      ? original.areaDisplay
      : (originalArea != null ? originalArea.toFixed(3) : null);
    const dimensionString = original.dimension || deriveRawDimension(row) || "";
    const danko = window.applyDankoRuleToDim(dimensionString);
    const tooltipBase = dimensionString ? dimensionString.replace(/x/g, "×") : "";
    row.danko = {
      ...danko,
      original: danko.original || dimensionString,
      tooltip: danko.changed && tooltipBase ? `Rounded from ${tooltipBase}` : (tooltipBase ? `Original ${tooltipBase}` : ""),
    };
    if (active && danko && danko.changed){
      if (Number.isFinite(danko.w)){
        row.width = danko.w;
        row.widthDisplay = String(danko.w);
      }else{
        row.width = original.width;
        row.widthDisplay = original.widthDisplay;
      }
      if (Number.isFinite(danko.h)){
        row.height = danko.h;
        row.heightDisplay = String(danko.h);
      }else{
        row.height = original.height;
        row.heightDisplay = original.heightDisplay;
      }
      row.danko.changed = true;
    }else{
      row.width = original.width;
      row.height = original.height;
      row.widthDisplay = original.widthDisplay;
      row.heightDisplay = original.heightDisplay;
      if (row.danko){
        row.danko.changed = false;
      }
    }
    row.area = originalArea != null ? originalArea : (Number.isFinite(row.area) ? row.area : 0);
    row.areaDisplay = originalAreaDisplay;
    if (!row.areaSource && original.pdfAreaValue != null){
      row.areaSource = "pdf";
    }
  });
}

function recalcProcessingPreview(){
  const rows = Array.isArray(appState.processing.rows) ? appState.processing.rows : [];
  if (!rows.length){
    appState.processing.preview = {
      text: "",
      groups: [],
      meta: {
        clientLabel: "—",
        orders: [],
        date: new Date(),
        rows: 0,
        decimalSeparator: appState.processing.options.decimalSeparator,
      },
    };
    return;
  }
  const preview = generateMotherSheet(rows, {
    restartPerGroup: appState.processing.options.restartPerGroup,
    decimalSeparator: appState.processing.options.decimalSeparator,
    normalizeLPtoG: appState.processing.options.normalizeLPtoG,
    headerOverrides: appState.processing.headerOverrides,
    groupDimensions: appState.processing.grouped,
    mergeAcrossOrders: appState.processing.options.mergeAcrossOrders,
  });
  appState.processing.preview = preview;
}

function setProcessingRoundingMessage(message){
  appState.processing.rounding.message = message || "";
  if (processingRoundingStatus){
    processingRoundingStatus.textContent = message || "";
  }
}

function updateProcessingRoundingControls(){
  const auto = !!appState.processing.options.autoDanko;
  const active = isProcessingRoundingActive();
  const hasRows = Array.isArray(appState.processing.rows) && appState.processing.rows.length > 0;
  if (processingRoundingBtn){
    if (!hasRows){
      processingRoundingBtn.textContent = "Apply Danko Rounding";
    }else{
      processingRoundingBtn.textContent = active ? "Undo Danko Rounding" : "Apply Danko Rounding";
    }
    processingRoundingBtn.disabled = auto || !hasRows;
  }
  if (processingAutoDankoToggle){
    processingAutoDankoToggle.checked = auto;
  }
  if (processingRoundingStatus){
    const message = appState.processing.rounding.message;
    if (message){
      processingRoundingStatus.textContent = message;
    }else if (auto){
      processingRoundingStatus.textContent = "Auto Danko rounding enabled. ✅";
    }else if (active){
      processingRoundingStatus.textContent = "Rounded values applied using Danko Rule ✅";
    }else{
      processingRoundingStatus.textContent = "";
    }
  }
}

function updateProcessingUI(){
  const listEl = processingOrderList;
  if (processingRestartToggle){
    processingRestartToggle.checked = !!appState.processing.options.restartPerGroup;
  }
  if (processingNormalizeToggle){
    processingNormalizeToggle.checked = !!appState.processing.options.normalizeLPtoG;
  }
  if (processingDecimalSelect){
    processingDecimalSelect.value = appState.processing.options.decimalSeparator === "dot" ? "dot" : "comma";
  }
  if (processingMergeOrdersToggle){
    processingMergeOrdersToggle.checked = !!appState.processing.options.mergeAcrossOrders;
  }
  if (listEl){
    if (!appState.processing.cart.length){
      listEl.classList.add("processing-empty");
      listEl.innerHTML = "No orders added yet.";
    }else{
      listEl.classList.remove("processing-empty");
      listEl.innerHTML = appState.processing.cart.map(entry => {
        const created = entry.createdAt ? formatDate(entry.createdAt) : "—";
        return `<div class="processing-order-item" data-id="${entry.id}">
          <header>
            <span>${escapeHtml(entry.orderLabel)}</span>
            <button class="btn small danger" data-remove-processing="${entry.id}">Remove</button>
          </header>
          <div class="muted small">${escapeHtml(entry.client || "—")} • ${escapeHtml(created)}</div>
        </div>`;
      }).join("");
    }
  }

  if (processingClearBtn){
    processingClearBtn.disabled = !appState.processing.cart.length;
  }

  if (processingGroupBtn){
    const hasRows = appState.processing.rows && appState.processing.rows.length;
    processingGroupBtn.disabled = !hasRows;
    processingGroupBtn.textContent = appState.processing.grouped ? "Ungroup Dimensions" : "Group Dimensions";
  }

  const preview = appState.processing.preview;
  if (processingMetaEl){
    const metaLine = preview.text ? formatProcessingMeta(preview.meta || {}) : "Mother Sheet – Client: — | Orders: — | Date: —";
    const rowsCount = preview.meta && preview.meta.rows ? preview.meta.rows : appState.processing.rows.length;
    processingMetaEl.innerHTML = `<span>${escapeHtml(metaLine)}</span><span class="muted small">Rows: ${rowsCount}</span>`;
  }

  if (processingPreviewEl){
    if (!preview.text){
      processingPreviewEl.textContent = "No orders added yet.";
    }else{
      processingPreviewEl.textContent = preview.text;
    }
  }

  updateProcessingRoundingControls();

  if (processingHeaderSelect){
    const groups = preview.groups || [];
    if (!groups.length){
      processingHeaderSelect.innerHTML = '<option value="" disabled selected>No groups available</option>';
      processingHeaderSelect.disabled = true;
      if (processingHeaderInput){
        processingHeaderInput.value = "";
        processingHeaderInput.disabled = true;
      }
      if (processingHeaderApply) processingHeaderApply.disabled = true;
      if (processingHeaderReset) processingHeaderReset.disabled = true;
      if (processingHeaderHint) processingHeaderHint.hidden = true;
    }else{
      processingHeaderSelect.disabled = false;
      if (processingHeaderInput) processingHeaderInput.disabled = false;
      if (processingHeaderApply) processingHeaderApply.disabled = false;
      if (processingHeaderReset) processingHeaderReset.disabled = false;
      const previousRaw = processingHeaderSelect.value;
      const options = groups.map(group => {
        const override = appState.processing.headerOverrides[group.raw] || "";
        const display = override || group.display || "(Header not set)";
        return `<option value="${escapeHtml(group.raw)}">${escapeHtml(display)}</option>`;
      }).join("");
      processingHeaderSelect.innerHTML = options;
      const selectedRaw = groups.some(group => group.raw === previousRaw) ? previousRaw : groups[0].raw;
      const override = appState.processing.headerOverrides[selectedRaw] || "";
      processingHeaderSelect.value = selectedRaw;
      if (processingHeaderInput){
        const selectedGroup = groups.find(group => group.raw === selectedRaw) || groups[0];
        processingHeaderInput.value = override || selectedGroup.display || "";
      }
      syncProcessingHeaderEditor();
    }
  }

  if (wysiwygToggle && wysiwygPreview){
    if (wysiwygToggle.checked){
      if (processingPreviewEl) processingPreviewEl.hidden = true;
      wysiwygPreview.hidden = false;
      renderMotherSheetHTML(preview);
    }else{
      if (processingPreviewEl) processingPreviewEl.hidden = false;
      wysiwygPreview.hidden = true;
    }
  }
}

function applyDankoRounding(processingSheetId){
  void processingSheetId;
  if (!Array.isArray(appState.processing.rows) || !appState.processing.rows.length){
    return false;
  }
  appState.processing.options.autoDanko = false;
  appState.processing.rounding.manualApplied = true;
  if (processingAutoDankoToggle) processingAutoDankoToggle.checked = false;
  setProcessingRoundingMessage("Rounded values applied using Danko Rule ✅");
  applyProcessingRoundingToRows();
  recalcProcessingPreview();
  updateProcessingUI();
  return true;
}

function groupProcessingDimensions(processingSheetId){
  void processingSheetId;
  if (!Array.isArray(appState.processing.rows) || !appState.processing.rows.length){
    return false;
  }
  appState.processing.grouped = true;
  rebuildProcessingRows();
  updateProcessingUI();
  return true;
}

if (wysiwygToggle){
  wysiwygToggle.addEventListener("change", ()=>{
    const show = !!wysiwygToggle.checked;
    if (show){
      if (processingPreviewEl) processingPreviewEl.hidden = true;
      if (wysiwygPreview){
        wysiwygPreview.hidden = false;
        renderMotherSheetHTML(appState.processing.preview || { groups: [], meta: {} });
      }
    }else{
      if (processingPreviewEl) processingPreviewEl.hidden = false;
      if (wysiwygPreview) wysiwygPreview.hidden = true;
    }
  });
}

function renderMotherSheetHTML(preview){
  if (!wysiwygPreview) return;
  const data = preview && typeof preview === "object" ? preview : { groups: [], meta: {} };
  wysiwygPreview.innerHTML = "";

  const root = document.createElement("div");
  root.id = "motherSheet";
  root.className = "mother-sheet";

  const metaLine = document.createElement("div");
  metaLine.className = "meta-line ms-title";
  metaLine.textContent = formatProcessingMeta(data.meta || {});
  root.appendChild(metaLine);

  const groups = Array.isArray(data.groups) ? data.groups : [];
  const decimalSeparator = data.meta?.decimalSeparator || appState.processing.options.decimalSeparator || "comma";
  if (!groups.length){
    const empty = document.createElement("p");
    empty.className = "muted";
    empty.textContent = "No orders added yet.";
    root.appendChild(empty);
  }else{
    groups.forEach(group => {
      const section = document.createElement("section");

      const title = document.createElement("h2");
      title.className = "ms-section-title ms-group-title";
      const headerText = group && group.headerText
        ? group.headerText
        : ((group && group.display) ? group.display : "(Header not set)");
      title.textContent = headerText;
      section.appendChild(title);

      const subSections = Array.isArray(group.sections) && group.sections.length ? group.sections : null;
      if (subSections){
        subSections.forEach(sub => {
          const orderInfo = document.createElement("div");
          orderInfo.className = "ms-order-info ms-orderline";
          const orderHeaderText = (sub && typeof sub.orderHeaderText === "string" && sub.orderHeaderText.trim().length)
            ? sub.orderHeaderText
            : (() => {
                const orderLabel = sub && sub.orderId ? sub.orderId : "—";
                const clientLabel = sub && sub.client ? sub.client : UNKNOWN_CLIENT_LABEL;
                return `[Order ${orderLabel} — ${clientLabel}]`;
              })();
          orderInfo.textContent = orderHeaderText;
          section.appendChild(orderInfo);
          section.appendChild(buildGroupTable(sub && Array.isArray(sub.lines) ? sub.lines : [], data));
        });
      }else{
        section.appendChild(buildGroupTable(group && Array.isArray(group.lines) ? group.lines : [], data));
      }

      root.appendChild(section);
    });
  }

  wysiwygPreview.appendChild(root);
  applyAppearanceToNode(root, appearanceSettings || DEFAULT_APPEARANCE_SETTINGS);
}

function buildGroupTable(lines, preview){
  const container = document.createElement("div");
  container.className = "ms-row-list ms-rows";
  const safeLines = Array.isArray(lines) ? lines : [];
  const separator = (preview && preview.meta && preview.meta.decimalSeparator) || appState.processing.options.decimalSeparator;

  if (!safeLines.length){
    const emptyRow = document.createElement("div");
    emptyRow.className = "ms-row muted";
    emptyRow.textContent = "No line items.";
    container.appendChild(emptyRow);
    return container;
  }

  safeLines.forEach(line => {
    const rowEl = document.createElement("div");
    rowEl.className = "ms-row";

    const idxSpan = document.createElement("span");
    idxSpan.className = "ms-row-index";
    idxSpan.textContent = line && line.idx != null ? String(line.idx) : "";
    rowEl.appendChild(idxSpan);

    const textSpan = document.createElement("span");
    textSpan.className = "ms-row-text";
    const widthText = formatProcessingNumber(line?.width, line?.widthDisplay, separator);
    const heightText = formatProcessingNumber(line?.height, line?.heightDisplay, separator);
    const qtyText = line && line.qty != null ? String(line.qty) : "";
    const rounded = line?.danko;

    textSpan.append(" – ", widthText, " × ", heightText);
    if (rounded?.changed){
      const tooltip = rounded.tooltip || `Rounded from ${(rounded.original || "").replace(/x/g, "×")}`;
      textSpan.append(" ");
      const approxSpan = document.createElement("span");
      approxSpan.textContent = "≈";
      approxSpan.title = tooltip;
      textSpan.append(approxSpan);
    }
    textSpan.append(" × ", qtyText);
    if (line?.invalid){
      textSpan.append(" ⚠");
    }

    rowEl.appendChild(textSpan);
    container.appendChild(rowEl);
  });

  return container;
}

function addOrderToProcessing(order){
  const entry = convertOrderToProcessingEntry(order);
  const entryKey = String(entry.id);
  const existingIndex = appState.processing.cart.findIndex(item => String(item.id) === entryKey);
  if (existingIndex >= 0){
    appState.processing.cart.splice(existingIndex, 1, entry);
  }else{
    appState.processing.cart.push(entry);
  }
  rebuildProcessingRows();
  updateProcessingUI();
  updateLabelsUI();
}

function removeOrderFromProcessing(orderId){
  const key = String(orderId);
  appState.processing.cart = appState.processing.cart.filter(entry => String(entry.id) !== key);
  rebuildProcessingRows();
  updateProcessingUI();
  updateLabelsUI();
}

function clearProcessing(){
  appState.processing.cart = [];
  appState.processing.rows = [];
  appState.processing.headerOverrides = {};
  appState.processing.grouped = false;
  appState.processing.preview = { text: "", groups: [], meta: { clientLabel: "—", orders: [], date: new Date(), rows: 0 } };
  syncProcessingStateProcessedRows();
  updateSpacerProcessingUI();
  updateProcessingUI();
  updateLabelsUI();
}

function setLabelsStatus(msg){
  if (labelsStatusEl){
    labelsStatusEl.textContent = msg || "";
  }
}

function deriveLabelDimension(row){
  const candidates = [
    row.dimension,
    row.dimension_display,
    row.dim,
  ];
  for (const candidate of candidates){
    if (candidate && String(candidate).trim().length){
      return String(candidate).trim().replace(/\s+/g, " ");
    }
  }
  const widthDisplay = row.widthDisplay != null ? String(row.widthDisplay).trim() : "";
  const heightDisplay = row.heightDisplay != null ? String(row.heightDisplay).trim() : "";
  const widthNumeric = Number.isFinite(row.width) ? String(row.width) : "";
  const heightNumeric = Number.isFinite(row.height) ? String(row.height) : "";
  if (widthDisplay && heightDisplay){
    return `${widthDisplay} × ${heightDisplay}`;
  }
  if (widthDisplay && heightNumeric){
    return `${widthDisplay} × ${heightNumeric}`;
  }
  if (widthNumeric && heightDisplay){
    return `${widthNumeric} × ${heightDisplay}`;
  }
  if (widthNumeric && heightNumeric){
    return `${widthNumeric} × ${heightNumeric}`;
  }
  if (widthDisplay || widthNumeric){
    return `${widthDisplay || widthNumeric} × ?`;
  }
  if (heightDisplay || heightNumeric){
    return `? × ${heightDisplay || heightNumeric}`;
  }
  return "";
}

function normalizePositionString(value){
  if (value == null) return "";
  return String(value).trim();
}

function pickPositionValue(row){
  if (!row || typeof row !== "object") return "";
  const candidates = [
    row.position,
    row.position_label,
    row.positionLabel,
    row.pos,
    row.position_display,
  ];
  for (const candidate of candidates){
    const normalized = normalizePositionString(candidate);
    if (normalized) return normalized;
  }
  return "";
}

function parsePositionParts(value){
  const normalized = normalizePositionString(value);
  if (!normalized) return { original: "", primary: null, secondary: null };
  const parts = normalized.split("-");
  const parseNumber = (token)=>{
    if (token == null) return null;
    const num = Number(token);
    return Number.isFinite(num) ? num : null;
  };
  return {
    original: normalized,
    primary: parseNumber(parts[0]),
    secondary: parseNumber(parts[1]),
  };
}

function comparePositionStrings(a, b){
  const pa = parsePositionParts(a);
  const pb = parsePositionParts(b);
  const aHasPrimary = pa.primary != null;
  const bHasPrimary = pb.primary != null;
  if (aHasPrimary && bHasPrimary && pa.primary !== pb.primary){
    return pa.primary - pb.primary;
  }
  if (aHasPrimary && !bHasPrimary) return -1;
  if (!aHasPrimary && bHasPrimary) return 1;

  const aHasSecondary = pa.secondary != null;
  const bHasSecondary = pb.secondary != null;
  if (aHasSecondary && bHasSecondary && pa.secondary !== pb.secondary){
    return pa.secondary - pb.secondary;
  }
  if (aHasSecondary && !bHasSecondary) return -1;
  if (!aHasSecondary && bHasSecondary) return 1;

  return pa.original.localeCompare(pb.original);
}

function dedupeAndSortPositions(values){
  if (!Array.isArray(values) || !values.length) return [];
  const seen = new Set();
  const unique = [];
  values.forEach(value=>{
    const normalized = normalizePositionString(value);
    if (!normalized || seen.has(normalized)) return;
    seen.add(normalized);
    unique.push(normalized);
  });
  unique.sort(comparePositionStrings);
  return unique;
}

function summarizePositionsList(values, limit = 6){
  const unique = dedupeAndSortPositions(values);
  if (!unique.length) return "";
  if (unique.length === 1) return unique[0];
  const display = unique.slice(0, limit);
  let summary = display.join(", ");
  const remaining = unique.length - display.length;
  if (remaining > 0){
    summary += ` (+${remaining})`;
  }
  return summary;
}

function collectProcessingLinePositions(line){
  if (!line || typeof line !== "object") return [];
  const bucket = [];
  if (Array.isArray(line.positions) && line.positions.length){
    bucket.push(...line.positions);
  }
  const fallback = pickPositionValue(line);
  if (fallback && fallback !== "(Grouped)") bucket.push(fallback);
  return dedupeAndSortPositions(bucket);
}

function collectRowPositions(row){
  if (!row || typeof row !== "object") return [];
  const bucket = [];
  if (Array.isArray(row.processing_positions) && row.processing_positions.length){
    bucket.push(...row.processing_positions);
  }else if (Array.isArray(row.positions) && row.positions.length){
    bucket.push(...row.positions);
  }
  const fallback = pickPositionValue(row);
  if (fallback) bucket.push(fallback);
  return dedupeAndSortPositions(bucket);
}

function formatProcessingPositionsSummary(row){
  if (!row) return "";
  const preset = normalizePositionString(row.processing_position_summary);
  if (preset) return preset;
  const list = collectRowPositions(row);
  return summarizePositionsList(list);
}

function buildOriginRowPayload(item){
  if (!item) return {
    position: "—",
    quantity: 1,
    orderId: "",
    client: "",
    widthOriginal: null,
    heightOriginal: null,
  };
  const quantityRaw = Number(item.qty ?? item.quantity ?? 0);
  const quantity = Number.isFinite(quantityRaw) && quantityRaw > 0 ? quantityRaw : 1;
  const original = item.__original || {};
  const positionValue = pickPositionValue(item) || "—";
  return {
    position: positionValue,
    positions: positionValue && positionValue !== "(Grouped)" ? [positionValue] : [],
    quantity,
    orderId: (item.orderId && String(item.orderId).trim()) || "",
    client: (item.client && String(item.client).trim()) || "",
    widthOriginal: Number.isFinite(original.width) ? original.width : (Number.isFinite(item.width) ? item.width : null),
    heightOriginal: Number.isFinite(original.height) ? original.height : (Number.isFinite(item.height) ? item.height : null),
    rawDimension: item.rawDimension || original.dimension || "",
  };
}

function sanitizeLabelRow(row){
  if (!row) return null;
  const quantitySources = [
    row.quantity, row.qty, row.Qty, row.QTY, row.qty_total, row.total_qty, row.qtyTotal
  ];
  let quantity = quantitySources.reduce((acc, value)=>{
    if (acc != null) return acc;
    if (value == null) return acc;
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : acc;
  }, null);
  if (!Number.isFinite(quantity) || quantity <= 0) quantity = 1;
  const orderNumber = [
    row.order_number, row.orderNumber, row.order_no, row.orderNo,
    row.order, row.orderId, row.order_id, row.orderLabel, row.order_label
  ].map(value => (value == null ? "" : String(value).trim())).find(value => value.length) || "";
  const type = (row.type || row.composition_display || row.composition_raw || row.glass_type || row.glassType || "").toString().trim();
  const dimension = deriveLabelDimension(row);
  const position = [
    row.position, row.position_label, row.positionLabel, row.pos, row.idx, row.index
  ].map(value => (value == null ? "" : String(value).trim())).find(value => value.length) || "";
  const areaValue = Number(row.area ?? row.computed_area ?? row.computedArea ?? 0);
  const area = Number.isFinite(areaValue) ? areaValue : 0;
  const client = [
    row.client, row.client_name, row.clientName, row.clientLabel, row.client_label, row.client_hint, row.customer
  ].map(value => (value == null ? "" : String(value).trim())).find(value => value.length) || "";
  const normalized = {
    order_number: orderNumber,
    type,
    dimension: dimension.length ? dimension : "—",
    position: position.length ? position : "—",
    quantity,
    area,
    client,
  };
  if (row.source){
    normalized.source = String(row.source);
  }
  const msIndexRaw = row.ms_index ?? row.msIndex ?? row.idx ?? row.index;
  const msIndex = Number(msIndexRaw);
  if (Number.isFinite(msIndex)){
    normalized.ms_index = msIndex;
  }
  if (Array.isArray(row.processing_positions) && row.processing_positions.length){
    normalized.processing_positions = dedupeAndSortPositions(row.processing_positions);
  }
  const summary = normalizePositionString(row.processing_position_summary);
  if (summary){
    normalized.processing_position_summary = summary;
  }
  return normalized;
}

function createLabelJob(source, rows, options = {}){
  if (!rows || !rows.length) return null;
  const normalized = rows.map(sanitizeLabelRow).filter(Boolean);
  if (!normalized.length) return null;
  const totals = computeTotals(normalized);
  const orders = collectOrderNumbers(normalized);
  const clientSet = new Set(normalized.map(row => row.client).filter(Boolean));
  const clientOverride = options.clientOverride && String(options.clientOverride).trim();
  const clientLabel = clientOverride || (clientSet.size === 1 ? Array.from(clientSet)[0] : (clientSet.size ? "(Mixed)" : "—"));
  const sourceKind = options.sourceKind || source;
  const sourceLabel = options.sourceLabel || sourceKind || source;
  const job = {
    id: `lbl-${Date.now()}-${appState.labels.nextId++}`,
    source: sourceKind || sourceLabel || "",
    sourceLabel: sourceLabel || "",
    createdAt: new Date().toISOString(),
    rows: normalized,
    units: totals.units,
    labels: normalized.reduce((acc,row)=> acc + Number(row.quantity || 0), 0),
    area: totals.area,
    orders,
    clientLabel,
  };
  if (options.primaryOrderId){
    job.primaryOrderId = String(options.primaryOrderId);
    job.orderId = job.primaryOrderId;
    if (!job.orders.includes(job.primaryOrderId)){
      job.orders.unshift(job.primaryOrderId);
    }
  }
  if (options.sourceDetail){
    job.sourceDetail = options.sourceDetail;
  }
  return job;
}

function addLabelsJob(source, rows, options = {}){
  const job = createLabelJob(source, rows, options);
  if (!job){
    setLabelsStatus(options.emptyMessage || "No printable rows found.");
    return null;
  }
  appState.labels.jobs.unshift(job);
  const labelCount = Number(job.labels || job.units || 0);
  const labelWord = labelCount === 1 ? "label" : "labels";
  let message = "";
  if (typeof options.formatMessage === "function"){
    message = options.formatMessage(job);
  }
  if (!message){
    message = options.successMessage || `Added ${labelCount} ${labelWord} from ${source}.`;
  }
  setLabelsStatus(message);
  updateLabelsUI();
  return job;
}

function collectProcessingSectionsForLabels(){
  const preview = appState.processing && appState.processing.preview;
  const groups = preview && Array.isArray(preview.groups) ? preview.groups : [];
  if (!groups.length) return [];
  const sections = [];
  const sectionBuckets = new Map();
  const getSectionBucket = (key, defaults = {}) => {
    const normalizedKey = String(key || "").trim() || defaults.key || "Processing";
    if (!sectionBuckets.has(normalizedKey)){
      const bucket = {
        key: normalizedKey,
        orderId: defaults.orderId || normalizedKey,
        client: defaults.client || UNKNOWN_CLIENT_LABEL,
        lines: [],
        sourceGroups: [],
      };
      sectionBuckets.set(normalizedKey, bucket);
      sections.push(bucket);
    }
    return sectionBuckets.get(normalizedKey);
  };
  groups.forEach((group, groupIndex)=>{
    const hasSections = Array.isArray(group.sections) && group.sections.length;
    if (hasSections){
      group.sections.forEach((section, sectionIndex)=>{
        const lines = Array.isArray(section.lines) ? section.lines : [];
        if (!lines.length) return;
        const rawOrder = String(section.orderId || "").trim();
        const fallbackOrder = rawOrder.length
          ? rawOrder
          : `${group.display || group.raw || "Processing"} ${groupIndex + 1}${group.sections.length > 1 ? `.${sectionIndex + 1}` : ""}`;
        const clientLabel = String(section.client || "").trim() || (preview?.meta?.clientLabel) || UNKNOWN_CLIENT_LABEL;
        const bucket = getSectionBucket(fallbackOrder, {
          orderId: fallbackOrder,
          client: clientLabel,
        });
        bucket.lines.push(...lines);
        bucket.sourceGroups.push(group.display || group.raw || `Processing Group ${groupIndex + 1}`);
      });
    }else{
      const lines = Array.isArray(group.lines) ? group.lines : [];
      if (!lines.length) return;
      const groupLabel = group.display || group.raw || `Processing Group ${groupIndex + 1}`;
      const clientLabel = preview?.meta?.clientLabel || UNKNOWN_CLIENT_LABEL;
      sections.push({
        key: `${groupLabel}#${groupIndex + 1}`,
        orderId: groupLabel,
        client: clientLabel,
        lines,
        sourceGroups: [groupLabel],
      });
    }
  });
  return sections;
}

function formatProcessingDimensionLabel(line){
  if (!line) return "—";
  const widthDisplay = line.widthDisplay != null ? String(line.widthDisplay).trim() : "";
  const heightDisplay = line.heightDisplay != null ? String(line.heightDisplay).trim() : "";
  const widthText = widthDisplay.length ? widthDisplay : (Number.isFinite(line.width) ? String(line.width) : "?");
  const heightText = heightDisplay.length ? heightDisplay : (Number.isFinite(line.height) ? String(line.height) : "?");
  return `${widthText || "?"} × ${heightText || "?"}`;
}

function buildProcessingLabelRows(line, orderId, clientLabel){
  if (!line) return [];
  const orderNumber = (line.orderId && String(line.orderId).trim()) || orderId || "—";
  const clientValue = (line.client && String(line.client).trim()) || clientLabel || UNKNOWN_CLIENT_LABEL;
  const qty = Number(line.qty || line.quantity || 0);
  const msIndex = Number(line.idx);
  const rawArea = Number(line.area ?? line.areaValue ?? line.areaRaw);
  const areaValue = Number.isFinite(rawArea) ? rawArea : 0;
  const positionsList = collectProcessingLinePositions(line);
  const fallbackPosition = normalizePositionString(line.position);
  const dimensionLabel = formatProcessingDimensionLabel(line);
  const rowsOut = [];
  const originRows = Array.isArray(line.originRows) && line.originRows.length
    ? line.originRows
    : [{
        position: fallbackPosition && fallbackPosition !== "(Grouped)" ? fallbackPosition : (positionsList[0] || line.position || "—"),
        quantity: line.qty ?? 1,
        orderId: orderNumber,
        client: clientValue,
      }];
  originRows.forEach(origin=>{
    const uniquePositions = dedupeAndSortPositions(origin.positions || [origin.position]);
    const targets = uniquePositions.length
      ? uniquePositions
      : (origin.position && origin.position !== "(Grouped)" ? [origin.position] : []);
    const positionTargets = targets.length ? targets : ["—"];
    const perPositionQtyRaw = Number(origin.quantity);
    const perPositionQty = Number.isFinite(perPositionQtyRaw) && perPositionQtyRaw > 0 ? perPositionQtyRaw : 1;
    positionTargets.forEach(positionValue=>{
      const safePosition = positionValue || "—";
      for (let i = 0; i < perPositionQty; i++){
        rowsOut.push({
          order_number: origin.orderId || orderNumber,
          orderId: origin.orderId || orderNumber,
          client: origin.client || clientValue,
          type: line.composition_display || line.composition_raw || "",
          dimension: dimensionLabel,
          dimension_display: dimensionLabel,
          widthDisplay: line.widthDisplay,
          heightDisplay: line.heightDisplay,
          quantity: 1,
          qty: 1,
          position: safePosition,
          area: areaValue,
          source: "processing",
          ms_index: Number.isFinite(msIndex) ? msIndex : null,
          processing_positions: [safePosition],
        });
      }
    });
  });
  return rowsOut;
}

function addLabelJobsFromProcessing(options = {}){
  const { silent = false } = options;
  const sections = collectProcessingSectionsForLabels();
  if (!sections.length){
    if (!silent) setLabelsStatus("Processing is empty.");
    return { addedJobs: [], skipped: [], totalRows: 0 };
  }

  const existingKeys = new Set(
    (appState.labels.jobs || [])
      .map(job => (job.sourceDetail || job.primaryOrderId || (Array.isArray(job.orders) && job.orders[0]) || "").trim())
      .filter(Boolean)
  );

  const addedJobs = [];
  const skipped = [];
  let totalRows = 0;

  sections.forEach(section => {
    const sectionKey = String(section.key || section.orderId || "").trim();
    if (!sectionKey){
      skipped.push("Unknown order");
      return;
    }
    if (existingKeys.has(sectionKey)){
      skipped.push(sectionKey);
      return;
    }
    const rows = (section.lines || []).reduce((acc, line)=>{
      const rowSet = buildProcessingLabelRows(line, section.orderId, section.client);
      if (Array.isArray(rowSet) && rowSet.length){
        acc.push(...rowSet);
      }
      return acc;
    }, []);
    if (!rows.length){
      skipped.push(`${sectionKey} (no rows)`);
      return;
    }
    const job = createLabelJob("processing", rows, {
      primaryOrderId: sectionKey,
      clientOverride: section.client,
      sourceLabel: "Processing",
      sourceKind: "processing",
      sourceDetail: sectionKey,
    });
    if (!job){
      skipped.push(sectionKey);
      return;
    }
    addedJobs.push(job);
    existingKeys.add(sectionKey);
    totalRows += Array.isArray(job.rows) ? job.rows.length : 0;
  });

  if (!addedJobs.length){
    const reason = skipped.length ? `Skipped: ${skipped.join(", ")}` : "No orders added.";
    if (!silent) setLabelsStatus(reason);
    return { addedJobs: [], skipped, totalRows: 0 };
  }

  addedJobs.reverse().forEach(job => appState.labels.jobs.unshift(job));
  updateLabelsUI();

  const skipNote = skipped.length ? ` Skipped: ${skipped.join(", ")}` : "";
  if (!silent){
    const itemWord = totalRows === 1 ? "label item" : "label items";
    setLabelsStatus(`Added ${totalRows} ${itemWord} from Processing.${skipNote}`);
    if (typeof showProcessingToast === "function"){
      showProcessingToast(`Added ${totalRows} ${itemWord} from Processing`);
    }
  }
  return { addedJobs, skipped, totalRows };
}

function updateLabelsSummary(){
  if (!labelsSummaryEl) return;
  const jobs = appState.labels.jobs || [];
  if (!jobs.length){
    labelsSummaryEl.innerHTML = "";
    return;
  }
  const totalLabels = jobs.reduce((acc, job)=> acc + Number(job.labels || job.units || 0), 0);
  const totalOrders = jobs.reduce((acc, job)=> acc + job.orders.length, 0);
  labelsSummaryEl.innerHTML = `<span class="pill">Jobs: ${jobs.length}</span> <span class="pill">Labels: ${totalLabels}</span> <span class="pill muted">Orders: ${totalOrders}</span>`;
}

function updateLabelsUI(){
  if (labelsFromProcessingBtn){
    const hasProcessing = !!(appState.processing.cart && appState.processing.cart.length);
    labelsFromProcessingBtn.disabled = !hasProcessing;
  }
  if (labelsJobsWrap){
    const jobs = appState.labels.jobs || [];
    if (!jobs.length){
      labelsJobsWrap.innerHTML = '<div class="labels-empty">No label jobs yet. Use the button above to add one.</div>';
    }else{
      let html = `<table><thead><tr>
        <th>#</th>
        <th>Created</th>
        <th>Source</th>
        <th>Orders</th>
        <th>Client</th>
        <th>Labels</th>
        <th>Preview</th>
        <th>Actions</th>
      </tr></thead><tbody>`;
      jobs.forEach((job, index)=>{
        const created = formatDate(job.createdAt);
        const orderSample = job.orders.slice(0,3).join(", ") || "—";
        const extraOrders = job.orders.length > 3 ? ` +${job.orders.length - 3}` : "";
        const previewRow = job.rows[0];
        const previewLines = [];
        if (previewRow){
          const previewOrder = previewRow.order_number || "—";
          const previewDim = previewRow.dimension || "—";
          previewLines.push(`<div class="muted small mono">${escapeHtml(previewOrder)} • ${escapeHtml(previewDim)}</div>`);
          previewLines.push(`<div class="muted small">${escapeHtml(previewRow.type || "—")} – Qty: ${previewRow.quantity}</div>`);
          if (job.rows.length > 1){
            previewLines.push(`<div class="muted small">+${job.rows.length - 1} more line(s)</div>`);
          }
        }
        html += `<tr>
          <td>${index + 1}</td>
          <td>${escapeHtml(created)}</td>
          <td>${escapeHtml(job.sourceLabel || job.source || "—")}</td>
          <td>${escapeHtml(orderSample + extraOrders)}</td>
          <td>${escapeHtml(job.clientLabel || "—")}</td>
          <td>${Number(job.labels || job.units || 0)}</td>
          <td>${previewLines.join("") || "—"}</td>
          <td>
            <div style="display:flex;gap:6px;flex-wrap:wrap">
              <button class="btn small" data-action="labels-download" data-id="${escapeHtml(job.id)}">Download PDF</button>
              <button class="btn small muted" data-action="labels-remove" data-id="${escapeHtml(job.id)}">Remove</button>
            </div>
          </td>
        </tr>`;
      });
      html += "</tbody></table>";
      labelsJobsWrap.innerHTML = html;
    }
  }
  updateLabelsSummary();
}

async function sendOrderToProcessing(orderId, options = {}){
  const order = await fetchOrder(orderId);
  if (!order || !order.rows || !order.rows.length){
    throw new Error("Order has no rows");
  }
  if (!canProcessingStatus(order.status)){
    throw new Error(`Order status ${historyStatusLabel(order.status)} is not eligible for Processing.`);
  }
  addOrderToProcessing(order);
  const {
    skipStatus = false,
    skipTab = false,
    showToast = false,
    statusMessage = "Order added to Processing.",
    toastMessage = "Added to Processing",
  } = options;
  if (showToast){
    showProcessingToast(toastMessage);
  }
  if (!skipStatus){
    setStatusMessage(statusMessage);
  }
  if (!skipTab){
    activateTab("processing");
  }
}

function syncProcessingHeaderEditor(){
  if (!processingHeaderSelect || processingHeaderSelect.disabled) return;
  const raw = processingHeaderSelect.value;
  const groups = appState.processing.preview.groups || [];
  const group = groups.find(item => item.raw === raw);
  if (!group){
    if (processingHeaderInput) processingHeaderInput.value = "";
    if (processingHeaderHint) processingHeaderHint.hidden = true;
    return;
  }
  const override = appState.processing.headerOverrides[raw] || "";
  if (processingHeaderInput) processingHeaderInput.value = override || group.display || "";
  if (processingHeaderHint){
    const invalidLines = group.lines.filter(line => line.invalid).length;
    if (invalidLines){
      processingHeaderHint.hidden = false;
      processingHeaderHint.textContent = `${invalidLines} line(s) have invalid dimensions in this group.`;
    }else{
      processingHeaderHint.hidden = true;
    }
  }
}

async function copyProcessingText(){
  const text = appState.processing.preview.text;
  if (!text){
    setStatusMessage("Nothing to copy.");
    return;
  }
  try{
    await navigator.clipboard.writeText(text);
    setStatusMessage("Mother Sheet copied to clipboard.");
  }catch(error){
    setStatusMessage("Copy failed: " + (error.message || error));
  }
}

async function copySpacerProcessingText(){
  updateSpacerProcessingUI();
  if (!spacerProcessingState.sections.length){
    setStatusMessage(
      spacerProcessingState.hasPreviewGroups
        ? "No valid spacer rows in processed order."
        : "No processed order loaded."
    );
    return;
  }
  try{
    await navigator.clipboard.writeText(spacerProcessingState.text);
    setStatusMessage("Spacer Processing copied to clipboard.");
  }catch(error){
    setStatusMessage("Copy failed: " + (error.message || error));
  }
}

async function exportSpacerProcessingPdf(){
  updateSpacerProcessingUI();
  if (!spacerProcessingState.sections.length){
    setStatusMessage(
      spacerProcessingState.hasPreviewGroups
        ? "No valid spacer rows in processed order."
        : "No processed order loaded."
    );
    return;
  }
  const outputText = String(spacerProcessingState.text || "");
  if (!outputText.trim()){
    setStatusMessage("No valid spacer rows in processed order.");
    return;
  }
  const layoutModel = ensureSpacerLayoutModel();
  const layoutPages = Array.isArray(layoutModel?.pages) ? layoutModel.pages : [];
  if (!layoutPages.length){
    setStatusMessage("No valid spacer rows in processed order.");
    return;
  }

  await ensurePdfLib();
  const { PDFDocument, StandardFonts, rgb } = PDFLib;
  const layoutConfig = layoutModel?.config || getSpacerLayoutConfig();
  const pdfDoc = await PDFDocument.create();
  const boldFont = await pdfDoc.embedFont(StandardFonts.CourierBold);
  const regularFont = await pdfDoc.embedFont(StandardFonts.Courier);
  const drawBlock = (page, block, x, startY) => {
    let cursorY = startY;
    if (!block || typeof block !== "object"){
      return cursorY;
    }
    if (block.type === "header"){
      const lines = Array.isArray(block.lines) ? block.lines : [""];
      lines.forEach(line => {
        page.drawText(line, {
          x,
          y: cursorY,
          size: layoutConfig.headerSizePt,
          font: regularFont,
          color: rgb(0.11, 0.13, 0.17),
        });
        cursorY -= layoutConfig.headerLeadingPt;
      });
      cursorY -= layoutConfig.headerRuleTopGapPt;
      page.drawLine({
        start: { x, y: cursorY },
        end: { x: x + layoutConfig.columnWidthPt, y: cursorY },
        thickness: layoutConfig.headerRuleThicknessPt,
        color: rgb(0.30, 0.33, 0.38),
      });
      cursorY -= layoutConfig.headerRuleBottomGapPt;
      return cursorY;
    }
    if (block.type === "group"){
      page.drawText(block.heightLine || "", {
        x,
        y: cursorY,
        size: layoutConfig.heightSizePt,
        font: boldFont,
        color: rgb(0.08, 0.1, 0.14),
      });
      cursorY -= layoutConfig.heightLeadingPt;
      cursorY -= layoutConfig.blankLeadingPt;
      const rowLines = Array.isArray(block.rowLines) ? block.rowLines : [];
      rowLines.forEach(line => {
        page.drawText(line, {
          x,
          y: cursorY,
          size: layoutConfig.rowSizePt,
          font: regularFont,
          color: rgb(0.08, 0.1, 0.14),
        });
        cursorY -= layoutConfig.rowLeadingPt;
      });
      cursorY -= layoutConfig.groupRuleTopGapPt;
      page.drawLine({
        start: { x, y: cursorY },
        end: { x: x + layoutConfig.columnWidthPt, y: cursorY },
        thickness: layoutConfig.groupRuleThicknessPt,
        color: rgb(0.55, 0.58, 0.63),
      });
      cursorY -= layoutConfig.groupRuleBottomGapPt;
      return cursorY;
    }
    return cursorY;
  };

  const pdfPages = [];
  layoutPages.forEach((layoutPage, pageIndex) => {
    const page = pdfDoc.addPage([layoutConfig.pageWidthPt, layoutConfig.pageHeightPt]);
    pdfPages.push(page);
    if (pageIndex === 0){
      const titleY = layoutConfig.pageHeightPt - layoutConfig.marginPt - layoutConfig.titleSizePt;
      const modeY = titleY - layoutConfig.titleLeadingPt;
      page.drawText("Spacer Processing", {
        x: layoutConfig.marginPt,
        y: titleY,
        size: layoutConfig.titleSizePt,
        font: boldFont,
        color: rgb(0.08, 0.1, 0.14),
      });
      page.drawText(layoutModel.modeText || getSpacerModeDisplayText(), {
        x: layoutConfig.marginPt,
        y: modeY,
        size: layoutConfig.modeSizePt,
        font: regularFont,
        color: rgb(0.35, 0.38, 0.42),
      });
    }

    const renderColumn = (blocks, columnIndex) => {
      const safeBlocks = Array.isArray(blocks) ? blocks : [];
      const x = layoutConfig.marginPt + (columnIndex * (layoutConfig.columnWidthPt + layoutConfig.columnGapPt));
      let cursorY = pageIndex === 0 ? layoutConfig.firstPageColumnTopPt : layoutConfig.columnTopPt;
      safeBlocks.forEach(block => {
        cursorY = drawBlock(page, block, x, cursorY);
      });
    };

    renderColumn(layoutPage?.leftBlocks, 0);
    renderColumn(layoutPage?.rightBlocks, 1);
  });

  const totalPages = pdfPages.length;
  pdfPages.forEach((p, index) => {
    const footer = `Page ${index + 1} / ${totalPages}`;
    const footerSize = 9;
    const footerWidth = regularFont.widthOfTextAtSize(footer, footerSize);
    p.drawText(footer, {
      x: layoutConfig.pageWidthPt - layoutConfig.marginPt - footerWidth,
      y: Math.max(8, layoutConfig.marginPt - 8),
      size: footerSize,
      font: regularFont,
      color: rgb(0.45, 0.48, 0.53),
    });
  });

  const pdfBytes = await pdfDoc.save();
  const blob = new Blob([pdfBytes], { type: "application/pdf" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "spacer-processing.pdf";
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
  setStatusMessage("Spacer Processing PDF exported.");
}

async function buildProcessingPdfBlob(){
  const preview = appState.processing.preview;
  const plainText = typeof preview?.text === "string" ? preview.text : "";
  if (!plainText.trim()){
    throw new Error("Nothing to export.");
  }
  await ensurePdfLib();
  const { PDFDocument, StandardFonts, rgb } = PDFLib;
  const mmToPt = 2.83464567;
  const pdfDoc = await PDFDocument.create();
  const regularFont = await pdfDoc.embedFont(StandardFonts.Helvetica);

  const layout = {
    pageWidth: 210 * mmToPt,
    pageHeight: 297 * mmToPt,
    margin: 12.7 * mmToPt,
    columnGap: 8 * mmToPt,
    fontSize: 11,
  };
  layout.lineHeight = layout.fontSize * 1.3;
  layout.contentWidth = layout.pageWidth - (layout.margin * 2);
  layout.columnWidth = (layout.contentWidth - layout.columnGap) / 2;
  layout.columnTop = layout.pageHeight - layout.margin;
  layout.columnBottom = layout.margin;

  const textColor = rgb(0, 0, 0);
  const toPdfText = value => String(value ?? "")
    .replace(/\u26a0/g, "!")
    .replace(/[^\x09\x0a\x0d\x20-\x7e\u00a0-\u00ff\u2013\u2014\u2018-\u201d\u2022\u2026\u20ac]/g, "?");
  const textWidth = text => regularFont.widthOfTextAtSize(toPdfText(text), layout.fontSize);

  let page = null;
  let columnIndex = 0;
  let cursorY = layout.columnTop - layout.fontSize;

  const currentColumnX = () => layout.margin + (columnIndex * (layout.columnWidth + layout.columnGap));
  const addPage = () => {
    page = pdfDoc.addPage([layout.pageWidth, layout.pageHeight]);
    columnIndex = 0;
    cursorY = layout.columnTop - layout.fontSize;
  };
  const advanceColumn = () => {
    if (columnIndex === 0){
      columnIndex = 1;
      cursorY = layout.columnTop - layout.fontSize;
    }else{
      addPage();
    }
  };
  const ensureLineSpace = () => {
    if (cursorY < layout.columnBottom){
      advanceColumn();
    }
  };
  const wrapLine = line => {
    const text = String(line ?? "");
    if (text === "") return [""];
    const indent = (text.match(/^\s*/) || [""])[0];
    const maxWidth = layout.columnWidth;
    if (textWidth(text) <= maxWidth) return [text];

    const words = text.trimEnd().split(/(\s+)/);
    const wrapped = [];
    let current = "";
    words.forEach(part => {
      if (!part) return;
      const next = current ? `${current}${part}` : part;
      if (current && textWidth(next) > maxWidth){
        wrapped.push(current.trimEnd());
        current = indent && part.trim() ? `${indent}${part.trimStart()}` : part.trimStart();
      }else{
        current = next;
      }
      while (current && textWidth(current) > maxWidth){
        let cut = current.length;
        while (cut > 1 && textWidth(current.slice(0, cut)) > maxWidth){
          cut -= 1;
        }
        wrapped.push(current.slice(0, cut));
        current = `${indent}${current.slice(cut)}`;
      }
    });
    if (current || !wrapped.length){
      wrapped.push(current.trimEnd());
    }
    return wrapped;
  };
  const drawTextLine = line => {
    ensureLineSpace();
    if (line !== ""){
      page.drawText(toPdfText(line), {
        x: currentColumnX(),
        y: cursorY,
        size: layout.fontSize,
        font: regularFont,
        color: textColor,
      });
    }
    cursorY -= layout.lineHeight;
  };

  addPage();
  plainText.split(/\r?\n/).forEach(sourceLine => {
    wrapLine(sourceLine).forEach(drawTextLine);
  });

  const pdfBytes = await pdfDoc.save();
  return new Blob([pdfBytes], { type: "application/pdf" });
}

async function exportProcessingPdf(options = {}){
  const activityId = options.activityId || startBackgroundActivity("Production PDF", {
    type: "production",
    detail: "Building Mother Sheet PDF…",
    openTab: "processing",
    retry: retryId => exportProcessingPdf({ activityId: retryId }),
  });
  try{
    updateBackgroundActivity(activityId, { detail: "Laying out production rows…", progress: 35 });
    const blob = await buildProcessingPdfBlob();
    updateBackgroundActivity(activityId, { detail: "Preparing download…", progress: 88 });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "mother-sheet.pdf";
    document.body.appendChild(a);
    a.click();
    a.remove();
    setStatusMessage("Mother Sheet PDF exported.");
    completeBackgroundActivity(activityId, {
      detail: "Mother Sheet is ready.",
      downloads: [{ label: "Mother Sheet PDF", url }],
      openTab: "processing",
    });
    return { blob, url };
  }catch(error){
    setStatusMessage(error.message || "Nothing to export.");
    failBackgroundActivity(activityId, error, { openTab: "processing" });
    throw error;
  }
}

function exportProcessingCsv(){
  const rowsData = Array.isArray(appState.processing.rows) ? appState.processing.rows : [];
  if (!rowsData.length){
    setStatusMessage("Nothing to export.");
    return;
  }
  const header = ["nr","width","height","qty","composition_display","composition_raw","orderId","client"];
  const rows = [header];
  const separator = appState.processing.options.decimalSeparator;
  rowsData.forEach((row, index) => {
    const original = row.__original || {};
    const widthText = formatProcessingNumber(original.width, original.widthDisplay, separator);
    const heightText = formatProcessingNumber(original.height, original.heightDisplay, separator);
    const compositionRaw = row.composition_raw || "";
    const compositionDisplay = normalizeHeaderForDisplay(compositionRaw, appState.processing.options.normalizeLPtoG);
    rows.push([
      index + 1,
      widthText,
      heightText,
      row.qty ?? "",
      compositionDisplay || compositionRaw,
      compositionRaw,
      row.orderId || "",
      row.client || "",
    ]);
  });
  const csv = rows.map(columns => columns.map(value => {
    const text = String(value ?? "");
    if (/[",\n]/.test(text)){
      return `"${text.replace(/"/g, '""')}"`;
    }
    return text;
  }).join(",")).join("\n");
  const blob = new Blob([csv], { type: "text/csv" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "mother-sheet.csv";
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
  setStatusMessage("Mother Sheet CSV exported.");
}

function renderWarningList(containerId, warnings){
  const container = typeof containerId === "string" ? document.getElementById(containerId) : containerId;
	  if (!container) return;
	  const list = container.querySelector("ul");
	  const seen = new Set();
	  const items = (Array.isArray(warnings) ? warnings : []).filter(item => {
	    const text = String(item || "").trim();
	    if (!text || seen.has(text)) return false;
	    seen.add(text);
	    return true;
	  });
  if (!items.length){
    container.hidden = true;
    if (list) list.innerHTML = "";
    return;
  }
  container.hidden = false;
  if (list){
    list.innerHTML = items.map(item => `<li>${escapeHtml(item)}</li>`).join("");
  }
}

const cssEscape = (window.CSS && typeof window.CSS.escape === "function")
  ? window.CSS.escape.bind(window.CSS)
  : (value) => String(value).replace(/["\\#.;]/g, "\\$&");

function withPreservedFocus(callback, options = {}){
  const { focusKey: explicitKey = null, selection: explicitSelection = null } = options;
  const active = document.activeElement;
  const isEditable = active && active.classList && active.classList.contains("cell") && active.dataset.key;
  let focusKey = explicitKey;
  let selectionRange = explicitSelection;
  if (isEditable && !focusKey){
    focusKey = active.dataset.key;
    selectionRange = [active.selectionStart ?? active.value.length, active.selectionEnd ?? active.value.length];
  }
  const scrollY = window.scrollY;
  callback();
  window.scrollTo(0, scrollY);
  if (focusKey){
    const selector = `[data-key="${cssEscape(focusKey)}"]`;
    const next = document.querySelector(selector);
    if (next){
      next.focus();
      if (selectionRange && typeof next.setSelectionRange === "function"){
        next.setSelectionRange(selectionRange[0], selectionRange[1]);
      }
    }
  }
}

function getRowDiagnostics(row){
  if (!row || typeof row !== "object") return null;
  const diagnostics = row.diagnostics || row.extraction_diagnostics || row.extractionDiagnostics;
  return diagnostics && typeof diagnostics === "object" ? diagnostics : null;
}

function hasActionableDiagnostics(row){
  const diagnostics = getRowDiagnostics(row);
  return diagnostics?.severity === "warning" || diagnostics?.severity === "error";
}

function getDiagnosticIssues(row){
  if (!hasActionableDiagnostics(row)) return [];
  const issues = getRowDiagnostics(row)?.issues;
  return Array.isArray(issues) ? issues : [];
}

function hasDiagnosticIssue(row, code){
  return getDiagnosticIssues(row).some(issue => String(issue?.code || "") === code);
}

function chooseFallbackTargetField(row){
  const codes = getDiagnosticIssues(row).map(issue => String(issue?.code || "").toUpperCase());
  const hasAny = (...items) => items.some(item => codes.includes(item));
  if (hasAny("MISSING_DIMENSION", "INVALID_DIMENSION", "INVALID_DIMENSION_FORMAT", "DIMENSION_OUT_OF_RANGE", "SUSPICIOUS_DIMENSION_SIZE", "AREA_MISMATCH", "POSSIBLE_DIMENSION_OCR_ERROR", "POSSIBLE_DIMENSION_FAMILY_MISMATCH")){
    return "dimension";
  }
  if (hasAny("INVALID_AREA", "MISSING_EXTRACTED_AREA", "INVALID_EXTRACTED_AREA")){
    return "area";
  }
  if (hasAny("MISSING_QUANTITY", "INVALID_QUANTITY")){
    return "quantity";
  }
  if (hasAny("GLASS_TYPE_UNCLEAR")){
    return "type";
  }
  if (hasAny("MISSING_POSITION", "POSITION_WARNING", "EMPTY_POSITION", "DUPLICATE_POSITION")){
    return "position";
  }
  return "dimension";
}

function diagnosticCalculatedArea(row){
  const value = getRowDiagnostics(row)?.computed?.calculated_area;
  if (value == null) return null;
  const numeric = Number(value);
  return Number.isFinite(numeric) ? numeric : null;
}

function clearDiagnosticsForManualEdit(row){
  if (!row || typeof row !== "object") return;
  delete row.diagnostics;
  delete row.extraction_diagnostics;
  delete row.extractionDiagnostics;
  delete row.area_mismatch;
}

function isDiagnosticsRefreshField(field){
  return ["dimension", "quantity", "area", "type", "position"].includes(field);
}

function cleanPosKey(position){
  if (!position) return [0, 0, ""];
  const [main, suffix = ""] = position.split("/");
  const [first, second] = (main || "").split("-");
  return [Number(first) || 0, Number(second) || 0, suffix];
}

function nextPositionForGroup(rows, order){
  const list = rows.filter(r => (r.order_number || "") === order);
  const maxFirst = Math.max(0, ...list.map(r => Number((r.position || "").split("-")[0]) || 0));
  return `${maxFirst + 1}-1`;
}

function recalcParsedTotals(bucket){
  const totals = computeTotals(bucket.rows || []);
  bucket.parsedUnits = totals.units;
  bucket.parsedArea = totals.area;
}

function addRowToGroup(scope, order){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const rows = bucket.rows ? [...bucket.rows] : [];
  const targetOrder = order || "";
  const template = rows.filter(r => (r.order_number || "") === targetOrder);
  const fallback = template.length ? template[template.length - 1] : rows[rows.length - 1] || {};
  const rid = `${scope}-new-${bucket.nextRid++}`;
  const newRow = {
    order_number: targetOrder,
    type: fallback?.type || "",
    dimension: "",
    position: nextPositionForGroup(rows, targetOrder),
    quantity: 1,
    area: 0,
    _rid: rid,
  };
  rows.push(newRow);
  rows.sort((a, b) => {
    const orderCmp = (a.order_number || "").localeCompare(b.order_number || "");
    if (orderCmp !== 0) return orderCmp;
    const keyA = cleanPosKey(a.position || "0-0");
    const keyB = cleanPosKey(b.position || "0-0");
    if (keyA[0] !== keyB[0]) return keyA[0] - keyB[0];
    if (keyA[1] !== keyB[1]) return keyA[1] - keyB[1];
    return keyA[2].localeCompare(keyB[2]);
  });
  bucket.rows = rows;
  bucket.rowWarnings = bucket.rowWarnings || {};
  bucket.rowWarnings[newRow._rid] = [];
  recalcParsedTotals(bucket);
  if (scope === "history"){
    updateHistoryDetailUI();
  }else{
    updateExtractUI();
  }
}

function buildRowDiagnosisContext(scope, index){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const rows = bucket.rows || [];
  const row = rows[index];
  return {
    order_number: row?.order_number || bucket.orderNumber || "",
    client: getClientName(bucket, ""),
    rows_before: rows.slice(Math.max(0, index - 3), index),
    rows_after: rows.slice(index + 1, index + 4),
    order_rows: rows,
    original_order_rows: bucket.originalRows || [],
  };
}

async function requestRowDiagnosis(scope, index){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const rows = bucket.rows || [];
  const row = rows[index];
  if (!row) return;
  const diagnostics = getRowDiagnostics(row);
  const rid = row._rid || String(index);
  bucket.rowDiagnoses = bucket.rowDiagnoses || {};
  if (!hasActionableDiagnostics(row)){
    bucket.rowDiagnoses[rid] = {
      severity: "ok",
      summary: "No issue detected for this row.",
      likely_cause: "Backend diagnostics did not report a row-level warning or error.",
      recommended_action: "MANUAL_REVIEW",
      confidence: 1,
      safe_to_auto_fix: false,
      suggested_fix: null,
    };
    if (scope === "history"){
      updateHistoryDetailUI();
    }else{
      updateExtractUI();
    }
    return;
  }
  bucket.rowDiagnoses[rid] = { loading: true };
  if (scope === "history"){
    updateHistoryDetailUI();
  }else{
    updateExtractUI();
  }
  try{
    const orderId = bucket.draftId || bucket.savedOrderId || bucket.order?.id || null;
    const orderContext = buildRowDiagnosisContext(scope, index);
    const response = await fetch(API_BASE + "/api/extraction/repair-row", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        order_id: orderId == null ? null : String(orderId),
        pdf_id: bucket.source_hash || bucket.order?.source_hash || null,
        row_index: index,
        row,
        diagnostics,
        target_field: chooseFallbackTargetField(row),
        nearby_rows: [...(orderContext.rows_before || []), ...(orderContext.rows_after || [])],
        order_rows: orderContext.order_rows || [],
        order_context: orderContext,
      }),
    });
    if (!response.ok){
      const message = await response.text();
      console.warn("OCR fallback row request failed", { status: response.status, body: message });
      if (response.status === 422){
        throw new Error("Repair diagnosis could not be completed because the request format was invalid.");
      }
      throw new Error(message || `HTTP ${response.status}`);
    }
    bucket.rowDiagnoses[rid] = await response.json();
    bucket.rowDiagnoses[rid].row_key = rid;
  }catch(error){
    bucket.rowDiagnoses[rid] = {
      row_key: rid,
      severity: "error",
      summary: "Diagnosis request failed.",
      likely_cause: error.message || String(error),
      recommended_action: "MANUAL_REVIEW",
      confidence: 0,
      safe_to_auto_fix: false,
      suggested_fix: null,
    };
  }
  if (scope === "history"){
    updateHistoryDetailUI();
  }else{
    updateExtractUI();
  }
}

function coerceSuggestionValue(field, value){
  if (field === "quantity"){
    const parsed = parseInt(value, 10);
    return Number.isFinite(parsed) ? Math.max(0, parsed) : 0;
  }
  if (field === "area"){
    const parsed = parseFloat(value);
    return Number.isFinite(parsed) ? Number(parsed.toFixed(3)) : 0;
  }
  return value == null ? "" : String(value);
}

function acceptRowSuggestion(scope, rowKey){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const rows = bucket.rows || [];
  const row = rows.find(item => String(item._rid || item.id || "") === String(rowKey));
  if (!row) return;
  const suggestion = bucket.rowDiagnoses && bucket.rowDiagnoses[rowKey];
  if (!suggestion || !suggestion.success || suggestion.suggested_value == null) return;
  const field = suggestion.target_field || "dimension";
  row._original_values = row._original_values || {};
  if (!(field in row._original_values)){
    row._original_values[field] = row[field];
  }
  row[field] = coerceSuggestionValue(field, suggestion.suggested_value);
  row._accepted_suggestions = Array.isArray(row._accepted_suggestions) ? row._accepted_suggestions : [];
  row._accepted_suggestions.push({
    field,
    original_value: suggestion.original_value,
    suggested_value: suggestion.suggested_value,
    method: suggestion.method,
    methods_used: suggestion.methods_used || [],
    accepted_at: new Date().toISOString(),
  });
  if ((suggestion.methods_used || []).includes("family_pattern_repair") || suggestion.method === "family_pattern_repair"){
    row.repaired_by_family_pattern = true;
  }
  if ((suggestion.methods_used || []).includes("pattern_repair") || suggestion.method === "pattern_repair"){
    row.repaired_by_pattern_engine = true;
  }
  clearDiagnosticsForManualEdit(row);
  delete bucket.rowDiagnoses[rowKey];
  const index = rows.indexOf(row);
  recalcParsedTotals(bucket);
  if (scope === "history"){
    updateHistoryDetailUI();
  }else{
    updateExtractUI();
  }
  refreshRowDiagnostics(scope, index);
}

function keepOriginalSuggestion(scope, rowKey){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  if (bucket.rowDiagnoses){
    delete bucket.rowDiagnoses[rowKey];
  }
  if (scope === "history"){
    updateHistoryDetailUI();
  }else{
    updateExtractUI();
  }
}

function focusRowField(scope, rowKey, field){
  const selector = `[data-scope="${cssEscape(scope)}"][data-field="${cssEscape(field)}"]`;
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const rows = bucket.rows || [];
  const index = rows.findIndex(item => String(item._rid || item.id || "") === String(rowKey));
  if (index < 0) return;
  const input = [...document.querySelectorAll(selector)].find(item => Number(item.dataset.index) === index);
  if (input){
    input.focus();
    if (typeof input.select === "function") input.select();
  }
}

async function refreshRowDiagnostics(scope, index){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const row = (bucket.rows || [])[index];
  if (!row) return;
  const rid = row._rid || String(index);
  const rowSignature = JSON.stringify({
    dimension: row.dimension || "",
    quantity: row.quantity ?? null,
    area: row.area ?? null,
    type: row.type || "",
    position: row.position || "",
  });
  try{
    const response = await fetch(API_BASE + "/api/extraction/diagnose-row", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        row,
        diagnostics: getRowDiagnostics(row),
        order_context: buildRowDiagnosisContext(scope, index),
      }),
    });
    if (!response.ok) return;
    const result = await response.json();
    const currentRow = (bucket.rows || []).find(item => (item._rid || "") === rid);
    if (!currentRow) return;
    const currentSignature = JSON.stringify({
      dimension: currentRow.dimension || "",
      quantity: currentRow.quantity ?? null,
      area: currentRow.area ?? null,
      type: currentRow.type || "",
      position: currentRow.position || "",
    });
    if (currentSignature !== rowSignature) return;
    if (result?.diagnostics){
      currentRow.diagnostics = result.diagnostics;
    }
    if (bucket.rowDiagnoses && rid){
      delete bucket.rowDiagnoses[rid];
    }
    if (scope === "history"){
      updateHistoryDetailUI();
    }else{
      updateExtractUI();
    }
  }catch(_error){
    // TODO: If backend diagnostics refresh is unavailable, keep existing
    // row diagnostics only; do not duplicate area validation in the browser.
  }
}

function renderRepairEvidenceValue(value){
  if (value == null || value === "") return "";
  if (Array.isArray(value)){
    if (!value.length) return "";
    return `<span class="mono">${escapeHtml(JSON.stringify(value.slice(0, 4)))}</span>`;
  }
  if (typeof value === "object"){
    return `<span class="mono">${escapeHtml(JSON.stringify(value))}</span>`;
  }
  return escapeHtml(String(value));
}

function renderRepairEvidence(evidence){
  if (!evidence || typeof evidence !== "object") return "";
  const lines = [];
  const family = evidence.family_pattern || evidence;
  const pattern = evidence.pattern_repair || evidence;
  const selected = family.selected_candidate || pattern.selected_candidate || null;
  const diagnosticCodes = evidence.diagnostic_codes || family.diagnostic_codes || pattern.diagnostic_codes || [];
  if (diagnosticCodes.length){
    lines.push(`<div><span class="muted">Diagnostics:</span> <span class="mono">${escapeHtml(diagnosticCodes.join(", "))}</span></div>`);
  }
  if (selected && selected.dimension){
    const selectedLabel = family.selected_candidate ? "Family candidate" : "Selected pattern";
    lines.push(`<div><span class="muted">${selectedLabel}:</span> <span class="mono">${escapeHtml(String(selected.dimension))}</span></div>`);
    if (selected.support_count != null){
      lines.push(`<div><span class="muted">Support:</span> ${escapeHtml(String(selected.support_count))} row(s)</div>`);
    }
    if (selected.sources && selected.sources.length){
      lines.push(`<div><span class="muted">Sources:</span> <span class="mono">${escapeHtml(selected.sources.join(", "))}</span></div>`);
    }
    if (selected.count != null){
      lines.push(`<div><span class="muted">Nearby support:</span> ${escapeHtml(String(selected.count))} row(s)</div>`);
    }
    if (selected.calculated_area != null){
      lines.push(`<div><span class="muted">Calculated area:</span> ${escapeHtml(String(selected.calculated_area))}</div>`);
    }
  }
  const ocr = evidence.ocr_fallback || evidence;
  if (ocr.method || evidence.method){
    lines.push(`<div><span class="muted">OCR method:</span> <span class="mono">${escapeHtml(String(ocr.method || evidence.method))}</span></div>`);
  }
  if (ocr.page != null){
    lines.push(`<div><span class="muted">Page:</span> ${escapeHtml(String(ocr.page))}</div>`);
  }
  if (Array.isArray(ocr.pages) && ocr.pages.length > 1){
    lines.push(`<div><span class="muted">Pages checked:</span> <span class="mono">${escapeHtml(ocr.pages.join(", "))}</span></div>`);
  }
  if (ocr.source){
    lines.push(`<div><span class="muted">Source:</span> <span class="mono">${escapeHtml(String(ocr.source))}</span></div>`);
  }
  const evidenceText = ocr.ocr_text || ocr.matched_text || evidence.ocr_text || evidence.matched_text || "";
  if (evidenceText){
    lines.push(`<div><span class="muted">Evidence:</span> <span class="mono">${escapeHtml(String(evidenceText))}</span></div>`);
  }
  if (ocr.page_context){
    lines.push(`<div><span class="muted">Page context:</span> <span class="mono">${escapeHtml(String(ocr.page_context))}</span></div>`);
  }
  if (Array.isArray(ocr.nearby_rows) && ocr.nearby_rows.length){
    lines.push(`<div><span class="muted">Nearby rows:</span> <span class="mono">${escapeHtml(JSON.stringify(ocr.nearby_rows.slice(0, 4)))}</span></div>`);
  }
  if (evidence.nearby_pattern){
    lines.push(`<div><span class="muted">Pattern:</span> ${escapeHtml(String(evidence.nearby_pattern))}</div>`);
  }
  if (!lines.length){
    Object.entries(evidence).slice(0, 4).forEach(([key, value]) => {
      const rendered = renderRepairEvidenceValue(value);
      if (rendered) lines.push(`<div><span class="muted">${escapeHtml(key)}:</span> ${rendered}</div>`);
    });
  }
  return lines.length ? `<div class="diagnosis-evidence">${lines.join("")}</div>` : "";
}

function renderRepairTrace(trace){
  if (!Array.isArray(trace) || !trace.length) return "";
  return `<ol class="diagnosis-trace">${trace.map(step => `<li>${escapeHtml(String(step))}</li>`).join("")}</ol>`;
}

function fixAllAreas(scope){
  setStatusMessage("Use the row Fix button to diagnose one warning at a time. No automatic change was made.");
}

const OCR_REPAIR_CONFIDENCE_MIN = 0.8;
const OCR_FIELD_REPAIR_KEYS = {
  dimension: "dimension_repaired",
  position: "position_repaired",
  quantity: "quantity_repaired",
  type: "type_repaired",
  area: "area_repaired",
};

function criticalFieldLabel(field){
  return field === "type" ? "glass type" : field;
}

function repairKeyForField(field){
  return OCR_FIELD_REPAIR_KEYS[field] || `${field}_repaired`;
}

function isCriticalFieldMissing(row, field){
  if (!row) return true;
  const value = field === "type" ? row.type : row[field];
  if (field === "quantity"){
    if (value == null || String(value).trim() === "") return true;
    const parsed = Number(value);
    return !Number.isFinite(parsed) || parsed <= 0;
  }
  if (field === "area"){
    if (value == null || String(value).trim() === "") return true;
    const parsed = Number(value);
    return !Number.isFinite(parsed) || parsed <= 0;
  }
  return !String(value || "").trim();
}

function missingCriticalFields(row){
  return ["dimension", "position", "quantity", "type", "area"].filter(field => isCriticalFieldMissing(row, field));
}

function normalizeRepairValue(field, value){
  if (field === "quantity"){
    const parsed = parseInt(value, 10);
    return Number.isFinite(parsed) ? parsed : value;
  }
  if (field === "area"){
    const parsed = parseFloat(String(value).replace(",", "."));
    return Number.isFinite(parsed) ? Number(parsed.toFixed(3)) : value;
  }
  return value == null ? "" : String(value).trim();
}

function refreshRowRepairWarning(row){
  if (!row) return;
  const warnings = row.repair_warnings && typeof row.repair_warnings === "object" ? row.repair_warnings : {};
  const activeFields = Object.keys(warnings).filter(field => isCriticalFieldMissing(row, field));
  if (!activeFields.length){
    delete row.repair_warning;
    row.needs_manual_review = false;
    row.repair_warnings = {};
    return;
  }
  row.repair_warnings = Object.fromEntries(activeFields.map(field => [field, warnings[field] || "Needs manual review"]));
  row.needs_manual_review = true;
  row.repair_warning = `Needs manual review: ${activeFields.map(criticalFieldLabel).join(", ")}`;
}

function clearOcrRepairForField(row, field){
  if (!row || !OCR_FIELD_REPAIR_KEYS[field]) return;
  const repairKey = repairKeyForField(field);
  delete row[repairKey];
  if (field === "type") delete row.glass_type_repaired;
  if (row.raw_ocr_value && typeof row.raw_ocr_value === "object"){
    delete row.raw_ocr_value[field];
  }
  if (Array.isArray(row.ocr_repaired_fields)){
    row.ocr_repaired_fields = row.ocr_repaired_fields.filter(item => item !== field);
  }
  if (row.repair_warnings && typeof row.repair_warnings === "object"){
    delete row.repair_warnings[field];
  }
  refreshRowRepairWarning(row);
  const stillRepaired = Object.keys(OCR_FIELD_REPAIR_KEYS).some(candidate => row[repairKeyForField(candidate)] != null && row[repairKeyForField(candidate)] !== "");
  if (!stillRepaired){
    delete row.repaired_by;
    delete row.repair_confidence;
  }
}

function rowHasOcrRepair(row, field){
  const repairKey = repairKeyForField(field);
  return row && row[repairKey] != null && String(row[repairKey]).trim() !== "";
}

function rowRepairBadgeTitle(row, field){
  const repairKey = repairKeyForField(field);
  const repaired = row?.[repairKey];
  const rawBase = row?.raw_base64_value && typeof row.raw_base64_value === "object" ? row.raw_base64_value[field] : row?.[field];
  const rawOcr = row?.raw_ocr_value && typeof row.raw_ocr_value === "object" ? row.raw_ocr_value[field] : repaired;
  const confidence = Number(row?.repair_confidence);
  const pieces = [
    `${criticalFieldLabel(field)} repaired by OCR`,
    `base64: ${rawBase ?? ""}`,
    `OCR: ${rawOcr ?? repaired ?? ""}`,
  ];
  if (Number.isFinite(confidence)) pieces.push(`confidence: ${Math.round(confidence * 100)}%`);
  if (row?.repaired_by) pieces.push(`method: ${row.repaired_by}`);
  return pieces.join(" | ");
}

function renderOcrRepairBadges(row){
  const badges = [];
  Object.keys(OCR_FIELD_REPAIR_KEYS).forEach(field => {
    if (rowHasOcrRepair(row, field)){
      badges.push(`<span class="ocr-repair-badge" title="${escapeHtml(rowRepairBadgeTitle(row, field))}">OCR ${escapeHtml(criticalFieldLabel(field))}</span>`);
    }
  });
  const repairWarnings = row?.repair_warnings && typeof row.repair_warnings === "object" ? row.repair_warnings : {};
  Object.keys(repairWarnings).forEach(field => {
    if (isCriticalFieldMissing(row, field)){
      badges.push(`<span class="ocr-repair-badge review" title="${escapeHtml(String(repairWarnings[field] || row.repair_warning || "Needs manual review"))}">Needs review</span>`);
    }
  });
  return badges;
}

function applyOcrFallbackResultToRow(row, field, result){
  if (!row) return false;
  row.raw_base64_value = row.raw_base64_value && typeof row.raw_base64_value === "object" ? row.raw_base64_value : {};
  if (!Object.prototype.hasOwnProperty.call(row.raw_base64_value, field)){
    row.raw_base64_value[field] = field === "type" ? (row.type || "") : (row[field] ?? "");
  }
  row.raw_ocr_value = row.raw_ocr_value && typeof row.raw_ocr_value === "object" ? row.raw_ocr_value : {};
  const rawOcr = result?.suggested_value ?? result?.evidence?.ocr_text ?? result?.evidence?.matched_text ?? "";
  row.raw_ocr_value[field] = rawOcr;
  const confidence = Number(result?.confidence);
  const repaired = !!result?.success
    && result.suggested_value != null
    && String(result.suggested_value).trim() !== ""
    && Number.isFinite(confidence)
    && confidence >= OCR_REPAIR_CONFIDENCE_MIN;
  if (repaired){
    const repairKey = repairKeyForField(field);
    row[repairKey] = normalizeRepairValue(field, result.suggested_value);
    if (field === "type") row.glass_type_repaired = row[repairKey];
    row.ocr_repaired_fields = Array.isArray(row.ocr_repaired_fields) ? row.ocr_repaired_fields : [];
    if (!row.ocr_repaired_fields.includes(field)) row.ocr_repaired_fields.push(field);
    row.repaired_by = result.method || result.repaired_by || "ocr_fallback";
    row.repair_confidence = Math.max(Number(row.repair_confidence) || 0, confidence);
    if (row.repair_warnings && typeof row.repair_warnings === "object"){
      delete row.repair_warnings[field];
    }
    refreshRowRepairWarning(row);
    return true;
  }
  row.repair_warnings = row.repair_warnings && typeof row.repair_warnings === "object" ? row.repair_warnings : {};
  row.repair_warnings[field] = "Needs manual review";
  refreshRowRepairWarning(row);
  return false;
}

async function recheckMissingFieldsWithOcr(){
  const bucket = appState.extract;
  const rows = bucket.rows || [];
  const orderId = bucket.draftId || bucket.savedOrderId;
  if (!orderId){
    setStatusMessage("OCR recheck needs a saved PDF draft.");
    return;
  }
  const targets = [];
  rows.forEach((row, index) => {
    missingCriticalFields(row).forEach(field => {
      if (!rowHasOcrRepair(row, field)){
        targets.push({ row, index, field });
      }
    });
  });
  if (!targets.length){
    setStatusMessage("No missing critical fields to recheck.");
    return;
  }
  const recheckBtn = document.getElementById("ocrRecheckMissing");
  if (recheckBtn) recheckBtn.disabled = true;
  let repairedCount = 0;
  let manualCount = 0;
  setStatusMessage(`Rechecking ${targets.length} field(s) with OCR…`, { showSpinner: true });
  for (const target of targets){
    const orderContext = buildRowDiagnosisContext("extract", target.index);
    try{
      const response = await fetch(API_BASE + "/api/extraction/ocr-fallback-row", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          order_id: String(orderId),
          pdf_id: bucket.source_hash || null,
          row_index: target.index,
          row: target.row,
          target_field: target.field,
          order_context: orderContext,
        }),
      });
      if (!response.ok) throw new Error(await response.text());
      const result = await response.json();
      if (applyOcrFallbackResultToRow(target.row, target.field, result)){
        repairedCount += 1;
      }else{
        manualCount += 1;
      }
    }catch(error){
      applyOcrFallbackResultToRow(target.row, target.field, {
        success: false,
        confidence: 0,
        reason: error.message || String(error),
      });
      manualCount += 1;
    }
  }
  updateExtractUI();
  setStatusMessage(`OCR recheck complete: ${repairedCount} repaired, ${manualCount} need manual review.`);
}

function removeRow(scope, rowKey){
  if (!rowKey) return;
  const key = String(rowKey);
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  bucket.rows = bucket.rows || [];
  const rows = bucket.rows;
  const index = rows.findIndex(row => {
    if (!row) return false;
    const rid = row._rid != null ? String(row._rid) : row.id != null ? String(row.id) : "";
    return rid === key;
  });
  if (index === -1) return;
  const [removedRow] = rows.splice(index, 1);
  if (bucket.rowWarnings && removedRow && removedRow._rid){
    delete bucket.rowWarnings[removedRow._rid];
  }
  if (bucket.rowDiagnoses && removedRow && removedRow._rid){
    delete bucket.rowDiagnoses[removedRow._rid];
  }
  recalcParsedTotals(bucket);
  if (scope === "history"){
    updateHistoryDetailUI();
  }else{
    window.__rows = rows;
    updateExtractUI();
  }
}

function renderRowDiagnosisPanel(diagnosis){
  if (!diagnosis) return "";
  if (diagnosis.loading){
    return '<div class="diagnosis-panel"><span class="spinner"></span> Diagnosing row...</div>';
  }
  if (Object.prototype.hasOwnProperty.call(diagnosis, "success")){
    const confidence = Number(diagnosis.confidence);
    const confidenceText = Number.isFinite(confidence) ? `${Math.round(confidence * 100)}%` : "—";
    const targetField = diagnosis.target_field || "dimension";
    const evidenceHtml = renderRepairEvidence(diagnosis.evidence || {});
    const traceHtml = renderRepairTrace(diagnosis.trace || []);
    const methods = Array.isArray(diagnosis.methods_used) && diagnosis.methods_used.length
      ? diagnosis.methods_used.join(" → ")
      : (diagnosis.method || "repair_orchestrator");
    const ocrMethod = diagnosis.method === "openai_vision_page_ocr" || methods.includes("openai_vision_page_ocr")
      ? `<div><span class="muted">OCR method:</span> <span class="mono">openai_vision_page_ocr</span></div>`
      : "";
    const suggestionHtml = diagnosis.success
      ? `<div><span class="muted">Original:</span> <span class="mono">${escapeHtml(String(diagnosis.original_value ?? ""))}</span></div>
        <div><span class="muted">Suggested:</span> <span class="mono">${escapeHtml(String(diagnosis.suggested_value ?? ""))}</span></div>`
      : `<div><span class="muted">Suggestion:</span> ${escapeHtml(diagnosis.reasoning || diagnosis.reason || "No safe suggestion is available yet.")}</div>`;
    const nextStep = diagnosis.recommended_next_step
      ? `<div><span class="muted">Next step:</span> <span class="mono">${escapeHtml(diagnosis.recommended_next_step)}</span></div>`
      : "";
    const actions = diagnosis.success
      ? `<div class="diagnosis-actions">
          <button type="button" class="btn small" data-accept-suggestion="${escapeHtml(diagnosis.row_key || "")}">Accept suggestion</button>
          <button type="button" class="btn small" data-keep-suggestion="${escapeHtml(diagnosis.row_key || "")}">Keep original</button>
          <button type="button" class="btn small" data-manual-suggestion="${escapeHtml(diagnosis.row_key || "")}" data-target-field="${escapeHtml(targetField)}">Manual edit</button>
        </div>`
      : `<div class="diagnosis-actions">
          <button type="button" class="btn small" data-keep-suggestion="${escapeHtml(diagnosis.row_key || "")}">Keep original</button>
          <button type="button" class="btn small" data-manual-suggestion="${escapeHtml(diagnosis.row_key || "")}" data-target-field="${escapeHtml(targetField)}">Manual edit</button>
        </div>`;
    return `<div class="diagnosis-panel">
      <div><strong>${diagnosis.success ? "SUGGESTION" : "NO SAFE SUGGESTION"}</strong></div>
      ${suggestionHtml}
      <div><span class="muted">Confidence:</span> ${escapeHtml(confidenceText)}</div>
      <div><span class="muted">Methods:</span> <span class="mono">${escapeHtml(methods)}</span></div>
      ${ocrMethod}
      ${evidenceHtml}
      <div><span class="muted">Reasoning:</span> ${escapeHtml(diagnosis.reasoning || diagnosis.reason || "Review this row manually.")}</div>
      ${traceHtml}
      ${nextStep}
      <div class="muted">No automatic change was made.</div>
      ${actions}
    </div>`;
  }
  const confidence = Number(diagnosis.confidence);
  const confidenceText = Number.isFinite(confidence) ? `${Math.round(confidence * 100)}%` : "—";
  return `<div class="diagnosis-panel">
    <div><strong>${escapeHtml(String(diagnosis.severity || "warning").toUpperCase())}</strong> ${escapeHtml(diagnosis.summary || "Review this row.")}</div>
    <div><span class="muted">Likely cause:</span> ${escapeHtml(diagnosis.likely_cause || "Manual review needed.")}</div>
    <div><span class="muted">Recommended action:</span> <span class="mono">${escapeHtml(diagnosis.recommended_action || "MANUAL_REVIEW")}</span></div>
    <div><span class="muted">Confidence:</span> ${escapeHtml(confidenceText)}</div>
    <div class="muted">No automatic change was made.</div>
  </div>`;
}

function renderEditableTable(scope, containerId, rows, rowWarnings){
  const container = typeof containerId === "string" ? document.getElementById(containerId) : containerId;
  if (!container) return;
  if (!Array.isArray(rows) || !rows.length){
    container.innerHTML = '<div class="empty-state">No rows yet — run an extraction.</div>';
    return;
  }
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const indexMap = new Map((bucket.rows || []).map((row, idx) => [row._rid || `rid-${idx}`, idx]));
  const groups = summarizeByOrder(rows);
  let globalIndex = 0;
  let html = "";
  for (const [order, items] of groups.entries()){
    const orderLabel = order || "—";
    html += `<div class="group-header" data-order="${escapeHtml(order)}" style="display:flex;justify-content:space-between;align-items:center;margin:16px 0 6px">
      <div style="font-weight:600">${escapeHtml(orderLabel)}</div>
      <div style="display:flex;gap:8px;align-items:center">
        <button type="button" class="btn small" data-add-row="${escapeHtml(order)}">Add row</button>
      </div>
    </div>`;
    html += `<table class="editable-table" data-order="${escapeHtml(order)}">
	      <thead>
	        <tr>
	          <th>#</th>
	          <th>Warnings</th>
	          <th data-col="order">Order</th>
	          <th data-col="type">Type</th>
	          <th data-col="dimension">Dimension</th>
	          <th data-col="position">Position</th>
	          <th data-col="qty">Quantity</th>
	          <th data-col="area">Area (m²)</th>
	          <th data-col="actions">Actions</th>
	        </tr>
	      </thead>
      <tbody>`;
    items.forEach(row => {
      if (!row._rid){
        row._rid = `${scope}-${Date.now()}-${globalIndex}`;
      }
      const rowIndex = indexMap.has(row._rid) ? indexMap.get(row._rid) : globalIndex;
      const warningBadges = [];
      const diagnostics = getRowDiagnostics(row);
      const diagnosticIssues = getDiagnosticIssues(row);
      const mismatch = hasDiagnosticIssue(row, "AREA_MISMATCH");
      const familyMismatch = hasDiagnosticIssue(row, "POSSIBLE_DIMENSION_FAMILY_MISMATCH");
      const computed = diagnosticCalculatedArea(row);
      const extractedArea = Number(diagnostics?.computed?.extracted_area ?? row.area ?? 0);
      if (mismatch && computed != null){
        const delta = Number.isFinite(extractedArea) ? (extractedArea - computed).toFixed(3) : "";
        const title = Number.isFinite(extractedArea)
          ? `Provided ${extractedArea.toFixed(3)} vs computed ${computed.toFixed(3)}`
          : `Computed ${computed.toFixed(3)}`;
        warningBadges.push(`<span class="pill warn" title="${escapeHtml(title)}">Δ ${escapeHtml(delta)} m²</span>`);
      }
      if (familyMismatch){
        warningBadges.push(
          `<span class="pill warn" title="Dimension is valid but may not match the nearby/order pattern.">Pattern</span>`
        );
      }
      const otherIssues = diagnosticIssues.filter(issue => !["AREA_MISMATCH", "POSSIBLE_DIMENSION_FAMILY_MISMATCH"].includes(String(issue?.code || "")));
      if (otherIssues.length){
        const issueText = otherIssues.map(issue => issue?.message || issue?.code || "Diagnostic warning").join("; ");
        warningBadges.push(`<span class="editable-badge" title="${escapeHtml(issueText)}">!</span>`);
      }
      warningBadges.push(...renderOcrRepairBadges(row));
      const warningBadge = warningBadges.join(" ");
      const inputBase = `data-scope="${scope}" data-index="${rowIndex}" autocomplete="off" class="cell"`;
      const orderKey = `${row._rid}:order_number`;
      const typeKey = `${row._rid}:type`;
      const dimKey = `${row._rid}:dimension`;
      const posKey = `${row._rid}:position`;
      const qtyKey = `${row._rid}:quantity`;
      const areaKey = `${row._rid}:area`;
      const fixBtn = hasActionableDiagnostics(row) ? `<button type="button" class="btn small" data-fix-area="${rowIndex}" title="Diagnose this warning">Fix</button>` : "";
      const removeBtn = `<button type="button" class="btn small danger" data-remove-row="${escapeHtml(row._rid)}">Remove</button>`;
      const actionButtons = [fixBtn, removeBtn].filter(Boolean).join(" ");
      html += `<tr data-index="${rowIndex}" data-order="${escapeHtml(order)}">
        <td>${globalIndex + 1}</td>
        <td>${warningBadge}</td>
        <td data-col="order"><input ${inputBase} data-field="order_number" data-key="${orderKey}" value="${escapeHtml(row.order_number || "")}" /></td>
        <td data-col="type"><input ${inputBase} data-field="type" data-key="${typeKey}" value="${escapeHtml(row.type || "")}" /></td>
        <td data-col="dimension"><input ${inputBase} data-field="dimension" data-key="${dimKey}" inputmode="text" value="${escapeHtml(row.dimension || "")}" /></td>
        <td data-col="position"><input ${inputBase} data-field="position" data-key="${posKey}" inputmode="text" value="${escapeHtml(row.position || "")}" /></td>
        <td data-col="qty"><input ${inputBase} data-field="quantity" data-key="${qtyKey}" type="number" inputmode="numeric" min="0" step="1" value="${escapeHtml(row.quantity ?? 0)}" /></td>
        <td data-col="area"><input ${inputBase} data-field="area" data-key="${areaKey}" type="number" inputmode="decimal" min="0" step="0.001" value="${escapeHtml(row.area ?? 0)}" /></td>
        <td data-col="actions">${actionButtons}</td>
      </tr>`;
      const diagnosis = bucket.rowDiagnoses && row._rid ? bucket.rowDiagnoses[row._rid] : null;
      if (diagnosis){
        html += `<tr class="diagnosis-row" data-index="${rowIndex}" data-order="${escapeHtml(order)}">
          <td colspan="9">${renderRowDiagnosisPanel(diagnosis)}</td>
        </tr>`;
      }
      globalIndex += 1;
    });
    html += "</tbody></table>";
  }
  container.innerHTML = html;
}

function parseFieldValue(field, rawValue){
  if (field === "quantity"){
    const parsed = parseInt(rawValue, 10);
    return Number.isFinite(parsed) ? parsed : 0;
  }
  if (field === "area"){
    const parsed = parseFloat(rawValue);
    return Number.isFinite(parsed) ? parsed : 0;
  }
  return rawValue ?? "";
}

function updateRowValue(scope, index, field, value, options = {}){
  const { commit = false } = options;
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const row = bucket.rows[index];
  if (!row) return;
  bucket.rows[index][field] = value;
  const rid = row._rid;
  if (bucket.rowWarnings && rid){
    delete bucket.rowWarnings[rid];
  }
  if (bucket.rowDiagnoses && rid){
    delete bucket.rowDiagnoses[rid];
  }
  if (field === "quantity"){
    row.quantity = Math.max(0, Number(row.quantity || 0));
  }
  if (isDiagnosticsRefreshField(field)){
    clearDiagnosticsForManualEdit(row);
    clearOcrRepairForField(row, field);
  }
  if (scope === "extract"){
    window.__rows = bucket.rows;
  }
  if (commit){
    recalcParsedTotals(bucket);
    if (scope === "history"){
      updateHistoryDetailUI();
    }else{
      updateExtractUI();
    }
  }else{
    updateTotalsUI(scope);
  }
}

function handleEditableInput(event){
  const input = event.target;
  if (!input.classList.contains("cell")) return;
  const field = input.dataset.field;
  const scope = input.dataset.scope;
  const index = Number(input.dataset.index);
  if (!field || !scope || Number.isNaN(index)) return;
  const parsed = parseFieldValue(field, input.value);
  updateRowValue(scope, index, field, parsed, { commit: false });
}

document.getElementById("tableWrap").addEventListener("input", handleEditableInput, true);
document.getElementById("historyTableWrap").addEventListener("input", handleEditableInput, true);
document.getElementById("extractNotes").addEventListener("input", (event)=>{
  appState.extract.notes = event.target.value;
});
document.getElementById("historyNotes").addEventListener("input", (event)=>{
  appState.historyDetail.notes = event.target.value;
});

document.getElementById("tableWrap").addEventListener("click", (event)=>{
  const sourceRow = event.target.closest("tr[data-index]");
  if (sourceRow){
    const rowIndex = Number(sourceRow.dataset.index);
    const matchedIssueIndex = currentExtractReviewIssues.findIndex(issue => issue.rowIndex === rowIndex);
    if (matchedIssueIndex >= 0){
      setActiveExtractReviewIssue(matchedIssueIndex, { scroll: false });
    }else{
      showSourceLocation(extractPdfPreview, extractSourceHighlight, appState.extract.rows[rowIndex], `Row ${rowIndex + 1}`);
    }
  }
  const addBtn = event.target.closest("[data-add-row]");
  if (addBtn){
    addRowToGroup("extract", addBtn.dataset.addRow || "");
    return;
  }
  const fixBtn = event.target.closest("[data-fix-area]");
  if (fixBtn){
    const idx = Number(fixBtn.dataset.fixArea);
    if (!Number.isNaN(idx)) requestRowDiagnosis("extract", idx);
  }
});

extractReviewIssuesEl?.addEventListener("click", event => {
  const issueButton = event.target.closest("[data-review-issue-index]");
  if (!issueButton) return;
  setActiveExtractReviewIssue(Number(issueButton.dataset.reviewIssueIndex), { focus: true });
});

extractPrevIssueBtn?.addEventListener("click", ()=> {
  setActiveExtractReviewIssue(activeReviewIssueIndex - 1, { focus: true });
});

extractNextIssueBtn?.addEventListener("click", ()=> {
  setActiveExtractReviewIssue(activeReviewIssueIndex + 1, { focus: true });
});

extractPdfPrev?.addEventListener("click", ()=> setSourcePdfPage("extract", sourcePdfState.extract.page - 1));
extractPdfNext?.addEventListener("click", ()=> setSourcePdfPage("extract", sourcePdfState.extract.page + 1));
orderSourcePrev?.addEventListener("click", ()=> setSourcePdfPage("order", sourcePdfState.order.page - 1));
orderSourceNext?.addEventListener("click", ()=> setSourcePdfPage("order", sourcePdfState.order.page + 1));

document.addEventListener("keydown", event => {
  if (!event.altKey || !panels.extract?.classList.contains("active") || newOrderWorkspace?.hidden) return;
  if (event.key === "ArrowUp" && currentExtractReviewIssues.length){
    event.preventDefault();
    setActiveExtractReviewIssue(activeReviewIssueIndex - 1, { focus: true });
  }else if (event.key === "ArrowDown" && currentExtractReviewIssues.length){
    event.preventDefault();
    setActiveExtractReviewIssue(activeReviewIssueIndex + 1, { focus: true });
  }
});

document.getElementById("historyTableWrap").addEventListener("click", (event)=>{
  const addBtn = event.target.closest("[data-add-row]");
  if (addBtn){
    addRowToGroup("history", addBtn.dataset.addRow || "");
    return;
  }
  const fixBtn = event.target.closest("[data-fix-area]");
  if (fixBtn){
    const idx = Number(fixBtn.dataset.fixArea);
    if (!Number.isNaN(idx)) requestRowDiagnosis("history", idx);
  }
});

document.addEventListener("click", (event)=>{
  const acceptBtn = event.target.closest("[data-accept-suggestion]");
  if (acceptBtn){
    const scope = acceptBtn.closest("#historyTableWrap") ? "history" : (acceptBtn.closest("#tableWrap") ? "extract" : null);
    if (scope) acceptRowSuggestion(scope, acceptBtn.getAttribute("data-accept-suggestion"));
    return;
  }
  const keepBtn = event.target.closest("[data-keep-suggestion]");
  if (keepBtn){
    const scope = keepBtn.closest("#historyTableWrap") ? "history" : (keepBtn.closest("#tableWrap") ? "extract" : null);
    if (scope) keepOriginalSuggestion(scope, keepBtn.getAttribute("data-keep-suggestion"));
    return;
  }
  const manualBtn = event.target.closest("[data-manual-suggestion]");
  if (manualBtn){
    const scope = manualBtn.closest("#historyTableWrap") ? "history" : (manualBtn.closest("#tableWrap") ? "extract" : null);
    if (scope){
      const rowKey = manualBtn.getAttribute("data-manual-suggestion");
      const field = manualBtn.getAttribute("data-target-field") || "dimension";
      keepOriginalSuggestion(scope, rowKey);
      setTimeout(() => focusRowField(scope, rowKey, field), 0);
    }
    return;
  }
  const btn = event.target.closest("[data-remove-row]");
  if (!btn) return;
  const rowKey = btn.getAttribute("data-remove-row");
  if (!rowKey) return;
  const scope = btn.closest("#historyTableWrap") ? "history" : (btn.closest("#tableWrap") ? "extract" : null);
  if (!scope) return;
  if (!confirm("Remove this line?")) return;
  removeRow(scope, rowKey);
});

const fixAllAreasBtn = document.getElementById("fixAllAreas");
if (fixAllAreasBtn){
  fixAllAreasBtn.addEventListener("click", ()=> fixAllAreas("extract"));
}
const ocrRecheckMissingBtn = document.getElementById("ocrRecheckMissing");
if (ocrRecheckMissingBtn){
  ocrRecheckMissingBtn.addEventListener("click", ()=> recheckMissingFieldsWithOcr());
}
const forceOcrExtractionBtn = document.getElementById("forceOcrExtraction");
if (forceOcrExtractionBtn){
  forceOcrExtractionBtn.addEventListener("click", async ()=>{
    if (!lastExtractPdfFile){
      setStatusMessage("Upload a PDF before forcing OCR.");
      return;
    }
    await handlePdfExtraction(lastExtractPdfFile, { forceOcr: true });
  });
}
const historyFixAllAreasBtn = document.getElementById("historyFixAllAreas");
if (historyFixAllAreasBtn){
  historyFixAllAreasBtn.addEventListener("click", ()=> fixAllAreas("history"));
}

function normalizeDimensionInput(value){
  if (!value) return "";
  return value
    .replace(/[×X]/g, "x")
    .replace(/\s+/g, "")
    .replace(/x+/g, "x");
}

function handleEditableBlur(event){
  const input = event.target;
  if (!input.classList.contains("cell")) return;
  const field = input.dataset.field;
  const scope = input.dataset.scope;
  const index = Number(input.dataset.index);
  if (!field || !scope || Number.isNaN(index)) return;
  let value = input.value;
  if (field === "dimension"){
    value = normalizeDimensionInput(value);
    input.value = value;
  }else if (field === "position"){
    value = value.trim();
    input.value = value;
  }else if (field === "quantity"){
    const parsedQty = parseInt(value, 10);
    value = Number.isFinite(parsedQty) ? String(parsedQty) : "0";
    input.value = value;
  }else if (field === "area"){
    const parsedArea = parseFloat(value);
    value = Number.isFinite(parsedArea) ? parsedArea.toFixed(3) : "0.000";
    input.value = value;
  }
  const parsed = parseFieldValue(field, value);
  updateRowValue(scope, index, field, parsed, { commit: true });
  if (isDiagnosticsRefreshField(field)){
    refreshRowDiagnostics(scope, index);
  }
}

document.getElementById("tableWrap").addEventListener("blur", handleEditableBlur, true);
document.getElementById("historyTableWrap").addEventListener("blur", handleEditableBlur, true);

function setErrorMessage(msg){
  if (!msg){
    errorEl.hidden = true;
    errorEl.textContent = "";
  }else{
    errorEl.hidden = false;
    errorEl.textContent = msg;
  }
}

function setHistoryStatus(msg){
  historyStatusEl.textContent = msg || "";
}

function updateHistoryOfflineBadge(){
  historyOfflineBadge.hidden = !historyState.offline;
}

function formatDate(value){
  if (!value) return "—";
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;
  return date.toLocaleString([], { dateStyle: "short", timeStyle: "short" });
}

function formatArea(value){
  const num = Number(value || 0);
  return num.toFixed(3);
}

// --- CSV + grouping helpers -------------------------------------------------
function groupWithinOrders(rows){
  const keyOf = (r) => `${(r.order_number||"").trim()}|${(r.type||"").trim()}|${(r.dimension||"").trim()}`;
  const map = new Map();
  for(const r of rows){
    const key = keyOf(r);
    if (!map.has(key)){
      map.set(key, {
        order_number: (r.order_number||"").trim(),
        type: r.type || "",
        dimension: r.dimension || "",
        position: "— (grouped)",
        quantity: 0,
        area: 0
      });
    }
    const agg = map.get(key);
    const qty = Number(r.quantity || 0);
    const area = Number(r.area || 0);
    agg.quantity += qty;
    agg.area += area;
  }
  return Array.from(map.values());
}

function toCSV(rows){
  const header = ["order_number","type","dimension","position","quantity","area"];
  const out = [header.join(",")];
  for(const r of rows){
    const vals = header.map(key=>{
      let value = r[key];
      if (typeof value === "string"){
        if (value.includes(",") || value.includes("\"")){
          value = "\"" + value.replace(/"/g, "\"\"") + "\"";
        }
      }
      return value;
    });
    out.push(vals.join(","));
  }
  return out.join("\n");
}

function summarizeByOrder(rows){
  const map = new Map();
  for(const r of rows){
    const key = (r.order_number || "").trim();
    if (!map.has(key)) map.set(key, []);
    map.get(key).push(r);
  }
  return map;
}

function computeTotals(rows){
  const units = rows.reduce((acc,row)=> acc + Number(row.quantity || 0), 0);
  const area = rows.reduce((acc,row)=> acc + Number(row.area || 0), 0);
  return { units, area };
}

function collectOrderNumbers(rows){
  return Array.from(new Set(rows.map(row => (row.order_number || "").trim()).filter(Boolean)));
}

// --- Invoice helpers -------------------------------------------------------
function getDefaultTypeCorrections(){
  return DEFAULT_TYPE_CORRECTIONS.map(entry => ({ ...entry }));
}

function normalizeTypeKey(str) {
  return (str || "")
    .toLowerCase()
    .replace(/\s+/g, "")
    .replace(/\++/g, "+");
}

function applyTypeCorrections(rawType) {
  const key = normalizeTypeKey(rawType);
  for (const entry of TYPE_CORRECTIONS) {
    if (normalizeTypeKey(entry.raw) === key) {
      return entry.corrected;
    }
  }
  return rawType;
}

function loadTypeCorrections(source, { fallbackToDefaults = true } = {}) {
  if (source && Array.isArray(source.typeCorrections)) {
    const cleaned = source.typeCorrections
      .map(entry => ({
        raw: (entry?.raw || "").trim(),
        corrected: (entry?.corrected || "").trim(),
      }))
      .filter(entry => entry.raw && entry.corrected);
    return cleaned;
  }
  try{
    const raw = localStorage.getItem(TYPE_CORRECTIONS_STORAGE_KEY);
    if (!raw) return getDefaultTypeCorrections();
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed)) return parsed;
  }catch(e){
    console.warn("Failed to load type corrections", e);
  }
  return fallbackToDefaults ? getDefaultTypeCorrections() : [];
}

function saveTypeCorrections() {
  try{
    localStorage.setItem(TYPE_CORRECTIONS_STORAGE_KEY, JSON.stringify(TYPE_CORRECTIONS));
  }catch(e){
    console.warn("Failed to save type corrections", e);
  }
}

function fixGlassTypos(text, rawSource){
  if (!text) return text;
  const replaced = text.replace(/stainat/gi, "satinat");
  if (replaced !== text){
    console.log("[GlassNormalize] raw:", rawSource, "→ normalized:", replaced);
  }
  return replaced;
}

function normalizeGlassKey(raw){
  const original = raw == null ? "" : String(raw);
  let str = original.toLowerCase().trim().replace(/^\s*\d+\s*vetri?\s*/i, "").replace(/\bvetri?\b/gi, "");
  if (!str) return "";
  str = str.replace(/\s+/g, " ").trim();
  str = fixGlassTypos(str, original);
  return str.replace(/\s+/g, "");
}

function getDefaultGlassPrices(){
  return {
    [normalizeGlassKey("4F")]: 1000,
    [normalizeGlassKey("4 Satinato")]: 1800,
    [normalizeGlassKey("4 LowE")]: 2000,
    [normalizeGlassKey("33.1F")]: 2500,
    [normalizeGlassKey("33.1 Satinato")]: 3500,
    [normalizeGlassKey("33.1 LowE")]: 3700,
  };
}

function getDefaultSpacerPrices(){
  return {
    6: { normal: 500, thermal: 1000 },
    9: { normal: 500, thermal: 1000 },
    10: { normal: 500, thermal: 1000 },
    12: { normal: 500, thermal: 1000 },
    14: { normal: 600, thermal: 1100 },
    16: { normal: 700, thermal: 1200 },
    18: { normal: 800, thermal: 1300 },
    20: { normal: 900, thermal: 1400 },
    22: { normal: 1000, thermal: 1500 },
    24: { normal: 1100, thermal: 1600 },
  };
}

function getDefaultInvoiceConfig(){
  return {
    glassPrices: getDefaultGlassPrices(),
    spacerPrices: getDefaultSpacerPrices(),
    typeCorrections: [],
  };
}

function normalizeInvoiceConfigPayload(payload){
  const defaults = getDefaultInvoiceConfig();
  const glassSource = (payload && typeof payload === "object" && (payload.glassPrices || payload.glass)) ? (payload.glassPrices || payload.glass) : {};
  const spacerSource = (payload && typeof payload === "object" && (payload.spacerPrices || payload.spacer)) ? (payload.spacerPrices || payload.spacer) : {};
  const glass = { ...defaults.glassPrices };
  const spacer = { ...defaults.spacerPrices };

  if (glassSource && typeof glassSource === "object"){
    Object.entries(glassSource).forEach(([key, value])=>{
      const norm = normalizeGlassKey(key);
      const price = Number(value);
      if (norm && Number.isFinite(price)){
        glass[norm] = price;
      }
    });
  }

  if (spacerSource && typeof spacerSource === "object"){
    Object.entries(spacerSource).forEach(([thickness, entry])=>{
      const thickNum = Number(thickness);
      if (!Number.isFinite(thickNum)) return;
      const normal = Number(entry?.normal);
      const thermal = Number(entry?.thermal);
      spacer[thickNum] = {
        normal: Number.isFinite(normal) ? normal : spacer[thickNum]?.normal,
        thermal: Number.isFinite(thermal) ? thermal : spacer[thickNum]?.thermal,
      };
    });
  }

  const typeCorrections = Array.isArray(payload?.typeCorrections)
    ? payload.typeCorrections
        .map(entry => ({
          raw: (entry?.raw || "").trim(),
          corrected: (entry?.corrected || "").trim(),
        }))
        .filter(entry => entry.raw && entry.corrected)
    : [];

  return {
    glassPrices: glass,
    spacerPrices: spacer,
    typeCorrections,
  };
}

const invoiceRuntime = {
  initialized: false,
  pendingPrompts: [],
  missingKeys: new Set(),
  modalPayload: null,
  extraGlassRows: 1,
  extraSpacerRows: 1,
  currentMissing: null,
  configPromise: null,
  jobsPromise: null,
  jobsLoaded: false,
};

const invoiceAddOrderState = {
  loading: false,
  orders: [],
  error: "",
  searchTerm: "",
};
const glassAliasCache = Object.create(null);
const glassAliasPending = new Map();

function clearGlassAliasCache(){
  Object.keys(glassAliasCache).forEach(key=>{
    delete glassAliasCache[key];
  });
  glassAliasPending.clear();
}

/**
 * @typedef {Object} LineAIAnalysis
 * @property {string} normalizedType
 * @property {string|null} glassKey
 * @property {"normal"|"thermal"} spacerMode
 * @property {boolean} isLaminated
 * @property {number} confidence
 * @property {string=} reason
 */

function groupInvoiceLinesByType(lines){
  const map = new Map();
  (lines || []).forEach(line=>{
    if (!line) return;
    const normType = normalizeGlassKey(line.type || "");
    const unit = Number(line.unitPricePerM2 || 0);
    const key = `${normType}|${unit}`;
    const qty = Number(line.quantity || 0);
    const area = Number(line.lineAreaTotal ?? ((line.area || 0) * (line.quantity || 0)) ?? 0);
    if (!map.has(key)){
      map.set(key, {
        index: map.size + 1,
        type: line.type || "",
        normalizedType: normType,
        quantity: 0,
        area: 0,
        unitPricePerM2: unit,
        lineTotal: 0,
        missing: !!line.missing,
        glassPricePerM2: Number(line.glassPricePerM2 || 0),
        spacerPricePerM2: Number(line.spacerPricePerM2 || 0),
      });
    }
    const agg = map.get(key);
    agg.quantity += qty;
    agg.area += area;
    agg.missing = agg.missing || !!line.missing;
    agg.lineTotal = agg.unitPricePerM2 * agg.area;
  });
  return Array.from(map.values()).map((item, idx)=> ({
    ...item,
    index: idx + 1,
  }));
}

function loadInvoiceCounter(){
  try{
    const raw = localStorage.getItem(INVOICE_COUNTER_KEY);
    const num = Number(raw);
    if (Number.isFinite(num) && num > 0){
      appState.invoices.counter = num;
      return;
    }
  }catch{/* ignore */}
  appState.invoices.counter = 1;
}

function saveInvoiceCounter(){
  try{
    localStorage.setItem(INVOICE_COUNTER_KEY, String(appState.invoices.counter));
  }catch{/* ignore */}
}

function nextInvoiceNumber(){
  const now = new Date();
  const y = now.getFullYear();
  const m = String(now.getMonth() + 1).padStart(2, "0");
  const d = String(now.getDate()).padStart(2, "0");
  const seq = String(appState.invoices.counter).padStart(3, "0");
  const id = `INV-${y}${m}${d}-${seq}`;
  appState.invoices.counter += 1;
  saveInvoiceCounter();
  return id;
}

function loadInvoicePrices(source){
  let payload = source;
  if (!payload){
    try{
      const raw = localStorage.getItem(INVOICE_PRICES_KEY);
      payload = raw ? JSON.parse(raw) : null;
    }catch{
      payload = null;
    }
  }
  const normalized = normalizeInvoiceConfigPayload(payload || {});
  appState.invoices.priceLists = {
    glass: normalized.glassPrices,
    spacer: normalized.spacerPrices,
  };
  clearGlassAliasCache();
  return normalized;
}

function saveInvoicePrices(){
  try{
    localStorage.setItem(INVOICE_PRICES_KEY, JSON.stringify(appState.invoices.priceLists));
  }catch{/* ignore */}
}

function buildInvoiceConfigPayload(){
  return {
    glassPrices: { ...(appState?.invoices?.priceLists?.glass || {}) },
    spacerPrices: { ...(appState?.invoices?.priceLists?.spacer || {}) },
    typeCorrections: Array.isArray(TYPE_CORRECTIONS)
      ? TYPE_CORRECTIONS.map(entry => ({
          raw: (entry?.raw || "").trim(),
          corrected: (entry?.corrected || "").trim(),
        })).filter(entry => entry.raw && entry.corrected)
      : [],
  };
}

async function applyInvoiceConfigToState(config, { cacheLocal = true, preferExistingTypeCorrections = false } = {}){
  const normalized = normalizeInvoiceConfigPayload(config || {});
  const existingCorrections = (preferExistingTypeCorrections && !normalized.typeCorrections.length && Array.isArray(TYPE_CORRECTIONS))
    ? TYPE_CORRECTIONS
    : [];
  const mergedCorrections = normalized.typeCorrections.length ? normalized.typeCorrections : existingCorrections;
  appState.invoices.priceLists = {
    glass: normalized.glassPrices,
    spacer: normalized.spacerPrices,
  };
  TYPE_CORRECTIONS = (mergedCorrections || []).map(entry => ({
    raw: (entry?.raw || "").trim(),
    corrected: (entry?.corrected || "").trim(),
  })).filter(entry => entry.raw && entry.corrected);
  if (cacheLocal){
    saveInvoicePrices();
    saveTypeCorrections();
  }
  if (invoiceRuntime.initialized){
    await recalcAllInvoices({ allowPrompt: false });
    updateInvoicesUI();
  }
  return {
    glassPrices: appState.invoices.priceLists.glass,
    spacerPrices: appState.invoices.priceLists.spacer,
    typeCorrections: TYPE_CORRECTIONS.map(entry => ({ ...entry })),
  };
}

async function fetchInvoiceConfigFromServer(){
  const res = await fetch(API_BASE + "/api/prices");
  if (!res.ok){
    const text = await res.text().catch(()=> "");
    throw new Error(text || `Server responded with ${res.status}`);
  }
  const data = await res.json();
  return normalizeInvoiceConfigPayload(data);
}

async function persistInvoiceConfigToServer(configOverride, { preferExistingTypeCorrections = false } = {}){
  const payload = configOverride || buildInvoiceConfigPayload();
  try{
    const res = await fetch(API_BASE + "/api/prices", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    if (!res.ok){
      const text = await res.text().catch(()=> "");
      throw new Error(text || `Server responded with ${res.status}`);
    }
    const data = await res.json();
    const normalized = normalizeInvoiceConfigPayload(data);
    await applyInvoiceConfigToState(normalized, { cacheLocal: true, preferExistingTypeCorrections });
    return normalized;
  }catch(error){
    console.warn("Failed to save pricing config to server", error);
    return null;
  }
}

async function syncInvoiceConfigFromServer({ silent = false } = {}){
  if (invoiceRuntime.configPromise) return invoiceRuntime.configPromise;
  invoiceRuntime.configPromise = (async ()=>{
    try{
      const config = await fetchInvoiceConfigFromServer();
      return await applyInvoiceConfigToState(config, { cacheLocal: true, preferExistingTypeCorrections: true });
    }catch(error){
      if (!silent) console.warn("Failed to fetch pricing config from server", error);
      return null;
    }finally{
      invoiceRuntime.configPromise = null;
    }
  })();
  return invoiceRuntime.configPromise;
}

function dedupeAndSortOrderNumbers(values){
  const unique = [];
  (values || []).forEach(num=>{
    const normalized = String(num || "").trim();
    if (!normalized) return;
    const lower = normalized.toLowerCase();
    if (!unique.some(existing => existing.toLowerCase() === lower)){
      unique.push(normalized);
    }
  });
  return unique;
}

function extractOrderUnits(order){
  return Number(pickFirstNumber([
    order?.units_total,
    order?.total_units,
    order?.parsed_units,
    order?.units,
  ]) || 0);
}

function extractOrderArea(order){
  return Number(pickFirstNumber([
    order?.area_total,
    order?.total_area,
    order?.parsed_area,
    order?.area,
  ]) || 0);
}

function createOrderMetadata(options = {}){
  return {
    orderNumber: options.orderNumber || "—",
    clientName: options.clientName || "—",
    createdAt: options.createdAt || new Date().toISOString(),
    units: Number(options.units || 0),
    area: Number(options.area || 0),
    amount: Number(options.amount || 0),
  };
}

function ensureJobOrders(job){
  if (!job) return;
  if (!Array.isArray(job.orderNumbers)){
    job.orderNumbers = [];
  }
  if (!Array.isArray(job.orders)){
    job.orders = [];
  }
  const metaMap = new Map();
  job.orders.forEach(entry=>{
    if (!entry) return;
    const key = String(entry.orderNumber || "").trim().toLowerCase();
    if (!key) return;
    metaMap.set(key, {
      orderNumber: entry.orderNumber || "—",
      clientName: entry.clientName || entry.client || "—",
      createdAt: entry.createdAt || entry.date || job.createdAt || new Date().toISOString(),
      units: Number(entry.units || 0),
      area: Number(entry.area || 0),
      amount: Number(entry.amount || entry.total || 0),
    });
  });
  const fallbackClient = formatInvoiceClientLabel(job) || job.client || "—";
  const fallbackDate = job.createdAt || new Date().toISOString();
  job.orders = job.orderNumbers.map(orderNumber=>{
    const key = String(orderNumber || "").trim();
    const normalizedKey = key.toLowerCase();
    const existing = metaMap.get(normalizedKey);
    if (existing){
      if (!existing.clientName) existing.clientName = fallbackClient;
      if (!existing.createdAt) existing.createdAt = fallbackDate;
      existing.orderNumber = key || existing.orderNumber || "—";
      return existing;
    }
    return createOrderMetadata({
      orderNumber: key || "—",
      clientName: fallbackClient,
      createdAt: fallbackDate,
    });
  });
}

function addOrdersMetadata(job, entries = []){
  if (!job || !Array.isArray(entries) || !entries.length) return;
  if (!Array.isArray(job.orders)){
    job.orders = [];
  }
  entries.forEach(entry=>{
    if (!entry || !entry.orderNumber) return;
    const normalized = entry.orderNumber.toLowerCase();
    const exists = job.orders.some(meta => String(meta.orderNumber || "").toLowerCase() === normalized);
    if (!exists){
      job.orders.push(createOrderMetadata(entry));
    }
  });
}

function updateJobOrdersFromCalc(job, calc){
  if (!job) return;
  ensureJobOrders(job);
  const stats = new Map();
  const lines = Array.isArray(calc?.lines) ? calc.lines : Array.isArray(job.calculated?.lines) ? job.calculated.lines : [];
  lines.forEach(line=>{
    const orderKeyRaw = (line.orderId && String(line.orderId).trim()) || (job.orderNumbers && job.orderNumbers[0]) || "";
    const orderKey = orderKeyRaw.toLowerCase();
    if (!orderKey) return;
    if (!stats.has(orderKey)){
      stats.set(orderKey, { units: 0, area: 0, amount: 0 });
    }
    const entry = stats.get(orderKey);
    entry.units += Number(line.quantity || 0);
    const areaValue = Number(line.area || line.lineAreaTotal || 0);
    entry.area += areaValue;
    entry.amount += Number(line.lineTotal || 0);
  });
  const fallbackClient = formatInvoiceClientLabel(job) || job.client || "—";
  const fallbackDate = job.createdAt || new Date().toISOString();
  const normalizedOrders = job.orderNumbers.map(orderNumber=>{
    const normalizedKey = String(orderNumber || "").trim().toLowerCase();
    const existing = (job.orders || []).find(meta => String(meta.orderNumber || "").toLowerCase() === normalizedKey) || createOrderMetadata({
      orderNumber: orderNumber || "—",
      clientName: fallbackClient,
      createdAt: fallbackDate,
    });
    const stat = stats.get(normalizedKey) || { units: 0, area: 0, amount: 0 };
    return {
      ...existing,
      orderNumber: orderNumber || existing.orderNumber || "—",
      clientName: existing.clientName || fallbackClient,
      createdAt: existing.createdAt || fallbackDate,
      units: Number(stat.units || 0),
      area: Number(stat.area || 0),
      amount: Number(stat.amount || 0),
    };
  });
  job.orders = normalizedOrders;
}

function normalizeInvoiceJob(job){
  if (!job || typeof job !== "object") return null;
  const orderNumbers = (() => {
    if (Array.isArray(job.orderNumbers)){
      return job.orderNumbers.map(v => String(v || "").trim()).filter(Boolean);
    }
    if (Array.isArray(job.order_numbers)){
      return job.order_numbers.map(v => String(v || "").trim()).filter(Boolean);
    }
    const legacy = pickFirstString([
      job.orderNumber,
      job.order_number,
      job.order,
      job.orderId,
      job.id,
    ]);
    return legacy ? [legacy] : [];
  })();
  const uniqueOrderNumbers = dedupeAndSortOrderNumbers(orderNumbers);
  const normalizedClients = (() => {
    if (Array.isArray(job.clients) && job.clients.length){
      return job.clients.map(name => {
        const text = String(name || "").trim();
        return text || "—";
      }).filter(Boolean);
    }
    const clientLabel = String(job.client || "").trim();
    return clientLabel ? [clientLabel] : ["—"];
  })();
  const primaryClient = normalizedClients[0] || "—";
  const rows = Array.isArray(job.rows) ? job.rows.map(r => ({ ...r })) : [];
  const rawRows = Array.isArray(job.rawRows) ? job.rawRows.map(r => ({ ...r })) : (rows.length ? rows.map(r => ({ ...r })) : []);
  const id = job.id || `inv-${Date.now()}`;
  const normalizedJob = {
    ...job,
    id,
    key: job.key || normalizeOrderKey(uniqueOrderNumbers, id),
    orderNumbers: uniqueOrderNumbers,
    clients: normalizedClients,
    client: primaryClient,
    rows,
    rawRows,
    status: (job.status || "draft").toLowerCase() === "finalized" ? "finalized" : "draft",
    discountMode: job.discountMode === "absolute" ? "absolute" : "percent",
    discountValue: Number(job.discountValue || 0) || 0,
    vatRate: Number(job.vatRate || 0) || 0,
    createdAt: job.createdAt || job.created_at || new Date().toISOString(),
    hidden: !!job.hidden,
  };
  ensureJobOrders(normalizedJob);
  return normalizedJob;
}

function normalizeInvoiceJobs(jobs){
  if (!Array.isArray(jobs)) return [];
  return jobs.map(normalizeInvoiceJob).filter(Boolean);
}

function formatInvoiceClientLabel(job){
  if (!job) return "—";
  let source = Array.isArray(job.clients) && job.clients.length
    ? job.clients
    : [];
  if (!source.length && job.client){
    source = [job.client];
  }
  if (!source.length && Array.isArray(job.orders)){
    source = job.orders.map(entry => entry?.clientName).filter(Boolean);
  }
  if (!source.length){
    source = ["—"];
  }
  const normalized = source.map(name => {
    const text = String(name || "").trim();
    return text || "—";
  });
  const unique = [];
  normalized.forEach(name=>{
    const lower = name.toLowerCase();
    if (!unique.some(existing => existing.toLowerCase() === lower)){
      unique.push(name);
    }
  });
  return unique.join(", ");
}

function addClientToJob(job, clientName){
  if (!job) return;
  const normalized = String(clientName || "").trim() || "—";
  if (!Array.isArray(job.clients)){
    job.clients = job.client ? [job.client] : [];
  }
  if (!job.clients.some(name => String(name || "").trim().toLowerCase() === normalized.toLowerCase())){
    job.clients.push(normalized);
  }
  job.client = job.clients[0] || normalized;
}

function getVisibleInvoiceJobs(){
  return (appState.invoices.jobs || []).filter(job => !job.hidden);
}

function hideInvoiceJobsContainingOrders(orderNumbers, keepId){
  const normalized = (orderNumbers || [])
    .map(num => String(num || "").trim().toLowerCase())
    .filter(Boolean);
  if (!normalized.length) return;
  const overlapSet = new Set(normalized);
  (appState.invoices.jobs || []).forEach(job=>{
    if (!job || job.id === keepId) return;
    const jobNumbers = (job.orderNumbers || []).map(num => String(num || "").trim().toLowerCase());
    const overlaps = jobNumbers.some(num => overlapSet.has(num));
    if (overlaps){
      job.hidden = true;
    }
  });
}

function unhideInvoiceJobsContainingOrders(orderNumbers){
  const normalized = (orderNumbers || [])
    .map(num => String(num || "").trim().toLowerCase())
    .filter(Boolean);
  if (!normalized.length) return;
  const matchSet = new Set(normalized);
  (appState.invoices.jobs || []).forEach(job=>{
    if (!job) return;
    const jobNumbers = (job.orderNumbers || []).map(num => String(num || "").trim().toLowerCase());
    if (jobNumbers.some(num => matchSet.has(num))){
      job.hidden = false;
    }
  });
}

function loadLegacyInvoiceJobs(){
  let stored = [];
  try{
    const raw = localStorage.getItem(INVOICE_JOBS_KEY);
    stored = raw ? JSON.parse(raw) : [];
  }catch{
    stored = [];
  }
  if (!Array.isArray(stored)) stored = [];
  return normalizeInvoiceJobs(stored);
}

function clearLegacyInvoiceJobs(){
  try{
    localStorage.removeItem(INVOICE_JOBS_KEY);
  }catch{/* ignore */}
}

async function fetchInvoiceJobsFromServer(){
  const res = await fetch(API_BASE + "/api/invoices");
  if (!res.ok){
    const text = await res.text().catch(()=> "");
    throw new Error(text || `Server responded with ${res.status}`);
  }
  const data = await res.json();
  return normalizeInvoiceJobs((data && data.jobs) || []);
}

async function ensureInvoiceJobsLoaded(){
  if (invoiceRuntime.jobsLoaded) return appState.invoices.jobs;
  if (invoiceRuntime.jobsPromise) return invoiceRuntime.jobsPromise;
  invoiceRuntime.jobsPromise = (async ()=>{
    let jobs = [];
    try{
      jobs = await fetchInvoiceJobsFromServer();
    }catch(error){
      console.warn("Failed to fetch invoices from server", error);
    }
    if (!jobs.length){
      const legacy = loadLegacyInvoiceJobs();
      if (legacy.length){
        const migrated = await saveInvoiceJobs(legacy);
        if (migrated){
          clearLegacyInvoiceJobs();
        }
        jobs = legacy;
      }
    }
    appState.invoices.jobs = normalizeInvoiceJobs(jobs);
    for (const job of appState.invoices.jobs){
      await recalcInvoiceJob(job, { allowPrompt: false });
    }
    invoiceRuntime.jobsLoaded = true;
    updateInvoicesUI();
    return appState.invoices.jobs;
  })();
  try{
    return await invoiceRuntime.jobsPromise;
  }finally{
    invoiceRuntime.jobsPromise = null;
  }
}

// Invoice jobs now sync to the server (Render disk) instead of localStorage
async function saveInvoiceJobs(jobsOverride){
  const normalized = normalizeInvoiceJobs(jobsOverride || appState.invoices.jobs);
  appState.invoices.jobs = normalized;
  const serialized = normalized.map(job => {
    const { calculated, ...rest } = job;
    return rest;
  });
  try{
    const res = await fetch(API_BASE + "/api/invoices", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ jobs: serialized }),
    });
    if (!res.ok){
      const text = await res.text().catch(()=> "");
      throw new Error(text || `Server responded with ${res.status}`);
    }
    invoiceRuntime.jobsLoaded = true;
    return true;
  }catch(error){
    console.warn("Failed to save invoices to server", error);
    setInvoiceStatus("Could not sync invoices. Changes are kept in this session.");
    invoiceRuntime.jobsLoaded = true;
    return false;
  }
}

function ensureInvoicesLoaded(){
  if (invoiceRuntime.initialized) return;
  TYPE_CORRECTIONS = loadTypeCorrections();
  loadInvoicePrices();
  loadInvoiceCounter();
  invoiceRuntime.initialized = true;
  syncInvoiceConfigFromServer({ silent: true });
  ensureInvoiceJobsLoaded();
}

function setInvoiceStatus(msg){
  if (invoiceStatusEl){
    invoiceStatusEl.textContent = msg || "";
  }
}

function normalizeOrderKey(numbers, fallback){
  const joined = Array.isArray(numbers) ? numbers.map(v => String(v || "").trim()).filter(Boolean).join(", ") : "";
  const base = joined || (fallback ? String(fallback).trim() : "");
  return base.toLowerCase();
}

function parseInvoiceOrderNumbers(order){
  if (!order) return [];
  if (Array.isArray(order.order_numbers) && order.order_numbers.length){
    return order.order_numbers.map(val => String(val || "").trim()).filter(Boolean);
  }
  const label = getOrderLabel(order);
  if (label && label !== "—"){
    return label.split(/\s*,\s*/).map(token => token.trim()).filter(Boolean);
  }
  const fallback = pickFirstString([order.order_number, order.orderNumber, order.id]);
  return fallback ? [fallback] : [];
}

function isOrderAlreadyInvoiced(order){
  if (!order) return false;
  const orderNumbers = dedupeAndSortOrderNumbers(parseInvoiceOrderNumbers(order));
  const normalizedNumbers = orderNumbers
    .map(num => String(num || "").trim().toLowerCase())
    .filter(Boolean);
  if ((!normalizedNumbers.length) && order.id != null){
    normalizedNumbers.push(String(order.id).trim().toLowerCase());
  }
  if (!normalizedNumbers.length) return false;
  const normalizedSet = new Set(normalizedNumbers);
  const jobs = Array.isArray(appState?.invoices?.jobs) ? appState.invoices.jobs : [];
  return jobs.some(job => {
    if (!job) return false;
    const jobNumbers = Array.isArray(job.orderNumbers) ? job.orderNumbers : [];
    if (jobNumbers.some(num => normalizedSet.has(String(num || "").trim().toLowerCase()))){
      return true;
    }
    const jobOrders = Array.isArray(job.orders) ? job.orders : [];
    if (jobOrders.some(entry => normalizedSet.has(String(entry?.orderNumber || "").trim().toLowerCase()))){
      return true;
    }
    const jobOrderIds = Array.isArray(job.order_ids) ? job.order_ids : [];
    if (jobOrderIds.some(id => normalizedSet.has(String(id || "").trim().toLowerCase()))){
      return true;
    }
    return false;
  });
}

function tokenizeComposition(typeString){
  const raw = String(typeString || "");
  const compact = raw.replace(/^\s*\d+\s*vetri?\s*/i, "").replace(/\s+/g, " ").trim();
  const firstDigitIdx = compact.search(/\d/);
  const core = firstDigitIdx >= 0 ? compact.slice(firstDigitIdx) : compact;
  const withoutMm = core.replace(/\b\d+\s*mm\b/gi, "").replace(/\bmm\b/gi, "");
  const tokens = withoutMm.split("+").map(t => t.trim()).filter(Boolean);
  return tokens;
}

function stripThermalDescriptors(token){
  return String(token || "")
    .replace(/\b(termico|caldo|c\.?caldo)\b/gi, "")
    .replace(/\s+/g, " ")
    .trim();
}

function normalizeGlassToken(token, opts = {}){
  const { forcePane = false } = opts;
  let cleaned = stripThermalDescriptors(token);
  if (!cleaned) return "";
  const numericOnly = /^\d+(?:[.,]\d+)?$/.test(cleaned);
  if (numericOnly && forcePane){
    cleaned = `${cleaned}F`;
  }
  return cleaned;
}

function parseGlassPanes(typeString){
  const tokens = tokenizeComposition(typeString);
  if (!tokens.length){
    return [];
  }
  const panes = [];
  tokens.forEach(token=>{
    const hasThermalWord = /\b(termico|caldo|c\.?caldo)\b/i.test(token);
    const cleanedToken = normalizeGlassToken(token, { forcePane: hasThermalWord });
    if (!cleanedToken) return;
    const numericCandidate = cleanedToken.replace(/\s+/g, "");
    const numeric = Number(numericCandidate.replace(",", "."));
    if (Number.isFinite(numeric) && !hasThermalWord){
      return; // spacer token
    }
    const norm = normalizeGlassKey(cleanedToken);
    if (!norm || norm.includes("vetri")) return;
    panes.push(cleanedToken);
  });
  return panes.length ? panes : tokens;
}

function getSpacerThickness(typeString){
  const tokens = tokenizeComposition(typeString);
  for (const token of tokens){
    const numericCandidate = token.replace(/\s+/g, "");
    const numeric = Number(numericCandidate.replace(",", "."));
    if (Number.isFinite(numeric)){
      return numeric;
    }
  }
  return null;
}

function getSpacerKind(typeString){
  const text = (typeString || "").toLowerCase();
  if (text.includes("termico") || text.includes("warm edge") || text.includes("warm-edge") || text.includes("warm") || text.includes("tgi")){
    return "thermal";
  }
  return "normal";
}

function parseInvoiceComposition(typeString){
  const tokens = tokenizeComposition(typeString);
  const panes = [];
  const spacerThicknesses = [];
  const glassTable = appState?.invoices?.priceLists?.glass || {};
  tokens.forEach(token=>{
    const hasThermalWord = /\b(termico|caldo|c\.?caldo)\b/i.test(token);
    const cleanedToken = normalizeGlassToken(token, { forcePane: hasThermalWord });
    if (!cleanedToken) return;
    const numericCandidate = cleanedToken.replace(/\s+/g, "");
    const numeric = Number(numericCandidate.replace(",", "."));
    const normalizedGlass = normalizeGlassKey(cleanedToken);
    if (Number.isFinite(numeric) && !hasThermalWord){
      spacerThicknesses.push(numeric);
    }else if (normalizedGlass && normalizedGlass.length && !normalizedGlass.includes("vetri")){
      panes.push(cleanedToken);
      if (!glassTable.hasOwnProperty(normalizedGlass)){
        // will trigger missing price prompt via lookup later
      }
    }
  });
  return {
    panes,
    spacerThicknesses,
    spacerKind: getSpacerKind(typeString),
  };
}

function mapInvoiceRow(row){
  if (!row) return null;
  const orderId = pickFirstString([
    row.orderId,
    row.order_id,
    row.order_number,
    row.orderNumber,
    row.order,
    row.orderLabel,
  ], null);
  const originalType = pickFirstString([
    row.type,
    row.composition_display,
    row.composition_raw,
    row.glass_type,
    row.glassType,
    row.description,
  ], "");
  const type = applyTypeCorrections(originalType);
  const dimension = deriveLabelDimension(row) || "—";
  const position = pickPositionValue(row) || "—";
  const qty = Math.max(0, Number(pickFirstNumber([
    row.quantity,
    row.qty,
    row.Qty,
    row.QTY,
    row.total_qty,
  ]) || 0));
  const area = (() => {
    const candidate = toNumericArea(row.area ?? row.area_m2 ?? row.areaM2 ?? row.computed_area);
    if (candidate != null) return candidate;
    return computeRowArea(row);
  })();
  return {
    orderId: orderId || null,
    rawType: originalType || null,
    type,
    dimension,
    position,
    quantity: qty,
    area: Number.isFinite(area) ? Number(area) : 0,
  };
}

function buildInvoiceJobFromOrder(order){
  const orderNumbers = dedupeAndSortOrderNumbers(parseInvoiceOrderNumbers(order));
  const normalizedKey = normalizeOrderKey(orderNumbers, order?.id);
  const clientName = pickFirstString([
    order.client,
    order.client_name,
    order.clientName,
    order.client_hint,
    order.customer,
  ], "—");
  const defaultOrderId = orderNumbers[0] || null;
  const mappedRows = Array.isArray(order?.rows)
    ? order.rows.map(row => mapInvoiceRow({
        orderId: row.orderId ?? row.order_id ?? row.order_number ?? defaultOrderId,
        order_number: row.order_number ?? defaultOrderId,
        ...row,
      })).filter(Boolean)
    : [];
  const createdAt = extractOrderDate(order) || new Date();
  const orderEntries = (orderNumbers.length ? orderNumbers : [defaultOrderId || "—"]).map(num => createOrderMetadata({
    orderNumber: num || "—",
    clientName,
    createdAt: createdAt.toISOString(),
    units: extractOrderUnits(order),
    area: extractOrderArea(order),
    amount: 0,
  }));
  return {
    id: order?.id ? `inv-${order.id}` : `inv-${Date.now()}`,
    key: normalizedKey,
    source: String(order?.source || "").toLowerCase() === "manual" ? "manual" : "pdf",
    orderNumbers,
    client: clientName || "—",
    clients: [clientName || "—"],
    orders: orderEntries,
    createdAt: createdAt.toISOString(),
    rows: mappedRows.map(r => ({ ...r })),
    rawRows: mappedRows.map(r => ({ ...r })),
    status: "draft",
    discountMode: "percent",
    discountValue: 0,
    vatRate: 0,
    invoiceNumber: order?.invoiceNumber || null,
    hidden: false,
    totalUnits: 0,
    totalArea: 0,
    totalAmount: 0,
  };
}

async function addInvoiceJobFromOrder(order){
  if (!order) return null;
  ensureInvoicesLoaded();
  await ensureInvoiceJobsLoaded();
  const job = normalizeInvoiceJob(buildInvoiceJobFromOrder(order));
  if (!job.rows.length){
    setInvoiceStatus("Order has no rows to invoice.");
    return null;
  }
  const lookupNumbers = new Set(job.orderNumbers.map(n => n.toLowerCase()));
  const existingIndex = appState.invoices.jobs.findIndex(item => {
    const itemSource = String(item?.source || "pdf").toLowerCase() === "manual" ? "manual" : "pdf";
    if (itemSource !== job.source) return false;
    const numbers = Array.isArray(item.orderNumbers) ? item.orderNumbers : [];
    return numbers.some(num => lookupNumbers.has(String(num || "").toLowerCase()));
  });
  if (existingIndex >= 0){
    const keep = appState.invoices.jobs[existingIndex];
    const merged = normalizeInvoiceJob({ ...keep });
    const missing = job.orderNumbers.filter(num => !merged.orderNumbers.some(existing => existing.toLowerCase() === num.toLowerCase()));
    if (missing.length){
      merged.orderNumbers = dedupeAndSortOrderNumbers([...merged.orderNumbers, ...missing]);
    }
    merged.key = normalizeOrderKey(merged.orderNumbers, merged.id);
    if (Array.isArray(job.clients)){
      job.clients.forEach(name => addClientToJob(merged, name));
    }else{
      addClientToJob(merged, job.client);
    }
    merged.createdAt = merged.createdAt || job.createdAt;
    merged.hidden = false;
    addOrdersMetadata(merged, job.orders || []);
    ensureJobOrders(merged);
    appState.invoices.jobs.splice(existingIndex, 1, merged);
    appState.invoices.activeJobId = merged.id;
    await recalcInvoiceJob(merged, { allowPrompt: true });
  }else{
    appState.invoices.activeJobId = job.id;
    await recalcInvoiceJob(job, { allowPrompt: true });
    appState.invoices.jobs.unshift(job);
  }
  await saveInvoiceJobs();
  updateInvoicesUI();
  setInvoiceStatus("Invoice job updated from History.");
  return job;
}

async function createInvoiceJobFromOrder(order){
  if (!order) return null;
  if (!canInvoiceStatus(order.status)){
    throw new Error(`Order status ${historyStatusLabel(order.status)} is not eligible for invoice.`);
  }
  if (!Array.isArray(order.rows) || !order.rows.length){
    if (order.id == null) return null;
    const detailed = await fetchOrder(order.id);
    if (!canInvoiceStatus(detailed.status)){
      throw new Error(`Order status ${historyStatusLabel(detailed.status)} is not eligible for invoice.`);
    }
    return addInvoiceJobFromOrder(detailed);
  }
  return addInvoiceJobFromOrder(order);
}

function setInvoiceAddOrderStatus(message){
  if (invoiceAddOrderStatus){
    invoiceAddOrderStatus.textContent = message || "";
  }
}

function closeInvoiceAddOrderModal(){
  if (invoiceAddOrderModal){
    invoiceAddOrderModal.hidden = true;
  }
}

function filterInvoiceAddOrderCandidates(job){
  if (!job) return [];
  const orders = Array.isArray(invoiceAddOrderState.orders) ? invoiceAddOrderState.orders : [];
  const searchTerm = (invoiceAddOrderState.searchTerm || "").trim().toLowerCase();
  const currentNumbers = (Array.isArray(job.orderNumbers) ? job.orderNumbers : [])
    .map(num => String(num || "").toLowerCase())
    .filter(Boolean);
  const currentSet = new Set(currentNumbers);
  return orders.filter(order => {
    const status = (order.status || "").toLowerCase();
    if (status !== "approved") return false;
    const numbers = parseInvoiceOrderNumbers(order);
    if (!numbers.length) return false;
    const normalized = numbers.map(num => String(num || "").toLowerCase());
    const overlapCurrent = normalized.some(num => currentSet.has(num));
    if (overlapCurrent) return false;
    if (searchTerm){
      const orderLabel = numbers.join(", ").toLowerCase();
      const clientLabel = String(pickFirstString([order.client, order.client_hint, order.client_name, order.customer], "—") || "").toLowerCase();
      const dateLabel = String(formatDate(order.created_at) || "").toLowerCase();
      const matchesSearch = orderLabel.includes(searchTerm)
        || clientLabel.includes(searchTerm)
        || dateLabel.includes(searchTerm);
      if (!matchesSearch) return false;
    }
    return true;
  });
}

function renderInvoiceAddOrderList(job){
  if (!invoiceAddOrderList) return;
  if (!job){
    invoiceAddOrderList.innerHTML = '<div class="invoice-empty">Select an invoice first.</div>';
    return;
  }
  if (invoiceAddOrderState.loading){
    invoiceAddOrderList.innerHTML = '<div class="invoice-empty">Loading approved orders…</div>';
    return;
  }
  const candidates = filterInvoiceAddOrderCandidates(job);
  const hasSearch = (invoiceAddOrderState.searchTerm || "").trim().length > 0;
  if (!candidates.length){
    const emptyMessage = hasSearch
      ? "No orders match your search."
      : "No matching approved orders available.";
    invoiceAddOrderList.innerHTML = `<div class="invoice-empty">${emptyMessage}</div>`;
    return;
  }
  const rows = candidates.map(order=>{
    const orderNumbers = parseInvoiceOrderNumbers(order);
    const created = formatDate(order.created_at);
    const client = pickFirstString([order.client, order.client_hint, order.client_name, order.customer], "—");
    const units = Number(pickFirstNumber([order.units_total, order.parsed_units, order.total_units])) || 0;
    const area = Number(pickFirstNumber([order.area_total, order.parsed_area, order.total_area])) || 0;
    const orderLabel = escapeHtml(orderNumbers.join(", "));
    return `<tr>
      <td>${escapeHtml(created)}</td>
      <td>${orderLabel}</td>
      <td>${escapeHtml(client)}</td>
      <td>${units}</td>
      <td>${formatArea(area)}</td>
      <td class="actions-cell" style="text-align:right"><button class="btn small" data-add-order-id="${escapeHtml(order.id)}">Select</button></td>
    </tr>`;
  }).join("");
  invoiceAddOrderList.innerHTML = `<table>
    <thead>
      <tr>
        <th>Date / Time</th>
        <th>Order number</th>
        <th>Client</th>
        <th>Units</th>
        <th>Area (m²)</th>
        <th>Actions</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>`;
}

async function loadInvoiceAddOrderCandidates(job){
  if (!job) return;
  invoiceAddOrderState.loading = true;
  setInvoiceAddOrderStatus("Loading approved orders…");
  renderInvoiceAddOrderList(job);
  try{
    const orders = await fetchAllApprovedOrders();
    invoiceAddOrderState.orders = Array.isArray(orders) ? orders : [];
    invoiceAddOrderState.error = "";
    setInvoiceAddOrderStatus(invoiceAddOrderState.orders.length ? "" : "No approved orders found.");
  }catch(error){
    invoiceAddOrderState.error = error?.message || String(error);
    setInvoiceAddOrderStatus("Failed to load approved orders.");
  }finally{
    invoiceAddOrderState.loading = false;
    renderInvoiceAddOrderList(job);
  }
}

function openInvoiceAddOrderModal(){
  const job = getActiveInvoiceJob();
  if (!job){
    setInvoiceStatus("Select an invoice job first.");
    return;
  }
  if (invoiceAddOrderHint){
    const orderLabel = (job.orderNumbers || []).join(", ") || "—";
    const clientLabel = formatInvoiceClientLabel(job) || "—";
    invoiceAddOrderHint.textContent = `${clientLabel} • Orders: ${orderLabel}`;
  }
  invoiceAddOrderState.searchTerm = "";
  if (invoiceAddOrderSearch){
    invoiceAddOrderSearch.value = "";
  }
  if (invoiceAddOrderModal){
    invoiceAddOrderModal.hidden = false;
  }
  setInvoiceAddOrderStatus("");
  invoiceAddOrderState.loading = true;
  renderInvoiceAddOrderList(job);
  loadInvoiceAddOrderCandidates(job);
}

async function handleInvoiceAddOrderSelect(orderId){
  ensureInvoicesLoaded();
  await ensureInvoiceJobsLoaded();
  const job = getActiveInvoiceJob();
  if (!job || !orderId) return;
  try{
    setInvoiceAddOrderStatus("Loading order…");
    const order = await fetchOrder(orderId);
    const incomingNumbers = dedupeAndSortOrderNumbers(parseInvoiceOrderNumbers(order));
    const defaultOrderId = incomingNumbers[0] || null;
    const mappedRows = Array.isArray(order?.rows)
      ? order.rows.map(row => mapInvoiceRow({
          orderId: row.orderId ?? row.order_id ?? row.order_number ?? defaultOrderId,
          order_number: row.order_number ?? defaultOrderId,
          ...row,
        })).filter(Boolean)
      : [];
    if (!mappedRows.length){
      setInvoiceAddOrderStatus("Selected order has no rows to import.");
      return;
    }
    const rawRows = ensureInvoiceRawRows(job);
    mappedRows.forEach(row => rawRows.push({ ...row }));
    if (!Array.isArray(job.orderNumbers)){
      job.orderNumbers = [];
    }
    const orderDate = extractOrderDate(order) || new Date();
    const orderClient = pickFirstString([
      order.client,
      order.client_name,
      order.clientName,
      order.client_hint,
      order.customer,
    ], "—");
    incomingNumbers.forEach(num=>{
      const normalized = String(num || "").toLowerCase();
      const exists = job.orderNumbers.some(existing => String(existing || "").toLowerCase() === normalized);
      if (!exists && num){
        job.orderNumbers.push(num);
      }
    });
    const orderEntries = incomingNumbers.map(num => createOrderMetadata({
      orderNumber: num,
      clientName: orderClient,
      createdAt: orderDate.toISOString(),
      units: extractOrderUnits(order),
      area: extractOrderArea(order),
      amount: 0,
    }));
    addOrdersMetadata(job, orderEntries);
    ensureJobOrders(job);
    job.orderNumbers = dedupeAndSortOrderNumbers(job.orderNumbers);
    job.key = normalizeOrderKey(job.orderNumbers, job.id);
    job.rows = rawRows;
    job.rawRows = rawRows;
    addClientToJob(job, orderClient);
    job.hidden = false;
    hideInvoiceJobsContainingOrders(incomingNumbers, job.id);
    appState.invoices.activeJobId = job.id;
    await recalcInvoiceJob(job, { allowPrompt: true });
    await saveInvoiceJobs();
    updateInvoicesUI();
    const label = incomingNumbers.length
      ? incomingNumbers.join(", ")
      : order?.order_number || orderId;
    setInvoiceAddOrderStatus(`Order ${label} added to invoice.`);
    renderInvoiceAddOrderList(job);
    setInvoiceStatus("Order added to invoice.");
  }catch(error){
    console.error("Failed to add order to invoice", error);
    setInvoiceAddOrderStatus("Failed to add order: " + (error.message || error));
  }
}

async function removeOrderFromInvoiceJob(jobId, orderNumber){
  await ensureInvoicesLoaded();
  await ensureInvoiceJobsLoaded();
  const jobs = appState.invoices.jobs || [];
  const idx = jobs.findIndex(job => job && job.id === jobId);
  if (idx < 0) return null;
  const job = jobs[idx];
  const normalizedTarget = String(orderNumber || "").trim().toLowerCase();
  if (!normalizedTarget){
    console.warn("removeOrderFromInvoiceJob called without order number");
    return job;
  }
  ensureInvoiceRawRows(job);
  const matchesTarget = value => String(value || "").trim().toLowerCase() === normalizedTarget;
  let orderWasPresent = (job.orderNumbers || []).some(matchesTarget);
  if (!orderWasPresent){
    orderWasPresent = (job.orders || []).some(entry => matchesTarget(entry?.orderNumber));
  }
  if (!orderWasPresent){
    return job;
  }
  job.orderNumbers = (job.orderNumbers || []).filter(num => !matchesTarget(num));
  job.orders = (job.orders || []).filter(entry => !matchesTarget(entry?.orderNumber));
  const filterRowsByOrder = rows=>{
    if (!Array.isArray(rows)) return [];
    return rows.filter(row=>{
      if (!row || typeof row !== "object") return true;
      const orderLabel = pickFirstString([
        row.orderId,
        row.order_id,
        row.order_number,
        row.orderNumber,
        row.order,
        row.orderLabel,
      ], "").trim().toLowerCase();
      return !orderLabel || orderLabel !== normalizedTarget;
    });
  };
  const filteredRawRows = filterRowsByOrder(job.rawRows);
  job.rawRows = filteredRawRows;
  job.rows = filteredRawRows;
  if (!job.orderNumbers.length || !job.rawRows.length){
    const removedJob = jobs.splice(idx, 1)[0];
    if (appState.invoices.activeJobId === removedJob.id){
      appState.invoices.activeJobId = null;
    }
    unhideInvoiceJobsContainingOrders([orderNumber]);
    await saveInvoiceJobs();
    return null;
  }
  const nextClients = [];
  (job.orders || []).forEach(entry=>{
    const text = String(entry?.clientName || "").trim();
    if (text && !nextClients.some(existing => existing.toLowerCase() === text.toLowerCase())){
      nextClients.push(text);
    }
  });
  if (nextClients.length){
    job.clients = nextClients;
    job.client = nextClients[0];
  }
  job.key = normalizeOrderKey(job.orderNumbers, job.id);
  job.hidden = false;
  ensureJobOrders(job);
  await recalcInvoiceJob(job, { allowPrompt: false });
  applyJobTotals(job, job.calculated);
  unhideInvoiceJobsContainingOrders([orderNumber]);
  await saveInvoiceJobs();
  return job;
}

function getActiveInvoiceJob(){
  if (!appState.invoices.activeJobId) return null;
  let job = appState.invoices.jobs.find(job => job.id === appState.invoices.activeJobId) || null;
  if (job && job.hidden){
    const fallback = getVisibleInvoiceJobs();
    if (fallback.length){
      job = fallback[0];
      appState.invoices.activeJobId = job.id;
    }else{
      job = null;
      appState.invoices.activeJobId = null;
    }
  }
  return job;
}

function selectInvoiceJob(id){
  appState.invoices.activeJobId = id || null;
}

function getSpacerPrice(thickness, kind){
  const table = appState.invoices.priceLists.spacer || {};
  if (!Number.isFinite(thickness)){
    return { price: 0, missing: { reason: "thickness", thickness: null, kind } };
  }
  const entry = table[thickness];
  if (!entry){
    return { price: 0, missing: { reason: "thickness", thickness, kind } };
  }
  const normalizedKind = kind === "thermal" ? "thermal" : "normal";
  const candidate = entry[normalizedKind];
  if (!Number.isFinite(candidate)){
    return { price: 0, missing: { reason: "kind", thickness, kind: normalizedKind } };
  }
  return { price: Number(candidate), missing: null };
}

function enqueueInvoicePrompt(payload){
  const key = payload.kind === "glass"
    ? `glass:${payload.normalized}`
    : `spacer:${payload.thickness}:${payload.kind || "unknown"}`;
  if (invoiceRuntime.missingKeys.has(key)) return;
  invoiceRuntime.missingKeys.add(key);
  invoiceRuntime.pendingPrompts.push(payload);
}

function triggerInvoicePrompt(){
  if (!invoiceRuntime.pendingPrompts.length) return;
  if (!invoicePromptLayer || !invoicePromptFields) return;
  if (!invoicePromptLayer.hidden) return;
  const activeJob = getActiveInvoiceJob();
  if (!activeJob){
    console.warn("Invoice prompt requested with no active job.");
    invoiceRuntime.pendingPrompts = [];
    return;
  }
  const payload = invoiceRuntime.pendingPrompts.shift();
  if (!payload || !payload.kind){
    console.warn("Invoice prompt missing payload", payload);
    return;
  }
  if (payload.jobId && payload.jobId !== activeJob.id){
    console.warn("Invoice prompt job mismatch; ignoring", payload);
    return;
  }
  const descriptor = payload.kind === "glass"
    ? (payload.label || payload.normalized || "Unknown glass")
    : (payload.description || `Spacer ${payload.thickness ?? "?"}mm (${payload.kind || "unknown"})`);
  invoiceRuntime.currentMissing = {
    kind: payload.kind,
    key: payload.kind === "glass" ? (payload.normalized || normalizeGlassKey(descriptor)) : String(payload.thickness || ""),
    label: descriptor,
    jobId: payload.jobId,
    spacerKind: payload.kind === "spacer" ? (payload.kind || "normal") : null,
    thickness: payload.thickness,
  };
  if (invoicePromptDescriptor){
    invoicePromptDescriptor.textContent = descriptor;
  }
  if (invoicePromptValue){
    invoicePromptValue.value = "";
    invoicePromptValue.classList.remove("input-error");
  }
  invoicePromptTitle.textContent = "New price";
  invoicePromptText.textContent = "I found a new glass or spacer type that has no price yet. Enter price per m² (ALL):";
  invoicePromptLayer.hidden = false;
}

function hideInvoicePrompt(){
  if (invoicePromptLayer){
    invoicePromptLayer.hidden = true;
  }
  invoiceRuntime.modalPayload = null;
  invoiceRuntime.currentMissing = null;
}

async function handleInvoicePromptSave(){
  await ensureInvoiceJobsLoaded();
  const payload = invoiceRuntime.currentMissing;
  if (!payload){
    console.warn("Invoice prompt save with no payload");
    hideInvoicePrompt();
    return;
  }
  const price = invoicePromptValue ? Number(invoicePromptValue.value) : NaN;
  if (!Number.isFinite(price) || price < 0){
    if (invoicePromptValue){
      invoicePromptValue.classList.add("input-error");
      setTimeout(()=> invoicePromptValue.classList.remove("input-error"), 600);
    }
    return;
  }
  if (payload.kind === "glass"){
    const normalized = normalizeGlassKey(payload.key || payload.label);
    if (!normalized){
      console.warn("Missing glass key for invoice prompt", payload);
      hideInvoicePrompt();
      return;
    }
    appState.invoices.priceLists.glass[normalized] = price;
    invoiceRuntime.missingKeys.delete(`glass:${normalized}`);
  }else if (payload.kind === "spacer"){
    const thicknessValue = Number(payload.thickness);
    if (!Number.isFinite(thicknessValue)){
      console.warn("Missing spacer thickness for invoice prompt", payload);
      hideInvoicePrompt();
      return;
    }
    const baseline = appState.invoices.priceLists.spacer[thicknessValue] || {};
    const spacerEntry = {
      normal: baseline.normal ?? getDefaultSpacerPrices()[thicknessValue]?.normal,
      thermal: baseline.thermal ?? getDefaultSpacerPrices()[thicknessValue]?.thermal,
    };
    const kind = (payload.spacerKind || "normal") === "thermal" ? "thermal" : "normal";
    spacerEntry[kind] = price;
    appState.invoices.priceLists.spacer[thicknessValue] = spacerEntry;
    invoiceRuntime.missingKeys.delete(`spacer:${thicknessValue}:${kind}`);
  }
  saveInvoicePrices();
  clearGlassAliasCache();
  const job = appState.invoices.jobs.find(item => item.id === payload.jobId);
  hideInvoicePrompt();
  if (job){
    await recalcInvoiceJob(job, { allowPrompt: true });
  }
  await saveInvoiceJobs();
  renderInvoicePriceEditor();
  updateInvoiceSummary();
  updateInvoicesUI();
  const saved = await persistInvoiceConfigToServer(buildInvoiceConfigPayload(), { preferExistingTypeCorrections: true });
  if (!saved){
    setInvoiceStatus("New price saved locally. Will sync when online.");
  }
}

function formatMoney(value){
  const num = Number.isFinite(Number(value)) ? Number(value) : 0;
  return num.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

function describeMissingPricing(missing){
  if (!missing) return "";
  if (missing.type === "glass"){
    const keyLabel = missing.normalized || missing.pane || missing?.details?.normalized || "unknown";
    return `Missing glass price for "${keyLabel}".`;
  }
  if (missing.type === "spacer"){
    const info = missing.details || {};
    const thick = info.thickness != null ? `${info.thickness} mm` : "unknown thickness";
    const kind = info.kind || "unknown";
    return `Missing spacer price (${kind}, ${thick}).`;
  }
  return "Missing pricing details.";
}

async function recalcInvoiceJob(job, options = {}){
  if (!job) return null;
  const { allowPrompt = false } = options;
  const promptAllowedForJob = allowPrompt && appState.invoices.activeJobId === job.id;
  const rawRows = ensureInvoiceRawRows(job);
  const lines = await buildInvoiceLinesFromRaw(rawRows, { promptAllowedForJob });
  lines.forEach(line=>{
    if (!Array.isArray(line.rawIndexes)) return;
    line.rawIndexes.forEach(idx=>{
      if (rawRows[idx]){
        rawRows[idx].spacerKind = line.spacerKind;
        rawRows[idx].type = line.type;
        rawRows[idx].aiAssisted = !!line.aiAssisted;
      }
    });
  });
  finalizeInvoiceTotals(job, lines);
  if (promptAllowedForJob && invoiceRuntime.pendingPrompts.length && !invoiceRuntime.currentMissing){
    triggerInvoicePrompt();
  }
  return job.calculated;
}

function ensureInvoiceRawRows(job){
  if (!job) return [];
  if (!Array.isArray(job.rawRows)){
    job.rawRows = Array.isArray(job.rows) ? job.rows.map(r => ({ ...r })) : [];
  }
  const defaultOrderId = Array.isArray(job.orderNumbers) && job.orderNumbers.length === 1 ? job.orderNumbers[0] : null;
  if (defaultOrderId){
    job.rawRows.forEach(row=>{
      if (!row || typeof row !== "object") return;
      const hasOrderId = row.orderId || row.order_id || row.order_number || row.orderNumber;
      if (!hasOrderId){
        row.orderId = defaultOrderId;
        row.order_number = defaultOrderId;
      }
    });
  }
  job.rows = job.rawRows;
  return job.rawRows;
}

async function buildInvoiceLinesFromRaw(rawRows, options = {}){
  const promptAllowedForJob = !!options.promptAllowedForJob;
  const groups = new Map();
  (rawRows || []).forEach((row, idx)=>{
    if (!row || typeof row !== "object") return;
    const orderId = pickFirstString([
      row.orderId,
      row.order_id,
      row.order_number,
      row.orderNumber,
      row.order,
      row.orderLabel,
    ], "");
    const normalizedOrder = orderId ? orderId.toLowerCase() : "";
    const originalTypeText = String(row.type || "").trim();
    const rawOriginal = row.rawType != null ? String(row.rawType) : null;
    const correctedTypeText = applyTypeCorrections(originalTypeText);
    if (row.type !== correctedTypeText){
      row.type = correctedTypeText;
    }
    const typeText = correctedTypeText;
    const normalizedType = normalizeGlassKey(typeText) || typeText.toLowerCase();
    const key = `${normalizedOrder}|${normalizedType}`;
    if (!groups.has(key)){
      groups.set(key, {
        displayType: typeText,
        normalizedType,
        orderId: orderId || null,
        qty: 0,
        area: 0,
        rawIndexes: [],
        spacerKind: row.spacerKind || null,
        rawType: rawOriginal && rawOriginal.trim() ? rawOriginal : null,
        aiAssisted: !!row.aiAssisted,
      });
    }
    const group = groups.get(key);
    if (!group.rawType && rawOriginal && rawOriginal.trim()){
      group.rawType = rawOriginal;
    }
    if (!group.spacerKind && row.spacerKind){
      group.spacerKind = row.spacerKind;
    }
    if (row.aiAssisted){
      group.aiAssisted = true;
    }
    const qty = Math.max(0, Number(row.quantity || 0));
    const areaPerUnit = (()=> {
      const direct = Number(row.area ?? row.area_m2 ?? row.areaM2);
      if (Number.isFinite(direct)) return direct;
      const computed = computeRowArea(row);
      return Number.isFinite(computed) ? computed : 0;
    })();
    group.qty += qty;
    group.area += areaPerUnit * qty;
    group.rawIndexes.push(idx);
  });

  const lines = [];
  const groupEntries = Array.from(groups.values());
  for (let index = 0; index < groupEntries.length; index += 1){
    const line = await computeInvoiceLineFromGroup(groupEntries[index], index, { promptAllowedForJob });
    lines.push(line);
  }
  return lines;
}

async function computeInvoiceLineFromGroup(group, index, options = {}){
  const promptAllowedForJob = !!options.promptAllowedForJob;
  const composition = parseInvoiceComposition(group.displayType || "");
  const glassPriceTable = appState.invoices.priceLists.glass || {};
  let glassPricePerM2 = 0;
  let aiAssisted = !!group.aiAssisted;
  let lineIsLaminated = false;
  let lineDisplayType = group.displayType || "";
  const glassPriceKeys = Object.keys(glassPriceTable);
  const glassIssues = [];
  for (const pane of composition.panes){
    const result = await resolveGlassType(pane, glassPriceKeys);
    if (result?.fromAi){
      aiAssisted = true;
    }
    const matchedKey = result?.match;
    const normalizedKey = matchedKey || normalizeGlassKey(pane);
    const priceKey = normalizedKey;
    const price = priceKey ? glassPriceTable[priceKey] : undefined;
    if (!Number.isFinite(price)){
      const canonicalLabel = priceKey || matchedKey || normalizeGlassKey(pane) || pane;
      console.log("[GlassPriceMissing] rawName:", pane, "normalizedKey:", canonicalLabel, "availableKeys:", Object.keys(glassPriceTable || {}));
      glassIssues.push({
        missing: { type: "glass", pane: canonicalLabel, normalized: canonicalLabel },
        prompt: { kind: "glass", label: canonicalLabel, normalized: canonicalLabel },
      });
    }else{
      glassPricePerM2 += Number(price);
    }
  }
  const inferredKind = (composition.spacerKind === "thermal" || composition.spacerKind === "normal")
    ? composition.spacerKind
    : null;
  let spacerModeResolved = false;
  let spacerMode = null;
  if (group.spacerKind === "thermal" || group.spacerKind === "normal"){
    spacerMode = group.spacerKind;
    spacerModeResolved = true;
  }else if (inferredKind){
    spacerMode = inferredKind;
    spacerModeResolved = true;
  }
  const spacerText = `${group.displayType || ""} ${group.rawType || ""}`;
  const caldoDetected = /\bcaldo\b/i.test(spacerText);
  if (caldoDetected){
    spacerMode = "thermal";
    spacerModeResolved = true;
    aiAssisted = true;
  }
  if (!spacerMode){
    spacerMode = "normal";
  }
  const thicknesses = Array.isArray(composition.spacerThicknesses) && composition.spacerThicknesses.length
    ? composition.spacerThicknesses
    : [];
  const needsGlassAi = glassIssues.length > 0;
  const needsSpacerAi = !spacerModeResolved;
  const needsHistoricalAi = group.aiAssisted === true;
  const rawLineText = group.rawType || group.displayType || "";
  if ((needsGlassAi || needsSpacerAi || needsHistoricalAi) && rawLineText){
    console.log("[GlassAI] Calling AI for raw line:", rawLineText);
    const analysis = await analyzeInvoiceLineWithAI(rawLineText, glassPriceKeys);
    console.log("[GlassAI] Result:", analysis);
    if (analysis){
      const confident = Number.isFinite(analysis.confidence) ? Number(analysis.confidence) >= 0.5 : false;
      if (confident){
        aiAssisted = true;
        if (analysis.normalizedType){
          const cleaned = analysis.normalizedType.trim();
          if (cleaned){
            lineDisplayType = cleaned;
          }
        }
        lineIsLaminated = Boolean(analysis.isLaminated);
        if (analysis.glassKey && glassIssues.length){
          const aiGlassKeyMatch = glassPriceKeys.find(key => key.toLowerCase() === String(analysis.glassKey).toLowerCase());
          const aiGlassKey = aiGlassKeyMatch || analysis.glassKey;
          const aiPrice = glassPriceTable[aiGlassKey];
          const missingCount = glassIssues.length;
          glassIssues.length = 0;
          if (Number.isFinite(aiPrice)){
            glassPricePerM2 += Number(aiPrice) * missingCount;
          }else{
            const normalizedAiKey = normalizeGlassKey(aiGlassKey) || aiGlassKey;
            console.log("[GlassPriceMissing] rawName:", analysis.glassKey, "normalizedKey:", normalizedAiKey, "availableKeys:", Object.keys(glassPriceTable || {}));
            glassIssues.push({
              missing: { type: "glass", pane: normalizedAiKey, normalized: normalizedAiKey },
              prompt: { kind: "glass", label: normalizedAiKey, normalized: normalizedAiKey },
            });
          }
        }
        if (analysis.spacerMode === "thermal" || analysis.spacerMode === "normal"){
          spacerMode = analysis.spacerMode;
          spacerModeResolved = true;
        }
      }
    }
  }
  let spacerPricePerM2 = 0;
  const spacerIssues = [];
  thicknesses.forEach(th=>{
    const spacer = getSpacerPrice(th, spacerMode);
    if (spacer.missing){
      spacerIssues.push({
        missing: { type: "spacer", details: spacer.missing },
        prompt: {
          kind: "spacer",
          thickness: th,
          kind: spacerMode,
          description: `Spacer ${th ?? "?"}mm (${spacerMode || "normal"})`,
        },
      });
    }
    spacerPricePerM2 += spacer.price || 0;
  });
  const missingEntries = [...glassIssues, ...spacerIssues];
  let lineMissing = null;
  let lineMissingPrice = false;
  if (missingEntries.length){
    lineMissing = missingEntries[0].missing;
    lineMissingPrice = missingEntries.some(entry => entry?.missing?.type === "glass");
    if (promptAllowedForJob){
      missingEntries.forEach(entry=>{
        enqueueInvoicePrompt(entry.prompt);
      });
    }
  }
  const unitPricePerM2 = glassPricePerM2 + spacerPricePerM2;
  const lineTotal = unitPricePerM2 * group.area;
  const normalizedDisplayType = (() => {
    const text = lineDisplayType || group.displayType || "";
    const normalized = normalizeGlassKey(text);
    return normalized || text.toLowerCase();
  })();
  return {
    index: index + 1,
    orderId: group.orderId || null,
    type: lineDisplayType,
    displayType: lineDisplayType,
    normalizedType: normalizedDisplayType,
    quantity: group.qty,
    area: group.area,
    glassPricePerM2,
    spacerPricePerM2,
    spacerKind: spacerMode,
    spacerMode,
    spacerThicknesses: thicknesses.slice(),
    spacerOverride: null,
    unitPricePerM2,
    lineTotal,
    lineAreaTotal: group.area,
    rawIndexes: group.rawIndexes || [],
    missing: lineMissing,
    missingPrice: lineMissingPrice,
    rawType: group.rawType || null,
    isLaminated: lineIsLaminated,
    aiAssisted,
  };
}

function finalizeInvoiceTotals(job, lines){
  let subtotal = 0;
  let units = 0;
  let areaTotal = 0;
  let missing = null;
  lines.forEach(line=>{
    const lineTotal = Number(line.lineTotal || 0);
    const qty = Number(line.quantity || 0);
    const area = Number(line.area || line.lineAreaTotal || 0);
    subtotal += lineTotal;
    units += qty;
    areaTotal += area;
    if (!missing && line.missing){
      missing = line.missing;
    }
  });
  const discountRaw = Math.max(0, Number(job.discountValue || 0));
  const discountValue = job.discountMode === "absolute"
    ? Math.min(discountRaw, subtotal)
    : subtotal * (discountRaw / 100);
  const taxable = Math.max(0, subtotal - discountValue);
  const vatValue = taxable * (Math.max(0, Number(job.vatRate || 0)) / 100);
  const total = taxable + vatValue;
  job.calculated = {
    lines,
    subtotal,
    discountValue,
    vatValue,
    total,
    units,
    area: areaTotal,
    missing,
  };
  applyJobTotals(job, job.calculated);
}

async function recalcAllInvoices(opts = {}){
  const { allowPrompt = false } = opts;
  const jobs = appState.invoices.jobs || [];
  for (const job of jobs){
    await recalcInvoiceJob(job, { allowPrompt });
  }
}

function applyJobTotals(job, calc){
  if (!job) return;
  const source = calc || job.calculated || {};
  job.totalUnits = Number(source.units || 0);
  job.totalArea = Number(source.area || 0);
  job.totalAmount = Number(source.total || 0);
  updateJobOrdersFromCalc(job, source);
}

function updateInvoiceSummary(){
  if (!invoiceSummaryEl) return;
  const jobs = getVisibleInvoiceJobs();
  if (!jobs.length){
    invoiceSummaryEl.innerHTML = "";
    return;
  }
  let units = 0;
  let area = 0;
  let amount = 0;
  jobs.forEach(job=>{
    const calc = job.calculated;
    if (calc){
      applyJobTotals(job, calc);
    }
    units += Number(job.totalUnits || 0);
    area += Number(job.totalArea || 0);
    amount += Number(job.totalAmount || 0);
  });
  invoiceSummaryEl.innerHTML = `<span class="pill">Jobs: ${jobs.length}</span><span class="pill">Units: ${units}</span><span class="pill">Area: ${formatArea(area)} m²</span><span class="pill">Total: ${formatMoney(amount)} ALL</span>`;
}

function applyInvoiceFilters(jobs){
  const filters = appState.invoices.filters || {};
  const clientQuery = (filters.client || "").trim().toLowerCase();
  const orderQuery = (filters.order || "").trim().toLowerCase();
  const statusFilter = (filters.status || "").trim().toLowerCase();
  return (jobs || []).filter(job => !job.hidden).filter(job=>{
    const clientLabel = formatInvoiceClientLabel(job).toLowerCase();
    const clientMatch = !clientQuery || clientLabel.includes(clientQuery);
    const orderLabel = (job.orderNumbers || []).join(", ").toLowerCase();
    const orderMatch = !orderQuery || orderLabel.includes(orderQuery);
    const statusMatch = !statusFilter || (job.status || "").toLowerCase() === statusFilter;
    return clientMatch && orderMatch && statusMatch;
  });
}

function findNextDraftInvoice(currentId){
  const filtered = applyInvoiceFilters(appState.invoices.jobs || []);
  if (!filtered.length) return null;
  const drafts = filtered.filter(job => (job.status || "").toLowerCase() !== "finalized");
  if (!drafts.length) return null;
  const startIndex = filtered.findIndex(job => job.id === currentId);
  if (startIndex >= 0){
    for (let i = startIndex + 1; i < filtered.length; i += 1){
      const job = filtered[i];
      if ((job.status || "").toLowerCase() !== "finalized") return job;
    }
    for (let i = 0; i <= startIndex; i += 1){
      const job = filtered[i];
      if ((job.status || "").toLowerCase() !== "finalized") return job;
    }
  }
  return drafts[0] || null;
}

function renderInvoiceJobs(){
  if (!invoiceJobsWrap) return;
  ensureInvoicesLoaded();
  const pagination = ensureInvoicePaginationState();
  const jobs = applyInvoiceFilters(appState.invoices.jobs || []);
  if (!jobs.length){
    appState.invoices.activeJobId = null;
    invoiceJobsWrap.innerHTML = '<div class="invoice-empty">No invoice jobs yet. Use the Invoice button in History.</div>';
    pagination.currentPage = 1;
    pagination.totalPages = 1;
    updateInvoicePaginationControls(0, 1, 1);
    return;
  }
  const filters = appState.invoices.filters || {};
  const clientQuery = (filters.client || "").trim().toLowerCase();
  const orderQuery = (filters.order || "").trim().toLowerCase();
  const rows = [];
  jobs.forEach(job=>{
    const calc = job.calculated;
    if (calc){
      applyJobTotals(job, calc);
    }
    ensureJobOrders(job);
    const statusClass = (job.status || "draft") === "finalized" ? "finalized" : "draft";
    const fallbackClient = formatInvoiceClientLabel(job) || "—";
    const fallbackDate = job.createdAt || new Date().toISOString();
    const orderEntries = Array.isArray(job.orders) && job.orders.length
      ? job.orders
      : [createOrderMetadata({
          orderNumber: job.orderNumbers && job.orderNumbers[0] ? job.orderNumbers[0] : "—",
          clientName: fallbackClient,
          createdAt: fallbackDate,
          units: job.totalUnits || 0,
          area: job.totalArea || 0,
          amount: job.totalAmount || 0,
        })];
    orderEntries.forEach(entry=>{
      const orderNumber = entry?.orderNumber || "—";
      const client = entry?.clientName || fallbackClient;
      const createdAt = entry?.createdAt || fallbackDate;
      const units = Number(entry?.units ?? 0) || 0;
      const areaValue = Number(entry?.area ?? 0) || 0;
      const amountValue = Number(entry?.amount ?? 0) || 0;
      const matchesClient = !clientQuery || client.toLowerCase().includes(clientQuery);
      const matchesOrder = !orderQuery || orderNumber.toLowerCase().includes(orderQuery);
      if (!matchesClient || !matchesOrder) return;
      const invoiceBadge = job.invoiceNumber
        ? `<div class="muted small">Invoice: ${escapeHtml(job.invoiceNumber)}</div>`
        : "";
      rows.push(`<tr data-job-id="${escapeHtml(job.id)}" data-order-number="${escapeHtml(orderNumber)}">
        <td>${escapeHtml(client)}</td>
        <td><div>${escapeHtml(orderNumber)}</div>${invoiceBadge}</td>
        <td>${escapeHtml(formatDate(createdAt))}</td>
        <td>${units}</td>
        <td>${formatArea(areaValue)}</td>
        <td>${formatMoney(amountValue)}</td>
        <td><span class="pill invoice-status-badge ${statusClass}">${escapeHtml(job.status || "draft")}</span></td>
        <td class="actions-cell">
          <button class="btn small" data-invoice-action="open" data-job-id="${escapeHtml(job.id)}" data-order-number="${escapeHtml(orderNumber)}">Open</button>
          <button class="btn small" data-invoice-action="pdf" data-job-id="${escapeHtml(job.id)}" data-order-number="${escapeHtml(orderNumber)}">Generate PDF</button>
          <button class="btn small danger" data-invoice-action="delete" data-job-id="${escapeHtml(job.id)}">Delete</button>
        </td>
      </tr>`);
    });
  });
  if (!rows.length){
    invoiceJobsWrap.innerHTML = '<div class="invoice-empty">No invoice rows match your filters.</div>';
    pagination.currentPage = 1;
    pagination.totalPages = 1;
    updateInvoicePaginationControls(0, 1, 1);
    return;
  }
  const pageSize = Math.max(1, Number(pagination.pageSize) || 5);
  pagination.pageSize = pageSize;
  const totalRows = rows.length;
  const totalPages = Math.max(1, Math.ceil(totalRows / pageSize));
  pagination.totalPages = totalPages;
  if (pagination.currentPage > totalPages){
    pagination.currentPage = totalPages;
  }
  if (pagination.currentPage < 1){
    pagination.currentPage = 1;
  }
  const startIndex = (pagination.currentPage - 1) * pageSize;
  const paginatedRows = rows.slice(startIndex, startIndex + pageSize);
  invoiceJobsWrap.innerHTML = `<table>
    <thead>
      <tr>
        <th>Client</th>
        <th>Order number</th>
        <th>Date</th>
        <th>Units</th>
        <th>Total area (m²)</th>
        <th>Total amount (ALL)</th>
        <th>Status</th>
        <th>Actions</th>
      </tr>
    </thead>
    <tbody>${paginatedRows.join("")}</tbody>
  </table>`;
  updateInvoicePaginationControls(totalRows, totalPages, pagination.currentPage);
  const hasActive = jobs.some(job => job.id === appState.invoices.activeJobId);
  if ((!appState.invoices.activeJobId || !hasActive) && jobs.length){
    appState.invoices.activeJobId = jobs[0].id;
  }
}

function updateInvoicePaginationControls(totalRows, totalPages, currentPage){
  if (!invoicePagination) return;
  const shouldHide = totalRows === 0;
  invoicePagination.hidden = shouldHide;
  if (invoicePaginationPrev){
    invoicePaginationPrev.disabled = currentPage <= 1 || shouldHide;
  }
  if (invoicePaginationNext){
    invoicePaginationNext.disabled = currentPage >= totalPages || shouldHide;
  }
}

function renderInvoiceDetail(){
  if (!invoiceLinesWrap) return;
  const job = getActiveInvoiceJob();
  if (!job){
    invoiceLinesWrap.innerHTML = '<div class="invoice-empty">Select an invoice job from the list.</div>';
    if (invoiceDetailMeta) invoiceDetailMeta.textContent = "";
    if (invoiceTotalsEl) invoiceTotalsEl.innerHTML = "";
    if (invoiceMissingNotice) invoiceMissingNotice.hidden = true;
    if (invoiceAddOrderBtn) invoiceAddOrderBtn.disabled = true;
    if (invoiceRecalcBtn) invoiceRecalcBtn.disabled = true;
    if (invoiceGeneratePdfBtn) invoiceGeneratePdfBtn.disabled = true;
    if (invoiceDeleteActiveBtn) invoiceDeleteActiveBtn.disabled = true;
    if (invoiceDiscountValue) invoiceDiscountValue.value = "";
    if (invoiceVatValue) invoiceVatValue.value = "";
    if (invoiceDiscountMode) invoiceDiscountMode.disabled = true;
    if (invoiceDiscountValue) invoiceDiscountValue.disabled = true;
    if (invoiceVatValue) invoiceVatValue.disabled = true;
    if (invoiceStatusSelect) invoiceStatusSelect.disabled = true;
    return;
  }
  const calc = job.calculated;
  if (invoiceDetailMeta){
    const orderNumbers = Array.isArray(job.orderNumbers) ? job.orderNumbers : [];
    const clientLabel = formatInvoiceClientLabel(job) || "—";
    const created = formatDate(job.createdAt);
    const invoiceNo = job.invoiceNumber ? ` • Invoice: ${escapeHtml(job.invoiceNumber)}` : "";
    const orderChips = orderNumbers.length
      ? orderNumbers.map(num => `<span class="order-chip"><span>${escapeHtml(num)}</span><button type="button" data-remove-order="${escapeHtml(num)}" aria-label="Remove order ${escapeHtml(num)}">×</button></span>`).join("")
      : '<span class="muted small">(none)</span>';
    invoiceDetailMeta.innerHTML = `
      <div>Client: ${escapeHtml(clientLabel)} • ${escapeHtml(created)}${invoiceNo}</div>
      <div class="order-chip-row">
        <span class="muted small">Orders:</span>
        <div class="order-chips">${orderChips}</div>
      </div>
    `;
  }
  if (invoiceDiscountMode){
    invoiceDiscountMode.value = job.discountMode || "percent";
    invoiceDiscountMode.disabled = false;
  }
  if (invoiceDiscountValue){
    invoiceDiscountValue.value = job.discountValue ?? 0;
    invoiceDiscountValue.disabled = false;
  }
  if (invoiceVatValue){
    invoiceVatValue.value = job.vatRate ?? 0;
    invoiceVatValue.disabled = false;
  }
  if (invoiceStatusSelect){
    invoiceStatusSelect.value = job.status || "draft";
    invoiceStatusSelect.disabled = false;
  }
  const missingMessage = describeMissingPricing(calc?.missing);
  if (invoiceMissingNotice){
    if (missingMessage){
      invoiceMissingNotice.hidden = false;
      invoiceMissingNotice.textContent = missingMessage + " Add a price to continue.";
    }else{
      invoiceMissingNotice.hidden = true;
    }
  }
  if (!calc){
    invoiceLinesWrap.innerHTML = '<div class="invoice-empty">Calculating invoice…</div>';
    if (invoiceTotalsEl) invoiceTotalsEl.innerHTML = "";
    if (invoiceAddOrderBtn) invoiceAddOrderBtn.disabled = false;
    if (invoiceRecalcBtn) invoiceRecalcBtn.disabled = false;
    if (invoiceGeneratePdfBtn) invoiceGeneratePdfBtn.disabled = true;
    if (invoiceDeleteActiveBtn) invoiceDeleteActiveBtn.disabled = false;
    return;
  }
  const lines = Array.isArray(calc?.lines) ? calc.lines : [];
  if (!lines.length){
    invoiceLinesWrap.innerHTML = '<div class="invoice-empty">No rows to show.</div>';
  }else{
    const body = lines.map(line=>{
      const missingBadge = line.missing ? '<span class="editable-badge">Missing price</span>' : "";
      const aiBadgeHtml = line.aiAssisted ? '<span class="ai-badge" title="AI-assisted match">AI</span>' : "";
      return `<tr>
        <td>${line.index}</td>
        <td data-col="type">
          <input type="text" data-type-edit="${line.index}" value="${escapeHtml(line.type || line.displayType || "")}" style="width:100%" />
          ${line.rawType ? `<div class="muted small" style="margin-top:4px">Original: ${escapeHtml(line.rawType)}</div>` : ""}
          ${missingBadge}${aiBadgeHtml}
        </td>
        <td>${Number(line.quantity || 0)}</td>
        <td>${formatArea(line.area || 0)}</td>
        <td class="mono" data-col="price">${formatMoney(line.glassPricePerM2)} </td>
        <td>
          <select data-spacer-kind="${line.index}" style="min-width:110px">
            <option value="normal" ${line.spacerKind === "thermal" ? "" : "selected"}>Normal</option>
            <option value="thermal" ${line.spacerKind === "thermal" ? "selected" : ""}>Thermal</option>
          </select>
        </td>
        <td class="mono" data-col="price">${formatMoney(line.spacerPricePerM2)}</td>
        <td class="mono" data-col="price">${formatMoney(line.unitPricePerM2)}</td>
        <td class="mono" data-col="price">${formatMoney(line.lineTotal)}</td>
      </tr>`;
    }).join("");
    invoiceLinesWrap.innerHTML = `<table class="invoice-lines-table"><thead>
      <tr>
        <th>#</th>
        <th>Type</th>
        <th>Qty</th>
        <th>Area (m²)</th>
        <th>Glass / m²</th>
        <th>Spacer type</th>
        <th>Spacer / m²</th>
        <th>Unit / m²</th>
        <th>Line total</th>
      </tr>
    </thead><tbody>${body}</tbody></table>`;
  }
  if (invoiceTotalsEl){
    invoiceTotalsEl.innerHTML = `
      <span class="pill">Subtotal: ${formatMoney(calc?.subtotal || 0)} ALL</span>
      <span class="pill">Discount: ${formatMoney(calc?.discountValue || 0)} ALL</span>
      <span class="pill">VAT: ${formatMoney(calc?.vatValue || 0)} ALL</span>
      <span class="pill">Total: ${formatMoney(calc?.total || 0)} ALL</span>
    `;
  }
  if (invoiceAddOrderBtn) invoiceAddOrderBtn.disabled = false;
  if (invoiceRecalcBtn) invoiceRecalcBtn.disabled = false;
  if (invoiceGeneratePdfBtn) invoiceGeneratePdfBtn.disabled = !!missingMessage;
  if (invoiceDeleteActiveBtn) invoiceDeleteActiveBtn.disabled = false;
}

function renderInvoicePriceEditor(){
  if (!invoiceGlassPricesWrap || !invoiceSpacerPricesWrap) return;
  const glassEntries = Object.entries(appState.invoices.priceLists.glass || {}).sort((a,b)=> a[0].localeCompare(b[0]));
  const spacerEntries = Object.entries(appState.invoices.priceLists.spacer || {}).sort((a,b)=> Number(a[0]) - Number(b[0]));
  const glassExtras = Array.from({ length: Math.max(1, invoiceRuntime.extraGlassRows) }).map(()=> ["", ""]);
  const spacerExtras = Array.from({ length: Math.max(1, invoiceRuntime.extraSpacerRows) }).map(()=> ["", { normal: "", thermal: "" }]);
  const glassRows = [...glassEntries, ...glassExtras].map(([key, price])=> `<tr data-glass-row>
    <td><input type="text" data-glass-key value="${escapeHtml(key)}" /></td>
    <td><input type="number" step="0.01" min="0" data-glass-price value="${price === "" ? "" : (price ?? "")}" /></td>
  </tr>`).join("");
  invoiceGlassPricesWrap.innerHTML = `<table><thead><tr><th>Glass type</th><th>Price (ALL/m²)</th></tr></thead><tbody>${glassRows}</tbody></table>`;

  const spacerRows = [...spacerEntries, ...spacerExtras].map(([thickness, entry])=>{
    const normalVal = entry?.normal ?? "";
    const thermalVal = entry?.thermal ?? "";
    return `<tr data-spacer-row>
      <td><input type="number" min="0" step="1" data-spacer-thickness value="${thickness || ""}" /></td>
      <td><input type="number" step="0.01" min="0" data-spacer-normal value="${normalVal === "" ? "" : normalVal}" /></td>
      <td><input type="number" step="0.01" min="0" data-spacer-thermal value="${thermalVal === "" ? "" : thermalVal}" /></td>
    </tr>`;
  }).join("");
  invoiceSpacerPricesWrap.innerHTML = `<table><thead><tr><th>Thickness (mm)</th><th>Normal (ALL/m²)</th><th>Thermal (ALL/m²)</th></tr></thead><tbody>${spacerRows}</tbody></table>`;
}

function renderTypeCorrectionsTable(){
  if (!typeCorrectionsTable) return;
  if (!Array.isArray(TYPE_CORRECTIONS) || !TYPE_CORRECTIONS.length){
    typeCorrectionsTable.innerHTML = '<div class="invoice-empty">No corrections yet.</div>';
    return;
  }
  const rows = TYPE_CORRECTIONS.map((entry, idx)=> `<tr data-correction-row>
    <td><input type="text" value="${escapeHtml(entry.raw || "")}" readonly style="width:100%" /></td>
    <td><input type="text" value="${escapeHtml(entry.corrected || "")}" readonly style="width:100%" /></td>
    <td class="actions-cell" style="text-align:right"><button class="btn small danger" data-delete-correction="${idx}">Delete</button></td>
  </tr>`).join("");
  typeCorrectionsTable.innerHTML = `<table><thead><tr><th>Raw type</th><th>Corrected type</th><th>Actions</th></tr></thead><tbody>${rows}</tbody></table>`;
}

function openTypeCorrectionsModal(){
  if (typeCorrectionsModal){
    typeCorrectionsModal.hidden = false;
  }
  if (typeCorrectionsRawInput){
    typeCorrectionsRawInput.value = "";
    typeCorrectionsRawInput.classList.remove("input-error");
    typeCorrectionsRawInput.focus();
  }
  if (typeCorrectionsCorrectedInput){
    typeCorrectionsCorrectedInput.value = "";
    typeCorrectionsCorrectedInput.classList.remove("input-error");
  }
}

function closeTypeCorrectionsModal(){
  if (typeCorrectionsModal){
    typeCorrectionsModal.hidden = true;
  }
}

async function handleTypeCorrectionsSave(){
  const rawValue = (typeCorrectionsRawInput?.value || "").trim();
  const correctedValue = (typeCorrectionsCorrectedInput?.value || "").trim();
  const markInvalid = (input)=>{
    if (!input) return;
    input.classList.add("input-error");
    setTimeout(()=> input.classList.remove("input-error"), 500);
  };
  let hasError = false;
  if (!rawValue){
    markInvalid(typeCorrectionsRawInput);
    hasError = true;
  }
  if (!correctedValue){
    markInvalid(typeCorrectionsCorrectedInput);
    hasError = true;
  }
  if (hasError) return;
  TYPE_CORRECTIONS.push({ raw: rawValue, corrected: correctedValue });
  closeTypeCorrectionsModal();
  const payload = buildInvoiceConfigPayload();
  await applyInvoiceConfigToState(payload, { cacheLocal: true, preferExistingTypeCorrections: true });
  const saved = await persistInvoiceConfigToServer(payload, { preferExistingTypeCorrections: true });
  updateInvoicesUI();
  setInvoiceStatus(saved ? "Type correction saved to server." : "Type correction saved locally.");
}

function updateInvoicesUI(){
  if (!invoiceRuntime.initialized) return;
  if (invoiceFilterClient && invoiceFilterClient.value !== (appState.invoices.filters.client || "")){
    invoiceFilterClient.value = appState.invoices.filters.client || "";
  }
  if (invoiceFilterOrder && invoiceFilterOrder.value !== (appState.invoices.filters.order || "")){
    invoiceFilterOrder.value = appState.invoices.filters.order || "";
  }
  if (invoiceFilterStatus && invoiceFilterStatus.value !== (appState.invoices.filters.status || "")){
    invoiceFilterStatus.value = appState.invoices.filters.status || "";
  }
  renderInvoiceJobs();
  renderInvoiceDetail();
  renderInvoicePriceEditor();
  renderTypeCorrectionsTable();
  updateInvoiceSummary();
}

function collectGlassPricesFromEditor(){
  if (!invoiceGlassPricesWrap) return {};
  const rows = Array.from(invoiceGlassPricesWrap.querySelectorAll("[data-glass-row]"));
  const map = {};
  rows.forEach(row=>{
    const keyInput = row.querySelector("[data-glass-key]");
    const priceInput = row.querySelector("[data-glass-price]");
    const rawKey = keyInput ? keyInput.value : "";
    const normalized = normalizeGlassKey(rawKey);
    const price = priceInput ? Number(priceInput.value) : NaN;
    if (normalized && Number.isFinite(price)){
      map[normalized] = price;
    }
  });
  return map;
}

function collectSpacerPricesFromEditor(){
  if (!invoiceSpacerPricesWrap) return {};
  const rows = Array.from(invoiceSpacerPricesWrap.querySelectorAll("[data-spacer-row]"));
  const map = {};
  rows.forEach(row=>{
    const thicknessInput = row.querySelector("[data-spacer-thickness]");
    const normalInput = row.querySelector("[data-spacer-normal]");
    const thermalInput = row.querySelector("[data-spacer-thermal]");
    const thickness = thicknessInput ? Number(thicknessInput.value) : NaN;
    const normal = normalInput ? Number(normalInput.value) : NaN;
    const thermal = thermalInput ? Number(thermalInput.value) : NaN;
    if (!Number.isFinite(thickness)) return;
    if (!Number.isFinite(normal) && !Number.isFinite(thermal)) return;
    map[thickness] = {
      normal: Number.isFinite(normal) ? normal : undefined,
      thermal: Number.isFinite(thermal) ? thermal : undefined,
    };
  });
  return map;
}

function normalizeGlassAliasToken(value){
  const original = String(value || "");
  const text = original.toLowerCase().replace(/\+/g, " ").replace(/\bmm\b/g, " ");
  const fixed = fixGlassTypos(text, original);
  return fixed.replace(/\s+/g, " ").trim();
}

function getGlassAliasCacheKey(value){
  const normalized = normalizeGlassAliasToken(value);
  return normalized.replace(/\s+/g, "");
}

function findLocalGlassMatch(targetCompact, candidates){
  if (!targetCompact) return null;
  const exact = candidates.find(entry => entry.compact === targetCompact);
  if (exact) return exact.key;
  const startsWith = candidates.find(entry => targetCompact.startsWith(entry.compact) || entry.compact.startsWith(targetCompact));
  if (startsWith) return startsWith.key;
  const contains = candidates.find(entry => targetCompact.includes(entry.compact) || entry.compact.includes(targetCompact));
  if (contains) return contains.key;
  return null;
}

async function resolveGlassType(rawName, knownTypes = []){
  if (!rawName || !Array.isArray(knownTypes) || !knownTypes.length){
    return { match: null, fromAi: false };
  }
  const cacheKey = getGlassAliasCacheKey(rawName);
  if (!cacheKey){
    return { match: null, fromAi: false };
  }
  if (Object.prototype.hasOwnProperty.call(glassAliasCache, cacheKey)){
    return glassAliasCache[cacheKey];
  }
  const candidates = knownTypes.map(type => ({
    key: type,
    normalized: normalizeGlassAliasToken(type),
    compact: getGlassAliasCacheKey(type),
  }));
  const localMatch = findLocalGlassMatch(cacheKey, candidates);
  if (localMatch){
    const result = { match: localMatch, fromAi: false };
    glassAliasCache[cacheKey] = result;
    return result;
  }
  if (glassAliasPending.has(cacheKey)){
    return glassAliasPending.get(cacheKey);
  }
  const pending = (async ()=>{
    try{
      console.log("[GlassAI] Calling AI glass matcher for:", rawName);
      const aiMatch = await callAiGlassMatch(rawName, knownTypes);
      console.log("[GlassAI] Result:", aiMatch);
      if (aiMatch){
        const resolved = knownTypes.find(type => type.toLowerCase() === aiMatch.toLowerCase()) || aiMatch;
        const result = { match: resolved, fromAi: true };
        glassAliasCache[cacheKey] = result;
        return result;
      }
    }catch(error){
      console.warn("AI glass resolver failed", error);
    }
    const fallback = { match: null, fromAi: false };
    glassAliasCache[cacheKey] = fallback;
    return fallback;
  })().finally(()=>{
    glassAliasPending.delete(cacheKey);
  });
  glassAliasPending.set(cacheKey, pending);
  return pending;
}

async function callAiGlassMatch(rawName, knownTypes){
  if (!rawName || !Array.isArray(knownTypes) || !knownTypes.length){
    return null;
  }
  const res = await fetch(API_BASE + "/api/invoices/ai/glass-match", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      raw_name: String(rawName),
      known_types: knownTypes,
    }),
  });
  if (!res.ok){
    const text = await res.text().catch(()=> "");
    throw new Error(text || `AI resolver responded with ${res.status}`);
  }
  const data = await res.json();
  const candidate = typeof data?.match === "string" ? data.match.trim() : "";
  if (!candidate){
    return null;
  }
  const match = knownTypes.find(type => type.toLowerCase() === candidate.toLowerCase());
  return match || null;
}

async function analyzeInvoiceLineWithAI(rawLine, knownGlassTypes){
  if (!rawLine || !Array.isArray(knownGlassTypes)){
    return null;
  }
  try{
    const res = await fetch(API_BASE + "/api/invoices/ai/analyze-line", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        raw_line: String(rawLine),
        known_glass_types: knownGlassTypes,
      }),
    });
    if (!res.ok){
      throw new Error(`AI analysis failed with ${res.status}`);
    }
    const data = await res.json();
    const parsed = data?.analysis;
    if (typeof parsed !== "object" || !parsed){
      return null;
    }
    const normalizedType = typeof parsed.normalizedType === "string" ? parsed.normalizedType.trim() : "";
    if (!normalizedType){
      return null;
    }
    let glassKey = null;
    if (parsed.glassKey != null){
      const candidate = String(parsed.glassKey).trim();
      if (!candidate){
        glassKey = null;
      }else{
        const match = knownGlassTypes.find(entry => entry.toLowerCase() === candidate.toLowerCase());
        if (!match){
          console.warn("AI analysis returned unknown glass key", candidate);
          return null;
        }
        glassKey = match;
      }
    }
    const spacerModeRaw = typeof parsed.spacerMode === "string" ? parsed.spacerMode.toLowerCase() : "";
    const spacerMode = spacerModeRaw === "thermal" ? "thermal" : spacerModeRaw === "normal" ? "normal" : null;
    if (!spacerMode){
      return null;
    }
    const isLaminated = Boolean(parsed.isLaminated);
    const confidence = Number(parsed.confidence);
    if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1){
      return null;
    }
    const reason = typeof parsed.reason === "string" ? parsed.reason.trim() : "";
    return {
      normalizedType,
      glassKey,
      spacerMode,
      isLaminated,
      confidence,
      reason,
    };
  }catch(error){
    console.warn("AI invoice line analysis failed", error);
    return null;
  }
}

// Invoice pricing/type corrections now save to the server (not just localStorage)
async function savePricesFromEditor(){
  const glass = collectGlassPricesFromEditor();
  const spacer = collectSpacerPricesFromEditor();
  appState.invoices.priceLists.glass = Object.keys(glass).length ? glass : getDefaultGlassPrices();
  const merged = { ...getDefaultSpacerPrices() };
  Object.entries(spacer).forEach(([thick, entry])=>{
    const t = Number(thick);
    if (!Number.isFinite(t)) return;
    merged[t] = {
      normal: Number.isFinite(entry.normal) ? entry.normal : merged[t]?.normal,
      thermal: Number.isFinite(entry.thermal) ? entry.thermal : merged[t]?.thermal,
    };
  });
  appState.invoices.priceLists.spacer = merged;
  invoiceRuntime.extraGlassRows = 1;
  invoiceRuntime.extraSpacerRows = 1;
  const payload = buildInvoiceConfigPayload();
  await applyInvoiceConfigToState(payload, { cacheLocal: true, preferExistingTypeCorrections: true });
  const saved = await persistInvoiceConfigToServer(payload, { preferExistingTypeCorrections: true });
  if (saved){
    setInvoiceStatus("Price list saved to server and invoices refreshed.");
  }else{
    setInvoiceStatus("Price list saved locally. Will sync when online.");
  }
}

async function deleteInvoiceJob(id){
  await ensureInvoiceJobsLoaded();
  const idx = appState.invoices.jobs.findIndex(job => job.id === id);
  if (idx < 0) return;
  appState.invoices.jobs.splice(idx, 1);
  if (appState.invoices.activeJobId === id){
    appState.invoices.activeJobId = null;
  }
  await saveInvoiceJobs();
  updateInvoicesUI();
  setInvoiceStatus("Invoice job deleted.");
}

function ensureInvoicePaginationState(){
  if (!appState.invoices.pagination){
    appState.invoices.pagination = { currentPage: 1, pageSize: 5, totalPages: 1 };
  }
  const pagination = appState.invoices.pagination;
  if (!Number.isFinite(pagination.pageSize) || pagination.pageSize <= 0){
    pagination.pageSize = 5;
  }
  if (!Number.isFinite(pagination.currentPage) || pagination.currentPage < 1){
    pagination.currentPage = 1;
  }
  if (!Number.isFinite(pagination.totalPages) || pagination.totalPages < 1){
    pagination.totalPages = 1;
  }
  return pagination;
}

function setInvoiceFilters(filters){
  ensureInvoicePaginationState();
  appState.invoices.filters = { ...appState.invoices.filters, ...filters };
  appState.invoices.pagination.currentPage = 1;
  updateInvoicesUI();
}

function goToInvoicePage(pageNumber){
  const pagination = ensureInvoicePaginationState();
  const totalPages = Math.max(1, pagination.totalPages || 1);
  const target = Math.min(Math.max(1, pageNumber), totalPages);
  if (target === pagination.currentPage) return;
  pagination.currentPage = target;
  updateInvoicesUI();
}

function changeInvoicePage(delta){
  const pagination = ensureInvoicePaginationState();
  goToInvoicePage(pagination.currentPage + delta);
}

async function generateInvoicePdf(job){
  if (!job) throw new Error("No invoice selected");
  await ensureInvoiceJobsLoaded();
  const calc = job.calculated || await recalcInvoiceJob(job, { allowPrompt: false });
  if (!calc){
    throw new Error("Unable to calculate invoice totals.");
  }
  if (calc?.missing){
    throw new Error("Cannot generate PDF: missing prices");
  }
  const groupedLines = Array.isArray(calc?.lines) ? calc.lines : [];
  await ensurePdfLib();
  const lib = window.PDFLib;
  if (!lib || !lib.PDFDocument){
    throw new Error("PDF library not loaded. Please check your connection and try again.");
  }
  const { PDFDocument, StandardFonts, rgb } = lib;
  const labelColor = typeof rgb === "function" ? rgb(0.5, 0.5, 0.55) : undefined;
  const mmToPtLocal = 2.83464567;
  const pageWidth = 210 * mmToPtLocal;
  const pageHeight = 297 * mmToPtLocal;
  const margin = 28;
  const lineHeight = 16;
  const tableFontSize = 10;
  const doc = await PDFDocument.create();
  const bold = await doc.embedFont(StandardFonts.HelveticaBold);
  const regular = await doc.embedFont(StandardFonts.Helvetica);
  let page = doc.addPage([pageWidth, pageHeight]);
  let y = pageHeight - margin;

  const invoiceNumber = job.invoiceNumber || nextInvoiceNumber();
  job.invoiceNumber = invoiceNumber;
  const orderLabel = (job.orderNumbers || []).join(", ") || "—";
  const headerDate = formatDate(new Date().toISOString());
  const clientNames = (() => {
    const names = new Set();
    const addName = (value)=>{
      const text = (value || "").trim();
      if (text) names.add(text);
    };
    addName(job.client);
    if (Array.isArray(job.clients)){
      job.clients.forEach(addName);
    }
    const sourceRows = Array.isArray(job.rawRows) ? job.rawRows : job.rows;
    if (Array.isArray(sourceRows)){
      sourceRows.forEach(row=>{
        addName(row?.client);
        addName(row?.client_name);
        addName(row?.clientName);
        addName(row?.client_label);
        addName(row?.customer);
      });
    }
    const list = Array.from(names);
    if (!list.length) return ["—"];
    return list;
  })();
  const clientLabelText = clientNames.length > 1
    ? `Clients: ${clientNames.join(", ")}`
    : `Client: ${clientNames[0]}`;

  const infoBlockX = margin;
  let infoY = pageHeight - margin;
  page.drawText("FATURA / INVOICE", { x: infoBlockX, y: infoY, size: 12, font: bold });
  infoY -= lineHeight;
  const metaLines = [
    clientLabelText,
    `Orders: ${orderLabel}`,
    `Invoice: ${invoiceNumber}`,
    `Date: ${headerDate}`,
  ];
  metaLines.forEach(line=>{
    page.drawText(line, { x: infoBlockX, y: infoY, size: 11, font: regular });
    infoY -= lineHeight;
  });

  y = infoY - 6;
  page.drawLine({ start: { x: margin, y }, end: { x: pageWidth - margin, y }, thickness: 1 });
  y -= lineHeight;

  const columns = [
    { key: "idx", title: "#", width: 20, align: "left" },
    { key: "type", title: "Type", width: 190, align: "left" },
    { key: "qty", title: "Qty", width: 50, align: "right" },
    { key: "area", title: "Area (m²)", width: 70, align: "right" },
    { key: "unit", title: "Unit (ALL/m²)", width: 90, align: "right" },
    { key: "total", title: "Total (ALL)", width: 90, align: "right" },
  ];
  const colPositions = {};
  let cursorX = margin;
  columns.forEach(col=>{
    colPositions[col.key] = { x: cursorX, width: col.width };
    cursorX += col.width;
  });

  const drawHeaders = ()=>{
    columns.forEach(col=>{
      page.drawText(col.title, { x: colPositions[col.key].x, y, size: tableFontSize, font: bold });
    });
    y -= lineHeight * 0.85;
    page.drawLine({ start: { x: margin, y }, end: { x: pageWidth - margin, y }, thickness: 0.5 });
    y -= lineHeight;
  };
  drawHeaders();

  const ensureSpace = (rowsNeeded)=>{
    if (y - rowsNeeded * lineHeight < margin){
      page = doc.addPage([pageWidth, pageHeight]);
      y = pageHeight - margin;
      drawHeaders();
    }
  };

  const drawOrderHeader = (orderNumber)=>{
    if (!orderNumber) return;
    const headerText = `Order ${orderNumber}`;
    const headerSize = Math.max(8, tableFontSize - 1);
    const headerX = colPositions.type.x;
    const headerOpts = { x: headerX, y, size: headerSize, font: regular };
    if (labelColor) headerOpts.color = labelColor;
    page.drawText(headerText, headerOpts);
    y -= lineHeight;
  };

  let lastOrderLabel = null;

  groupedLines.forEach(line=>{
    const orderLabel = line.orderId ? String(line.orderId).trim() : "";
    const normalizedOrder = orderLabel.toLowerCase();
    const isNewOrder = orderLabel && normalizedOrder !== lastOrderLabel;
    const rowsNeeded = 1 + (isNewOrder ? 1 : 0);
    ensureSpace(rowsNeeded);
    if (isNewOrder){
      drawOrderHeader(orderLabel);
    }
    const rowValues = {
      idx: String(line.index || ""),
      type: line.type || "",
      qty: String(line.quantity || 0),
      area: formatArea(line.area || 0),
      unit: formatMoney(line.unitPricePerM2 || 0),
      total: formatMoney(line.lineTotal || 0),
    };
    columns.forEach(col=>{
      const pos = colPositions[col.key];
      const text = rowValues[col.key] || "";
      let x = pos.x;
      if (col.align === "center"){
        const width = regular.widthOfTextAtSize(text, tableFontSize);
        x = pos.x + (pos.width - width) / 2;
      }else if (col.align === "right"){
        const width = regular.widthOfTextAtSize(text, tableFontSize);
        x = pos.x + pos.width - width;
      }
      page.drawText(text, { x, y, size: tableFontSize, font: regular });
    });
    lastOrderLabel = orderLabel ? normalizedOrder : lastOrderLabel;
    y -= lineHeight;
  });

  y -= lineHeight;
  page.drawLine({ start: { x: margin, y }, end: { x: pageWidth - margin, y }, thickness: 0.5 });
  y -= lineHeight;
  const summaryItems = [
    ["Subtotal", formatMoney(calc?.subtotal || 0)],
    ["Discount", formatMoney(calc?.discountValue || 0)],
    [`VAT ${job.vatRate || 0}%`, formatMoney(calc?.vatValue || 0)],
    ["Total", formatMoney(calc?.total || 0)],
  ];
  const totalsStartX = colPositions.total.x - 20;
  y -= 6;
  summaryItems.forEach((item, idx)=>{
    ensureSpace(1);
    const isTotal = idx === summaryItems.length - 1;
    const size = isTotal ? 12 : 11;
    const font = isTotal ? bold : bold;
    const labelWidth = font.widthOfTextAtSize(item[0], size);
    const valueWidth = font.widthOfTextAtSize(item[1], size);
    const labelX = totalsStartX;
    const valueX = colPositions.total.x + colPositions.total.width - valueWidth;
    page.drawText(item[0], { x: labelX, y, size, font });
    page.drawText(item[1], { x: valueX, y, size, font });
    y -= lineHeight;
  });

  await saveInvoiceJobs();
  const pdfBytes = await doc.save();
  const blob = new Blob([pdfBytes], { type: "application/pdf" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  const safeOrder = orderLabel.replace(/\s+/g, "-") || "invoice";
  a.href = url;
  a.download = `${invoiceNumber || safeOrder}.pdf`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
}


function updateExtractSummary(){
  const rows = appState.extract.rows || [];
  const totals = computeTotals(rows);
  const orderNumbers = collectOrderNumbers(rows);
  const summaryEl = document.getElementById("summary");
  const orderInfoEl = document.getElementById("orderInfo");
  if (summaryEl){
    if (!rows.length){
      summaryEl.textContent = "";
    }else{
      summaryEl.innerHTML = `Totals — <span class="pill">Units: ${totals.units}</span> <span class="pill">Area: ${formatArea(totals.area)} m²</span>`;
    }
  }
  if (orderInfoEl){
    const parts = [];
    if (appState.extract.draftId){
      parts.push(`Draft #${appState.extract.draftId}`);
    }
    if (!appState.extract.draftId && appState.extract.savedOrderId){
      parts.push(`Saved #${appState.extract.savedOrderId}`);
    }
    if (!orderNumbers.length){
      parts.push("Order not detected");
    }else if (orderNumbers.length === 1){
      parts.push(`Order: ${orderNumbers[0]}`);
    }else{
      parts.push(`Orders: ${orderNumbers.join(", ")}`);
    }
    orderInfoEl.textContent = parts.join(" • ");
  }
}

function updateHistorySummary(){
  const rows = appState.historyDetail.rows || [];
  const totals = computeTotals(rows);
  const orderNumbers = collectOrderNumbers(rows);
  const summaryEl = document.getElementById("historySummary");
  const orderInfoEl = document.getElementById("historyOrderInfo");
  if (summaryEl){
    if (!rows.length){
      summaryEl.textContent = "";
    }else{
      summaryEl.innerHTML = `Totals — <span class="pill">Units: ${totals.units}</span> <span class="pill">Area: ${formatArea(totals.area)} m²</span>`;
    }
  }
  if (orderInfoEl){
    if (!orderNumbers.length){
      orderInfoEl.textContent = "Order not detected";
    }else if (orderNumbers.length === 1){
      orderInfoEl.textContent = `Order: ${orderNumbers[0]}`;
    }else{
      orderInfoEl.textContent = `Orders: ${orderNumbers.join(", ")}`;
    }
  }
}

function updateDeclaredBanner(elementId, declaredUnits, declaredArea, parsedUnits, parsedArea, orderTotalDiagnostics = null){
  const el = document.getElementById(elementId);
  if (!el) return;
  if (orderTotalDiagnostics && (orderTotalDiagnostics.severity === "warning" || orderTotalDiagnostics.severity === "error")){
    el.hidden = false;
    el.innerHTML = `<strong style="display:block;margin-bottom:4px">PDF totals mismatch</strong><div>${escapeHtml(orderTotalDiagnostics.message || "Extracted totals do not match PDF totals.")}</div>`;
    return;
  }
  const issues = [];
  if (declaredUnits != null && parsedUnits != null && declaredUnits !== parsedUnits){
    issues.push(`Declared ${declaredUnits}, Parsed ${parsedUnits}`);
  }
  if (
    declaredArea != null && parsedArea != null &&
    Math.abs(parseFloat(declaredArea) - parseFloat(parsedArea)) > 0.05
  ){
    const delta = (parseFloat(parsedArea) - parseFloat(declaredArea)).toFixed(3);
    issues.push(`Declared ${Number(declaredArea).toFixed(3)} m², Parsed ${Number(parsedArea).toFixed(3)} m² → Δ ${delta} m²`);
  }
  if (!issues.length){
    el.hidden = true;
    el.innerHTML = "";
    return;
  }
  el.hidden = false;
  el.innerHTML = `<strong style="display:block;margin-bottom:4px">Declared totals mismatch</strong><div>${issues.map(escapeHtml).join('<br>')}</div>`;
}

function isExtractLowConfidenceEnabled(){
  const toggle = document.getElementById("lowConfidenceOnly");
  return !!(toggle && toggle.checked);
}

function isLowConfidenceRow(row, rowWarnings){
  if (!row) return false;
  return hasActionableDiagnostics(row);
}

function getExtractRowsForView(){
  const rows = appState.extract.rows || [];
  if (!isExtractLowConfidenceEnabled()) return rows;
  const warningMap = appState.extract.rowWarnings || {};
  return rows.filter(row => isLowConfidenceRow(row, warningMap[row._rid]));
}

function reviewIssueMessage(value, fallback = "Review this extracted value."){
  if (value && typeof value === "object"){
    return String(value.message || value.summary || value.reason || value.code || fallback);
  }
  const text = String(value || "").trim();
  return text || fallback;
}

function collectExtractReviewIssues(){
  const issues = [];
  const rows = appState.extract.rows || [];
  rows.forEach((row, rowIndex) => {
    missingCriticalFields(row).forEach(field => {
      issues.push({
        rowIndex,
        field,
        severity: "error",
        label: `Row ${rowIndex + 1} · Missing ${criticalFieldLabel(field)}`,
        message: `Enter a valid ${criticalFieldLabel(field)} before approval.`,
      });
    });
    const rowWarnings = appState.extract.rowWarnings?.[row._rid] || [];
    (Array.isArray(rowWarnings) ? rowWarnings : [rowWarnings]).filter(Boolean).forEach(warning => {
      issues.push({
        rowIndex,
        field: chooseFallbackTargetField(row),
        severity: "warning",
        label: `Row ${rowIndex + 1} · Extraction warning`,
        message: reviewIssueMessage(warning),
      });
    });
    getDiagnosticIssues(row).forEach(issue => {
      issues.push({
        rowIndex,
        field: chooseFallbackTargetField({ ...row, diagnostics: { ...getRowDiagnostics(row), issues: [issue] } }),
        severity: String(issue?.severity || "warning").toLowerCase(),
        label: `Row ${rowIndex + 1} · ${String(issue?.code || "Validation").replaceAll("_", " ")}`,
        message: reviewIssueMessage(issue),
      });
    });
  });
  (appState.extract.warnings || []).forEach(warning => {
    issues.push({
      rowIndex: null,
      field: "",
      severity: "warning",
      label: "Order warning",
      message: reviewIssueMessage(warning),
    });
  });
  return issues;
}

function syncExtractReviewIssue(options = {}){
  const issue = currentExtractReviewIssues[activeReviewIssueIndex];
  document.querySelectorAll("#tableWrap tr.review-row-active").forEach(row => row.classList.remove("review-row-active"));
  if (!issue || issue.rowIndex == null){
    if (extractSourceHighlight && !issue) extractSourceHighlight.hidden = true;
    return;
  }
  const rowElement = document.querySelector(`#tableWrap tr[data-index="${issue.rowIndex}"]`);
  if (rowElement){
    rowElement.classList.add("review-row-active");
    if (options.scroll !== false) rowElement.scrollIntoView({ behavior: "smooth", block: "center" });
    if (options.focus){
      rowElement.querySelector(`[data-field="${issue.field}"]`)?.focus({ preventScroll: true });
    }
  }
  showSourceLocation(
    extractPdfPreview,
    extractSourceHighlight,
    appState.extract.rows[issue.rowIndex],
    `Row ${issue.rowIndex + 1}`,
  );
}

function setActiveExtractReviewIssue(index, options = {}){
  if (!currentExtractReviewIssues.length){
    activeReviewIssueIndex = -1;
    syncExtractReviewIssue();
    return;
  }
  const count = currentExtractReviewIssues.length;
  activeReviewIssueIndex = ((Number(index) || 0) % count + count) % count;
  document.querySelectorAll("[data-review-issue-index]").forEach(item => {
    const active = Number(item.dataset.reviewIssueIndex) === activeReviewIssueIndex;
    item.classList.toggle("active", active);
    item.setAttribute("aria-current", active ? "true" : "false");
  });
  syncExtractReviewIssue(options);
}

function renderExtractReviewIssues(){
  currentExtractReviewIssues = collectExtractReviewIssues();
  if (activeReviewIssueIndex >= currentExtractReviewIssues.length) activeReviewIssueIndex = currentExtractReviewIssues.length - 1;
  if (extractIssueCount) extractIssueCount.textContent = String(currentExtractReviewIssues.length);
  if (extractPrevIssueBtn) extractPrevIssueBtn.disabled = currentExtractReviewIssues.length < 2;
  if (extractNextIssueBtn) extractNextIssueBtn.disabled = currentExtractReviewIssues.length < 2;
  if (extractReviewIssuesEl){
    extractReviewIssuesEl.innerHTML = currentExtractReviewIssues.length
      ? currentExtractReviewIssues.map((issue, index) => `
        <button type="button" class="review-issue ${issue.severity === "error" ? "error" : ""}${index === activeReviewIssueIndex ? " active" : ""}" data-review-issue-index="${index}">
          <span class="review-issue-marker" aria-hidden="true">${issue.severity === "error" ? "!" : "•"}</span>
          <span><strong>${escapeHtml(issue.label)}</strong><small>${escapeHtml(issue.message)}</small></span>
        </button>
      `).join("")
      : `<div class="review-issue-empty ${appState.extract.rows.length ? "success" : ""}">${
          appState.extract.rows.length ? "No extraction issues detected." : "Issues found during extraction will appear here."
        }</div>`;
  }
  if (activeReviewIssueIndex >= 0) syncExtractReviewIssue({ scroll: false });
}

function updateExtractUI(){
  withPreservedFocus(() => {
    const groupToggleEl = document.getElementById("groupToggle");
    if (groupToggleEl){
      groupToggleEl.disabled = false;
      if (groupToggleEl.checked !== groupState){
        groupToggleEl.checked = groupState;
      }
    }

    recalcParsedTotals(appState.extract);
    const rowsForView = getExtractRowsForView();

    if (groupState){
      render(rowsForView, { scope: "extract" });
    }else{
      renderEditableTable("extract", "tableWrap", rowsForView, appState.extract.rowWarnings || {});
    }

    renderWarningList("extractWarnings", appState.extract.warnings);
    updateTotalsUI("extract");

    const approveBtn = document.getElementById("approveSave");
    if (approveBtn) approveBtn.disabled = !appState.extract.draftId || !(appState.extract.rows && appState.extract.rows.length);
    const rerunBtn = document.getElementById("rerun");
    if (rerunBtn) rerunBtn.disabled = !appState.extract.draftId;
    const ocrRecheckBtn = document.getElementById("ocrRecheckMissing");
    if (ocrRecheckBtn){
      const hasMissingFields = (appState.extract.rows || []).some(row => missingCriticalFields(row).some(field => !rowHasOcrRepair(row, field)));
      const hasPdfContext = appState.extract.extractionMethod === "base64_pdf_visual" || appState.extract.extractionMethod === "legacy_ocr" || appState.extract.extractionMethod === "forced_ocr";
      ocrRecheckBtn.disabled = !appState.extract.draftId || !hasPdfContext || !hasMissingFields;
    }
    const forceOcrBtn = document.getElementById("forceOcrExtraction");
    if (forceOcrBtn) forceOcrBtn.disabled = !lastExtractPdfFile;
    const notesEl = document.getElementById("extractNotes");
    if (notesEl && notesEl.value !== appState.extract.notes){
      notesEl.value = appState.extract.notes || "";
    }
  });
  window.__rows = appState.extract.rows;
  renderExtractReviewIssues();
  updateLabelsUI();
}

function confidenceLabel(confidence){
  if (!Number.isFinite(confidence)) return { text: "—", level: "medium" };
  if (confidence >= 0.85) return { text: "High", level: "high" };
  if (confidence >= 0.55) return { text: "Medium", level: "medium" };
  return { text: "Low", level: "low" };
}

function buildSummaryHtml(rows, totalsParam, options = {}){
  const scope = options.scope || "extract";
  const totals = totalsParam || computeTotals(rows);
  const groups = summarizeByOrder(rows);
  let html = `Totals — <span class="pill">Units: ${totals.units}</span> <span class="pill">Area: ${formatArea(totals.area)} m²</span>`;
  if (scope === "extract"){
    const confidence = Number(appState.extract.confidence);
    const badge = confidenceLabel(confidence);
    html += ` <span>Confidence:</span> <span class="pill confidence ${badge.level}">${badge.text}</span>`;
  }
  if (groups.size > 1){
    const orderList = [...groups.keys()].filter(Boolean);
    if (orderList.length){
      html += ` <span class="muted small">Multiple orders: ${orderList.join(", ")}</span>`;
    }
  }
  return html;
}

function updateOrderInfo(scope){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const rows = scope === "extract" ? getExtractRowsForView() : (bucket.rows || []);
  const orderNumbers = collectOrderNumbers(rows);
  const targetId = scope === "history" ? "historyOrderInfo" : "orderInfo";
  const el = document.getElementById(targetId);
  if (!el) return;
  const pieces = [];
  if (!orderNumbers.length){
    pieces.push("Order number not detected.");
  }else if (orderNumbers.length === 1){
    pieces.push(`Order: ${orderNumbers[0]}`);
  }else{
    pieces.push(`Orders: ${orderNumbers.join(", ")}`);
  }
  const clientName = scope === "history"
    ? getClientName(historyState.selectedOrder, "")
    : getClientName(appState.extract, "");
  pieces.push(`Client: ${(clientName && clientName.trim()) ? clientName : "—"}`);
  el.textContent = pieces.join(" • ");

  if (scope === "extract"){
    const orderMeta = document.getElementById("extractMetaOrderNo");
    const dateMeta = document.getElementById("extractMetaDate");
    const clientMeta = document.getElementById("extractMetaClient");
    if (orderMeta){
      orderMeta.value = orderNumbers.length ? orderNumbers.join(", ") : "";
      orderMeta.placeholder = "—";
    }
    if (dateMeta){
      const ts = appState.extract.extractedAt ? new Date(appState.extract.extractedAt) : null;
      dateMeta.value = ts && !Number.isNaN(ts.getTime()) ? ts.toLocaleDateString() : "";
      dateMeta.placeholder = "—";
    }
    if (clientMeta){
      const clientText = (clientName && clientName.trim()) ? clientName.trim() : "";
      clientMeta.value = clientText;
      clientMeta.placeholder = "—";
    }
  }
}

function updateTotalsUI(scope){
  const bucket = scope === "history" ? appState.historyDetail : appState.extract;
  const rows = scope === "extract" ? getExtractRowsForView() : (bucket.rows || []);
  const summaryId = scope === "history" ? "historySummary" : "summary";
  const summaryEl = document.getElementById(summaryId);
  const totals = computeTotals(rows);
  if (summaryEl){
    summaryEl.innerHTML = buildSummaryHtml(rows, totals, { scope });
  }
  bucket.parsedUnits = totals.units;
  bucket.parsedArea = totals.area;
  if (scope === "history"){
    updateDeclaredBanner("historyDeclaredBanner", bucket.declaredUnits, bucket.declaredArea, bucket.parsedUnits, bucket.parsedArea, bucket.orderTotalDiagnostics);
  }else{
    updateDeclaredBanner("declaredBanner", bucket.declaredUnits, bucket.declaredArea, bucket.parsedUnits, bucket.parsedArea, bucket.orderTotalDiagnostics);
  }
  updateOrderInfo(scope);
  updateFixAllState(scope);
}

function updateFixAllState(scope){
  if (scope === "history"){
    const btn = document.getElementById("historyFixAllAreas");
    if (!btn) return;
    btn.hidden = true;
  }else{
    const btn = document.getElementById("fixAllAreas");
    if (!btn) return;
    btn.hidden = true;
  }
}

function updateHistoryDetailUI(){
  withPreservedFocus(() => {
    const container = document.getElementById("historyTableWrap");
    const empty = document.getElementById("historyDetailEmpty");
    const approveBtn = document.getElementById("historyApprove");
    const invoiceBtn = document.getElementById("historyInvoice");
    const archiveBtn = document.getElementById("historyArchive");
    const order = appState.historyDetail.order;
    recalcParsedTotals(appState.historyDetail);
    const fixAllBtn = document.getElementById("historyFixAllAreas");
	    if (!appState.historyDetail.order){
	      if (container) container.innerHTML = "";
	      renderWarningList("historyWarnings", []);
	      updateDeclaredBanner("historyDeclaredBanner", null, null, null, null);
	      if (approveBtn){
	        approveBtn.hidden = false;
	        approveBtn.disabled = true;
	      }
      if (empty) empty.style.display = "block";
      historyPrintBtn.disabled = true;
      historyCsvBtn.disabled = true;
      if (historyProcessingBtn) historyProcessingBtn.disabled = true;
      if (invoiceBtn) invoiceBtn.disabled = true;
      if (archiveBtn) archiveBtn.disabled = true;
      historyMetaEl.textContent = "";
      if (fixAllBtn) fixAllBtn.hidden = true;
      updateTotalsUI("history");
      return;
    }
    const status = normalizeHistoryStatusValue(order.status);
    const clientName = getClientName(order);
    const client = `Client: ${clientName}`;
    const confidence = formatHistoryConfidence(order.confidence);
    const metaPieces = [
      `Created: ${formatDate(order.created_at)}`,
      `Updated: ${formatDate(order.updated_at || order.created_at)}`,
      client,
      `Source: ${order.source || "—"}`,
      `Status: ${historyStatusLabel(status)}`,
      `Confidence: ${confidence}`,
      `Version: ${Number(order.version || 1)}`,
    ];
    historyMetaEl.textContent = metaPieces.join(" • ");
	    historyPrintBtn.disabled = !(order.rows && order.rows.length);
	    historyCsvBtn.disabled = !canCsvStatus(status);
	    if (historyProcessingBtn) historyProcessingBtn.disabled = !canProcessingStatus(status) || !(order.rows && order.rows.length);
	    if (invoiceBtn) invoiceBtn.disabled = !canInvoiceStatus(status) || !(order.rows && order.rows.length);
	    if (archiveBtn) archiveBtn.disabled = !canArchiveStatus(status);
	    if (status === "draft" || status === "reviewed"){
	      renderEditableTable("history", "historyTableWrap", appState.historyDetail.rows, appState.historyDetail.rowWarnings || {});
	      if (approveBtn){
	        approveBtn.hidden = false;
	        approveBtn.disabled = !canApproveStatus(status) || !(appState.historyDetail.rows && appState.historyDetail.rows.length);
	      }
	      const toggle = document.getElementById("historyGroupToggle");
	      if (toggle){
	        toggle.checked = false;
	        toggle.disabled = true;
	      }
	    }else{
	      render(appState.historyDetail.rows, { scope: "history" });
	      if (approveBtn){
	        approveBtn.hidden = true;
	        approveBtn.disabled = true;
	      }
	      const toggle = document.getElementById("historyGroupToggle");
	      if (toggle) toggle.disabled = false;
      if (!appState.historyDetail.rows || !appState.historyDetail.rows.length){
        if (container) container.innerHTML = '<div class="empty-state">No rows saved for this order.</div>';
      }
    }
    renderWarningList("historyWarnings", appState.historyDetail.warnings);
    updateTotalsUI("history");
    const notesEl = document.getElementById("historyNotes");
    if (notesEl && notesEl.value !== appState.historyDetail.notes){
      notesEl.value = appState.historyDetail.notes || "";
    }
    if (empty) empty.style.display = "none";
  });
  updateLabelsUI();
}

function render(rows, opts={}){
  const scope = opts.scope || "extract";
  if (!(scope in renderTargets)) return;
  if (rows === undefined || rows === null){
    rows = renderState[scope].rows || [];
  }else{
    renderState[scope].rows = Array.isArray(rows) ? rows : [];
  }
  const sourceRows = Array.isArray(rows) ? rows : [];
  const useGrouped = groupState;
  const rowsForView = useGrouped ? groupWithinOrders(sourceRows) : sourceRows;
  renderState[scope].displayed = rowsForView;

  const target = renderTargets[scope];
  const tableWrap = document.getElementById(target.tableId);
  const summaryEl = document.getElementById(target.summaryId);
  const orderInfoEl = document.getElementById(target.orderInfoId);

  if (!tableWrap) return;

  if (!sourceRows.length){
    tableWrap.innerHTML = "";
    if (summaryEl) summaryEl.textContent = "";
    if (orderInfoEl) orderInfoEl.textContent = "";
    return;
  }

  const groups = summarizeByOrder(rowsForView);
  const orderList = [...groups.keys()].filter(Boolean);
  const totals = computeTotals(rowsForView);

  if (summaryEl){
    let html = `Totals — <span class="pill">Units: ${totals.units}</span> <span class="pill">Area: ${formatArea(totals.area)} m²</span>`;
    if (groups.size > 1){
      html += `<span class="muted small">Multiple orders: ${orderList.join(", ")}</span>`;
    }
    summaryEl.innerHTML = html;
  }

  if (orderInfoEl){
    if (!orderList.length){
      orderInfoEl.textContent = "Order number not detected.";
    }else if (orderList.length === 1){
      orderInfoEl.textContent = `Order: ${orderList[0]}`;
    }else{
      orderInfoEl.textContent = `Orders: ${orderList.join(", ")}`;
    }
  }

  let html = "";
  for(const [order, rowsForOrder] of groups.entries()){
    const total = computeTotals(rowsForOrder);
    if (groups.size > 1){
      html += `<div style="padding:12px 14px"><div style="font-weight:600">${order || "No order number"}</div><div class="muted small">Units: ${total.units} • Area: ${formatArea(total.area)} m²</div></div>`;
    }
    html += `<table class="editable-table read-only-table"><thead><tr>
      <th>#</th>
      <th>Order</th>
      <th data-col="type">Type</th>
      <th data-col="dimension">Dimension</th>
      <th data-col="position">Position</th>
      <th data-col="qty">Quantity</th>
      <th data-col="area">Area (m²)</th>
    </tr></thead><tbody>`;
    rowsForOrder.forEach((row, idx)=>{
      html += `<tr>
        <td>${idx + 1}</td>
        <td class="mono">${row.order_number || ""}</td>
        <td data-col="type">${row.type || ""}</td>
        <td class="mono" data-col="dimension">${row.dimension ? row.dimension : "—"}</td>
        <td class="mono" data-col="position">${row.position || ""}</td>
        <td data-col="qty">${row.quantity}</td>
        <td data-col="area">${formatArea(row.area)}</td>
      </tr>`;
    });
    html += "</tbody></table>";
  }
  tableWrap.innerHTML = html;
}

function getDisplayedRows(scope="extract"){
  if (scope === "extract"){
    const sourceRows = getExtractRowsForView();
    if (groupState){
      return groupWithinOrders(sourceRows);
    }
    return sourceRows;
  }
  if (scope === "history"){
    const status = normalizeHistoryStatusValue(appState.historyDetail.order && appState.historyDetail.order.status);
    if (canEditDraftStatus(status)){
      return appState.historyDetail.rows || [];
    }
    if (groupState){
      return groupWithinOrders(appState.historyDetail.rows || []);
    }
    const state = renderState[scope];
    if (state && state.displayed && state.displayed.length) return state.displayed;
    return appState.historyDetail.rows || [];
  }
  return [];
}

function ensureHistoryLoaded(){
  if (historyState.loading){
    return historyState.loadingPromise;
  }
  if (!historyState.loadedOnce || historyState.needsRefresh){
    historyState.loadingPromise = loadHistory({ resetOffset: historyState.needsRefresh });
    return historyState.loadingPromise;
  }
  return Promise.resolve();
}

function getCurrentYearString(){
  return String(new Date().getFullYear());
}

function normalizeHistoryYear(value){
  const fallback = "all";
  if (value == null) return fallback;
  const text = String(value).trim().toLowerCase();
  if (!text) return fallback;
  if (text === "all") return "all";
  if (/^\d{4}$/.test(text)) return text;
  return fallback;
}

function readHistoryYearFromUrl(){
  const params = new URLSearchParams(window.location.search);
  const raw = params.get("year");
  if (raw == null || String(raw).trim() === "") return "all";
  return normalizeHistoryYear(raw);
}

function writeHistoryYearToUrl(yearValue){
  const normalized = normalizeHistoryYear(yearValue);
  const params = new URLSearchParams(window.location.search);
  params.set("year", normalized);
  const query = params.toString();
  const next = window.location.pathname + (query ? "?" + query : "") + window.location.hash;
  window.history.replaceState(null, "", next);
}

function getHistoryCacheKey(){
  const yearPart = normalizeHistoryYear(historyState.year || getCurrentYearString());
  return `${HISTORY_CACHE_KEY_BASE}.year.${yearPart}`;
}

function readHistoryCache(){
  try{
    const raw = localStorage.getItem(getHistoryCacheKey());
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? parsed : [];
  }catch{
    return [];
  }
}

function writeHistoryCache(items){
  try{
    localStorage.setItem(getHistoryCacheKey(), JSON.stringify(items));
  }catch{
    /* ignore quota errors */
  }
}

function updateHistoryCache(newItems){
  const map = new Map();
  const merged = [...newItems, ...readHistoryCache()];
  merged.forEach(item=>{
    if (!item || typeof item.id === "undefined") return;
    map.set(item.id, item);
  });
  const sorted = Array.from(map.values()).sort((a,b)=>{
    const da = new Date(a.created_at || 0).getTime();
    const db = new Date(b.created_at || 0).getTime();
    return db - da;
  });
  writeHistoryCache(sorted.slice(0, HISTORY_CACHE_LIMIT));
}

function removeFromHistoryCache(id){
  const idKey = String(id);
  const filtered = readHistoryCache().filter(item => String(item.id) !== idKey);
  writeHistoryCache(filtered);
}

async function fetchOrders(params){
  const search = new URLSearchParams();
  if (params.limit) search.set("limit", String(params.limit));
  if (params.offset) search.set("offset", String(params.offset));
  if (params.query) search.set("query", params.query);
  if (params.client) search.set("client", params.client);
  if (params.status) search.set("status", params.status);
  if (params.date_from) search.set("date_from", params.date_from);
  if (params.date_to) search.set("date_to", params.date_to);
  if (params.approved_only) search.set("approved_only", String(params.approved_only));
  if (params.year) search.set("year", String(params.year));
  const url = API_BASE + "/orders" + (search.toString() ? "?" + search.toString() : "");
  const res = await fetch(url);
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

async function fetchOrder(id){
  const res = await fetch(API_BASE + "/orders/" + id);
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

async function fetchAnalysisSignals(summary){
  const res = await fetch(API_BASE + "/analysis/signals", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ summary }),
  });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  const data = await res.json();
  return Array.isArray(data?.signals) ? data.signals : [];
}

async function fetchAnalysisSummary(settings){
  const search = new URLSearchParams();
  if (settings?.startDate && !settings?.allTime) search.set("start_date", settings.startDate);
  if (settings?.endDate && !settings?.allTime) search.set("end_date", settings.endDate);
  search.set("compare_previous", String(!!settings?.comparePrevious));
  if (settings?.client) search.set("client", settings.client);
  if (settings?.glassType) search.set("glass_type", settings.glassType);
  search.set("tolerance_mm", String(Number(settings?.tolerance) || 0));
  search.set("orientation_agnostic", String(!!settings?.orientationAgnostic));
  search.set("all_time", String(!!settings?.allTime));
  const res = await fetch(API_BASE + "/analysis/summary?" + search.toString());
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

async function deleteOrder(id){
  const numericId = Number(id);
  const pathId = Number.isFinite(numericId) ? String(numericId) : encodeURIComponent(String(id));
  const res = await fetch(API_BASE + "/orders/" + pathId, { method: "DELETE" });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
}

async function downloadOrderCsv(id){
  const res = await fetch(API_BASE + "/orders/" + id + "/csv");
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  const blob = await res.blob();
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "order-" + id + ".csv";
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
}

async function downloadAllCsv(){
  const search = new URLSearchParams();
  const normalizedYear = normalizeHistoryYear(historyState.year || "all");
	  search.set("year", normalizedYear);
	  if (historyState.query) search.set("query", historyState.query);
	  if (historyState.client) search.set("client", historyState.client);
	  search.set("status", historyState.approvedOnly && !historyState.status ? "approved" : (historyState.status || ""));
	  if (historyState.dateFrom) search.set("date_from", historyState.dateFrom);
	  if (historyState.dateTo) search.set("date_to", historyState.dateTo);
	  if (historyState.approvedOnly && !historyState.status) search.set("approved_only", "true");
  const requestUrl = API_BASE + "/orders/export.csv" + (search.toString() ? "?" + search.toString() : "");
  const res = await fetch(requestUrl);
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  const blob = await res.blob();
  const blobUrl = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = blobUrl;
  a.download = "orders-export.csv";
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(blobUrl);
}

function workspaceFileUrl(url){
  if (!url) return "";
  if (/^https?:\/\//i.test(url)) return url;
  if (/^blob:/i.test(url)) return url;
  return API_BASE + url;
}

const WORKSPACE_ORDER_TOKEN_RE = /\b(?:R-?)?\d{2}-\d{3,5}\b/gi;

function normalizeWorkspaceOrderToken(value){
  const text = String(value || "").trim().toUpperCase().replace(/\s+/g, "");
  if (!text) return "";
  if (/^R-?\d{2}-\d{3,5}$/.test(text)){
    return "R-" + text.replace(/^R-?/, "");
  }
  if (/^\d{2}-\d{3,5}$/.test(text)){
    return "R-" + text;
  }
  return text;
}

function extractWorkspaceOrderTokens(message){
  const matches = String(message || "").match(WORKSPACE_ORDER_TOKEN_RE) || [];
  const seen = new Set();
  const tokens = [];
  matches.forEach(match => {
    const normalized = normalizeWorkspaceOrderToken(match);
    if (!normalized || seen.has(normalized)) return;
    seen.add(normalized);
    tokens.push(normalized);
  });
  return tokens;
}

function inferWorkspaceProcessingMode(message, count = 0){
  const lower = String(message || "").toLowerCase();
  const keepOrderBoundaries = lower.includes("keep orders separate")
    || lower.includes("keep them separate")
    || lower.includes("don't mix orders")
    || lower.includes("do not mix orders")
    || lower.includes("without mixing orders")
    || lower.includes("no merge across orders");
  if (keepOrderBoundaries && (lower.includes("together") || lower.includes("same time") || lower.includes("combined") || lower.includes("one sheet"))){
    return "combined";
  }
  if (lower.includes("separately") || lower.includes("separate jobs") || lower.includes("separate processing") || lower.includes("one by one")){
    return "separate";
  }
  if (lower.includes("together") || lower.includes("same time") || lower.includes("combined") || lower.includes("one sheet")){
    return "combined";
  }
  return count > 1 ? "combined" : "single";
}

function inferWorkspaceMergeAcrossOrders(message){
  const lower = String(message || "").toLowerCase();
  if (lower.includes("keep orders separate")
    || lower.includes("keep them separate")
    || lower.includes("don't mix orders")
    || lower.includes("do not mix orders")
    || lower.includes("without mixing orders")
    || lower.includes("no merge across orders")
    || lower.includes("do not merge across orders")
    || lower.includes("don't merge across orders")){
    return false;
  }
  return lower.includes("merge across orders")
    || lower.includes("merge orders")
    || lower.includes("mix orders")
    || lower.includes("mix same glass across orders")
    || lower.includes("combine same glass across orders");
}

function inferWorkspaceProcessIntent(message){
  const text = String(message || "").trim();
  const lower = text.toLowerCase();
  const tokens = extractWorkspaceOrderTokens(text);
  const mode = inferWorkspaceProcessingMode(text, tokens.length);
  const mergeAcrossOrders = inferWorkspaceMergeAcrossOrders(text);
  if (lower.includes("process latest approved")){
    return { type: "process_latest" };
  }
  if (lower.includes("process selected") && workspaceState.selectedOrderIds.size){
    return { type: "process", identifiers: Array.from(workspaceState.selectedOrderIds), mode: workspaceState.selectedOrderIds.size > 1 ? "combined" : "single", mergeAcrossOrders };
  }
  if ((lower.includes("process this") || lower.includes("process selected")) && workspaceState.selectedOrderId){
    return { type: "process", identifier: workspaceState.selectedOrderId, identifiers: [workspaceState.selectedOrderId], mode: "single", mergeAcrossOrders: false };
  }
  if (lower.includes("process") && tokens.length){
    return { type: "process", identifier: tokens[0], identifiers: tokens, mode, mergeAcrossOrders };
  }
  if (lower.includes("generate labels") && tokens.length){
    return { type: "process", identifier: tokens[0], identifiers: tokens, mode, mergeAcrossOrders };
  }
  return null;
}

function getWorkspaceSelectedOrderIds(){
  return Array.from(workspaceState.selectedOrderIds || []).filter(Boolean);
}

function getWorkspaceSelectedOrderNumbers(){
  return getWorkspaceSelectedOrderIds()
    .map(id => workspaceState.selectedOrderNumbers.get(String(id)) || "")
    .filter(Boolean);
}

function flattenWorkspaceVisibleOrders(){
  const groups = workspaceState.queue?.groups || {};
  const visible = [];
  Object.entries(groups).forEach(([group, items]) => {
    (Array.isArray(items) ? items : []).forEach(item => {
      visible.push({
        order_id: String(item.order_id || item.id || ""),
        order_number: String(item.order_number || ""),
        client_name: item.client_name || null,
        status: item.status || "",
        queue_group: group,
        pieces: item.total_pieces ?? null,
        area: item.total_area_m2 ?? null,
        warnings_count: item.warnings_count ?? null,
        has_processing_pdf: !!item.processing_pdf_url,
        has_labels_pdf: !!item.labels_pdf_url,
      });
    });
  });
  return visible;
}

function getWorkspaceSelectedOrders(){
  const selectedIds = new Set(getWorkspaceSelectedOrderIds().map(String));
  return flattenWorkspaceVisibleOrders().filter(item => selectedIds.has(String(item.order_id)));
}

function buildWorkspaceAgentContext(){
  const selectedIds = getWorkspaceSelectedOrderIds();
  const selectedNumbers = getWorkspaceSelectedOrderNumbers();
  const selectedOrderId = selectedIds.length === 1 ? selectedIds[0] : (workspaceState.selectedOrderId || null);
  const selectedOrderNumber = selectedNumbers.length === 1 ? selectedNumbers[0] : (workspaceState.selectedOrderNumber || null);
  return {
    current_page: "Workspace",
    workspace_session_id: workspaceState.sessionId,
    selected_order_id: selectedOrderId || null,
    selected_order_number: selectedOrderNumber || null,
    selected_order_ids: selectedIds,
    selected_order_numbers: selectedNumbers,
    selected_orders: getWorkspaceSelectedOrders(),
    visible_orders: flattenWorkspaceVisibleOrders(),
    last_agent_action: workspaceState.lastAgentAction || null,
    latest_pending_action_id: workspaceState.latestPendingAction?.pending_action_id || null,
    latest_pending_action: workspaceState.latestPendingAction || null,
  };
}

function updateWorkspaceSelectionUI(){
  const selectedIds = getWorkspaceSelectedOrderIds();
  if (workspaceSelectionStatusEl){
    workspaceSelectionStatusEl.textContent = selectedIds.length ? `${selectedIds.length} order${selectedIds.length === 1 ? "" : "s"} selected` : "";
  }
  if (workspaceProcessSelectedBtn){
    workspaceProcessSelectedBtn.disabled = workspaceState.assistantLoading || selectedIds.length < 1;
  }
  if (workspaceClearSelectionBtn){
    workspaceClearSelectionBtn.disabled = workspaceState.assistantLoading || selectedIds.length < 1;
  }
  if (workspaceCommandInput){
    workspaceCommandInput.placeholder = selectedIds.length > 1 ? `${selectedIds.length} orders selected` : "Factory workflow command...";
  }
  document.querySelectorAll(".workspace-order-card").forEach(card => {
    const id = String(card.dataset.workspaceOrderId || "");
    card.classList.toggle("selected", !!id && workspaceState.selectedOrderIds.has(id));
    const btn = card.querySelector('[data-workspace-action="select"]');
    if (btn) btn.textContent = (!!id && workspaceState.selectedOrderIds.has(id)) ? "Selected" : "Select";
  });
}

function toggleWorkspaceOrderSelection(id, orderNumber){
  const key = String(id || "").trim();
  if (!key) return;
  if (workspaceState.selectedOrderIds.has(key)){
    workspaceState.selectedOrderIds.delete(key);
    workspaceState.selectedOrderNumbers.delete(key);
  }else{
    workspaceState.selectedOrderIds.add(key);
    workspaceState.selectedOrderNumbers.set(key, String(orderNumber || "").trim());
    workspaceState.selectedOrderId = key;
    workspaceState.selectedOrderNumber = String(orderNumber || "").trim();
  }
  updateWorkspaceSelectionUI();
}

function getLatestApprovedWorkspaceOrder(){
  const approved = workspaceState.queue?.groups?.approved_ready;
  return Array.isArray(approved) && approved.length ? approved[0] : null;
}

async function resolveWorkspaceOrder(identifier){
  const text = String(identifier || "").trim();
  if (!text) throw new Error("Order number is required.");
  if (/^\d+$/.test(text)){
    return fetchOrder(text);
  }
  const normalizedToken = normalizeWorkspaceOrderToken(text);
  const queryVariants = [text, normalizedToken];
  if (/^R-\d{2}-\d{3,5}$/i.test(normalizedToken)){
    queryVariants.push(normalizedToken.slice(2));
  }
  const seen = new Set();
  const items = [];
  for (const query of queryVariants.filter(Boolean)){
    const data = await fetchOrders({ query, year: "all", limit: 100, offset: 0 });
    (Array.isArray(data?.items) ? data.items : []).forEach(item => {
      const key = String(item.id || JSON.stringify(item));
      if (seen.has(key)) return;
      seen.add(key);
      items.push(item);
    });
  }
  const normalized = normalizedToken.toLowerCase();
  const exact = items.filter(item => (item.order_numbers || []).some(num => normalizeWorkspaceOrderToken(num).toLowerCase() === normalized));
  const matches = exact.length ? exact : items;
  if (!matches.length){
    throw new Error(`I couldn't find order ${text}.`);
  }
  if (matches.length > 1 && !exact.length){
    throw new Error("I found multiple matching orders. Please choose one.");
  }
  return fetchOrder(matches[0].id);
}

function getOrderWarningList(order){
  const warnings = [];
  (order?.warnings || []).forEach(item => {
    if (item) warnings.push(String(item));
  });
  const rowWarnings = order?.row_warnings || {};
  if (rowWarnings && typeof rowWarnings === "object"){
    Object.entries(rowWarnings).forEach(([key, values]) => {
      (values || []).forEach(value => {
        if (value) warnings.push(`Row ${key}: ${value}`);
      });
    });
  }
  return warnings;
}

function createWorkspaceDownloadUrl(blob){
  return URL.createObjectURL(blob);
}

function rememberWorkspaceModuleFile(record){
  workspaceState.moduleFiles.unshift(record);
  workspaceState.moduleFiles = workspaceState.moduleFiles.slice(0, 20);
  renderWorkspaceFiles(workspaceState.recentFiles || []);
}

function getProcessingSheetIdForOrder(order){
  return `processing-${order?.id || getOrderLabel(order) || Date.now()}`;
}

function getProcessingSheetIdForOrders(orders){
  const ids = (orders || []).map(order => order?.id || getOrderLabel(order)).filter(Boolean);
  return `processing-${ids.join("-") || Date.now()}`;
}

async function fetchWorkspaceQueue(){
  const res = await fetch(API_BASE + "/api/workspace/queue");
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

async function fetchWorkspaceRecentFiles(){
  const res = await fetch(API_BASE + "/api/workspace/recent-files");
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

function telegramFileUrl(path){
  if (!path) return "";
  if (/^https?:\/\//i.test(path)) return path;
  return API_BASE + path;
}

function formatFileSize(bytes){
  const value = Number(bytes || 0);
  if (!Number.isFinite(value) || value <= 0) return "—";
  if (value < 1024) return `${value} B`;
  if (value < 1024 * 1024) return `${(value / 1024).toFixed(1)} KB`;
  return `${(value / (1024 * 1024)).toFixed(2)} MB`;
}

async function fetchTelegramFiles(){
  const params = new URLSearchParams();
  if (telegramFilesState.query) params.set("query", telegramFilesState.query);
  if (telegramFilesState.status) params.set("status", telegramFilesState.status);
  params.set("touched", telegramFilesState.touched || "false");
  const res = await fetch(API_BASE + "/telegram-files" + (params.toString() ? "?" + params.toString() : ""));
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

function setTelegramFilesStatus(message){
  if (telegramFilesStatusEl) telegramFilesStatusEl.textContent = message || "";
}

function setTelegramLiveStatus(state, label){
  if (!telegramLiveStatusEl) return;
  telegramLiveStatusEl.classList.remove("connected", "reconnecting");
  if (state) telegramLiveStatusEl.classList.add(state);
  const labelEl = telegramLiveStatusEl.querySelector("span:last-child");
  if (labelEl) labelEl.textContent = label || "";
}

function updateTelegramFilesBadge(){
  if (!telegramFilesBadge) return;
  const count = Number(telegramFilesState.untouchedCount || 0);
  if (!Number.isFinite(count) || count <= 0){
    telegramFilesBadge.hidden = true;
    telegramFilesBadge.textContent = "0";
    return;
  }
  telegramFilesBadge.hidden = false;
  telegramFilesBadge.textContent = count > 9 ? "9+" : String(count);
  telegramFilesBadge.setAttribute("aria-label", `${count} new Telegram file${count === 1 ? "" : "s"}`);
}

function applyTelegramCounts(data){
  const counts = data?.counts && typeof data.counts === "object" ? data.counts : data;
  if (!counts || typeof counts !== "object") return;
  if (counts.untouched_count !== undefined) telegramFilesState.untouchedCount = Number(counts.untouched_count || 0);
  if (counts.queued_count !== undefined) telegramFilesState.queuedCount = Number(counts.queued_count || 0);
  if (counts.processing_count !== undefined) telegramFilesState.processingCount = Number(counts.processing_count || 0);
  if (counts.failed_count !== undefined) telegramFilesState.failedCount = Number(counts.failed_count || 0);
  updateTelegramFilesBadge();
}

function telegramStatusBadge(status){
  const normalized = String(status || "received").trim().toLowerCase() || "received";
  return `<span class="telegram-status ${escapeHtml(normalized)}">${escapeHtml(normalized)}</span>`;
}

function telegramDuplicateBadge(file){
  const status = String(file?.duplicate_status || "").trim().toLowerCase();
  if (status === "duplicate"){
    return `<span class="telegram-status duplicate" title="${escapeHtml(file?.duplicate_reason || "Exact duplicate file")}">DUPLICATE</span>`;
  }
  if (status === "possible_duplicate"){
    return `<span class="telegram-status possible_duplicate" title="${escapeHtml(file?.duplicate_reason || "Possible duplicate order")}">POSSIBLE DUPLICATE</span>`;
  }
  return "";
}

function telegramHandlingBadge(file){
  const touched = Boolean(file?.touched);
  const label = touched ? "Touched" : "New / Untouched";
  return `<span class="telegram-touch-status ${touched ? "touched" : "new"}">${escapeHtml(label)}</span>`;
}

function telegramHandlingProgressBadges(file){
  const labelsPrinted = Boolean(file?.labels_printed);
  const orderOpened = Boolean(file?.linked_order_opened);
  const pdfPrinted = Boolean(file?.pdf_printed ?? file?.pdfPrinted);
  return `<span class="telegram-handling-progress">
    <span class="telegram-handling-pill pdf ${pdfPrinted ? "done" : ""}" title="${pdfPrinted ? "PDF printed" : "PDF not printed yet"}">${pdfPrinted ? "PDF PRINTED" : "PDF"}</span>
    <span class="telegram-handling-pill ${labelsPrinted ? "done" : ""}" title="${labelsPrinted ? "Labels printed" : "Labels not printed yet"}">Labels</span>
    <span class="telegram-handling-pill ${orderOpened ? "done" : ""}" title="${orderOpened ? "Linked order opened" : "Linked order not opened yet"}">Order</span>
  </span>`;
}

function renderTelegramFiles(){
  if (!telegramFilesListEl) return;
  if (telegramFilesState.loading){
    telegramFilesListEl.innerHTML = '<div class="workspace-empty">Loading Telegram files...</div>';
    return;
  }
  const items = Array.isArray(telegramFilesState.items) ? telegramFilesState.items : [];
  if (!items.length){
    telegramFilesListEl.innerHTML = '<div class="workspace-empty">No Telegram PDF files found.</div>';
    return;
  }
  telegramFilesListEl.innerHTML = items.map(file => {
    const linkedOrderId = file.linked_order_id || "";
    const duplicateStatus = String(file.duplicate_status || "").trim().toLowerCase();
    const isExactDuplicate = duplicateStatus === "duplicate";
    const isPossibleDuplicate = duplicateStatus === "possible_duplicate";
    const linked = linkedOrderId ? (file.linked_order || null) : null;
    const orderNumbers = linked && Array.isArray(linked.order_numbers) && linked.order_numbers.length
      ? linked.order_numbers.join(", ")
      : "";
    const linkedLabel = linkedOrderId
      ? linked
        ? `${orderNumbers || `#${linked.id}`} • ${historyStatusLabel(linked.status)}`
        : `Linked order #${linkedOrderId}`
      : "No linked order yet";
    const sender = file.telegram_sender_name || (file.telegram_chat_id ? `Chat ${file.telegram_chat_id}` : "Telegram");
    const viewUrl = telegramFileUrl(file.view_url);
    const downloadUrl = telegramFileUrl(file.download_url);
    const duplicateWarning = isPossibleDuplicate
      ? `<div class="telegram-file-warning">${escapeHtml(file.duplicate_reason || "This looks like a possible duplicate. Review before creating another order.")}</div>`
      : "";
    return `<article class="telegram-file-card ${file.touched ? "" : "untouched"} ${isExactDuplicate ? "duplicate" : ""} ${isPossibleDuplicate ? "possible-duplicate" : ""}" data-telegram-file-id="${escapeHtml(file.id)}">
      <div>
        <div class="telegram-file-title">${escapeHtml(file.original_filename || "Telegram order.pdf")}</div>
        <div class="telegram-file-meta">
          <span>${escapeHtml(formatDate(file.received_at))}</span>
          <span>${escapeHtml(sender)}</span>
          <span>${escapeHtml(formatFileSize(file.file_size))}</span>
          ${telegramHandlingBadge(file)}
          ${telegramHandlingProgressBadges(file)}
          ${telegramStatusBadge(file.extraction_status)}
          ${telegramDuplicateBadge(file)}
          <span>${escapeHtml(linkedLabel)}</span>
        </div>
        ${duplicateWarning}
      </div>
      <div class="telegram-file-actions">
        <button type="button" class="btn small" data-telegram-action="view" data-id="${escapeHtml(file.id)}">View PDF</button>
        <a class="btn small" href="${escapeHtml(downloadUrl)}" download>Download</a>
        ${isExactDuplicate && file.duplicate_of_file_id ? `<button type="button" class="btn small" data-telegram-action="view" data-id="${escapeHtml(file.duplicate_of_file_id)}">Open original</button>` : ""}
        <button type="button" class="btn small" data-telegram-action="print" data-id="${escapeHtml(file.id)}">Print</button>
	        ${file.touched ? `<button type="button" class="btn small" disabled>Touched</button>` : `<button type="button" class="btn small" data-telegram-action="touch" data-id="${escapeHtml(file.id)}">Mark touched</button>`}
	        ${linkedOrderId ? `<button type="button" class="btn small" data-telegram-action="order" data-id="${escapeHtml(file.id)}" data-order-id="${escapeHtml(linkedOrderId)}">Open linked order</button>` : `<button type="button" class="btn small" disabled title="No linked order yet" aria-label="No linked order yet">Open linked order</button>`}
        ${file.linked_order_id && !isExactDuplicate ? `<button type="button" class="btn small" data-telegram-action="labels" data-id="${escapeHtml(file.id)}">Print Labels</button>` : `<button type="button" class="btn small" disabled title="${isExactDuplicate ? "Exact duplicate" : "No linked order yet"}" aria-label="${isExactDuplicate ? "Exact duplicate" : "No linked order yet"}">Print Labels</button>`}
        <button type="button" class="btn small warn" data-telegram-action="delete" data-id="${escapeHtml(file.id)}">Delete</button>
      </div>
    </article>`;
  }).join("");
  const untouched = Number(telegramFilesState.untouchedCount || 0);
  setTelegramFilesStatus(`${items.length} file${items.length === 1 ? "" : "s"}${untouched > 0 ? ` • ${untouched} new` : ""}`);
}

async function loadTelegramFiles(){
  if (!telegramFilesListEl) return;
  telegramFilesState.loading = true;
  setTelegramFilesStatus("Loading...");
  renderTelegramFiles();
  try{
    const data = await fetchTelegramFiles();
    telegramFilesState.items = Array.isArray(data?.items) ? data.items : [];
    applyTelegramCounts(data);
    telegramFilesState.loadedOnce = true;
    setTelegramFilesStatus("");
  }catch(error){
    telegramFilesState.items = [];
    setTelegramFilesStatus("Unable to load Telegram files.");
    telegramFilesListEl.innerHTML = `<div class="workspace-empty">Unable to load Telegram files: ${escapeHtml(error.message || error)}</div>`;
  }finally{
    telegramFilesState.loading = false;
    renderTelegramFiles();
  }
}

async function refreshTelegramFilesBadge(){
  try{
    const data = await fetchTelegramFiles();
    applyTelegramCounts(data);
  }catch(error){
    console.warn("Telegram files badge refresh failed", error);
  }
}

function getActiveTabName(){
  const active = document.querySelector(".tab-bar .tab.active");
  return active ? active.dataset.tab || "" : "";
}

function getTelegramFileById(id){
  return (telegramFilesState.items || []).find(item => String(item.id) === String(id)) || null;
}

function telegramFileMatchesFilters(file){
  if (!file) return false;
  if (file.deleted) return false;
  const touchedFilter = String(telegramFilesState.touched || "false").trim().toLowerCase();
  if (touchedFilter === "true" && !file.touched) return false;
  if (touchedFilter === "false" && file.touched) return false;
  const status = String(telegramFilesState.status || "").trim().toLowerCase();
  if (status && String(file.extraction_status || "").trim().toLowerCase() !== status) return false;
  const query = String(telegramFilesState.query || "").trim().toLowerCase();
  if (!query) return true;
  return String(file.original_filename || "").toLowerCase().includes(query)
    || String(file.telegram_sender_name || "").toLowerCase().includes(query)
    || String(file.telegram_chat_id || "").toLowerCase().includes(query);
}

function mergeTelegramFileIntoList(file, { prepend = false } = {}){
  if (!file || file.id == null) return false;
  const items = Array.isArray(telegramFilesState.items) ? telegramFilesState.items.slice() : [];
  const existingIndex = items.findIndex(item => String(item.id) === String(file.id));
  const matches = telegramFileMatchesFilters(file);
  if (!matches){
    if (existingIndex >= 0){
      items.splice(existingIndex, 1);
      telegramFilesState.items = items;
      return true;
    }
    return false;
  }
  if (existingIndex >= 0){
    items[existingIndex] = { ...items[existingIndex], ...file };
  }else if (prepend){
    items.unshift(file);
  }else{
    items.push(file);
  }
  telegramFilesState.items = items;
  return true;
}

function updateTelegramFileTouchedLocally(fileId, touchedAt){
  const file = getTelegramFileById(fileId);
  const wasUntouched = file && !file.touched;
  if (file){
    file.touched = true;
    file.touched_at = touchedAt || new Date().toISOString();
    if (!telegramFileMatchesFilters(file)){
      telegramFilesState.items = (telegramFilesState.items || []).filter(item => String(item.id) !== String(fileId));
    }
  }
  if (telegramFilesState.selectedFile && String(telegramFilesState.selectedFile.id) === String(fileId)){
    telegramFilesState.selectedFile = file || { ...telegramFilesState.selectedFile, touched: true, touched_at: touchedAt || new Date().toISOString() };
  }
  if (wasUntouched){
    telegramFilesState.untouchedCount = Math.max(0, Number(telegramFilesState.untouchedCount || 0) - 1);
  }
  updateTelegramFilesBadge();
  renderTelegramFiles();
}

function mergeTelegramFileUpdate(updatedFile){
  if (!updatedFile || updatedFile.id == null) return;
  mergeTelegramFileIntoList(updatedFile);
  if (telegramFilesState.selectedFile && String(telegramFilesState.selectedFile.id) === String(updatedFile.id)){
    telegramFilesState.selectedFile = { ...telegramFilesState.selectedFile, ...updatedFile };
  }
}

function updateTelegramFilePdfPrintedLocally(fileId, printedAt){
  const timestamp = printedAt || new Date().toISOString();
  const file = getTelegramFileById(fileId) || telegramFilesState.selectedFile || { id: fileId };
  const updatedFile = { ...file, id: fileId, pdf_printed: true, pdfPrinted: true, pdf_printed_at: timestamp };
  mergeTelegramFileUpdate(updatedFile);
  renderTelegramFiles();
}

async function markTelegramFileHandlingStep(fileId, step){
  if (!fileId || !step) return null;
  const endpoint = step === "labels"
    ? "mark-labels-printed"
    : step === "order"
      ? "mark-linked-order-opened"
      : "";
  if (!endpoint) return null;
  const res = await fetch(API_BASE + `/telegram-files/${encodeURIComponent(fileId)}/${endpoint}`, { method: "POST" });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  const data = await res.json();
  mergeTelegramFileUpdate(data?.file);
  applyTelegramCounts(data);
  renderTelegramFiles();
  return data?.file || null;
}

async function markTelegramFilePdfPrinted(fileId){
  if (!fileId) return null;
  console.info(`mark pdf printed called for telegram file ${fileId}`);
  updateTelegramFilePdfPrintedLocally(fileId);
  const res = await fetch(API_BASE + `/telegram-files/${encodeURIComponent(fileId)}/mark-pdf-printed`, { method: "POST" });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  const data = await res.json();
  mergeTelegramFileUpdate(data?.file);
  applyTelegramCounts(data);
  try{
    const refreshed = await fetchTelegramFiles();
    telegramFilesState.items = Array.isArray(refreshed?.items) ? refreshed.items : telegramFilesState.items;
    applyTelegramCounts(refreshed);
  }catch(refreshError){
    console.warn("Telegram files refresh after PDF print marker failed", refreshError);
  }
  renderTelegramFiles();
  return data?.file || null;
}

function confirmTelegramFileDelete(file){
  const linked = file?.linked_order || null;
  const linkedStatus = normalizeHistoryStatusValue(linked?.status || "");
  const canDeleteDraft = Boolean(file?.linked_order_id && linkedStatus === "draft");
  const hasProtectedOrder = Boolean(file?.linked_order_id && linkedStatus && linkedStatus !== "draft");
  return new Promise(resolve => {
    const layer = document.createElement("div");
    layer.className = "modal-layer";
    layer.innerHTML = `
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="telegramDeleteTitle">
        <h3 id="telegramDeleteTitle">Delete this Telegram file?</h3>
        <p>This hides it from Telegram Files. Original records are preserved.</p>
        ${canDeleteDraft ? `
          <label class="inline-check">
            <input type="checkbox" id="telegramDeleteLinkedOrder" checked />
            <span>Also delete linked draft order</span>
          </label>
        ` : ""}
        ${hasProtectedOrder ? `<p class="muted small">Linked order is not draft and will not be deleted.</p>` : ""}
        <div class="modal-actions">
          <button type="button" class="btn small" data-delete-cancel>Cancel</button>
          <button type="button" class="btn small warn" data-delete-confirm>Delete</button>
        </div>
      </div>
    `;
    const cleanup = value => {
      layer.remove();
      resolve(value);
    };
    layer.addEventListener("click", event => {
      if (event.target === layer || event.target.closest("[data-delete-cancel]")){
        cleanup(null);
      }else if (event.target.closest("[data-delete-confirm]")){
        cleanup({
          alsoDeleteLinkedOrder: canDeleteDraft && Boolean(layer.querySelector("#telegramDeleteLinkedOrder")?.checked),
        });
      }
    });
    document.body.appendChild(layer);
  });
}

async function deleteTelegramFile(file){
  if (!file?.id) return;
  const decision = await confirmTelegramFileDelete(file);
  if (!decision) return;
  try{
    const params = new URLSearchParams();
    if (decision.alsoDeleteLinkedOrder) params.set("also_delete_linked_order", "true");
    const res = await fetch(API_BASE + `/telegram-files/${encodeURIComponent(file.id)}${params.toString() ? `?${params.toString()}` : ""}`, { method: "DELETE" });
    if (!res.ok){
      const text = await res.text();
      throw new Error(text || ("HTTP " + res.status));
    }
    const data = await res.json();
    mergeTelegramFileUpdate(data?.file);
    applyTelegramCounts(data);
    if (data?.linked_order_deleted){
      historyState.needsRefresh = true;
      analysisState.allOrdersDirty = true;
    }
    renderTelegramFiles();
    setTelegramFilesStatus(data?.warning || "Telegram file deleted.");
  }catch(error){
    console.warn("Telegram file delete failed", error);
    setTelegramFilesStatus("Delete failed: " + (error.message || error));
  }
}

async function markTelegramFileTouched(fileId){
  if (!fileId) return;
  updateTelegramFileTouchedLocally(fileId);
  try{
    const res = await fetch(API_BASE + `/telegram-files/${encodeURIComponent(fileId)}/touch`, { method: "POST" });
    if (!res.ok){
      const text = await res.text();
      throw new Error(text || ("HTTP " + res.status));
    }
    const data = await res.json();
    mergeTelegramFileUpdate(data?.file);
    applyTelegramCounts(data);
    renderTelegramFiles();
  }catch(error){
    setTelegramFilesStatus("Unable to mark file touched.");
    console.warn("Telegram touch failed", error);
    await loadTelegramFiles();
  }
}

async function printTelegramLinkedLabels(file){
  if (!file || !file.linked_order_id){
    setTelegramFilesStatus("No linked order yet.");
    return;
  }
  setTelegramFilesStatus("Generating labels...");
  try{
    const order = await fetchOrder(file.linked_order_id);
    const rows = Array.isArray(order?.rows) ? order.rows : [];
    if (!rows.length){
      throw new Error("Linked order has no printable rows.");
    }
    await handlePrint(rows);
    try{
      await markTelegramFileHandlingStep(file.id, "labels");
      setTelegramFilesStatus("Labels ready.");
    }catch(markError){
      console.warn("Telegram labels marker failed", markError);
      setTelegramFilesStatus("Labels ready, but handling progress was not saved.");
    }
  }catch(error){
    console.warn("Telegram labels print failed", error);
    setTelegramFilesStatus("Failed to generate labels: " + (error.message || error));
  }
}

async function printTelegramOriginalPdf(file){
  if (!file?.id) return;
  try{
    printTelegramPdf(file);
  }catch(printError){
    console.warn("Telegram PDF print trigger failed", printError);
  }
  try{
    await markTelegramFilePdfPrinted(file.id);
  }catch(error){
    console.warn("Telegram PDF print marker failed", error);
    setTelegramFilesStatus("PDF print opened, but PDF printed status was not saved.");
  }
}

function handleTelegramFileEvent(payload){
  if (!payload || typeof payload !== "object") return;
  applyTelegramCounts(payload);
  const type = payload.type || "";
  const file = payload.file || null;
  if (type === "telegram_counts_updated"){
    return;
  }
  let changed = false;
  if (file){
    changed = mergeTelegramFileIntoList(file, { prepend: type === "telegram_file_created" });
    if (telegramFilesState.selectedFile && String(telegramFilesState.selectedFile.id) === String(file.id)){
      telegramFilesState.selectedFile = { ...telegramFilesState.selectedFile, ...file };
    }
  }
  if (type === "telegram_file_created"){
    if (getActiveTabName() === "telegram"){
      setTelegramFilesStatus("New Telegram order received ✅");
    }else if (typeof setStatusMessage === "function"){
      setStatusMessage("New Telegram order received ✅");
    }
  }
  if (changed && !telegramFilesState.loading){
    renderTelegramFiles();
  }
}

function parseTelegramEvent(event){
  try{
    return JSON.parse(event.data || "{}");
  }catch(error){
    console.warn("Invalid Telegram event payload", error);
    return null;
  }
}

function connectTelegramFileEvents(){
  if (!window.EventSource || telegramFilesState.events) return;
  const source = new EventSource(API_BASE + "/events/telegram-files");
  telegramFilesState.events = source;
  setTelegramLiveStatus("reconnecting", "Connecting...");
  source.onopen = () => {
    const shouldResync = telegramFilesState.eventReconnectPending;
    telegramFilesState.eventReconnectPending = false;
    setTelegramLiveStatus("connected", "Live");
    if (shouldResync){
      loadTelegramFiles();
    }
  };
  source.onerror = () => {
    telegramFilesState.eventReconnectPending = true;
    setTelegramLiveStatus("reconnecting", "Reconnecting...");
  };
  ["telegram_file_created", "telegram_file_updated", "telegram_counts_updated"].forEach(eventName => {
    source.addEventListener(eventName, event => {
      handleTelegramFileEvent(parseTelegramEvent(event));
    });
  });
}

function openTelegramPdf(file){
  if (!file || !telegramPdfModal || !telegramPdfFrame) return;
  telegramFilesState.selectedFile = file;
  const viewUrl = telegramFileUrl(file.view_url);
  const downloadUrl = telegramFileUrl(file.download_url);
  telegramPdfTitle.textContent = file.original_filename || "Telegram PDF";
  telegramPdfMeta.textContent = `${formatDate(file.received_at)} • ${file.telegram_sender_name || file.telegram_chat_id || "Telegram"} • ${formatFileSize(file.file_size)}`;
  telegramPdfDownloadLink.href = downloadUrl;
  telegramPdfFrame.src = viewUrl;
  telegramPdfModal.hidden = false;
}

function closeTelegramPdf(){
  if (telegramPdfModal) telegramPdfModal.hidden = true;
  if (telegramPdfFrame) telegramPdfFrame.removeAttribute("src");
  telegramFilesState.selectedFile = null;
}

function printTelegramPdf(file){
  if (!file) return false;
  const popup = window.open(telegramFileUrl(file.view_url), "_blank", "noopener");
  if (!popup) return false;
  popup.addEventListener("load", () => {
    try{ popup.focus(); popup.print(); }catch{/* browser PDF viewer may handle printing */}
  }, { once: true });
  return true;
}

async function postWorkspaceProcessOrder(identifier){
  const payload = {};
  if (String(identifier || "").trim().match(/^\d+$/)){
    payload.order_id = String(identifier);
  }else{
    payload.order_number = String(identifier || "").trim();
  }
  const res = await fetch(API_BASE + "/api/workspace/process-order", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

async function postWorkspaceAgent(message){
  const context = buildWorkspaceAgentContext();
  const res = await fetch(API_BASE + "/api/agent/workspace", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      message,
      selected_order_id: context.selected_order_id || undefined,
      selected_order_number: context.selected_order_number || undefined,
      context,
    }),
  });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

async function postWorkspaceSmartChat(message){
  const context = buildWorkspaceAgentContext();
  const res = await fetch(API_BASE + "/api/agent/smart-chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ message, context }),
  });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

async function postWorkspaceConfirmAction(decision){
  const pending = workspaceState.latestPendingAction;
  if (!pending?.pending_action_id){
    throw new Error("There is no pending action to continue. Tell me which order to process.");
  }
  const res = await fetch(API_BASE + "/api/workspace/confirm-action", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      action: "pending_action",
      pending_action_id: pending.pending_action_id,
      decision,
      context: buildWorkspaceAgentContext(),
    }),
  });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

function workspaceGroupLabel(key){
  const labels = {
    needs_review: "Needs Review",
    approved_ready: "Approved / Ready to Process",
    processing_done: "Processing Done",
    labels_ready: "Labels Ready",
    finished: "Finished",
  };
  return labels[key] || key;
}

function renderWorkspaceFiles(files){
  if (!workspaceRecentFilesEl) return;
  const serverItems = Array.isArray(files) ? files : [];
  const items = [...(workspaceState.moduleFiles || []), ...serverItems];
  if (!items.length){
    workspaceRecentFilesEl.innerHTML = '<div class="workspace-empty">No production files generated yet.</div>';
    return;
  }
  workspaceRecentFilesEl.innerHTML = items.map(item => {
    const processingUrl = workspaceFileUrl(item.processing_pdf_url);
    const labelsUrl = workspaceFileUrl(item.labels_pdf_url);
    const modeLabel = item.mode === "combined" ? " · Mode: Combined" : (item.mode === "separate" ? " · Mode: Separate" : "");
    const mergeLabel = item.mode === "combined" ? ` · Merge across orders: ${item.mergeAcrossOrders || item.merge_across_orders ? "Yes" : "No"}` : "";
    return `<div class="workspace-file-item">
      <div>
        <div><strong>${escapeHtml(item.order_number || "—")}</strong> ${item.client_name ? `<span class="muted small">${escapeHtml(item.client_name)}</span>` : ""}</div>
        <div class="workspace-file-meta">${escapeHtml(formatDate(item.generated_at))} · ${escapeHtml(item.batch_status || "ready")}${item.module_workflow ? " · Processing module" : ""}${escapeHtml(modeLabel)}${escapeHtml(mergeLabel)}</div>
      </div>
      <div class="workspace-file-actions">
        ${item.workspace_job_id ? `<button type="button" class="btn small" data-workspace-open="workspace-processing" data-workspace-job-id="${escapeHtml(item.workspace_job_id)}">Open Job</button>` : ""}
        ${processingUrl ? `<a class="btn small" href="${escapeHtml(processingUrl)}" target="_blank" rel="noopener">Processing PDF</a>` : ""}
        ${labelsUrl ? `<a class="btn small" href="${escapeHtml(labelsUrl)}" target="_blank" rel="noopener">Labels PDF</a>` : ""}
      </div>
    </div>`;
  }).join("");
}

function renderAssistantFiles(files, options = {}){
  if (!workspaceAssistantFilesEl) return;
  const processingUrl = workspaceFileUrl(files?.processing_pdf_url);
  const labelsUrl = workspaceFileUrl(files?.labels_pdf_url);
  const workspaceJobAttr = options.workspaceJobId ? ` data-workspace-job-id="${escapeHtml(options.workspaceJobId)}"` : "";
  workspaceAssistantFilesEl.innerHTML = [
    options.openProcessing ? `<button type="button" class="btn small" data-workspace-open="workspace-processing"${workspaceJobAttr}>Open Workspace Processing Sheet</button>` : "",
    processingUrl ? `<a class="btn small" href="${escapeHtml(processingUrl)}" target="_blank" rel="noopener">Download Processing PDF</a>` : "",
    options.openLabels ? `<button type="button" class="btn small" data-workspace-open="workspace-labels"${workspaceJobAttr}>Open Label Job</button>` : "",
    labelsUrl ? `<a class="btn small" href="${escapeHtml(labelsUrl)}" target="_blank" rel="noopener">Download Labels PDF</a>` : "",
  ].filter(Boolean).join("");
}

function ensureWorkspaceProcessingJobView(){
  let panel = document.getElementById("workspaceProcessingJobView");
  if (panel) return panel;
  const processingMain = processingMetaEl ? processingMetaEl.closest(".processing-main") : null;
  if (!processingMain) return null;
  panel = document.createElement("div");
  panel.id = "workspaceProcessingJobView";
  panel.className = "workspace-job-view";
  panel.hidden = true;
  processingMain.insertBefore(panel, processingMain.firstChild);
  return panel;
}

function hideWorkspaceProcessingJobView(){
  const panel = document.getElementById("workspaceProcessingJobView");
  if (panel){
    panel.hidden = true;
    panel.innerHTML = "";
  }
}

function renderWorkspaceProcessingJobView(job){
  const panel = ensureWorkspaceProcessingJobView();
  if (!panel || !job) return;
  const processingUrl = workspaceFileUrl(job.processing_pdf_url);
  const labelsUrl = workspaceFileUrl(job.labels_pdf_url);
  const orders = Array.isArray(job.order_numbers) && job.order_numbers.length ? job.order_numbers.join(", ") : (job.order_number || "Workspace job");
  const preview = job.preview_text || job.processing_state?.preview?.text || "No preview text captured.";
  const modeLabel = job.mode === "combined" ? "Mode: Combined" : (job.mode === "separate" ? "Mode: Separate" : "Mode: Single");
  const mergeLabel = job.mode === "combined" ? ` · Merge across orders: ${job.mergeAcrossOrders || job.merge_across_orders ? "Yes" : "No"}` : "";
  panel.hidden = false;
  panel.innerHTML = `<header>
    <div>
      <span class="badge">Workspace job — isolated from manual sheet</span>
      <h3>${escapeHtml(orders)}</h3>
      <div class="muted small">${escapeHtml(modeLabel)}${escapeHtml(mergeLabel)} · Your manual Processing tab was not changed.</div>
    </div>
    <div class="workspace-file-actions">
      <button type="button" class="btn small" data-workspace-job-action="copy" data-workspace-job-id="${escapeHtml(job.id || job.workspace_job_id || "")}">Copy Text</button>
      ${processingUrl ? `<a class="btn small" href="${escapeHtml(processingUrl)}" target="_blank" rel="noopener">Download PDF</a>` : ""}
      ${labelsUrl ? `<a class="btn small" href="${escapeHtml(labelsUrl)}" target="_blank" rel="noopener">Download Labels</a>` : ""}
      <button type="button" class="btn small" data-workspace-job-action="close">Close</button>
    </div>
  </header>
  <div class="workspace-job-preview">${escapeHtml(preview)}</div>`;
}

function openWorkspaceProcessingJob(jobId){
  const job = findWorkspaceProcessingJob(jobId) || (workspaceState.processingJobs || [])[0];
  activateTab("processing");
  if (!job){
    appendWorkspaceMessage("assistant", "I could not find that Workspace processing job in this session.");
    return;
  }
  renderWorkspaceProcessingJobView(job);
}

function renderWorkspacePendingAction(pending){
  if (!workspaceAssistantFilesEl) return;
  if (!pending){
    workspaceAssistantFilesEl.innerHTML = "";
    return;
  }
  const warningCounts = {};
  (pending.warnings || []).forEach(item => {
    const key = item.order_number || "Order";
    warningCounts[key] = (warningCounts[key] || 0) + 1;
  });
  const orders = (pending.order_numbers || []).map(number => {
    const count = warningCounts[number] || 0;
    return `<li>${escapeHtml(number)}${count ? ` — ${count} warning${count === 1 ? "" : "s"}` : ""}</li>`;
  }).join("");
  workspaceAssistantFilesEl.innerHTML = `<div class="workspace-pending-card">
    <strong>Warnings found ⚠️</strong>
    <ul>${orders}</ul>
    <div class="workspace-file-actions" style="justify-content:flex-start">
      <button type="button" class="btn small primary" data-workspace-pending-decision="continue">Continue Anyway</button>
      <button type="button" class="btn small" data-workspace-pending-decision="open_review">Open Review</button>
      <button type="button" class="btn small" data-workspace-pending-decision="cancel">Cancel</button>
    </div>
  </div>`;
}

function appendWorkspaceMessage(role, message){
  if (!workspaceChatLogEl) return;
  const div = document.createElement("div");
  div.className = `workspace-message ${role === "user" ? "user" : "assistant"}`;
  div.textContent = message || "";
  workspaceChatLogEl.appendChild(div);
  workspaceChatLogEl.scrollTop = workspaceChatLogEl.scrollHeight;
}

function appendSmartChatMessage(role, message){
  if (!workspaceSmartChatLogEl) return;
  const div = document.createElement("div");
  div.className = `workspace-message ${role === "user" ? "user" : "assistant"}`;
  div.textContent = message || "";
  workspaceSmartChatLogEl.appendChild(div);
  workspaceSmartChatLogEl.scrollTop = workspaceSmartChatLogEl.scrollHeight;
}

function setWorkspaceBusy(busy, message = ""){
  workspaceState.assistantLoading = !!busy;
  if (workspaceSendBtn) workspaceSendBtn.disabled = !!busy;
  if (workspaceAssistantStatusEl) workspaceAssistantStatusEl.textContent = message;
  document.querySelectorAll("[data-workspace-action], [data-workspace-prompt]").forEach(btn => {
    const isBlockedProcess = btn.dataset.workspaceAction === "process" && btn.dataset.canProcess === "false";
    btn.disabled = !!busy || isBlockedProcess;
  });
  updateWorkspaceSelectionUI();
}

function setSmartChatBusy(busy, message = ""){
  workspaceState.smartChatLoading = !!busy;
  if (workspaceSmartChatSendBtn) workspaceSmartChatSendBtn.disabled = !!busy;
  if (workspaceSmartChatStatusEl) workspaceSmartChatStatusEl.textContent = message;
  document.querySelectorAll("[data-smart-chat-prompt]").forEach(btn => {
    btn.disabled = !!busy;
  });
}

function renderWorkspaceQueue(queue){
  if (!workspaceQueueEl) return;
  const sourceGroups = queue?.groups || {};
  const groups = {};
  Object.entries(sourceGroups).forEach(([key, value]) => {
    groups[key] = Array.isArray(value) ? value.slice() : [];
  });
  const locallyProcessedIds = new Set();
  (workspaceState.moduleFiles || []).forEach(item => {
    if (item.order_id) locallyProcessedIds.add(String(item.order_id));
    (item.order_ids || []).forEach(id => locallyProcessedIds.add(String(id)));
  });
  if (locallyProcessedIds.size){
    groups.approved_ready = (groups.approved_ready || []).filter(item => {
      if (!locallyProcessedIds.has(String(item.order_id || item.id || ""))) return true;
      groups.labels_ready = groups.labels_ready || [];
      const file = (workspaceState.moduleFiles || []).find(entry => {
        const itemId = String(item.order_id || item.id || "");
        return String(entry.order_id || "") === itemId || (entry.order_ids || []).map(String).includes(itemId);
      });
      groups.labels_ready.unshift({ ...item, ...(file || {}), status: "in_production" });
      return false;
    });
  }
  const order = ["needs_review", "approved_ready", "processing_done", "labels_ready", "finished"];
  workspaceQueueEl.innerHTML = order.map(key => {
    const items = Array.isArray(groups[key]) ? groups[key] : [];
    const cards = items.length ? items.map(item => {
      const processingUrl = workspaceFileUrl(item.processing_pdf_url);
      const labelsUrl = workspaceFileUrl(item.labels_pdf_url);
      const canProcess = key === "approved_ready";
      const itemId = String(item.order_id || item.id || "");
      const selected = workspaceState.selectedOrderIds.has(itemId);
      return `<div class="workspace-order-card ${selected ? "selected" : ""}" data-workspace-order-id="${escapeHtml(itemId)}" data-workspace-order-number="${escapeHtml(item.order_number || "")}">
        <div class="workspace-order-top">
          <div style="min-width:0">
            <div class="workspace-order-number mono">${escapeHtml(item.order_number || "—")}</div>
            <div class="workspace-order-client">${escapeHtml(item.client_name || "—")}</div>
          </div>
          ${historyStatusBadgeHtml(item.status)}
        </div>
        <div class="workspace-order-metrics">
          <span>${Number(item.total_pieces || 0)} pcs</span>
          <span>${Number(item.total_area_m2 || 0).toFixed(3)} m²</span>
          <span>${Number(item.warnings_count || 0)} warn</span>
        </div>
        <div class="workspace-file-meta">${escapeHtml(formatDate(item.approved_at || item.created_at))}</div>
        <div class="workspace-actions">
          <button type="button" class="btn small" data-workspace-action="select" data-id="${escapeHtml(itemId)}" data-order="${escapeHtml(item.order_number || "")}">${selected ? "Selected" : "Select"}</button>
          <button type="button" class="btn small" data-workspace-action="open" data-id="${escapeHtml(itemId)}">Open</button>
          <button type="button" class="btn small" data-workspace-action="check" data-order="${escapeHtml(item.order_number || "")}">Check</button>
          <button type="button" class="btn small primary" data-workspace-action="process" data-can-process="${canProcess ? "true" : "false"}" data-id="${escapeHtml(itemId)}" ${canProcess ? "" : "disabled"}>Process</button>
          <button type="button" class="btn small" data-workspace-action="labels" data-order="${escapeHtml(item.order_number || "")}">Labels</button>
          ${processingUrl ? `<a class="btn small" href="${escapeHtml(processingUrl)}" target="_blank" rel="noopener">Processing PDF</a>` : ""}
          ${labelsUrl ? `<a class="btn small" href="${escapeHtml(labelsUrl)}" target="_blank" rel="noopener">Labels PDF</a>` : ""}
        </div>
      </div>`;
    }).join("") : '<div class="workspace-empty">No orders.</div>';
    return `<section class="workspace-group">
      <div class="workspace-group-title"><span>${escapeHtml(workspaceGroupLabel(key))}</span><span class="pill">${items.length}</span></div>
      ${cards}
    </section>`;
  }).join("");
}

async function loadWorkspace(){
  if (!workspaceQueueEl) return;
  workspaceState.loading = true;
  if (workspaceQueueStatusEl) workspaceQueueStatusEl.textContent = "Loading...";
  if (workspaceFilesStatusEl) workspaceFilesStatusEl.textContent = "Loading...";
  try{
    const [queue, files] = await Promise.all([fetchWorkspaceQueue(), fetchWorkspaceRecentFiles()]);
    workspaceState.queue = queue;
    workspaceState.recentFiles = Array.isArray(files?.items) ? files.items : [];
    renderWorkspaceQueue(queue);
    renderWorkspaceFiles(workspaceState.recentFiles);
    if (workspaceQueueStatusEl) workspaceQueueStatusEl.textContent = "";
    if (workspaceFilesStatusEl) workspaceFilesStatusEl.textContent = "";
  }catch(error){
    if (workspaceQueueStatusEl) workspaceQueueStatusEl.textContent = "Workspace failed to load.";
    if (workspaceQueueEl) workspaceQueueEl.innerHTML = `<div class="workspace-empty">${escapeHtml(error.message || error)}</div>`;
  }finally{
    workspaceState.loading = false;
  }
}

function setAwaStatus(message){
  if (awaStatusEl) awaStatusEl.textContent = message || "";
}

function awaStatusBadge(value){
  const normalized = String(value || "review").trim().toLowerCase().replace(/\s+/g, "_") || "review";
  const label = normalized.replace(/_/g, " ");
  return `<span class="status-badge awa-${escapeHtml(normalized)}">${escapeHtml(label)}</span>`;
}

function awaActionButtonLabel(action){
  const type = String(action?.type || "");
  if (type.includes("process")) return "Open Workspace";
  if (type.includes("label")) return "Open Labels";
  if (type.includes("invoice")) return "Open Invoice";
  if (type.includes("download")) return "Open Workspace";
  return "Open History";
}

function awaRelatedModule(action){
  const type = String(action?.type || "");
  if (type.includes("label")) return "labels";
  if (type.includes("invoice")) return "invoices";
  if (type.includes("process") || type.includes("download")) return "workspace";
  return "history";
}

async function fetchAwaJson(path, options){
  const res = await fetch(API_BASE + path, options || {});
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || ("HTTP " + res.status));
  }
  return res.json();
}

function renderAwaSummary(){
  const summary = awaState.summary || {};
  if (awaSuggestedTodayEl) awaSuggestedTodayEl.textContent = String(summary.suggested_actions_today ?? 0);
  if (awaReadyToProcessEl) awaReadyToProcessEl.textContent = String(summary.ready_to_process ?? 0);
  if (awaNeedsReviewEl) awaNeedsReviewEl.textContent = String(summary.needs_review ?? 0);
  if (awaWaitingApprovalEl) awaWaitingApprovalEl.textContent = String(summary.waiting_approval ?? 0);
}

function renderAwaTimeline(){
  if (!awaTimelineEl) return;
  const items = Array.isArray(awaState.timeline) ? awaState.timeline : [];
  if (!items.length){
    awaTimelineEl.innerHTML = '<div class="workspace-empty">No AWA activity yet.</div>';
    return;
  }
  awaTimelineEl.innerHTML = items.map(item => `
    <div class="awa-timeline-item">
      <div class="awa-timeline-time">${escapeHtml(formatDate(item.timestamp))}</div>
      <div>
        <div class="awa-timeline-title">
          <span>${escapeHtml(item.title || "Activity")}</span>
          ${awaStatusBadge(item.status)}
        </div>
        <div class="awa-timeline-text">${item.order_number ? `<span class="mono">${escapeHtml(item.order_number)}</span> · ` : ""}${escapeHtml(item.explanation || "")}</div>
      </div>
    </div>
  `).join("");
}

function renderAwaSuggestions(){
  if (!awaSuggestionsEl) return;
  const items = Array.isArray(awaState.suggestions) ? awaState.suggestions : [];
  if (!items.length){
    awaSuggestionsEl.innerHTML = '<div class="workspace-empty">No suggested AWA actions right now.</div>';
    return;
  }
  awaSuggestionsEl.innerHTML = items.map(action => {
    const status = String(action.status || "suggested").toLowerCase();
    const approvedOrRejected = status === "approved" || status === "rejected";
    const orderLabel = (action.order_numbers || []).length ? action.order_numbers.join(", ") : "No specific order";
    const confidence = Math.round(Number(action.confidence || 0) * 100);
    return `<article class="awa-suggestion-card" data-awa-action-id="${escapeHtml(action.id)}">
      <div class="awa-suggestion-header">
        <div>
          <div class="awa-suggestion-title">${escapeHtml(action.title || "Suggested action")}</div>
          <div class="awa-suggestion-meta">
            <span class="mono">${escapeHtml(orderLabel)}</span>
            <span>${Number.isFinite(confidence) ? confidence : 0}% confidence</span>
            ${awaStatusBadge(action.safety_level)}
            ${awaStatusBadge(status)}
          </div>
        </div>
      </div>
      <div class="muted small">${escapeHtml(action.explanation || "")}</div>
      ${action.result?.message ? `<div class="awa-warning">${escapeHtml(action.result.message)}</div>` : ""}
      <div class="awa-suggestion-actions">
        <button type="button" class="btn small primary" data-awa-action="approve" ${approvedOrRejected ? "disabled" : ""}>Approve</button>
        <button type="button" class="btn small" data-awa-action="reject" ${approvedOrRejected ? "disabled" : ""}>Reject</button>
        <button type="button" class="btn small" data-awa-action="explain">Explain</button>
        <button type="button" class="btn small" data-awa-action="open">${escapeHtml(awaActionButtonLabel(action))}</button>
      </div>
    </article>`;
  }).join("");
}

async function loadAwa(){
  if (!awaSuggestionsEl) return;
  awaState.loading = true;
  setAwaStatus("Loading AWA signals...");
  try{
    const [summary, timeline, suggestions] = await Promise.all([
      fetchAwaJson("/api/awa/summary"),
      fetchAwaJson("/api/awa/timeline"),
      fetchAwaJson("/api/awa/suggestions"),
    ]);
    awaState.summary = summary || {};
    awaState.timeline = Array.isArray(timeline?.items) ? timeline.items : [];
    awaState.suggestions = Array.isArray(suggestions?.items) ? suggestions.items : [];
    renderAwaSummary();
    renderAwaTimeline();
    renderAwaSuggestions();
    setAwaStatus("AWA Beta is watching safely. No actions run without approval.");
  }catch(error){
    setAwaStatus("Unable to load AWA: " + (error.message || error));
    awaSuggestionsEl.innerHTML = `<div class="workspace-empty">${escapeHtml(error.message || error)}</div>`;
  }finally{
    awaState.loading = false;
  }
}

function findAwaSuggestion(actionId){
  return (awaState.suggestions || []).find(item => String(item.id) === String(actionId)) || null;
}

function mergeAwaSuggestion(suggestion){
  if (!suggestion?.id) return;
  const items = Array.isArray(awaState.suggestions) ? awaState.suggestions.slice() : [];
  const index = items.findIndex(item => String(item.id) === String(suggestion.id));
  if (index >= 0) items[index] = { ...items[index], ...suggestion };
  else items.unshift(suggestion);
  awaState.suggestions = items;
  renderAwaSuggestions();
}

function appendAwaMessage(role, text){
  if (!awaChatLogEl) return;
  const div = document.createElement("div");
  div.className = `workspace-message ${role === "user" ? "user" : "assistant"}`;
  div.textContent = text || "";
  awaChatLogEl.appendChild(div);
  awaChatLogEl.scrollTop = awaChatLogEl.scrollHeight;
}

async function explainAwaAction(actionId){
  try{
    const data = await fetchAwaJson("/api/awa/explain", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({ action_id: actionId }),
    });
    appendAwaMessage("assistant", data.message || "No explanation returned.");
    setAwaStatus("AWA explanation ready.");
  }catch(error){
    setAwaStatus("Explain failed: " + (error.message || error));
  }
}

async function runAwaAction(actionId, action){
  const path = action === "reject" ? "/api/awa/reject-action" : "/api/awa/approve-action";
  setAwaStatus(action === "reject" ? "Rejecting suggestion..." : "Recording supervised approval...");
  try{
    const data = await fetchAwaJson(path, {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({ action_id: actionId }),
    });
    mergeAwaSuggestion(data.suggestion);
    setAwaStatus(data.message || "AWA action updated.");
    appendAwaMessage("assistant", data.message || "AWA action updated. No production data was changed.");
  }catch(error){
    setAwaStatus("AWA action failed: " + (error.message || error));
  }
}

async function askAwa(question){
  const text = String(question || "").trim();
  if (!text || awaState.chatLoading) return;
  awaState.chatLoading = true;
  if (awaChatSendBtn) awaChatSendBtn.disabled = true;
  appendAwaMessage("user", text);
  try{
    const data = await fetchAwaJson("/api/awa/explain", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({ question: text }),
    });
    appendAwaMessage("assistant", data.message || "AWA has no recommendation for that yet.");
  }catch(error){
    appendAwaMessage("assistant", "AWA could not answer: " + (error.message || error));
  }finally{
    awaState.chatLoading = false;
    if (awaChatSendBtn) awaChatSendBtn.disabled = false;
  }
}

async function refreshWorkspaceAfterAction(){
  await loadWorkspace();
  historyState.needsRefresh = true;
  analysisState.allOrdersDirty = true;
}

function getWorkspaceOrderRows(order){
  return Array.isArray(order?.rows) ? order.rows : [];
}

function summarizeWorkspaceOrders(orders){
  const glassTypes = new Set();
  let totalPieces = 0;
  let totalArea = 0;
  (orders || []).forEach(order => {
    const rows = getWorkspaceOrderRows(order);
    if (!rows.length){
      totalPieces += Number(order?.total_pieces || order?.units || 0) || 0;
      totalArea += Number(order?.total_area_m2 || order?.total_area || order?.area_total || 0) || 0;
      return;
    }
    rows.forEach(row => {
      const qty = Number(row.quantity ?? row.qty ?? 0);
      totalPieces += Number.isFinite(qty) && qty > 0 ? qty : 1;
      const area = Number(String(row.area ?? row.area_m2 ?? row.areaM2 ?? 0).replace(",", "."));
      if (Number.isFinite(area)) totalArea += area;
      const type = String(row.type || row.composition_display || row.composition_raw || row.glass_type || row.glassType || "").trim();
      if (type) glassTypes.add(type);
    });
  });
  return {
    total_pieces: totalPieces,
    total_area_m2: Number(totalArea.toFixed(3)),
    glass_type_count: glassTypes.size,
  };
}

async function resolveWorkspaceOrders(identifiers){
  const requested = (Array.isArray(identifiers) ? identifiers : [identifiers])
    .map(item => normalizeWorkspaceOrderToken(item) || String(item || "").trim())
    .filter(Boolean);
  if (!requested.length){
    throw new Error("Tell me which order number to process.");
  }
  const found = [];
  const missing = [];
  const seenIds = new Set();
  for (const identifier of requested){
    try{
      const order = await resolveWorkspaceOrder(identifier);
      const key = String(order.id || getOrderLabel(order));
      if (!seenIds.has(key)){
        seenIds.add(key);
        found.push(order);
      }
    }catch(error){
      missing.push(identifier);
    }
  }
  if (missing.length){
    const foundLabels = found.map(getOrderLabel).filter(Boolean);
    const foundText = foundLabels.length ? `I found ${foundLabels.join(", ")}, but ` : "";
    throw new Error(`${foundText}I couldn't find ${missing.join(", ")}. No files were created.`);
  }
  return found;
}

function validateWorkspaceOrdersForProcessing(orders, options = {}){
  if (options.force) return;
  const blocked = [];
  (orders || []).forEach(order => {
    const orderNumber = getOrderLabel(order);
    const status = normalizeHistoryStatusValue(order.status);
    if (status !== "approved"){
      blocked.push(`${orderNumber} is ${historyStatusLabel(status)}`);
    }
  });
  if (blocked.length){
    throw new Error(`${blocked.join(", ")}. I stopped before processing.`);
  }
}

function getWorkspaceWarningNotes(orders){
  return (orders || []).map(order => {
    const warnings = getOrderWarningList(order);
    return {
      orderNumber: getOrderLabel(order),
      count: warnings.length,
      warnings,
    };
  }).filter(item => item.count > 0);
}

function cloneWorkspaceStateValue(value){
  if (typeof structuredClone === "function"){
    try{
      return structuredClone(value);
    }catch(error){
      console.warn("structuredClone failed for Workspace state; falling back to JSON clone", error);
    }
  }
  return JSON.parse(JSON.stringify(value));
}

function replaceObjectContents(target, source){
  Object.keys(target).forEach(key => delete target[key]);
  Object.assign(target, source || {});
}

function createEmptyProcessingState(baseProcessing = appState.processing){
  return {
    cart: [],
    rows: [],
    preview: { text: "", groups: [], meta: { clientLabel: "—", orders: [], date: new Date(), rows: 0 } },
    grouped: false,
    options: cloneWorkspaceStateValue(baseProcessing?.options || {
      restartPerGroup: false,
      decimalSeparator: "comma",
      normalizeLPtoG: true,
      mergeAcrossOrders: false,
      autoDanko: false,
    }),
    headerOverrides: {},
    rounding: {
      manualApplied: false,
      message: "",
    },
  };
}

function snapshotManualProcessingWorkspace(){
  return {
    processing: cloneWorkspaceStateValue(appState.processing),
    labels: cloneWorkspaceStateValue(appState.labels),
    processingState: cloneWorkspaceStateValue(processingState),
    spacerProcessingState: cloneWorkspaceStateValue(spacerProcessingState),
    labelsStatus: labelsStatusEl ? labelsStatusEl.textContent : "",
  };
}

function restoreManualProcessingWorkspace(snapshot, options = {}){
  const { labelNextIdFloor = null } = options;
  appState.processing = cloneWorkspaceStateValue(snapshot.processing);
  appState.labels = cloneWorkspaceStateValue(snapshot.labels);
  if (Number.isFinite(labelNextIdFloor)){
    appState.labels.nextId = Math.max(Number(appState.labels.nextId || 1), labelNextIdFloor);
  }
  replaceObjectContents(processingState, cloneWorkspaceStateValue(snapshot.processingState));
  replaceObjectContents(spacerProcessingState, cloneWorkspaceStateValue(snapshot.spacerProcessingState));
  if (labelsStatusEl) labelsStatusEl.textContent = snapshot.labelsStatus || "";
  updateSpacerProcessingUI();
  updateProcessingUI();
  updateLabelsUI();
}

function beginWorkspaceIsolatedProcessingState(snapshot){
  appState.processing = createEmptyProcessingState(snapshot.processing);
  appState.labels = { jobs: [], nextId: Number(snapshot.labels?.nextId || 1) };
  replaceObjectContents(processingState, { processedRows: [], previewGroups: [] });
  rebuildProcessingRows();
  updateProcessingUI();
  updateLabelsUI();
}

function rememberWorkspaceProcessingJob(job){
  if (!job) return null;
  workspaceState.processingJobs = [job, ...(workspaceState.processingJobs || []).filter(item => item.id !== job.id)].slice(0, 25);
  return job;
}

function findWorkspaceProcessingJob(jobId){
  const id = String(jobId || "");
  if (!id) return null;
  return (workspaceState.processingJobs || []).find(job => String(job.id) === id)
    || (workspaceState.moduleFiles || []).find(item => String(item.workspace_job_id || item.batch_id) === id)
    || null;
}

function findLabelJobForProcessingSection(section){
  const sectionKey = String(section?.key || section?.orderId || "").trim();
  if (!sectionKey) return null;
  return (appState.labels.jobs || []).find(job => {
    const key = String(job.sourceDetail || job.primaryOrderId || "").trim();
    return key === sectionKey || (Array.isArray(job.orders) && job.orders.includes(sectionKey));
  }) || null;
}

async function processValidatedOrdersViaExistingModules(orders, mode = "combined", options = {}){
  if (!orders.length){
    throw new Error("No orders selected for processing.");
  }
  const orderNumbers = orders.map(getOrderLabel);
  const steps = [];
  const manualSnapshot = snapshotManualProcessingWorkspace();
  let tempLabelNextId = Number(manualSnapshot.labels?.nextId || 1);
  const mergeAcrossOrders = mode === "combined" && !!options.mergeAcrossOrders;
  try{
    beginWorkspaceIsolatedProcessingState(manualSnapshot);
    appState.processing.options.mergeAcrossOrders = mergeAcrossOrders;
    (orders || []).forEach(order => addOrderToProcessing(order));
    steps.push(orders.length > 1 ? `Added ${orders.length} orders to one isolated Workspace Processing job` : "Added to isolated Workspace Processing job");
    if (orders.length > 1){
      steps.push(mergeAcrossOrders ? "Merge across orders enabled" : "Kept order boundaries separate");
    }

    const processingSheetId = orders.length > 1 ? getProcessingSheetIdForOrders(orders) : getProcessingSheetIdForOrder(orders[0]);
    applyDankoRounding(processingSheetId);
    steps.push("Applied Danko rounding");
    groupProcessingDimensions(processingSheetId);
    steps.push("Grouped dimensions");

    const previewText = appState.processing.preview?.text || "";
    const processingStateSnapshot = cloneWorkspaceStateValue(appState.processing);
    const processingBlob = await buildProcessingPdfBlob();
    const processingUrl = createWorkspaceDownloadUrl(processingBlob);
    steps.push(orders.length > 1 ? "Exported combined Processing PDF" : "Exported Processing PDF");

    const labelResult = addLabelJobsFromProcessing({ silent: true });
    const sections = collectProcessingSectionsForLabels();
    const labelJobs = sections.map(findLabelJobForProcessingSection).filter(Boolean);
    const missingLabelSections = sections.filter(section => !findLabelJobForProcessingSection(section));
    if (!labelJobs.length || missingLabelSections.length){
      const skipped = labelResult.skipped?.length ? ` Skipped: ${labelResult.skipped.join(", ")}` : "";
      throw new Error(`Labels could not be created from Processing.${skipped}`);
    }
    const labelRows = labelJobs.reduce((acc, job) => {
      if (Array.isArray(job.rows)) acc.push(...job.rows);
      return acc;
    }, []);
    tempLabelNextId = Number(appState.labels.nextId || tempLabelNextId);
    const labelJobsSnapshot = cloneWorkspaceStateValue(labelJobs);
    steps.push("Added labels from Processing");

    const labelsBlob = await buildLabelsPdfBlob(labelRows);
    const labelsUrl = createWorkspaceDownloadUrl(labelsBlob);
    steps.push(orders.length > 1 ? "Exported combined Labels PDF" : "Exported Labels PDF");

    const clients = Array.from(new Set(orders.map(order => getClientName(order, "")).filter(Boolean)));
    const summary = summarizeWorkspaceOrders(orders);
    const workspaceJobId = `workspace-job-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const generatedAt = new Date().toISOString();
    const workspaceJob = rememberWorkspaceProcessingJob({
      id: workspaceJobId,
      source: "workspace_agent",
      status: "ready",
      mode: orders.length > 1 ? mode : "single",
      mergeAcrossOrders,
      merge_across_orders: mergeAcrossOrders,
      processing_sheet_id: processingSheetId,
      label_job_id: labelJobsSnapshot.map(job => job.id).join(","),
      label_job_ids: labelJobsSnapshot.map(job => job.id),
      order_id: orders[0].id,
      order_ids: orders.map(order => order.id).filter(Boolean),
      order_number: orderNumbers.join(", "),
      order_numbers: orderNumbers,
      client_name: clients.length === 1 ? clients[0] : (clients.length ? "(Mixed)" : ""),
      client_names: clients,
      created_at: generatedAt,
      updated_at: generatedAt,
      options_state: cloneWorkspaceStateValue(processingStateSnapshot.options || {}),
      header_state: cloneWorkspaceStateValue(processingStateSnapshot.headerOverrides || {}),
      raw_processing_rows: cloneWorkspaceStateValue(processingStateSnapshot.rows || []),
      rounded_rows: cloneWorkspaceStateValue(processingStateSnapshot.rows || []),
      grouped_rows: cloneWorkspaceStateValue(processingStateSnapshot.preview?.groups || []),
      preview_text: previewText,
      processing_state: processingStateSnapshot,
      label_jobs: labelJobsSnapshot,
      label_rows: cloneWorkspaceStateValue(labelRows),
      processing_pdf_url: processingUrl,
      labels_pdf_url: labelsUrl,
      summary,
    });
    const record = {
      module_workflow: true,
      source: "workspace_agent",
      workspace_job_id: workspaceJobId,
      batch_id: processingSheetId,
      order_id: orders[0].id,
      order_ids: orders.map(order => order.id).filter(Boolean),
      order_number: orderNumbers.join(", "),
      order_numbers: orderNumbers,
      client_name: clients.length === 1 ? clients[0] : (clients.length ? "(Mixed)" : ""),
      generated_at: generatedAt,
      batch_status: "workspace_job_ready",
      mode: orders.length > 1 ? mode : "single",
      mergeAcrossOrders,
      merge_across_orders: mergeAcrossOrders,
      processing_pdf_url: processingUrl,
      labels_pdf_url: labelsUrl,
      preview_text: previewText,
      label_job_id: labelJobsSnapshot.map(job => job.id).join(","),
      label_job_ids: labelJobsSnapshot.map(job => job.id),
    };
    rememberWorkspaceModuleFile(record);
    return {
      mode: orders.length > 1 ? mode : "single",
      mergeAcrossOrders,
      orders,
      order: orders[0],
      orderNumber: orderNumbers[0],
      orderNumbers,
      steps,
      summary,
      files: {
        processing_pdf_url: processingUrl,
        labels_pdf_url: labelsUrl,
      },
      processingSheetId,
      workspaceJobId,
      workspaceJob,
      labelJob: labelJobsSnapshot[0],
      labelJobs: labelJobsSnapshot,
      previewText,
      isolated: true,
      warningNotes: getWorkspaceWarningNotes(orders),
    };
  }finally{
    restoreManualProcessingWorkspace(manualSnapshot, { labelNextIdFloor: tempLabelNextId });
  }
}

async function processOrdersViaExistingModules(identifiers, mode = "combined", options = {}){
  const orders = await resolveWorkspaceOrders(identifiers);
  validateWorkspaceOrdersForProcessing(orders, options);
  const normalizedMode = mode === "separate" ? "separate" : (orders.length > 1 ? "combined" : "single");
  const mergeAcrossOrders = normalizedMode === "combined" && !!options.mergeAcrossOrders;
  if (normalizedMode === "separate" && orders.length > 1){
    const results = [];
    for (const order of orders){
      results.push(await processValidatedOrdersViaExistingModules([order], "single", { ...options, mergeAcrossOrders: false }));
    }
    return { mode: "separate", mergeAcrossOrders: false, results, orderNumbers: orders.map(getOrderLabel), files: results[0]?.files || {}, steps: [] };
  }
  return processValidatedOrdersViaExistingModules(orders, normalizedMode, { ...options, mergeAcrossOrders });
}

async function processOrderViaExistingModules(identifier){
  const result = await processOrdersViaExistingModules([identifier], "single");
  const order = result.order;
  const orderNumber = getOrderLabel(order);
  return { ...result, orderNumber };
}

function formatWorkspaceProcessResultMessage(result){
  const warningNotes = Array.isArray(result?.warningNotes) && result.warningNotes.length
    ? `\n\nNote:\n${result.warningNotes.map(item => `${item.orderNumber} had ${item.count} previous warning${item.count === 1 ? "" : "s"}, but it was already approved, so I continued.`).join("\n")}`
    : "";
  if (result?.mode === "combined"){
    const summary = result.summary || {};
    const mergeText = result.mergeAcrossOrders
      ? "Processed together with merge across orders enabled."
      : "Processed together, keeping orders separate.";
    return `Done ✅
${mergeText}

Your manual Processing tab was not changed.

Mode: Combined
Merge across orders: ${result.mergeAcrossOrders ? "Yes" : "No"}

Orders:
${result.orderNumbers.map((number, idx) => {
  const client = getClientName(result.orders[idx], "");
  return `- ${number}${client ? ` — ${client}` : ""}`;
}).join("\n")}

Summary:
- Total pieces: ${summary.total_pieces || 0}
- Total area: ${Number(summary.total_area_m2 || 0).toFixed(3)} m²
- Glass types: ${summary.glass_type_count || 0}
${warningNotes}

Steps completed:
${result.steps.map(step => `✓ ${step}`).join("\n")}`;
  }
  if (result?.mode === "separate"){
    return `Done ✅
Processed ${result.results.length} orders separately in isolated Workspace processing jobs.

Your manual Processing tab was not changed.

Orders:
${result.results.map(item => `- ${item.orderNumber}`).join("\n")}
${(result.results || []).flatMap(item => item.warningNotes || []).length ? `\nNote:\n${(result.results || []).flatMap(item => item.warningNotes || []).map(item => `${item.orderNumber} had ${item.count} previous warning${item.count === 1 ? "" : "s"}, but it was already approved, so I continued.`).join("\n")}` : ""}

Each order has its own Processing PDF and Labels PDF in Recent Production Files.`;
  }
  return `Done ✅
Order ${result.orderNumber} was processed in a separate Workspace processing job.

Your manual Processing tab was not changed.
${warningNotes}

Steps completed:
${result.steps.map(step => `✓ ${step}`).join("\n")}`;
}

function rememberWorkspaceLastAgentAction(result){
  if (!result) return;
  const latestFile = (workspaceState.moduleFiles || [])[0] || null;
  workspaceState.lastAgentAction = {
    type: result.mode === "combined" ? "process_orders_combined" : (result.mode === "separate" ? "process_orders_separate" : "process_order"),
    status: "success",
    order_numbers: result.orderNumbers || (result.orderNumber ? [result.orderNumber] : []),
    mode: result.mode === "single" ? null : (result.mode || null),
    mergeAcrossOrders: result.mode === "combined" ? !!result.mergeAcrossOrders : false,
    processing_sheet_id: result.processingSheetId || latestFile?.batch_id || null,
    label_job_id: result.labelJob?.id || result.label_job_id || latestFile?.label_job_id || null,
    workspace_job_id: result.workspaceJobId || latestFile?.workspace_job_id || null,
    files: result.files || (latestFile ? {
      processing_pdf_url: latestFile.processing_pdf_url,
      labels_pdf_url: latestFile.labels_pdf_url,
    } : null),
  };
}

function showWorkspaceLastFiles(){
  const latest = workspaceState.lastAgentAction?.files ? workspaceState.lastAgentAction : null;
  const fallback = (workspaceState.moduleFiles || [])[0] || null;
  const files = latest?.files || (fallback ? {
    processing_pdf_url: fallback.processing_pdf_url,
    labels_pdf_url: fallback.labels_pdf_url,
  } : null);
  const orderNumbers = latest?.order_numbers?.length ? latest.order_numbers.join(", ") : (fallback?.order_number || "latest order");
  if (!files){
    throw new Error("No production files have been generated in this Workspace session.");
  }
  appendWorkspaceMessage("assistant", `Labels are ready for ${orderNumbers}.`);
  renderAssistantFiles(files, {
    openProcessing: true,
    openLabels: true,
    workspaceJobId: latest?.workspace_job_id || fallback?.workspace_job_id || null,
  });
}

function isWorkspaceContinueMessage(message){
  const lower = String(message || "").trim().toLowerCase();
  return ["continue", "continue anyway", "yes continue", "proceed", "go ahead", "do it", "yes", "ok continue"].includes(lower);
}

async function handleWorkspaceAgentResponse(response){
  if (response?.pending_action){
    workspaceState.latestPendingAction = response.pending_action;
    workspaceState.lastAgentAction = {
      type: response.pending_action.type || "process_orders",
      status: "blocked",
      order_numbers: response.pending_action.order_numbers || [],
      mode: response.pending_action.mode || null,
      processing_sheet_id: null,
      label_job_id: null,
      files: null,
    };
    appendWorkspaceMessage("assistant", `${response.message || "I found warnings in one or more orders and stopped before creating production files."}\n${(response.warnings || []).slice(0, 8).map(item => `- ${item.order_number}: ${item.message || item}`).join("\n")}`);
    renderWorkspacePendingAction(response.pending_action);
    return true;
  }
  const frontendAction = (response.actions || []).find(action => action.type === "process_via_existing_modules");
  if (frontendAction){
    const identifiers = frontendAction.identifiers || frontendAction.order_numbers || [frontendAction.identifier];
    const result = await processOrdersViaExistingModules(identifiers, frontendAction.mode || "single", {
      force: !!frontendAction.force,
      confirmedPendingActionId: frontendAction.confirmed_pending_action_id || null,
      mergeAcrossOrders: !!frontendAction.mergeAcrossOrders,
    });
    workspaceState.latestPendingAction = null;
    rememberWorkspaceLastAgentAction(result);
    appendWorkspaceMessage("assistant", formatWorkspaceProcessResultMessage(result));
    renderAssistantFiles(result.files, {
      openProcessing: true,
      openLabels: true,
      workspaceJobId: result.workspaceJobId || result.results?.[0]?.workspaceJobId || workspaceState.lastAgentAction?.workspace_job_id || null,
    });
    await refreshWorkspaceAfterAction();
    return true;
  }
  const contextAction = (response.actions || []).find(action => action.type === "show_last_agent_files" || action.type === "open_last_processing_sheet" || action.type === "open_last_label_job" || action.type === "open_review");
  if (contextAction?.type === "show_last_agent_files"){
    showWorkspaceLastFiles();
    return true;
  }
  if (contextAction?.type === "open_last_processing_sheet"){
    openWorkspaceProcessingJob(workspaceState.lastAgentAction?.workspace_job_id || null);
    appendWorkspaceMessage("assistant", response.message || "Opened the Processing sheet.");
    return true;
  }
  if (contextAction?.type === "open_last_label_job"){
    openWorkspaceProcessingJob(workspaceState.lastAgentAction?.workspace_job_id || null);
    appendWorkspaceMessage("assistant", response.message || "Opened the label job.");
    return true;
  }
	  if (contextAction?.type === "open_review"){
	    const firstId = (contextAction.order_ids || [])[0];
	    if (firstId) await openSharedOrderDetail(firstId, { activateHistoryTab: true });
	    appendWorkspaceMessage("assistant", response.message || "Opened review.");
	    return true;
	  }
  return false;
}

async function runWorkspaceCommand(message){
  const text = String(message || "").trim();
  if (!text || workspaceState.assistantLoading) return;
  appendWorkspaceMessage("user", text);
  renderAssistantFiles({});
  setWorkspaceBusy(true, "Working...");
  try{
    const lowerText = text.toLowerCase();
    if (isWorkspaceContinueMessage(text)){
      const response = workspaceState.latestPendingAction
        ? await postWorkspaceConfirmAction("continue")
        : await postWorkspaceAgent(text);
      if (await handleWorkspaceAgentResponse(response)) return;
      appendWorkspaceMessage("assistant", response.message || "Done.");
      return;
    }
    if (((lowerText.includes("latest processed") || lowerText.includes("latest labels")) && (lowerText.includes("label") || lowerText.includes("download")))
      || (lowerText.includes("download") && lowerText.includes("labels") && lowerText.includes("again"))){
      showWorkspaceLastFiles();
      return;
    }
    if (lowerText.includes("open") && lowerText.includes("processing") && (lowerText.includes("that") || lowerText.includes("last") || lowerText.includes("sheet"))){
      openWorkspaceProcessingJob(workspaceState.lastAgentAction?.workspace_job_id || null);
      appendWorkspaceMessage("assistant", "Opened the Workspace Processing sheet.");
      return;
    }
    const processIntent = inferWorkspaceProcessIntent(text);
    if (processIntent){
      if (!workspaceState.queue){
        await loadWorkspace();
      }
      const response = await postWorkspaceAgent(text);
      if (await handleWorkspaceAgentResponse(response)) return;
      appendWorkspaceMessage("assistant", response.message || "Done.");
      return;
    }
    const response = await postWorkspaceAgent(text);
    if (await handleWorkspaceAgentResponse(response)) return;
    appendWorkspaceMessage("assistant", response.message || "Done.");
    renderAssistantFiles(response.files || {});
    if (response.status && response.status !== "help"){
      await refreshWorkspaceAfterAction();
    }
  }catch(error){
    appendWorkspaceMessage("assistant", "Workspace action failed: " + (error.message || error));
  }finally{
    setWorkspaceBusy(false, "");
  }
}

function renderSmartChatActions(actions){
  if (!workspaceSmartChatActionsEl) return;
  const safeActions = Array.isArray(actions) ? actions : [];
  const sendAction = safeActions.find(action => action && action.kind === "send_to_factory");
  if (!sendAction){
    workspaceSmartChatActionsEl.innerHTML = "";
    return;
  }
  const message = sendAction.payload?.message || "";
  workspaceSmartChatActionsEl.innerHTML = `<button type="button" class="btn small" data-smart-chat-action="send-to-factory" data-message="${escapeHtml(message)}">${escapeHtml(sendAction.label || "Send to Factory Assistant")}</button>`;
}

async function runSmartChat(message){
  const text = String(message || "").trim();
  if (!text || workspaceState.smartChatLoading) return;
  appendSmartChatMessage("user", text);
  renderSmartChatActions([]);
  setSmartChatBusy(true, "Thinking...");
  try{
    const response = await postWorkspaceSmartChat(text);
    const backendMessage = typeof response?.message === "string" ? response.message : "";
    appendSmartChatMessage("assistant", backendMessage || "Smart Chat returned no message.");
    renderSmartChatActions(response.actions || []);
  }catch(error){
    appendSmartChatMessage("assistant", "Smart Chat failed: " + (error.message || error));
  }finally{
    setSmartChatBusy(false, "");
  }
}

async function processWorkspaceOrder(identifier){
  if (!identifier || workspaceState.assistantLoading) return;
  appendWorkspaceMessage("user", `Process order ${identifier}`);
  renderAssistantFiles({});
  setWorkspaceBusy(true, "Processing order...");
  try{
    const response = await postWorkspaceAgent(`Process order ${identifier}`);
    if (!(await handleWorkspaceAgentResponse(response))){
      appendWorkspaceMessage("assistant", response.message || "Done.");
    }
  }catch(error){
    appendWorkspaceMessage("assistant", "Processing failed: " + (error.message || error));
  }finally{
    setWorkspaceBusy(false, "");
  }
}

if (workspaceRefreshBtn){
  workspaceRefreshBtn.addEventListener("click", ()=> loadWorkspace());
}
if (awaRefreshBtn){
  awaRefreshBtn.addEventListener("click", ()=> loadAwa());
}
if (awaSuggestionsEl){
  awaSuggestionsEl.addEventListener("click", event => {
    const btn = event.target.closest("[data-awa-action]");
    if (!btn) return;
    const card = btn.closest("[data-awa-action-id]");
    const actionId = card?.dataset.awaActionId || "";
    const action = btn.dataset.awaAction;
    if (!actionId) return;
    if (action === "approve" || action === "reject"){
      if (action === "approve" && !confirm("Approve this AWA Beta suggestion? This records approval only; no production workflow will run yet.")){
        return;
      }
      runAwaAction(actionId, action);
    }else if (action === "explain"){
      explainAwaAction(actionId);
    }else if (action === "open"){
      const suggestion = findAwaSuggestion(actionId);
      activateTab(awaRelatedModule(suggestion));
    }
  });
}
if (awaChatForm){
  awaChatForm.addEventListener("submit", event => {
    event.preventDefault();
    const message = awaChatInput ? awaChatInput.value : "";
    if (awaChatInput) awaChatInput.value = "";
    askAwa(message);
  });
}
if (workspaceProcessSelectedBtn){
  workspaceProcessSelectedBtn.addEventListener("click", ()=> {
    const count = workspaceState.selectedOrderIds.size;
    if (!count) return;
    runWorkspaceCommand(count > 1 ? "Process selected orders together" : "Process selected order");
  });
}
if (workspaceClearSelectionBtn){
  workspaceClearSelectionBtn.addEventListener("click", ()=> {
    workspaceState.selectedOrderIds.clear();
    workspaceState.selectedOrderNumbers.clear();
    workspaceState.selectedOrderId = "";
    workspaceState.selectedOrderNumber = "";
    updateWorkspaceSelectionUI();
  });
}
if (workspaceCommandForm){
  workspaceCommandForm.addEventListener("submit", event => {
    event.preventDefault();
    const message = workspaceCommandInput ? workspaceCommandInput.value : "";
    if (workspaceCommandInput) workspaceCommandInput.value = "";
    runWorkspaceCommand(message);
  });
}
if (workspacePromptButtons){
  workspacePromptButtons.addEventListener("click", event => {
    const btn = event.target.closest("[data-workspace-prompt]");
    if (!btn) return;
    runWorkspaceCommand(btn.dataset.workspacePrompt || btn.textContent || "");
  });
}
if (workspaceSmartChatForm){
  workspaceSmartChatForm.addEventListener("submit", event => {
    event.preventDefault();
    const message = workspaceSmartChatInput ? workspaceSmartChatInput.value : "";
    if (workspaceSmartChatInput) workspaceSmartChatInput.value = "";
    runSmartChat(message);
  });
}
if (workspaceSmartPromptButtons){
  workspaceSmartPromptButtons.addEventListener("click", event => {
    const btn = event.target.closest("[data-smart-chat-prompt]");
    if (!btn) return;
    runSmartChat(btn.dataset.smartChatPrompt || btn.textContent || "");
  });
}
if (workspaceSmartChatActionsEl){
  workspaceSmartChatActionsEl.addEventListener("click", event => {
    const btn = event.target.closest("[data-smart-chat-action='send-to-factory']");
    if (!btn) return;
    const message = btn.dataset.message || "";
    if (workspaceCommandInput){
      workspaceCommandInput.value = message;
      workspaceCommandInput.focus();
    }
    if (workspaceAssistantStatusEl){
      workspaceAssistantStatusEl.textContent = "Copied from Smart Chat. Press Send to run it.";
    }
  });
}
if (workspaceAssistantFilesEl){
  workspaceAssistantFilesEl.addEventListener("click", event => {
    const pendingBtn = event.target.closest("[data-workspace-pending-decision]");
    if (pendingBtn){
      const decision = pendingBtn.dataset.workspacePendingDecision;
      if (decision === "cancel"){
        postWorkspaceConfirmAction("cancel").then(response => {
          workspaceState.latestPendingAction = null;
          renderAssistantFiles({});
          appendWorkspaceMessage("assistant", response.message || "Cancelled. No production files were created.");
        }).catch(error => appendWorkspaceMessage("assistant", "Workspace action failed: " + (error.message || error)));
      }else if (decision === "open_review"){
        postWorkspaceConfirmAction("open_review").then(response => handleWorkspaceAgentResponse(response)).catch(error => appendWorkspaceMessage("assistant", "Workspace action failed: " + (error.message || error)));
      }else{
        runWorkspaceCommand("continue");
      }
      return;
    }
    const btn = event.target.closest("[data-workspace-open]");
    if (!btn) return;
    const target = btn.dataset.workspaceOpen;
    if (target === "workspace-processing" || target === "workspace-labels"){
      openWorkspaceProcessingJob(btn.dataset.workspaceJobId || workspaceState.lastAgentAction?.workspace_job_id || null);
    }else if (target === "processing"){
      activateTab("processing");
    }else if (target === "labels"){
      activateTab("labels");
    }
  });
}
document.addEventListener("click", event => {
  const btn = event.target.closest("[data-workspace-job-action]");
  if (!btn) return;
  const action = btn.dataset.workspaceJobAction;
  if (action === "close"){
    hideWorkspaceProcessingJobView();
    return;
  }
  if (action === "copy"){
    const job = findWorkspaceProcessingJob(btn.dataset.workspaceJobId);
    const text = job?.preview_text || job?.processing_state?.preview?.text || "";
    if (!text){
      setStatusMessage("No Workspace job text to copy.");
      return;
    }
    navigator.clipboard.writeText(text)
      .then(() => setStatusMessage("Workspace Processing text copied."))
      .catch(() => setStatusMessage("Copy failed."));
  }
});
if (workspaceRecentFilesEl){
  workspaceRecentFilesEl.addEventListener("click", event => {
    const btn = event.target.closest("[data-workspace-open]");
    if (!btn) return;
    const target = btn.dataset.workspaceOpen;
    if (target === "workspace-processing" || target === "workspace-labels"){
      openWorkspaceProcessingJob(btn.dataset.workspaceJobId || null);
    }
  });
}
if (workspaceQueueEl){
  workspaceQueueEl.addEventListener("click", async event => {
    const card = event.target.closest(".workspace-order-card");
    if (card){
      workspaceState.selectedOrderId = card.dataset.workspaceOrderId || "";
      workspaceState.selectedOrderNumber = card.dataset.workspaceOrderNumber || "";
    }
    const btn = event.target.closest("[data-workspace-action]");
    if (!btn) return;
    const action = btn.dataset.workspaceAction;
    const id = btn.dataset.id || workspaceState.selectedOrderId;
    const orderNumber = btn.dataset.order || workspaceState.selectedOrderNumber || id;
	    if (action === "select"){
	      toggleWorkspaceOrderSelection(id, orderNumber);
	    }else if (action === "open"){
	      await openSharedOrderDetail(id, { activateHistoryTab: true });
    }else if (action === "check"){
      await runWorkspaceCommand(`Check order ${orderNumber}`);
    }else if (action === "process"){
      await processWorkspaceOrder(id || orderNumber);
    }else if (action === "labels"){
      await runWorkspaceCommand(`Generate labels for order ${orderNumber}`);
    }
  });
}

if (telegramFilesRefreshBtn){
  telegramFilesRefreshBtn.addEventListener("click", ()=> loadTelegramFiles());
}
let telegramFilesSearchTimer = null;
if (telegramFilesSearchInput){
  telegramFilesSearchInput.addEventListener("input", event => {
    telegramFilesState.query = (event.target.value || "").trim();
    clearTimeout(telegramFilesSearchTimer);
    telegramFilesSearchTimer = setTimeout(()=> loadTelegramFiles(), 250);
  });
}
if (telegramFilesStatusFilter){
  telegramFilesStatusFilter.addEventListener("change", event => {
    telegramFilesState.status = event.target.value || "";
    loadTelegramFiles();
  });
}
if (telegramFilesTouchedFilter){
  telegramFilesTouchedFilter.addEventListener("change", event => {
    telegramFilesState.touched = event.target.value || "false";
    loadTelegramFiles();
  });
}
if (telegramFilesListEl){
  telegramFilesListEl.addEventListener("click", async event => {
    const actionBtn = event.target.closest("[data-telegram-action]");
    if (!actionBtn) return;
    const action = actionBtn.dataset.telegramAction;
    const file = getTelegramFileById(actionBtn.dataset.id || actionBtn.closest("[data-telegram-file-id]")?.dataset.telegramFileId);
    if (action === "order"){
      const orderId = actionBtn.dataset.orderId;
      if (orderId){
        const openedOrder = await openSharedOrderDetail(orderId, { activateHistoryTab: true });
        if (openedOrder && file?.linked_order_id && String(file.linked_order_id) === String(orderId)){
          try{
            await markTelegramFileHandlingStep(file.id, "order");
          }catch(error){
            console.warn("Telegram linked order open marker failed", error);
            setTelegramFilesStatus("Linked order opened, but handling progress was not saved.");
          }
        }
      }
      return;
    }
    if (action === "view"){
      openTelegramPdf(file || {
        id: actionBtn.dataset.id,
        original_filename: "Telegram original",
        view_url: `/telegram-files/${encodeURIComponent(actionBtn.dataset.id)}/view`,
        download_url: `/telegram-files/${encodeURIComponent(actionBtn.dataset.id)}/download`,
      });
      return;
    }
    if (!file) return;
    if (action === "print"){
      printTelegramOriginalPdf(file);
    }else if (action === "touch"){
      markTelegramFileTouched(file.id);
    }else if (action === "labels"){
      printTelegramLinkedLabels(file);
    }else if (action === "delete"){
      deleteTelegramFile(file);
    }
  });
}
if (telegramPdfCloseBtn){
  telegramPdfCloseBtn.addEventListener("click", closeTelegramPdf);
}
if (telegramPdfModal){
  telegramPdfModal.addEventListener("click", event => {
    if (event.target === telegramPdfModal) closeTelegramPdf();
  });
}
if (telegramPdfPrintBtn){
  telegramPdfPrintBtn.addEventListener("click", () => {
    const file = telegramFilesState.selectedFile;
    printTelegramOriginalPdf(file);
  });
}

async function fetchAllApprovedOrdersForInvoicing(){
  const all = [];
  let offset = 0;
  const limit = 100;
  let hasMore = true;
  while (hasMore){
    const params = {
      limit,
      offset,
      status: "approved",
      year: normalizeHistoryYear(historyState.year || "all"),
      query: historyState.query || undefined,
      client: historyState.client || undefined,
      date_from: historyState.dateFrom || undefined,
      date_to: historyState.dateTo || undefined,
    };
    const data = await fetchOrders(params);
    const items = Array.isArray(data?.items) ? data.items : [];
    all.push(...items);
    hasMore = !!data?.has_more && items.length > 0;
    offset += limit;
  }
  return all;
}

async function loadHistory(opts={}){
  if (historyState.queue && !opts.ignoreQueue){
    if (!Object.keys(historyState.queueItems || {}).length) await loadHistoryWorkQueue();
    applyHistoryQueue(historyState.queue);
    return;
  }
  const resetOffset = !!opts.resetOffset;
  if (resetOffset) historyState.offset = 0;
  historyState.loading = true;
  historyState.error = "";
  setHistoryStatus("Loading history…");
  updateHistoryOfflineBadge();
  historyPageEl.textContent = String(Math.floor(historyState.offset / historyState.limit) + 1);
  renderOrdersList();

  try{
    const data = await fetchOrders({
      query: historyState.query || undefined,
      client: historyState.client || undefined,
      status: historyState.status || undefined,
      date_from: historyState.dateFrom || undefined,
      date_to: historyState.dateTo || undefined,
      approved_only: historyState.approvedOnly ? "true" : undefined,
      year: normalizeHistoryYear(historyState.year || "all"),
      limit: historyState.limit,
      offset: historyState.offset
    });
    historyState.items = data.items || [];
    historyState.hasMore = !!data.has_more;
    historyState.offline = false;
    historyState.loadedOnce = true;
    historyState.needsRefresh = false;
    updateHistoryCache(historyState.items);
    analysisState.allOrdersDirty = true;
    renderOrdersList();
    updateHistoryOfflineBadge();
    setHistoryStatus(historyState.items.length ? "" : "No saved orders yet.");
  }catch(error){
    const cached = readHistoryCache();
    if (cached.length){
      let filtered = cached;
      if (historyState.status){
        filtered = filtered.filter(item => normalizeHistoryStatusValue(item.status) === normalizeHistoryStatusValue(historyState.status));
      }
      if (historyState.query){
        const term = historyState.query.trim().toLowerCase();
        filtered = filtered.filter(item => {
          const client = getClientName(item, "").toLowerCase();
          const nums = String((item.order_numbers || []).join(", ") || "").toLowerCase();
          return client.includes(term) || nums.includes(term);
        });
      }
      if (historyState.client){
        const term = historyState.client.trim().toLowerCase();
        filtered = filtered.filter(item => getClientName(item, "").toLowerCase().includes(term));
      }
      if (historyState.approvedOnly){
        filtered = filtered.filter(item => normalizeHistoryStatusValue(item.status) === "approved");
      }
      if (historyState.dateFrom){
        const fromTs = new Date(`${historyState.dateFrom}T00:00:00`).getTime();
        filtered = filtered.filter(item => new Date(item.created_at || 0).getTime() >= fromTs);
      }
      if (historyState.dateTo){
        const toTs = new Date(`${historyState.dateTo}T23:59:59`).getTime();
        filtered = filtered.filter(item => new Date(item.created_at || 0).getTime() <= toTs);
      }
      historyState.items = filtered;
      historyState.hasMore = false;
      historyState.offline = true;
      historyState.loadedOnce = true;
      historyState.needsRefresh = false;
      renderOrdersList();
      updateHistoryOfflineBadge();
      setHistoryStatus("Offline mode — showing cached results.");
      analysisState.allOrdersDirty = true;
    }else{
      historyState.items = [];
      historyState.error = "Unable to load history: " + (error.message || error);
      renderOrdersList();
      setHistoryStatus(historyState.error);
    }
  }finally{
    historyState.loading = false;
    historyPageEl.textContent = String(Math.floor(historyState.offset / historyState.limit) + 1);
    renderOrdersList();
    updateHistoryControls();
  }
}

async function handleHistoryInvoiceAllClick(){
  const rangeParts = [];
  if (historyState.dateFrom) rangeParts.push(`from ${historyState.dateFrom}`);
  if (historyState.dateTo) rangeParts.push(`to ${historyState.dateTo}`);
  const scopeLabel = rangeParts.length ? rangeParts.join(" ") : "current filter scope";
  if (!confirm(`Create invoice jobs for all APPROVED orders (${scopeLabel})?`)){
    return;
  }
  try{
    setHistoryStatus("Creating invoices for all approved orders…");
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const approvedOrders = await fetchAllApprovedOrdersForInvoicing();
    let created = 0;
    let skipped = 0;
    for (const order of approvedOrders){
      if (!order || order.id == null){
        continue;
      }
      if (isOrderAlreadyInvoiced(order)){
        skipped += 1;
        continue;
      }
      const job = await createInvoiceJobFromOrder(order);
      if (job){
        created += 1;
      }
    }
    updateInvoicesUI();
    setHistoryStatus(`Created ${created} invoice jobs, skipped ${skipped} already invoiced orders.`);
  }catch(error){
    console.error("Invoice all error", error);
    setHistoryStatus("Error creating invoices for all orders.");
  }
}

function updateHistoryControls(){
  document.getElementById("historyPrev").disabled = historyState.offset <= 0 || historyState.loading;
  document.getElementById("historyNext").disabled = historyState.loading || !historyState.hasMore;
}

const HISTORY_STATUS_LABELS = {
  draft: "Draft",
  reviewed: "Reviewed",
  approved: "Approved",
  in_production: "In Production",
  completed: "Completed",
  archived: "Archived",
};

function normalizeHistoryStatusValue(status){
  const value = String(status || "").trim().toLowerCase().replace(/[\s-]+/g, "_");
  if (!value) return "draft";
  if (value === "inproduction") return "in_production";
  return value;
}

function historyStatusLabel(status){
  const normalized = normalizeHistoryStatusValue(status);
  return HISTORY_STATUS_LABELS[normalized] || HISTORY_STATUS_LABELS.draft;
}

function historyStatusBadgeHtml(status){
  const normalized = normalizeHistoryStatusValue(status);
  return `<span class="history-status-badge ${escapeHtml(normalized)}">${escapeHtml(historyStatusLabel(normalized))}</span>`;
}

function canEditDraftStatus(status){
  const normalized = normalizeHistoryStatusValue(status);
  return normalized === "draft" || normalized === "reviewed";
}

function canApproveStatus(status){
  const normalized = normalizeHistoryStatusValue(status);
  return normalized === "draft" || normalized === "reviewed";
}

function canProcessingStatus(status){
  return ["approved", "in_production", "completed"].includes(normalizeHistoryStatusValue(status));
}

function canInvoiceStatus(status){
  return ["approved", "in_production", "completed"].includes(normalizeHistoryStatusValue(status));
}

function canArchiveStatus(status){
  return normalizeHistoryStatusValue(status) !== "archived";
}

function canCsvStatus(status){
  return ["approved", "in_production", "completed"].includes(normalizeHistoryStatusValue(status));
}

function closeHistoryActionMenus(){
  document.querySelectorAll(".history-row-actions[open]").forEach(menu => {
    menu.open = false;
  });
}

function openHistoryDrawer(){
  if (historyDrawerLayer) historyDrawerLayer.hidden = false;
  activateTab("orderdetail");
}

function closeHistoryDrawer(){
  if (panels.orderdetail?.classList.contains("active")){
    activateTab("history");
  }
}

const HISTORY_QUEUE_LABELS = {
  new: "New",
  needs_review: "Needs review",
  ready_approval: "Ready for approval",
  ready_production: "Ready for production",
  failed: "Failed",
  completed: "Completed",
};

function normalizeQueueOrder(item){
  return {
    ...item,
    id: item.id ?? item.order_id,
    order_numbers: Array.isArray(item.order_numbers) && item.order_numbers.length
      ? item.order_numbers
      : (item.order_number ? [item.order_number] : []),
    units_total: Number(item.units_total ?? item.total_pieces ?? 0),
    area_total: Number(item.area_total ?? item.total_area_m2 ?? 0),
    warnings_count: Number(item.warnings_count || 0),
  };
}

function queueDraftBucket(item){
  const status = normalizeHistoryStatusValue(item.status);
  if (status === "reviewed") return "ready_approval";
  const warnings = Number(item.warnings_count || 0);
  if (warnings > 0) return "needs_review";
  const created = new Date(item.created_at || 0).getTime();
  const ageHours = created ? (Date.now() - created) / 3_600_000 : Infinity;
  return ageHours <= 48 ? "new" : "ready_approval";
}

function renderHistoryQueue(){
  const idByQueue = {
    new: "historyQueueNew",
    needs_review: "historyQueueNeedsReview",
    ready_approval: "historyQueueReadyApproval",
    ready_production: "historyQueueReadyProduction",
    failed: "historyQueueFailed",
    completed: "historyQueueCompleted",
  };
  Object.entries(idByQueue).forEach(([queue, id]) => {
    const element = document.getElementById(id);
    if (element) element.textContent = historyState.queueLoading ? "…" : String(historyState.queueCounts[queue] ?? 0);
  });
  document.querySelectorAll("[data-history-queue]").forEach(button => {
    button.classList.toggle("active", button.dataset.historyQueue === historyState.queue);
  });
  if (historyQueueActiveLabel){
    historyQueueActiveLabel.textContent = historyState.queue
      ? HISTORY_QUEUE_LABELS[historyState.queue] || "Queue"
      : "All orders";
  }
}

function filterHistoryQueueItems(items){
  let filtered = (items || []).slice();
  if (historyState.query){
    const term = historyState.query.toLowerCase();
    filtered = filtered.filter(item => {
      const orderText = (item.order_numbers || []).join(", ").toLowerCase();
      return orderText.includes(term) || getClientName(item, "").toLowerCase().includes(term);
    });
  }
  if (historyState.client){
    const term = historyState.client.toLowerCase();
    filtered = filtered.filter(item => getClientName(item, "").toLowerCase().includes(term));
  }
  if (historyState.status){
    filtered = filtered.filter(item => normalizeHistoryStatusValue(item.status) === normalizeHistoryStatusValue(historyState.status));
  }
  if (historyState.approvedOnly){
    filtered = filtered.filter(item => normalizeHistoryStatusValue(item.status) === "approved");
  }
  if (historyState.dateFrom){
    const from = new Date(`${historyState.dateFrom}T00:00:00`).getTime();
    filtered = filtered.filter(item => new Date(item.created_at || 0).getTime() >= from);
  }
  if (historyState.dateTo){
    const to = new Date(`${historyState.dateTo}T23:59:59`).getTime();
    filtered = filtered.filter(item => new Date(item.created_at || 0).getTime() <= to);
  }
  return filtered;
}

function applyHistoryQueue(queue){
  if (!queue) return;
  historyState.queue = queue;
  historyState.items = filterHistoryQueueItems(historyState.queueItems[queue] || []);
  historyState.offset = 0;
  historyState.hasMore = false;
  historyState.error = "";
  historyState.loading = false;
  historyPageEl.textContent = "1";
  renderHistoryQueue();
  renderOrdersList();
  updateHistoryControls();
  setHistoryStatus(historyState.items.length ? "" : `No orders in ${HISTORY_QUEUE_LABELS[queue] || "this queue"}.`);
}

async function loadHistoryWorkQueue(options = {}){
  if (historyState.queueLoading) return;
  historyState.queueLoading = true;
  if (historyQueueStatus) historyQueueStatus.textContent = "Refreshing work queue…";
  renderHistoryQueue();
  try{
    const [queueResult, telegramResult] = await Promise.allSettled([
      fetchWorkspaceQueue(),
      fetchTelegramFiles(),
    ]);
    if (queueResult.status !== "fulfilled") throw queueResult.reason;
    const groups = queueResult.value?.groups || {};
    const reviewCandidates = (groups.needs_review || []).map(normalizeQueueOrder);
    const queueItems = {
      new: reviewCandidates.filter(item => queueDraftBucket(item) === "new"),
      needs_review: reviewCandidates.filter(item => queueDraftBucket(item) === "needs_review"),
      ready_approval: reviewCandidates.filter(item => queueDraftBucket(item) === "ready_approval"),
      ready_production: (groups.approved_ready || []).map(normalizeQueueOrder),
      failed: [],
      completed: (groups.finished || []).map(normalizeQueueOrder),
    };
    if (telegramResult.status === "fulfilled") applyTelegramCounts(telegramResult.value);
    historyState.queueItems = queueItems;
    historyState.queueCounts = Object.fromEntries(
      Object.entries(queueItems).map(([key, items]) => [key, items.length]),
    );
    historyState.queueCounts.failed = Number(telegramFilesState.failedCount || 0);
    workspaceState.queue = queueResult.value;
    if (historyQueueStatus){
      historyQueueStatus.textContent = `Updated ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
    }
  }catch(error){
    if (historyQueueStatus) historyQueueStatus.textContent = "Work queue is temporarily unavailable.";
    console.warn("Work queue refresh failed", error);
  }finally{
    historyState.queueLoading = false;
    renderHistoryQueue();
    if (historyState.queue && historyState.queue !== "failed") applyHistoryQueue(historyState.queue);
  }
}

function selectedHistoryOrders(){
  return Array.from(historyState.selectedIds)
    .map(id => historyState.selectedOrders.get(String(id)))
    .filter(Boolean);
}

function updateHistorySelectionUI(){
  const selected = selectedHistoryOrders();
  const count = selected.length;
  if (historySelectionCount) historySelectionCount.textContent = `${count} selected`;
  if (historyClearSelection) historyClearSelection.disabled = count === 0;
  if (historyBatchApprove){
    historyBatchApprove.disabled = !count || !selected.every(order => canApproveStatus(order.status));
  }
  if (historyBatchProduction){
    historyBatchProduction.disabled = !count || !selected.every(order => normalizeHistoryStatusValue(order.status) === "approved");
  }
  if (historyBatchArchive){
    historyBatchArchive.disabled = !count || !selected.every(order => canArchiveStatus(order.status));
  }
  document.querySelectorAll("#historyListWrap [data-history-select]").forEach(input => {
    input.checked = historyState.selectedIds.has(String(input.dataset.historySelect));
  });
  const selectAll = document.getElementById("historySelectAllVisible");
  if (selectAll){
    const visibleIds = historyState.items.map(item => String(item.id ?? item.order_id)).filter(Boolean);
    selectAll.checked = !!visibleIds.length && visibleIds.every(id => historyState.selectedIds.has(id));
    selectAll.indeterminate = visibleIds.some(id => historyState.selectedIds.has(id)) && !selectAll.checked;
  }
}

function clearHistorySelection(){
  historyState.selectedIds.clear();
  historyState.selectedOrders.clear();
  updateHistorySelectionUI();
}

	function formatHistoryConfidence(value){
	  if (value === null || value === undefined || value === "") return "—";
	  const number = Number(value);
	  if (!Number.isFinite(number)) return "—";
	  return `${Math.round(number * 100)}%`;
	}

function renderOrdersList(){
  const container = document.getElementById("historyListWrap");
  if (!container) return;
  if (historyState.loading){
    container.innerHTML = '<div class="history-list-loading"><span class="spinner"></span><span>Loading orders…</span></div>';
    return;
  }
  if (historyState.error){
    container.innerHTML = `<div class="history-empty error">${escapeHtml(historyState.error)}</div>`;
    return;
  }
  if (!historyState.items.length){
    container.innerHTML = '<div class="empty-state">No orders found for the selected filters.</div>';
    return;
  }
  const rows = historyState.items.map(order=>{
    const orderId = String(order.id ?? order.order_id ?? "");
    const normalizedStatus = normalizeHistoryStatusValue(order.status);
    const orderNumbers = (order.order_numbers || []).join(", ") || "—";
    const client = getClientName(order);
    const created = formatDate(order.created_at);
    const updated = formatDate(order.updated_at || order.created_at);
    const pieces = Number(order.units_total || 0);
    const confidence = formatHistoryConfidence(order.confidence);
    const actions = `
      <details class="history-row-actions">
        <summary class="btn small">Actions</summary>
        <div class="history-actions-menu">
          <button data-action="open" data-id="${order.id}">Open order</button>
          <button data-action="edit-draft" data-id="${order.id}" ${canEditDraftStatus(normalizedStatus) ? "" : "disabled"}>Edit draft</button>
          <button data-action="approve" data-id="${order.id}" ${canApproveStatus(normalizedStatus) ? "" : "disabled"}>Approve</button>
          <button data-action="processing" data-id="${order.id}" ${canProcessingStatus(normalizedStatus) ? "" : "disabled"}>Send to Processing</button>
          <button data-action="labels" data-id="${order.id}">Generate Labels</button>
          <button data-action="archive" data-id="${order.id}" class="warn" ${canArchiveStatus(normalizedStatus) ? "" : "disabled"}>Archive</button>
          <button data-action="delete" data-id="${order.id}" class="warn">Delete order</button>
        </div>
      </details>
    `;
    return `<tr data-id="${escapeHtml(orderId)}" class="${historyState.selectedIds.has(orderId) ? "selected" : ""}">
      <td class="history-select-cell" data-label="Select"><input type="checkbox" data-history-select="${escapeHtml(orderId)}" aria-label="Select order ${escapeHtml(orderNumbers)}" ${historyState.selectedIds.has(orderId) ? "checked" : ""}></td>
      <td class="mono" data-label="Order number">${escapeHtml(orderNumbers)}</td>
      <td data-col="client" data-label="Client">${escapeHtml(client)}</td>
      <td data-label="Date">${escapeHtml(created)}</td>
      <td data-label="Status">${historyStatusBadgeHtml(normalizedStatus)}</td>
      <td class="mono" data-label="Units">${pieces}</td>
      <td class="mono" data-label="Area">${formatArea(order.area_total || 0)}</td>
      <td data-col="confidence" data-label="Confidence">${escapeHtml(confidence)}</td>
      <td class="history-last-updated" data-col="updated" data-label="Last updated">${escapeHtml(updated)}</td>
      <td data-col="actions" data-label="Actions">${actions}</td>
    </tr>`;
  }).join("");
  container.innerHTML = `<table class="history-list-table">
    <thead>
      <tr>
        <th class="history-select-cell"><input type="checkbox" id="historySelectAllVisible" aria-label="Select all visible orders"></th>
        <th>Order number</th>
        <th>Client</th>
        <th>Date</th>
        <th>Status</th>
        <th>Total pieces</th>
        <th>Total area</th>
        <th data-col="confidence">Confidence</th>
        <th class="history-last-updated" data-col="updated">Last updated</th>
        <th data-col="actions">Actions</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>`;
  updateHistorySelectionUI();
}

async function openOrderFromList(id){
  if (!id) return null;
  openHistoryDrawer();
  historyMetaEl.textContent = "Loading order…";
  historyDetailEmpty.style.display = "none";
  historyPrintBtn.disabled = true;
  historyCsvBtn.disabled = true;
  if (historyRawTextEl) historyRawTextEl.textContent = "Loading raw extraction text…";
  if (historyStatusTimelineEl){
    historyStatusTimelineEl.className = "history-status-timeline history-empty";
    historyStatusTimelineEl.textContent = "Loading status history…";
  }
  try{
    const order = await fetchOrder(id);
    historyState.selectedOrder = order;
    renderOrderDetail();
    return order;
  }catch(error){
    setHistoryStatus("Failed to load order: " + (error.message || error));
    return null;
  }
}

async function openSharedOrderDetail(id, { activateHistoryTab = false } = {}){
  if (!id) return;
  if (activateHistoryTab){
    activateTab("history");
    await ensureHistoryLoaded();
  }
  return openOrderFromList(id);
}

function renderHistoryStatusTimeline(events){
  if (!historyStatusTimelineEl) return;
  if (!Array.isArray(events) || !events.length){
    historyStatusTimelineEl.className = "history-status-timeline history-empty";
    historyStatusTimelineEl.textContent = "No status history available.";
    return;
  }
  const html = events
    .slice()
    .sort((a, b)=> new Date(b.changed_at || 0).getTime() - new Date(a.changed_at || 0).getTime())
    .map(event => {
      const toLabel = historyStatusLabel(event.to_status || event.status);
      const fromLabel = event.from_status ? historyStatusLabel(event.from_status) : "Created";
      const when = formatDate(event.changed_at);
      const note = event.note ? `<div>${escapeHtml(event.note)}</div>` : "";
      const reason = event.reason ? `<div class="meta">Reason: ${escapeHtml(event.reason)}</div>` : "";
      return `<div class="history-status-timeline-item">
        <div><strong>${escapeHtml(fromLabel)}</strong> → <strong>${escapeHtml(toLabel)}</strong></div>
        <div class="meta">${escapeHtml(when)}</div>
        ${reason}
        ${note}
      </div>`;
    })
    .join("");
  historyStatusTimelineEl.className = "history-status-timeline";
  historyStatusTimelineEl.innerHTML = html;
}

function sourcePdfElements(kind){
  if (kind === "order"){
    return {
      container: orderDetailSourceFrame,
      canvas: orderSourceCanvas,
      label: orderSourcePageLabel,
      previous: orderSourcePrev,
      next: orderSourceNext,
    };
  }
  return {
    container: extractPdfPreview,
    canvas: extractPdfCanvas,
    label: extractPdfPageLabel,
    previous: extractPdfPrev,
    next: extractPdfNext,
  };
}

function updateSourcePdfControls(kind){
  const state = sourcePdfState[kind];
  const elements = sourcePdfElements(kind);
  if (elements.label) elements.label.textContent = `Page ${state.page || 1} of ${state.pages || 1}`;
  if (elements.previous) elements.previous.disabled = !state.document || state.page <= 1;
  if (elements.next) elements.next.disabled = !state.document || state.page >= state.pages;
}

async function renderSourcePdfPage(kind, requestedPage){
  const state = sourcePdfState[kind];
  const { container, canvas } = sourcePdfElements(kind);
  if (!state?.document || !container || !canvas || container.hidden || container.clientWidth < 40) return;
  const pageNumber = Math.max(1, Math.min(Number(requestedPage) || state.page || 1, state.pages));
  state.page = pageNumber;
  updateSourcePdfControls(kind);
  const token = ++state.renderToken;
  try{
    const page = await state.document.getPage(pageNumber);
    const baseViewport = page.getViewport({ scale: 1 });
    const availableWidth = Math.max(260, container.clientWidth - 20);
    const scale = Math.max(0.3, Math.min(1.6, availableWidth / baseViewport.width));
    const viewport = page.getViewport({ scale });
    const pixelRatio = Math.min(window.devicePixelRatio || 1, 2);
    const context = canvas.getContext("2d");
    canvas.width = Math.ceil(viewport.width * pixelRatio);
    canvas.height = Math.ceil(viewport.height * pixelRatio);
    canvas.style.width = `${Math.ceil(viewport.width)}px`;
    canvas.style.height = `${Math.ceil(viewport.height)}px`;
    context.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0);
    await page.render({ canvasContext: context, viewport }).promise;
    if (token !== state.renderToken) return;
  }catch(error){
    console.warn(`Unable to render ${kind} PDF page`, error);
  }
}

async function loadSourcePdf(kind, source){
  const state = sourcePdfState[kind];
  const { container } = sourcePdfElements(kind);
  if (!state || !container || !source) return;
  state.renderToken += 1;
  state.document = null;
  state.page = 1;
  state.pages = 0;
  updateSourcePdfControls(kind);
  try{
    const pdfjsLib = await ensurePdfJs();
    let bytes;
    if (source instanceof Blob){
      bytes = new Uint8Array(await source.arrayBuffer());
    }else{
      const value = String(source);
      const prefix = "data:application/pdf;base64,";
      if (!value.startsWith(prefix)) throw new Error("Unsupported PDF source");
      bytes = base64ToBytes(value.slice(prefix.length));
    }
    const loadingTask = pdfjsLib.getDocument({ data: new Uint8Array(bytes), useWorkerFetch: false });
    const documentRef = await loadingTask.promise;
    state.document = documentRef;
    state.pages = documentRef.numPages;
    state.page = 1;
    updateSourcePdfControls(kind);
    await renderSourcePdfPage(kind, 1);
  }catch(error){
    console.warn(`Unable to load ${kind} PDF preview`, error);
  }
}

function resetSourcePdf(kind){
  const state = sourcePdfState[kind];
  if (!state) return;
  state.renderToken += 1;
  state.document = null;
  state.page = 1;
  state.pages = 0;
  const { canvas } = sourcePdfElements(kind);
  const context = canvas?.getContext("2d");
  if (context && canvas) context.clearRect(0, 0, canvas.width, canvas.height);
  updateSourcePdfControls(kind);
}

function setSourcePdfPage(kind, page){
  const state = sourcePdfState[kind];
  if (!state?.document) return;
  renderSourcePdfPage(kind, page);
}

function selectOrderDetailView(viewName){
  const target = ["summary", "items", "files", "activity"].includes(viewName) ? viewName : "summary";
  document.querySelectorAll("[data-order-detail-view]").forEach(button => {
    const active = button.dataset.orderDetailView === target;
    button.classList.toggle("active", active);
    button.setAttribute("aria-selected", String(active));
  });
  document.querySelectorAll("[data-order-detail-panel]").forEach(panel => {
    const active = panel.dataset.orderDetailPanel === target;
    panel.classList.toggle("active", active);
    panel.hidden = !active;
  });
  if (target === "files") renderSourcePdfPage("order", sourcePdfState.order.page);
}

function orderLifecycleSteps(status){
  const normalized = normalizeHistoryStatusValue(status);
  const steps = [
    { key: "received", label: "Received" },
    { key: "extracted", label: "Extracted" },
    { key: "reviewed", label: "Review" },
    { key: "approved", label: "Approved" },
    { key: "in_production", label: "Production" },
    { key: "completed", label: "Completed" },
  ];
  const currentIndexByStatus = {
    draft: 2,
    reviewed: 2,
    approved: 3,
    in_production: 4,
    completed: 5,
    archived: 5,
  };
  const currentIndex = currentIndexByStatus[normalized] ?? 2;
  return steps.map((step, index) => ({
    ...step,
    state: index < currentIndex ? "complete" : (index === currentIndex ? "current" : "upcoming"),
  }));
}

function getRowSourceLocation(row){
  if (!row || typeof row !== "object") return null;
  const location = row.row_location || row.source_location || row.location;
  return location && typeof location === "object" ? location : null;
}

function updatePdfFramePage(frame, page){
  if (!frame || frame.hidden) return;
  setSourcePdfPage(frame === orderDetailSourceFrame ? "order" : "extract", page);
}

function showSourceLocation(frame, highlight, row, fallbackLabel = "Source row"){
  const location = getRowSourceLocation(row);
  if (!highlight) return;
  if (!location){
    highlight.hidden = false;
    highlight.textContent = `${fallbackLabel} selected · exact page location is unavailable.`;
    return;
  }
  const page = Math.max(1, Number(location.page) || 1);
  const matchedText = String(location.matched_text || "").trim();
  highlight.hidden = false;
  highlight.innerHTML = `<strong>Page ${page}</strong>${matchedText ? ` · ${escapeHtml(matchedText)}` : ""}`;
  updatePdfFramePage(frame, page);
}

function renderOrderLifecycle(order){
  if (!orderLifecycleEl) return;
  orderLifecycleEl.innerHTML = orderLifecycleSteps(order?.status).map((step, index) => `
    <div class="order-lifecycle-step ${step.state}">
      <span class="order-lifecycle-marker">${step.state === "complete" ? "✓" : index + 1}</span>
      <span>${escapeHtml(step.label)}</span>
    </div>
  `).join("");
}

function countOrderReviewIssues(rows, warningsMap, warnings){
  let count = Array.isArray(warnings) ? warnings.length : 0;
  (rows || []).forEach(row => {
    count += missingCriticalFields(row).length;
    count += getDiagnosticIssues(row).length;
    const rowWarnings = warningsMap?.[row._rid] || [];
    count += Array.isArray(rowWarnings) ? rowWarnings.length : 0;
  });
  return count;
}

function renderOrderReviewSummary(order){
  if (!orderDetailReviewSummary) return;
  const issueCount = countOrderReviewIssues(
    appState.historyDetail.rows,
    appState.historyDetail.rowWarnings,
    appState.historyDetail.warnings,
  );
  const rowCount = appState.historyDetail.rows.length;
  const confidence = formatHistoryConfidence(order?.confidence);
  orderDetailReviewSummary.innerHTML = `
    <div><span>Items</span><strong>${rowCount}</strong></div>
    <div><span>Review issues</span><strong class="${issueCount ? "has-issues" : ""}">${issueCount}</strong></div>
    <div><span>Confidence</span><strong>${escapeHtml(confidence)}</strong></div>
  `;
}

function renderOrderSource(order){
  const extraction = order?.extraction || {};
  const source = typeof extraction.raw_input === "string" ? extraction.raw_input : "";
  const isPdf = source.startsWith("data:application/pdf;base64,");
  if (orderDetailSourceMeta){
    const method = extraction.extraction_method || order?.extraction_method || order?.source || "Saved extraction";
    orderDetailSourceMeta.textContent = isPdf ? `${method} · original PDF` : `${method} · text source`;
  }
  if (orderDetailSourceFrame){
    orderDetailSourceFrame.hidden = !isPdf;
    if (isPdf) loadSourcePdf("order", source);
    else resetSourcePdf("order");
  }
  if (orderDetailSourceEmpty){
    orderDetailSourceEmpty.hidden = isPdf;
    orderDetailSourceEmpty.textContent = source
      ? "This order was created from text. The raw input is available beside the production files."
      : "No source PDF is stored for this order.";
  }
  if (orderDetailSourceHighlight){
    orderDetailSourceHighlight.hidden = true;
    orderDetailSourceHighlight.textContent = "";
  }
}

function orderFileMatches(item, order){
  if (!item || !order) return false;
  if (item.order_id != null && String(item.order_id) === String(order.id)) return true;
  const orderNumbers = new Set(
    (Array.isArray(order.order_numbers) ? order.order_numbers : [order.order_number])
      .filter(Boolean)
      .map(value => normalizeWorkspaceOrderToken(value)),
  );
  return orderNumbers.has(normalizeWorkspaceOrderToken(item.order_number));
}

function renderOrderFilesList(items){
  if (!orderDetailFiles) return;
  if (!items.length){
    orderDetailFiles.innerHTML = '<div class="overview-empty">No production files generated yet.</div>';
    return;
  }
  orderDetailFiles.innerHTML = items.map(item => {
    const processingUrl = workspaceFileUrl(item.processing_pdf_url);
    const labelsUrl = workspaceFileUrl(item.labels_pdf_url);
    return `<div class="workspace-file-item">
      <div>
        <strong>${escapeHtml(item.order_number || "Production batch")}</strong>
        <div class="workspace-file-meta">${escapeHtml(formatDate(item.generated_at))} · ${escapeHtml(item.batch_status || "ready")}</div>
      </div>
      <div class="workspace-file-actions">
        ${processingUrl ? `<a class="btn small" href="${escapeHtml(processingUrl)}" target="_blank" rel="noopener">Processing PDF</a>` : ""}
        ${labelsUrl ? `<a class="btn small" href="${escapeHtml(labelsUrl)}" target="_blank" rel="noopener">Labels PDF</a>` : ""}
      </div>
    </div>`;
  }).join("");
}

async function loadOrderDetailFiles(order){
  if (!orderDetailFiles || !order) return;
  orderDetailFiles.innerHTML = '<div class="overview-empty">Loading production files…</div>';
  try{
    const cached = [...(workspaceState.moduleFiles || []), ...(workspaceState.recentFiles || [])];
    let items = cached.filter(item => orderFileMatches(item, order));
    const payload = await fetchWorkspaceRecentFiles();
    const recent = Array.isArray(payload?.items) ? payload.items : [];
    workspaceState.recentFiles = recent;
    items = [...(workspaceState.moduleFiles || []), ...recent].filter(item => orderFileMatches(item, order));
    if (String(historyState.selectedOrder?.id) === String(order.id)) renderOrderFilesList(items);
  }catch{
    const cached = [...(workspaceState.moduleFiles || []), ...(workspaceState.recentFiles || [])]
      .filter(item => orderFileMatches(item, order));
    renderOrderFilesList(cached);
  }
}

function renderOrderDetailEnhancements(order){
  if (!order) return;
  const label = getOrderLabel(order);
  const client = getClientName(order);
  const status = normalizeHistoryStatusValue(order.status);
  if (orderDetailTitle) orderDetailTitle.textContent = label === "—" ? `Order #${order.id}` : label;
  if (orderDetailStatus) orderDetailStatus.innerHTML = historyStatusBadgeHtml(status);
  if (orderDetailClient) orderDetailClient.textContent = client === "—" ? "Client not specified" : client;
  if (pageTitle) pageTitle.textContent = label === "—" ? "Order Detail" : label;
  if (pageSubtitle) pageSubtitle.textContent = `${client === "—" ? "Unknown client" : client} · ${historyStatusLabel(status)}`;
  if (mobilePageTitle) mobilePageTitle.textContent = label === "—" ? "Order Detail" : label;
  document.title = `${label === "—" ? "Order Detail" : label} — Order Extractor`;
  renderOrderLifecycle(order);
  renderOrderReviewSummary(order);
  renderOrderSource(order);
  selectOrderDetailView(status === "draft" || status === "reviewed" ? "items" : "summary");
  loadOrderDetailFiles(order);
}

function renderOrderDetail(){
  const order = historyState.selectedOrder;
  if (!order){
    appState.historyDetail.order = null;
    appState.historyDetail.rows = [];
    appState.historyDetail.originalRows = [];
    appState.historyDetail.rowWarnings = {};
    appState.historyDetail.rowDiagnoses = {};
    appState.historyDetail.warnings = [];
    appState.historyDetail.notes = "";
    appState.historyDetail.orderTotalDiagnostics = null;
    if (historyRawTextEl) historyRawTextEl.textContent = "No raw extraction text available.";
    renderHistoryStatusTimeline([]);
    updateHistoryDetailUI();
    return;
  }

  appState.historyDetail.order = order;
  const rows = Array.isArray(order.rows) ? order.rows.map(row => ({ ...row })) : [];
  appState.historyDetail.originalRows = rows.map(row => ({ ...row }));
  const warningsSource = order.row_warnings || {};
  const warningsMap = {};
  appState.historyDetail.nextRid = 0;
  const timestampBase = Date.now();
  appState.historyDetail.rows = rows.map((row, idx) => {
    const rid = row._rid || `hist-${timestampBase}-${appState.historyDetail.nextRid++}`;
    const enriched = { ...row, _rid: rid };
    const rowWarn = Array.isArray(warningsSource)
      ? warningsSource[idx]
      : warningsSource[idx] || warningsSource[rid];
    if (rowWarn && rowWarn.length){
      warningsMap[rid] = rowWarn;
    }
    return enriched;
  });
  appState.historyDetail.rowWarnings = warningsMap;
  appState.historyDetail.rowDiagnoses = {};
  appState.historyDetail.warnings = order.warnings || [];
  appState.historyDetail.notes = order.notes || "";
  appState.historyDetail.declaredUnits = order.declared_units ?? null;
  appState.historyDetail.declaredArea = order.declared_area ?? null;
  appState.historyDetail.parsedUnits = order.parsed_units ?? null;
  appState.historyDetail.parsedArea = order.parsed_area ?? null;
  appState.historyDetail.orderTotalDiagnostics = order.order_total_diagnostics || null;
  const extraction = order.extraction || {};
  let rawText = extraction.prepared_text || extraction.raw_input || "";
  if (typeof rawText === "string" && rawText.startsWith("data:application/pdf;base64,")){
    rawText = "[Binary PDF source omitted. This order was extracted from a PDF upload.]";
  }
  if (historyRawTextEl){
    historyRawTextEl.textContent = rawText || "No raw extraction text available.";
  }
  renderHistoryStatusTimeline(order.status_history || []);
  updateHistoryDetailUI();
  renderOrderDetailEnhancements(order);
}

async function updateOrderStatusRequest(orderId, status, note){
  const res = await fetch(`${API_BASE}/orders/${orderId}/status`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ status, note: note || undefined }),
  });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || (`HTTP ${res.status}`));
  }
  const payload = await res.json();
  return payload?.order || payload;
}

function buildHistoryRowsPayload(rows){
  return (rows || []).map(row => ({
    order_number: row.order_number || "",
    type: row.type || "",
    dimension: row.dimension || "",
    position: row.position || "",
    quantity: Number(row.quantity || 0),
    area: Number(row.area || 0),
  }));
}

async function approveHistoryOrderById(orderId, note = ""){
  const detail = await fetchOrder(orderId);
  const status = normalizeHistoryStatusValue(detail.status);
  if (!canApproveStatus(status)){
    throw new Error(`Order status is ${historyStatusLabel(status)} and cannot be approved.`);
  }
  const rowsPayload = buildHistoryRowsPayload(detail.rows || []);
  return approveDraft(orderId, rowsPayload, note || detail.notes || "", detail);
}

async function refreshHistoryAfterBatch(){
  historyState.needsRefresh = true;
  analysisState.allOrdersDirty = true;
  clearHistorySelection();
  await loadHistoryWorkQueue({ force: true });
  if (!historyState.queue) await loadHistory({ resetOffset: true, ignoreQueue: true });
}

async function runBatchApprove(orderIds, activityId){
  updateBackgroundActivity(activityId, { detail: `Approving ${orderIds.length} orders…`, progress: 10 });
  const failures = [];
  let approved = 0;
  for (let index = 0; index < orderIds.length; index += 1){
    const id = orderIds[index];
    try{
      const detail = await fetchOrder(id);
      if (canApproveStatus(detail.status)){
        await approveHistoryOrderById(id);
        approved += 1;
      }else if (normalizeHistoryStatusValue(detail.status) !== "approved"){
        failures.push(`#${id}: ${historyStatusLabel(detail.status)}`);
      }
    }catch(error){
      failures.push(`#${id}: ${error.message || error}`);
    }
    updateBackgroundActivity(activityId, {
      progress: 10 + Math.round(((index + 1) / orderIds.length) * 80),
      detail: `${index + 1} of ${orderIds.length} checked`,
    });
  }
  await refreshHistoryAfterBatch();
  if (failures.length) throw new Error(`${failures.length} order${failures.length === 1 ? "" : "s"} could not be approved. ${failures.slice(0, 2).join(" · ")}`);
  completeBackgroundActivity(activityId, {
    detail: `${approved} order${approved === 1 ? "" : "s"} approved.`,
    openTab: "history",
  });
}

async function runBatchProduction(orderIds, activityId){
  updateBackgroundActivity(activityId, { detail: "Loading approved orders…", progress: 12 });
  const orders = await Promise.all(orderIds.map(id => fetchOrder(id)));
  validateWorkspaceOrdersForProcessing(orders);
  updateBackgroundActivity(activityId, { detail: "Building processing sheet…", progress: 40 });
  const result = await processValidatedOrdersViaExistingModules(orders, orders.length > 1 ? "combined" : "single", {
    mergeAcrossOrders: false,
  });
  updateBackgroundActivity(activityId, { detail: "Preparing production downloads…", progress: 85 });
  const downloads = [
    result.files?.processing_pdf_url ? { label: "Processing PDF", url: result.files.processing_pdf_url } : null,
    result.files?.labels_pdf_url ? { label: "Labels PDF", url: result.files.labels_pdf_url } : null,
  ].filter(Boolean);
  completeBackgroundActivity(activityId, {
    detail: `Production files ready for ${orders.length} order${orders.length === 1 ? "" : "s"}.`,
    downloads,
    openTab: "workspace",
  });
  clearHistorySelection();
}

async function runBatchArchive(orderIds, activityId){
  updateBackgroundActivity(activityId, { detail: `Archiving ${orderIds.length} orders…`, progress: 10 });
  const failures = [];
  for (let index = 0; index < orderIds.length; index += 1){
    try{
      const detail = await fetchOrder(orderIds[index]);
      if (canArchiveStatus(detail.status)) await updateOrderStatusRequest(orderIds[index], "archived");
    }catch(error){
      failures.push(`#${orderIds[index]}: ${error.message || error}`);
    }
    updateBackgroundActivity(activityId, {
      progress: 10 + Math.round(((index + 1) / orderIds.length) * 80),
      detail: `${index + 1} of ${orderIds.length} checked`,
    });
  }
  await refreshHistoryAfterBatch();
  if (failures.length) throw new Error(`${failures.length} order${failures.length === 1 ? "" : "s"} could not be archived.`);
  completeBackgroundActivity(activityId, {
    detail: `${orderIds.length} order${orderIds.length === 1 ? "" : "s"} archived.`,
    openTab: "history",
  });
}

document.getElementById("historyListWrap").addEventListener("click", async (event)=>{
  const btn = event.target.closest("button[data-action]");
  if (!btn) return;
  closeHistoryActionMenus();
  const rawId = btn.dataset.id;
  if (rawId == null || rawId === "") return;
  const numericId = Number(rawId);
  const id = Number.isFinite(numericId) ? numericId : rawId;
	    const action = btn.dataset.action;
	    if (action === "open"){
	      await openSharedOrderDetail(id);
	    }else if (action === "edit-draft"){
	      await openSharedOrderDetail(id);
  }else if (action === "approve"){
    try{
      setHistoryStatus("Approving order…");
      await approveHistoryOrderById(id);
      historyState.needsRefresh = true;
      analysisState.allOrdersDirty = true;
      analysisState.orderRowCache.delete(id);
      await loadHistory({ resetOffset: false });
	      await openSharedOrderDetail(id);
      setHistoryStatus("Order approved.");
    }catch(error){
      setHistoryStatus("Approval failed: " + (error.message || error));
    }
  }else if (action === "processing"){
    try{
      await sendOrderToProcessing(id, {
        skipStatus: true,
        skipTab: true,
        showToast: true,
        toastMessage: "Added to Processing",
      });
      setHistoryStatus("Added to Processing.");
    }catch(error){
      setHistoryStatus("Failed to add to Processing: " + (error.message || error));
    }
  }else if (action === "csv"){
    try{
      await downloadOrderCsv(id);
      setHistoryStatus("");
    }catch(error){
      setHistoryStatus("Download failed: " + (error.message || error));
    }
  }else if (action === "labels"){
    try{
      const order = await fetchOrder(id);
      if (order.rows && order.rows.length){
        await handlePrint(order.rows);
        setHistoryStatus("Labels ready.");
      }else{
        setHistoryStatus("No rows to print for this order.");
      }
    }catch(error){
      setHistoryStatus("Failed to generate labels: " + (error.message || error));
    }
  }else if (action === "archive"){
    if (!confirm("Archive this order?")) return;
    try{
      await updateOrderStatusRequest(id, "archived");
      historyState.needsRefresh = true;
      analysisState.allOrdersDirty = true;
      await loadHistory({ resetOffset: false });
	      if (historyState.selectedOrder && String(historyState.selectedOrder.id) === String(id)){
	        await openSharedOrderDetail(id);
	      }
      setHistoryStatus("Order archived.");
    }catch(error){
      setHistoryStatus("Archive failed: " + (error.message || error));
    }
  }else if (action === "delete"){
    const label = btn.closest("tr")?.querySelector("td")?.textContent?.trim() || `#${id}`;
    if (!confirm(`Delete order ${label}? This cannot be undone.`)) return;
    try{
      await deleteOrder(id);
      historyState.needsRefresh = true;
      analysisState.allOrdersDirty = true;
      analysisState.orderRowCache.delete(id);
      if (historyState.selectedOrder && String(historyState.selectedOrder.id) === String(id)){
        historyState.selectedOrder = null;
        appState.historyDetail.order = null;
        appState.historyDetail.rows = [];
        appState.historyDetail.warnings = [];
        closeHistoryDrawer();
        updateHistoryDetailUI();
      }
      await loadHistory({ resetOffset: false });
      setHistoryStatus("Order deleted.");
    }catch(error){
      setHistoryStatus("Delete failed: " + (error.message || error));
    }
  }
});

document.getElementById("historyListWrap").addEventListener("change", event => {
  const selectAll = event.target.closest("#historySelectAllVisible");
  if (selectAll){
    historyState.items.forEach(order => {
      const id = String(order.id ?? order.order_id ?? "");
      if (!id) return;
      if (selectAll.checked){
        historyState.selectedIds.add(id);
        historyState.selectedOrders.set(id, order);
      }else{
        historyState.selectedIds.delete(id);
        historyState.selectedOrders.delete(id);
      }
    });
    renderOrdersList();
    return;
  }
  const checkbox = event.target.closest("[data-history-select]");
  if (!checkbox) return;
  const id = String(checkbox.dataset.historySelect || "");
  const order = historyState.items.find(item => String(item.id ?? item.order_id) === id);
  if (checkbox.checked && order){
    historyState.selectedIds.add(id);
    historyState.selectedOrders.set(id, order);
  }else{
    historyState.selectedIds.delete(id);
    historyState.selectedOrders.delete(id);
  }
  renderOrdersList();
});

document.querySelectorAll("[data-history-queue]").forEach(button => {
  button.addEventListener("click", ()=> {
    const queue = button.dataset.historyQueue;
    clearHistorySelection();
    if (queue === "failed"){
      historyState.queue = "failed";
      renderHistoryQueue();
      telegramFilesState.status = "failed";
      if (telegramFilesStatusFilter) telegramFilesStatusFilter.value = "failed";
      activateTab("telegram");
      loadTelegramFiles();
      return;
    }
    applyHistoryQueue(queue);
  });
});

document.getElementById("historyQueueAll")?.addEventListener("click", ()=> {
  historyState.queue = "";
  clearHistorySelection();
  renderHistoryQueue();
  loadHistory({ resetOffset: true, ignoreQueue: true });
});
document.getElementById("historyQueueRefresh")?.addEventListener("click", ()=> loadHistoryWorkQueue({ force: true }));
historySelectVisible?.addEventListener("click", ()=> {
  historyState.items.forEach(order => {
    const id = String(order.id ?? order.order_id ?? "");
    if (!id) return;
    historyState.selectedIds.add(id);
    historyState.selectedOrders.set(id, order);
  });
  renderOrdersList();
});
historyClearSelection?.addEventListener("click", clearHistorySelection);
historyBatchApprove?.addEventListener("click", async ()=> {
  const ids = Array.from(historyState.selectedIds);
  if (!ids.length || !confirm(`Approve ${ids.length} selected order${ids.length === 1 ? "" : "s"}?`)) return;
  const activityId = startBackgroundActivity("Batch approval", {
    type: "approval",
    detail: `Preparing ${ids.length} orders…`,
    openTab: "history",
    retry: retryId => runBatchApprove(ids, retryId),
  });
  setActivityCenterOpen(true);
  try{
    await runBatchApprove(ids, activityId);
  }catch(error){
    failBackgroundActivity(activityId, error);
  }
});
historyBatchProduction?.addEventListener("click", async ()=> {
  const ids = Array.from(historyState.selectedIds);
  if (!ids.length || !confirm(`Generate combined production and label files for ${ids.length} selected order${ids.length === 1 ? "" : "s"}?`)) return;
  const activityId = startBackgroundActivity("Production file generation", {
    type: "production",
    detail: `Preparing ${ids.length} orders…`,
    openTab: "workspace",
    retry: retryId => runBatchProduction(ids, retryId),
  });
  setActivityCenterOpen(true);
  try{
    await runBatchProduction(ids, activityId);
  }catch(error){
    failBackgroundActivity(activityId, error);
  }
});
historyBatchArchive?.addEventListener("click", async ()=> {
  const ids = Array.from(historyState.selectedIds);
  if (!ids.length || !confirm(`Archive ${ids.length} selected order${ids.length === 1 ? "" : "s"}?`)) return;
  const activityId = startBackgroundActivity("Batch archive", {
    type: "archive",
    detail: `Preparing ${ids.length} orders…`,
    openTab: "history",
    retry: retryId => runBatchArchive(ids, retryId),
  });
  setActivityCenterOpen(true);
  try{
    await runBatchArchive(ids, activityId);
  }catch(error){
    failBackgroundActivity(activityId, error);
  }
});

document.addEventListener("click", (event)=>{
  const target = event.target;
  if (target && target.closest && target.closest(".history-row-actions")){
    return;
  }
  closeHistoryActionMenus();
});

document.getElementById("historyPrev").addEventListener("click", ()=>{
  if (historyState.offset <= 0) return;
  historyState.offset = Math.max(historyState.offset - historyState.limit, 0);
  loadHistory();
});

document.getElementById("historyNext").addEventListener("click", ()=>{
  if (!historyState.hasMore) return;
  historyState.offset += historyState.limit;
  loadHistory();
});

if (historyInvoiceAllBtn){
  historyInvoiceAllBtn.addEventListener("click", ()=>{
    handleHistoryInvoiceAllClick();
  });
}

document.getElementById("historyRefresh").addEventListener("click", ()=>{
  loadHistory();
});

document.getElementById("historyDownloadAll").addEventListener("click", async ()=>{
  setHistoryStatus("Preparing CSV export…");
  try{
    await downloadAllCsv();
    setHistoryStatus("Export ready.");
  }catch(error){
    setHistoryStatus("Failed to export: " + (error.message || error));
  }
});

document.getElementById("historyCsv").addEventListener("click", async ()=>{
  const order = historyState.selectedOrder;
  if (!order) return;
  if (!canCsvStatus(order.status)){
    setHistoryStatus("CSV export is allowed only for approved/production/completed orders.");
    return;
  }
  try{
    await downloadOrderCsv(order.id);
    setHistoryStatus("");
  }catch(error){
    setHistoryStatus("Download failed: " + (error.message || error));
  }
});

document.getElementById("historyProcessing").addEventListener("click", async ()=>{
  const order = historyState.selectedOrder;
  if (!order) return;
  if (!canProcessingStatus(order.status)){
    setHistoryStatus("This order status cannot be sent to Processing.");
    return;
  }
  try{
    await sendOrderToProcessing(order.id, {
      skipStatus: true,
      skipTab: true,
      showToast: true,
      toastMessage: "Added to Processing",
    });
    setHistoryStatus("Added to Processing.");
  }catch(error){
    setHistoryStatus("Failed to add to Processing: " + (error.message || error));
  }
});

if (historyInvoiceBtn){
  historyInvoiceBtn.addEventListener("click", async ()=>{
    const order = historyState.selectedOrder;
    if (!order) return;
    if (!canInvoiceStatus(order.status)){
      setHistoryStatus("Only approved/production/completed orders can be invoiced.");
      return;
    }
    try{
      const full = Array.isArray(order.rows) && order.rows.length ? order : await fetchOrder(order.id);
      const job = await createInvoiceJobFromOrder(full);
      if (job){
        setHistoryStatus("Invoice job added. Check the Invoices tab to review.");
      }
    }catch(error){
      setHistoryStatus("Failed to add to invoice: " + (error.message || error));
    }
  });
}

if (historyArchiveBtn){
  historyArchiveBtn.addEventListener("click", async ()=>{
    const order = historyState.selectedOrder;
    if (!order) return;
    if (!canArchiveStatus(order.status)){
      setHistoryStatus("Order is already archived.");
      return;
    }
    if (!confirm("Archive this order?")) return;
    try{
      const updated = await updateOrderStatusRequest(order.id, "archived", appState.historyDetail.notes || "");
      historyState.selectedOrder = updated;
      renderOrderDetail();
      historyState.needsRefresh = true;
      analysisState.allOrdersDirty = true;
      await loadHistory({ resetOffset: false });
      setHistoryStatus("Order archived.");
    }catch(error){
      setHistoryStatus("Archive failed: " + (error.message || error));
    }
  });
}

if (historyDrawerClose){
  historyDrawerClose.addEventListener("click", ()=> closeHistoryDrawer());
}

document.querySelectorAll("[data-order-detail-view]").forEach(button => {
  button.addEventListener("click", ()=> selectOrderDetailView(button.dataset.orderDetailView));
});

document.addEventListener("keydown", event=>{
  if (event.key === "Escape" && panels.orderdetail?.classList.contains("active")){
    closeHistoryDrawer();
  }
});

document.getElementById("historyTableWrap")?.addEventListener("focusin", event => {
  const rowElement = event.target.closest("tr[data-index]");
  const row = appState.historyDetail.rows[Number(rowElement?.dataset.index)];
  if (row) showSourceLocation(orderDetailSourceFrame, orderDetailSourceHighlight, row, "Order item");
});

if (processingOrderList){
  processingOrderList.addEventListener("click", (event)=>{
    const btn = event.target.closest("[data-remove-processing]");
    if (!btn) return;
    const id = btn.dataset.removeProcessing;
    if (!id) return;
    removeOrderFromProcessing(id);
    setStatusMessage("Order removed from Processing.");
  });
}

if (processingClearBtn){
  processingClearBtn.addEventListener("click", ()=>{
    clearProcessing();
    setStatusMessage("Processing sheet cleared.");
  });
}

if (processingRoundingBtn){
  processingRoundingBtn.addEventListener("click", ()=>{
    const active = isProcessingRoundingActive();
    if (active){
      appState.processing.rounding.manualApplied = false;
      if (appState.processing.options.autoDanko){
        appState.processing.options.autoDanko = false;
        if (processingAutoDankoToggle) processingAutoDankoToggle.checked = false;
      }
      setProcessingRoundingMessage("Rounded values removed.");
      applyProcessingRoundingToRows();
      recalcProcessingPreview();
      updateProcessingUI();
      return;
    }
    applyDankoRounding();
  });
}

if (processingAutoDankoToggle){
  processingAutoDankoToggle.addEventListener("change", event => {
    appState.processing.options.autoDanko = !!event.target.checked;
    if (appState.processing.options.autoDanko){
      appState.processing.rounding.manualApplied = false;
      setProcessingRoundingMessage("Auto Danko rounding enabled. ✅");
    }else{
      setProcessingRoundingMessage("Auto Danko rounding disabled.");
    }
    applyProcessingRoundingToRows();
    recalcProcessingPreview();
    updateProcessingUI();
  });
}

if (processingGroupBtn){
  processingGroupBtn.addEventListener("click", ()=>{
    if (appState.processing.grouped){
      appState.processing.grouped = false;
      rebuildProcessingRows();
      updateProcessingUI();
      setStatusMessage("Grouping disabled.");
      return;
    }
    groupProcessingDimensions();
    setStatusMessage("Dimensions grouped.");
  });
}

if (processingRestartToggle){
  processingRestartToggle.addEventListener("change", (event)=>{
    appState.processing.options.restartPerGroup = !!event.target.checked;
    rebuildProcessingRows();
    updateProcessingUI();
  });
}

if (processingNormalizeToggle){
  processingNormalizeToggle.addEventListener("change", (event)=>{
    appState.processing.options.normalizeLPtoG = !!event.target.checked;
    rebuildProcessingRows();
    updateProcessingUI();
  });
}

if (processingDecimalSelect){
  processingDecimalSelect.addEventListener("change", (event)=>{
    appState.processing.options.decimalSeparator = event.target.value === "dot" ? "dot" : "comma";
    rebuildProcessingRows();
    updateProcessingUI();
  });
}

if (processingMergeOrdersToggle){
  processingMergeOrdersToggle.addEventListener("change", (event)=>{
    appState.processing.options.mergeAcrossOrders = !!event.target.checked;
    rebuildProcessingRows();
    updateProcessingUI();
  });
}

if (processingHeaderSelect){
  processingHeaderSelect.addEventListener("change", ()=>{
    syncProcessingHeaderEditor();
  });
}

if (processingHeaderApply){
  processingHeaderApply.addEventListener("click", ()=>{
    if (!processingHeaderSelect || processingHeaderSelect.disabled) return;
    if (!processingHeaderInput) return;
    const raw = processingHeaderSelect.value;
    if (!raw) return;
    const value = (processingHeaderInput.value || "").trim();
    if (value){
      appState.processing.headerOverrides[raw] = value;
    }else{
      delete appState.processing.headerOverrides[raw];
    }
    rebuildProcessingRows();
    updateProcessingUI();
  });
}

if (processingHeaderReset){
  processingHeaderReset.addEventListener("click", ()=>{
    if (!processingHeaderSelect || processingHeaderSelect.disabled) return;
    const raw = processingHeaderSelect.value;
    if (!raw) return;
    delete appState.processing.headerOverrides[raw];
    rebuildProcessingRows();
    updateProcessingUI();
  });
}

if (processingCopyBtn){
  processingCopyBtn.addEventListener("click", ()=>{ copyProcessingText(); });
}

if (spacerMergeOrdersToggle){
  spacerMergeOrdersToggle.checked = !!spacerProcessingState.mergeAcrossOrders;
  spacerMergeOrdersToggle.addEventListener("change", (event)=>{
    spacerProcessingState.mergeAcrossOrders = !!event.target.checked;
    updateSpacerProcessingUI();
  });
}

if (spacerPdfPreviewToggle){
  spacerPdfPreviewToggle.checked = !!spacerProcessingState.pdfPreview;
  spacerPdfPreviewToggle.addEventListener("change", (event)=>{
    spacerProcessingState.pdfPreview = !!event.target.checked;
    updateSpacerProcessingUI();
  });
}

if (spacerWysiwygToggle){
  spacerWysiwygToggle.checked = !!spacerProcessingState.liveHtmlPreview;
  spacerWysiwygToggle.addEventListener("change", (event)=>{
    spacerProcessingState.liveHtmlPreview = !!event.target.checked;
    updateSpacerProcessingUI();
  });
}

if (spacerCopyAllBtn){
  spacerCopyAllBtn.addEventListener("click", async ()=>{
    try{
      await copySpacerProcessingText();
    }catch(error){
      setStatusMessage("Copy failed: " + (error.message || error));
    }
  });
}

if (spacerGeneratePdfBtn){
  spacerGeneratePdfBtn.addEventListener("click", async ()=>{
    try{
      await exportSpacerProcessingPdf();
    }catch(error){
      setStatusMessage("PDF export failed: " + (error.message || error));
    }
  });
}

if (processingExportPdfBtn){
  processingExportPdfBtn.addEventListener("click", async ()=>{
    try{
      await exportProcessingPdf();
    }catch(error){
      setStatusMessage("PDF export failed: " + (error.message || error));
    }
  });
}

if (processingExportCsvBtn){
  processingExportCsvBtn.addEventListener("click", ()=>{
    try{
      exportProcessingCsv();
    }catch(error){
      setStatusMessage("CSV export failed: " + (error.message || error));
    }
  });
}

if (labelsFromProcessingBtn){
  labelsFromProcessingBtn.addEventListener("click", ()=>{
    addLabelJobsFromProcessing();
  });
}

if (labelsJobsWrap){
  labelsJobsWrap.addEventListener("click", async (event)=>{
    const btn = event.target.closest("button[data-action]");
    if (!btn) return;
    const id = btn.dataset.id;
    const action = btn.dataset.action;
    if (!id || !action) return;
    const index = appState.labels.jobs.findIndex(job => job.id === id);
    if (index < 0){
      setLabelsStatus("Label job not found.");
      return;
    }
    const job = appState.labels.jobs[index];
    if (action === "labels-download"){
      try{
        await handlePrint(job.rows);
        setLabelsStatus("Labels ready (downloaded).");
      }catch(error){
        console.error("Label download failed", error);
        setLabelsStatus("Error generating labels.");
      }
    }else if (action === "labels-remove"){
      appState.labels.jobs.splice(index, 1);
      setLabelsStatus("Label job removed.");
      updateLabelsUI();
    }
  });
}

if (invoiceFilterClient){
  invoiceFilterClient.addEventListener("input", event=>{
    ensureInvoicesLoaded();
    setInvoiceFilters({ client: event.target.value });
  });
}

if (invoiceFilterOrder){
  invoiceFilterOrder.addEventListener("input", event=>{
    ensureInvoicesLoaded();
    setInvoiceFilters({ order: event.target.value });
  });
}

if (invoiceFilterStatus){
  invoiceFilterStatus.addEventListener("change", event=>{
    ensureInvoicesLoaded();
    setInvoiceFilters({ status: event.target.value });
  });
}

if (invoicePaginationPrev){
  invoicePaginationPrev.addEventListener("click", ()=>{
    ensureInvoicesLoaded();
    changeInvoicePage(-1);
  });
}

if (invoicePaginationNext){
  invoicePaginationNext.addEventListener("click", ()=>{
    ensureInvoicesLoaded();
    changeInvoicePage(1);
  });
}

if (invoiceAddGlassBtn){
  invoiceAddGlassBtn.addEventListener("click", ()=>{
    ensureInvoicesLoaded();
    invoiceRuntime.extraGlassRows += 1;
    renderInvoicePriceEditor();
  });
}

if (invoiceAddSpacerBtn){
  invoiceAddSpacerBtn.addEventListener("click", ()=>{
    ensureInvoicesLoaded();
    invoiceRuntime.extraSpacerRows += 1;
    renderInvoicePriceEditor();
  });
}

if (invoiceAddOrderBtn){
  invoiceAddOrderBtn.addEventListener("click", ()=>{
    ensureInvoicesLoaded();
    openInvoiceAddOrderModal();
  });
}

if (invoiceDetailMeta){
  invoiceDetailMeta.addEventListener("click", async event=>{
    const btn = event.target.closest("[data-remove-order]");
    if (!btn) return;
    const orderNumber = btn.dataset.removeOrder;
    if (!orderNumber) return;
    const warning = `Remove order ${orderNumber} from this invoice?\nThis will remove all lines belonging to that order, but it will NOT delete the original order from History.`;
    if (!confirm(warning)) return;
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    try{
      const updated = await removeOrderFromInvoiceJob(job.id, orderNumber);
      if (!updated){
        setInvoiceStatus("Invoice order removed.");
      }else{
        setInvoiceStatus(`Order ${orderNumber} removed from invoice.`);
      }
      updateInvoicesUI();
    }catch(error){
      console.error("Failed to remove order from invoice", error);
      setInvoiceStatus("Failed to remove order: " + (error?.message || error));
    }
  });
}

if (invoiceAddOrderClose){
  invoiceAddOrderClose.addEventListener("click", ()=>{
    closeInvoiceAddOrderModal();
  });
}

if (invoiceAddOrderModal){
  invoiceAddOrderModal.addEventListener("click", event=>{
    if (event.target === invoiceAddOrderModal){
      closeInvoiceAddOrderModal();
    }
  });
}

if (invoiceAddOrderSearch){
  invoiceAddOrderSearch.addEventListener("input", event=>{
    invoiceAddOrderState.searchTerm = event.target.value || "";
    ensureInvoicesLoaded();
    const job = getActiveInvoiceJob();
    renderInvoiceAddOrderList(job);
  });
}

if (invoiceAddOrderList){
  invoiceAddOrderList.addEventListener("click", async event=>{
    const btn = event.target.closest("[data-add-order-id]");
    if (!btn) return;
    await handleInvoiceAddOrderSelect(btn.dataset.addOrderId);
  });
}

if (invoiceSavePricesBtn){
  invoiceSavePricesBtn.addEventListener("click", async ()=>{
    ensureInvoicesLoaded();
    await savePricesFromEditor();
  });
}

if (invoiceRefreshPricesBtn){
  invoiceRefreshPricesBtn.addEventListener("click", async ()=>{
    ensureInvoicesLoaded();
    await recalcAllInvoices();
    updateInvoicesUI();
    setInvoiceStatus("Draft invoices recalculated.");
  });
}

if (typeCorrectionsAddBtn){
  typeCorrectionsAddBtn.addEventListener("click", ()=>{
    ensureInvoicesLoaded();
    openTypeCorrectionsModal();
  });
}

if (typeCorrectionsCancelBtn){
  typeCorrectionsCancelBtn.addEventListener("click", ()=>{
    closeTypeCorrectionsModal();
  });
}

if (typeCorrectionsSaveBtn){
  typeCorrectionsSaveBtn.addEventListener("click", async ()=>{
    ensureInvoicesLoaded();
    await handleTypeCorrectionsSave();
  });
}

if (typeCorrectionsModal){
  typeCorrectionsModal.addEventListener("click", event=>{
    if (event.target === typeCorrectionsModal){
      closeTypeCorrectionsModal();
    }
  });
}

if (typeCorrectionsRawInput){
  typeCorrectionsRawInput.addEventListener("input", ()=>{
    typeCorrectionsRawInput.classList.remove("input-error");
  });
}

if (typeCorrectionsCorrectedInput){
  typeCorrectionsCorrectedInput.addEventListener("input", ()=>{
    typeCorrectionsCorrectedInput.classList.remove("input-error");
  });
}

if (typeCorrectionsTable){
  typeCorrectionsTable.addEventListener("click", async event=>{
    const btn = event.target.closest("[data-delete-correction]");
    if (!btn) return;
    const idx = Number(btn.dataset.deleteCorrection);
    if (!Number.isFinite(idx)) return;
    TYPE_CORRECTIONS.splice(idx, 1);
    const payload = buildInvoiceConfigPayload();
    await applyInvoiceConfigToState(payload, { cacheLocal: true, preferExistingTypeCorrections: true });
    const saved = await persistInvoiceConfigToServer(payload, { preferExistingTypeCorrections: true });
    updateInvoicesUI();
    setInvoiceStatus(saved ? "Type correction removed." : "Type correction removed locally.");
  });
}

if (invoiceJobsWrap){
  invoiceJobsWrap.addEventListener("click", async (event)=>{
    const btn = event.target.closest("[data-invoice-action]");
    if (!btn) return;
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const action = btn.dataset.invoiceAction;
    const jobId = btn.dataset.jobId || btn.dataset.id || btn.closest("tr")?.dataset.jobId;
    const orderNumber = btn.dataset.orderNumber || btn.closest("tr")?.dataset.orderNumber || "";
    if (!action || !jobId) return;
    const lowerOrder = (orderNumber || "").toLowerCase();
    const job = appState.invoices.jobs.find(item => item.id === jobId)
      || (lowerOrder ? appState.invoices.jobs.find(item => (item.orderNumbers || []).some(num => String(num || "").toLowerCase() === lowerOrder)) : null);
    if (action === "open"){
      if (!job){
        setInvoiceStatus("Invoice job not found.");
        return;
      }
      appState.invoices.activeJobId = job.id;
      updateInvoicesUI();
    }else if (action === "pdf"){
      if (!job) return;
      try{
        await generateInvoicePdf(job);
        setInvoiceStatus("Invoice PDF ready.");
      }catch(error){
        setInvoiceStatus(error?.message || "Failed to generate PDF.");
      }
    }else if (action === "delete"){
      if (!job){
        setInvoiceStatus("Invoice job not found.");
        return;
      }
      const targetOrder = orderNumber || (job.orderNumbers && job.orderNumbers[0]) || "";
      const label = targetOrder ? `Remove order ${targetOrder} from this invoice?` : "Remove this order from the invoice?";
      if (!confirm(label)) return;
      try{
        const updated = await removeOrderFromInvoiceJob(job.id, targetOrder);
        if (!updated){
          setInvoiceStatus("Invoice job removed.");
        }else{
          setInvoiceStatus(targetOrder ? `Order ${targetOrder} removed from invoice.` : "Order removed from invoice.");
        }
        updateInvoicesUI();
      }catch(error){
        console.error("Failed to remove order from invoice", error);
        setInvoiceStatus("Failed to remove order: " + (error?.message || error));
      }
    }
  });
}

if (invoiceLinesWrap){
  invoiceLinesWrap.addEventListener("change", async event=>{
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const typeInput = event.target.closest("input[data-type-edit]");
    const spacerKindSelect = event.target.closest("select[data-spacer-kind]");
    if (!typeInput && !spacerKindSelect) return;
    const idx = Number((typeInput || spacerKindSelect).dataset.typeEdit || (typeInput || spacerKindSelect).dataset.spacerKind);
    if (!Number.isFinite(idx)) return;
    const job = getActiveInvoiceJob();
    if (!job || !Array.isArray(job.rows)) return;
    const rawRows = ensureInvoiceRawRows(job);
    const lines = Array.isArray(job.calculated?.lines) ? job.calculated.lines : [];
    const line = lines.find(l => l.index === idx);

    if (typeInput){
      const newType = String(typeInput.value || "").trim();
      const correctedType = applyTypeCorrections(newType);
      const finalType = correctedType || newType;
      if (typeInput.value !== finalType){
        typeInput.value = finalType;
      }
      if (line){
        line.type = finalType;
        line.displayType = finalType;
        line.normalizedType = normalizeGlassKey(finalType) || finalType.toLowerCase();
        if (Array.isArray(line.rawIndexes)){
          line.rawIndexes.forEach(i=>{
            if (rawRows[i]){
              rawRows[i].type = finalType;
            }
          });
        }
      }else{
        const rowIndex = idx - 1;
        if (rowIndex >= 0 && rowIndex < rawRows.length){
          rawRows[rowIndex].type = finalType;
        }
      }
    }else if (spacerKindSelect){
      const kindValue = spacerKindSelect.value === "thermal" ? "thermal" : "normal";
      if (line){
        line.spacerKind = kindValue;
        line.spacerMode = kindValue;
        if (Array.isArray(line.rawIndexes)){
          line.rawIndexes.forEach(i=>{
            if (rawRows[i]){
              rawRows[i].spacerKind = kindValue;
            }
          });
        }
      }
    }

    job.rows = rawRows;
    await recalcInvoiceJob(job, { allowPrompt: true });
    await saveInvoiceJobs();
    updateInvoicesUI();
  });
}

if (invoiceRecalcBtn){
  invoiceRecalcBtn.addEventListener("click", async ()=>{
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    await recalcInvoiceJob(job, { allowPrompt: true });
    await saveInvoiceJobs();
    updateInvoicesUI();
    setInvoiceStatus("Invoice recalculated.");
  });
}

if (invoiceGeneratePdfBtn){
  invoiceGeneratePdfBtn.addEventListener("click", async ()=>{
    ensureInvoicesLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    try{
      await generateInvoicePdf(job);
      setInvoiceStatus("Invoice PDF ready.");
    }catch(error){
      setInvoiceStatus(error?.message || "Failed to generate PDF.");
    }
  });
}

if (invoiceDeleteActiveBtn){
  invoiceDeleteActiveBtn.addEventListener("click", async ()=>{
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    if (!confirm("Delete this invoice job?")) return;
    await deleteInvoiceJob(job.id);
  });
}

if (invoiceDiscountMode){
  invoiceDiscountMode.addEventListener("change", async event=>{
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    job.discountMode = event.target.value === "absolute" ? "absolute" : "percent";
    await recalcInvoiceJob(job, { allowPrompt: true });
    await saveInvoiceJobs();
    updateInvoicesUI();
  });
}

if (invoiceDiscountValue){
  invoiceDiscountValue.addEventListener("change", async event=>{
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    job.discountValue = Math.max(0, Number(event.target.value) || 0);
    await recalcInvoiceJob(job, { allowPrompt: true });
    await saveInvoiceJobs();
    updateInvoicesUI();
  });
}

if (invoiceVatValue){
  invoiceVatValue.addEventListener("change", async event=>{
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    job.vatRate = Math.max(0, Number(event.target.value) || 0);
    await recalcInvoiceJob(job, { allowPrompt: true });
    await saveInvoiceJobs();
    updateInvoicesUI();
  });
}

if (invoiceStatusSelect){
  invoiceStatusSelect.addEventListener("change", async event=>{
    ensureInvoicesLoaded();
    await ensureInvoiceJobsLoaded();
    const job = getActiveInvoiceJob();
    if (!job) return;
    const newStatus = event.target.value === "finalized" ? "finalized" : "draft";
    job.status = newStatus;
    await saveInvoiceJobs();
    if (newStatus === "finalized"){
      const nextDraft = findNextDraftInvoice(job.id);
      if (nextDraft){
        appState.invoices.activeJobId = nextDraft.id;
      }
    }
    updateInvoicesUI();
  });
}

if (invoicePromptCancel){
  invoicePromptCancel.addEventListener("click", ()=>{
    invoiceRuntime.pendingPrompts = [];
    invoiceRuntime.missingKeys.clear();
    hideInvoicePrompt();
  });
}

if (invoicePromptSave){
  invoicePromptSave.addEventListener("click", async ()=>{
    await handleInvoicePromptSave();
  });
}

document.getElementById("historyApprove").addEventListener("click", async ()=>{
  const order = historyState.selectedOrder;
  if (!order) return;
  if (!canApproveStatus(order.status)){
    setHistoryStatus("This order status cannot be approved.");
    return;
  }
  const rows = buildHistoryRowsPayload(appState.historyDetail.rows || []);
  setHistoryStatus("Approving draft…");
  try{
    const resp = await approveDraft(order.id, rows, appState.historyDetail.notes, order);
    setHistoryStatus("Order approved.");
    historyState.needsRefresh = true;
    analysisState.allOrdersDirty = true;
    analysisState.orderRowCache.delete(order.id);
    try{
      await loadHistory({ resetOffset: false });
    }catch(err){
      console.warn("History refresh failed after approval", err);
    }
    try{
      const refreshed = await fetchOrder(order.id);
      historyState.selectedOrder = refreshed;
      renderOrderDetail();
    }catch(err){
      console.warn("Failed to reload approved order", err);
    }
    showSavedToast(resp?.saved_order_id || order.id);
  }catch(error){
    console.error("Order approval failed", error);
    let fallbackHandled = false;
    // Try to reload the order detail directly
    try{
      const snapshot = await fetchOrder(order.id);
      if (snapshot && normalizeHistoryStatusValue(snapshot.status) === "approved"){
        historyState.selectedOrder = snapshot;
        renderOrderDetail();
        historyState.needsRefresh = true;
        analysisState.allOrdersDirty = true;
        analysisState.orderRowCache.delete(order.id);
        try{
          await loadHistory({ resetOffset: false });
        }catch(loadErr){
          console.warn("History refresh failed after fallback", loadErr);
        }
        setHistoryStatus("Order approved (auto-recovered).");
        showSavedToast(snapshot.id || order.id);
        fallbackHandled = true;
      }
    }catch(fallbackErr){
      console.warn("Fallback approval fetch failed", fallbackErr);
    }

    // If direct fetch failed, see whether the history list already shows the order as approved
    if (!fallbackHandled){
      try{
        await loadHistory({ resetOffset: false });
        const inHistory = historyState.items.find(item => item.id === order.id);
        if (inHistory && normalizeHistoryStatusValue(inHistory.status) === "approved"){
          historyState.selectedOrder = await fetchOrder(order.id).catch(()=> ({ ...order, status: "approved" }));
          renderOrderDetail();
          setHistoryStatus("Order approved (history shows success).");
          showSavedToast(order.id);
          analysisState.allOrdersDirty = true;
          analysisState.orderRowCache.delete(order.id);
          fallbackHandled = true;
        }
      }catch(historyErr){
        console.warn("History reload after approval error failed", historyErr);
      }
    }

    if (!fallbackHandled){
      setHistoryStatus("Approval failed: " + (error && error.message ? error.message : String(error)));
    }
  }
});

document.getElementById("historyPrint").addEventListener("click", async ()=>{
  const rows = getDisplayedRows("history");
  if (!rows.length){
    setHistoryStatus("No rows to print.");
    return;
  }
  setHistoryStatus("Generating labels…");
  try{
    await handlePrint(rows);
    setHistoryStatus("Labels ready.");
  }catch(error){
    setHistoryStatus("Failed to generate labels: " + (error.message || error));
  }
});

document.getElementById("historyLimit").addEventListener("change", (event)=>{
  historyState.limit = Number(event.target.value) || 25;
  historyState.offset = 0;
  loadHistory({ resetOffset: true });
});

historyState.year = normalizeHistoryYear(readHistoryYearFromUrl() || "all");
if (historyStatusFilterSelect) historyStatusFilterSelect.value = historyState.status;
if (historyClientFilterInput) historyClientFilterInput.value = historyState.client;
if (historyDateFromInput) historyDateFromInput.value = historyState.dateFrom;
if (historyDateToInput) historyDateToInput.value = historyState.dateTo;
if (historyApprovedOnlyToggle) historyApprovedOnlyToggle.checked = historyState.approvedOnly;

if (historyStatusFilterSelect){
  historyStatusFilterSelect.addEventListener("change", (event)=>{
    historyState.status = event.target.value || "";
    if (historyState.status && historyApprovedOnlyToggle && historyApprovedOnlyToggle.checked){
      historyApprovedOnlyToggle.checked = false;
      historyState.approvedOnly = false;
    }
    historyState.offset = 0;
    loadHistory({ resetOffset: true });
  });
}

if (historyClientFilterInput){
  let timer = null;
  historyClientFilterInput.addEventListener("input", event=>{
    historyState.client = (event.target.value || "").trim();
    historyState.offset = 0;
    clearTimeout(timer);
    timer = setTimeout(()=> loadHistory({ resetOffset: true }), 250);
  });
}

if (historyDateFromInput){
  historyDateFromInput.addEventListener("change", event=>{
    historyState.dateFrom = event.target.value || "";
    historyState.offset = 0;
    loadHistory({ resetOffset: true });
  });
}

if (historyDateToInput){
  historyDateToInput.addEventListener("change", event=>{
    historyState.dateTo = event.target.value || "";
    historyState.offset = 0;
    loadHistory({ resetOffset: true });
  });
}

if (historyApprovedOnlyToggle){
  historyApprovedOnlyToggle.addEventListener("change", event=>{
    historyState.approvedOnly = !!event.target.checked;
    if (historyState.approvedOnly){
      historyState.status = "";
      if (historyStatusFilterSelect) historyStatusFilterSelect.value = "";
    }
    historyState.offset = 0;
    loadHistory({ resetOffset: true });
  });
}

let historySearchTimer = null;
if (historySearchInput){
  historySearchInput.addEventListener("input", (event)=>{
    const value = event.target.value.trim();
    historyState.query = value;
    historyState.offset = 0;
    clearTimeout(historySearchTimer);
    historySearchTimer = setTimeout(()=> loadHistory({ resetOffset: true }), 250);
  });
}

// ---------- LLM extraction + PDF handling -----------------------------------
async function callAPI(text){
  setErrorMessage("");
  setStatusMessage("Extracting…", { showSpinner: true });
  const res = await fetch(API_BASE + "/extract", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text })
  });
  if (!res.ok){
    const message = await res.text();
    throw new Error(message || ("HTTP " + res.status));
  }
  const data = await res.json();
  setStatusMessage("Done.");
  return data;
}

async function callPdfAPI(file, options = {}){
  setErrorMessage("");
  const forceOcr = !!options.forceOcr;
  setStatusMessage(forceOcr ? "Extracting PDF with OCR…" : "Extracting PDF…", { showSpinner: true });
  const formData = new FormData();
  formData.append("file", file, file.name || "upload.pdf");
  const endpoint = API_BASE + "/extract_pdf" + (forceOcr ? "?force_ocr=true" : "");
  const res = await fetch(endpoint, {
    method: "POST",
    body: formData
  });
  if (!res.ok){
    const message = await res.text();
    throw new Error(message || ("HTTP " + res.status));
  }
  const data = await res.json();
  setStatusMessage("Done.");
  return data;
}

async function approveDraft(orderId, rows, notes, clientSource = {}){
  const clientName = getClientName(clientSource, "");
  const payload = {
    rows,
    notes: notes || undefined,
    client_name: clientName || undefined,
  };
  const res = await fetch(`${API_BASE}/orders/${orderId}/approve`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload)
  });
  if (!res.ok){
    const text = await res.text();
    throw new Error(text || (`HTTP ${res.status}`));
  }
  return res.json();
}

function applyExtractionResult(data){
  const rows = Array.isArray(data?.rows) ? data.rows.map(row => ({ ...row })) : [];
  appState.extract.originalRows = rows.map(row => ({ ...row }));
  const warningsSource = data?.row_warnings || {};
  appState.extract.nextRid = 0;
  const warningsMap = {};
  const timestampBase = Date.now();
  appState.extract.rows = rows.map((row, idx) => {
    const rid = row._rid || `ex-${timestampBase}-${appState.extract.nextRid++}`;
    const enriched = { ...row, _rid: rid };
    const rowWarn = Array.isArray(warningsSource)
      ? warningsSource[idx]
      : warningsSource[idx] || warningsSource[rid];
    if (rowWarn && rowWarn.length){
      warningsMap[rid] = rowWarn;
    }
    return enriched;
  });
  appState.extract.rowWarnings = warningsMap;
  appState.extract.rowDiagnoses = {};
  appState.extract.warnings = data?.warnings || [];
  if (data?.created_new_version && data?.protected_order_id){
    const protectedId = data.protected_order_id;
    appState.extract.warnings = [
      ...appState.extract.warnings,
      `protected_order_preserved: approved/reviewed order #${protectedId} was not overwritten; a new draft version was created.`,
    ];
  }
  appState.extract.draftId = data?.draft_order_id || null;
  appState.extract.status = data?.status || (data?.saved_order_id ? "approved" : "draft");
  appState.extract.savedOrderId = data?.saved_order_id || null;
  appState.extract.source_hash = data?.source_hash || "";
  appState.extract.extractionMethod = data?.extraction_method || "";
  appState.extract.orderNumber = data?.order_number || "";
  appState.extract.appliedCorrections = data?.applied_corrections || [];
  appState.extract.notes = "";
  appState.extract.declaredUnits = data?.declared_units ?? null;
  appState.extract.declaredArea = data?.declared_area ?? null;
  appState.extract.parsedUnits = data?.parsed_units ?? null;
  appState.extract.parsedArea = data?.parsed_area ?? null;
  appState.extract.orderTotalDiagnostics = data?.order_total_diagnostics || null;
  appState.extract.confidence = Number.isFinite(Number(data?.confidence)) ? Number(data.confidence) : null;
  appState.extract.extractedAt = new Date().toISOString();
  const clientName = getClientName(data);
  appState.extract.client_name = clientName === "—" ? "" : clientName;
  appState.extract.client = clientName;
  if (extractSourceMeta){
    const fileName = extractSourceMeta.dataset.fileName || "";
    const method = appState.extract.extractionMethod
      ? appState.extract.extractionMethod.replaceAll("_", " ")
      : "extracted";
    extractSourceMeta.textContent = fileName ? `${fileName} · ${method}` : `Pasted text · ${method}`;
  }
  historyState.needsRefresh = true;
  analysisState.allOrdersDirty = true;
  if (data?.saved_order_id){
    analysisState.orderRowCache.delete(data.saved_order_id);
  }
  updateExtractUI();
  if (data?.saved_order_id){
    showSavedToast(data.saved_order_id);
  }
}

function clearExtractPdfPreview(){
  if (extractPdfPreview){
    extractPdfPreview.hidden = true;
  }
  resetSourcePdf("extract");
  if (extractPdfPreviewEmpty) extractPdfPreviewEmpty.hidden = false;
  if (extractSourceHighlight){
    extractSourceHighlight.hidden = true;
    extractSourceHighlight.textContent = "";
  }
  if (extractSourceMeta){
    extractSourceMeta.dataset.fileName = "";
    extractSourceMeta.textContent = "Pasted text";
  }
}

function showExtractPdfPreview(file){
  if (!file || !extractPdfPreview) return;
  extractPdfPreview.hidden = false;
  if (extractPdfPreviewEmpty) extractPdfPreviewEmpty.hidden = true;
  if (extractSourceHighlight) extractSourceHighlight.hidden = true;
  if (extractSourceMeta){
    extractSourceMeta.dataset.fileName = file.name || "Uploaded PDF";
    extractSourceMeta.textContent = `${file.name || "Uploaded PDF"} · ${formatFileSize(file.size)}`;
  }
  loadSourcePdf("extract", file);
}

async function handleTextExtraction(text, options = {}){
  const value = (text || "").trim();
  if (!value) return;
  const activityId = options.activityId || startBackgroundActivity("Text extraction", {
    type: "extraction",
    detail: "Sending pasted order text…",
    openTab: "extract",
    retry: retryId => handleTextExtraction(value, { activityId: retryId }),
  });
  try{
    lastExtractPdfFile = null;
    clearExtractPdfPreview();
    updateBackgroundActivity(activityId, { detail: "Extracting order fields…", progress: 30 });
    const data = await callAPI(value);
    updateBackgroundActivity(activityId, { detail: "Validating extracted rows…", progress: 82 });
    applyExtractionResult(data);
    completeBackgroundActivity(activityId, {
      detail: `${appState.extract.rows.length} row${appState.extract.rows.length === 1 ? "" : "s"} extracted and ready for review.`,
      openTab: "extract",
    });
  }catch(error){
    setStatusMessage("");
    setErrorMessage("Error: " + (error.message || error));
    failBackgroundActivity(activityId, error, { openTab: "extract" });
  }
}

async function handlePdfExtraction(file, options = {}){
  if (!file) return;
  const forceOcr = !!options.forceOcr;
  const activityId = options.activityId || startBackgroundActivity(forceOcr ? "OCR extraction" : "PDF extraction", {
    type: forceOcr ? "ocr" : "extraction",
    detail: `${file.name || "Order PDF"} · preparing pages`,
    openTab: "extract",
    retry: retryId => handlePdfExtraction(file, { ...options, activityId: retryId }),
  });
  try{
    lastExtractPdfFile = file;
    showExtractPdfPreview(file);
    updateBackgroundActivity(activityId, {
      detail: forceOcr ? "Running OCR across PDF pages…" : "Reading PDF pages…",
      progress: 25,
    });
    const data = await callPdfAPI(file, options);
    updateBackgroundActivity(activityId, { detail: "Validating extracted rows…", progress: 84 });
    applyExtractionResult(data);
    completeBackgroundActivity(activityId, {
      detail: `${file.name || "PDF"} · ${appState.extract.rows.length} row${appState.extract.rows.length === 1 ? "" : "s"} ready for review.`,
      openTab: "extract",
    });
  }catch(error){
    setStatusMessage("");
    setErrorMessage("Error: " + (error.message || error));
    failBackgroundActivity(activityId, error, { openTab: "extract" });
  }
}

document.getElementById("extract").addEventListener("click", async ()=>{
  const text = document.getElementById("input").value.trim();
  if (!text) return;
  await handleTextExtraction(text);
});

document.getElementById("rerun").addEventListener("click", async ()=>{
  if (lastExtractPdfFile){
    await handlePdfExtraction(lastExtractPdfFile);
    return;
  }
  const text = document.getElementById("input").value.trim();
  if (!text) return;
  await handleTextExtraction(text);
});

document.getElementById("approveSave").addEventListener("click", async ()=>{
  const draftId = appState.extract.draftId;
  if (!draftId){
    setStatusMessage("No draft to approve yet.");
    return;
  }
  const rows = (appState.extract.rows || []).map(row => ({
    order_number: row.order_number || "",
    type: row.type || "",
    dimension: row.dimension || "",
    position: row.position || "",
    quantity: Number(row.quantity || 0),
    area: Number(row.area || 0),
  }));
  setStatusMessage("Approving draft…");
  try{
    const resp = await approveDraft(draftId, rows, appState.extract.notes, appState.extract);
    const updatedRows = Array.isArray(resp?.order?.rows) ? resp.order.rows.map(r => ({ ...r })) : rows;
    appState.extract.originalRows = updatedRows.map(row => ({ ...row }));
    const warningsSource = resp?.row_warnings || {};
    const warningsMap = {};
    const timestampBase = Date.now();
    appState.extract.nextRid = 0;
    appState.extract.rows = updatedRows.map((row, idx) => {
      const rid = row._rid || `ex-${timestampBase}-${appState.extract.nextRid++}`;
      const enriched = { ...row, _rid: rid };
      const rowWarn = Array.isArray(warningsSource)
        ? warningsSource[idx]
        : warningsSource[idx] || warningsSource[rid];
      if (rowWarn && rowWarn.length){
        warningsMap[rid] = rowWarn;
      }
      return enriched;
    });
    appState.extract.rowWarnings = warningsMap;
    appState.extract.rowDiagnoses = {};
    appState.extract.warnings = resp?.warnings || [];
    appState.extract.draftId = null;
    appState.extract.status = "approved";
    appState.extract.savedOrderId = resp?.saved_order_id || draftId;
    appState.extract.declaredUnits = resp?.declared_units ?? appState.extract.declaredUnits;
    appState.extract.declaredArea = resp?.declared_area ?? appState.extract.declaredArea;
    appState.extract.parsedUnits = resp?.parsed_units ?? appState.extract.parsedUnits;
    appState.extract.parsedArea = resp?.parsed_area ?? appState.extract.parsedArea;
    appState.extract.orderTotalDiagnostics = resp?.order_total_diagnostics || null;
    updateExtractUI();
    historyState.needsRefresh = true;
    analysisState.allOrdersDirty = true;
    analysisState.orderRowCache.delete(resp?.saved_order_id || draftId);
    loadHistory();
    setStatusMessage("Order approved.");
    if (resp?.saved_order_id){
      showSavedToast(resp.saved_order_id);
    }
  }catch(error){
    setStatusMessage("Approval failed.");
    setErrorMessage("Approval error: " + (error.message || error));
  }
});

document.getElementById("copyCsv").addEventListener("click", async ()=>{
  const rows = getDisplayedRows("extract");
  if (!rows.length){
    setStatusMessage("Nothing to copy.");
    return;
  }
  const csv = toCSV(rows);
  try{
    await navigator.clipboard.writeText(csv);
    setStatusMessage("CSV copied.");
  }catch{
    setStatusMessage("Copy failed — select and copy manually.");
  }
});

document.getElementById("downloadCsv").addEventListener("click", ()=>{
  const rows = getDisplayedRows("extract");
  if (!rows.length){
    setStatusMessage("Nothing to download.");
    return;
  }
  const csv = toCSV(rows);
  const blob = new Blob([csv], { type: "text/csv" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "order.csv";
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
});

async function handlePrint(rows, options = {}){
  if (!rows || !rows.length){
    throw new Error("No rows to print.");
  }
  const activityId = options.activityId || startBackgroundActivity("Label generation", {
    type: "labels",
    detail: `Preparing labels from ${rows.length} row${rows.length === 1 ? "" : "s"}…`,
    openTab: "labels",
    retry: retryId => handlePrint(rows, { activityId: retryId }),
  });
  try{
    updateBackgroundActivity(activityId, { detail: "Building label pages…", progress: 30 });
    const blob = await buildLabelsPdfBlob(rows);
    updateBackgroundActivity(activityId, { detail: "Preparing label download…", progress: 88 });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "labels.pdf";
    document.body.appendChild(a);
    a.click();
    a.remove();
    completeBackgroundActivity(activityId, {
      detail: "Labels are ready to download again when needed.",
      downloads: [{ label: "Labels PDF", url }],
      openTab: "labels",
    });
    return { blob, url };
  }catch(error){
    failBackgroundActivity(activityId, error, { openTab: "labels" });
    throw error;
  }
}

async function buildLabelsPdfBlob(rows){
  if (!rows || !rows.length){
    throw new Error("No rows to print.");
  }
  const pdfBytes = await generateLabelsPdf(rows);
  return new Blob([pdfBytes], { type: "application/pdf" });
}

document.getElementById("printLabels").addEventListener("click", async ()=>{
  const rows = getDisplayedRows("extract");
  if (!rows.length){
    setStatusMessage("No rows to print — extract first.");
    return;
  }
  setStatusMessage("Generating labels…");
  try{
    await handlePrint(rows);
    setStatusMessage("Labels ready (downloaded).");
  }catch(error){
    console.error(error);
    setStatusMessage("Error generating labels.");
  }
});

const extractGenerateLabelsBtn = document.getElementById("extractGenerateLabels");
if (extractGenerateLabelsBtn){
  extractGenerateLabelsBtn.addEventListener("click", ()=>{
    const printBtn = document.getElementById("printLabels");
    if (printBtn) printBtn.click();
  });
}

const extractAddRowBtn = document.getElementById("extractAddRow");
if (extractAddRowBtn){
  extractAddRowBtn.addEventListener("click", ()=>{
    addRowToGroup("extract", appState.extract.orderNumber || "");
  });
}

const lowConfidenceOnlyToggle = document.getElementById("lowConfidenceOnly");
if (lowConfidenceOnlyToggle){
  lowConfidenceOnlyToggle.addEventListener("change", ()=>{
    setStatusMessage(lowConfidenceOnlyToggle.checked ? "Low confidence filter view enabled." : "");
    updateExtractUI();
  });
}

function isPdfFile(file){
  if (!file) return false;
  const type = (file.type || "").toLowerCase();
  const name = (file.name || "").toLowerCase();
  return type === "application/pdf" || name.endsWith(".pdf");
}

function isPlainTextFile(file){
  if (!file) return false;
  const type = (file.type || "").toLowerCase();
  const name = (file.name || "").toLowerCase();
  return type.startsWith("text/") || name.endsWith(".txt");
}

(function(){
  const dropZone = document.getElementById("pdfDropZone");
  const fileInput = document.getElementById("pdfInput");
  if (!dropZone || !fileInput) return;

  const prevent = (event)=>{
    event.preventDefault();
    event.stopPropagation();
  };

  ["dragenter","dragover"].forEach(evt=>{
    dropZone.addEventListener(evt, (event)=>{
      prevent(event);
      dropZone.classList.add("hover");
      if (event.dataTransfer) event.dataTransfer.dropEffect = "copy";
    });
  });

  ["dragleave","dragend"].forEach(evt=>{
    dropZone.addEventListener(evt, (event)=>{
      prevent(event);
      dropZone.classList.remove("hover");
    });
  });

  dropZone.addEventListener("drop", async (event)=>{
    prevent(event);
    dropZone.classList.remove("hover");
    setErrorMessage("");
    const files = event.dataTransfer && event.dataTransfer.files;
    if (files && files.length){
      const file = files[0];
      if (isPdfFile(file)){
        await handlePdfExtraction(file);
        return;
      }
	      if (isPlainTextFile(file)){
	        const text = await file.text();
	        if (text && text.trim()){
	          document.getElementById("input").value = text;
	          await handleTextExtraction(text);
	          return;
	        }
	      }
	      const scanFiles = getSupportedGlobalScanFiles(files);
	      if (scanFiles.length){
	        globalScanDropState.pendingFiles = scanFiles;
	        setGlobalScanDropOverlay(true, { loading: true });
	        activateTab("scanstudio");
	        return;
	      }
	    }
    const textData = event.dataTransfer ? event.dataTransfer.getData("text") : "";
    if (textData && textData.trim()){
      document.getElementById("input").value = textData;
      await handleTextExtraction(textData);
      return;
    }
    setStatusMessage("Unsupported drop. Use a PDF or text.");
  });

  dropZone.addEventListener("click", ()=> fileInput.click());
  dropZone.addEventListener("keydown", event=>{
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    fileInput.click();
  });
  fileInput.addEventListener("change", async (event)=>{
    const file = event.target.files && event.target.files[0];
    event.target.value = "";
    if (!file) return;
    if (!isPdfFile(file)){
      setStatusMessage("Please choose a PDF file.");
      return;
    }
    await handlePdfExtraction(file);
  });
})();

// --------- Label helper utilities (unchanged core) -------------------------
function dataURLToBytes(dataUrl){
  try{
    const base64 = (dataUrl.split(",")[1] || "");
    const bin = atob(base64);
    const len = bin.length;
    const bytes = new Uint8Array(len);
    for (let i = 0; i < len; i++) bytes[i] = bin.charCodeAt(i);
    return bytes;
  }catch{
    return null;
  }
}

function bytesToDataURL(bytes){
  if (!bytes) return null;
  const arr = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
  let binary = "";
  const chunk = 0x8000;
  for (let i = 0; i < arr.length; i += chunk){
    const slice = arr.subarray(i, i + chunk);
    binary += String.fromCharCode.apply(null, slice);
  }
  return "data:image/png;base64," + btoa(binary);
}

function bytesToBase64(bytes){
  if (!bytes) return "";
  const arr = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
  let binary = "";
  const chunk = 0x8000;
  for (let i = 0; i < arr.length; i += chunk){
    const slice = arr.subarray(i, i + chunk);
    binary += String.fromCharCode.apply(null, slice);
  }
  return btoa(binary);
}

function base64ToBytes(base64){
  const bin = atob(String(base64 || ""));
  const bytes = new Uint8Array(bin.length);
  for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
  return bytes;
}

function fileToDataURL(file){
  return new Promise((resolve, reject)=>{
    const reader = new FileReader();
    reader.onload = () => resolve(reader.result);
    reader.onerror = reject;
    reader.readAsDataURL(file);
  });
}

async function ensurePdfLib(){
  if (window.PDFLib) return;
  await new Promise((resolve, reject)=>{
    const script = document.createElement("script");
    script.src = "https://cdn.jsdelivr.net/npm/pdf-lib/dist/pdf-lib.min.js";
    script.onload = resolve;
    script.onerror = ()=> reject(new Error("Failed to load pdf-lib"));
    document.head.appendChild(script);
  });
}

let pdfJsLibPromise = null;
async function ensurePdfJs(){
  if (window.pdfjsLib) return window.pdfjsLib;
  if (!pdfJsLibPromise){
    pdfJsLibPromise = import("https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/build/pdf.mjs").then(mod => {
      mod.GlobalWorkerOptions.workerSrc = "https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/build/pdf.worker.mjs";
      window.pdfjsLib = mod;
      return mod;
    });
  }
  return pdfJsLibPromise;
}

const pdfEditorState = {
  fileName: "",
  originalPdfBytes: null,
  workingPdfBytes: null,
  pdfDocument: null,
  pages: [],
  overlays: [],
  activeVisibleIndex: 0,
  zoom: 1,
  fitWidth: true,
  tool: "select",
  selectedOverlayId: null,
  dirty: false,
  renderToken: 0,
  drag: null,
  inlineTextBlock: null,
  textBlocks: [],
  hasTextLayer: false,
  hoveredTextBlockId: null,
  activeTextBlockId: null,
  selectedTextBlockId: null,
  isEditingText: false,
  nativeTextEdit: null,
  activeViewport: null,
  exportUrl: null,
};

function setPdfEditorStatus(message, isError = false){
  if (!pdfEditorStatusEl) return;
  pdfEditorStatusEl.textContent = message || "";
  pdfEditorStatusEl.classList.toggle("error", !!isError);
}

function setPdfEditorDirty(dirty){
  pdfEditorState.dirty = !!dirty;
  if (pdfEditorUnsavedEl) pdfEditorUnsavedEl.hidden = !pdfEditorState.dirty;
}

function getVisiblePdfPages(){
  return pdfEditorState.pages.filter(page => !page.deleted);
}

function getActivePdfPageState(){
  const pages = getVisiblePdfPages();
  if (!pages.length) return null;
  const index = Math.min(Math.max(0, pdfEditorState.activeVisibleIndex), pages.length - 1);
  pdfEditorState.activeVisibleIndex = index;
  return pages[index] || null;
}

function createPdfOverlay(type, pageIndex, rect, data = {}){
  return {
    id: `pdf-ov-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    type,
    pageIndex,
    x: rect.x,
    y: rect.y,
    width: rect.width,
    height: rect.height,
    rotation: 0,
    opacity: data.opacity ?? (type === "highlight" ? 0.35 : 1),
    fontSize: data.fontSize || 14,
    fontFamily: data.fontFamily || "Helvetica",
    color: data.color || "#111827",
    backgroundColor: data.backgroundColor || (type === "textEdit" ? "#ffffff" : "transparent"),
    alignment: data.alignment || "left",
    content: data.content || "",
    originalText: data.originalText || "",
    replacementText: data.replacementText || "",
    detectedFromTextLayer: !!data.detectedFromTextLayer,
    baselineY: Number.isFinite(data.baselineY) ? data.baselineY : null,
    transform: Array.isArray(data.transform) ? data.transform.slice() : [],
    pageScale: Number.isFinite(data.pageScale) ? data.pageScale : null,
    pageRotation: Number.isFinite(data.pageRotation) ? data.pageRotation : 0,
    sourceWidth: Number.isFinite(data.sourceWidth) ? data.sourceWidth : rect.width,
    sourceHeight: Number.isFinite(data.sourceHeight) ? data.sourceHeight : rect.height,
    originalBounds: data.originalBounds ? { ...data.originalBounds } : { x: rect.x, y: rect.y, width: rect.width, height: rect.height },
    replacementPosition: data.replacementPosition ? { ...data.replacementPosition } : { x: rect.x, y: rect.y },
    replacementBounds: data.replacementBounds ? { ...data.replacementBounds } : { x: rect.x, y: rect.y, width: rect.width, height: rect.height },
    pending: !!data.pending,
    applied: !!data.applied,
    textBlockId: data.textBlockId || "",
    imageDataUrl: data.imageDataUrl || "",
    path: Array.isArray(data.path) ? data.path : [],
  };
}

function isPdfTextEditTool(tool = pdfEditorState.tool){
  return tool === "textEdit" || tool === "nativeTextEdit";
}

function getPdfToolLabel(tool){
  if (tool === "textEdit") return "Safe Text Edit";
  if (tool === "nativeTextEdit") return "Native Text Edit";
  if (tool === "text") return "Text Box";
  return tool || "select";
}

function clampPdfRect(rect){
  const x = Math.min(Math.max(rect.x, 0), 0.995);
  const y = Math.min(Math.max(rect.y, 0), 0.995);
  const width = Math.min(Math.max(rect.width, 0.01), 1 - x);
  const height = Math.min(Math.max(rect.height, 0.01), 1 - y);
  return { x, y, width, height };
}

function getPdfPointerPoint(event){
  if (!pdfEditorPageFrame) return null;
  const rect = pdfEditorPageFrame.getBoundingClientRect();
  if (!rect.width || !rect.height) return null;
  return {
    x: Math.min(Math.max((event.clientX - rect.left) / rect.width, 0), 1),
    y: Math.min(Math.max((event.clientY - rect.top) / rect.height, 0), 1),
  };
}

function screenToPdfCoords(point, pageWidth, pageHeight){
  return {
    x: point.x * pageWidth,
    y: pageHeight - (point.y * pageHeight),
  };
}

function pdfToScreenCoords(point, pageWidth, pageHeight){
  return {
    x: pageWidth ? point.x / pageWidth : 0,
    y: pageHeight ? (pageHeight - point.y) / pageHeight : 0,
  };
}

function textItemToViewportRect(item, viewport, pdfjsLib){
  const transform = pdfjsLib.Util.transform(viewport.transform, item.transform);
  const fontSize = Math.max(6, Math.hypot(transform[2], transform[3]));
  const width = Math.max(4, (Number(item.width) || String(item.str || "").length * fontSize * 0.5) * viewport.scale);
  const height = Math.max(6, fontSize * 1.18);
  const left = transform[4];
  const top = transform[5] - fontSize;
  const baseline = transform[5];
  return { left, top, width, height, fontSize, baseline, transform };
}

function normalizePdfHexColor(value, fallback = "#000000"){
  const text = String(value || "").trim();
  if (/^#[0-9a-f]{6}$/i.test(text)) return text;
  if (/^#[0-9a-f]{3}$/i.test(text)){
    return "#" + text.slice(1).split("").map(ch => ch + ch).join("");
  }
  const rgbMatch = text.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/i);
  if (rgbMatch){
    return rgbToHex(Number(rgbMatch[1]), Number(rgbMatch[2]), Number(rgbMatch[3]));
  }
  return fallback;
}

function rgbToHex(r, g, b){
  const toHex = value => Math.min(255, Math.max(0, Math.round(value || 0))).toString(16).padStart(2, "0");
  return `#${toHex(r)}${toHex(g)}${toHex(b)}`;
}

function samplePdfCanvasBackgroundColor(rect){
  if (!pdfEditorCanvas) return "#ffffff";
  const canvas = pdfEditorCanvas;
  const x = Math.max(0, Math.floor(rect.x * canvas.width));
  const y = Math.max(0, Math.floor(rect.y * canvas.height));
  const w = Math.max(1, Math.min(canvas.width - x, Math.ceil(rect.width * canvas.width)));
  const h = Math.max(1, Math.min(canvas.height - y, Math.ceil(rect.height * canvas.height)));
  if (w <= 0 || h <= 0) return "#ffffff";
  try{
    const ctx = canvas.getContext("2d", { willReadFrequently: true });
    const data = ctx.getImageData(x, y, w, h).data;
    const buckets = new Map();
    let fallback = [255, 255, 255, 0];
    for (let i = 0; i < data.length; i += 16){
      const r = data[i];
      const g = data[i + 1];
      const b = data[i + 2];
      const lum = 0.2126 * r + 0.7152 * g + 0.0722 * b;
      if (lum > fallback[3]) fallback = [r, g, b, lum];
      if (lum < 135) continue;
      const qr = Math.round(r / 16) * 16;
      const qg = Math.round(g / 16) * 16;
      const qb = Math.round(b / 16) * 16;
      const key = `${qr},${qg},${qb}`;
      buckets.set(key, (buckets.get(key) || 0) + 1);
    }
    let bestKey = null;
    let bestCount = 0;
    buckets.forEach((count, key) => {
      if (count > bestCount){
        bestKey = key;
        bestCount = count;
      }
    });
    if (bestKey){
      const [r, g, b] = bestKey.split(",").map(Number);
      return rgbToHex(r, g, b);
    }
    return rgbToHex(fallback[0], fallback[1], fallback[2]);
  }catch(error){
    console.warn("PDF background color sampling failed", error);
    return "#ffffff";
  }
}

function viewportRectToNormalized(rect, viewport){
  return clampPdfRect({
    x: rect.left / viewport.width,
    y: rect.top / viewport.height,
    width: rect.width / viewport.width,
    height: rect.height / viewport.height,
  });
}

function rectFromPdfPoints(start, end){
  return clampPdfRect({
    x: Math.min(start.x, end.x),
    y: Math.min(start.y, end.y),
    width: Math.abs(end.x - start.x),
    height: Math.abs(end.y - start.y),
  });
}

function estimateTextEditWidth(text, fontSize, scale = 1){
  const value = String(text || "");
  const units = Array.from(value).reduce((sum, ch) => {
    if (ch === " ") return sum + 0.32;
    if ("ilI.,'`|".includes(ch)) return sum + 0.28;
    if ("mwMW@#%&".includes(ch)) return sum + 0.82;
    if (/[0-9]/.test(ch)) return sum + 0.56;
    return sum + 0.52;
  }, 0);
  return Math.max(fontSize * scale * 0.5, units * fontSize * scale);
}

function getTextEditOriginalBounds(textEdit){
  return textEdit.originalBounds || {
    x: textEdit.x || 0,
    y: textEdit.y || 0,
    width: textEdit.width || 0.01,
    height: textEdit.height || 0.01,
  };
}

function getTextEditReplacementPosition(textEdit){
  return textEdit.replacementPosition || {
    x: textEdit.x || 0,
    y: textEdit.y || 0,
  };
}

function setTextEditReplacementPosition(textEdit, position){
  const fontSize = Math.max(6, Number(textEdit.fontSize) || 12);
  const pageWidthPx = pdfEditorPageFrame?.clientWidth || 1;
  const pageHeightPx = pdfEditorPageFrame?.clientHeight || 1;
  const scale = pdfEditorState.zoom || 1;
  const textWidthPx = estimateTextEditWidth(textEdit.replacementText || textEdit.content || "", fontSize, scale);
  const textHeightPx = fontSize * scale * 1.12;
  const widthNorm = textWidthPx / pageWidthPx;
  const heightNorm = textHeightPx / pageHeightPx;
  const x = Math.min(Math.max(Number(position.x) || 0, 0), Math.max(0, 1 - Math.min(widthNorm, 0.98)));
  const y = Math.min(Math.max(Number(position.y) || 0, 0), Math.max(0, 1 - Math.min(heightNorm, 0.98)));
  textEdit.replacementPosition = { x, y };
  textEdit.replacementBounds = {
    x,
    y,
    width: Math.max(textEdit.width || 0.01, widthNorm),
    height: Math.max(textEdit.height || 0.01, heightNorm),
  };
}

function moveTextEditReplacementBy(textEdit, dx, dy){
  const current = getTextEditReplacementPosition(textEdit);
  setTextEditReplacementPosition(textEdit, {
    x: current.x + dx,
    y: current.y + dy,
  });
}

function resetTextEditReplacementPosition(textEdit){
  const original = getTextEditOriginalBounds(textEdit);
  setTextEditReplacementPosition(textEdit, { x: original.x, y: original.y });
}

function calculateTextEditVisualBounds(textEdit, scale = 1){
  const fontSize = Math.max(6, Number(textEdit.fontSize) || 12);
  const scaledFontSize = fontSize * scale;
  const text = String(textEdit.replacementText ?? textEdit.content ?? "");
  const pageWidthPx = pdfEditorPageFrame?.clientWidth || 1;
  const pageHeightPx = pdfEditorPageFrame?.clientHeight || 1;
  const originalBounds = getTextEditOriginalBounds(textEdit);
  const replacementPosition = getTextEditReplacementPosition(textEdit);
  const original = {
    x: (originalBounds.x || 0) * pageWidthPx,
    y: (originalBounds.y || 0) * pageHeightPx,
    width: Math.max(1, (originalBounds.width || 0.01) * pageWidthPx),
    height: Math.max(1, (originalBounds.height || 0.01) * pageHeightPx),
  };
  const padX = Math.max(2, Math.min(4, scaledFontSize * 0.14));
  const padY = Math.max(1, Math.min(3, scaledFontSize * 0.12));
  const textWidth = estimateTextEditWidth(text, fontSize, scale);
  const textHeight = Math.max(original.height, scaledFontSize * 1.12);
  const originalBaselinePx = Number.isFinite(textEdit.baselineY)
    ? textEdit.baselineY * pageHeightPx
    : original.y + original.height * 0.82;
  const replacement = {
    x: (replacementPosition.x ?? originalBounds.x ?? 0) * pageWidthPx,
    y: (replacementPosition.y ?? originalBounds.y ?? 0) * pageHeightPx,
  };
  const coverLeft = Math.max(0, original.x - padX);
  const coverTop = Math.max(0, Math.min(original.y - padY, originalBaselinePx - (scaledFontSize * 0.86) - padY));
  const coverWidth = Math.max(original.width + padX * 2, textWidth + padX * 2);
  const coverHeight = Math.max(original.height + padY * 2, textHeight + padY * 2);
  const textLeft = replacement.x + padX;
  const textTop = replacement.y;
  const wrapperLeft = Math.min(coverLeft, textLeft - padX);
  const wrapperTop = Math.min(coverTop, textTop - padY);
  const wrapperRight = Math.max(coverLeft + coverWidth, textLeft + textWidth + padX);
  const wrapperBottom = Math.max(coverTop + coverHeight, textTop + textHeight + padY);
  return {
    wrapper: { x: wrapperLeft, y: wrapperTop, width: wrapperRight - wrapperLeft, height: wrapperBottom - wrapperTop },
    cover: { x: coverLeft, y: coverTop, width: coverWidth, height: coverHeight },
    text: { x: textLeft, y: textTop, width: textWidth, height: textHeight, baselineY: textTop + scaledFontSize * 0.86 },
    padding: { x: padX, y: padY },
    fontSize: scaledFontSize,
  };
}

function calculateTextEditPdfBounds(textEdit, pageWidth, pageHeight){
  const fontSize = Math.max(6, Number(textEdit.fontSize) || 12);
  const text = String(textEdit.replacementText ?? textEdit.content ?? "");
  const originalBounds = getTextEditOriginalBounds(textEdit);
  const replacementPosition = getTextEditReplacementPosition(textEdit);
  const original = {
    x: (originalBounds.x || 0) * pageWidth,
    yTop: (originalBounds.y || 0) * pageHeight,
    width: Math.max(1, (originalBounds.width || 0.01) * pageWidth),
    height: Math.max(1, (originalBounds.height || 0.01) * pageHeight),
  };
  const padX = Math.max(2, Math.min(4, fontSize * 0.14));
  const padY = Math.max(1, Math.min(3, fontSize * 0.12));
  const textWidth = estimateTextEditWidth(text, fontSize, 1);
  const textHeight = Math.max(original.height, fontSize * 1.12);
  const baselineFromTop = Number.isFinite(textEdit.baselineY)
    ? textEdit.baselineY * pageHeight
    : original.yTop + original.height * 0.82;
  const originalTextTop = baselineFromTop - (fontSize * 0.86);
  const coverTop = Math.max(0, Math.min(original.yTop - padY, originalTextTop - padY));
  const coverLeft = Math.max(0, original.x - padX);
  const coverWidth = Math.max(original.width + padX * 2, textWidth + padX * 2);
  const coverHeight = Math.max(original.height + padY * 2, textHeight + padY * 2);
  const replacementTop = (replacementPosition.y ?? originalBounds.y ?? 0) * pageHeight;
  const replacementLeft = (replacementPosition.x ?? originalBounds.x ?? 0) * pageWidth;
  return {
    cover: {
      x: coverLeft,
      y: pageHeight - coverTop - coverHeight,
      width: coverWidth,
      height: coverHeight,
    },
    text: {
      x: replacementLeft + padX,
      y: pageHeight - replacementTop - fontSize,
      width: textWidth,
      height: textHeight,
    },
    fontSize,
  };
}

function setPdfEditorTool(tool){
  pdfEditorState.tool = tool || "select";
  if (pdfEditorToolLabel) pdfEditorToolLabel.textContent = getPdfToolLabel(pdfEditorState.tool);
  document.querySelectorAll("[data-pdf-tool]").forEach(btn => {
    btn.classList.toggle("active", btn.dataset.pdfTool === pdfEditorState.tool);
  });
  if (pdfEditorTextLayer){
    pdfEditorTextLayer.classList.toggle("text-edit-active", isPdfTextEditTool());
  }
  if (pdfEditorPageFrame){
    pdfEditorPageFrame.classList.toggle("text-edit-mode", isPdfTextEditTool());
  }
  pdfEditorState.isEditingText = isPdfTextEditTool();
  if (pdfEditorState.tool !== "nativeTextEdit") pdfEditorState.nativeTextEdit = null;
  if (!isPdfTextEditTool()){
    pdfEditorState.hoveredTextBlockId = null;
    pdfEditorState.activeTextBlockId = null;
    pdfEditorState.selectedTextBlockId = null;
    if (pdfEditorTextLayer){
      pdfEditorTextLayer.querySelectorAll(".pdf-text-block").forEach(el => el.classList.remove("selected"));
    }
  }
  if (isPdfTextEditTool() && pdfEditorState.pdfDocument && !pdfEditorState.hasTextLayer){
    setPdfEditorStatus("This PDF has no editable text layer. You can still add text boxes manually.");
  }
  closePdfInlineEditor();
  renderPdfProperties();
}

function selectPdfOverlay(id){
  pdfEditorState.selectedOverlayId = id || null;
  if (id) pdfEditorState.nativeTextEdit = null;
  const overlay = pdfEditorState.overlays.find(item => item.id === pdfEditorState.selectedOverlayId);
  pdfEditorState.selectedTextBlockId = overlay && overlay.textBlockId && overlay.pending ? overlay.textBlockId : null;
  if (pdfEditorTextLayer){
    pdfEditorTextLayer.querySelectorAll(".pdf-text-block").forEach(el => {
      el.classList.toggle("selected", !!pdfEditorState.selectedTextBlockId && el.dataset.textBlockId === pdfEditorState.selectedTextBlockId);
    });
  }
  renderPdfOverlays();
  renderPdfProperties();
}

async function loadPdfIntoEditor(file){
  if (!file || !isPdfFile(file)){
    setPdfEditorStatus("Please choose a valid PDF file.", true);
    return;
  }
  try{
    setPdfEditorStatus("Loading PDF...");
    const pdfjsLib = await ensurePdfJs();
    const bytes = new Uint8Array(await file.arrayBuffer());
    const loadingTask = pdfjsLib.getDocument({ data: new Uint8Array(bytes), useWorkerFetch: false });
    const doc = await loadingTask.promise;
    pdfEditorState.fileName = file.name || "document.pdf";
    pdfEditorState.originalPdfBytes = new Uint8Array(bytes);
    pdfEditorState.workingPdfBytes = new Uint8Array(bytes);
    pdfEditorState.pdfDocument = doc;
    pdfEditorState.pages = Array.from({ length: doc.numPages }, (_, idx) => ({
      id: `page-${idx + 1}-${Date.now()}`,
      sourceIndex: idx,
      rotation: 0,
      deleted: false,
    }));
    pdfEditorState.overlays = [];
    pdfEditorState.activeVisibleIndex = 0;
    pdfEditorState.zoom = 1;
    pdfEditorState.fitWidth = true;
    pdfEditorState.selectedOverlayId = null;
    pdfEditorState.hoveredTextBlockId = null;
    pdfEditorState.activeTextBlockId = null;
    pdfEditorState.selectedTextBlockId = null;
    pdfEditorState.isEditingText = false;
    pdfEditorState.nativeTextEdit = null;
    pdfEditorState.textBlocks = [];
    pdfEditorState.hasTextLayer = false;
    pdfEditorState.activeViewport = null;
    setPdfEditorDirty(false);
    setPdfEditorStatus(doc.numPages > 150 ? "Large PDF loaded. Pages render one at a time for performance." : "PDF loaded.");
    await renderPdfEditor();
  }catch(error){
    console.error("PDF Editor load failed", error);
    setPdfEditorStatus("Could not open this PDF. It may be damaged, encrypted, or unsupported.", true);
  }
}

async function renderPdfEditor(){
  const hasPdf = !!pdfEditorState.pdfDocument;
  if (pdfEditorEmpty) pdfEditorEmpty.hidden = hasPdf;
  if (pdfEditorDocumentWrap) pdfEditorDocumentWrap.hidden = !hasPdf;
  renderPdfThumbnails();
  renderPdfProperties();
  updatePdfToolbarState();
  if (hasPdf){
    await renderPdfActivePage();
  }
}

function updatePdfToolbarState(){
  const visible = getVisiblePdfPages();
  const hasPages = visible.length > 0;
  if (pdfEditorPageIndicator) pdfEditorPageIndicator.textContent = hasPages ? `${pdfEditorState.activeVisibleIndex + 1} / ${visible.length}` : "0 / 0";
  if (pdfEditorPageCount) pdfEditorPageCount.textContent = hasPages ? `${visible.length} page${visible.length === 1 ? "" : "s"}` : "No PDF";
  [
    pdfEditorPrevPage,
    pdfEditorNextPage,
    pdfEditorZoomOut,
    pdfEditorZoomIn,
    pdfEditorFitWidth,
    pdfEditorRotateLeft,
    pdfEditorRotateRight,
    pdfEditorDeletePage,
    pdfEditorAddImage,
    pdfEditorExport,
    pdfEditorSendExtraction,
  ].forEach(btn => { if (btn) btn.disabled = !hasPages; });
  if (pdfEditorPrevPage) pdfEditorPrevPage.disabled = !hasPages || pdfEditorState.activeVisibleIndex <= 0;
  if (pdfEditorNextPage) pdfEditorNextPage.disabled = !hasPages || pdfEditorState.activeVisibleIndex >= visible.length - 1;
}

function renderPdfThumbnails(){
  if (!pdfEditorThumbs) return;
  const visible = getVisiblePdfPages();
  if (!pdfEditorState.pdfDocument || !visible.length){
    pdfEditorThumbs.innerHTML = `<div class="empty-state">${pdfEditorState.pdfDocument ? "All pages are deleted." : "Upload a PDF to see pages."}</div>`;
    updatePdfToolbarState();
    return;
  }
  pdfEditorThumbs.innerHTML = visible.map((page, idx) => `
    <div class="pdf-thumb ${idx === pdfEditorState.activeVisibleIndex ? "active" : ""}" draggable="true" data-page-id="${escapeHtml(page.id)}" data-visible-index="${idx}">
      <div class="pdf-thumb-title">
        <span>Page ${idx + 1}</span>
        <span class="muted">src ${page.sourceIndex + 1}${page.rotation ? ` / ${page.rotation}°` : ""}</span>
      </div>
      <div class="pdf-thumb-box">PDF page ${page.sourceIndex + 1}</div>
    </div>
  `).join("");
  updatePdfToolbarState();
}

async function renderPdfActivePage(){
  const pageState = getActivePdfPageState();
  if (!pageState || !pdfEditorCanvas || !pdfEditorPageFrame || !pdfEditorState.pdfDocument) return;
  const token = ++pdfEditorState.renderToken;
  try{
    setPdfEditorStatus("Rendering page...");
    const page = await pdfEditorState.pdfDocument.getPage(pageState.sourceIndex + 1);
    if (token !== pdfEditorState.renderToken) return;
    const baseViewport = page.getViewport({ scale: 1, rotation: pageState.rotation });
    let scale = pdfEditorState.zoom;
    if (pdfEditorState.fitWidth && pdfEditorStage){
      const available = Math.max(320, pdfEditorStage.clientWidth - 70);
      scale = Math.min(2.2, Math.max(0.45, available / baseViewport.width));
      pdfEditorState.zoom = scale;
    }
    const viewport = page.getViewport({ scale, rotation: pageState.rotation });
    pdfEditorState.activeViewport = viewport;
    const outputScale = Math.min(window.devicePixelRatio || 1, 2);
    const canvas = pdfEditorCanvas;
    const ctx = canvas.getContext("2d", { alpha: false });
    canvas.width = Math.floor(viewport.width * outputScale);
    canvas.height = Math.floor(viewport.height * outputScale);
    canvas.style.width = `${viewport.width}px`;
    canvas.style.height = `${viewport.height}px`;
    pdfEditorPageFrame.style.width = `${viewport.width}px`;
    pdfEditorPageFrame.style.height = `${viewport.height}px`;
    pdfEditorPageFrame.classList.toggle("text-edit-mode", isPdfTextEditTool());
    ctx.setTransform(outputScale, 0, 0, outputScale, 0, 0);
    await page.render({ canvasContext: ctx, viewport }).promise;
    if (token !== pdfEditorState.renderToken) return;
    const hasTextLayer = await renderPdfTextLayer(page, viewport);
    renderPdfOverlays();
    if (hasTextLayer) setPdfEditorStatus("");
  }catch(error){
    console.error("PDF Editor render failed", error);
    setPdfEditorStatus("Page render failed. Try a different PDF or reduce zoom.", true);
  }
}

async function renderPdfTextLayer(page, viewport){
  if (!pdfEditorTextLayer) return;
  pdfEditorTextLayer.innerHTML = "";
  pdfEditorState.textBlocks = [];
  pdfEditorState.hasTextLayer = false;
  pdfEditorTextLayer.classList.toggle("text-edit-active", isPdfTextEditTool());
  try{
    const pdfjsLib = await ensurePdfJs();
    const text = await page.getTextContent();
    const items = (text.items || []).filter(item => String(item.str || "").trim());
    if (!items.length){
      setPdfEditorStatus("This PDF has no editable text layer. You can still add text boxes manually.");
      renderPdfProperties();
      return false;
    }
    const pageState = getActivePdfPageState();
    pdfEditorState.hasTextLayer = true;
    const textStyles = text.styles || {};
    const frag = document.createDocumentFragment();
    items.slice(0, 1200).forEach((item, index) => {
      const rect = textItemToViewportRect(item, viewport, pdfjsLib);
      const normalized = viewportRectToNormalized(rect, viewport);
      const style = textStyles[item.fontName] || {};
      const fontFamily = style.fontFamily || item.fontName || "Helvetica";
      const color = normalizePdfHexColor(style.color || style.fill || style.fillStyle, "#000000");
      const block = {
        id: `pdf-text-${pageState ? pageState.sourceIndex : 0}-${index}`,
        pageIndex: pageState ? pageState.sourceIndex : 0,
        text: item.str,
        x: normalized.x,
        y: normalized.y,
        width: normalized.width,
        height: normalized.height,
        fontSize: Math.max(6, rect.fontSize / viewport.scale),
        fontFamily,
        color,
        backgroundColor: samplePdfCanvasBackgroundColor(normalized),
        baselineY: rect.baseline / viewport.height,
        transform: Array.isArray(item.transform) ? item.transform.slice() : [],
        viewportTransform: Array.isArray(rect.transform) ? rect.transform.slice() : [],
        pageScale: viewport.scale,
        pageRotation: pageState ? pageState.rotation : 0,
        sourceWidth: normalized.width,
        sourceHeight: normalized.height,
        hasTextLayer: true,
      };
      pdfEditorState.textBlocks.push(block);
      const span = document.createElement("span");
      span.className = "pdf-text-block";
      span.classList.toggle("selected", block.id === pdfEditorState.selectedTextBlockId);
      span.dataset.textBlockId = block.id;
      span.dataset.originalText = item.str;
      span.style.left = `${rect.left}px`;
      span.style.top = `${rect.top}px`;
      span.style.width = `${rect.width}px`;
      span.style.height = `${rect.height}px`;
      span.title = "Click to edit this text";
      span.addEventListener("mouseenter", () => {
        if (isPdfTextEditTool()) pdfEditorState.hoveredTextBlockId = block.id;
      });
      span.addEventListener("mouseleave", () => {
        if (pdfEditorState.hoveredTextBlockId === block.id) pdfEditorState.hoveredTextBlockId = null;
      });
      span.addEventListener("click", event => {
        if (!isPdfTextEditTool()) return;
        event.preventDefault();
        event.stopPropagation();
        if (pdfEditorState.tool === "nativeTextEdit"){
          selectNativePdfTextBlock(block.id);
        }else{
          selectPdfTextBlock(block.id);
        }
      });
      frag.appendChild(span);
    });
    pdfEditorTextLayer.appendChild(frag);
    renderPdfProperties();
    return true;
  }catch(error){
    console.warn("PDF text layer unavailable", error);
    setPdfEditorStatus("This PDF has no editable text layer. You can still add text boxes manually.");
    renderPdfProperties();
    return false;
  }
}

function openPdfInlineTextEditor(span){
  if (!pdfEditorInlineEditor || !pdfEditorInlineText || !pdfEditorPageFrame) return;
  const pageState = getActivePdfPageState();
  if (!pageState) return;
  const frameRect = pdfEditorPageFrame.getBoundingClientRect();
  const spanRect = span.getBoundingClientRect();
  pdfEditorState.inlineTextBlock = {
    pageIndex: pageState.sourceIndex,
    originalText: span.dataset.originalText || "",
    x: (spanRect.left - frameRect.left) / frameRect.width,
    y: (spanRect.top - frameRect.top) / frameRect.height,
    width: spanRect.width / frameRect.width,
    height: spanRect.height / frameRect.height,
  };
  pdfEditorInlineText.value = pdfEditorState.inlineTextBlock.originalText;
  pdfEditorInlineEditor.style.left = `${Math.max(8, spanRect.left - frameRect.left)}px`;
  pdfEditorInlineEditor.style.top = `${Math.max(8, spanRect.top - frameRect.top + spanRect.height + 6)}px`;
  pdfEditorInlineEditor.hidden = false;
  pdfEditorInlineText.focus();
  pdfEditorInlineText.select();
}

function selectPdfTextBlock(blockId){
  const block = pdfEditorState.textBlocks.find(item => item.id === blockId);
  if (!block) return;
  pdfEditorState.nativeTextEdit = null;
  pdfEditorState.selectedTextBlockId = block.id;
  pdfEditorState.activeTextBlockId = block.id;
  pdfEditorState.isEditingText = true;
  let overlay = pdfEditorState.overlays.find(item => item.type === "textEdit" && item.textBlockId === block.id);
  if (!overlay){
    overlay = createPdfOverlay("textEdit", block.pageIndex, clampPdfRect(block), {
      originalText: block.text,
      replacementText: block.text,
      content: block.text,
      fontSize: Math.max(6, Number((block.fontSize || 12).toFixed(1))),
      fontFamily: block.fontFamily || "Helvetica",
      color: normalizePdfHexColor(block.color, "#000000"),
      backgroundColor: normalizePdfHexColor(block.backgroundColor, "#ffffff"),
      detectedFromTextLayer: true,
      baselineY: block.baselineY,
      transform: block.transform,
      pageScale: block.pageScale,
      pageRotation: block.pageRotation,
      sourceWidth: block.sourceWidth,
      sourceHeight: block.sourceHeight,
      pending: true,
      textBlockId: block.id,
    });
    pdfEditorState.overlays.push(overlay);
  }
  pdfEditorState.selectedOverlayId = overlay.id;
  if (pdfEditorTextLayer){
    pdfEditorTextLayer.querySelectorAll(".pdf-text-block").forEach(el => {
      el.classList.toggle("selected", el.dataset.textBlockId === block.id);
    });
  }
  renderPdfOverlays();
  renderPdfProperties();
}

function selectNativePdfTextBlock(blockId){
  const block = pdfEditorState.textBlocks.find(item => item.id === blockId);
  if (!block) return;
  pdfEditorState.selectedOverlayId = null;
  pdfEditorState.selectedTextBlockId = block.id;
  pdfEditorState.activeTextBlockId = block.id;
  pdfEditorState.isEditingText = true;
  pdfEditorState.nativeTextEdit = {
    blockId: block.id,
    pageIndex: block.pageIndex,
    originalText: block.text,
    replacementText: block.text,
    bounds: {
      x: block.x,
      y: block.y,
      width: block.width,
      height: block.height,
    },
    fontSize: Math.max(6, Number((block.fontSize || 12).toFixed(1))),
    fontFamily: block.fontFamily || "Helvetica",
    color: normalizePdfHexColor(block.color, "#000000"),
    nativeEditMode: "content_stream_first",
    preserveNearbyLines: true,
    warnings: [],
    status: "Native edit available",
    reason: "",
  };
  if (pdfEditorTextLayer){
    pdfEditorTextLayer.querySelectorAll(".pdf-text-block").forEach(el => {
      el.classList.toggle("selected", el.dataset.textBlockId === block.id);
    });
  }
  setPdfEditorStatus("Native edit available");
  renderPdfOverlays();
  renderPdfProperties();
}

function updateNativePdfTextEditProp(prop, value){
  const edit = pdfEditorState.nativeTextEdit;
  if (!edit) return;
  if (prop === "replacementText"){
    edit.replacementText = value;
  }else if (prop === "fontSize"){
    edit.fontSize = Math.min(96, Math.max(6, Number(value) || 12));
  }else if (prop === "color"){
    edit.color = normalizePdfHexColor(value, "#000000");
  }else if (prop === "preserveNearbyLines"){
    edit.preserveNearbyLines = !!value;
  }
  edit.status = "Native edit available";
  edit.reason = "";
}

async function reloadPdfEditorWorkingPdf(bytes){
  const pdfjsLib = await ensurePdfJs();
  const nextBytes = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
  const loadingTask = pdfjsLib.getDocument({ data: new Uint8Array(nextBytes), useWorkerFetch: false });
  const doc = await loadingTask.promise;
  pdfEditorState.workingPdfBytes = new Uint8Array(nextBytes);
  pdfEditorState.pdfDocument = doc;
  if (pdfEditorState.pages.length !== doc.numPages){
    pdfEditorState.pages = Array.from({ length: doc.numPages }, (_, idx) => ({
      id: `page-${idx + 1}-${Date.now()}`,
      sourceIndex: idx,
      rotation: 0,
      deleted: false,
    }));
    pdfEditorState.activeVisibleIndex = 0;
  }
  pdfEditorState.textBlocks = [];
  pdfEditorState.hasTextLayer = false;
  pdfEditorState.activeViewport = null;
}

async function applyNativePdfTextEdit(){
  const edit = pdfEditorState.nativeTextEdit;
  const bytes = pdfEditorState.workingPdfBytes || pdfEditorState.originalPdfBytes;
  if (!edit || !bytes) return;
  try{
    edit.status = "Trying native replacement...";
    edit.reason = "";
    setPdfEditorStatus("Trying native replacement...");
    renderPdfProperties();
    const res = await fetch(API_BASE + "/api/pdf-editor/native-text-replace", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        pdfBase64: bytesToBase64(bytes),
        pageIndex: edit.pageIndex,
        originalText: edit.originalText,
        replacementText: edit.replacementText,
        bounds: edit.bounds,
        fontSize: edit.fontSize,
        fontFamily: edit.fontFamily,
        nativeEditMode: edit.nativeEditMode || "content_stream_first",
        preserveNearbyLines: edit.preserveNearbyLines !== false,
      }),
    });
    const payload = await res.json().catch(() => ({}));
    if (!res.ok){
      throw new Error(payload.detail || payload.reason || `HTTP ${res.status}`);
    }
    if (!payload.success || !payload.editedPdfBase64){
      edit.status = "Native edit failed. Use Safe Edit instead.";
      edit.reason = payload.reason || "UNKNOWN_NATIVE_EDIT_ERROR";
      edit.warnings = Array.isArray(payload.warnings) ? payload.warnings : [];
      setPdfEditorStatus(`Native edit failed. Use Safe Edit instead. ${edit.reason}`, true);
      renderPdfProperties();
      return;
    }
    const nativeWarnings = Array.isArray(payload.warnings) ? payload.warnings : [];
    await reloadPdfEditorWorkingPdf(base64ToBytes(payload.editedPdfBase64));
    pdfEditorState.nativeTextEdit = null;
    pdfEditorState.selectedOverlayId = null;
    pdfEditorState.hoveredTextBlockId = null;
    pdfEditorState.activeTextBlockId = null;
    pdfEditorState.selectedTextBlockId = null;
    pdfEditorState.isEditingText = false;
    setPdfEditorDirty(true);
    await renderPdfEditor();
    if (nativeWarnings.length){
      setPdfEditorStatus("Native edit used cleanup fallback and may affect background details.", true);
    }else{
      setPdfEditorStatus("Native edit applied.");
    }
  }catch(error){
    console.error("Native PDF text edit failed", error);
    if (pdfEditorState.nativeTextEdit){
      pdfEditorState.nativeTextEdit.status = "Native edit failed. Use Safe Edit instead.";
      pdfEditorState.nativeTextEdit.reason = error.message || "UNKNOWN_NATIVE_EDIT_ERROR";
      pdfEditorState.nativeTextEdit.warnings = [];
    }
    setPdfEditorStatus("Native edit failed. Use Safe Edit instead. " + (error.message || error), true);
    renderPdfProperties();
  }
}

function fallbackNativeToSafeTextEdit(){
  const edit = pdfEditorState.nativeTextEdit;
  if (!edit) return;
  const replacement = edit.replacementText;
  setPdfEditorTool("textEdit");
  selectPdfTextBlock(edit.blockId);
  const overlay = pdfEditorState.overlays.find(item => item.id === pdfEditorState.selectedOverlayId && item.type === "textEdit");
  if (overlay){
    overlay.replacementText = replacement;
    overlay.content = replacement;
    overlay.fontSize = edit.fontSize || overlay.fontSize;
    overlay.color = edit.color || overlay.color;
  }
  setPdfEditorStatus("Safe Text Edit fallback is ready.");
  renderPdfOverlays();
  renderPdfProperties();
}

function applyPendingPdfTextEdit(){
  const overlay = pdfEditorState.overlays.find(item => item.id === pdfEditorState.selectedOverlayId && item.type === "textEdit");
  if (!overlay) return;
  overlay.pending = false;
  overlay.applied = true;
  overlay.content = overlay.replacementText || "";
  setPdfEditorDirty(true);
  setPdfEditorStatus("Text replacement saved for export.");
  pdfEditorState.selectedOverlayId = pdfEditorState.tool === "select" ? overlay.id : null;
  pdfEditorState.hoveredTextBlockId = null;
  pdfEditorState.selectedTextBlockId = null;
  pdfEditorState.activeTextBlockId = null;
  pdfEditorState.isEditingText = false;
  if (pdfEditorTextLayer){
    pdfEditorTextLayer.querySelectorAll(".pdf-text-block").forEach(el => el.classList.remove("selected"));
  }
  renderPdfOverlays();
  renderPdfProperties();
}

function cancelPendingPdfTextEdit(){
  const overlay = pdfEditorState.overlays.find(item => item.id === pdfEditorState.selectedOverlayId && item.type === "textEdit");
  if (!overlay) return;
  if (overlay.pending){
    pdfEditorState.overlays = pdfEditorState.overlays.filter(item => item.id !== overlay.id);
  }
  pdfEditorState.selectedOverlayId = null;
  pdfEditorState.selectedTextBlockId = null;
  pdfEditorState.activeTextBlockId = null;
  pdfEditorState.isEditingText = false;
  if (pdfEditorTextLayer){
    pdfEditorTextLayer.querySelectorAll(".pdf-text-block").forEach(el => el.classList.remove("selected"));
  }
  renderPdfOverlays();
  renderPdfProperties();
}

function closePdfInlineEditor(){
  pdfEditorState.inlineTextBlock = null;
  if (pdfEditorInlineEditor) pdfEditorInlineEditor.hidden = true;
}

function savePdfInlineTextEdit(){
  const block = pdfEditorState.inlineTextBlock;
  if (!block || !pdfEditorInlineText) return;
  const replacement = pdfEditorInlineText.value;
  if (replacement !== block.originalText){
    const overlay = createPdfOverlay("textEdit", block.pageIndex, clampPdfRect(block), {
      originalText: block.originalText,
      replacementText: replacement,
      content: replacement,
      fontSize: Math.max(9, Math.round(block.height * (pdfEditorPageFrame?.clientHeight || 720) * 0.82)),
      backgroundColor: "#ffffff",
      detectedFromTextLayer: true,
    });
    pdfEditorState.overlays.push(overlay);
    pdfEditorState.selectedOverlayId = overlay.id;
    setPdfEditorDirty(true);
  }
  closePdfInlineEditor();
  renderPdfOverlays();
  renderPdfProperties();
}

function renderPdfOverlays(){
  if (!pdfEditorOverlayLayer || !pdfEditorPageFrame) return;
  const pageState = getActivePdfPageState();
  pdfEditorOverlayLayer.innerHTML = "";
  if (!pageState) return;
  const pageOverlays = pdfEditorState.overlays.filter(overlay => overlay.pageIndex === pageState.sourceIndex);
  const width = pdfEditorPageFrame.clientWidth || 1;
  const height = pdfEditorPageFrame.clientHeight || 1;
  pageOverlays.forEach(overlay => {
    const el = document.createElement("div");
    el.className = `pdf-overlay-item pdf-overlay-${String(overlay.type || "").toLowerCase()}`;
    if (overlay.type === "textEdit") el.classList.add("text-edit-overlay");
    const isSelected = overlay.id === pdfEditorState.selectedOverlayId;
    const isAppliedTextEdit = overlay.type === "textEdit" && !overlay.pending;
    const showTextEditSelectOutline = isAppliedTextEdit && isSelected && pdfEditorState.tool === "select";
    el.classList.toggle("selected", isSelected && (!isAppliedTextEdit || showTextEditSelectOutline));
    el.classList.toggle("pending", !!overlay.pending);
    el.classList.toggle("applied", isAppliedTextEdit);
    el.classList.toggle("select-outline", showTextEditSelectOutline);
    el.dataset.overlayId = overlay.id;
    const visualBounds = overlay.type === "textEdit" ? calculateTextEditVisualBounds(overlay, pdfEditorState.zoom || 1) : null;
    el.style.left = `${visualBounds ? visualBounds.wrapper.x : overlay.x * width}px`;
    el.style.top = `${visualBounds ? visualBounds.wrapper.y : overlay.y * height}px`;
    el.style.width = `${visualBounds ? visualBounds.wrapper.width : overlay.width * width}px`;
    el.style.height = `${visualBounds ? visualBounds.wrapper.height : overlay.height * height}px`;
    el.style.opacity = String(overlay.opacity ?? 1);
    if (overlay.type === "textEdit"){
      el.style.backgroundColor = "transparent";
      el.style.borderColor = overlay.pending || showTextEditSelectOutline ? "rgba(47,107,255,.8)" : "transparent";
    }
    if (overlay.type === "textEdit"){
      const cover = document.createElement("div");
      cover.className = "text-edit-cover";
      cover.style.left = `${visualBounds.cover.x - visualBounds.wrapper.x}px`;
      cover.style.top = `${visualBounds.cover.y - visualBounds.wrapper.y}px`;
      cover.style.width = `${visualBounds.cover.width}px`;
      cover.style.height = `${visualBounds.cover.height}px`;
      cover.style.backgroundColor = overlay.backgroundColor || "#ffffff";
      const textEl = document.createElement("div");
      textEl.className = "text-edit-replacement-text";
      textEl.textContent = overlay.replacementText || "";
      textEl.style.left = `${visualBounds.text.x - visualBounds.wrapper.x}px`;
      textEl.style.top = `${visualBounds.text.y - visualBounds.wrapper.y}px`;
      textEl.style.color = overlay.color || "#000000";
      textEl.style.fontFamily = "Arial, Helvetica, sans-serif";
      textEl.style.fontSize = `${Math.max(6, visualBounds.fontSize)}px`;
      textEl.style.lineHeight = "1";
      textEl.style.textAlign = overlay.alignment || "left";
      textEl.addEventListener("pointerdown", event => startTextEditReplacementDrag(event, overlay));
      el.appendChild(cover);
      el.appendChild(textEl);
    }else if (overlay.type === "text"){
      el.textContent = overlay.content;
      el.style.color = overlay.color || "#000000";
      el.style.fontFamily = "Arial, Helvetica, sans-serif";
      el.style.fontSize = `${Math.max(6, (overlay.fontSize || 14) * (pdfEditorState.zoom || 1))}px`;
      el.style.lineHeight = "1";
      el.style.textAlign = overlay.alignment || "left";
    }else if (overlay.type === "drawing"){
      const points = (overlay.path || []).map(point => `${(point.x - overlay.x) / overlay.width * 100},${(point.y - overlay.y) / overlay.height * 100}`).join(" ");
      el.innerHTML = `<svg viewBox="0 0 100 100" preserveAspectRatio="none"><polyline points="${escapeHtml(points)}" fill="none" stroke="${escapeHtml(overlay.color || "#111827")}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    }else if ((overlay.type === "image" || overlay.type === "signature") && overlay.imageDataUrl){
      el.innerHTML = `<img src="${escapeHtml(overlay.imageDataUrl)}" alt="PDF overlay image">`;
    }
    el.addEventListener("click", event => {
      event.stopPropagation();
      selectPdfOverlay(overlay.id);
    });
    pdfEditorOverlayLayer.appendChild(el);
  });
}

function startTextEditReplacementDrag(event, overlay){
  if (!overlay || overlay.type !== "textEdit") return;
  if (pdfEditorState.tool !== "select" && pdfEditorState.tool !== "textEdit") return;
  event.preventDefault();
  event.stopPropagation();
  pdfEditorState.selectedOverlayId = overlay.id;
  pdfEditorState.drag = {
    type: "moveTextEdit",
    overlayId: overlay.id,
    startClientX: event.clientX,
    startClientY: event.clientY,
    startPosition: { ...getTextEditReplacementPosition(overlay) },
  };
  renderPdfOverlays();
  renderPdfProperties();
}

function handleTextEditReplacementDrag(event){
  const drag = pdfEditorState.drag;
  if (!drag || drag.type !== "moveTextEdit" || !pdfEditorPageFrame) return false;
  const overlay = pdfEditorState.overlays.find(item => item.id === drag.overlayId && item.type === "textEdit");
  if (!overlay) return false;
  const frameWidth = pdfEditorPageFrame.clientWidth || 1;
  const frameHeight = pdfEditorPageFrame.clientHeight || 1;
  const dx = (event.clientX - drag.startClientX) / frameWidth;
  const dy = (event.clientY - drag.startClientY) / frameHeight;
  setTextEditReplacementPosition(overlay, {
    x: drag.startPosition.x + dx,
    y: drag.startPosition.y + dy,
  });
  renderPdfOverlays();
  renderPdfProperties();
  event.preventDefault();
  return true;
}

function endTextEditReplacementDrag(){
  const drag = pdfEditorState.drag;
  if (!drag || drag.type !== "moveTextEdit") return false;
  pdfEditorState.drag = null;
  setPdfEditorDirty(true);
  renderPdfOverlays();
  renderPdfProperties();
  return true;
}

function nudgeSelectedTextEdit(dxPx, dyPx){
  const overlay = pdfEditorState.overlays.find(item => item.id === pdfEditorState.selectedOverlayId && item.type === "textEdit");
  if (!overlay || !pdfEditorPageFrame) return false;
  moveTextEditReplacementBy(
    overlay,
    dxPx / (pdfEditorPageFrame.clientWidth || 1),
    dyPx / (pdfEditorPageFrame.clientHeight || 1),
  );
  setPdfEditorDirty(true);
  renderPdfOverlays();
  renderPdfProperties();
  return true;
}

function renderPdfProperties(){
  if (!pdfEditorSelectedProps || !pdfEditorTextEdits) return;
  const selected = pdfEditorState.overlays.find(overlay => overlay.id === pdfEditorState.selectedOverlayId);
  if (pdfEditorState.nativeTextEdit){
    const edit = pdfEditorState.nativeTextEdit;
    pdfEditorSelectedProps.className = "";
    pdfEditorSelectedProps.innerHTML = `
      <label class="pdf-prop-field">
        <span>Type</span>
        <input type="text" value="nativeTextEdit" readonly>
      </label>
      <label class="pdf-prop-field">
        <span>Status</span>
        <input type="text" value="${escapeHtml(edit.status || "Native edit available")}" readonly>
      </label>
      ${edit.reason ? `
        <div class="empty-state">${escapeHtml(edit.reason)}</div>
      ` : ""}
      <label class="pdf-prop-field">
        <span>Selected original text</span>
        <textarea readonly>${escapeHtml(edit.originalText || "")}</textarea>
      </label>
      <label class="pdf-prop-field">
        <span>Replacement text</span>
        <textarea data-pdf-native-prop="replacementText">${escapeHtml(edit.replacementText || "")}</textarea>
      </label>
      <label class="pdf-prop-field">
        <span>Font size</span>
        <input type="number" min="6" max="96" data-pdf-native-prop="fontSize" value="${Number(edit.fontSize || 12)}">
      </label>
      <label class="pdf-prop-field">
        <span>Text color</span>
        <input type="color" data-pdf-native-prop="color" value="${escapeHtml(normalizePdfHexColor(edit.color, "#000000"))}">
      </label>
      <label class="pdf-prop-field pdf-check-field">
        <span>Preserve background / lines</span>
        <input type="checkbox" data-pdf-native-prop="preserveNearbyLines" ${edit.preserveNearbyLines !== false ? "checked" : ""}>
      </label>
      ${Array.isArray(edit.warnings) && edit.warnings.length ? `
        <div class="empty-state">Native edit used cleanup fallback and may affect background details.</div>
      ` : ""}
      <div class="controls" style="margin:0">
        <button type="button" class="btn primary small" id="pdfEditorApplyNativeTextEdit">Apply Native Edit</button>
        <button type="button" class="btn small" id="pdfEditorFallbackSafeTextEdit">Use Safe Edit Instead</button>
      </div>
    `;
  }else if (!selected){
    pdfEditorSelectedProps.className = "empty-state";
    if (isPdfTextEditTool() && pdfEditorState.pdfDocument && !pdfEditorState.hasTextLayer){
      pdfEditorSelectedProps.innerHTML = "This PDF has no editable text layer. You can still add text boxes manually.";
    }else if (pdfEditorState.tool === "textEdit"){
      pdfEditorSelectedProps.innerHTML = "Safe Text Edit covers original text and draws replacement text. Click a highlighted text block to edit it.";
    }else if (pdfEditorState.tool === "nativeTextEdit"){
      pdfEditorSelectedProps.innerHTML = "Native Text Edit attempts to replace real PDF text. Click a highlighted text block to start.";
    }else{
      pdfEditorSelectedProps.innerHTML = "Select an annotation or editable text block.";
    }
  }else{
    pdfEditorSelectedProps.className = "";
    const isText = selected.type === "text" || selected.type === "textEdit";
    pdfEditorSelectedProps.innerHTML = `
      <label class="pdf-prop-field">
        <span>Type</span>
        <input type="text" value="${escapeHtml(selected.type)}" readonly>
      </label>
      ${selected.type === "textEdit" ? `
        <label class="pdf-prop-field">
          <span>Selected original text</span>
          <textarea readonly>${escapeHtml(selected.originalText || "")}</textarea>
        </label>
      ` : ""}
      ${isText ? `
        <label class="pdf-prop-field">
          <span>${selected.type === "textEdit" ? "Replacement text" : "Text"}</span>
          <textarea data-pdf-prop="content">${escapeHtml(selected.type === "textEdit" ? selected.replacementText : selected.content)}</textarea>
        </label>
        <label class="pdf-prop-field">
          <span>Font size</span>
          <input type="number" min="6" max="96" data-pdf-prop="fontSize" value="${Number(selected.fontSize || 14)}">
        </label>
        <label class="pdf-prop-field">
          <span>Color</span>
          <input type="color" data-pdf-prop="color" value="${escapeHtml(normalizePdfHexColor(selected.color, "#000000"))}">
        </label>
        ${selected.type === "textEdit" ? `
          <label class="pdf-prop-field">
            <span>Cover color</span>
            <input type="color" data-pdf-prop="backgroundColor" value="${escapeHtml(normalizePdfHexColor(selected.backgroundColor, "#ffffff"))}">
          </label>
          <label class="pdf-prop-field">
            <span>X position</span>
            <input type="number" min="0" max="100" step="0.1" data-pdf-prop="replacementX" value="${Number(((getTextEditReplacementPosition(selected).x || 0) * 100).toFixed(1))}">
          </label>
          <label class="pdf-prop-field">
            <span>Y position</span>
            <input type="number" min="0" max="100" step="0.1" data-pdf-prop="replacementY" value="${Number(((getTextEditReplacementPosition(selected).y || 0) * 100).toFixed(1))}">
          </label>
        ` : ""}
      ` : ""}
      <label class="pdf-prop-field">
        <span>Opacity</span>
        <input type="range" min="0.1" max="1" step="0.05" data-pdf-prop="opacity" value="${Number(selected.opacity ?? 1)}">
      </label>
      ${selected.type === "textEdit" ? `
        <div class="controls" style="margin:0">
          <button type="button" class="btn primary small" id="pdfEditorApplyTextEdit">${selected.pending ? "Apply" : "Update"}</button>
          <button type="button" class="btn small" id="pdfEditorResetTextPosition">Reset position</button>
          <button type="button" class="btn small" id="pdfEditorCancelTextEdit">Cancel</button>
        </div>
      ` : ""}
      <button type="button" class="btn danger small" id="pdfEditorDeleteOverlay">Delete selected</button>
    `;
  }
  const textEdits = pdfEditorState.overlays.filter(overlay => overlay.type === "textEdit" && !overlay.pending);
  pdfEditorTextEdits.innerHTML = textEdits.length ? textEdits.map(overlay => `
    <button type="button" class="pdf-edit-row" data-select-pdf-overlay="${escapeHtml(overlay.id)}">
      <strong>Page ${overlay.pageIndex + 1}</strong><br>
      <span>${escapeHtml(overlay.originalText || "")}</span><br>
      <span>→ ${escapeHtml(overlay.replacementText || "")}</span>
    </button>
  `).join("") : `<div class="muted small">No text replacements yet.</div>`;
}

function updateSelectedPdfOverlayProp(prop, value){
  const overlay = pdfEditorState.overlays.find(item => item.id === pdfEditorState.selectedOverlayId);
  if (!overlay) return;
  if (prop === "content"){
    if (overlay.type === "textEdit"){
      overlay.replacementText = value;
      overlay.content = value;
      setTextEditReplacementPosition(overlay, getTextEditReplacementPosition(overlay));
    }else{
      overlay.content = value;
    }
  }else if (prop === "fontSize"){
    overlay.fontSize = Math.min(96, Math.max(6, Number(value) || 14));
    if (overlay.type === "textEdit") setTextEditReplacementPosition(overlay, getTextEditReplacementPosition(overlay));
  }else if (prop === "opacity"){
    overlay.opacity = Math.min(1, Math.max(0.1, Number(value) || 1));
  }else if (prop === "color" || prop === "backgroundColor"){
    overlay[prop] = value;
  }else if (prop === "replacementX" || prop === "replacementY"){
    const current = getTextEditReplacementPosition(overlay);
    setTextEditReplacementPosition(overlay, {
      x: prop === "replacementX" ? (Number(value) || 0) / 100 : current.x,
      y: prop === "replacementY" ? (Number(value) || 0) / 100 : current.y,
    });
  }
  if (!overlay.pending) setPdfEditorDirty(true);
  renderPdfOverlays();
}

function addPdfOverlayAtPoint(point){
  const pageState = getActivePdfPageState();
  if (!pageState) return;
  let overlay = null;
  if (pdfEditorState.tool === "text"){
    overlay = createPdfOverlay("text", pageState.sourceIndex, clampPdfRect({ x: point.x, y: point.y, width: 0.22, height: 0.045 }), {
      content: "New text",
    });
  }
  if (!overlay) return;
  pdfEditorState.overlays.push(overlay);
  pdfEditorState.selectedOverlayId = overlay.id;
  setPdfEditorDirty(true);
  renderPdfOverlays();
  renderPdfProperties();
}

function finishPdfShapeOverlay(endPoint){
  const drag = pdfEditorState.drag;
  const pageState = getActivePdfPageState();
  if (!drag || !pageState) return;
  if (drag.type === "rectangle" || drag.type === "highlight"){
    const rect = rectFromPdfPoints(drag.start, endPoint);
    if (rect.width > 0.015 && rect.height > 0.015){
      const overlay = createPdfOverlay(drag.type, pageState.sourceIndex, rect, {});
      pdfEditorState.overlays.push(overlay);
      pdfEditorState.selectedOverlayId = overlay.id;
      setPdfEditorDirty(true);
    }
  }else if (drag.type === "drawing" && drag.path.length > 1){
    const xs = drag.path.map(point => point.x);
    const ys = drag.path.map(point => point.y);
    const rect = clampPdfRect({
      x: Math.min(...xs),
      y: Math.min(...ys),
      width: Math.max(...xs) - Math.min(...xs),
      height: Math.max(...ys) - Math.min(...ys),
    });
    if (rect.width > 0.005 && rect.height > 0.005){
      const overlay = createPdfOverlay("drawing", pageState.sourceIndex, rect, {
        path: drag.path,
        color: "#111827",
      });
      pdfEditorState.overlays.push(overlay);
      pdfEditorState.selectedOverlayId = overlay.id;
      setPdfEditorDirty(true);
    }
  }
  pdfEditorState.drag = null;
  renderPdfOverlays();
  renderPdfProperties();
}

async function addPdfImageOverlay(file){
  const pageState = getActivePdfPageState();
  if (!file || !pageState) return;
  const dataUrl = await fileToDataURL(file);
  const overlay = createPdfOverlay("signature", pageState.sourceIndex, { x: 0.12, y: 0.12, width: 0.28, height: 0.12 }, {
    imageDataUrl: dataUrl,
  });
  pdfEditorState.overlays.push(overlay);
  pdfEditorState.selectedOverlayId = overlay.id;
  setPdfEditorDirty(true);
  renderPdfOverlays();
  renderPdfProperties();
}

function movePdfPage(fromVisibleIndex, toVisibleIndex){
  const visible = getVisiblePdfPages();
  const fromPage = visible[fromVisibleIndex];
  const toPage = visible[toVisibleIndex];
  if (!fromPage || !toPage || fromPage.id === toPage.id) return;
  const fromRaw = pdfEditorState.pages.findIndex(page => page.id === fromPage.id);
  const toRaw = pdfEditorState.pages.findIndex(page => page.id === toPage.id);
  const [page] = pdfEditorState.pages.splice(fromRaw, 1);
  pdfEditorState.pages.splice(toRaw, 0, page);
  pdfEditorState.activeVisibleIndex = toVisibleIndex;
  setPdfEditorDirty(true);
  renderPdfEditor();
}

function rotateActivePdfPage(delta){
  const page = getActivePdfPageState();
  if (!page) return;
  page.rotation = ((page.rotation + delta) % 360 + 360) % 360;
  setPdfEditorDirty(true);
  renderPdfEditor();
}

function deleteActivePdfPage(){
  const visible = getVisiblePdfPages();
  const page = getActivePdfPageState();
  if (!page) return;
  page.deleted = true;
  pdfEditorState.selectedOverlayId = null;
  pdfEditorState.activeVisibleIndex = Math.min(pdfEditorState.activeVisibleIndex, Math.max(0, visible.length - 2));
  setPdfEditorDirty(true);
  renderPdfEditor();
}

function hexToPdfRgb(hex){
  const value = String(hex || "#000000").replace("#", "");
  const n = parseInt(value.length === 3 ? value.split("").map(ch => ch + ch).join("") : value, 16);
  if (!Number.isFinite(n)) return PDFLib.rgb(0, 0, 0);
  return PDFLib.rgb(((n >> 16) & 255) / 255, ((n >> 8) & 255) / 255, (n & 255) / 255);
}

function pdfRectToPage(rect, pageWidth, pageHeight){
  const width = rect.width * pageWidth;
  const height = rect.height * pageHeight;
  const topLeft = screenToPdfCoords({ x: rect.x, y: rect.y }, pageWidth, pageHeight);
  return {
    x: topLeft.x,
    y: topLeft.y - height,
    width,
    height,
  };
}

function getPdfTextDrawY(overlay, rect, pageHeight){
  const fontSize = Math.max(6, Number(overlay.fontSize) || 12);
  if (Number.isFinite(overlay.baselineY)){
    return pageHeight - (overlay.baselineY * pageHeight) - (fontSize * 0.18);
  }
  return rect.y + Math.max(2, rect.height - fontSize);
}

async function embedPdfEditorImage(doc, dataUrl){
  const bytes = dataURLToBytes(dataUrl);
  if (!bytes) return null;
  const mime = String(dataUrl || "").slice(0, 32).toLowerCase();
  if (mime.includes("jpeg") || mime.includes("jpg")){
    return doc.embedJpg(bytes);
  }
  return doc.embedPng(bytes);
}

async function exportEditedPdfBlob(){
  if (!pdfEditorState.originalPdfBytes) throw new Error("No PDF loaded.");
  await ensurePdfLib();
  const { PDFDocument, StandardFonts, degrees, rgb } = PDFLib;
  const sourceBytes = pdfEditorState.workingPdfBytes || pdfEditorState.originalPdfBytes;
  const sourceDoc = await PDFDocument.load(new Uint8Array(sourceBytes));
  const outputDoc = await PDFDocument.create();
  const regular = await outputDoc.embedFont(StandardFonts.Helvetica);
  const visiblePages = getVisiblePdfPages();
  if (!visiblePages.length) throw new Error("All pages are deleted.");

  for (const pageState of visiblePages){
    const [copiedPage] = await outputDoc.copyPages(sourceDoc, [pageState.sourceIndex]);
    const currentAngle = copiedPage.getRotation().angle || 0;
    copiedPage.setRotation(degrees(((currentAngle + pageState.rotation) % 360 + 360) % 360));
    outputDoc.addPage(copiedPage);
    const width = copiedPage.getWidth();
    const height = copiedPage.getHeight();
    const overlays = pdfEditorState.overlays.filter(overlay => overlay.pageIndex === pageState.sourceIndex && !overlay.pending);
    for (const overlay of overlays){
      const rect = pdfRectToPage(overlay, width, height);
      const opacity = Math.min(1, Math.max(0.05, Number(overlay.opacity ?? 1)));
      if (overlay.type === "textEdit"){
        const bounds = calculateTextEditPdfBounds(overlay, width, height);
        copiedPage.drawRectangle({
          x: bounds.cover.x,
          y: bounds.cover.y,
          width: bounds.cover.width,
          height: bounds.cover.height,
          color: hexToPdfRgb(overlay.backgroundColor || "#ffffff"),
          opacity: 1,
        });
        copiedPage.drawText(String(overlay.replacementText || ""), {
          x: bounds.text.x,
          y: bounds.text.y,
          size: bounds.fontSize,
          font: regular,
          color: hexToPdfRgb(overlay.color || "#000000"),
          opacity,
          maxWidth: Math.max(8, bounds.text.width + 2),
        });
      }else if (overlay.type === "text"){
        copiedPage.drawText(String(overlay.content || ""), {
          x: rect.x + 2,
          y: rect.y + Math.max(2, rect.height - (overlay.fontSize || 12)),
          size: Math.max(6, Number(overlay.fontSize) || 12),
          font: regular,
          color: hexToPdfRgb(overlay.color || "#111827"),
          opacity,
          maxWidth: Math.max(8, rect.width - 4),
        });
      }else if (overlay.type === "highlight"){
        copiedPage.drawRectangle({
          x: rect.x,
          y: rect.y,
          width: rect.width,
          height: rect.height,
          color: rgb(1, 0.88, 0.2),
          opacity,
        });
      }else if (overlay.type === "rectangle"){
        copiedPage.drawRectangle({
          x: rect.x,
          y: rect.y,
          width: rect.width,
          height: rect.height,
          borderColor: hexToPdfRgb(overlay.color || "#2f6bff"),
          borderWidth: 2,
          opacity,
        });
      }else if (overlay.type === "drawing"){
        const path = Array.isArray(overlay.path) ? overlay.path : [];
        for (let i = 1; i < path.length; i++){
          const a = pdfRectToPage({ x: path[i - 1].x, y: path[i - 1].y, width: 0, height: 0 }, width, height);
          const b = pdfRectToPage({ x: path[i].x, y: path[i].y, width: 0, height: 0 }, width, height);
          copiedPage.drawLine({
            start: { x: a.x, y: a.y },
            end: { x: b.x, y: b.y },
            thickness: 1.5,
            color: hexToPdfRgb(overlay.color || "#111827"),
            opacity,
          });
        }
      }else if ((overlay.type === "image" || overlay.type === "signature") && overlay.imageDataUrl){
        const img = await embedPdfEditorImage(outputDoc, overlay.imageDataUrl);
        if (img){
          copiedPage.drawImage(img, {
            x: rect.x,
            y: rect.y,
            width: rect.width,
            height: rect.height,
            opacity,
          });
        }
      }
    }
  }
  const bytes = await outputDoc.save();
  return new Blob([bytes], { type: "application/pdf" });
}

async function downloadEditedPdf(){
  try{
    setPdfEditorStatus("Exporting edited copy...");
    const blob = await exportEditedPdfBlob();
    if (pdfEditorState.exportUrl) URL.revokeObjectURL(pdfEditorState.exportUrl);
    pdfEditorState.exportUrl = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const base = (pdfEditorState.fileName || "edited.pdf").replace(/\.pdf$/i, "");
    a.href = pdfEditorState.exportUrl;
    a.download = `${base}-edited.pdf`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    setPdfEditorStatus("Edited copy exported. Original PDF bytes were not changed.");
    setPdfEditorDirty(false);
  }catch(error){
    console.error("PDF Editor export failed", error);
    setPdfEditorStatus("Export failed: " + (error.message || error), true);
  }
}

async function sendEditedPdfToExtraction(){
  try{
    setPdfEditorStatus("Preparing edited copy for extraction...");
    const blob = await exportEditedPdfBlob();
    const file = new File([blob], `${(pdfEditorState.fileName || "edited.pdf").replace(/\.pdf$/i, "")}-edited.pdf`, { type: "application/pdf" });
    activateTab("extract");
    await handlePdfExtraction(file);
  }catch(error){
    console.error("PDF Editor send to extraction failed", error);
    setPdfEditorStatus("Could not send edited PDF to extraction: " + (error.message || error), true);
  }
}

function bindPdfEditorEvents(){
  if (bindPdfEditorEvents.bound) return;
  bindPdfEditorEvents.bound = true;
  if (pdfEditorUploadBtn) pdfEditorUploadBtn.addEventListener("click", ()=> pdfEditorFileInput && pdfEditorFileInput.click());
  if (pdfEditorFileInput){
    pdfEditorFileInput.addEventListener("change", async event => {
      const file = event.target.files && event.target.files[0];
      event.target.value = "";
      if (file) await loadPdfIntoEditor(file);
    });
  }
  if (pdfEditorDropZone){
    ["dragenter","dragover"].forEach(evt => {
      pdfEditorDropZone.addEventListener(evt, event => {
        event.preventDefault();
        event.stopPropagation();
        pdfEditorDropZone.classList.add("hover");
      });
    });
    ["dragleave","dragend"].forEach(evt => {
      pdfEditorDropZone.addEventListener(evt, event => {
        event.preventDefault();
        event.stopPropagation();
        pdfEditorDropZone.classList.remove("hover");
      });
    });
    pdfEditorDropZone.addEventListener("drop", async event => {
      event.preventDefault();
      event.stopPropagation();
      pdfEditorDropZone.classList.remove("hover");
      const file = event.dataTransfer && event.dataTransfer.files && event.dataTransfer.files[0];
      if (file) await loadPdfIntoEditor(file);
    });
    pdfEditorDropZone.addEventListener("click", ()=> pdfEditorFileInput && pdfEditorFileInput.click());
  }
  document.querySelectorAll("[data-pdf-tool]").forEach(btn => {
    btn.addEventListener("click", ()=> setPdfEditorTool(btn.dataset.pdfTool));
  });
  if (pdfEditorPrevPage) pdfEditorPrevPage.addEventListener("click", ()=> {
    pdfEditorState.activeVisibleIndex = Math.max(0, pdfEditorState.activeVisibleIndex - 1);
    renderPdfEditor();
  });
  if (pdfEditorNextPage) pdfEditorNextPage.addEventListener("click", ()=> {
    pdfEditorState.activeVisibleIndex = Math.min(getVisiblePdfPages().length - 1, pdfEditorState.activeVisibleIndex + 1);
    renderPdfEditor();
  });
  if (pdfEditorZoomOut) pdfEditorZoomOut.addEventListener("click", ()=> {
    pdfEditorState.fitWidth = false;
    pdfEditorState.zoom = Math.max(0.35, pdfEditorState.zoom - 0.15);
    renderPdfActivePage();
  });
  if (pdfEditorZoomIn) pdfEditorZoomIn.addEventListener("click", ()=> {
    pdfEditorState.fitWidth = false;
    pdfEditorState.zoom = Math.min(3, pdfEditorState.zoom + 0.15);
    renderPdfActivePage();
  });
  if (pdfEditorFitWidth) pdfEditorFitWidth.addEventListener("click", ()=> {
    pdfEditorState.fitWidth = true;
    renderPdfActivePage();
  });
  if (pdfEditorRotateLeft) pdfEditorRotateLeft.addEventListener("click", ()=> rotateActivePdfPage(-90));
  if (pdfEditorRotateRight) pdfEditorRotateRight.addEventListener("click", ()=> rotateActivePdfPage(90));
  if (pdfEditorDeletePage) pdfEditorDeletePage.addEventListener("click", deleteActivePdfPage);
  if (pdfEditorAddImage) pdfEditorAddImage.addEventListener("click", ()=> pdfEditorImageInput && pdfEditorImageInput.click());
  if (pdfEditorImageInput){
    pdfEditorImageInput.addEventListener("change", async event => {
      const file = event.target.files && event.target.files[0];
      event.target.value = "";
      if (file) await addPdfImageOverlay(file);
    });
  }
  if (pdfEditorExport) pdfEditorExport.addEventListener("click", downloadEditedPdf);
  if (pdfEditorSendExtraction) pdfEditorSendExtraction.addEventListener("click", sendEditedPdfToExtraction);
  if (pdfEditorInlineCancel) pdfEditorInlineCancel.addEventListener("click", closePdfInlineEditor);
  if (pdfEditorInlineSave) pdfEditorInlineSave.addEventListener("click", savePdfInlineTextEdit);
  if (pdfEditorThumbs){
    pdfEditorThumbs.addEventListener("click", event => {
      const thumb = event.target.closest(".pdf-thumb");
      if (!thumb) return;
      pdfEditorState.activeVisibleIndex = Number(thumb.dataset.visibleIndex) || 0;
      renderPdfEditor();
    });
    pdfEditorThumbs.addEventListener("dragstart", event => {
      const thumb = event.target.closest(".pdf-thumb");
      if (thumb) event.dataTransfer.setData("text/plain", thumb.dataset.visibleIndex);
    });
    pdfEditorThumbs.addEventListener("dragover", event => {
      const thumb = event.target.closest(".pdf-thumb");
      if (!thumb) return;
      event.preventDefault();
      thumb.classList.add("drag-over");
    });
    pdfEditorThumbs.addEventListener("dragleave", event => {
      const thumb = event.target.closest(".pdf-thumb");
      if (thumb) thumb.classList.remove("drag-over");
    });
    pdfEditorThumbs.addEventListener("drop", event => {
      const thumb = event.target.closest(".pdf-thumb");
      if (!thumb) return;
      event.preventDefault();
      thumb.classList.remove("drag-over");
      movePdfPage(Number(event.dataTransfer.getData("text/plain")), Number(thumb.dataset.visibleIndex));
    });
  }
  if (pdfEditorOverlayLayer){
    pdfEditorOverlayLayer.addEventListener("pointerdown", event => {
      if (event.target.closest(".pdf-overlay-item")) return;
      const point = getPdfPointerPoint(event);
      if (!point) return;
      if (pdfEditorState.tool === "text"){
        addPdfOverlayAtPoint(point);
        return;
      }
      if (pdfEditorState.tool === "rectangle" || pdfEditorState.tool === "highlight"){
        pdfEditorState.drag = { type: pdfEditorState.tool, start: point };
        pdfEditorOverlayLayer.setPointerCapture(event.pointerId);
      }else if (pdfEditorState.tool === "draw"){
        pdfEditorState.drag = { type: "drawing", start: point, path: [point] };
        pdfEditorOverlayLayer.setPointerCapture(event.pointerId);
      }else{
        selectPdfOverlay(null);
      }
    });
    pdfEditorOverlayLayer.addEventListener("pointermove", event => {
      if (handleTextEditReplacementDrag(event)) return;
      if (!pdfEditorState.drag) return;
      const point = getPdfPointerPoint(event);
      if (!point) return;
      if (pdfEditorState.drag.type === "drawing"){
        pdfEditorState.drag.path.push(point);
      }
    });
    pdfEditorOverlayLayer.addEventListener("pointerup", event => {
      if (endTextEditReplacementDrag()) return;
      if (!pdfEditorState.drag) return;
      const point = getPdfPointerPoint(event) || pdfEditorState.drag.start;
      finishPdfShapeOverlay(point);
      try{ pdfEditorOverlayLayer.releasePointerCapture(event.pointerId); }catch{/* ignore */}
    });
  }
  if (pdfEditorSelectedProps){
    pdfEditorSelectedProps.addEventListener("input", event => {
      const nativeInput = event.target.closest("[data-pdf-native-prop]");
      if (nativeInput){
        updateNativePdfTextEditProp(nativeInput.dataset.pdfNativeProp, nativeInput.type === "checkbox" ? nativeInput.checked : nativeInput.value);
        return;
      }
      const input = event.target.closest("[data-pdf-prop]");
      if (input) updateSelectedPdfOverlayProp(input.dataset.pdfProp, input.value);
    });
    pdfEditorSelectedProps.addEventListener("click", event => {
      const nativeApplyBtn = event.target.closest("#pdfEditorApplyNativeTextEdit");
      if (nativeApplyBtn){
        applyNativePdfTextEdit();
        return;
      }
      const nativeFallbackBtn = event.target.closest("#pdfEditorFallbackSafeTextEdit");
      if (nativeFallbackBtn){
        fallbackNativeToSafeTextEdit();
        return;
      }
      const applyBtn = event.target.closest("#pdfEditorApplyTextEdit");
      if (applyBtn){
        applyPendingPdfTextEdit();
        return;
      }
      const cancelBtn = event.target.closest("#pdfEditorCancelTextEdit");
      if (cancelBtn){
        cancelPendingPdfTextEdit();
        return;
      }
      const resetBtn = event.target.closest("#pdfEditorResetTextPosition");
      if (resetBtn){
        const overlay = pdfEditorState.overlays.find(item => item.id === pdfEditorState.selectedOverlayId && item.type === "textEdit");
        if (overlay){
          resetTextEditReplacementPosition(overlay);
          setPdfEditorDirty(true);
          renderPdfOverlays();
          renderPdfProperties();
        }
        return;
      }
      const deleteBtn = event.target.closest("#pdfEditorDeleteOverlay");
      if (!deleteBtn) return;
      pdfEditorState.overlays = pdfEditorState.overlays.filter(overlay => overlay.id !== pdfEditorState.selectedOverlayId);
      pdfEditorState.selectedOverlayId = null;
      pdfEditorState.selectedTextBlockId = null;
      pdfEditorState.activeTextBlockId = null;
      pdfEditorState.isEditingText = false;
      if (pdfEditorTextLayer){
        pdfEditorTextLayer.querySelectorAll(".pdf-text-block").forEach(el => el.classList.remove("selected"));
      }
      setPdfEditorDirty(true);
      renderPdfOverlays();
      renderPdfProperties();
    });
  }
  if (pdfEditorTextEdits){
    pdfEditorTextEdits.addEventListener("click", event => {
      const btn = event.target.closest("[data-select-pdf-overlay]");
      if (!btn) return;
      const overlay = pdfEditorState.overlays.find(item => item.id === btn.dataset.selectPdfOverlay);
      if (!overlay) return;
      const visible = getVisiblePdfPages();
      const idx = visible.findIndex(page => page.sourceIndex === overlay.pageIndex);
      if (idx >= 0) pdfEditorState.activeVisibleIndex = idx;
      pdfEditorState.selectedOverlayId = overlay.id;
      renderPdfEditor();
    });
  }
  window.addEventListener("resize", () => {
    if (panels.pdfeditor && panels.pdfeditor.classList.contains("active") && pdfEditorState.fitWidth){
      renderPdfActivePage();
    }
  });
  window.addEventListener("pointermove", event => {
    handleTextEditReplacementDrag(event);
  });
  window.addEventListener("pointerup", () => {
    endTextEditReplacementDrag();
  });
  window.addEventListener("keydown", event => {
    if (!panels.pdfeditor || !panels.pdfeditor.classList.contains("active")) return;
    if (!["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(event.key)) return;
    const step = event.shiftKey ? 10 : 1;
    const dx = event.key === "ArrowLeft" ? -step : event.key === "ArrowRight" ? step : 0;
    const dy = event.key === "ArrowUp" ? -step : event.key === "ArrowDown" ? step : 0;
    if (nudgeSelectedTextEdit(dx, dy)){
      event.preventDefault();
    }
  });
  setPdfEditorTool("select");
}

const mmToPt = 2.83464567;
const pageSize = { w: 100 * mmToPt, h: 40 * mmToPt };

async function loadLogoCandidates(name){
  const basePath = window.location.pathname.replace(/[^/]*$/, "");
  const candidates = [
    `${name}.png`, `${name}.PNG`,
    `${basePath}${name}.png`, `${basePath}${name}.PNG`,
    `assets/${name}.png`, `assets/${name}.PNG`
  ];
  for (const path of candidates){
    try{
      const res = await fetch(path, { cache: "no-cache" });
      if (res.ok){
        const buffer = await res.arrayBuffer();
        return new Uint8Array(buffer);
      }
    }catch{/* ignore */}
  }
  return null;
}

function dataUrlFromImage(img){
  try{
    const canvas = document.createElement("canvas");
    canvas.width = img.naturalWidth || img.width;
    canvas.height = img.naturalHeight || img.height;
    const ctx = canvas.getContext("2d");
    ctx.drawImage(img, 0, 0);
    return canvas.toDataURL("image/png");
  }catch{
    return null;
  }
}

async function loadLogoViaImage(name){
  const basePath = window.location.pathname.replace(/[^/]*$/, "");
  const candidates = [
    `${name}.png`, `${name}.PNG`,
    `${basePath}${name}.png`, `${basePath}${name}.PNG`,
    `assets/${name}.png`, `assets/${name}.PNG`
  ];
  for (const src of candidates){
    const found = await new Promise((resolve)=>{
      const img = new Image();
      img.onload = ()=>{
        const dataUrl = dataUrlFromImage(img);
        if (!dataUrl) return resolve(null);
        try{
          const bytes = dataURLToBytes(dataUrl);
          resolve(bytes);
        }catch{
          resolve(null);
        }
      };
      img.onerror = ()=> resolve(null);
      img.src = src;
    });
    if (found && found.byteLength) return found;
  }
  return null;
}

async function preloadLogos(){
  let keliBytes = null;
  let ceBytes = null;
  const localKeli = localStorage.getItem("logo_keli_b64");
  const localCe = localStorage.getItem("logo_ce_b64");
  if (localKeli) keliBytes = dataURLToBytes(localKeli);
  if (localCe) ceBytes = dataURLToBytes(localCe);
  if (!keliBytes) keliBytes = await loadLogoViaImage("logokeli");
  if (!ceBytes) ceBytes = await loadLogoViaImage("ce");
  if (!keliBytes){
    const fetched = await loadLogoCandidates("logokeli");
    if (fetched && fetched.byteLength){
      keliBytes = fetched;
      try{
        const dataUrl = bytesToDataURL(fetched);
        if (dataUrl) localStorage.setItem("logo_keli_b64", dataUrl);
      }catch{/* ignore storage errors */}
    }
  }
  if (!ceBytes){
    const fetched = await loadLogoCandidates("ce");
    if (fetched && fetched.byteLength){
      ceBytes = fetched;
      try{
        const dataUrl = bytesToDataURL(fetched);
        if (dataUrl) localStorage.setItem("logo_ce_b64", dataUrl);
      }catch{/* ignore storage errors */}
    }
  }
  return { keliBytes, ceBytes };
}

async function generateLabelsPdf(rows){
  await ensurePdfLib();
  const { PDFDocument, StandardFonts } = PDFLib;
  const pdfDoc = await PDFDocument.create();
  const bold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const regular = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const { keliBytes, ceBytes } = await preloadLogos();
  let keliImg, ceImg;
  if (keliBytes){
    const data = keliBytes instanceof Uint8Array ? keliBytes : new Uint8Array(keliBytes);
    keliImg = await pdfDoc.embedPng(data);
  }
  if (ceBytes){
    const data = ceBytes instanceof Uint8Array ? ceBytes : new Uint8Array(ceBytes);
    ceImg = await pdfDoc.embedPng(data);
  }
  const today = new Date().toLocaleDateString("en-GB").replaceAll("/", ".");
  for (const row of rows){
    const qty = Number(row.quantity || 0) || 1;
    const labelSource = String(row.source || "").toLowerCase();
    const msIndexValue = Number(row.ms_index ?? row.msIndex);
    const isProcessingLabel = labelSource === "processing";
    for (let i = 0; i < qty; i++){
      const page = pdfDoc.addPage([pageSize.w, pageSize.h]);
      const margin = 16;
      const spacing = 12;
      let y = pageSize.h - margin;
      if (keliImg) page.drawImage(keliImg, { x: margin, y: y - 20, width: 60, height: 20 });
      if (ceImg) page.drawImage(ceImg, { x: (pageSize.w - 30) / 2, y: y - 18, width: 30, height: 18 });
      page.drawText(today, { x: pageSize.w - margin - 60, y: y - 14, size: 10, font: bold });
      page.drawLine({ start: { x: margin, y: y - 24 }, end: { x: pageSize.w - margin, y: y - 24 }, thickness: 0.8 });
      y -= (24 + spacing);
      const dimension = row.dimension && row.dimension.length ? row.dimension : "—";
      let positionText = row.position || "—";
      if (isProcessingLabel){
        const summary = formatProcessingPositionsSummary(row);
        if (summary){
          positionText = summary;
        }else if (!positionText || positionText === "(Grouped)"){
          positionText = "—";
        }
      }
      const orderNo = row.order_number || "—";
      const type = row.type || "";
      page.drawText(`Order No.: ${orderNo}   Pos: ${positionText}   Dim: ${dimension}`, {
        x: margin, y, size: 9, font: bold, maxWidth: pageSize.w - margin * 2
      });
      y -= 14;
      page.drawText(`Glass Type: ${type}`, { x: margin, y, size: 10, font: bold, maxWidth: pageSize.w - margin * 2 });
      y -= 16;
      const showProcessingMsIndex = isProcessingLabel && Number.isFinite(msIndexValue);
      if (showProcessingMsIndex){
        const footerText = String(Math.trunc(msIndexValue));
        const footerSize = 18;
        const msTopSpacing = 8;
        const msBottomSpacing = 12;
        const msVerticalOffset = -16; // push down visually by 20px
        const textWidth = bold.widthOfTextAtSize(footerText, footerSize);
        const footerX = (pageSize.w - textWidth) / 2;
        const footerY = y + (16 - msTopSpacing) + msVerticalOffset;
        page.drawText(footerText, { x: footerX, y: footerY, size: footerSize, font: bold });
        y = footerY - msBottomSpacing;
      }else{
        page.drawText("KELI ALBANIA PVC", { x: margin, y, size: 10, font: regular });
      }
    }
  }
  return pdfDoc.save();
}

(function(){
  const statusEl = document.getElementById("logoStatus");
  const setStatus = (msg)=> { if (statusEl) statusEl.textContent = msg; };
  const keliInput = document.getElementById("logoKeli");
  const ceInput = document.getElementById("logoCe");
  if (keliInput){
    keliInput.addEventListener("change", async (event)=>{
      const file = event.target.files && event.target.files[0];
      if (!file) return;
      try{
        const dataUrl = await fileToDataURL(file);
        localStorage.setItem("logo_keli_b64", dataUrl);
        setStatus("Saved logokeli.png ✔");
      }catch{
        setStatus("Failed to save logokeli.");
      }
    });
  }
  if (ceInput){
    ceInput.addEventListener("change", async (event)=>{
      const file = event.target.files && event.target.files[0];
      if (!file) return;
      try{
        const dataUrl = await fileToDataURL(file);
        localStorage.setItem("logo_ce_b64", dataUrl);
        setStatus("Saved ce.png ✔");
      }catch{
        setStatus("Failed to save ce.");
      }
    });
  }
})();

const iguState = {
  search: "",
  results: [],
  selected: new Map(),
  ordersData: [],
  calculated: null,
  loadingList: false,
  loadingOrders: false,
  settings: { ...IGU_SETTINGS_DEFAULTS },
};

function setIguStatus(message){
  const statusEl = document.getElementById("iguStatus");
  if (statusEl) statusEl.textContent = message || "";
}

function getIguOrderKey(order){
  if (!order) return "";
  const numbers = Array.isArray(order.order_numbers)
    ? order.order_numbers.map(value => value == null ? "" : String(value).trim()).filter(Boolean)
    : [];
  if (numbers.length) return numbers[0];
  return pickFirstString([
    order.order_number,
    order.orderNumber,
    order.order,
    order.orderId,
    order.order_id,
    order.id,
  ], "");
}

function renderIguOrderList(){
  const container = document.getElementById("iguOrderList");
  if (!container) return;
  if (iguState.loadingList){
    container.innerHTML = `<div class="igu-breakdown-empty">Loading orders…</div>`;
    return;
  }
  if (!iguState.results.length){
    const message = iguState.search ? "No orders found." : "Search to load orders.";
    container.innerHTML = `<div class="igu-breakdown-empty">${message}</div>`;
    return;
  }
  const rows = iguState.results.map(order => {
    const key = getIguOrderKey(order);
    if (!key) return "";
    const label = getOrderLabel(order);
    const client = order.client || order.client_hint || "—";
    const created = formatDate(order.created_at) || "—";
    const checked = iguState.selected.has(key) ? "checked" : "";
    return `<label class="igu-order-item">
      <input type="checkbox" data-igu-order="${escapeHtml(key)}" ${checked} />
      <div>
        <div><strong>${escapeHtml(label)}</strong></div>
        <div class="igu-order-meta">${escapeHtml(client)} • ${escapeHtml(created)}</div>
      </div>
    </label>`;
  }).join("");
  container.innerHTML = rows || `<div class="igu-breakdown-empty">No orders found.</div>`;
}

function renderIguSelectedChips(){
  const container = document.getElementById("iguSelectedChips");
  if (!container) return;
  if (!iguState.selected.size){
    container.innerHTML = `<span class="muted small">None</span>`;
    return;
  }
  const chips = Array.from(iguState.selected.entries()).map(([key, order])=>{
    const label = getOrderLabel(order);
    return `<span class="order-chip" data-igu-chip="${escapeHtml(key)}">
      ${escapeHtml(label)}
      <button type="button" data-igu-remove="${escapeHtml(key)}" title="Remove">×</button>
    </span>`;
  }).join("");
  container.innerHTML = chips;
}

function updateIguSelectionStatus(){
  const count = iguState.selected.size;
  const loadBtn = document.getElementById("iguLoadSelected");
  const clearBtn = document.getElementById("iguClearSelected");
  if (loadBtn) loadBtn.disabled = iguState.loadingOrders || !count;
  if (clearBtn) clearBtn.disabled = !count;
  if (!count){
    setIguStatus("Select orders to calculate.");
    return;
  }
  setIguStatus(`Selected ${count} order${count === 1 ? "" : "s"}.`);
}

async function loadIguOrderList(){
  iguState.loadingList = true;
  renderIguOrderList();
  const params = new URLSearchParams();
  params.set("year", "all");
  params.set("limit", "25");
  if (iguState.search){
    params.set("query", iguState.search);
  }
  try{
    const res = await fetch(API_BASE + "/orders?" + params.toString());
    if (!res.ok){
      const text = await res.text().catch(()=> "");
      throw new Error(text || `Server responded with ${res.status}`);
    }
    const data = await res.json();
    iguState.results = Array.isArray(data.items) ? data.items : [];
  }catch(error){
    console.warn("IGU history list load failed", error);
    iguState.results = [];
    setIguStatus("Unable to load history orders.");
  }finally{
    iguState.loadingList = false;
    renderIguOrderList();
  }
}

async function fetchIguOrdersByIds(ids){
  const params = new URLSearchParams();
  params.set("ids", ids.join(","));
  const res = await fetch(API_BASE + "/api/history/orders?" + params.toString());
  if (!res.ok){
    const text = await res.text().catch(()=> "");
    throw new Error(text || `Server responded with ${res.status}`);
  }
  const data = await res.json();
  return Array.isArray(data.orders) ? data.orders : [];
}

async function loadIguSelectedOrders(){
  const ids = Array.from(iguState.selected.keys()).filter(Boolean);
  if (!ids.length){
    setIguStatus("Select orders to load.");
    return;
  }
  iguState.loadingOrders = true;
  updateIguSelectionStatus();
  setIguStatus("Loading order data…");
  try{
    const orders = await fetchIguOrdersByIds(ids);
    iguState.ordersData = orders;
    iguState.calculated = null;
    recalcIguTotals();
    const count = orders.length;
    setIguStatus(`Loaded ${count} order${count === 1 ? "" : "s"}.`);
  }catch(error){
    console.warn("IGU order data load failed", error);
    setIguStatus("Unable to load order data.");
  }finally{
    iguState.loadingOrders = false;
    updateIguSelectionStatus();
  }
}

function normalizeIguNumber(value){
  if (value === "" || value === null || value === undefined) return null;
  const num = Number(value);
  return Number.isFinite(num) ? num : null;
}

function computeIguTotals(orders, settings){
  const rows = [];
  let totalSpacerMm = 0;
  let totalButylKg = 0;
  let missingPerimeter = 0;
  let missingSpacer = 0;
  const density = normalizeIguNumber(settings.density) ?? IGU_SETTINGS_DEFAULTS.density;
  const boxWeight = normalizeIguNumber(settings.boxWeightKg) ?? IGU_SETTINGS_DEFAULTS.boxWeightKg;
  const useOverride = settings.spacerSource === "override";
  const overrideWidth = normalizeIguNumber(settings.spacerOverride);
  (orders || []).forEach(order => {
    const orderId = order.order_id || order.orderId || order.id || "—";
    const items = Array.isArray(order.items) ? order.items : [];
    items.forEach(item => {
      const width = normalizeIguNumber(item.width_mm);
      const height = normalizeIguNumber(item.height_mm);
      const qty = Math.max(0, Number(item.quantity) || 0);
      let perimeter = normalizeIguNumber(item.perimeter_mm);
      if (perimeter == null && width != null && height != null){
        perimeter = 2 * (width + height);
      }
      if (perimeter == null){
        missingPerimeter += 1;
      }
      let spacerWidth = useOverride ? overrideWidth : normalizeIguNumber(item.spacer_width_mm);
      if (spacerWidth == null){
        missingSpacer += 1;
      }
      const spacerTotalMm = perimeter != null ? perimeter * qty : 0;
      const spacerTotalM = spacerTotalMm / 1000;
      let butylKg = 0;
      if (spacerWidth != null && spacerTotalMm > 0){
        const crossSectionMm2 = spacerWidth * IGU_INNER_FILL_MM;
        const volumeMm3 = spacerTotalMm * crossSectionMm2;
        const volumeCm3 = volumeMm3 / 1000;
        const massG = volumeCm3 * density;
        butylKg = massG / 1000;
      }
      totalSpacerMm += spacerTotalMm;
      totalButylKg += butylKg;
      rows.push({
        orderId,
        position: item.position || "—",
        width,
        height,
        quantity: qty,
        spacerM: spacerTotalM,
        butylKg,
      });
    });
  });
  const spacerBaseM = totalSpacerMm / 1000;
  const butylBaseKg = totalButylKg;
  let spacerM = spacerBaseM;
  let butylKg = butylBaseKg;
  if (settings.wasteSpacerEnabled){
    spacerM *= 1 + ((normalizeIguNumber(settings.wasteSpacerPct) ?? 0) / 100);
  }
  if (settings.wasteButylEnabled){
    butylKg *= 1 + ((normalizeIguNumber(settings.wasteButylPct) ?? 0) / 100);
  }
  const boxesNeeded = butylKg > 0 && boxWeight > 0 ? Math.ceil(butylKg / boxWeight) : 0;
  return {
    rows,
    totals: {
      spacerM,
      spacerBaseM,
      butylKg,
      butylBaseKg,
      boxesNeeded,
    },
    warnings: {
      missingPerimeter,
      missingSpacer,
    },
  };
}

function formatIguNumber(value, decimals){
  if (!Number.isFinite(value)) return "—";
  return formatNumber(value, { decimals });
}

function renderIguWarnings(warnings){
  const container = document.getElementById("iguWarnings");
  if (!container) return;
  const list = container.querySelector("ul");
  const messages = [];
  if (warnings.missingPerimeter){
    messages.push(`Missing perimeter or dimensions for ${warnings.missingPerimeter} item${warnings.missingPerimeter === 1 ? "" : "s"}.`);
  }
  if (warnings.missingSpacer){
    messages.push(`Missing spacer width for ${warnings.missingSpacer} item${warnings.missingSpacer === 1 ? "" : "s"}.`);
  }
  if (!messages.length){
    container.hidden = true;
    if (list) list.innerHTML = "";
    return;
  }
  if (list){
    list.innerHTML = messages.map(message => `<li>${escapeHtml(message)}</li>`).join("");
  }
  container.hidden = false;
}

function renderIguResults(){
  const spacerEl = document.getElementById("iguTotalSpacer");
  const butylEl = document.getElementById("iguTotalButyl");
  const boxesEl = document.getElementById("iguTotalBoxes");
  const spacerBaseEl = document.getElementById("iguTotalSpacerBase");
  const butylBaseEl = document.getElementById("iguTotalButylBase");
  const calc = iguState.calculated || { totals: { spacerM: 0, spacerBaseM: 0, butylKg: 0, butylBaseKg: 0, boxesNeeded: 0 }, warnings: { missingPerimeter: 0, missingSpacer: 0 } };
  const totals = calc.totals;
  if (spacerEl) spacerEl.textContent = formatIguNumber(totals.spacerM || 0, 2);
  if (butylEl) butylEl.textContent = formatIguNumber(totals.butylKg || 0, 3);
  if (boxesEl) boxesEl.textContent = formatIguNumber(totals.boxesNeeded || 0, 0);
  if (spacerBaseEl){
    spacerBaseEl.textContent = iguState.settings.wasteSpacerEnabled
      ? `Base: ${formatIguNumber(totals.spacerBaseM || 0, 2)} m`
      : "";
  }
  if (butylBaseEl){
    butylBaseEl.textContent = iguState.settings.wasteButylEnabled
      ? `Base: ${formatIguNumber(totals.butylBaseKg || 0, 3)} kg`
      : "";
  }
  renderIguWarnings(calc.warnings || { missingPerimeter: 0, missingSpacer: 0 });
}

function renderIguBreakdown(){
  const container = document.getElementById("iguBreakdownWrap");
  if (!container) return;
  const rows = iguState.calculated ? (iguState.calculated.rows || []) : [];
  if (!rows.length){
    container.innerHTML = `<div class="igu-breakdown-empty">Select orders to see item totals.</div>`;
    return;
  }
  const body = rows.map(row => {
    const widthText = row.width != null ? formatIguNumber(row.width, 0) : "—";
    const heightText = row.height != null ? formatIguNumber(row.height, 0) : "—";
    const dimension = (row.width != null && row.height != null) ? `${widthText} x ${heightText}` : "—";
    return `<tr>
      <td>${escapeHtml(String(row.orderId || "—"))}</td>
      <td>${escapeHtml(String(row.position || "—"))}</td>
      <td>${escapeHtml(dimension)}</td>
      <td>${formatIguNumber(row.quantity || 0, 0)}</td>
      <td>${formatIguNumber(row.spacerM || 0, 2)}</td>
      <td>${formatIguNumber(row.butylKg || 0, 3)}</td>
    </tr>`;
  }).join("");
  container.innerHTML = `<table>
    <thead>
      <tr>
        <th>Order ID</th>
        <th>Position</th>
        <th>Width x Height</th>
        <th>Quantity</th>
        <th>Spacer (m)</th>
        <th>Butyl (kg)</th>
      </tr>
    </thead>
    <tbody>${body}</tbody>
  </table>`;
}

function readIguSettings(){
  const overrideRadio = document.getElementById("iguSpacerSourceOverride");
  const overrideInput = document.getElementById("iguSpacerOverride");
  iguState.settings.spacerSource = overrideRadio && overrideRadio.checked ? "override" : "item";
  if (overrideInput){
    overrideInput.disabled = iguState.settings.spacerSource !== "override";
    const overrideValue = normalizeIguNumber(overrideInput.value);
    iguState.settings.spacerOverride = overrideValue;
  }
  const densityInput = document.getElementById("iguDensity");
  if (densityInput){
    iguState.settings.density = normalizeIguNumber(densityInput.value) ?? IGU_SETTINGS_DEFAULTS.density;
  }
  const boxWeightInput = document.getElementById("iguBoxWeight");
  if (boxWeightInput){
    iguState.settings.boxWeightKg = normalizeIguNumber(boxWeightInput.value) ?? IGU_SETTINGS_DEFAULTS.boxWeightKg;
  }
  const wasteSpacerToggle = document.getElementById("iguWasteSpacer");
  const wasteSpacerPct = document.getElementById("iguWasteSpacerPct");
  if (wasteSpacerToggle){
    iguState.settings.wasteSpacerEnabled = !!wasteSpacerToggle.checked;
  }
  if (wasteSpacerPct){
    iguState.settings.wasteSpacerPct = normalizeIguNumber(wasteSpacerPct.value) ?? IGU_SETTINGS_DEFAULTS.wasteSpacerPct;
  }
  const wasteButylToggle = document.getElementById("iguWasteButyl");
  const wasteButylPct = document.getElementById("iguWasteButylPct");
  if (wasteButylToggle){
    iguState.settings.wasteButylEnabled = !!wasteButylToggle.checked;
  }
  if (wasteButylPct){
    iguState.settings.wasteButylPct = normalizeIguNumber(wasteButylPct.value) ?? IGU_SETTINGS_DEFAULTS.wasteButylPct;
  }
}

function recalcIguTotals(){
  readIguSettings();
  iguState.calculated = computeIguTotals(iguState.ordersData || [], iguState.settings);
  renderIguResults();
  renderIguBreakdown();
}

function initIguCalculator(){
  const searchEl = document.getElementById("iguOrderSearch");
  if (!searchEl) return;
  const orderListEl = document.getElementById("iguOrderList");
  const loadBtn = document.getElementById("iguLoadSelected");
  const clearBtn = document.getElementById("iguClearSelected");
  const chipContainer = document.getElementById("iguSelectedChips");
  const spacerSourceRadios = [
    document.getElementById("iguSpacerSourceItem"),
    document.getElementById("iguSpacerSourceOverride"),
  ];
  const settingsInputs = [
    document.getElementById("iguSpacerOverride"),
    document.getElementById("iguDensity"),
    document.getElementById("iguBoxWeight"),
    document.getElementById("iguWasteSpacer"),
    document.getElementById("iguWasteSpacerPct"),
    document.getElementById("iguWasteButyl"),
    document.getElementById("iguWasteButylPct"),
  ];

  let searchTimer = null;
  searchEl.addEventListener("input", (event)=>{
    iguState.search = String(event.target.value || "").trim();
    clearTimeout(searchTimer);
    searchTimer = setTimeout(()=> loadIguOrderList(), 250);
  });

  if (orderListEl){
    orderListEl.addEventListener("change", (event)=>{
      const checkbox = event.target.closest("input[data-igu-order]");
      if (!checkbox) return;
      const key = checkbox.dataset.iguOrder;
      const order = iguState.results.find(item => getIguOrderKey(item) === key);
      if (checkbox.checked){
        if (order) iguState.selected.set(key, order);
      }else{
        iguState.selected.delete(key);
      }
      renderIguSelectedChips();
      updateIguSelectionStatus();
    });
  }

  if (chipContainer){
    chipContainer.addEventListener("click", (event)=>{
      const btn = event.target.closest("[data-igu-remove]");
      if (!btn) return;
      const key = btn.dataset.iguRemove;
      iguState.selected.delete(key);
      renderIguSelectedChips();
      renderIguOrderList();
      updateIguSelectionStatus();
    });
  }

  if (loadBtn){
    loadBtn.addEventListener("click", ()=> loadIguSelectedOrders());
  }
  if (clearBtn){
    clearBtn.addEventListener("click", ()=>{
      iguState.selected.clear();
      iguState.ordersData = [];
      iguState.calculated = null;
      renderIguSelectedChips();
      renderIguOrderList();
      recalcIguTotals();
      updateIguSelectionStatus();
    });
  }

  spacerSourceRadios.forEach(radio => {
    if (!radio) return;
    radio.addEventListener("change", ()=> recalcIguTotals());
  });
  settingsInputs.forEach(input => {
    if (!input) return;
    input.addEventListener("input", ()=> recalcIguTotals());
  });

  renderIguSelectedChips();
  renderIguResults();
  renderIguBreakdown();
  updateIguSelectionStatus();
  loadIguOrderList();
}

const manualOrderForm = document.getElementById("manualOrderForm");
const manualOrderFormTitle = document.getElementById("manualOrderFormTitle");
const manualClientName = document.getElementById("manualClientName");
const manualOrderNumber = document.getElementById("manualOrderNumber");
const manualOrderDate = document.getElementById("manualOrderDate");
const manualOrderStatus = document.getElementById("manualOrderStatus");
const manualOrderNotes = document.getElementById("manualOrderNotes");
const manualOrderRows = document.getElementById("manualOrderRows");
const manualOrderError = document.getElementById("manualOrderError");
const manualOrderDuplicateWarning = document.getElementById("manualOrderDuplicateWarning");
const manualOrderFormSummary = document.getElementById("manualOrderFormSummary");
const manualOrderAddRow = document.getElementById("manualOrderAddRow");
const manualOrderSaveDraft = document.getElementById("manualOrderSaveDraft");
const manualOrderApprove = document.getElementById("manualOrderApprove");
const manualOrderCancelEdit = document.getElementById("manualOrderCancelEdit");
const manualOrdersList = document.getElementById("manualOrdersList");
const manualOrdersListStatus = document.getElementById("manualOrdersListStatus");
const manualOrderSearch = document.getElementById("manualOrderSearch");
const manualOrderStatusFilter = document.getElementById("manualOrderStatusFilter");

function manualToday(){
  const now = new Date();
  return new Date(now.getTime() - now.getTimezoneOffset() * 60000).toISOString().slice(0, 10);
}

function newManualRow(source = {}){
  return {
    uid: `manual-row-${manualOrdersState.nextRowId++}`,
    id: source.id || null,
    position: source.position == null ? "" : String(source.position),
    glass_type: source.glass_type == null ? "" : String(source.glass_type),
    width_mm: source.width_mm == null ? "" : String(source.width_mm),
    height_mm: source.height_mm == null ? "" : String(source.height_mm),
    quantity: source.quantity == null ? "1" : String(source.quantity),
    area_override_m2: source.area_override_m2 == null ? "" : String(source.area_override_m2),
    notes: source.notes == null ? "" : String(source.notes),
  };
}

const MANUAL_ROW_FIELD_ORDER = [
  "position",
  "glass_type",
  "width_mm",
  "height_mm",
  "quantity",
  "area_override_m2",
  "notes",
];

function incrementManualPosition(value, fallback){
  const text = String(value ?? "").trim();
  const match = text.match(/^(.*?)(\d+)$/);
  if (!match) return String(fallback);
  const digits = match[2];
  const incremented = String(Number(digits) + 1);
  const nextDigits = digits.length > 1 && digits.startsWith("0")
    ? incremented.padStart(digits.length, "0")
    : incremented;
  return `${match[1]}${nextDigits}`;
}

function nextManualRowDefaults(){
  const previous = manualOrdersState.rows[manualOrdersState.rows.length - 1] || null;
  const nextPosition = previous
    ? incrementManualPosition(previous.position, manualOrdersState.rows.length + 1)
    : "1";
  return newManualRow({
    position: nextPosition,
    glass_type: previous?.glass_type || "",
    quantity: 1,
  });
}

function focusManualRowField(rowIndex, field){
  const row = manualOrdersState.rows[rowIndex];
  if (!row || !MANUAL_ROW_FIELD_ORDER.includes(field)) return;
  const tr = manualOrderRows?.querySelector(`[data-manual-row="${CSS.escape(row.uid)}"]`);
  const input = tr?.querySelector(`[data-manual-field="${field}"]`);
  if (!input) return;
  input.focus();
  if (typeof input.select === "function") input.select();
}

function appendManualRow({ focusField = "position" } = {}){
  const row = nextManualRowDefaults();
  manualOrdersState.rows.push(row);
  renderManualRows();
  focusManualRowField(manualOrdersState.rows.length - 1, focusField);
  return row;
}

function manualCalculatedArea(row){
  const width = Number(row?.width_mm);
  const height = Number(row?.height_mm);
  const quantity = Number(row?.quantity);
  if (!(width > 0) || !(height > 0) || !Number.isInteger(quantity) || quantity <= 0) return 0;
  return Math.round((width * height * quantity / 1_000_000) * 1000) / 1000;
}

function manualFinalArea(row){
  const overrideText = String(row?.area_override_m2 ?? "").trim();
  if (overrideText !== ""){
    const override = Number(overrideText);
    if (Number.isFinite(override) && override >= 0) return Math.round(override * 1000) / 1000;
  }
  return manualCalculatedArea(row);
}

function manualErrorMessage(detail, fallback){
  if (Array.isArray(detail)){
    return detail.map(item => item?.msg || String(item)).join(" ");
  }
  if (typeof detail === "string" && detail.trim()) return detail;
  return fallback;
}

async function manualApi(path, options = {}){
  const response = await fetch(API_BASE + path, {
    ...options,
    headers: {
      ...(options.body ? { "Content-Type": "application/json" } : {}),
      ...(options.headers || {}),
    },
  });
  let data = null;
  try{
    data = await response.json();
  }catch{
    data = null;
  }
  if (!response.ok){
    throw new Error(manualErrorMessage(data?.detail, `Request failed (${response.status}).`));
  }
  return data;
}

function setManualFormError(message){
  if (!manualOrderError) return;
  manualOrderError.hidden = !message;
  manualOrderError.textContent = message || "";
}

function setManualDuplicateWarning(message){
  if (!manualOrderDuplicateWarning) return;
  manualOrderDuplicateWarning.hidden = !message;
  manualOrderDuplicateWarning.textContent = message || "";
}

function setManualFormMode(viewOnly){
  manualOrdersState.viewOnly = !!viewOnly;
  const editor = document.querySelector(".manual-order-editor");
  editor?.classList.toggle("manual-view-mode", manualOrdersState.viewOnly);
  [manualClientName, manualOrderNumber, manualOrderDate, manualOrderStatus, manualOrderNotes].forEach(input => {
    if (input) input.disabled = manualOrdersState.viewOnly;
  });
  if (manualOrderAddRow) manualOrderAddRow.hidden = manualOrdersState.viewOnly;
  if (manualOrderSaveDraft) manualOrderSaveDraft.hidden = manualOrdersState.viewOnly;
  if (manualOrderApprove) manualOrderApprove.hidden = manualOrdersState.viewOnly;
  if (manualOrderCancelEdit){
    manualOrderCancelEdit.textContent = manualOrdersState.viewOnly ? "Close view" : "Reset";
  }
}

function resetManualOrderForm(){
  manualOrdersState.editingId = null;
  manualOrdersState.rows = [newManualRow({ position: "1" })];
  manualOrdersState.saveStatus = "draft";
  if (manualClientName) manualClientName.value = "";
  if (manualOrderNumber) manualOrderNumber.value = "";
  if (manualOrderDate) manualOrderDate.value = manualToday();
  if (manualOrderStatus) manualOrderStatus.value = "draft";
  if (manualOrderNotes) manualOrderNotes.value = "";
  if (manualOrderFormTitle) manualOrderFormTitle.textContent = "Create manual order";
  if (manualOrderSaveDraft) manualOrderSaveDraft.textContent = "Save Draft";
  setManualFormMode(false);
  setManualFormError("");
  setManualDuplicateWarning("");
  renderManualRows();
}

function renderManualRows(){
  if (!manualOrderRows) return;
  const disabled = manualOrdersState.viewOnly ? "disabled" : "";
  manualOrderRows.innerHTML = manualOrdersState.rows.map(row => {
    const calculated = manualCalculatedArea(row);
    const finalArea = manualFinalArea(row);
    return `<tr data-manual-row="${escapeHtml(row.uid)}">
      <td><input type="text" data-manual-field="position" value="${escapeHtml(row.position)}" ${disabled}></td>
      <td><input type="text" data-manual-field="glass_type" value="${escapeHtml(row.glass_type)}" ${disabled}><span class="manual-row-error" data-manual-error="glass_type"></span></td>
      <td><input type="number" min="0.001" step="any" data-manual-field="width_mm" value="${escapeHtml(row.width_mm)}" ${disabled}><span class="manual-row-error" data-manual-error="width_mm"></span></td>
      <td><input type="number" min="0.001" step="any" data-manual-field="height_mm" value="${escapeHtml(row.height_mm)}" ${disabled}><span class="manual-row-error" data-manual-error="height_mm"></span></td>
      <td><input type="number" min="1" step="1" data-manual-field="quantity" value="${escapeHtml(row.quantity)}" ${disabled}><span class="manual-row-error" data-manual-error="quantity"></span></td>
      <td><input type="text" data-manual-calculated value="${calculated.toFixed(3)}" readonly tabindex="-1"></td>
      <td><input type="number" min="0" step="0.001" data-manual-field="area_override_m2" value="${escapeHtml(row.area_override_m2)}" ${disabled}></td>
      <td><input type="text" data-manual-final value="${finalArea.toFixed(3)}" readonly tabindex="-1"></td>
      <td><input type="text" data-manual-field="notes" value="${escapeHtml(row.notes)}" ${disabled}></td>
      <td>${manualOrdersState.viewOnly ? "" : `<button type="button" class="btn small danger" data-manual-remove-row="${escapeHtml(row.uid)}" aria-label="Remove row" tabindex="-1">×</button>`}</td>
    </tr>`;
  }).join("");
  updateManualFormSummary();
}

function updateManualRowComputed(tr, row){
  const calculated = manualCalculatedArea(row);
  const finalArea = manualFinalArea(row);
  const calculatedInput = tr?.querySelector("[data-manual-calculated]");
  const finalInput = tr?.querySelector("[data-manual-final]");
  if (calculatedInput) calculatedInput.value = calculated.toFixed(3);
  if (finalInput) finalInput.value = finalArea.toFixed(3);
  updateManualFormSummary();
}

function updateManualFormSummary(){
  if (!manualOrderFormSummary) return;
  const quantity = manualOrdersState.rows.reduce((sum, row) => {
    const value = Number(row.quantity);
    return sum + (Number.isInteger(value) && value > 0 ? value : 0);
  }, 0);
  const area = manualOrdersState.rows.reduce((sum, row) => sum + manualFinalArea(row), 0);
  manualOrderFormSummary.textContent = `${manualOrdersState.rows.length} row${manualOrdersState.rows.length === 1 ? "" : "s"} · ${quantity} piece${quantity === 1 ? "" : "s"} · ${area.toFixed(3)} m²`;
}

function clearManualValidation(){
  manualOrderForm?.querySelectorAll(".manual-input-error").forEach(input => input.classList.remove("manual-input-error"));
  manualOrderForm?.querySelectorAll("[data-manual-error]").forEach(el => { el.textContent = ""; });
}

function markManualFieldError(input, message){
  input?.classList.add("manual-input-error");
  const cellError = input?.closest("td")?.querySelector(`[data-manual-error="${input.dataset.manualField}"]`);
  if (cellError) cellError.textContent = message;
}

function validateManualOrderForm(){
  clearManualValidation();
  const errors = [];
  if (!manualClientName?.value.trim()){
    manualClientName?.classList.add("manual-input-error");
    errors.push("Client name is required.");
  }
  if (!manualOrderNumber?.value.trim()){
    manualOrderNumber?.classList.add("manual-input-error");
    errors.push("Order number is required.");
  }
  if (!manualOrderDate?.value){
    manualOrderDate?.classList.add("manual-input-error");
    errors.push("Order date is required.");
  }
  if (!manualOrdersState.rows.length) errors.push("At least one row is required.");
  manualOrdersState.rows.forEach((row, index) => {
    const tr = manualOrderRows?.querySelector(`[data-manual-row="${CSS.escape(row.uid)}"]`);
    const glassInput = tr?.querySelector('[data-manual-field="glass_type"]');
    const widthInput = tr?.querySelector('[data-manual-field="width_mm"]');
    const heightInput = tr?.querySelector('[data-manual-field="height_mm"]');
    const quantityInput = tr?.querySelector('[data-manual-field="quantity"]');
    if (!row.glass_type.trim()){
      markManualFieldError(glassInput, "Required");
      errors.push(`Row ${index + 1}: glass type is required.`);
    }
    if (!(Number(row.width_mm) > 0)){
      markManualFieldError(widthInput, "Must be > 0");
      errors.push(`Row ${index + 1}: width must be greater than zero.`);
    }
    if (!(Number(row.height_mm) > 0)){
      markManualFieldError(heightInput, "Must be > 0");
      errors.push(`Row ${index + 1}: height must be greater than zero.`);
    }
    const quantity = Number(row.quantity);
    if (!Number.isInteger(quantity) || quantity <= 0){
      markManualFieldError(quantityInput, "Positive whole number");
      errors.push(`Row ${index + 1}: quantity must be a positive integer.`);
    }
    const override = String(row.area_override_m2).trim();
    if (override !== "" && (!(Number(override) >= 0) || !Number.isFinite(Number(override)))){
      tr?.querySelector('[data-manual-field="area_override_m2"]')?.classList.add("manual-input-error");
      errors.push(`Row ${index + 1}: area override must be zero or greater.`);
    }
  });
  setManualFormError(errors[0] || "");
  return errors.length === 0;
}

function buildManualOrderPayload(status){
  return {
    client_name: manualClientName.value.trim(),
    order_number: manualOrderNumber.value.trim(),
    order_date: manualOrderDate.value,
    notes: manualOrderNotes.value.trim(),
    status,
    rows: manualOrdersState.rows.map(row => ({
      position: row.position.trim(),
      glass_type: row.glass_type.trim(),
      width_mm: Number(row.width_mm),
      height_mm: Number(row.height_mm),
      quantity: Number(row.quantity),
      area_override_m2: String(row.area_override_m2).trim() === "" ? null : Number(row.area_override_m2),
      notes: row.notes.trim(),
    })),
  };
}

async function checkManualOrderDuplicate(){
  const orderNumber = manualOrderNumber?.value.trim();
  const token = ++manualOrdersState.duplicateCheckToken;
  if (!orderNumber){
    setManualDuplicateWarning("");
    return false;
  }
  const params = new URLSearchParams({ order_number: orderNumber });
  if (manualOrdersState.editingId) params.set("exclude_id", String(manualOrdersState.editingId));
  try{
    const result = await manualApi(`/manual-orders/check-number?${params.toString()}`);
    if (token !== manualOrdersState.duplicateCheckToken) return false;
    setManualDuplicateWarning(result.exists ? `Warning: order number "${orderNumber}" already exists in Manual Orders.` : "");
    return !!result.exists;
  }catch{
    return false;
  }
}

async function saveManualOrder(status){
  if (manualOrdersState.viewOnly || !validateManualOrderForm()) return;
  const payload = buildManualOrderPayload(status);
  const editingId = manualOrdersState.editingId;
  setManualFormError("");
  if (manualOrderSaveDraft) manualOrderSaveDraft.disabled = true;
  if (manualOrderApprove) manualOrderApprove.disabled = true;
  try{
    const saved = await manualApi(
      editingId ? `/manual-orders/${editingId}` : "/manual-orders",
      {
        method: editingId ? "PUT" : "POST",
        body: JSON.stringify(payload),
      },
    );
    if (saved.duplicate_warning){
      setManualDuplicateWarning(`Warning: order number "${saved.order_number}" also exists in Manual Orders.`);
    }
    if (manualOrdersListStatus){
      manualOrdersListStatus.textContent = `${saved.order_number} saved as ${saved.status}.`;
    }
    await loadManualOrders();
    await openManualOrder(saved.id, false);
  }catch(error){
    setManualFormError(error.message || String(error));
  }finally{
    if (manualOrderSaveDraft) manualOrderSaveDraft.disabled = false;
    if (manualOrderApprove) manualOrderApprove.disabled = false;
  }
}

function manualStatusBadge(status){
  const normalized = String(status || "draft").toLowerCase();
  const labels = {
    draft: "Draft",
    approved: "Approved",
    processing: "Processing",
    finished: "Finished",
    cancelled: "Cancelled",
  };
  return `<span class="history-status-badge ${escapeHtml(normalized)}">${escapeHtml(labels[normalized] || "Draft")}</span>`;
}

function renderManualOrdersList(){
  if (!manualOrdersList) return;
  if (manualOrdersState.loading){
    manualOrdersList.innerHTML = '<div class="history-list-loading"><span class="spinner"></span><span>Loading manual orders…</span></div>';
    return;
  }
  if (!manualOrdersState.items.length){
    manualOrdersList.innerHTML = '<div class="empty-state">No manual orders found.</div>';
    return;
  }
  const body = manualOrdersState.items.map(order => {
    const canProduce = ["approved", "processing"].includes(order.status);
    const canOutput = ["approved", "processing", "finished"].includes(order.status);
    return `<tr>
      <td class="mono">${escapeHtml(order.order_number)}</td>
      <td data-col="client">${escapeHtml(order.client_name)}</td>
      <td>${escapeHtml(order.order_date)}</td>
      <td>${Number(order.row_count || 0)}</td>
      <td>${Number(order.total_quantity || 0)}</td>
      <td>${Number(order.total_area_m2 || 0).toFixed(3)}</td>
      <td>${manualStatusBadge(order.status)}</td>
      <td data-col="actions"><div class="manual-order-actions">
        <button type="button" class="btn small tertiary" data-manual-action="open" data-id="${order.id}">Open</button>
        <button type="button" class="btn small" data-manual-action="edit" data-id="${order.id}">Edit</button>
        <button type="button" class="btn small tertiary" data-manual-action="duplicate" data-id="${order.id}">Duplicate</button>
        <button type="button" class="btn small" data-manual-action="processing" data-id="${order.id}" ${canProduce ? "" : "disabled"}>Processing</button>
        <button type="button" class="btn small" data-manual-action="labels" data-id="${order.id}" ${canOutput ? "" : "disabled"}>Labels</button>
        <button type="button" class="btn small" data-manual-action="invoice" data-id="${order.id}" ${canOutput ? "" : "disabled"}>Invoice</button>
        <button type="button" class="btn small danger" data-manual-action="delete" data-id="${order.id}">Delete</button>
      </div></td>
    </tr>`;
  }).join("");
  manualOrdersList.innerHTML = `<table class="manual-orders-table">
    <thead><tr>
      <th>Order No.</th><th>Client</th><th>Date</th><th>Rows</th><th>Total quantity</th>
      <th>Total m²</th><th>Status</th><th>Actions</th>
    </tr></thead>
    <tbody>${body}</tbody>
  </table>`;
}

async function loadManualOrders(){
  manualOrdersState.loading = true;
  renderManualOrdersList();
  const params = new URLSearchParams({ limit: "200" });
  if (manualOrdersState.search) params.set("query", manualOrdersState.search);
  if (manualOrdersState.status) params.set("status", manualOrdersState.status);
  try{
    const data = await manualApi(`/manual-orders?${params.toString()}`);
    manualOrdersState.items = Array.isArray(data.items) ? data.items : [];
    manualOrdersState.loaded = true;
    if (manualOrdersListStatus){
      manualOrdersListStatus.textContent = `${manualOrdersState.items.length} manual order${manualOrdersState.items.length === 1 ? "" : "s"}`;
    }
  }catch(error){
    manualOrdersState.items = [];
    if (manualOrdersListStatus) manualOrdersListStatus.textContent = error.message || String(error);
  }finally{
    manualOrdersState.loading = false;
    renderManualOrdersList();
  }
}

async function openManualOrder(orderId, viewOnly){
  try{
    const order = await manualApi(`/manual-orders/${orderId}`);
    manualOrdersState.editingId = order.id;
    manualOrdersState.rows = (order.rows || []).map(newManualRow);
    if (!manualOrdersState.rows.length) manualOrdersState.rows = [newManualRow({ position: "1" })];
    manualClientName.value = order.client_name || "";
    manualOrderNumber.value = order.order_number || "";
    manualOrderDate.value = order.order_date || manualToday();
    manualOrderStatus.value = order.status || "draft";
    manualOrderNotes.value = order.notes || "";
    manualOrderFormTitle.textContent = viewOnly ? `View ${order.order_number}` : `Edit ${order.order_number}`;
    if (manualOrderSaveDraft) manualOrderSaveDraft.textContent = "Save Changes";
    setManualFormMode(viewOnly);
    setManualFormError("");
    setManualDuplicateWarning("");
    renderManualRows();
    document.querySelector(".manual-order-editor")?.scrollIntoView({ behavior: "smooth", block: "start" });
    return order;
  }catch(error){
    if (manualOrdersListStatus) manualOrdersListStatus.textContent = error.message || String(error);
    return null;
  }
}

function manualOrderToShared(order, { perUnitArea = false } = {}){
  const statusMap = {
    draft: "draft",
    approved: "approved",
    processing: "in_production",
    finished: "completed",
    cancelled: "archived",
  };
  return {
    id: `manual-${order.id ?? order.manual_order_id}`,
    source: "manual",
    client_name: order.client_name,
    client: order.client_name,
    order_number: order.order_number,
    order_numbers: [order.order_number],
    order_date: order.order_date,
    created_at: order.created_at || order.order_date,
    status: statusMap[order.status] || "draft",
    units_total: order.total_quantity || (order.rows || []).reduce((sum, row) => sum + Number(row.quantity || 0), 0),
    area_total: order.total_area_m2 || (order.rows || []).reduce((sum, row) => sum + Number(row.final_area_m2 || 0), 0),
    rows: (order.rows || []).map(row => {
      const quantity = Number(row.quantity || 0);
      const finalArea = Number(row.final_area_m2 ?? row.area ?? 0);
      return {
        id: row.id,
        order_number: order.order_number,
        position: row.position || "",
        type: row.glass_type || row.type || "",
        glass_type: row.glass_type || row.type || "",
        width_mm: Number(row.width_mm),
        height_mm: Number(row.height_mm),
        dimension: row.dimension || `${Number(row.width_mm)}x${Number(row.height_mm)}`,
        quantity,
        area: perUnitArea && quantity > 0 ? finalArea / quantity : finalArea,
        final_area_m2: finalArea,
        source: "manual",
        notes: row.notes || "",
      };
    }),
  };
}

function manualInvoicePricingIssues(order){
  const glassPrices = appState.invoices.priceLists.glass || {};
  const spacerPrices = appState.invoices.priceLists.spacer || {};
  const issues = new Set();
  (order.rows || []).forEach(row => {
    const composition = parseInvoiceComposition(row.type || "");
    if (!composition.panes.length){
      issues.add(`unrecognized glass "${row.type || "unknown"}"`);
    }
    composition.panes.forEach(pane => {
      const key = normalizeGlassKey(pane);
      if (!key || !Object.prototype.hasOwnProperty.call(glassPrices, key) || !Number.isFinite(Number(glassPrices[key]))){
        issues.add(`glass "${pane}"`);
      }
    });
    composition.spacerThicknesses.forEach(thickness => {
      const kind = composition.spacerKind === "thermal" ? "thermal" : "normal";
      const price = spacerPrices[thickness]?.[kind];
      if (!Number.isFinite(Number(price))) issues.add(`${kind} spacer ${thickness} mm`);
    });
  });
  return Array.from(issues);
}

async function handleManualOrderAction(action, orderId){
  if (!action || !orderId) return;
  if (action === "open"){
    await openManualOrder(orderId, true);
    return;
  }
  if (action === "edit"){
    await openManualOrder(orderId, false);
    return;
  }
  if (action === "duplicate"){
    try{
      const copy = await manualApi(`/manual-orders/${orderId}/duplicate`, { method: "POST" });
      await loadManualOrders();
      await openManualOrder(copy.id, false);
      if (manualOrdersListStatus) manualOrdersListStatus.textContent = `Created draft ${copy.order_number}.`;
    }catch(error){
      if (manualOrdersListStatus) manualOrdersListStatus.textContent = error.message || String(error);
    }
    return;
  }
  if (action === "delete"){
    const summary = manualOrdersState.items.find(item => String(item.id) === String(orderId));
    if (!confirm(`Delete manual order ${summary?.order_number || orderId}?`)) return;
    try{
      await manualApi(`/manual-orders/${orderId}`, { method: "DELETE" });
      if (String(manualOrdersState.editingId) === String(orderId)) resetManualOrderForm();
      await loadManualOrders();
    }catch(error){
      if (manualOrdersListStatus) manualOrdersListStatus.textContent = error.message || String(error);
    }
    return;
  }
  if (action === "processing"){
    try{
      const normalized = await manualApi(`/manual-orders/${orderId}/processing`, { method: "POST" });
      const shared = manualOrderToShared({
        ...normalized,
        id: normalized.manual_order_id,
        total_quantity: (normalized.rows || []).reduce((sum, row) => sum + Number(row.quantity || 0), 0),
        total_area_m2: (normalized.rows || []).reduce((sum, row) => sum + Number(row.final_area_m2 || 0), 0),
      });
      addOrderToProcessing(shared);
      showProcessingToast(`${normalized.order_number} added to Processing`);
      if (String(manualOrdersState.editingId) === String(normalized.manual_order_id) && manualOrderStatus){
        manualOrderStatus.value = "processing";
      }
      await loadManualOrders();
      activateTab("processing");
    }catch(error){
      if (manualOrdersListStatus) manualOrdersListStatus.textContent = error.message || String(error);
    }
    return;
  }
  const order = await manualApi(`/manual-orders/${orderId}`);
  if (action === "labels"){
    const shared = manualOrderToShared(order);
    const job = addLabelsJob("Manual Order", shared.rows, {
      primaryOrderId: order.order_number,
      clientOverride: order.client_name,
      sourceKind: "manual",
      sourceLabel: "Manual Order",
      sourceDetail: `manual-${order.id}`,
      successMessage: `Added quantity-based labels for ${order.order_number}.`,
    });
    if (job){
      activateTab("labels");
    }
    return;
  }
  if (action === "invoice"){
    try{
      ensureInvoicesLoaded();
      await syncInvoiceConfigFromServer({ silent: true });
      const shared = manualOrderToShared(order, { perUnitArea: true });
      const issues = manualInvoicePricingIssues(shared);
      if (issues.length){
        if (manualOrdersListStatus){
          manualOrdersListStatus.textContent = `Invoice not generated. Configure prices for: ${issues.join(", ")}.`;
        }
        return;
      }
      const job = await createInvoiceJobFromOrder(shared);
      if (job && manualOrdersListStatus){
        manualOrdersListStatus.textContent = `Invoice job generated for ${order.order_number}.`;
      }
    }catch(error){
      if (manualOrdersListStatus) manualOrdersListStatus.textContent = error.message || String(error);
    }
  }
}

function ensureManualOrdersReady(){
  if (!manualOrdersState.rows.length) resetManualOrderForm();
  if (!manualOrdersState.loaded) loadManualOrders();
  else renderManualOrdersList();
}

function initManualOrders(){
  if (!manualOrderForm) return;
  resetManualOrderForm();
  manualOrderForm.addEventListener("submit", event => {
    event.preventDefault();
    const requested = event.submitter?.dataset?.manualSaveStatus || manualOrdersState.saveStatus;
    const status = requested === "approved"
      ? "approved"
      : (manualOrdersState.editingId ? manualOrderStatus.value : "draft");
    saveManualOrder(status);
  });
  manualOrderForm.addEventListener("click", event => {
    const saveButton = event.target.closest("[data-manual-save-status]");
    if (saveButton) manualOrdersState.saveStatus = saveButton.dataset.manualSaveStatus;
    const removeButton = event.target.closest("[data-manual-remove-row]");
    if (!removeButton) return;
    manualOrdersState.rows = manualOrdersState.rows.filter(row => row.uid !== removeButton.dataset.manualRemoveRow);
    if (!manualOrdersState.rows.length) manualOrdersState.rows.push(newManualRow({ position: "1" }));
    renderManualRows();
  });
  manualOrderRows?.addEventListener("input", event => {
    const input = event.target.closest("[data-manual-field]");
    const tr = input?.closest("[data-manual-row]");
    if (!input || !tr) return;
    const row = manualOrdersState.rows.find(item => item.uid === tr.dataset.manualRow);
    if (!row) return;
    row[input.dataset.manualField] = input.value;
    input.classList.remove("manual-input-error");
    const error = input.closest("td")?.querySelector(`[data-manual-error="${input.dataset.manualField}"]`);
    if (error) error.textContent = "";
    updateManualRowComputed(tr, row);
  });
  manualOrderRows?.addEventListener("keydown", event => {
    if (manualOrdersState.viewOnly || event.isComposing) return;
    const input = event.target.closest("[data-manual-field]");
    const tr = input?.closest("[data-manual-row]");
    const field = input?.dataset.manualField;
    if (!input || !tr || !MANUAL_ROW_FIELD_ORDER.includes(field)) return;
    const rowIndex = manualOrdersState.rows.findIndex(row => row.uid === tr.dataset.manualRow);
    if (rowIndex < 0) return;

    if (event.key === "Tab"){
      const fieldIndex = MANUAL_ROW_FIELD_ORDER.indexOf(field);
      if (event.shiftKey){
        if (fieldIndex > 0){
          event.preventDefault();
          focusManualRowField(rowIndex, MANUAL_ROW_FIELD_ORDER[fieldIndex - 1]);
        }else if (rowIndex > 0){
          event.preventDefault();
          focusManualRowField(rowIndex - 1, MANUAL_ROW_FIELD_ORDER[MANUAL_ROW_FIELD_ORDER.length - 1]);
        }
      }else{
        event.preventDefault();
        if (fieldIndex < MANUAL_ROW_FIELD_ORDER.length - 1){
          focusManualRowField(rowIndex, MANUAL_ROW_FIELD_ORDER[fieldIndex + 1]);
        }else if (rowIndex === manualOrdersState.rows.length - 1){
          appendManualRow({ focusField: "position" });
        }else{
          focusManualRowField(rowIndex + 1, "position");
        }
      }
      return;
    }

    if (event.altKey || event.ctrlKey || event.metaKey || event.shiftKey) return;
    if (event.key === "ArrowDown" || event.key === "Enter"){
      event.preventDefault();
      if (rowIndex === manualOrdersState.rows.length - 1){
        appendManualRow({ focusField: field });
      }else{
        focusManualRowField(rowIndex + 1, field);
      }
    }else if (event.key === "ArrowUp" && rowIndex > 0){
      event.preventDefault();
      focusManualRowField(rowIndex - 1, field);
    }
  });
  manualOrderAddRow?.addEventListener("click", () => appendManualRow());
  document.getElementById("manualOrderNew")?.addEventListener("click", resetManualOrderForm);
  manualOrderCancelEdit?.addEventListener("click", resetManualOrderForm);
  manualOrderNumber?.addEventListener("blur", checkManualOrderDuplicate);
  document.getElementById("manualOrdersRefresh")?.addEventListener("click", loadManualOrders);
  manualOrderStatusFilter?.addEventListener("change", () => {
    manualOrdersState.status = manualOrderStatusFilter.value;
    loadManualOrders();
  });
  let manualSearchTimer = null;
  manualOrderSearch?.addEventListener("input", () => {
    manualOrdersState.search = manualOrderSearch.value.trim();
    clearTimeout(manualSearchTimer);
    manualSearchTimer = setTimeout(loadManualOrders, 250);
  });
  manualOrdersList?.addEventListener("click", event => {
    const button = event.target.closest("[data-manual-action]");
    if (!button || button.disabled) return;
    handleManualOrderAction(button.dataset.manualAction, button.dataset.id);
  });
}

initManualOrders();

	initScanStudio().catch(error => {
	  console.error("Scan Studio init failed", error);
	});
	bindPdfEditorEvents();
	initGlobalScanDropRouting();
	window.addEventListener("beforeunload", cleanupGlobalScanDropRouting, { once: true });
	initIguCalculator();
updateExtractUI();
updateHistoryDetailUI();
updateProcessingUI();
syncProcessingStateProcessedRows();
updateSpacerProcessingUI();
updateLabelsUI();
connectTelegramFileEvents();
refreshTelegramFilesBadge();
