...
async function run(engine,name,base){
  ...
  try{
    ...
    assert.match(await page.locator('#bridgePickerCount').innerText(),/8 selected rows · 22 total quantity/);
    // Two synchronous clicks emulate rapid duplicate dispatch without reopening the dialog.
    await page.locator('#bridgePickerAdd').evaluate(button=>{button.click();button.click();});
    ...
    await page.locator('#bridgePass').fill('4F outer pane / pass 1');
    assert(await page.locator('#bridgeDownload').isDisabled());
    await page.locator('#bridgeConfirmed').check();
    ...
    await page.locator('#bridgeAdd').click();
    assert.equal(await page.locator('[data-bridge-select]:disabled').count(),8);
    await page.locator('#bridgePickerCancel').click();
    ...
    assert(!await page.locator('#bridgeConfirmed').isChecked());
    ...
    await page.locator('#bridgeAdd').click();
    assert.equal(await page.locator('#bridgePickerSection option').count(),2);
    ...
    await page.locator('[data-bridge-order="0"]').check();
    await page.locator('#bridgePickerAdd').click();
    assert.equal(await page.locator('#bridgeRows tbody tr').count(),14);
    await page.locator('#bridgeAdd').click();
    assert(await page.locator('#bridgePickerSection option').nth(1).evaluate(option=>option.disabled));
    ...
    console.log(`${name}: empty, selection, rapid clicks, exact downloaded CSV, duplicates, source changes, refresh, busy guard, removal/clear isolation, geometry, cross-section and mobile checks passed`);
